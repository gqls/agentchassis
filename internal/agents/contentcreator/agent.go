...
func (a *Agent) buildEnhancedContentPrompt(req RequestPayload, memories []models.MemoryEntry, config *models.AgentConfig) string {
	...
	if block := a.voiceStyleBlock(a.ctx); block != "" {
...
// voiceStyleBlock returns the house voice rules for this generation.
//
// The rules are NOT stored here. They live in exactly one place — the
// `voice_style_block` row of `agent_default_configs` — and this reads them from
// there. A Go const briefly held a second copy on 2026-07-27; that was the
// defect in bugs_open/121 and it is gone.
//
// The override the owner asked for is request-level, not a config switch: a
// request that carries its own instructions in `req.Data.Prompt` has them
// rendered into the prompt, and the block itself states that explicit request
// instructions outrank it.
//
// An unavailable block degrades to "no house voice", never to a failed
// generation. Losing the voice on one post is recoverable; failing every
// content build is not.
func (a *Agent) voiceStyleBlock(ctx context.Context) string {
	block, ok := voicestyle.Get(ctx, func(c context.Context) (string, error) {
		if a.db == nil {
			return "", fmt.Errorf("no database handle")
		}
		var t sql.NullString
		err := a.db.QueryRow(c, voicestyle.SQL).Scan(&t)
		return t.String, err
	})
	if !ok {
		a.logger.Warn("house voice block unavailable; generating without it",
			zap.String("config_name", voicestyle.ConfigName))
	}
	return block
}
