...
// componentSourceBaselineRoute is the file that OWNS the repair of every
// grandfathered finding. A baseline entry without a live route is an excuse
// rather than a deferral, so the emitter stamps it and a test asserts the file
// it names exists.
const componentSourceBaselineRoute = "bugs_open/362_HANDOFF_2026-08-22_sixty_nine_grandfathered_phantom_source_fields.md"

// componentSourceBaselineDoc is the header the emitter writes into the file, so
// the explanation travels with the data rather than living only in a commit.
const componentSourceBaselineDoc = "bugs_open/309 candidate 2 - the FROZEN grandfather baseline for the " +
	"at-rest source-vocabulary audit. DERIVED FROM THE SHIPPED RULE via " +
	"`config-key-audit --component-source-vocabulary --emit-baseline`, never hand-written and never from a " +
	"re-implementation of the classification (the first cut was generated by a SQL restatement of the rule, " +
	"which the council's debug_historian seat correctly objected to: a baseline built by a second predicate is " +
	"the very drift this mode exists to prevent, arriving through the back door). THIS FILE MAY ONLY EVER " +
	"SHRINK. It is keyed on the exact (component_id, field, source, class) tuple, so an entry can only excuse " +
	"the one finding it was written for - a changed source on the same field, or a new bad field on the same " +
	"component, is a NEW finding and turns the job red. Every entry is dated 2026-08-22 and the loader REFUSES " +
	"any other date: the baseline is closed, and a new finding is fixed or routed, never baselined. " +
	"Grandfathering is CONDITIONAL for the dormant components (live_instances_at_baseline = 0): if one is " +
	"deployed, the job goes red. An entry matching nothing live is STALE and also turns the job red - that is " +
	"the ratchet's pawl, and it is how repairs burn this file down. Being here downgrades the EXIT CODE and " +
	"never the REPORT: the daily doc_notes row names the whole population every run."
...
func emitComponentSourceVocabulary(args []string) {
	...
	for i := 0; i < len(args); i++ {
		...
		case args[i] == "--emit-baseline":
			emitBaseline = true
		default:
			fmt.Fprintf(os.Stderr,
				"config-key-audit --component-source-vocabulary: unrecognised argument %q "+
					"(want: [--baseline <file>] [--report] [--json] [--emit-baseline])\n", args[i])
			os.Exit(2)
		}
	}

	// --emit-baseline REGENERATES the file, so it must not require the file it is
	// about to write (and must not be graded against a stale one).
	var baseline map[string]componentSourceBaselineEntry
	var err error
	if emitBaseline {
		baseline = map[string]componentSourceBaselineEntry{}
	} else if baseline, err = loadComponentSourceBaseline(baselinePath); err != nil {
		fmt.Fprintf(os.Stderr,
	...
	if emitBaseline {
		// The baseline is DERIVED FROM THE SHIPPED RULE, never from a
		// re-implementation of it.
		//
		// The first cut of this file was generated by a SQL query that
		// re-stated the classification in psql — and the council's
		// debug_historian seat was right to object: a baseline built by a
		// second predicate is exactly the drift this whole mode exists to
		// make unrepresentable, arriving through the back door. It happened
		// to agree (the audit graded all 69 as grandfathered, zero stale,
		// which is what proves it), but "happened to agree" is not the
		// property we want — "cannot disagree" is.
		emitComponentSourceBaseline(findings)
		return
	}
...
// emitComponentSourceBaseline writes a baseline file to stdout, derived from
// findings the SHIPPED rule produced. Regenerating is a deliberate act, not a
// routine one: the file it writes is CLOSED on the date below, so this is for
// re-deriving the same census mechanically (to prove the committed file matches
// the rule), never for absorbing new findings — those are fixed or routed.
func emitComponentSourceBaseline(findings []componentSourceFinding) {
	entries := componentSourceBaselineEntriesFrom(findings)
	out := componentSourceBaselineFile{Doc: componentSourceBaselineDoc, Entries: entries}
	enc := json.NewEncoder(os.Stdout)
	enc.SetIndent("", "  ")
	if err := enc.Encode(out); err != nil {
		fmt.Fprintf(os.Stderr, "config-key-audit --component-source-vocabulary --emit-baseline: %v\n", err)
		os.Exit(2)
	}
	fmt.Fprintf(os.Stderr, "emitted %d baseline entries from the shipped rule\n", len(entries))
}

// componentSourceBaselineEntriesFrom is the emitter's DECISION, split out from
// its I/O so a test can exercise it rather than restate it.
//
// Split out after a mutation proof SURVIVED: the round-trip test built its own
// entries with the same field assignments, so corrupting the emitter's date
// stamp changed nothing the test could see. A test that duplicates the logic it
// is meant to pin cannot fail for the right reason — the estate's
// "a mock's own bookkeeping cannot assert a NEGATIVE" landmine, met head on.
func componentSourceBaselineEntriesFrom(findings []componentSourceFinding) []componentSourceBaselineEntry {
	entries := make([]componentSourceBaselineEntry, 0, len(findings))
	for _, f := range findings {
		entries = append(entries, componentSourceBaselineEntry{
			ComponentID:             f.ComponentID,
			Component:               f.Component,
			Field:                   f.Field,
			Source:                  f.Source,
			Class:                   f.Class,
			LiveInstancesAtBaseline: f.LiveInstances,
			Baselined:               componentSourceBaselineClosedOn,
			Route:                   componentSourceBaselineRoute,
		})
	}
