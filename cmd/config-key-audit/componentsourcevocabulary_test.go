...
// TestBaselineRoundTripsThroughTheShippedRule pins the provenance property the
// council's debug_historian seat asked for: the baseline must be DERIVED from
// the same function the audit grades with, not from a re-implementation that
// happens to agree.
//
// The first cut of the shipped file was generated by a SQL restatement of the
// classification. It turned out to be tuple-for-tuple identical to what the Go
// rule emits — but "happened to agree" is not the property; "cannot disagree"
// is. This test holds the round trip: what the emitter writes, the loader
// accepts, and every entry then grandfathers the exact finding it came from.
func TestBaselineRoundTripsThroughTheShippedRule(t *testing.T) {
	rows := []componentRow{
		{ID: "c1", Name: "alpha", LiveInstances: 32,
			InputSchema: schema(t, map[string]string{"a": "site_specs.blog.x", "ok": "llm"})},
		{ID: "c2", Name: "beta", LiveInstances: 0,
			InputSchema: schema(t, map[string]string{"b": "query.featured_post", "c": "config"})},
	}
	first := componentSourceFindings(rows, liveAspects(), nil)
	if len(first) != 3 {
		t.Fatalf("fixture should produce 3 findings, got %d", len(first))
	}

	// Emit, via the same code path the CLI uses, and read it back.
	dir := t.TempDir()
	path := filepath.Join(dir, "b.json")
	// THE EMITTER'S OWN DECISION FUNCTION — not a restatement of it. Building
	// the entries here instead is what let a date-stamp mutation survive.
	entries := componentSourceBaselineEntriesFrom(first)
	raw, err := json.MarshalIndent(componentSourceBaselineFile{
		Doc: componentSourceBaselineDoc, Entries: entries}, "", "  ")
	if err != nil {
		t.Fatal(err)
	}
	if err := os.WriteFile(path, raw, 0o600); err != nil {
	...
	loaded, err := loadComponentSourceBaseline(path)
	if err != nil {
		t.Fatalf("the emitter wrote a file its own loader refuses: %v", err)
	}

	// Re-grade the SAME state against it: everything must be grandfathered,
	// nothing stale. Any drift between emitter and grader shows up here.
	second := componentSourceFindings(rows, liveAspects(), loaded)
	ung, woke, stale := componentSourceReds(second, staleBaselineEntries(loaded, second))
	if ung != 0 || woke != 0 || stale != 0 {
		t.Fatalf("a baseline emitted from these findings does not grandfather them: "+
			"%d ungrandfathered, %d woken, %d stale — the emitter and the grader disagree",
			ung, woke, stale)
	}
	// Assert on what the EMITTER produced, not on the constant it reads. A
	// constant check passes while the emitter stamps "" — the same
	// duplicate-the-logic mistake this test was already refactored for once.
	for _, e := range entries {
		if e.Route == "" {
			t.Fatalf("emitted entry %s.%s carries no route — a grandfathered finding "+
				"with no owning file is an excuse, not a deferral", e.Component, e.Field)
		}
		if _, err := os.Stat(filepath.Join("../..", e.Route)); err != nil {
			t.Fatalf("emitted entry %s.%s routes to %q, which does not exist",
				e.Component, e.Field, e.Route)
		}
		if e.Baselined != componentSourceBaselineClosedOn {
			t.Fatalf("emitted entry %s.%s is dated %q, not the closure date %q — the "+
				"emitter would write a file its own loader refuses",
				e.Component, e.Field, e.Baselined, componentSourceBaselineClosedOn)
		}
	}
}
