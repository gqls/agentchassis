...
func TestWriteBaseline_RecordsCanonicalCoverageIncludingCleanComponents(t *testing.T) {
	...
	if err := writeBaseline(p, []finding{mkFinding("alpha", "a1", "empty_heading")}, analysed); err != nil {
	...
	if got["alpha-clone"] {
		t.Fatalf("the FILE must record canonical names only, or a clone and its representative "+
			"disagree about coverage: %v", onDisk.Components)
	}
	if !got["alpha"] || !got["gamma"] {
		t.Fatalf("written coverage lost a component: %v", onDisk.Components)
	}
	if len(onDisk.Components) != 2 {
		t.Fatalf("clone and representative must collapse to one entry: %v", onDisk.Components)
	}
