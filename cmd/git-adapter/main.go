...
func main() {
	...
	go func() {
		...
		if err := adapter.Run(); err != nil {
			appLogger.Error("Git adapter error", zap.Error(err))
			cancel()
		}
	}()
	...
	appLogger.Info("Shutdown signal received, shutting down git adapter",
	...
	appLogger.Info("Git adapter stopped")
}
