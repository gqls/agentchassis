...
# RESOLVED 2026-08-14 — tfvars now holds the RECORDED value, proven to authenticate.
#
# History, kept because the mistake is the instructive part: when this variable was added
# (2026-08-12) a fresh 32-character password was GENERATED for it. `pgbouncer_admin`
# already had a working 20-character password in the `pgbouncer-userlist` secret, and
# pgbouncer authenticates against that file ALONE (`auth_type = md5`, `auth_file`; there is
# no `auth_query`/`auth_user`), so the generated value authenticated nothing and the console
# refused with `FATAL: password authentication failed`.
#
# ⚠ NEVER GENERATE A VALUE FOR THIS KEY. It is one half of a pair Terraform does not own
# both ends of — the other half is the hand-applied `pgbouncer-userlist` secret. For a
# credential that must MATCH something, the task is RECORD, never CREATE; a generated value
# is indistinguishable from a correct one until something authenticates.
#
# The two failures mean opposite things and both read as "the console is broken":
#   FATAL: not allowed                     -> wrong USER (auth OK, not in admin_users)
#   FATAL: password authentication failed  -> right user, wrong PASSWORD
#
# ⏳ NOT LIVE UNTIL THE NEXT APPLY. `047-base-configs` applies as part of `make release`, so
# until that runs, `personae-platform-secrets.PGBOUNCER_ADMIN_PASSWORD` still carries the
# old generated string. It self-corrects on the next release; no manual step is owed.
# Meanwhile the console IS reachable using the userlist value directly.
#
# Full account, the two wrong calls behind it, and the SHOW POOLS reading this unblocked:
#   docs/agent_docs/docs024_key_docs_latest/bugfix_246_shared_pool_ownership/RUNBOOK_shared_pool_ownership.md §9
variable "pgbouncer_admin_password" {
  description = "Password for the pgbouncer_admin console user (SHOW POOLS/SHOW CLIENTS). MUST be the EXISTING value from the pgbouncer-userlist secret — RECORD it, never generate one, because pgbouncer authenticates against that file alone. Recorded and proven to authenticate 2026-08-14."
  type        = string
  sensitive   = true
}
