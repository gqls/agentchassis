...
> ### ▶▶▶ YOU ARE HERE (2026-06-25 ~17:30)
> **Two tracks running in parallel:**
>
> **Track 1 — full-site rebuild (IN FLIGHT):** All 8 pages queued for
> `needs_page` rebuild to confirm the hero fix and regenerated components
> (gauntlet-cta, system-stats, gauntlet-interface, archetype-result-card)
> render correctly site-wide. Watch them complete, then visually verify.
> This does NOT fill the three empty shells — that is Track 2.
>
> **Track 2 — provocation pipeline (STARTING):** The empty shells
> (provocation-card, lobby-grid, brief-explanation) get content via the
> runtime JS pipeline. Diagnostics confirmed a clean slate — nothing exists
> yet. Phase 2 (JS against static test JSON) has its own runbook:
> **RUNBOOK_phase2_provocation_js.md**. Full design in
...
- [x] **Hero CSS variable fix verified** — magenta CTA, dark background, on-brand copy (2026-06-25)
- [x] **gauntlet-interface stale column fixed** — template_variable_count 0→33, quality 30→100
- [x] **Full 8-page rebuild queued** (2026-06-25 ~16:30)
- [x] **DECISION: empty-schema shell components** — Option 2 (runtime JS pipeline) chosen; see Step 7 + PLAN_spark_provocation_pipeline.md
- [~] **Full 8-page rebuild re-run** (queued 2026-06-25 ~17:30, in flight)
- [ ] **Verify all 8 pages** after rebuild completes — hero, gauntlet-cta, system-stats correct
- [ ] **Track 2: provocation pipeline Phase 2** — see RUNBOOK_phase2_provocation_js.md
...
## Step 7 — Empty-schema shell components (DECISION NEEDED)

**Symptom:** provocation-card, lobby-grid, brief-explanation render as blank
card outlines — empty inputs, unlabelled buttons, empty stat slots. Visible on
index and about pages.

**Root cause:** These components have `input_schema = {}` (empty) AND no
js_snippets apply to them. They have no content mechanism at all:
- Not LLM-filled — no schema fields for the content writer to populate
- Not JS-filled — no js_snippets to populate them client-side
They render hardcoded HTML structure (grids, inputs, stat strips) with no
`{{.field}}` slots and no runtime data path.

A page rebuild CANNOT fix this — there is nothing to fill.

**Verify the empty schemas:**
```sql
SELECT function, input_schema,
       (LENGTH(html_template) - LENGTH(REPLACE(html_template, '{{.', '')))
           / LENGTH('{{.') AS actual_slots
FROM content_components
WHERE function IN ('provocation-card', 'lobby-grid', 'brief-explanation')
  AND is_active = true AND forked_from IS NULL;
```
Expected: `input_schema = {}`, `actual_slots = 0` for all three.

**Verify no JS populates them:**
```sql
SELECT name, applies_to FROM js_snippets
WHERE is_active = true
  AND EXISTS (
    SELECT 1 FROM jsonb_array_elements_text(applies_to) AS a(elem)
    WHERE a.elem IN ('provocation-card', 'lobby-grid', 'brief-explanation')
  );
```
Expected: 0 rows.

**DECISION MADE: Option 2 (runtime JS pipeline).**

This was the original intended design per the Spark v1 roadmap — provocations are
daily-regenerated content shown via client-side JS reading a static JSON file.
Option 1 (static build-time content) would freeze a single set of provocations
permanently, defeating the daily-content product.

The full build plan is in **PLAN_spark_provocation_pipeline.md**. Summary:
- The JS delivery mechanism already exists (`js_snippets` +
  `render_js_snippets_for_site` + `site-asset-renderer`).
- The news feed pipeline is the proven template for the data layer.
- Build sequence: (Phase 1) confirm nothing exists [Step 8 above]; (Phase 2)
  prove JS against static test JSON; (Phase 3) build the data pipeline cloning
  the news structure; (Phase 4) schedule daily.
- The genuinely new piece is a `provocation-generator` agent (LLM:
  topic→provocation+AI take). Most other pieces are reused or cloned.

**Page structure storage:** There is no `page_plan` site_specs aspect. The
per-page section breakdown is on the pages table or within roadmap/strategy specs.
To see what sections a page was planned to have, inspect the pages table directly
rather than looking for a page_plan spec.

## Step 8 — Confirm no Spark content pipeline exists (run before building)

Before building the provocation pipeline (see PLAN_spark_provocation_pipeline.md),
confirm none of it already exists. Run all five:

```sql
-- 1. content_sources for vonc.com?
SELECT id, name, source_type, config, next_fetch_at, is_active
FROM content_sources
WHERE site_id = '9ec3b9ee-5b08-461b-b4f8-9e1e03579c74';

-- 2. content_feed_items for vonc.com?
SELECT COUNT(*) AS item_count, MAX(created_at) AS latest
FROM content_feed_items
WHERE site_id = '9ec3b9ee-5b08-461b-b4f8-9e1e03579c74';

-- 3. Scheduled tasks for provocation/spark/vonc?
SELECT task_name, task_type, schedule, is_active, config
FROM scheduled_tasks
WHERE task_name ILIKE '%provocation%' OR task_name ILIKE '%spark%'
   OR task_name ILIKE '%vonc%' OR config::text ILIKE '%provocation%';

-- 4. Agent definitions for provocation generation?
SELECT type, display_name FROM agent_definitions
WHERE type ILIKE '%provocation%' OR type ILIKE '%spark%'
   OR display_name ILIKE '%provocation%' OR description ILIKE '%provocation%';

-- 5. js_snippets for provocation/daily content?
SELECT name, applies_to, description FROM js_snippets
WHERE name ILIKE '%provocation%' OR description ILIKE '%provocation%'
   OR description ILIKE '%daily%' OR applies_to::text ILIKE '%provocation%';
```

If all return empty/zero: build from scratch per the plan, cloning the news
feed pipeline structure. If any return rows: extend the existing work rather
than duplicate.

**The news feed pipeline (006_news_feed_pipeline) is the proven template** —
the provocation pipeline mirrors it with a generative LLM step instead of
relevance scoring, producing `/data/provocations.json` instead of
