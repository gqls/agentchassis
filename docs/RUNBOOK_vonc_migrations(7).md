...
- [ ] **Fix CSS variable injection** — theme CSS not injecting into pages (see Step 6)
...
**Symptom (2026-06-24 ~15:30):** Index page renders with wrong colours. Hero CTA
button is dark blue (`#0f3460` fallback) instead of magenta (`#fc5c7d`). The
`/* Theme-specific styles injected here: */` block in `<head>` is empty.
CSS custom properties (`--color-primary`, `--color-background`, `--accent-color` etc.)
are not being set.
