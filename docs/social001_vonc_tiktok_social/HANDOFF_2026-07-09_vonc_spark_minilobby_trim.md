...
**Method (safest, and it is the pattern that has worked here).** Do **not** attempt a multi-line SQL
`REPLACE` of the block — nested markup makes anchored replaces fragile. Instead: dump the template
and the instance (§5), edit them offline, and `UPDATE … SET html_template = $full_new_text$…$…$`
with the complete new text, verifying by **length delta** (expect roughly −1,200 to −1,600 bytes for
the removed grid, plus the CSS rule change). Keep the `data-runtime-fill` marker intact. Take dated
backups first.
