...
> Next action: finalise the section plan reading, then design the js_snippets +
> data pipeline. See Step 7 (rewritten below).
...
*Option 1 — Regenerate with content schemas (build-time content):*
Raise `needs_component_regeneration` for each, but the regeneration must produce
a real `input_schema` with LLM fields. The current empty-schema versions give the
component-creator nothing to work from, so the regeneration spec must include a
`section_type` and `description` rich enough for the LLM to design appropriate
fields. After regeneration, a `needs_page` rebuild fills them via the content writer.
Simpler; fits existing pipeline. Loses the "live daily content" dynamic.

*Option 2 — Build runtime JS pipeline (runtime content):*
Add `js_snippets` rows (applies_to including these functions) that fetch and render
daily provocation data client-side, plus the backend API the JS calls. Matches the
original product vision (daily-changing provocations) but requires building the
data API and the snippets. Larger scope.

**Recommendation:** Option 1 for the first deployable version — get real content
on the page now. Option 2 later as a product enhancement when the daily-content
backend exists. Do not mix: a component is either schema-filled or JS-filled,
not both.
