...
#
# RUN IT FROM ANYWHERE — the script cd's itself.
#
# cmd/bundle lives in the CONTEXTKIT tree, which is its OWN GO MODULE. `go run`
# only builds packages inside the current module, so bundle must be invoked from
# inside contextkit (that also makes its defaults -assembler-bin ./cmd/assembler
# and -dbcontext-bin ./cmd/dbcontext resolve). Consequences, from cmd/assembler's
# source: it does os.ReadFile on -constitution and each -doc (read relative to the
# PROCESS CWD), while -scope/-include are resolved against -root. So:
#   -root                      = chassis root (absolute)
#   -scope / -include          = relative to the chassis root
#   -analysis/-constitution/-doc/-out = ABSOLUTE (cwd is contextkit, not the chassis)
#
...
[ -d "$CK_ABS/cmd/bundle" ] || { echo "cmd/bundle not found at $CK_ABS/cmd/bundle — set CK=<relative path>" >&2; exit 1; }
[ -f "$CONSTITUTION" ] || { echo "constitution not found: $ROOT_ABS/$CONSTITUTION" >&2; exit 1; }
if [ -f "$CK_ABS/go.mod" ]; then
  echo "note: contextkit is a separate go module — bundle runs from $CK_ABS (scopes stay relative to $ROOT_ABS)"
fi

# absolutise everything the assembler ReadFile()s directly (cwd will be contextkit)
ANALYSIS_ABS=$(readlink -f "$ANALYSIS")
CONSTITUTION_ABS="$ROOT_ABS/$CONSTITUTION"
case "$CONSTITUTION" in /*) CONSTITUTION_ABS="$CONSTITUTION";; esac
...
for v in DOC_CONTRACTS DOC_ARCH DOC_DYNAMIC DOC_DEBUG_B DOC_LIFECYCLE; do
  [ -n "${!v}" ] || { echo "doc not found for $v under $D" >&2; exit 1; }
  printf -v "$v" '%s' "$ROOT_ABS/${!v}"      # -doc is read relative to cwd (contextkit) => absolutise
done
...
ARGS=(
  -analysis "$ANALYSIS_ABS"
  -root "$ROOT_ABS"
  -constitution "$CONSTITUTION_ABS"
  -step debug
  -task "$TASK"
...
# go run must execute inside contextkit's module; -root keeps scopes pointing at the chassis.
cd "$CK_ABS"
