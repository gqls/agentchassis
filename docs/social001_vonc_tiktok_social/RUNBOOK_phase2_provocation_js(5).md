...
blank (the committed provocations.json has icons but stub title/desc — a data
gap, not a code bug); (2) the **path decision is now settled** — PD-1 confirmed
...
- [~] **P2-5** Card fills (headline/take/stats/lobby-icons/CTAs ✓). REMAINING: lobby card titles + descriptions blank — committed JSON has stub title/desc; fix the data.
