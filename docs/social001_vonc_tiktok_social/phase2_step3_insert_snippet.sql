-- ============================================================
-- Phase 2 Step 3: insert the provocation-card loader js_snippet
-- ============================================================
-- FIRST verify schema (column names) before running the INSERT:
--   \d js_snippets
-- The render action reads: name, description, js_content, applies_to (jsonb),
-- is_active. Confirm these plus any NOT NULL columns without defaults.

-- Dollar-quoted ($js$) so the JS body needs no escaping.

INSERT INTO js_snippets (name, description, applies_to, js_content, is_active)
VALUES (
  'provocation-card-loader',
  'Fills the provocation-card shell from /data/provocations.json (daily provocation + mini-lobby). Fails gracefully.',
  '["provocation-card"]'::jsonb,
  $js$/* provocation-card-loader
...
-- Verify it landed and will be picked up for vonc (index uses provocation-card):
SELECT name, applies_to, is_active, LENGTH(js_content) AS js_len
