...
## 2026-07-09 — STATE: shared base across three sites, verified NEUTRAL; two wiring gaps remain
**Sharing (the important fact).** This component has `forked_from IS NULL`, i.e. it is a row in the
cross-site shared library keyed by `function`, and it is used by **three** sites: vonc.com,
idea.uk (1244516d-014d-421c-88c6-090bb1e9552a) and robot-hands.com
(00ff3af5-dad8-4770-9f70-3edc267a3c92). The 083 regeneration therefore reached all three.
**Why that was safe, and the rule it establishes.** `store_generated_component` blocks a regeneration
that DROPS or RENAMES a field on a shared component — the guard exists because of the `fdd92ad4`
system-stats incident, where renaming a field in place silently emptied every dependent. Our regen
went 0 → 20 fields (pure addition), so the guard passed and nothing was stranded. Audit of the result:
the template contains **no Spark vocabulary** (gauntlet / provocation / archetype / contrarian / take
all absent) and the only non-generated fields are two generic static CTA labels ("Get Started",
"Learn More") plus the optional `illustration_url`. The base is neutral, so the other two sites
inherited a working how-it-works filled with **their own** copy, not vonc's voice.
**Field sources now:** 18 `llm`, 2 `static` (the CTA labels), 1 `site_assets.illustration`. The three
stat pairs were re-sourced `static` → `llm` in parallel work, which removed the generic
"Happy Customers"-style fallbacks that had been rendering on vonc's live page.
**Dependent state:** idea.uk 2 instances deployed; robot-hands 3 deployed, 1 still `pending` on stale
rendered HTML awaiting that site's own content rebuild. Owned by those threads.
**RULE (carry forward).** Regenerate a shared base only for neutral, purely-additive improvements.
Site-specific voice must **fork** (`forked_from = 58363894-9db9-4d2f-81ac-c47b54d97fc3`) — the
deliberate migration the code comments prescribe. Note also that a direct SQL UPDATE bypasses both the
field guard and the `component_versions` snapshot.
**Outstanding (unchanged):** the illustration column renders empty — `illustration_game_master` exists
as an asset but `ensureAssets` surfaces only hero/logo; and both CTAs still link to `#` placeholders.
`Categories:` shared-component (verified clean), content-vs-runtime-mismatch (illustration pending),
cta-graph (placeholders pending)
