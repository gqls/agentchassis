...
**Opened:** 2026-07-05  •  **Status:** DELIVERED 2026-07-08, live-confirmed 2026-07-09 — component 70d6662a created with both generation-time guards; page live at /provocations/index.html; loader installed and filling; ghost-row fix deployed. History: NOTES_provocations-archive-list.md
...
## AS BUILT (2026-07-09) — deviations from this spec, for the next component like it
- The generated template named the stat text span explicitly
  (`provocations-archive__item-stat-value`), which is cleaner than lobby-grid's "the span without the
  dot class" hunt. Prefer an explicit class in future specs.
- Both generation-time guards held on the first attempt: `has_marker = t`, `has_inline_script = f`.
  This was the first live validation that instructing component-creator to emit `data-runtime-fill`
  in the section tag and to forbid `script` elements works — it removed the post-hoc marker SQL
  (there was no Part B this time) and made the extraction-bug class impossible for this component.
- The description passed to component-creator was deliberately **quote-free** (attribute values named
  in prose) to survive the kcat/JSON trigger pipeline. Keep doing this.
- One defect the spec did not anticipate: `hidden` on the clone template is a UA-stylesheet
  `display: none` and loses to the component's own `.provocations-archive__item { display: grid; }`.
  A clone-template spec must also require `[data-…-template] { display: none; }` in the CSS.
