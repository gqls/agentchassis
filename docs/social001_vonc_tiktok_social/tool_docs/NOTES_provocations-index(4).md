...
## 2026-07-08 — CLOSED: ghost-row fix deployed, page complete
fix_archive_template_display.sql applied (template_fixed=t, instance_fixed=t); 085 rerender (corr
cd5f553e) redeployed at 2026-07-08 16:30:43; rendered_len 7455 → 7671 (+216 = hiding rule inserted
into base selector + its mobile-media-query copy, REPLACE fired twice as intended). DB verified; live
grep for the display:none rule expected 2 after Actions push. provocations-index DONE: page live, 8
archive rows fill, empty state hides, ghost dot-row eliminated, CTA left as Option B (parked).
`Categories:` css-specificity (RESOLVED), cta-graph (parked, Option B), silent-noop-success
(resolved), planning-gap (resolved), empty-shell/mode-b-template (resolved via runtime-fill)


## 2026-07-09 — Live confirm; page fully closed
`curl -s https://vonc.com/provocations/index.html | grep -c 'data-archive-template] { display: none;'`
returns **2** (was 0) — the deployed page carries the hiding rule in both the base selector and its
mobile media-query copy, so the ghost clone-template row is gone from what a visitor sees. Database
evidence (instance `rendered_len` 7671) and the served artifact now agree. Nothing outstanding on this
page: it is live, the eight archive rows fill at runtime, the empty state hides, and every primary CTA
on the site resolves here. The CTA at the foot of the page ("Enter today's Arena" → `/index.html`) is
left as-is by decision (Option B) until a real take-filing arena exists; the interactive surface today
is the Gauntlet tool at `/tools/gauntlet/index.html`.
`Categories:` css-specificity (resolved), silent-noop-success (resolved), planning-gap (resolved),
cta-graph (parked)
