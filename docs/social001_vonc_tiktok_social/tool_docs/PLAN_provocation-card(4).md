...
**Method — CORRECTED 2026-07-09.** Docs 003/002: `content_data` is the source of truth and *HTML
patching was rejected as an edit mechanism* (patched `rendered_html` is lost on the next re-render).
`fix_component_template_action` already has a `remove_element` fix type; its header defers
page_components content changes to the section-editor workflow. 003 names two re-render paths — the
light `rerender_page_sections` (a `page_rerender` item; re-renders from stored `content_data` via
`RenderComponentAction`, no LLM; **escalates to a full rebuild if `content_data` is NULL**) and the
full `needs_page` path — while `rerender_single_page` (our `rerender-index-vonc.sh`) only ASSEMBLES
pre-rendered components and will not pick up a template change. provocation-card is Mode-B, so its
`content_data` may be NULL: probe first. **Run `bundle_minilobby_trim.sh` to settle the path before
editing.** Fallback if no supported path exists: template UPDATE with full new text (never a
multi-line REPLACE of nested markup), verified by length delta, propagated by a `page_rerender` item
— not by hand-patching the instance. Dated backups first. Keep `data-runtime-fill` intact.

**Redeploy = two separate deploys.** Loader → `083_trigger-asset-renderer-vonc.sh` (bundle stays at
3 active snippets). Template → the re-render path the bundle confirms (`page_rerender`, or `needs_page`
if content_data is NULL), and only then the assemble-only `rerender-index-vonc.sh` if the workflow
does not already deploy.
...
### Status 2026-07-09
Trim not started. The method is under a read-only bundle (`/tmp/bundle_minilobby_trim.md`); nothing is
edited until its verdict lands. Handed to the follow-on chat with HANDOFF_2026-07-09 (§0 states the
position, §4 the task, §4.0 the bundle, §5 the D0 `content_data` probe that decides the re-render path).
