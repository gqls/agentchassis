...
## 2026-07-09 (evening) — Trim blocked on a bundle verdict, by choice
A read-only context bundle was assembled to settle the trim's METHOD before any edit:
`/tmp/bundle_minilobby_trim.md` (script `bundle_minilobby_trim.sh`). It scopes
`fix_component_template_action` (which already ships a `remove_element` fix type),
`store_generated_component` (its dependent-marking symbols), `rerender_single_page` (assemble-only),
`v3_site_actions.go:RenderComponentAction`, `rerender_page_sections_action`,
`render_js_snippets_for_site_action`, `save_page_sections_action`, `section_editor_actions` and the
registry, alongside docs 003/002/022/016b/020, the `\d` for the eight relevant tables, and
vonc.com/index runtime evidence.
The verdict must answer: which action changes `html_template`; which re-renders
`page_components.rendered_html` from a **changed template** (light `rerender_page_sections` via
`RenderComponentAction` vs full `needs_page` vs assemble-only `rerender_single_page`); what a Mode-B
section with empty/NULL `content_data` does, given 003's escalation rule; whether attributes
hand-added to `rendered_html` survive a re-render; and how a changed `js_snippets.js_content` reaches
`/assets/js/snippets.js`. Run the D0 `content_data` probe alongside it — that row decides which
re-render path is even available.
`Categories:` method-correction, reuse, overlap/role-collision (pending)
