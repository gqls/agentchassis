...
| 4 | `brief-explanation` ships `<img src="">` — illustration assets exist but `filename`/`storage_path` NULL, nothing under `/assets/images/` | undeployed asset *(re-diagnosed: stale render)* | **generic** | `undeployed_assets` **exists and is enabled** — but never fired for vonc because its host **design-discovery-agent has never run on vonc** (0 work items created_by it; improvement-sweep off since 2026-05-02, and manual passes drove the *completeness* agent only) | light re-render (`page_rerender` item, `reason: image_landed` → `rerender_page_sections`) | **DONE, live 2026-07-11.** Root cause was NOT an undeployed asset by then: the imagery workstream had already committed all 16 vonc assets to git (`/assets/images/`, all HTTP 200). The section's render simply **predated the section-imagery resolver** — schema source `site_assets.illustration` resolves via `site_plan_imagery` kind-alias (index:2 → `illustration_gauntlet_cta`). One triaged `page_rerender` item + dispatch filled the src; both runtime-fill shells stayed md5-pristine. Residual finding: `undeployed_assets` infers deployment from rendered_html *usage*, not from the repo — an asset committed but referenced nowhere still flags as undeployed |
...
| 8 | `page_components.build_status` has **no CHECK constraint** — free text. This is the root enabler of #1 | missing invariant | **generic** | n/a | migration `049_page_components_build_status_check.sql` | **DONE, live 2026-07-11** — pre-flight surveys clean (data: deployed/pending only; writers: deployed/pending/approved literals + content-reviewer's 'approved'); constraint applied and negative-tested (bogus value loudly rejected) |
...
| 13 | `provocations.json` still ships a dead `lobby` key | stale data | **site-specific (vonc)** | n/a | dropped by hand (sites repo commit `c244ddc`) | **DONE, live 2026-07-11** — no loader read `data.lobby` (verified against live snippets.js); git copy was md5-identical to live, confirming the sites repo as source of truth; keys now `generated_at/today/arena/archive`. Note: no Phase-3 emitter exists yet — all four prior commits to the file were hand-made |
