...
**Created:** 2026-06-25  •  **Updated:** 2026-07-09 (live-verified close; handoff doc created for a fresh chat; next = mini-lobby trim)
**Parent plan:** PLAN_spark_provocation_pipeline.md
**Parent runbook:** RUNBOOK_vonc_migrations.md (Track 2)


**Task (read this first if you are new to this runbook).** vonc.com is "Spark" — an AI-driven
daily-provocation site built end-to-end by the agentchassis website-builder (Go actions + Postgres +
Kafka agents; deploys git → GitHub Actions → Backblaze B2). The site's centrepiece sections are
intentionally EMPTY at build time and are filled in the visitor's browser: loaders bundled into
/assets/js/snippets.js fetch /data/provocations.json and populate the daily provocation card and the
six-room arena grid (sections marked data-runtime-fill so the assembler keeps their empty shells).
This runbook tracks that runtime-fill mechanism and its fallout, appendix by appendix. The current
work: /provocations/index.html — the destination of the header nav and every primary CTA — returns
404 because the site plan never gave that page any sections and seven builds completed as silent
no-ops; we are building it properly via a new self-contained runtime-fill component
(provocations-archive-list), then a plan-section row, then the page build, then its loader + data.
Jump to the >>> YOU ARE HERE <<< block just below for exactly where things stand.


---

## >>> YOU ARE HERE (2026-07-09 — provocations-index CLOSED + live-verified; next = provocation-card mini-lobby trim) <<<

**Fresh chat? Start with `HANDOFF_2026-07-09_vonc_spark_minilobby_trim.md`** — it stands alone
(orientation, DONE state with ids, the next task's full scope and method, data to collect, commands,
gotchas, backlog, file inventory). This runbook remains the operative detail for the phase-2 runtime
-fill work. Live confirm 2026-07-08: `curl … | grep -c 'data-archive-template] { display: none;'` → 2
(was 0) — the deployed archive page carries the fix; ghost row gone.

**Decisions just made.** The loader action stays UNCHANGED in the repo: the table-first alteration is
withdrawn and the user is reverting with `load_page_sections_from_spec_action.ORIGINAL.go` (outputs;
6-byte whitespace drift accepted; `git restore` is the byte-exact alternative). NB the revert takes
effect in the cluster only on the next chassis build/push — running pods carry the table-first
behaviour until then. Plan-storage authority = 029 Q1, decided in the planner/reconciler thread.

**State.** provocations-archive-list component 70d6662a-0e6f-478d-bc2e-b9e8e5eaeb37 created with both
generation-time guards (marker in template, no inline script); plan row e006a35e in
site_plan_sections; pages.sections = ["provocations-archive-list"] (confirmed). Build item
manual-build-provocations-eb3cdf42-d865-4790-ae78-29edbadf09ab: claimed 16:33:21 → **complete
16:38:15 (~5 min), no error** — the ten silent no-ops all finished in 33–65s, so this has the shape
of the FIRST REAL BUILD of the page.

**THE ONE GATE — PASSED (2026-07-07).** All three artifacts delivered: (1) pages row
build_status=deployed, deployed_at 2026-07-06 16:38:00 (inside the eb3cdf42 window; NB last_built_at
left empty — minor bookkeeping inconsistency, on the audit list); (2) the section instance rendered
at 7455 bytes with marker_in_rendered=t, has_clone_template=t, has_empty_state=t; (3) live 200 +
data-component present; git trail "Rerender: provocations/index.html" (9900517). Page-HTML checks on
the deployed file: snippets.js IS referenced in this page's head (loader delivery live); header copy
in voice (THE RECORD / "Every provocation on record." / "...The positions are permanent. The splits
don't lie."); empty state + CTA (/index.html, "Enter today's Arena") correct; no <no value>. The
queries below are retained as the reference verification set:
```sql
-- (1) page row state (was 'planned' since 2026-06-22)
SELECT build_status, deployed_at, last_built_at
FROM pages WHERE id = 'e4b3b195-919f-45ad-854e-201d3e846ea8';

-- (2) the rendered section instance
SELECT pc.position, cc.function, pc.build_status,
       LENGTH(COALESCE(pc.rendered_html,'')) AS rendered_len,
       (pc.rendered_html LIKE '%data-runtime-fill%')            AS marker_in_rendered,
       (pc.rendered_html LIKE '%data-archive-template%')        AS has_clone_template,
       (pc.rendered_html LIKE '%provocations-archive__empty%')  AS has_empty_state
FROM page_components pc JOIN content_components cc ON cc.id = pc.component_id
WHERE pc.page_id = 'e4b3b195-919f-45ad-854e-201d3e846ea8'::uuid
ORDER BY pc.position;
```
```
# (3) live check (allow a few minutes for Actions → B2)
curl -s -o /dev/null -w "%{http_code}\n" https://vonc.com/provocations/index.html
curl -s https://vonc.com/provocations/index.html | grep -o 'data-component="[^"]*"'
```
Expected: (1) build_status off 'planned' + timestamps; (2) one row, provocations-archive-list,
rendered_len in the thousands, all three flags t; (3) 200 + the data-component present (empty state
visible — correct until Step 4).

**STEP 4 RESULT (2026-07-07): DONE.** 4.1 insert verified (js_len 3281); 4.2 renderer COMPLETED
(corr d6b54136), bundle header **3 active snippet(s)**; 4.3 `"archive"` served; 4.4 screenshot: all
EIGHT rows render (dates/titles/teasers/dot-stats), empty state hidden, index unaffected. ONE
cosmetic defect found in the screenshot + ONE design question raised — see the new CURRENT STEP
below the procedure.

**Step 4 — make the archive list FILL (explicit procedure, retained for reference).**

*Plain terms:* the page is live but its list is deliberately empty — a runtime-fill shell. Filling it
needs (a) the loader JS stored as a `js_snippets` row, (b) that row bundled into the site-wide
`/assets/js/snippets.js` by the site-asset-renderer, (c) the `archive` data published in
`/data/provocations.json`. Then look at the page. Four actions, in order, each with a check.

**4.1 Install the loader row (psql).** In the usual session
(`kubectl -n ai-persona-system exec -it postgres-clients-0 -- psql -U clients_user -d clients_db`)
paste the whole of `provocations_archive_install.sql` (outputs). Full text inlined here for
self-containment:
...
Expected: `INSERT 0 1`, then the verify row
`provocations-archive-loader | js_len ≈ 3280 | ["provocations-archive-list"] | t`. *(Ran 2026-07-07: js_len 3281 vs source 3287 — the familiar ±6-byte paste drift; bundle + browser confirm complete.)*

**4.2 Rebuild the JS bundle (asset-renderer trigger).**

```
bash scripts/initial_messages/210_vonc_trigger/083_trigger-asset-renderer-vonc.sh
```

Runs the renderer workflow (load site → concatenate active js_snippets → git-commit
assets/js/snippets.js → GitHub Actions → B2). Check completion with the printed correlation id:

```sql
SELECT status, current_step FROM orchestration_states
WHERE correlation_id = '<CORRELATION_ID from the script>'::uuid;
```

Expected `COMPLETED | complete`. Then after Actions propagates (a few minutes):

```
curl -s https://vonc.com/assets/js/snippets.js | head -3
curl -s https://vonc.com/assets/js/snippets.js | grep -c provocations-archive-loader
```

Expected: the bundle header says **3 active snippet(s)**; the grep returns >= 1.

**4.3 Publish the data.** In the `sites` repo checkout, REPLACE `vonc.com/data/provocations.json`
with the contents of `provocations.sample.json` (outputs, v3 — keeps today/lobby/arena so the index
loaders keep working; adds `archive`, 8 entries). Commit + push. Check after propagation:

```
curl -s https://vonc.com/data/provocations.json | grep -c '"archive"'
```

Expected `1`.

**4.4 Browser check.** Open https://vonc.com/provocations/index.html, hard refresh (Ctrl+Shift+R).
Expected: EIGHT rows fill (date | title | teaser | pulsing-dot stat), the "Nothing filed yet…" line
disappears, rows navigate on click; https://vonc.com/index.html unchanged. If rows don't appear:
dev-tools → Network → provocations.json must be 200 and snippets.js must contain the archive loader;
report findings.

Ordering: 4.1 MUST precede 4.2 (the renderer bundles what's in the table). 4.3 is independent, but
the page only fills once BOTH the new bundle and the data are live.

---

**CLOSED (2026-07-08).** Ghost-row fix applied (template_fixed=t, instance_fixed=t) and redeployed
via 085 (corr cd5f553e, deployed_at 2026-07-08 16:30:43); rendered_len 7455 → 7671 (+216 = the hiding
rule inserted into BOTH the base selector and its mobile-media-query copy — the REPLACE correctly
fired twice). DB verified; live confirm =
`curl -s https://vonc.com/provocations/index.html | grep -c 'data-archive-template] { display: none;'`
→ expect 2, dot-row gone on hard refresh. CTA graph left as Option B (parked). provocations-index
thread DONE: page live, 8 rows fill, ghost row eliminated.

**NEXT STEP → provocation-card mini-lobby trim** (confirmed decision — the overlap is live on the
index: provocation-card renders its own 4-card mini-lobby AND lobby-grid renders the 6-card arena).
Scope: remove the `.pc-card-grid` block from the provocation-card template (component 6163ff14) and
the `data.lobby` fill from provocation_card_loader.js; keep the headline provocation + stats + CTAs.
Then a bundle re-render (asset renderer) for the loader + an index rerender for the template. Detailed
plan to be drafted when we start it. Framework preventions queued behind (role-aware ≥1-section
invariant; complete_error fails loudly; last_built_at is dead through build AND rerender).

---

*(historical current-step block, 2026-07-07 — retained)*
**CURRENT STEP (2026-07-07) → template-row fix + CTA decision.**
*Defect:* the hidden clone template renders as an empty row with a lone dot above "5 Jul". Cause:
the component CSS `.provocations-archive__item { display: grid; }` is an AUTHOR rule and overrides
the `hidden` attribute's UA-stylesheet `display: none`. Fix = a more specific author rule in the
template + current instance: run `fix_archive_template_display.sql` (outputs; anchored + guarded;
expect template_fixed=t, instance_fixed=t).
*Design question (user raised):* "Enter today's Arena" goes to /index.html — the spec'd static
fallback — and there is no arena page. The interactive surface EXISTS: the Gauntlet tool at
/tools/gauntlet/index.html (deployed; gauntlet-interface js_content live). The CTA graph is
CIRCULAR (hero → archive; archive → home; gauntlet-cta → archive; only nav/footer reach the tool).
OPTION A (recommended, minimal): retarget the archive CTA to /tools/gauntlet/index.html — SQL in
chat/notes; rides the same rerender. OPTION B: leave until the real take-filing arena exists. The
broader graph (site_specs.cta.primary_url, hero) = a deliberate CTA-map pass, queued (those URLs
are baked into rendered sections; a proper refresh is a section rebuild, not string surgery).
*STATUS UPDATE (2026-07-07 ~16:20):* CTA decision = **OPTION B** (leave the graph until the real
take-filing arena exists; retarget SQL parked in the notes). 085 script created correctly and RUN
(corr 0ea43016) — page redeployed (deployed_at 16:20:17) — **but rendered_len is still 7455**, i.e.
the display fix was NOT in the instance when it redeployed (the fix adds ~110 chars → ~7565; no
template_fixed/instance_fixed output was seen). The ghost dot-row is presumably still live
(`curl -s https://vonc.com/provocations/index.html | grep -c 'data-archive-template] { display: none;'`
→ expect 0 until fixed). REMAINING: run fix_archive_template_display.sql (expect t/t; a 0-row result
= anchor mismatch, stop and check the stored text) → re-run 085 → hard refresh (dot-row gone; curl
grep → 1; rendered_len ~7565). NB last_built_at stays NULL through builds AND rerenders — the column
is currently dead; audit-list item confirmed.

*Redeploy:* create the page's rerender trigger by copying the index one —
`bash make_085_rerender_provocations.sh` (outputs; shows the diff) — then run
`085_rerender-provocations-vonc.sh`, confirm COMPLETED, hard-refresh the page: dot-row gone
(+ CTA on the Gauntlet if Option A).


**Queued after:** provocation-card mini-lobby trim (confirmed decision — overlap is live on the
index); framework preventions (role-aware ≥1-section invariant; complete_error fails loudly;
auditor rules); Q1 evidence hand-off to the planner/reconciler thread. Parallel-track note: the
inline-script EXTRACTION pattern is live for gauntlet-interface / latest-news / archetype-quiz
(js_content + `<script src=` refs, no raw inline) — provocation-card + lobby-grid cosmetic scripts
are candidates for the same treatment later.

---

## History: status as of 2026-07-05 → 2026-07-06 (superseded — see YOU ARE HERE above)

**Where we are.** Index page: complete and live — six sections deployed; provocation-card and
lobby-grid are runtime-filled shells (data-runtime-fill) whose loaders in /assets/js/snippets.js
fill them from /data/provocations.json (`today` + `arena`); verified in the browser. Shared-component
check: closed, no contamination (neutral brief-explanation base; guard held; general-hero stray
inactive). **Open problem: /provocations/index.html is a 404** — the destination of the header nav
and every primary CTA. Cause confirmed (App. I): the plan has ZERO sections for `provocations-index`
(the DISTINCT page_name confirm shows the plan covers only: about, archetypes, contact, index,
tool-archetype-taster-quiz, tool-gauntlet), and seven build/rerender items completed as silent
no-ops. Do NOT fire a bare needs_page — it would be silent no-op #8. Fix order: component → plan
section row → build → loader + data.

**Constants (full ids).**
- site_id `9ec3b9ee-5b08-461b-b4f8-9e1e03579c74`  •  current plan id `77493277-f510-47ea-aa27-8fca415743d6`
- provocations-index page id `e4b3b195-919f-45ad-854e-201d3e846ea8` (url /provocations/index.html)
- index page id `b4d24f8e-fccd-49df-9dad-aa56a0b20a68`

### Step 0 — remaining confirms (run first)
```sql
-- 0a. provocations-index pages.sections fallback (expect []). NB an earlier check queried
--     ecb637c1-845f-46bf-b174-9c92a43f9586 which is TOOL-GAUNTLET, not this page — that result
--     (["gauntlet-interface","archetype-result-card"]) proved the fallback mechanism exists, but
--     this page's own fallback is still unconfirmed:
SELECT sections FROM pages WHERE id = 'e4b3b195-919f-45ad-854e-201d3e846ea8';

-- 0b. Confirm the current plan id matches the constant:
SELECT id, is_current FROM site_plans
WHERE site_id = '9ec3b9ee-5b08-461b-b4f8-9e1e03579c74' AND is_current = true;

-- 0c. What do existing plan-section rows carry for the style ids (mirror the pattern)?
SELECT page_name, ordering, component_name, component_version_id, palette_id, layout_id, typography_set_id
FROM site_plan_sections
WHERE plan_id = '77493277-f510-47ea-aa27-8fca415743d6' AND page_name = 'index'
ORDER BY ordering LIMIT 3;
```

### Step 1 — create the component (provocations-archive-list)
Spec: `SPEC_provocations-archive-list.md` (self-contained header + clone-template archive list;
data-runtime-fill emitted AT GENERATION; NO inline script; llm header fields). Trigger =
084 script, 083 dual-placement pattern (section_type top-level + full spec object), function name
pinned. Verify after: one active row, function='provocations-archive-list', schema populated,
template carries data-runtime-fill and NO `<script>`:
```sql
SELECT id, function, is_active, quality_score, schema_field_count,
       (html_template LIKE '%data-runtime-fill%') AS has_marker,
       (html_template LIKE '%<script%')           AS has_inline_script   -- expect f
FROM content_components
WHERE function = 'provocations-archive-list' AND forked_from IS NULL
ORDER BY is_active DESC, updated_at DESC;
```

### Step 2 — insert the plan-section row (after Step 1 verifies)
One row, ordering 0, copying the style-id pattern from an existing index row (INSERT-SELECT so
nothing is hardcoded beyond names):
```sql
INSERT INTO site_plan_sections (plan_id, page_name, ordering, component_name,
                                palette_id, layout_id, typography_set_id)
SELECT plan_id, 'provocations-index', 0, 'provocations-archive-list',
       palette_id, layout_id, typography_set_id
FROM site_plan_sections
WHERE plan_id = '77493277-f510-47ea-aa27-8fca415743d6' AND page_name = 'index' AND ordering = 0
RETURNING id, page_name, ordering, component_name;

-- confirm:
SELECT ordering, component_name FROM site_plan_sections sps
JOIN site_plans sp ON sp.id = sps.plan_id
WHERE sp.site_id = '9ec3b9ee-5b08-461b-b4f8-9e1e03579c74' AND sp.is_current = true
  AND sps.page_name = 'provocations-index' ORDER BY sps.ordering;
```

### EXECUTION STATE (2026-07-05): Steps 0 and 2 DONE; Step 3 FIRED EARLY; Step 1 is the open one
Step 0 confirmed: pages.sections = [] for e4b3b195; plan id 77493277 is current; style ids on
existing plan rows are all NULL (the INSERT-SELECT copied NULLs — matches the pattern).
Step 2 DONE: plan row e006a35e-9258-49f0-a99a-f11eba409bc9 (provocations-index / 0 /
provocations-archive-list). Step 3 was FIRED BEFORE Step 1 — the build ran against a plan whose only
section names a component that does not exist yet (Step-1 verify: 0 rows). pages.build_status is
still 'planned'. Inspect what that early build did (and whether it raised a needs_new_component
child — if such a child COMPLETED, the ValidateInputContract patch is live and the loop built the
component itself; if it FAILED with missing required fields [section_type], the patch is not
deployed and the 084 manual trigger is the path):
```sql
SELECT item_type, item_key, status, handler_agent, attempt_count,
       substring(COALESCE(error,''),1,200) AS err, created_at, completed_at
FROM site_work_items
WHERE site_id = '9ec3b9ee-5b08-461b-b4f8-9e1e03579c74'::uuid
  AND created_at > NOW() - INTERVAL '60 minutes'
ORDER BY created_at DESC;
```
Then: run 084_create_provocations-archive-list_vonc.sh (unless the loop already built it) → Step-1
verify (has_marker=t, has_inline_script=f, schema_field_count>0) → RE-FIRE Step 3 (fresh item_key —
the INSERT below is safe to re-run as-is, gen_random_uuid gives a new key) → watch → Step 4.

**UPDATE 2026-07-06:** the early build (f7e6429e) completed with NO children — the pipeline does NOT
self-heal a plan row naming a nonexistent component (silent no-op variant; no needs_new_component
raised, so no evidence either way on the validator-patch deploy). 084 then ran (corr 7b5f96df):
component CREATED — id 70d6662a-0e6f-478d-bc2e-b9e8e5eaeb37, quality 100, 8 fields, **has_marker=t,
has_inline_script=f** — first live validation of the generation-time guards; Step 4 needs NO marker
SQL this time. Step 3 re-fired (448ee097): triaged → claimed 14:01:24 → complete 14:01:57 (~33s,
plausible for a one-section page), no error. VERIFY NOW (full UUIDs):
```sql
SELECT build_status, deployed_at, last_built_at
FROM pages WHERE id = 'e4b3b195-919f-45ad-854e-201d3e846ea8';
...
ORDER BY pc.position;
```
then `curl -s -o /dev/null -w "%{http_code}" https://vonc.com/provocations/index.html` → 200 and
`curl -s https://vonc.com/provocations/index.html | grep -o 'data-component="[^"]*"'` →
provocations-archive-list present (empty state visible until Step 4).
**UPDATE 2026-07-06 (later):** builds 448ee097 + (pending) 986c106f no-op because the workflow
gates on the PAGE RECORD's own sections: load_page_record finds the page by name and returns
sections parsed from pages.sections (ours = [] → section_count 0) → graceful complete with only
site_record in output. The plan row targeted the right concept, wrong column. Precedent: the
provocation blog page built with ZERO plan rows (pages.sections drove it); tool-gauntlet's
pages.sections is populated. CONFIRM with query B (index vs provocations pages.sections) + the
page-build-handler workflow dump, THEN:
```sql
UPDATE pages SET sections = '["provocations-archive-list"]'::jsonb, updated_at = NOW()
WHERE id = 'e4b3b195-919f-45ad-854e-201d3e846ea8' AND sections = '[]'::jsonb
RETURNING name, sections;
```
→ re-fire the Step-3 needs_page INSERT (fresh key) → watch → verify (pages.build_status,
page_components row, curl 200). Keep the plan row e006a35e — it is the structural source of truth;
buildable pages carry BOTH. NB voncdocs2.txt arrived empty; diagnostics re-requested.

**UPDATE 2026-07-06 (mechanism confirmed + Step 4 ready).** Workflow dump: the silent complete is
the `complete_error` step (a complete_workflow, output [page_content, site_record], success_message
"Content writer skipped — page has no sections defined") — reached when ready_count==0; the writer
never runs so results carry only site_record (the diagnostic signature). Section sources for page
builds: site_specs aspect `site_plan` (authoritative) → pages.sections (fallback) → the
site_plan_sections TABLE IS NOT READ by this path. Query B confirmed the gate; the pages.sections
UPDATE is applied (["provocations-archive-list"]); the next needs_page attempt runs with it. Watch
with the standing poll, then verify (pages.build_status/last_built_at; the page_components row:
marker_in_rendered / has_clone_template / has_empty_state; curl 200 + data-component grep).
STEP-4 PACK IS READY in outputs: provocations_archive_loader.js (list-only; header/CTA are
build-time), provocations.sample.json v3 (adds `archive`, 8 entries), provocations_archive_install.sql
(Part A only — the marker was emitted at generation, no Part B exists). Run order after the page
verifies: install SQL → trigger-asset-renderer-vonc.sh → commit JSON v3 to vonc.com/data/
provocations.json → browser verify (header copy, archive rows filled, empty state hidden, CTA).

For Step 4 the loader targets the GENERATED DOM (dump is authoritative over the spec's class names):
```sql
SELECT f.key, f.value->>'source' AS source, f.value->>'fallback' AS fallback
FROM content_components cc, LATERAL jsonb_each(cc.input_schema->'fields') f
WHERE cc.id = '70d6662a-0e6f-478d-bc2e-b9e8e5eaeb37' ORDER BY f.key;
...
VALUES
  ('9ec3b9ee-5b08-461b-b4f8-9e1e03579c74'::uuid, 'manual', 'needs_page',
   'Build provocations-index (now HAS a plan section; was 404 destination of all primary CTAs)',
...
```sql
SELECT item_key, status, attempt_count, claimed_by, claimed_at, completed_at,
       substring(COALESCE(error,''),1,200) AS err
FROM site_work_items
WHERE site_id = '9ec3b9ee-5b08-461b-b4f8-9e1e03579c74'::uuid
  AND item_key LIKE 'manual-build-provocations-%'
ORDER BY created_at DESC LIMIT 3;
```
Expect triaged → claimed (build-dispatch-loop) → complete, no error. Then
`SELECT build_status, deployed_at FROM pages WHERE id='e4b3b195-919f-45ad-854e-201d3e846ea8';`
and `curl -s -o /dev/null -w "%{http_code}" https://vonc.com/provocations/index.html` → 200.
The page ships with the archive EMPTY-STATE visible until Step 4 lands (acceptable interim).

### Step 4 — loader + data *(superseded — the explicit procedure is at YOU ARE HERE Step 4.1–4.4 above)*
Loader `provocations-archive-loader` (clone-template fill from `data.archive`) as a js_snippet →
site-asset-renderer → extend /data/provocations.json with the `archive` object → verify cards in a
browser. Deliverables produced at this step; same file destinations as App. H.

### Parallel/queued (unchanged)
provocation-card mini-lobby trim (confirmed decision, redundancy live on the index);
store-validation hardening; illustration wiring; other-page audit (incl. why `provocation` blog page
is also absent from the plan yet built — pages.sections fallback precedent); framework prevention
items from guide §9 "Page build completes having built nothing" (planner ≥1-section invariant,
zero-planned vs zero-ready handler guard, rerender skip-warn, auditor linked+planned + URL-presence
rules).
...
### (historical position marker — 2026-06-29 ~19:00; superseded)
...
## Appendix F — Index content-section deferral FIXED; provocation-card truncation repair + rebuild (historical marker — 2026-07-02 ~19:50)
...
JOIN site_plans sp ON sp.id=sps.plan_id WHERE sp.site_id='9ec3b9ee-5b08-461b-b4f8-9e1e03579c74' AND sp.is_current ORDER BY 1;`
plus `SELECT sections FROM pages WHERE id='e4b3b195-919f-45ad-854e-201d3e846ea8';` — expect '[]').
