...
## RESULT — FIX VERIFIED (2026-07-03)
Deployed the patched rerender_single_page_action.go (data-runtime-fill exemption) + ran the
marker SQL (template_marked=t, rendered_marked=t) + ran rerender-index-vonc.sh (assemble-only,
correlation 5ba01d82, success). Ground-truth check:
  curl -s https://vonc.com/index.html | grep -o 'data-component="[^"]*"'
  → hero, provocation-card, provocation-card, gauntlet-cta, brief-explanation, system-stats
=> FIVE sections; provocation-card RESTORED to the live page; lobby-grid correctly ABSENT.
(provocation-card appears twice = its <section> tag + its own inline script's
querySelector('[data-component="provocation-card"]') — ONE section, not a duplicate; one
page_components row at position 2.)
Log confirmation (optional, Info not Debug): `getPageSections: filtered empty sections`
"skipped":1 "kept":5 (fires when skipped>0) + a Warn naming lobby-grid. Earlier grep misses
were my wrong literal string (zap JSON) + saved logs* not spanning the rerender window.

OUTSTANDING (not this fix):
 - provocation-card is a runtime-filled shell — curl shows the empty shell; confirm it FILLS
   in a BROWSER (loader in snippets.js runs client-side; needs /data/provocations.json present
   + current — Phase 3). Proven filling in Phase 2.
 - lobby-grid build (real content, or a runtime-fill loader → then it earns data-runtime-fill)
   — its own task; absence is correct until then.
STATUS: section-drop for provocation-card CLOSED (structural fix in the assembler; runtime-fill
sections are now first-class via data-runtime-fill).
