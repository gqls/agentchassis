...
**Created:** 2026-06-25  •  **Updated:** 2026-07-07 (gate PASSED — page live at /provocations/; current step = Step 4 loader+data)
...
## >>> YOU ARE HERE (2026-07-07, gate passed) <<<
...
**CURRENT STEP → Step 4 (pack ready in outputs, run order):**
1. psql: `provocations_archive_install.sql` (Part A only — there is NO marker part; it was emitted at
   generation) → verify js_len > 0 for 'provocations-archive-loader'.
2. bash `trigger-asset-renderer-vonc.sh` → snippets.js bundle commit.
3. git: commit `provocations.sample.json` v3 (adds `archive`, 8 entries) as
   vonc.com/data/provocations.json.
4. Browser: /provocations/index.html — header copy (build-time), archive rows filled by the loader,
   empty state hidden, CTA back to today's provocation; index page unaffected.
...
### Step 4 — loader + data (mirrors the lobby-grid pack, App. H)
