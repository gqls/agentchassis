...
> ### ▶▶▶ YOU ARE HERE (2026-06-25 ~17:00)
> All structural rendering bugs are fixed and the 8-page rebuild has been queued.
> The hero renders correctly (verified by screenshot). The **one open decision**
> is Step 7: the empty-schema shell components (provocation-card, lobby-grid,
> brief-explanation) need a content mechanism. **Decision made: Option 2 (runtime
> JS pipeline).** The JS infrastructure already exists (`js_snippets` +
> `render_js_snippets_for_site` + `site-asset-renderer`); what's missing is the
> `js_snippets` rows for these components and the daily-provocation data source.
> Section plan now located (site_plans → site_plan_sections). Full design is in
> PLAN_spark_provocation_pipeline.md. Next action: run Step 8 diagnostics to
> confirm no pipeline exists, then Phase 2 of the plan (JS against static test JSON).
...
- [ ] **DECISION: empty-schema shell components** — provocation-card, lobby-grid, brief-explanation have no content mechanism (see Step 7)
- [ ] **Verify all 8 pages** after rebuild completes
