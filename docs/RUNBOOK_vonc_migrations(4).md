...
**IMPORTANT — always look up page IDs inline. Never use placeholder strings.**
The correct pattern is to subquery or look up the UUID immediately and embed it
directly in the INSERT. Placeholder strings like `<tool-gauntlet id>` cause the
item to be claimed and fail before a fix UPDATE can run.

If placeholder rows are inserted by mistake:
1. Check if they are still present: `SELECT spec->>'page_id' FROM site_work_items WHERE ...`
2. If present, DELETE them filtering on the placeholder string (not the intended UUID).
3. Reinsert with real UUIDs inline.
4. If `DELETE 0` — the retry mechanism already purged them after failure. Reinsert only.

**Lesson from 2026-06-24:** A fix UPDATE that filters on the intended UUID value
(not the placeholder) will match already-correct rows and leave placeholder rows
untouched, reporting misleading `UPDATE N` counts.
