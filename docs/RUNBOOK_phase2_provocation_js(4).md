...
### >>> YOU ARE HERE (2026-06-25 ~19:30)
Diagnosis is complete. The snippet is in the DB. The immediate next action is a
**decision**, not a build: confirm whether the provocation fetch logic belongs in
the component's own inline script (Path 1) or stays as the js_snippet we inserted
(Path 2) — see the **PATH DECISION** section. Two confirmation queries (PD-1, PD-2)
settle it. Whichever path: the JSON shape and fill logic we wrote are reused as-is.
To get a visual proof on vonc right now without resolving the path, run P2-4
(trigger site-asset-renderer) + P2-2c (data file) → P2-5 (verify).
...
- [ ] **P2-4** Trigger `site-asset-renderer` for vonc → rebuild snippets.js (script ready)
- [ ] **P2-2c** Get `provocations.json` onto the site the proper way (see DATA DEPLOY — do NOT hand-commit)
- [ ] **P2-5** Verify the provocation-card fills in the browser (needs P2-4 **and** P2-2c)
- [ ] **P2-6** Extend to lobby-grid + brief-explanation

**Path decision** (resolve before the framework build, so we build the right thing):

- [ ] **PD-1** Fetch `latest-news` component template — does it `fetch()` JSON in an inline `<script>`? (Confirms news uses Path 1.)
- [ ] **PD-2** Check `provocation-card` extracted `js_content` — is the existing inline script already deployed as `/tools/assets/provocation-card.js`?
- [ ] **PD-3** Decide Path 1 (inline-in-component, auto-deploys on rerender) vs Path 2 (js_snippet, needs site-asset-renderer). Record the decision + reason.
...
- [ ] **FX-6** Gap 3: auto-deploy the JS when it changes — *only needed if PD-3 picks Path 2*; if Path 1, page rerender already ships the component JS
