...
An **identity / ownership / entitlement** layer the platform does not have today:
owners/accounts, entitlements/subscriptions, and `owner_id` on sites. It is the
connective tissue every pricing model needs, and it touches three points (intake
gate, build trigger, maintenance gate). **Design the seam now** (the abstraction
interface, where it is checked, the ownership column); build the billing depth
later.
