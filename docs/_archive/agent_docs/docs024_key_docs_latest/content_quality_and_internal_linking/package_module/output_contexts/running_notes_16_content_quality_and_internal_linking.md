...
**Also pending from June-02 work (not yet closed):** `reconcile_section_data` + `flag_page_image_rebuild` need registry entries in `registry.go`; the section-data reconciler still needs a host (loop check or post-build finalize step). Per `HANDOFF_2026-06-02_hero_resolver_and_section_data_reconciler.md`. `flag_page_image_rebuild` is already referenced by the live image-build-handler workflow → must be registered before the next image build or that step errors.
