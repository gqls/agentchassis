...
5. **Re-fire provision (`gpu-provisioner` standalone) and read
   `await_responses_topic`** [OPEN — the real test of D4]: `system.responses.
   gpu-provisioner` = fix live; `system.agent.gpu-provisioner.responses` = §6
   caveat, will hang as 17:10 did. Manual orchestrate fire: set the Kafka message
   key to `correlation_id` (`kcat -k "$CORRELATION"`) to match the real producer.
6. **Trigger `iter_0`** via the existing mechanism with `input_data` containing
