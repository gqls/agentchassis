...
- The `aws` CLI examples below assume `--endpoint-url https://s3.us-east-005.backblazeb2.com`
  with your B2 key id/secret exported as the AWS creds. Use whatever B2 client you
  normally use; the keys/prefixes are what matter.
...
aws --endpoint-url https://s3.us-east-005.backblazeb2.com \
    s3 cp bundle.tar.gz s3://personae-model-training/finetuning/scripts/bundle.tar.gz
aws --endpoint-url https://s3.us-east-005.backblazeb2.com \
    s3 ls s3://personae-model-training/finetuning/scripts/bundle.tar.gz
```
Pass criteria: the listing shows a fresh timestamp/size. Optional belt-and-braces: pull
it back and confirm `run.sh` contains the `--upload-manifest` block and `02_train` has the
...
### 4b. Launch via your usual flywheel-C trigger
Kick off a `training-launcher` run the way you normally do (the model-trainer /
flywheel-C orchestration that creates the `training_runs` row and dispatches the
launcher). Note the `training_run_id` it creates — you will watch B2 under it.
...
```bash
# checkpoints appear DURING the run (re-run as it progresses):
aws --endpoint-url https://s3.us-east-005.backblazeb2.com \
    s3 ls s3://personae-model-training/finetuning/checkpoints/<run_id>/
# the final adapter appears at the end (after RUN_SH_DONE):
aws --endpoint-url https://s3.us-east-005.backblazeb2.com \
    s3 ls s3://personae-model-training/finetuning/artefacts/<run_id>/
```
