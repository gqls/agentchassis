...
- **OPEN ISSUE (under investigation) — presign_checkpoints loop output has no presigned URLs.**
  The `loop_complete` results carry the raw checkpoint KEYS
  (`finetuning/checkpoints/70156952…/ckpt-<N>.tar.gz`) under fields `page_html`,
  `rendered_html`, `presign_checkpoints_item`, `original_item` — NO `ckpt_presign`, NO
  presigned PUT URL. `page_html`/`rendered_html` are NOT from our substep (output_field
  `ckpt_presign`), so the loop's `substep_output_fields` looks wrong/stale (resembles a
  rendering-loop default). This breaks `flatten_checkpoint_urls → assemble →` the manifest's
  checkpoint URLs. NOT root-caused — need launcher-pod `presign_one`/`prepare_object_url`-PUT
  logs + the loop-expansion `substep_output_fields` + whether `flatten` errored. `presign_final`
  (outside the loop) is unaffected, so the run may still train and upload the final adapter.
...
- presign_checkpoints `loop_complete` results show raw keys under `page_html`/`rendered_html`/
  `<loop>_item` and no `ckpt_presign`/presigned URL → loop `substep_output_fields` not derived
  from the substep's `output_field` (OPEN, under investigation 2026-06-06).
