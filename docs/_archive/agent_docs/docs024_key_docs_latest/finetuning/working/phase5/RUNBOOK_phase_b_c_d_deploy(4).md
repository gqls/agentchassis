...
JSON
```
- **`export_id`** is the PK of `training_exports.runs` (the dataset export to train on).
  Find a valid one — and **verify it actually has rows**, because `runs.rows_exported` can
  disagree with the real `training_exports.rows` (an export row existed with
  `rows_exported=1957` but 0 actual rows, which failed the run at `prepare_training_data`):
  ```sql
  ...
  ```
  Pick an `id` whose `actual_rows` is non-zero (iter_0 used `146a9a12-…`, 1958 rows). Replace
  `<VALID_EXPORT_ID>`. `actual_rows` governs run length (see 4a).
