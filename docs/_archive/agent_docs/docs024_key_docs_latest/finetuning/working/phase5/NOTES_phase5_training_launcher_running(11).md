...
NEXT (reasonable steps, in order): (1) classifier action — parse `STATUS=` + `reachable` → verdict (pure Go, unit-testable); testable NOW against the running box via a one-off `ssh_get_status` (expect `STATUS=ALIVE`; `STATUS=DONE_OK` in ~9h). (2) terminal `mark_training_run` sibling (running→complete|failed). (3) `find_active_training_instances` query action (thunder_instances WHERE status='running' AND training_run_id IS NOT NULL). (4) monitor agent definition (thin workflow: find → per-instance spawn → probe → classify → reconcile+decommission) + scheduler wiring. Static-only so far (no Go toolchain here) — `go build ./...` pending in-repo.
