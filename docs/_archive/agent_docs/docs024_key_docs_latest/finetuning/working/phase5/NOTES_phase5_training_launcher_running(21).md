...
DEPLOY/VERIFY IN PROGRESS (2026-06-04): migrations 106/107/108 applied (both defs `active`). First manual worker test fired a quoted heredoc still holding placeholder ids (`<fabfd7fa…>`) → bogus id, no-op (row stayed 0/NULL/running, as predicted). Re-fired with real ids (corr `dc5fab5f-…`) → row STILL 0/NULL/running AND no chassis log under `-l app=agent-chassis` across 1h. Reading: it stopped at/before the probe (`reset_streak` writes `last_probe_at` — never ran; `bump_streak` never ran or the counter would read 1). Most likely the `app=agent-chassis` selector does not match the generic chassis pods (which consume `generic.requests` and run the worker in-process), so the grep hit the wrong/no pods — NOT confirmed. NEXT: `kubectl -n ai-persona-system get pods --show-labels | grep -i chassis` to find the real label + re-grep (or grep agent type `thunder-training-monitor-worker`); if pods are present but still no trace, check `generic.requests` consumer-group lag. **Monitor NOT verified; do NOT enable the schedule yet.**
