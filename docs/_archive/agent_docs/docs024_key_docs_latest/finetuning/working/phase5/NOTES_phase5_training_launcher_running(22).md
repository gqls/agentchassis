...
- **Training-monitor: BUILT, verifying.** `running → complete/failed` reconcile +
  release-on-completion is now the `thunder-training-monitor` orchestrator +
  `-worker` (106/107/108 + 5 actions). Deploy applied; worker test not yet
  confirmed (see §5 DEPLOY/VERIFY — chassis-log/selector thread open). Not enabled.
- **`prepare_artefact_url` / `prepare_object_url` PUT path** is the basis of the
  upload plan (`PLAN_checkpoint_and_artefact_upload_b2.md`) — extend to PUT + batch,
  pre-mint per-object write-only URLs at launch. Still untested end-to-end.
