...
### 2.5.1 Phase 2 — automating flywheel C (in build, 2026-05)

Design originally decided 2026-04-23. Chassis drives, GPU VM serves. The GPU
VM becomes a compute service the chassis calls, rather than a host that needs
to reach into the cluster. This keeps credentials out of the VM, makes the VM
easy to rebuild or replace, and fits the existing orchestrator/agent pattern.

**Status as of 2026-05-12:** chassis-side agents wired and verified
end-to-end; GPU-provisioning lifecycle (thunder-adapter Phase 3) in active
build with provision and decommission actions delivered, reaper pending.
See `STATUS_thunder_adapter_2026-05-12.md` for current sub-phase status.
...
**Chassis side — components (current shape):**

The pipeline split into more pieces than the original design once we got into
it. End-to-end orchestration runs cleanly; what's left is wiring real
implementations behind the stubs.

| Agent | Status | Purpose |
|---|---|---|
| `training-data-preparer` | Live | Reads `training_exports.rows`, formats JSONL, uploads to B2 with presigned URL. Returns `dataset_uri`, `row_count`, `size_bytes`. |
| `gpu-provisioner` | Stub deployed; **real impl delivered as 3.6 (migration 029), awaiting deploy** | Migration 029 + chassis v1.0.1014 swaps the mock workflow for one that uses the new `dispatch_thunder_provision` action — publishes `provision_instance` to thunder-adapter and awaits the response. External contract unchanged: same inputs (`training_run_id`, `hyperparameters`), same response shape (`instance_ip`, `ssh_user`, `ssh_key_secret_name`, `provisioning_id`, `thunder_identifier`, `provisioned_at`). |
| `training-launcher` | Stub (migration 023) | Currently returns mock data. Will POST the dataset URL + hyperparameters to the provisioned VM's `/jobs` endpoint, poll until complete, fetch the adapter to B2, then trigger `decommission_instance`. |
| `model-trainer` | Live as orchestrator | Sequences the three specialists via `call_preparer → call_provisioner → call_launcher`. Input_mapping resolves auto-unwrap from `.response`, `output_field` singular convention adopted after the path-resolution refactor (see chassis input_mapping.go v2). |
| `model-evaluator` | Not yet built | Will run the held-out eval against the produced LoRA adapter. Existing v1 eval set (50 cases, post-2026-04-23 cutoff, claude-opus-4-7 judge) is the starting point. |
| `training-flywheel-orchestrator` | Not yet built | Will chain `training-data-exporter → model-trainer → model-evaluator → conditional swap_agent_model`. |

**GPU provisioning surface (thunder-adapter, in build):**

Rather than having `gpu-provisioner` agent directly call Thunder's API
synchronously, the agent dispatches a Kafka message to a dedicated adapter
(`thunder-adapter`). This matches the existing adapter pattern used for
GitHub, image generation, and web scraping. Benefits:

- Provisioning and decommissioning become normal request/response actions
  with all the standard correlation, retry, and validation handling
- Per-provider cap and concurrency limits enforced in one place via
  `thunder_provision_check` view (singleton config + active count + 24h spend)
- Reaper as a scheduled task instead of in-process timers
- New providers (RunPod, Lambda) become parallel adapters with the same shape

Schema (migration 025, applied):

```
thunder_config         (singleton row: daily_cap_usd, max_concurrent_instances,
                        default_hard_uptime_hours, default_hourly_rate_usd,
                        estimated_new_run_cost_usd, is_paused, pause_reason)

thunder_instances      (id UUID, thunder_instance_id TEXT, instance_type,
                        instance_ip, ssh_user, ssh_key_secret_name, status
                        (CHECK: provisioning/running/decommissioning/
                        decommissioned/reaped/lost/failed), max_uptime_hours,
                        training_run_id FK, requested_by, hourly_rate_usd,
                        cost_usd, provisioned_at, running_since,
                        decommissioned_at)

thunder_spend_24h      (view: SUM(cost_usd) WHERE created_at > NOW() - 24h)

thunder_provision_check (view: derives can_provision boolean + denial_reason
                         from config + active count + 24h spend)
```

**Schema additions — model lifecycle** (deferred to model-evaluator build):
...
The `model_training_runs` table is already in use — Phase 1 manual run
produced row `f02b586a-def2-478e-97fd-81b0d10951fb`.
...
**Preconditions before treating Phase 2 as complete:**

1. ✅ Phase 1 complete — manual training run proved the scripts work
2. ✅ Phase 1 produced a baseline LoRA + eval output we trust as the "working" reference point
3. ⏳ Flywheel D eval harness — v1 eval set exists; harness can be invoked manually but not yet wired into an automated `model-evaluator` agent
4. ⏳ VM has stable public endpoint (DNS, TLS) — Thunder Compute instances have public IPs; durable DNS is what's missing for production
5. ✅ thunder-adapter Phase 3.0–3.5 deployed and verified (provision + decommission + reaper). Phase 3.6 (gpu-provisioner real impl) delivered, awaiting deploy of chassis v1.0.1014 + migration 029 + first real provision verification.
...
- [x] First real training dataset exported (1,958 rows, export_id `146a9a12-...`)
- [x] Flywheel C Phase 1 — manual 70B training on Thunder Compute proved the
      scripts. Held-out eval of 50 cases run; aborted on a single truncation
      anomaly in `llm_call_log.response_text` (1 of 50, post-cutoff sample;
      tracked as a TODO, not a blocker).
- [x] Migration 025 — `thunder_instances`, `thunder_config`, `thunder_spend_24h`,
      `thunder_provision_check` view, daily cap and concurrent-instance limits.
- [x] thunder-adapter Phase 2 (skeleton) deployed and verified at v1.0.1010.
- [x] thunder-adapter Phase 3.0–3.5 deployed and verified (2026-05-14):
      provision/decommission action handlers wired, thunder-reaper scheduled
      task confirmed end-to-end via synthetic test (cost computed at $3.61 for
      2h × $1.80, 404-idempotent on both API delete and Secret delete).
...
- [ ] 1-of-50 truncation bug in held-out eval (`llm_call_log.response_text`)
      — discovered during flywheel C Phase 1 eval. Investigate after thunder
      lifecycle is verified end-to-end with a real provision.
- [ ] Remove credential-leak debug logging in `platform/storage/s3.go NewS3Client`
      (still printing env var values; flagged in earlier sessions).
- [ ] Document the `orchestration_states` `owner_agent_type='generic'` filter
      gotcha in `016_debugging_guide_v2.md` — top-level chassis-resident
      workflows are owned by `generic`, not their configured agent_type. Use
      `collected_data->'config'->>'agent_type'` or
      `orchestration_name LIKE 'sched-<task>-%'` instead.

### 4.3 Open tasks (larger, flywheel completion)
- [x] First real export — done; 1,958 rows from page-content-writer +
      knowledge-extractor populations.
- [x] Dry-run Unsloth end-to-end on a GPU to validate the flow.
- [ ] Write an A/B scoring script — produce outputs from Claude and the local
      ...
      (Held-out eval set v1 = 50 cases with post-2026-04-23 cutoff exists;
      judge model = claude-opus-4-7.)
- [ ] Bring the GPU Ollama endpoint up reliably (`ollama-gpu` currently DOWN).
- [x] thunder-adapter lifecycle (Phase 3) — provision, decommission, reaper
      all wired and tested via synthetic.
- [ ] **Verify the real provision pathway** — build chassis v1.0.1014, apply
      migration 029 (replaces gpu-provisioner stub), set daily_cap_usd=5,
      provision via kcat → gpu-provisioner → thunder-adapter, verify
      thunder_instances row populated with real instance_ip, decommission via
      reaper or kcat, confirm cost computed. This is the last unexercised
      pathway in Phase 3.
- [ ] Replace `training-launcher` (migration 023) stub with real implementation
      that SSH-execs the training script on the provisioned VM. Not blocking
      iter_1 — but iter_1 will need it to run hands-free.
- [ ] After thunder-adapter is verified end-to-end: trigger iter_1 training
      run using the same 1,958-row dataset to validate the full automated
      pipeline against the manual Phase 1 result.
...
- **Where does training happen?** Thunder Compute single-A100 worked in Phase 1 testing. Other options: RunPod, Lambda, local 3090/4090. Trade-off: spot cost vs queue time vs ops complexity. Thunder is now wired into the chassis pipeline via thunder-adapter — switching providers would mean a different adapter.
- **On-cluster training pod or manual?** Manual proved the path in Phase 1. The thunder-adapter + provision/decommission lifecycle is the long-term answer — the automation is in flight.
...
- 2026-05-14 (thunder-adapter Phase 3.6 delivered, awaiting deploy) — Real
  implementation for gpu-provisioner replaces migration 022's stub. New chassis
  action `dispatch_thunder_provision` mirrors the decommission dispatch pattern:
  publishes provision_instance to system.adapter.thunder.requests with optional
  GPU/sizing overrides from input_data, returns AwaitResponse:true so the
  chassis pauses the workflow until the adapter completes the create+wait
  cycle (~3-5 min typical). Migration 029 UPDATEs the gpu-provisioner
  agent_definition's default_config to swap the mock workflow for a real two-step
  one (dispatch_provision → complete_workflow). External contract preserved:
  same inputs (training_run_id, hyperparameters), same response shape
  (instance_ip, ssh_user, ssh_key_secret_name, provisioning_id,
  thunder_identifier, provisioned_at). Model-trainer's
  call_provisioner → call_launcher chain works without changes. Pending:
  build chassis v1.0.1014, push, apply migration 029, do the real provision
  verification (Option 1 from STATUS doc §5).
- 2026-05-14 (thunder-adapter Phase 3.5 deployed and verified) — thunder-reaper
  scheduled task complete and exercised end-to-end. Synthetic test inserted a
  fake row with running_since = NOW() - 2h and max_uptime_hours = 1 (cap
  exceeded by 1h); kicked the scheduler with last_triggered_at = NULL. Within
  one tick the reaper fired, dispatched decommission_instance via the new
  dispatch_thunder_decommission action, the adapter ran DecommissionAction
  (Thunder API 404 → idempotent success, Secret 404 → idempotent success),
  cost computed at $3.61 for 2h × $1.80 per the hourly_rate_usd column, row
  transitioned to status='decommissioned'. The observability cross-check
  initially showed zero rows under owner_agent_type='thunder-reaper' — a
  filter trap: top-level chassis-resident workflows are owned by 'generic',
  with the configured agent_type living in
  collected_data->'config'->>'agent_type' and orchestration_name following
  sched-<task-name>-<timestamp>. Recorded as a debugging-guide TODO. Phase 3
  lifecycle is now structurally complete; only the real-provision pathway
  remains untested (deferred to 3.6 deploy).
...
- 2026-05-12 (thunder-adapter Phase 3.4 delivered, 3.5 pending) —
  ...
  scheduled task (15-min sweep finding instances older than max_uptime_hours,
  dispatching decommission for each), then deploy + manual verify + replace
  gpu-provisioner stub (migration 022) with real Kafka dispatch to
  thunder-adapter.
...
- 2026-05-12 (thunder-adapter Phase 3.0/3.1/3.2 delivered) — Three structural
  ...
  to create/get/delete; no list/watch/update.
