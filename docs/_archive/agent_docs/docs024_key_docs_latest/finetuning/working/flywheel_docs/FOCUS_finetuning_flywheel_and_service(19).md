...
- **Bucket / key convention (verified 2026-05-22 against the live training-data-preparer agent_def):** bucket `finetuning`; dataset key `finetuning/datasets/{export_id}/training.jsonl`; artefact key `finetuning/artefacts/{training_run_id}/adapter.tar.gz`. Callers pass `export_id` / `training_run_id`; the adapter builds the keys. `model_lifecycle.training_runs` has **no** `dataset_uri` column, so the agent_def's `s3_bucket` + `s3_key_template` are the authority for where data lives. Expiry: dataset 60min, artefact 24h (must outlive a training run).
- **Credentials:** the adapter builds its own `storage.S3Client` (reusing `platform/storage`, mirroring agent.go's optional-storage pattern). B2 keys (`B2_APPLICATION_KEY_ID` / `B2_APPLICATION_KEY`) live in the k8s secret **`personae-storage-secrets`** — NOT `personae-platform-secrets` (an earlier thunder-adapter manifest comment wrongly claimed the latter; verified against the cluster and the agent-chassis deployment, both read B2 from `personae-storage-secrets`). The adapter manifest must `envFrom` that secret plus set `TRAINING_BUCKET=finetuning` and `S3_ENDPOINT`. If creds are absent at startup the storage client stays nil and the two actions return `storage_unavailable` (provision/decommission still work).
