...
| `gpu-provisioner` | Stub (migration 022) | Currently returns mock data. Will dispatch `provision_instance` via Kafka to thunder-adapter once Phase 3 is verified. |
...
5. ⏳ thunder-adapter Phase 3 complete (provision + decommission + reaper) and verified end-to-end
...
- [x] thunder-adapter Phase 2 (skeleton) deployed and verified at v1.0.1010.
      Kafka consumer/producer wired, DB connection live, health endpoints
      responding, round-trip via kcat confirms validator-tier headers correct.
...
- [ ] Complete thunder-adapter Phase 3: provision + decommission actions are
      written, awaiting deploy + manual verification cycle. Reaper (3.5) still
      to write. See STATUS_thunder_adapter_2026-05-12.md for the full picture.
- [ ] Replace `gpu-provisioner` (migration 022) and `training-launcher`
      (migration 023) stubs with real implementations that dispatch via Kafka
      to thunder-adapter. Currently both return mock data — model-trainer's
      orchestration completes end-to-end but no real GPU work happens yet.
- [ ] After thunder-adapter is verified: trigger iter_1 training run using
      the same 1,958-row dataset to validate the full automated pipeline
      against the manual Phase 1 result.
