...
If the adapter needs to manage k8s resources (e.g. SSH key Secrets for
thunder-adapter Phase 3+), create a dedicated Role/RoleBinding with the
minimum verbs needed — never `cluster-admin`, never broad `*` verbs.

**A trap**: k8s RBAC's `resourceNames` field does NOT support glob patterns or
prefixes. It's either an exact-name allowlist or omitted entirely. For
adapters that create resources with dynamic identifiers (e.g.
`thunder-ssh-<uuid>` where UUID is unknown in advance), `resourceNames` has
to be omitted. The minimum-permission knob is instead the verbs list: include
only what the adapter actually uses.
...
rules:
  - apiGroups: [""]
    resources: ["secrets"]
    # resourceNames omitted: secret names are thunder-ssh-<uuid>, generated
    # at provision time. The practical scope-limit is in the verbs list
    # (create/get/delete only — no list, watch, or update) and in the
    # adapter's Go code, which only creates Secrets with the thunder-ssh-
    # prefix.
...
Structural note: this Role is bound to `ai-persona-app`, which other adapters
also use. They inherit the Secret verbs even though they don't exercise them.
Acceptable while all adapters are trusted first-party code. If stricter
isolation becomes important, split into per-adapter ServiceAccounts (one
ServiceAccount per Deployment, each bound to only its own Role).
