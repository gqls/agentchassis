...
*v2_34 (2026-06-11): added §0 item 24 — documentation claims are assumptions: point-of-use verification plus the dated-claim convention (`[checked YYYY-MM-DD: evidence]`). Prompted by 019's stale "isn't built yet" claim (the split was fully built) and by same-day staleness of freshly written docs when a design decision changed after the text.*
...
**24. A document's claims about code, schema, or row state are assumptions like any other — verify them at point of use, and date what you verify.** Concrete instance (2026-06-11): `019_tool_library` stated the template/JS split "isn't built yet"; the code had it fully built (`separateInlineJS` → `js_content` → `collectJSAssets`), and trusting the doc would have meant designing the mechanism from scratch — a reuse-before-creating violation caused by a *doc*, not by a missed grep. The same day, the freshly written tool-doc-header docs went stale within hours when the strip-placement decision changed after the text was written — so scheduled "freshness sweeps" structurally cannot keep up. Staleness is a coupling property, not an age property. The disciplines:
- (a) **Point of use, not on a schedule.** Before a doc claim becomes load-bearing for a design, migration, or patch, re-ground that specific claim against the thing it describes — grep for code claims, `\d` for schema claims, SELECT for row-state claims. This is item 13's `\d`-before-SQL generalised to documentation.
- (b) **Date what you verify — claim-level, never doc-level.** When writing or re-confirming a falsifiable claim, attach the exact greppable tag `[checked YYYY-MM-DD: <evidence>]`, where evidence is the command or source location (`grep -n separateInlineJS store_generated_component_action.go`; `\d content_components`). Re-checking updates the date in place — one date means "last checked", not a history. Tag only load-bearing falsifiable claims (code existence/behaviour, schema, row state); rationale and intent stay untagged. Never stamp a whole document "verified" — a doc-level stamp blesses every claim in it and decays silently.
- (c) **Negative claims carry their falsifying command.** "X isn't built" is the hardest claim class — existence checks cannot falsify absence. State it checkably: `no tool path writes js_content [checked 2026-06-11: grep -rn js_content across the tool actions — no writes]`. The tag tells the next reader how to re-check in ten seconds.
- (d) **An undated falsifiable claim is unverified.** Read it as a hypothesis; your grep wins over the doc. The fixed `[checked ` token also makes claim dates mechanically extractable for the planned tier-1 `doc_drift` discovery check and later commit-triggered doc flagging — but those catch renames/removals only, never stale negatives or semantic drift; (a) remains the control that is always current.
- (e) **Docs update in the same change as the decision.** The writer is the first drift source — the same-change rule is the docs analogue of shipping an interface change and its implementations together (item 18); the tag convention then makes that rule auditable.
