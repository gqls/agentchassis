...
This is the load-bearing architectural decision for everything past Phase 4a. Cross-cluster dispatch only becomes *useful* (not just "the contract works") when remote agents can read and write the same Postgres the parent uses. Almost every agent type does. Phase 4's "restrict to no-DB agents" is a smoke-test crutch, not a destination.

The chassis code is already cross-cluster-friendly: `dbCfg.SSLMode` is parameterised, `pgxpool.HealthCheckPeriod = 30s` catches stale connections, the pool is env-tunable, retries exist, and `default_query_exec_mode=simple_protocol` is set for PgBouncer transaction-mode. **No chassis code changes are required**. The work is in the topology, the secrets, and the runbook.

### 8.1 The five questions any topology must answer

1. **Reachability** — How does an agent Pod on AWS resolve and connect to a Postgres in primary cluster?
2. **Auth** — How is the connection authenticated and encrypted, and how are credentials and certs rotated?
3. **Failure mode** — When the link goes down, what does the agent see, and how fast?
4. **Pool sizing** — How many connections will N agents × M clusters open at primary Postgres, and can primary survive it?
5. **Schema awareness** — Does the per-client schema routing (search_path-driven) still work when the connection is multi-hopped through PgBouncer + tunnel?

Any topology that doesn't have a clean answer to all five is incomplete.

### 8.2 Options, scored against the five questions

| Option | Reachability | Auth | Failure mode | Pool sizing | Schema awareness |
|---|---|---|---|---|---|
| **A. Public Postgres LB + TLS + IP allow-list** | DNS to public LB | TLS+SCRAM, IP allow-list | Hangs until TCP timeout (slow) | Direct = N×M connections | Works (same DB) |
| **B. Local PgBouncer on remote → public Postgres LB on primary** | Local in-cluster DNS resolves to local PgBouncer | TLS+SCRAM at PgBouncer→Postgres | Local PgBouncer fails fast | Multiplexed (good) | Works |
| **C. Local PgBouncer on remote → private tunnel (Tailscale/VPN) → in-cluster Postgres** | Tunnel exposes primary's in-cluster DNS or a stable IP | No public exposure; tunnel-level auth + mTLS Postgres | Tunnel daemon health is observable | Multiplexed (good) | Works |
| **D. Read replica per region + writes cross-cloud** | Local for reads, remote for writes | Same as A or C | Reads survive partition; writes hang | Halved | Lag handling required — complex |
| **E. API-driven (no direct DB from remote)** | HTTP to core-manager on primary | TLS+JWT | API call returns error fast | One pool at core-manager | core-manager handles it |

**A** is the simplest and **the wrong default** for production. Any public Postgres endpoint is a security ticking clock — Postgres CVEs, credential leaks, IP allow-list drift as NAT egress changes. Acceptable only for early staging.

**D** is an optimisation, not a topology. Skip until reads dominate.

**E** is architecturally cleanest but enormous scope — every read/write in the chassis becomes an API call. Right answer eventually, wrong answer now.

That leaves **B** and **C**. Both keep the chassis env vars identical across clusters (every agent connects to `pgbouncer-clients.ai-persona-system.svc.cluster.local:6432`); the only difference is what that name resolves to in each cluster. The choice between them is whether Postgres is publicly exposed or not.

### 8.3 Recommendation: Option C (Local PgBouncer + private tunnel)

For production. Concretely:

- **Primary cluster:** Postgres and PgBouncer stay exactly where they are. No public exposure. No new LoadBalancer. No new TLS cert burden beyond what already exists internally.
- **Remote cluster:** Deploy a thin PgBouncer (single Pod, small footprint) that connects upstream to primary's PgBouncer (or directly to Postgres). The remote agent Pods connect to the *local* PgBouncer using exactly the same `pgbouncer-clients.ai-persona-system.svc.cluster.local` DNS name. Zero env-var differences from primary.
- **Network layer:** Tailscale (cross-cloud, minimal infra) or cloud-native VPN (better latency within a single cloud's product family, e.g. AWS Transit Gateway, GCP Cloud VPN). Tailscale's biggest win is that it works identically AWS↔GCP↔Hetzner↔Kind-on-laptop, which matches the way `010-infrastructure` is structured.

Why this beats option B:
- Postgres stays in-cluster. The blast radius of a Postgres CVE drops to "anyone with cluster access" — the same as today.
- Tunnel health is a first-class signal (Tailscale status, VPN tunnel-up metric). The agent's connection error becomes "PgBouncer can't reach upstream" rather than "TCP timeout to 51.x.x.x" — far more diagnosable.
- Credential rotation is in-cluster only. No "did the cert on the public LB get rotated?" calendar items.

Why it beats option A:
- No public attack surface.
- Connection multiplexing (Postgres sees N PgBouncer connections from each remote, not N × agents).
- Agents see DB errors within seconds rather than waiting on TCP RTO.

### 8.4 Robustness controls

These apply whether you pick B or C, but matter more with C because the failure modes are clearer.

#### TLS / encryption

- **Set `sslmode=verify-full` for cross-cluster connections.** Currently chassis uses `sslmode=disable` (fine in-cluster, dangerous through a tunnel that anyone could imitate). Per-cluster overlay:
  ```yaml
  # remote-cluster overlay configmap
  SERVICE_INFRASTRUCTURE_CLIENTS_DATABASE_SSLMODE: "verify-full"
  SERVICE_INFRASTRUCTURE_TEMPLATES_DATABASE_SSLMODE: "verify-full"
  ```
  Primary keeps `disable` for in-cluster speed. The `DatabaseConfig` struct (`platform/config`) already routes this — no code change.
- **CA bundle:** mount the primary's Postgres CA into remote spawned Pods as a configmap. `PGSSLROOTCERT` env var points at it. Cert renewal becomes a configmap update.

#### Connection pooling — three layers

| Layer | Where | What it does | Sizing |
|---|---|---|---|
| Agent pool (pgxpool) | In every agent Pod | Multiplexes the agent's own queries | `MaxConns=3, MinConns=0` (current default — fine) |
| Local PgBouncer | Remote cluster | Multiplexes all agents in this cluster onto few upstream conns | `pool_size = 10–20` per database, `max_client_conn = 500` |
| Primary PgBouncer | Primary cluster | Multiplexes everyone | `pool_size = 25–50` per database, `max_client_conn = 2000` |

Rule of thumb for the upstream side: each remote cluster contributes `pool_size` (10–20) connections at primary's PgBouncer. With 5 remote clusters that's 50–100 extra connections at primary. Comfortably within typical limits.

**Stay in transaction-pool mode.** Session-pool mode breaks the multiplexing benefit, and session-bound features (prepared statements, advisory locks, `SET search_path`) are already worked around in the codebase via `default_query_exec_mode=simple_protocol`.

#### Schema routing (per-client schemas)

The chassis sets `search_path` per-client per-query via the connection's session. Under transaction-pool mode this is per-transaction — already handled. Cross-cluster doesn't change this: the remote agent issues the same `SET search_path` statement at the start of its transaction, the local PgBouncer forwards to upstream's PgBouncer, upstream forwards to Postgres. Per-client isolation is preserved end-to-end.

**Watch:** if any code path sets `search_path` outside a transaction expecting it to persist, that path silently breaks under transaction pooling. Audit before going cross-cluster. (This isn't a new problem; primary already uses transaction pooling for pgbouncer-clients.)

#### Retry and circuit-breaking

- Existing 5-attempt exponential backoff in `NewPostgresConnection` is fine for startup but doesn't help mid-conversation. For cross-cluster, a transient tunnel blip should not fail a workflow.
- pgxpool's `HealthCheckPeriod = 30s` already invalidates dead connections — that's the main protection.
- For long-running operations (large transactions, batch inserts), wrap them in a retry helper that classifies errors: retryable (`53000`, `57P03`, network) vs permanent (constraint violations, syntax errors). The codebase has scattered ad-hoc retries; consolidate one helper if not already done. Not strictly required for MVP but worth adding before Phase 5 goes live.

#### Failure modes — explicit handling

| Failure | What primary sees | What remote sees | Recovery |
|---|---|---|---|
| Tunnel down | No change | Agents get `connection refused` from local PgBouncer in <1s; orchestration fails fast | Tunnel restart; agents will be re-dispatched by normal retry logic |
| Primary Postgres down | All clusters' agents fail | Same | Primary recovery; agents retry |
| Remote PgBouncer OOM | No change | Agents fail at local DNS lookup or local connect | PgBouncer restart in remote cluster |
| Cert expiry (TLS) | All connections refused | Same | Configmap rotation + PgBouncer reload (no agent restart needed) |
| Schema migration in flight | Slow queries; locks | Same, with tunnel latency added | Run migrations during low-dispatch windows |

**The most important property of option C:** every failure has a single component to look at first, and that component is in the cluster you're already SSH'd into. Option A has the failure spread across three layers (LB, public network, Postgres).

#### Observability

- Add `prometheus-pgbouncer-exporter` next to each PgBouncer (local + primary). Already a standard Strimzi-deployment pattern. Metrics: `pgbouncer_pools_client_active_connections`, `pgbouncer_pools_server_active_connections`, `pgbouncer_pools_max_wait_us`.
- Track per-cluster connection counts at primary: `pg_stat_activity.application_name` should include the remote cluster id. Set `application_name` in the connection string per-cluster (`?application_name=agent-aws_eu_west_2`).
- Alert on: tunnel down >30s, primary `pg_stat_activity` count >80% of `max_connections`, any cluster's `pool_size` exhausted.

#### Disaster scenarios — write down the runbook entries

1. **"All remote clusters lost DB access at once."** → Tunnel up but auth failed (cert?) OR primary PgBouncer crashed OR primary Postgres crashed. Order to check: primary Postgres status → primary PgBouncer logs → cert expiry on the upstream TLS → tunnel daemon health on each remote.
2. **"Remote cluster's agents all hang."** → Tunnel up, but local PgBouncer can't reach upstream. Check local PgBouncer logs first; restart it; if that doesn't fix, restart the tunnel daemon.
3. **"Primary Postgres connection count climbing without obvious cause."** → A remote PgBouncer mis-configured (e.g. accidentally session-mode) is holding connections. Check `pg_stat_activity` grouped by `application_name`.
4. **"Slow queries appear from a specific cluster only."** → Network latency. Verify tunnel route hasn't flipped to a worse path; check tunnel bandwidth saturation.

### 8.5 Phased adoption

The current Phase 4 in this doc skips DB access on remote clusters. The Postgres strategy below is what turns Phase 4 from "smoke test" into "useful":

- **Phase 4a (in current MVP):** Restrict remote clusters to agents that don't need DB. Validate dispatch contract end-to-end.
- **Phase 5 — Remote DB access via Option C:**
  1. Deploy Tailscale (or chosen VPN) operator/agent on primary and remote clusters. Verify primary's `pgbouncer-clients.ai-persona-system.svc.cluster.local` resolves and accepts connections from the remote node network. **From the remote cluster**: `kubectl run psql-test --rm -it --image=postgres:16 -- psql -h <primary-pgbouncer-tailscale-IP> -U clients_user -d clients_db -c '\dn'`. Should succeed before going further.
  2. Deploy local PgBouncer on remote cluster at the same in-cluster DNS name (`pgbouncer-clients.ai-persona-system.svc.cluster.local`). Configure it to use the tunnel-resolved upstream.
  3. Set per-cluster overlay env vars: `SSLMODE=verify-full`, `application_name` includes cluster id, CA configmap mounted, password secret replicated.
  4. Dispatch a small DB-needing agent (e.g. `briefing-agent`) to the remote cluster. Verify the row appears in the expected primary DB schema.
  5. Watch `pg_stat_activity` on primary for the duration. Connection count should be flat (multiplexed), not climbing.
- **Phase 6 — Hardening:** prometheus exporters, alerts, retry-classification helper, runbook drills.

### 8.6 Anti-patterns to refuse

- **Direct `psql://...public-IP...` URLs in agent_definitions or workflow configs.** Postgres endpoints are infrastructure config, not orchestration config. Keep them in cluster-level configmaps/secrets only.
- **Different DSNs per agent type for the same DB.** All agents in a cluster must use the same logical DSN; routing by cluster is OK, routing by agent type within a cluster is a footgun.
- **Session-pool mode anywhere** in the new setup. Even if "just easier for now". Transaction-pool only.
- **Trusting `application_name` for security.** It's diagnostic. Real auth is the user/role + IP/network restrictions.
- **Sharing the `clients_user` role across clusters without per-cluster credentials.** Make each cluster's user distinct (`clients_user_aws_eu_west_2`, etc.) so when a credential leaks, you revoke that one cluster's access without taking the rest down.

### 8.7 What does NOT change in the chassis

To be clear: across all options A/B/C and all phases, **the agent code stays identical**. The pgxpool config, the retry logic, the schema-routing — all unchanged. The whole strategy is "make the network look the same to every Pod regardless of which cluster it's in". This is intentional and is what makes the dispatch contract clean.

The only code touch we might want before Phase 5 is a consolidated retry helper for cross-cluster transients — and even that is a polish item, not a blocker.
