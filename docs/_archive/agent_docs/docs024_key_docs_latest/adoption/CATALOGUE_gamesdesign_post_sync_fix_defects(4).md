...
*Fix (two-part, gated on reads):* **(A)** auto-complete must verify `page_components` exist before completing a `needs_content_page`, else re-queue — needs the orchestration-engine lost-response code (not in the actions dump); belongs with the Lever-C reliability thread. **(B)** gate `deployed`+`stamped` on real content so a 0-component page is never marked deployed — needs the landing-page deploy path. *Immediate unstick:* `UPDATE pages SET build_status='needs_rebuild', built_from_plan_version=NULL WHERE … name='index'` (recurs if the response is lost again until A is fixed).
