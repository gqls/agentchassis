...
*Tentative cause (to verify, not concluded):* the deploy path marks `deployed` independently of commit success (in tool-recreation, `update_status('deployed')` precedes `deploy_page`/`git_commit`), and the git-adapter's `updateRef` is `force:false` with no retry — so a concurrent commit to the shared `sites` repo loses with a non-fast-forward and **fails silently** (FOCUS_dispatch open item 4). A homepage committing amongst many during the cascade is a plausible loser. Alternatives not yet ruled out: empty-assembly skip (0 `page_components`) or a path bug. Decisive reads pending (homepage `page_components` count; its rerender work-item error; git-adapter logs for a 422 on `index.html`).
