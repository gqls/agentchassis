...
*Cause (updated 2026-06-04 after the decisive read — git race RULED OUT):* the homepage has **`n_rendered=0`** (zero components), and the git-adapter logs show commits **succeeding** (no 422 / non-fast-forward). So it is the **empty-assembly** case, not a commit race: no components → the rerender has nothing to assemble → it skips the commit → no file. Worse, it is `build_status=deployed`+`stamped` despite having no content — and since `built_from_plan_version` is only stamped by `UpdatePageStatusAction`'s deployed branch under the current plan, a deploy path marked it deployed+stamped with nothing to deploy. Consequence: the reconciler's `decideEmit` treats it as done and **never re-emits a build**, so it stays fileless permanently — the same "marked deployed independent of real content/commit" smell as A1, on a `landing` page. Sub-cause pending two reads: (a) does the plan give the homepage any sections, and when was `deployed_at` set; (b) did any `needs_page`/`needs_composition` ever run for it. Planner gap (no sections planned) vs composition gap (sections planned, no components built) → different fixes. Fix will be two-part: repair why the homepage has no components, and stop a 0-component page being marked deployed+stamped so the reconciler rebuilds it.
