...
**A1. Tool and game *pages* did not deploy at all. [FIX SHIPPED — PARTIALLY VERIFIED, 2026-06-03]**
...
*Verification (2026-06-03 adoption, correlation `e9609749`):* **Parser fix confirmed working.** The four tools whose recreation has completed — `tool-ttk-calculator`, `tool-ehp-calculator`, `tool-drop-rate-simulator`, `tool-progression-architect` — show `n_rendered=1`, `build_status=deployed`, and `tools/progression-architect/index.html` is committed to the repo (first ever deployed tool page with a `page_component`). This answers the prior open question — the tool HTML *does* reach `save_page_sections`, so the `clean_html`-flow concern is moot. The other 7 tools/games are still `triaged`/`planned` (not yet built) — **not an A1 correctness problem**; they're throttled by the dispatch-throughput limitation (one-site-per-tick + NOT-EXISTS exclusion + claim-timeout stalls — see Family J below), being drained naturally. `stamped=f` on the deployed tools is expected pre-drain (they deployed before the plan existed; `sync` fills the version on its pass, which hasn't completed because the cascade is still draining).

*Remaining to fully close A1:* let the run drain, then re-run the decisive read — A1 closes if all 11 show `n_rendered≥1`, `deployed`, `stamped=t` after sync, with files under `/tools/<slug>/` and `/games/<slug>/`. Note the fixes apply to *new* builds; pre-existing tool rows are not retroactively deployed.
