...
- **Stripe is live and earning.** It takes a single £29 payment per report through Stripe Checkout
  (live mode) and delivers the report on the verified `checkout.session.completed` webhook — proven
  end-to-end with a real card on 2026-06-14. (With no Stripe keys set it falls back to a Fake provider
  for local testing.)
...
- **The product pipeline is proven end-to-end with real money** — request → operator confirm (engine
  runs, draft stored) → approve → buyer pays by card → live webhook → report delivered by email.
...
**1. Rewrite the landing/site pages** — the live priority. The current page reads as though the
visitor already knows what idea.uk is; rewrite the copy for a cold first-time visitor (the niche:
people who run a website and want a paid-for tool on it), in plain language, and give it a professional
design grounded in the frontend-design skill (not the current style). The landing page is embedded
(`//go:embed page.html`), so it needs a rebuild + redeploy. Its own focused pass.

(**Stripe is DONE** — live and earning; full setup is in the runbook's "Stripe billing — setup". The
key is a **restricted** key scoped to `Checkout Sessions: Write` only. Refunds are **manual in the
Stripe dashboard** — there is **no `/refund` endpoint**. Gotcha worth keeping: a single stray character
in `STRIPE_WEBHOOK_SECRET` — here a `0` appended on a paste — fails the signature check with a 400 and
stalls the order at `awaiting_payment`; the secret must be byte-exact, and resending the event recovers
the paid order. `PLAN_stripe_billing_integration.md` remains a *separate* plan for the chassis
build/host product, not the £29 report.)

**2. Optional PDF of the report.** Deliver the report as a PDF (attachment or link) alongside the
email — a tangible artefact makes the £29 spend feel more worth it. Constraint: the service is
stdlib-only and builds offline (`GOPROXY=off`), so this means either relaxing that to vendor a Go PDF
library, or rendering the existing report HTML to PDF. Not built yet; its own focused pass.
(The related language/layout work is **done**: the plain-text report was rewritten to clean prose,
the prompts tightened, and the report now also goes out as **styled HTML email** with a plain-text
fallback. The UTF-8 mailer fix handles the garbled characters.)
...
**4. Sibling tools beyond the one-off ideas report (product direction).** idea.uk currently produces a
single ideas report. But the best answer for some businesses isn't an "AI product idea you build" at
all — it's an ongoing tool we run for them. Example: a small farm in Devon would value a standing watch
that searches the web regularly for new restaurants opening nearby, new chefs arriving, food-market
listings, etc., and alerts them to fresh leads. That's a different shape (a monitoring-and-alert
service, not a one-shot report) the idea engine doesn't cover. So: when the engine can't find a strong
"build-it" AI idea — or alongside one — there's room for a family of *sibling tools* under idea.uk,
e.g. a "local opportunity radar" that monitors and alerts. Worth designing as its own small tool that
reuses the engine's research/verify pieces. Captured for later, not scheduled.
...
- **Stripe live mode** — DONE (live and earning since 2026-06-14; see the runbook's "Stripe billing —
  setup").
