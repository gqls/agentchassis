...
### 2.9 STATE RESET mid-investigation (2026-05-26, step 9)

Diagnostic queries L/M/N1/N2 all returned **0 rows**, though J/K (previous step) had just returned those same games, `tool-game-*` duplicates, and `needs_tool_recreation` items. A `LEFT JOIN` cannot drop the page row, so the pages/items no longer exist under those names. Between steps 8 and 9 the parallel adoption chat (and/or a reaper) **re-ran adoption/planning or cleared work items** — consistent with the build_status drift already noted.

**Implication:** step-8 snapshots (and any name-specific diagnostic built on them) are stale. Re-baseline before further diagnosis. Queries R1 (full page inventory + `has_widget`), R2 (current work-item state), R3 (recent `adoption_crawl`) added to §7. The substantive questions (does correct routing land a widget; origin of tool/game duplicates) are unchanged but must be re-pointed at live rows. T4/T5 remain valid pending re-baseline (the specific duplicates may be gone/renamed). General lesson: snapshot the inventory at the start of each session and treat name-specific reads as time-sensitive while the other chat is active.
...
-- ── Step 9: L/M/N all returned 0 rows — state was reset between steps. Re-baseline first. ──

-- R1) Current full page inventory + whether each page carries a widget. Name-agnostic.
SELECT p.page_type, p.name, p.build_status, p.status,
       to_char(p.updated_at, 'MM-DD HH24:MI') AS updated,
       (SELECT count(*) FROM page_components pc WHERE pc.page_id = p.id) AS sections,
       EXISTS (SELECT 1 FROM page_components pc
               JOIN content_components cc ON cc.id = pc.component_id
               WHERE pc.page_id = p.id
                 AND cc.component_level IN ('tool','game') AND cc.is_active = true) AS has_widget
FROM pages p JOIN sites s ON s.id = p.site_id
WHERE s.domain = 'gamesdesign.co.uk'
ORDER BY p.page_type, p.name;

-- R2) Current work-item state (queued/running now + recency).
SELECT item_type, handler_agent, status, count(*) AS n,
...
-- R3) Recent adoption crawls (does a fresh one explain the wipe?).
SELECT to_char(created_at, 'MM-DD HH24:MI') AS at, id::text
FROM research_results rr JOIN sites s ON s.id = rr.site_id
WHERE s.domain = 'gamesdesign.co.uk' AND rr.result_type = 'adoption_crawl'
...
- **2026-05-26 (step 9)** — Diagnostics L/M/N1/N2 all returned 0 rows, but the same games / `tool-game-*` / `needs_tool_recreation` items existed in step 8. State was reset between steps (parallel adoption chat re-ran adoption/planning or a reaper cleared items). Stale snapshots discarded; added re-baseline queries R1 (inventory + `has_widget`), R2 (work-item state), R3 (recent adoption crawl). §2.9 logged. Diagnostics to be re-pointed at live rows once R1–R3 land. Lesson: snapshot inventory at session start; treat name-specific reads as time-sensitive while the other chat is active.
