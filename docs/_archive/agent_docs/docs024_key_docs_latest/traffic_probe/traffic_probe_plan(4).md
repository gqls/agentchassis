...
### Decision 5 (OPEN) — gating backend-requiring sections off static sites
A capability match (section requires backend ⊆ site's deploy target provides),
not a site-type match. Layered proposal:
- **Layer 1 (now, zero DDL):** component carries `suitable_site_types:
  ["intent-probe"]`; planner's load_components gains
  `AND suitable_site_types = '[]'::jsonb` so restricted components are opt-in
  (reachable only via roadmap section_types). One-line query change in the
  build-site-planner workflow JSON.
- **Layer 2 (when backend sections multiply):** `sites.deploy_config.capabilities`
  vs component `semantic_tags` requirements (`requires-backend` already set on
  intent-probe), enforced by one audit check → site_work_items findings.
Operator to confirm Layer 1 (esp. the planner query change) before applying.
