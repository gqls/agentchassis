...
| landmine rows ↔ file | **546 entries both sides, 0 key mismatches, 0 strays, 0 `·` left in any `subject_key`**; 2,758 rows. ⚠ Other lanes' churn shows up here constantly and is NOT a regression: `NOT IN DB` = appended but not yet dispatched; `KEY MISMATCH` = synced, then its footprint line was rewritten. Two of each were live within an hour of this doc. **The discriminator that matters: if a DB key contains `·`, or reads as one long prose string while the file's read as short targets, that entry was synced by an OLD parse — a `·` in a DB key is the only shape that would mean the 08-14 splitter regressed.** Both cases are the owning lane's to re-sync, and `refootprinted` now catches them by identity | `./scripts/landmines-keys-check.py` |
