...
| CLC-015 | `input_schema` house dialect enforced at the table: `chk_input_schema_no_legacy_dialect` (+ `IsLegacyInputSchemaDialect`, birth-path twin) | constraint LIVE 2026-08-16 10:24Z (437 applied, council APPROVED, refusal induced); Go half inert until roll | A CHECK constraint refuses the retired JSON-Schema `input_schema` dialect (top-level `properties`) for EVERY producer — seeds, scripts, Go, admin, restores — because all four reintroductions since the 07-21 "extinct" census were hand SQL and no Go gate would have seen one; 437 converts the last 3 rows by exactly `SchemaContentFields`' projection. ⚠ `WarnLegacyDialect` firing now means the CONSTRAINT is gone, not that someone seeded the old shape | component-lifecycle.md |
