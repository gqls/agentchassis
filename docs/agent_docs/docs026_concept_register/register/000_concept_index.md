...
| WDS-016 | `spec.suggestion` is the one item-spec steering channel; `spec.content_guidance` is aliased onto it at load time | built | Four emitters wrote a rewrite brief under a key no reader reads, so the brief was discarded and the rewrite proceeded steered by nothing, reporting complete | work-dispatch.md |
