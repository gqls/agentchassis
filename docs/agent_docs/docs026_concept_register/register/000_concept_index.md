...
| WFA-011 | Offline shared-`output_field` detector (`config-key-audit --shared-output-fields`) | deployed (standalone CLI tool) | Flags two steps sharing an `output_field` that are transitively reachable over the FULL routing graph and are different actions. Ack-file ratchet reports NEW pairs and STALE acks. Online CronJob half NOT built. | workflow-authoring.md |
