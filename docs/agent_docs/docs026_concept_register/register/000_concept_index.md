...
| FIX-059 | Seat length budget applier (one block, snapshot-then-write, refuses hand-authored blocks) | deployed | Candidate 4's reorder was refuted by measurement; the length budget is the half the evidence credits. 10 targets, 3 chosen by FIX-058's alert | fix-loop.md |
