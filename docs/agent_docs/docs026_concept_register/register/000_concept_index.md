...
| SCR-006 | Deprecated aliases for LITERAL config settings (ActionInputSpec.DeprecatedConfigKeys, ResolveConfigSetting) | built, adopted by 4 actions, inert until the next chassis roll | A renamed step-config SETTING can keep its old name working in three declared lines, and the audit lists who still writes it. ActionInputSpec.Deprecated cannot do this — it resolves the old key's value as a data path — which is why the domain→pipeline rename half-landed on three actions (bugs_open/136) | adopting-and-scraping.md |
