...
| SEO-003 | Per-page search-exclusion opt-in (`pages.noindex` → `injectRobotsNoindex`) | migration APPLIED + row-verified; Go half inert until the next chassis roll | Opt-in boolean, default false, injecting a robots-exclusion meta on the rerender path only — LANDMINE: the `assemble_page` build path does NOT read it | seo.md |
