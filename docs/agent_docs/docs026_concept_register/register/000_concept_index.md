...
| RSH-008 | `agenterrors`: the ONE `agent_error_log` writer, below the import cycle that forced 19 copies | deployed (LIVE v1.0.1262, pod-proven both replicas) | Writer moved to a LEAF package both sides of `orchestration -> actions` can import; 19 hand-copied INSERTs (8/9/10/11/13 cols, `orchestration_id` absent from nine) converted onto it. LANDMINE: provenance must be set EXPLICITLY or the merge inherits the running step's. | resilience-self-heal.md |
