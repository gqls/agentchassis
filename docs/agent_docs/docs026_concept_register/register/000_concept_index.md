...
| LCO-008 | `CacheBreakpointMarker`: opt-in Anthropic prompt caching on the shared AI client | built, unrolled | Sentinel in a prompt template splits it into a cached shared prefix + per-call suffix; absent the marker the request is byte-identical, so the seam's other ~40 callers are unaffected. ⚠ `input_tokens` becomes the UNCACHED REMAINDER once any caller opts in | llm-call-observability.md |
