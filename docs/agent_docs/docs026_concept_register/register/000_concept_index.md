...
| WFA-005 | `output_format` is a live step contract + one corrective re-ask on unparsable JSON | built (inert until roll) | The key 100 live LLM steps write was read by nothing; now it selects the JSON instructions AND buys a single corrective re-ask when the answer will not parse. 32 agents inherit it. | workflow-authoring.md |
