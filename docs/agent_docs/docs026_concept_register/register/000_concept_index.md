...
| WDS-015 | `triage_detected_items` reports the SITE's dispatchable state beside its call-scoped `has_items` | built | Three live agents run the same site-wide promoter; the copy the improvement loop branched on always ran last, so it read `promoted: 0` and terminated on "site is clean" | work-dispatch.md |
