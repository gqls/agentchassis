...
| WFA-013 | Optional-key BUDGET census (`config-key-audit --optional-key-budget`, `scripts/audit-optional-key-budget.sh`) | built + tested 2026-08-13; report-only until the owner rules a budget N | RFC_022's counter: `len(spec.Optional)` per action joined against DISTINCT live carriers (WalkSteps); `over_budget` fires only on SHARED (≥2 carriers) actions past N. Live census: 118 actions declare optional keys, 21 shared; top = analyse_repo_local 12, append_doc_note 11 (8 carriers). Wrapper reads refusal from EMPTY stdout, never exit code (`go run` folds exit 2 into 1) | workflow-authoring.md |
