...
| VIZ-013 | write_render_audit_findings: the render audit's drain | built + tested; inert until image roll + config tail (A0.3b) | Files firm contrast → contrast_failure/css-patch-agent and attributed broken images → undeployed_asset/asset-deployer (co-dedup); born detected so 286's single promoter drains them. over_image/overflow/unattributed/locked counted loudly, not filed. Closes bugs_open/122 candidate 2 | visualisation-and-charts.md |
| VIZ-014 | Legible-ink companions: the renderer names BOTH directions of readable | code committed, NOT live | `--color-<x>-text` = the ink ON an x fill (accent_text, derived since 07-27, declared by 0 of 18 layouts, never emitted); `--color-<x>-ink` = x ITSELF made legible as an ink (new). Step 12 of RenderCSSFromSpecAction, after the token aliases. grounds is a SLICE and scheme-independent — both deliberate, both test-pinned | visualisation-and-charts.md |
