...
| PLAN-048 | Twin-identity reconciliation: layered matching in the reconciler + a writer guard that stops the identity being re-derived | LIVE v1.0.1288, artefact-verified, council APPROVED; enabled on NO site — all three switches default OFF and dark-launch while off | Prevents a re-plan minting a second pages row for a page already realised under another spelling; 71 live rows are not fixed points of CanonicalisePage, so the reconciler alone cannot hold — both write surfaces must honour the realised identity. Deliberately refuses both-deployed pairs (owner decision) | site-plan-and-reconciler.md |
...
| CLM-022 | The register reaches the tools that ENCODE its facts: fence-level `facts` + the daily sweep's per-tool fan-out | built 2026-08-16, guards induced red; Go inert until roll, no live declaration yet | A criteria fence names WHICH register facts its tool encodes (ids only, never values); when one moves the daily evidence sweep files improve_tool (fork + no no_auto_fix) or the new fact_drift_review (everything else, incl. every evidence-only drift). bugs_open/225's class: the register guarded copy, not code | claims-verification.md |
