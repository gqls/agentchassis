...
| IMP-053 | site_unreachable discovery check + availability rotation driver (does the site SERVE?) | built | Probes https://<domain>/ as a visitor would and files/self-clears a high-severity alert item when a live site does not serve (bugs_open/236 522 half — lendzy served 522 indefinitely with every internal signal green); 4-hourly rotation driver held as mig 368_HOLD until the image rolls | improvement-loop.md |
