...
| PLAN-048 | Twin-identity reconciliation: layered matching in the reconciler + a writer guard that stops the identity being re-derived | committed in full; ALL THREE site switches default OFF (all three layers dark-launch and count while off) | Prevents a re-plan minting a second pages row for a page already realised under another spelling; 71 live rows are not fixed points of CanonicalisePage, so the reconciler alone cannot hold — both write surfaces must honour the realised identity. Deliberately refuses both-deployed pairs (owner decision) | site-plan-and-reconciler.md |
