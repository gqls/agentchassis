...
| PBP-039 | Plan-time structural-key carry: a regeneration falls back to the page's own deployed content_data for a non-llm field whose source resolves nothing | built+committed 2026-08-11, INERT till the chassis rolls; Go only, no migration/seed/config; council submitted alongside the commit | A tone_shift regeneration dropped all 11 resolver-sourced URL keys from a live homepage section — five `<img src="">` plus five card links and a CTA that VANISHED (the gated ones fail more quietly than the ungated). The LLM never had those keys: they are `site_assets.*`/`site_specs.*` sourced, they resolved to nothing on that site, `on_missing` defaulted to skip_field, and save_page_sections REPLACES while rerender_page_sections MERGES — which is why the values had survived every re-render since May and died on the first true regeneration. planSection now carries them from the deployed row (lazy preload, live resolution always wins, llm fields never carried, empty never carried), and a required field resolving NOWHERE writes a durable STRUCTURAL_KEY_CARRY_MISS row. bugs_open/238. | page-build-pipeline.md |
