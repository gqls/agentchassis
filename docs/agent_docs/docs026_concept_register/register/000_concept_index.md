...
| FIX-059 | Seat length budget applier (one block, snapshot-then-write, refuses hand-authored blocks) | deployed, effect measured | Took review_editquality from 98.3% to 55.0% of an unchanged cap — the budget alone, separated from the cap raise it shipped with the first time | fix-loop.md |
