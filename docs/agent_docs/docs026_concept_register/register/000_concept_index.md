...
| CLC-014 | Per-instance component scoping (`{{.InstanceID}}`) and the reuse-safety detector `DetectInstanceCollisions` | built 2026-08-15, wired on all three render paths, NOT yet exercised by any live template | A per-INSTANCE template value distinct from the per-COMPONENT `{{.ComponentID}}`, so a component carrying element ids and scripts can appear twice on one page; token is `c<position>` because position is unique per page and stable across re-renders, where slot_name is neither. Plus a checker for the three reasons repetition breaks: duplicate element ids, multiple `window.onload` assignments, and inline scripts declaring into global scope. ⚠ `{{.ComponentID}}` resolves to the shared component row id on two of the three paths and CANNOT be used for this | component-lifecycle.md |
