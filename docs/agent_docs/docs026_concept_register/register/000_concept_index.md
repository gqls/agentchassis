...
| VIZ-014 | Legible-ink companions: the renderer names BOTH directions of readable | LIVE + config applied (migs 338/368) + 10/10 closures measured 08-10 | `--color-<x>-text` = the ink ON an x fill (accent_text, derived since 07-27, declared by 0 of 18 layouts, never emitted); `--color-<x>-ink` = x ITSELF made legible as an ink (new). Step 12 of RenderCSSFromSpecAction, after the token aliases. grounds is a SLICE and scheme-independent — both deliberate, both test-pinned | visualisation-and-charts.md |
