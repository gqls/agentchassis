...
| CQ-021 | ScanDeployedClaims — claims audit shared by check and revalidator | built, inert until roll | claims_unverified items can now be auto-retracted; TWO producers on one item_type; the evidence_base register is DATA, so a close does not prove the copy changed | content-quality.md |
