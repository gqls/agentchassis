...
| CQ-021 | ScanDeployedClaims — claims audit shared by check and revalidator | built, inert until roll; council REVISE r1 | claims_unverified items can now be auto-retracted; ONE producer (entry first said TWO — corrected); the evidence_base register is DATA, so a close does not prove the copy changed | content-quality.md |
