...
| CQ-021 | ScanDeployedClaims — claims audit shared by check and revalidator | **LIVE + PROVEN**; both gates live on v1.0.1291 (2026-08-12, negative-control verified); **council APPROVED r7** after 6 REVISE | claims_unverified items can now be auto-retracted; ONE producer (entry first said TWO — corrected); a close requires BOTH that the page moved AND that every cited text has gone from its own slot — the seat's own snippet-based suggestion was measured (18/41 vs 40/41) and rejected as failing OPEN | content-quality.md |
