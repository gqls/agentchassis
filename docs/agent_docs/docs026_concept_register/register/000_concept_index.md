...
| BLD-019 | Build provenance: commit sha stamped into every backend binary (`pkg/buildinfo`) and image (OCI `revision` label) | built 2026-08-10, **inert until the fleet rolls**; **council round 1 REJECTED — guardian veto on SCOPE, not soundness (corr 44fa6a98); seats disagreed, so an owner decision is owed, NOT a resubmission**. Not ratified — read bugs_open/153's status block | `ref_build` computed the commit at makefile:119 only to echo and discard it; now it reaches the binary via `-ldflags` and the image via OCI labels, and `verify-agent-images` reads both back. Retires per-fix marker hunting, which produced three false "did my fix ship?" readings in one day (bugs_open/153). LANDMINE: `-X` on an unlinked package is SILENTLY ignored, so each service's one-line main.go import is load-bearing; and a `-tree` build is stamped `<sha>-tree` on purpose so WIP can never wear a clean sha. | build-pipeline.md |
