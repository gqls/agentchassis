...
| IMP-051 | asset_reference_404 discovery check (a subresource reference is probed, not merely counted) | live-not-yet-observed | A discovery check that resolves every `<script src>` and `<link rel~=stylesheet>` on a site's deployed page... | improvement-loop.md |
