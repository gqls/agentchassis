...
- **verify-later:** whether a component PLAN is ever actually written and read by a gate — the DOC-067 test applies here too, *present capability is not use*. Concretely: `SELECT subject_key FROM doc_plans WHERE subject_type='component'` returning a row, and an S6 run citing its fence. **STILL OPEN as of 2026-07-31 evening and deliberately so** — the probe wrote such a row, watched a gate read it, and then deleted it, which settles the *mechanism* and leaves this line exactly where it was. Do not read the runtime evidence above as closing it.
