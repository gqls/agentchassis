...
- **status:** built, tested against real data, live-verified against the real GitHub/Postgres endpoints. **Not yet deployed** — `make deploy-bugs-open-staleness-sweep` has not been run; paused for an explicit go-ahead before the first new K8s compute resource this session has created (Part A / DOC-069 only ever touched DB rows).
- **status-evidence:** a local test harness (`scripts/test-bugs-open-staleness-sweep-local.py`, swaps every GitHub API call for the equivalent `git` command against this working tree — no network, no credentials) ran the real logic against all 65 real `bugs_open/*.md` files: 501 citations extracted, 74 likely-stale + 36 ambiguous + 0 errors. Separately, live-verified the actual GitHub/Postgres integration with the real production secret: `GET .../commits/087_towards_multiple_domains` → 200; `GET .../contents/bugs_open/155_....md?ref=...` → 200; `GET .../git/trees/...?recursive=1` → untruncated, 9,330 entries, exactly 65 `bugs_open/*.md` paths (matches the local count exactly). `postgres-clients` confirmed as a live `ClusterIP` Service in `ai-persona-system`; `CLIENTS_DB_PASSWORD` and `GITHUB_READ_TOKEN` confirmed present on the live `personae-platform-secrets` secret — no secretKeyRef in the manifest is a guess carried over from another service's overlay. `kubectl kustomize` + `kubectl apply --dry-run=client` both pass on the rendered manifest.
