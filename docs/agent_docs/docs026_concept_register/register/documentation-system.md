...
- **status:** **DB half LIVE; Go gate still UNEXERCISED** (corrected 2026-07-31 — this line previously read *"submitted, NOT live … migration 273 deliberately NOT applied"*, which went stale the same day). Council **APPROVED** (correlation `e5673868-7c5b-489c-931a-7ba59b959b91`, r1 REVISE → r2 approved, 11 approve / 3 advisory). **Migration 273 was applied by hand 2026-07-31** (`psql -f` + `--record-only`, per 270's precedent, because the runner's `--apply` takes every pending file and several are unsafe); both CHECKs now allow `component` and `doc_notes` kept `landmine`. The Go half is in the running binary **by build date only** — pods built 2026-07-31 08:49:09 UTC, after commit `c659e312b` (07-30 19:28 UTC). **Necessary, not sufficient:** `doc_plans` holds **0** component rows, so nothing has ever passed through `docResolveSubject` for this type and `docSubjectGateReason` has never been observed accepting it. Until that is watched, "component travelling docs work" is a claim about the database alone.
- **status-evidence:** `TestValidDocSubjectTypes_LockstepWithMigrationCheck` passes with both halves and **FAILS with the Go half alone** (`split contract: validDocSubjectTypes = [action component experience experience-pattern pipeline tool] but 218_experience_register_substrate.sql sets the CHECK to [...]`) — mutation-proven 2026-07-30, so the build gate demonstrably catches migration 184's failure mode. `platform/...` and `internal/...` build clean; the actions package tests pass.
...
- **verify-later:** whether a component PLAN is ever actually written and read by a gate — the DOC-067 test applies here too, *present capability is not use*. Concretely: `SELECT subject_key FROM doc_plans WHERE subject_type='component'` returning a row, and an S6 run citing its fence.

### DOC-069 — `landmine-verifier`: single-pass fact-check agent for one LANDMINES.md entry
- **status:** built, DB-applied, **NOT yet exercised end to end live** — first real run got through 4 of 5 steps and failed on a dependency (DOC-068's `'landmine'` gap, above), which is fixed in the working tree but not yet in a rolled image.
- **status-evidence:** `agent_definitions` row present (`type='landmine-verifier'`, version 1); live test run (correlation `846f63d9-52cb-4bef-bb7a-8cd5ac48b4a6`) reached `load_entry` → `derive_checks` → `run_checks` → `verify` before failing at `persist_verdict` on the pre-existing `append_doc_note` gap. Re-run owed after the next chassis roll.
...
- **sources:** `architecture_review/RFC_005_targeted_review_for_docs_that_feed_the_fleet.md` (owner ruling 2026-07-31); `docs024_key_docs_latest/doc_review_mechanisms/PLAN_2026-07-31_doc_review_mechanisms.md`; `sql_for_agents/276_landmine_verifier.sql`
...
- **verify-later:** end-to-end pass after the next chassis roll; whether the trigger ever gets wired to new/changed entries automatically (currently manual, per RFC_005 §3.2's own open question) rather than by hand per entry; whether the weekly `bugs_open/` staleness sweep (RFC_005 §3.3, not yet built) ends up sharing `diagnose_code_lookup` the same way
