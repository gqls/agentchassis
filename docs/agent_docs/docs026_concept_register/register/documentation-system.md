...
- **stage2-verified (2026-07-14):** abandoned → partial — find -iname '*deploy_from_context_packs*' finds it live (non-archived) at docs/agent_docs/docs024_key_docs_latest/adoption/docubundle/GUIDE_deploy_from_context_packs.md (git log: commit d183079dd), byte-identical (diff empty) to the archived docubundle_idea_golive copy cited as evidence of abandonment. Stage-1's sea...
...
### DOC-067 — LANDMINES.md + the footprint sync + the SessionStart hook (the incident→landmine→automation middle rung)
- **status:** deployed and exercised (2026-07-29) — 14 entries, 27 `doc_notes` rows, hook live in `.claude/settings.json`
- **status-evidence:** `./scripts/landmines-sync.py --check` reports in sync; `SELECT count(*) FROM doc_notes WHERE categories ? 'landmine'` = 34 (27 synced + 7 pre-existing); the hook pipe-tested with the harness payload returns valid JSON and matched `cmd/` against a dirty `cmd/config-key-audit/main.go`; a forced parser exception exits 0 with empty stdout. Adoption is real and not just mine: of the 14 entries, 6 were appended by other threads within hours.
- **what:** The rung between `WRONG_CALLS.md` (the incident, retrospective) and a lint or council seat (full automation). A **landmine** is a trap that fires when you TOUCH a file/table/command, where the wrong result looks like the right one — read *before* you have a symptom, unlike `016b` §9 which you reach *with* one. Three parts: (1) `docs024_key_docs_latest/LANDMINES.md`, fleet-wide append-only, every entry carrying an explicit **footprint**; (2) `scripts/landmines-sync.py`, one-way markdown→`doc_notes` (`subject_type='landmine'`, `subject_key`=footprint, `source`=`LANDMINES.md#<slug>`), owning only its own rows so other threads' are structurally unreachable; (3) `scripts/landmines-session-start.py`, a `SessionStart` hook injecting entries whose footprint matches a file already dirty in the tree. `scripts/landmines_lib.py` is the single shared parser so the two consumers cannot drift.
- **the landmine this itself carries:** **query `categories ? 'landmine'`, NEVER `subject_type='landmine'`** — the 7 rows written 2026-07-27/28 by other threads are `subject_type='action'` because `doc_notes_subject_type_check` did not permit anything else until migration 270. The category spans both conventions; the subject_type does not. Also: the markdown is authoritative, so **hand-writing a landmine row is pointless** — the next sync replaces it.
- **delivery, all three routes live:** SessionStart hook (path-shaped footprints, working-tree match); **council seats via migration 271** — `load_schema_hint` is one `query_database` step whose `schema_hint` output is already in all 17 seats' `input_fields`, so one query change reaches every seat without touching a seat config (which `099_SYNC_gate_roster.py` owns and CLAUDE.md forbids hand-patching); and grep for table/command/symbol footprints, which cannot match a path.
- **the open review question:** **does a seat ever USE it?** The corpus is now reachable and visible, which is not the same as consulted — the proposal's own test was *"present text is not use"* and it is still unpassed. Also **unshipped by choice:** relevance-gating the index (proven read-only, 16 landmines → 2 on a real submission, both on point) — `load_schema_hint` has no `error_step`, so a param that failed to resolve would take every council submission with it. Needs a `subject_key` length floor of ≥6 (`Bash`, `cmd/` are 4 and would match prose). **Size is the live risk:** +5,558 bytes × ~17 seat prompts today, growing with every entry.
- **sources:** `architecture_review/PROPOSAL_D9_landmines_as_a_footprinted_corpus.md` (open as D10, ruled 2026-07-29); `architecture_review/DECISIONS_open_for_owner_2026-07-26_architecture_seat.md` §D10; `sql_for_agents/270_doc_notes_landmine_subject_type.sql`; CLAUDE.md routing block (five destinations)
- **relations:** `WRONG_CALLS.md` (rung 1); `016b` §9 (symptom-first failure patterns); `scripts/pattern-check.py` `shared-ledger-not-appended` (rung 3, guards both ledgers); `doc_notes` travelling PLAN/NOTES convention (DOC-005 family)
- **verify-later:** whether any seat's `checks` array ever cites a landmine row — the proposal's own test was *"present text is not use"*, and that test is not yet passed

### DOC-068 — `subject_type='component'`: travelling docs for a section component (the ladder's substrate)
- **status:** **BOTH HALVES LIVE AND OBSERVED — capability proven, not yet USED** (superseded 2026-07-31 evening; the two earlier wordings are kept below because the *sequence* of what we could prove is the useful part). Council **APPROVED** (correlation `e5673868-7c5b-489c-931a-7ba59b959b91`, r1 REVISE → r2 approved, 11 approve / 3 advisory). **Migration 273 was applied by hand 2026-07-31** (`psql -f` + `--record-only`, per 270's precedent, because the runner's `--apply` takes every pending file and several are unsafe); both CHECKs now allow `component` and `doc_notes` kept `landmine`. **The Go half is now watched rather than inferred:** the running pod printed its own vocabulary — see status-evidence. **Still NOT a claim that anything uses it:** `doc_plans` is back to **0** component rows (the probe PLAN was written and deleted in the same session), so no real component contract exists yet. "The gate accepts component" and "components have travelling docs" are different statements; only the first is proven.
  - **superseded wording 1** (2026-07-30): *"submitted, NOT live … migration 273 deliberately NOT applied"* — went stale the same day.
  - **superseded wording 2** (2026-07-31 midday): *"DB half LIVE; Go gate still UNEXERCISED … in the running binary by build date only — pods built 2026-07-31 08:49:09 UTC, after commit `c659e312b`. Necessary, not sufficient."* That wording was correct and deliberately conservative for 10 hours, and it is what prompted the probe. **A build date argues; it does not observe.**
- **status-evidence:** **RUNTIME, 2026-07-31 evening — the pod printed its own list.** Probe correlation `8f564028-6fc6-488c-96d2-c2e362b243b2`, pods `v1.0.1215` (both replicas), `COMPLETED`: a `load_doc_context` step with `subject_type='component'` returned `has_plan=true`, `length(plan_body)=827` (byte-identical to the generated body) and a non-empty `criteria_json` — so `docResolveSubject` accepted the type, the body travelled back through the Go action, and `extractCriteriaBlock` found the fence. **The control arm is the load-bearing half:** a second step with a deliberately invalid type errored, and its message is `docSubjectTypesQuoted()`'s runtime rendering of `validDocSubjectTypes` **as compiled into the running pod** — `subject_type must be one of 'tool', 'pipeline', 'experience', 'action', 'experience-pattern', 'component', 'landmine'`. That control is what makes the green arm mean anything (it proves the route could fail) and it incidentally corroborates DOC-067/DOC-069's `'landmine'` fix being live on the same image. Instrument + procedure: DOC-070; `staged_component_build/RUNBOOK` §12; NOTES 2026-07-31 evening carries the verbatim read-out, because `orchestration_states` is retention-clocked. **Scope of the claim:** what was watched is the shared list via **one** of its two callers; `persist_diagnosis_note` was not dispatched, and the open review question below is untouched. Build-time evidence, unchanged: `TestValidDocSubjectTypes_LockstepWithMigrationCheck` passes with both halves and **FAILS with the Go half alone** (`split contract: validDocSubjectTypes = [action component experience experience-pattern pipeline tool] but 218_experience_register_substrate.sql sets the CHECK to [...]`) — mutation-proven 2026-07-30, so the build gate demonstrably catches migration 184's failure mode. `platform/...` and `internal/...` build clean; the actions package tests pass.
- **what:** Extends the travelling-docs substrate (DOC-003/TL-017) from tools/pipelines/experiences/actions/patterns to **section components**, so a component can carry a PLAN with its ```criteria fence and append-only NOTES with its repairs — the substrate `features_open/027`'s S0–S7 stage ladder reads and writes. Keyed by `content_components.function`. Two enforcement points move together: `validDocSubjectTypes` (`platform/orchestration/actions/doc_subjects_common.go`, the single Go gate for `write_doc_plan` / `append_doc_note` / `load_doc_context` / `persist_diagnosis_note`) and both DB CHECKs (migration 273).
- **the design decision worth reusing:** **the PLAN is the fleet-wide contract; the NOTES are the per-site verdicts** — and the schema already encoded it. `doc_notes` has `site_id`, `doc_plans` does not. A component's template is fleet-shared (one `content_components` row serves 11 sites for `info-card-grid`) so a site-less PLAN is *correct*; S4–S7 are per-site, per-page facts and land in NOTES. **Do not add `site_id` to `doc_plans` to "fix" this** — the asymmetry is the design.
- **the landmine this itself carries:** **the two tables do NOT share a vocabulary — `doc_notes` also allows `'landmine'` (migration 270) and `doc_plans` does not.** Re-adding `doc_notes`' constraint from `doc_plans`' array — the obvious way to make them agree — drops it and orphans the live landmine corpus (57 rows measured 2026-07-30, owned by other threads, synced from `LANDMINES.md`). Migration 273 refuses to run if `'landmine'` is absent, for exactly this reason. Second landmine: **subject keys are immutable** — `RekeyTravellingDocs` is wired only to `rename_tool_identity`, so a renamed component `function` orphans its docs **silently**, and `bugs_open/136` is a live half-landed `*_domain`→`*_pipeline` rename of precisely that shape.
  **CORRECTED 2026-07-31 — this bullet flagged the risk but understated it: `validDocSubjectTypes` was not merely at risk of losing `'landmine'`, it had NEVER HAD it.** Found live (not by inspection) when `landmine-verifier` (DOC-069)'s first real run failed at `append_doc_note` with the DB already accepting `'landmine'` on `doc_notes` and the Go gate rejecting it — the exact split this whole entry describes, two days old by then. Fixed commit `7290433f2`: added `'landmine'` to `validDocSubjectTypes`, and rewrote `TestValidDocSubjectTypes_LockstepWithMigrationCheck` to check the UNION of `doc_plans_subject_type_check` and `doc_notes_subject_type_check` rather than `doc_plans` alone — the old test's blind spot was exactly why this shipped undetected (migration 270 correctly never touches `doc_plans`, so a test watching only `doc_plans` had nothing to see). `go build`/`vet`/`test` clean; council-submitted (`cb405b7b-c32a-463c-9687-ee3b52acc2fb`); **inert in the running fleet until the next chassis image roll** — do not read this correction as "fixed live" until that's confirmed separately.
- **the open review question:** **is policy strong enough for the rename path?** Immutability is recorded in a Go comment and enforced by nothing; the submission names this as an objection it would accept, with the alternative being enforcement in the write path. Also open: `persist_diagnosis_note` now accepts component subjects as a *consequence* of single-sourcing — judged correct (a diagnosis about a component is what a note trail is for, the same argument the checklist made for `action`) and **decided rather than inherited**, per `subject_type_addition.md` item 3, but no test asserts that branch specifically. And the ordering: migration-before-image recreates 184's split, mitigated only by the fact that **no producer of component docs exists yet**, so an early apply is inert rather than broken.
- **sources:** `docs024_key_docs_latest/staged_component_build/` (PROPOSAL/PLAN/RUNBOOK/NOTES, lane adopted 2026-07-30); `features_open/027`; `sql_for_agents/273_doc_subjects_component.sql`; `experience_register/design/subject_type_addition.md` (the four-enforcement-point checklist this followed); `bugs_open/064` (what happens when only the DB half moves)
- **relations:** DOC-003 (per-tool travelling docs); TL-017 (criteria live in the PLAN); DOC-067 (`'landmine'`, migration 270 — the constraint this must not narrow); CLC-012 (`teaser-reveal-panel`, the first intended subject); `features_open/026` (the render vantage point S5/S6 need)
- **verify-later:** whether a component PLAN is ever actually written and read by a gate — the DOC-067 test applies here too, *present capability is not use*. Concretely: `SELECT subject_key FROM doc_plans WHERE subject_type='component'` returning a row, and an S6 run citing its fence. **PARTIALLY CLOSED 2026-08-02, and the split is the point.** `teaser-reveal-panel` now has a real, persisted row (`staged_component_build` lane; not deleted, unlike the 2026-07-31 probe) — a 12-check criteria fence, mutation-proven (12/12 mutants caught, every check watched red under its own mutant, not just watched to pass), read back OUT of the DB and re-run byte-identical to what was written. **That closes "written and read" for the client-side evaluator** (`try_fence.go`, which calls the platform's own `RunChecksAction` directly). **It does NOT close "an S6 run citing its fence"**: that fence has not been dispatched through `browser-runner-adapter` in the cluster, because `request_browser_run` (`platform/orchestration/actions/tool_acceptance_actions.go`) resolves its target page by `pages.name`, one function to exactly one page — an assumption true for a tool and false for this component, which is placed on 5 pages across 2 sites. Wiring that dispatch is a separate, still-undecided design choice (extend `request_browser_run` with an opt-in page/site override, or a sibling action) — see `staged_component_build/HANDOFF_2026-07-31c` §4 and its NOTES. So: the PLAN half of this verify-later line is answered; the S6-dispatch half remains open, now for a named and understood reason rather than an unexplored one.
  > **CLOSED 2026-08-02 — see DOC-072.** The design choice is decided (sibling action), built, shipped on `v1.0.1231`, and dispatched for real: `teaser-reveal-panel`'s fence ran through `browser-runner-adapter` in the cluster (correlation `e6a258eb-6ba1-44df-b344-16e42443975f`), 15/15 passed, plus a negative control on the new placement-resolution logic that failed exactly as designed. "An S6 run citing its fence" — this entry's own verify-later — is now a fact, not an open question. What is NOT re-proven by this run: DOC-072 itself explains why re-running the checks' own mutation proof through the cluster was judged unnecessary.

### DOC-069 — `landmine-verifier`: single-pass fact-check agent for one LANDMINES.md entry, with automatic dispatch
- **status:** **built, DB-applied, auto-dispatch wired — and the "CONFIRMED end to end" claim below was FALSIFIED by `bugs_open/163` (2026-07-31→08-03): the pipeline ran end to end, but its `symbol` checks could never resolve.** The lookup's clause builder AND-ed every token of the `path:Symbol` form — the very form the `derive_checks` prompt defines — against the `symbol` column, which never holds a path, so every path-bearing symbol check returned 0 rows by construction. Measured 2026-08-03, pre-fix: 21 verdicts all-history — 16 `NEEDS_HUMAN_REVIEW`, 4 `STILL_VALID` (each via `content`/`ls`/directory checks), 1 `STALE`; **no path-bearing SYMBOL check ever returned a row** (163's fleet measurement: 23 of 23 path-bearing → 0). The two live runs cited below were real but did not exercise a resolving symbol check (run 1 confirmed via `content`/`ls`; run 2 was `NEEDS_HUMAN_REVIEW`). > **CORRECTED 2026-08-03, same session:** an earlier wording of this banner said *"0 of 20 verdicts ever CONFIRMED"* — measured with `body LIKE '%CONFIRMED%'`, **a string this verdict vocabulary never emits** (`STILL_VALID`/`STALE`/`NEEDS_HUMAN_REVIEW`), so that zero could never have been non-zero. Caught by reading the first post-fix verdict rather than the counter. The check-level claim was and is correct; the verdict-level one overstated. **Fixed in Go 2026-08-03** (`bugfix_163_symbol_lookup` lane, commit `c3b02f035`: path half → `path` column, name half → token-AND on `symbol`, `:LINE` suffix degrades to a path check, a path-qualified miss discloses where the bare name resolves) — the fix is in `answerCodeCheck`, so it reaches this agent, the council verify tier and the diagnosis loop identically. Status is LIVE only when a post-roll verifier run mechanically CONFIRMs a symbol; see the 163 lane's close-out. **The lesson for this register: "CONFIRMED end to end" was written after runs whose confirming arm could not fire — a pass from a blind check outlives the blindness.** RFC_005 §3.2 status otherwise unchanged; §3.3 is separate.
- **status-evidence:** two live runs, both against genuinely real (not staged) data. (1) correlation `11ef1686-a28b-4665-8865-0f7e239e640f`, re-run of the original test entry after the `v1.0.1215` roll: COMPLETED, verdict `STILL_VALID` citing the exact unchanged code (`resolveStorageURIFromAsset`, `StoreAssetAction`, `updateContentDataField`) at commit `d98010e8`. (2) correlation `2ca82ffe-bd92-472b-958f-3391f7cbafa6`, auto-dispatched by the new `scripts/landmines-verify-dispatch.sh` against a genuinely new entry another thread appended to `LANDMINES.md` **mid-session, with no manual identification of which entry needed it**: verdict `NEEDS_HUMAN_REVIEW` — correctly declined to guess, because the code-lookup index is stale (last indexed 2026-07-28) relative to both the relevant fix commit (`71680ad513`, 2026-07-30) and current HEAD. Both verdicts are queryable: `SELECT subject_key, body FROM doc_notes WHERE categories ? 'landmine-verification';`.
- **what:** Owner-ratified `architecture_review/RFC_005` §3.2 — a NARROW alternative to routing `LANDMINES.md` review through the full multi-seat council (wrong tool for a fact-check task; would dilute the architecture seat's signal). One entry per invocation: loads its `doc_notes` body (`categories ? 'landmine'`), an LLM pass derives `code_checks` from the `footprint` field, `diagnose_code_lookup` (the SAME action the diagnose loop uses for its own evidence citations — reused, not reimplemented) runs them fresh, a second LLM pass judges STILL_VALID / STALE / NEEDS_HUMAN_REVIEW, and `append_doc_note` persists the verdict as a NEW `doc_notes` row (`categories: landmine-verification`) — it never edits `LANDMINES.md` itself, which stays human-curated and append-only. Composed entirely of existing actions; no new Go code for the agent.
- **the dispatch mechanism:** `scripts/landmines-sync.py` gained `existing_bodies()` (a `jsonb_object_agg` fetch, deliberately NOT the line-split `-t -A` text `existing_sources()` uses — bodies are genuine multi-line prose and a naive line-split would silently truncate every multi-line body to its first line) and a `changed` list alongside the existing `new`/`gone`, printed as machine-parseable `NEEDS_VERIFICATION:<source>` lines after `--apply`. `scripts/landmines-verify-dispatch.sh` wraps the sync and fires `scripts/trigger-landmine-verifier.sh` once per new-or-changed entry (2s apart, to avoid bursting `kubectl run` pods into the `kafka` namespace). Unchanged entries are never re-verified by this path — that is the staleness sweep's job (§3.3), not this one's.
- **a genuine second bug found writing the dispatcher, not hypothetically:** the same first test run that surfaced two entries flagged a **slug collision** — `slugify()` hashes on TITLE alone, so two entries with an identical title collapse to one dict key in `landmines-sync.py`'s `want`, and the earlier one is silently dropped from every sync, forever, with no error anywhere. Confirmed live: a duplicate `snapshot_agent` heading (identical title, two line numbers) was doing exactly this. Added a collision detector to `landmines-sync.py` as a permanent safety net; a different thread fixed the specific duplicate concurrently mid-session (`bcecb65e3`) while the detector was being written — the fix and the detector are independent, both needed.
- **known limitation, by design:** `the check` field is free text and sometimes names a shell/kubectl command (`strings /app/agent-chassis`, `grep -ac ...`) that no existing action can run from inside a workflow. Those entries fall back to LLM judgement on internal consistency only, and the verdict says so via `NEEDS_HUMAN_REVIEW` rather than guessing past what could actually be checked.
- **the landmine this itself found:** see DOC-068's correction above — the agent's first live run is what surfaced the `validDocSubjectTypes` gap, not a code review. A single-pass verifier for one class of drift (`LANDMINES.md` entries going stale) ended up demonstrating a second, unrelated class (a Go/DB vocabulary split) purely by being a new, real caller of `append_doc_note` with a value nothing had exercised that path with before.
- **sources:** `architecture_review/RFC_005_targeted_review_for_docs_that_feed_the_fleet.md` (owner ruling 2026-07-31); `docs024_key_docs_latest/doc_review_mechanisms/PLAN_2026-07-31_doc_review_mechanisms.md`; `sql_for_agents/276_landmine_verifier.sql`; `scripts/landmines-sync.py`, `scripts/landmines-verify-dispatch.sh`, `scripts/trigger-landmine-verifier.sh`
- **relations:** DOC-067 (LANDMINES.md + sync — the corpus this reads); DOC-068 (the `'landmine'` subject_type gap this run exposed); diagnosis-loop register (`diagnose_code_lookup`, reused here)
- **verify-later:** whether the weekly `bugs_open/` staleness sweep (RFC_005 §3.3, not yet built) ends up sharing `diagnose_code_lookup` the same way; whether the code-lookup index's freshness gap (found above, ~3 days stale as of 2026-07-31) is already tracked elsewhere or needs its own finding

### DOC-070 — `PROBE_doc_subject_go_gate.sh`: make the running binary print its own doc-subject vocabulary
- **status:** **built and used to a PASS**, 2026-07-31 (`staged_component_build/scripts/PROBE_doc_subject_go_gate.sh` + `gen_probe_plan_sql.py`). One real run: correlation `8f564028-6fc6-488c-96d2-c2e362b243b2` on `v1.0.1215`, closing DOC-068's Go half.
- **what:** The runtime half of `subject_type_addition.md`'s four-enforcement-point checklist, which until now had a build-time test and a DB query but **no way to observe the Go gate in production**. Fires two `load_doc_context` steps in ONE dispatch: the `subject_type` under test, then a deliberately invalid one. The invalid arm must error, and its message is `docSubjectTypesQuoted()`'s **runtime** join of `validDocSubjectTypes` — i.e. the vocabulary as compiled into the pod answering the request. Works for any doc-subject type, not just `component`.
- **the design decision worth reusing — the control arm is not redundancy, it is what makes the result mean anything.** A green probe alone cannot be told apart from a probe that never ran. So the script defines **three** verdicts, and names the third: PASS (test arm returned, control arm errored, read-out lists your type), FAIL (test arm errored — and its message names every type the binary *does* accept), and **VOID** (`current_step='complete'` with no `probe%` rows in `processing_history` — the override did not take and nothing was tested). Naming VOID is what stops it being filed as a pass. Same lesson as TL-036's mutation prover, applied to a gate instead of a fence.
- **the second decision worth reusing — the test travels in the message, so there is nothing to install and nothing to clean up.** `selectWorkflow`'s Priority 1 (`platform/messaging/processor.go:922-928`) takes an inline `config.workflow` ahead of any DB lookup, so **no `agent_definitions` row is written** — no snapshot, no deactivation, no scratch row for a later session to trip over. The payload names an `agent_type` that deliberately does not exist, so a fallthrough lands on the pod's own `generic` definition, whose whole workflow is a no-op `complete` step: a misfire is **inert as well as visible**. Prefer this to seeding a scratch agent whenever a probe only needs local actions.
- **why a grep or a build date cannot substitute:** the vocabulary entries are short string literals, which Go compiles to immediate comparisons that never reach rodata — so `grep -ac` on the binary is nonzero for words like `component` on a binary that does *not* support the type. The runtime-built error string is the only readable surface. Full trap: `LANDMINES.md`, "Neither a grep nor a build date can prove a doc `subject_type` reached the Go gate".
- **the landmine it exposed on its way in:** **two Go gates over one shared list, with different messages** — `docResolveSubject` (`write_doc_plan` / `append_doc_note` / `load_doc_context`) says `subject_type must be one of …`; `docSubjectGateReason` (`persist_diagnosis_note` only) says `unsupported subject_type "…"`. Two handoffs told the next session to expect the second from a route that can only produce the first, a string absent on both pass and fail (`WRONG_CALLS.md`, 2026-07-31). **Say which gate you watched.** Membership carries to both because the slice is single-sourced, but only one was dispatched.
- **sources:** `docs024_key_docs_latest/staged_component_build/RUNBOOK` §12 (procedure + gotchas), NOTES 2026-07-31 evening (verbatim read-out — `orchestration_states` is retention-clocked, so it is pasted, not linked), `HANDOFF_2026-07-31b` §3 (the open action it closed); `experience_register/design/subject_type_addition.md` §6 (the checklist step it implements)
- **relations:** DOC-068 (what it proved); DOC-003/TL-017 (the substrate); DOC-067/DOC-069 (`'landmine'` — corroborated live by the same read-out, incidentally); TL-036 (same "watch every branch fail before quoting it" discipline, for fences); `bugs_closed/064` (the split-contract class this finally gives a runtime check)
- **verify-later:** whether the next `subject_type` addition actually runs it (the checklist now names it, but a named step is not a used one); whether it is worth extending to dispatch `persist_diagnosis_note` too, closing the second caller by observation rather than by shared-slice argument

### DOC-071 — `bugs-open-staleness-sweep`: weekly cheap-pass citation check over `bugs_open/*.md`, deliberately NOT a chassis agent
- **status:** **DEPLOYED AND CONFIRMED LIVE, 2026-07-31.** RFC_005 §3.3 is now fully done — both §3.2 (`landmine-verifier`, DOC-069) and §3.3 (this) are complete.
- **status-evidence:** pre-deploy — a local test harness (`scripts/test-bugs-open-staleness-sweep-local.py`, swaps every GitHub API call for the equivalent `git` command against this working tree — no network, no credentials) ran the real logic against all 65 real `bugs_open/*.md` files: 501 citations extracted, 74 likely-stale + 36 ambiguous + 0 errors. Separately, live-verified the actual GitHub/Postgres integration with the real production secret before deploying anything: `GET .../commits/087_towards_multiple_domains` → 200; `GET .../contents/bugs_open/155_....md?ref=...` → 200; `GET .../git/trees/...?recursive=1` → untruncated, 9,330 entries, exactly 65 `bugs_open/*.md` paths. **Post-deploy — a real scheduled-shape run, not just the dry-run:** `make deploy-bugs-open-staleness-sweep` created the CronJob cleanly; a manually-triggered Job (`bugs-open-staleness-sweep-manual-20260731-201728-k4755`) ran to `Succeeded` in ~70s against the REAL GitHub API (65 files, 448 citations, 93 flagged: 64 likely-stale/29 ambiguous/0 errors). Verified the write at the artefact, not the pod's `INSERT 0 1` log line: queried `doc_notes` directly and confirmed one real row (id `59e13f82-0d17-4c95-82a2-f95dd4734f3d`, `subject_type='pipeline'`, `subject_key='bugs-open-staleness'`, `body_len=17147`, `created_at 2026-07-31 19:18:41 UTC`).
- **what:** Owner-ratified `architecture_review/RFC_005` §3.3 — the "cheap first pass" staleness sweep over open bug files: extracts `path:line`/`path:line-range`/`path:Symbol` citations from each `bugs_open/*.md` file and checks whether the cited path/line/symbol still exists at a pinned ref. Never edits or closes a bug file — a flag means cited evidence moved or changed, not a verdict that the bug is fixed; the fixed-AND-live bar for closing stays a human/thread call. Writes ONE `doc_notes` row per run (`subject_type='pipeline'`, `subject_key='bugs-open-staleness'` — reuses the existing `pipeline` subject_type `diagnose_triage_action.go` already uses for its own sweep report, so this needed no migration and no `validDocSubjectTypes` change).
- **the decision this entry exists to explain — why NOT a chassis agent, contra this workstream's own PLAN sketch:** `PLAN_2026-07-31_doc_review_mechanisms.md` originally sketched Part B as "a CronJob-triggered agent, same family as build-pipeline-trigger" (i.e. `scheduled_tasks` + the live `kafka-scheduler` Deployment — confirmed live and real, not a dead mechanism, while researching this). That shape turned out to be blocked: there is no generic chassis action to list a repo directory or read an arbitrary non-`.go` file, and the one function that could — `ReadSymbolBody`'s whole-file fallback — was closed as a hazard the same day in `bugs_closed/145` (an unconditional `findFile` now gates every read on the analyser's Go-only `Output`). Building this as a chassis agent would have meant either reviving that exact hazard shape or adding new Go code to `diagnose_read_repo_files` and taking it through council review before it could run live. Presented both shapes to the owner; the owner chose script + K8s CronJob — zero new platform Go code, zero LLM cost (which is what RFC_005 §3.3's "cheap pass" actually asked for), ships without a council-gate wait.
- **the mechanism:** modelled directly on the live `database-backup` CronJob (`deployments/kustomize/services/database-backup/base/cronjob.yaml`): `postgres:16-alpine` (psql already present; `apk add python3` at runtime, the same pattern that image already uses for `mysql-client`/`aws-cli`), connects to Postgres **directly** (`PGPASSWORD` + `psql -h postgres-clients`) — never `kubectl exec`, because `ai-persona-app` has no `pods/exec` RBAC grant in `ai-persona-system` (checked; only `kafka`, for the unrelated topic-cleanup CronJob, has one). Repo content comes from the GitHub Contents/Trees API with the existing read-only `GITHUB_READ_TOKEN` — the exact same mechanism `diagnose_read_repo_files_action.go` already uses for reads, called directly via Python stdlib `urllib` instead of through the chassis action layer. `git-adapter` was checked and ruled out as an alternative: it is Kafka-only and write-only (commit/branch/PR), no read endpoint exists to bypass. One recursive tree fetch per run resolves even bare-filename citations (`agent.go:429`, no directory) by basename lookup against the whole repo tree.
- **a real bug the local test harness caught before any live deploy:** `resolve_path`'s exact-path branch originally returned `kind="exact"` even when the cited path did not exist (with `resolved=None`), but the caller only special-cased `kind == "not-found"` — a missing full-path citation fell through and called `fetch_raw(None, ref)`, which would have crashed the very first real run. Fixed by making `resolve_path` return a consistent three-way (`resolved` / `ambiguous` / `not-found`) where `resolved` is guaranteed non-`None` only for `resolved`.
- **a design correction made from reading real output, not guessed in advance:** the first cut lumped every flag into one flat list. "Ambiguous bare filename, matches 2+ paths" turned out to be the largest category (36 of 110 on the real corpus) and is a citation-quality signal, not a staleness signal — e.g. `registry.go:819` matches both `platform/orchestration/actions/registry.go` and `.../discovery_checks/registry.go`, and neither file moved. Split the report into three labelled sections (likely-stale / ambiguous / check-errors) so a reader's eye goes to genuine staleness first.
- **the landmine this avoided walking into:** see `bugs_closed/145`'s own entry — this sweep needed exactly the read capability that bug just closed off (arbitrary non-`.go` file content, unbounded), and building it as a chassis action extension would have reopened that exact class of hazard for a second, unrelated caller. Reading via the GitHub Contents API from outside the chassis action layer entirely sidesteps it rather than reopening it.
- **known limitation, by design:** only the cheap pass — no re-running of each bug's own "how to verify a fix" command against live HEAD/DB. RFC_005 §3.3 itself phases that as a later, sampled increment, not the initial build. Also: `SWEEP_REF` is pinned by hand in the CronJob manifest (currently `087_towards_multiple_domains`, the platform's actual live working branch, not `main` — see CLAUDE.md's own note that `main` is routinely hundreds of commits behind) and nobody yet owns updating it when the working branch changes; `sweep.py` refuses rather than silently falling back if it's unset or unresolvable, matching `090_TRIGGER_needs_diagnosis_v1.sh`'s own ref discipline, but the manual-bump cost itself is real and unowned.
- **sources:** `architecture_review/RFC_005_targeted_review_for_docs_that_feed_the_fleet.md` (owner ruling 2026-07-31); `docs024_key_docs_latest/doc_review_mechanisms/PLAN_2026-07-31_doc_review_mechanisms.md`; `deployments/kustomize/services/bugs-open-staleness-sweep/` (cronjob.yaml, sweep.py, kustomization + overlay); `scripts/test-bugs-open-staleness-sweep-local.py`; `bugs_closed/145` (the hazard this design avoids reopening)
- **relations:** DOC-069 (`landmine-verifier` — the sibling §3.2 mechanism, built as a chassis agent because its equivalent read need was Go-symbol-shaped, not arbitrary-file-shaped); `diagnose_triage_action.go` (the `pipeline` subject_type precedent this reuses); `database-backup` CronJob (the deployment pattern this copies); kafka-scheduler / `scheduled_tasks` (the mechanism this deliberately did NOT use, and why)
- **verify-later:** ~~whether the first live run's flag counts hold up once run against the real GitHub API on a schedule~~ **CONFIRMED 2026-08-03**: the CronJob's first real unattended Sunday firing (`2026-08-02 06:01:12 UTC`, not a manual trigger) produced `doc_notes` row `25603776-66d0-467c-9a35-443109ad52ad` (18,888 bytes) — 62 files, 516 citations, 97 flagged (64 stale/33 ambiguous/0 errors), consistent with the manual run's 65/448/93. Still open: whether anyone actually reads the weekly `doc_notes` row once this ships — the exact "unadopted ledger" risk this whole RFC was raised to avoid

### DOC-072 — `request_component_browser_run`: S6 dispatch for a section component (DOC-068's remaining half)
- **status:** **LIVE, PROVEN IN THE CLUSTER, AND COUNCIL-APPROVED.** Built, `go build`/`go vet`/`go test` clean; committed `f6bfb7e6e`; shipped on `v1.0.1231` (still live on `v1.0.1238` as of 2026-08-03), pod-verified both replicas (positive marker `request_component_browser_run` 6 hits + distinctive error-string marker 1 hit, both replicas; sanity negative-control string 0 hits, confirming the grep pipeline itself works); **dispatched for real**, correlation `e6a258eb-6ba1-44df-b344-16e42443975f`, `COMPLETED` in 31s. **Council verdict 2026-08-03: APPROVED** (`33d00513-2fd8-4872-ad5a-a19c24a1ae0b`, 11 reviewers, 3 advisory objections, none blocking, `gated_by_truncation: false`). One objection (`prior_art_librarian`, medium) was factually correct and is recorded below as a real design lesson, not dismissed; one (`guardian`, medium — "no test covers the extraction") was checked and found factually wrong — two existing Go tests already exercise the exact call path end to end.
- **status-evidence:** Real run against `teaser-reveal-panel`'s live placement: `browser_run.response.summary` = `{"passed":15,"failed":0,"skipped":9}`, all 9 skips read `"not run on profile mobile"` (the fence's own intentional gating, not the `"<type> not implemented"` defect class that reads as a false pass) — same 15/15 `try_fence.go` already proved offline, now proven reachable through the cluster dispatch path. `judge_acceptance_results` needed no change, confirmed live: `acceptance_verdict.all_passed = true`. **The control arm, in the SAME dispatch:** a `neg_control` step pointed `page_id_field` at a real, active page on the SAME site that does NOT carry this component (deliberately not a nonexistent UUID, which would only prove `sql.ErrNoRows` works) — it failed with the exact message the new code was written to produce: `request_component_browser_run: component "teaser-reveal-panel" is not placed on page fc505ab2-... (or that page is inactive)`, read from `collected_data->'__step_error'` per RUNBOOK §10 (a failed step still reports `status=COMPLETED`). That is what makes the control mean something — it names the JOIN predicate as the thing doing the rejecting, not a generic adapter or SQL failure landing on the same step by coincidence.
- **what:** A new orchestration action, sibling to `request_browser_run`, that sends the same `run_checks` request to `browser-runner-adapter` for a **section component** rather than a tool. The two actions differ only in how the target page is resolved: a tool's page names itself after the tool (`pages.name IN (function, 'tool-'||function)`); a component's placement is many-to-many (`page_components`), so the target page is given explicitly (`page_id_field`, default `input_data.spec.page_id`) and then **checked** against the real placement — `page_components.page_id = $1 JOIN content_components ON content_components.function = $2`, `pages.status='active'` — rather than derived from the function alone. A wrong or stale `page_id` fails closed with an error, never a silent pass against the wrong page. Everything downstream of page resolution (profiles, `capture_renders`, envelope build, Kafka send, the awaited-response shape) is shared with the tool action via one extracted helper, `dispatchBrowserRun`, rather than duplicated — matching the reasoning this file already used for `envelopePaths`. `judge_acceptance_results` needed no change: it already keys off `function`, never off how the page was resolved.
- **the design decision worth reusing:** a **sibling action**, not an opt-in branch on `request_browser_run`. `request_browser_run` is the one path every one of the fleet's tool-acceptance runs already depends on; a branch — even opt-in, even inert for every existing caller — adds a second resolution strategy to a function this lane had already shown was easy to under-cost (D5′, same PLAN: "I costed a change by reading one enforcement point... there were two"). A sibling action makes the blast radius on the tool path provably zero (nothing about its source changed) rather than argued zero. Full reasoning: `staged_component_build/PLAN_2026-07-30_staged_component_build.md` D9.
- **the design MISS the council caught, recorded rather than defended:** `prior_art_librarian` (medium, council round 1) pointed out that `RequestBrowserRunAction` already had a `url_field` override predating this session, bypassing the `pages.name` lookup entirely — checked and confirmed true (`tool_acceptance_actions.go:160`, original code). That means a strictly SMALLER design existed and was missed: a standalone resolver action doing only the `page_components`/`content_components` placement JOIN, writing a URL into `collected_data`, then calling `request_browser_run` **completely unchanged** via its existing `url_field`/`function_field` overrides — no `dispatchBrowserRun` extraction needed at all, zero lines touched in the tool action's own file. Exactly D5′'s lesson repeated: read the target's main resolution branch closely, missed the escape-hatch field one line below it. Not reverted — the shipped code is correct, tested (see status-evidence and the next bullet), and proven live, and reworking working code for a marginally smaller equivalent is a real cost nobody has asked for — but recorded here as the thing to check FIRST on the next sibling action anyone proposes: does the target action already have an override field before concluding it needs a new resolution branch or a new sibling.
- **the objection that was checked and found FALSE, not just answered:** `guardian` (medium) argued "no existing test exercises this call path, so 'no behaviour change' rests on manual diff alone." Checked against the actual test file rather than accepted or argued with: `TestRequestBrowserRunPayloadCarriesCaptureRenders` and `TestRequestBrowserRunCaptureRendersDefaultsOff` (`tool_acceptance_actions_test.go:411,427`, both predating this session) call `RequestBrowserRunAction` against a mock producer and assert on the actual produced Kafka payload — i.e. they already exercise `RequestBrowserRunAction → dispatchBrowserRun → Producer.ProduceWithValidation` end to end, and both passed after the extraction. The objection was reasonable to raise but factually answerable, and was answered with the test file, not a re-assertion.
- **the landmine it was built to avoid, and this time actually checked rather than assumed:** the same shape DOC-068 itself already names twice in this lane — machinery that reads as generic turning out to encode an assumption specific to what it was first built for (`request_browser_run`'s own page lookup, and separately `prove_fence_can_fail.go`'s mutant list). This time the check was run before the claim: the dispatch above proves the new action's own generality (a real placement, resolved and checked against the live DB) rather than merely asserting it.
- **what this run does NOT re-prove, deliberately:** that each individual check inside the fence can independently fail. That was already established offline the same day, mutation-proven 12/12, by `prove_fence_can_fail_teaser_reveal_panel.go` (TL-036) — calling the identical `RunChecksAction.Execute` the live dispatch also calls. Re-running that same mutation set through the cluster would corroborate, not newly establish, the checks' own falsifiability, at real Playwright cost (two of the twelve mutants took ~30s each). What today's dispatch adds is the one thing with no offline equivalent: proof that `RequestComponentBrowserRunAction`'s placement resolution itself fails closed against the real DB, through the real dispatch path — `try_fence.go` is handed a URL directly and never resolves a page ID at all.
- **sources:** `platform/orchestration/actions/tool_acceptance_actions.go` (`RequestComponentBrowserRunAction`, `dispatchBrowserRun`), `platform/orchestration/actions/registry.go` (dispatch entry); `staged_component_build/PLAN_2026-07-30_staged_component_build.md` D9 + P2 close-out; `staged_component_build/NOTES_staged_component_build.md` "2026-08-02 — P2 CLOSED" (full dispatch read-out); `staged_component_build/SUMMARY_2026-08-02b_the_dispatch_gap_is_closed.md`
- **relations:** DOC-068 (`subject_type='component'` — this closes its remaining verify-later half); TL-036 (the fence + mutation-prover this dispatches, `teaser-reveal-panel`'s 12-check fence, now proven reachable live too); CLC-012 (`teaser-reveal-panel`, the first proven subject)
- **verify-later:** whether a real `agent_definitions` row (rather than the inline `config.workflow` override this proof used) ever gets seeded for a component-acceptance agent, and whether any other component's placement shape (more than one site's worth on the SAME page, or a component nested under `parent_instance_id`) exercises a path this one placement didn't.

### DOC-073 — `pick-pod-marker`: choose pod-grep markers a binary can actually contain
- **status:** built, validated on the incident's own commit, used to a live PASS 2026-08-03
- **status-evidence:** run against `87ea0a5e7` (RFC_009 B — the commit whose comment-only phrase "declared skip_field but never gated" a lane probed with hours after the landmine warning against exactly that): the phrase is classified `[comment] NOT in the binary`, and `template invents %d business fact(s)…` verifies as compiled; the emitted every-replica probe returned marker 2× / negative-control 0 on all live chassis replicas the same evening (across a mid-session roll — the replicaset changed between probes and the numbers held).
- **what:** `scripts/pick-pod-marker.py <commit> [--package ./cmd/<svc>]`. The mechanical form of the deploy-verification practice ("a roll is not evidence your fix shipped"): the commit's added non-test `.go` string literals only NOMINATE candidates; the verdict is a binary built from `git archive <commit>` (structurally immune to shared-tree WIP), scanned per literal. Prints: verified markers ranked (fewest occurrences, prose over regex fragments); the comment-only/unused literals explicitly as the trap; a synthetic negative control plus, where available, a REMOVED-literal control (expect 0 — proves the roll carried the change, not just that the grep pipeline works); and a ready-to-paste every-replica `kubectl exec` probe.
- **the bug it found in itself, kept as a rule:** its own first suggested marker was IN the binary but probed **0/0** on every pod — the literal spanned a `U+2026` and `strings` splits its output at non-ASCII bytes, so a marker crossing a multibyte char greps zero against a binary that contains it. Markers are now printable-ASCII only. A probe pipeline has its own landmines; the negative control alone would not have caught this (it was 0 for the right reason while the positive was 0 for the wrong one).
- **sources:** `scripts/pick-pod-marker.py`; `bugfix_140_contact_info_fabrication/HANDOFF_2026-08-03_continue_here.md` item 5; the WRONG_CALLS/LANDMINES pair on comment-only markers (2026-08-03)
- **relations:** the "prove a deploy at the artefact" practice family (MEMORY: prove-a-deploy-at-the-artefact-index); DOC-070 (`PROBE_doc_subject_go_gate.sh` — the runtime-print sibling for config vocabularies)
- **verify-later:** whether the `--package` default (`./cmd/agent-chassis`) covers the services other lanes actually probe, and whether anyone uses the removed-literal negative control in anger
