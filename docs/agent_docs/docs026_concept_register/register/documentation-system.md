...
- **the open review question:** D10(c) — delivery to **council seats** is NOT wired. `doc_notes` appears in `council-gate.default_config` only as a keyword→seat *trigger* (the `tooling_provenance` seat), not as a context load, so no seat reads landmine rows yet. Rows exist and are queryable; nothing queries them.
