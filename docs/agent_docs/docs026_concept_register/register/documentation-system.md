...
- **verify-later:** whether the first live run's flag counts hold up once run against the real GitHub API on a schedule (the dry-run above used local `git`, a faithful but not identical substitute for the network path); whether anyone actually reads the weekly `doc_notes` row once this ships — the exact "unadopted ledger" risk this whole RFC was raised to avoid
