...
- **verify-later:** whether a component PLAN is ever actually written and read by a gate — the DOC-067 test applies here too, *present capability is not use*. Concretely: `SELECT subject_key FROM doc_plans WHERE subject_type='component'` returning a row, and an S6 run citing its fence. **PARTIALLY CLOSED 2026-08-02, and the split is the point.** `teaser-reveal-panel` now has a real, persisted row (`staged_component_build` lane; not deleted, unlike the 2026-07-31 probe) — a 12-check criteria fence, mutation-proven (12/12 mutants caught, every check watched red under its own mutant, not just watched to pass), read back OUT of the DB and re-run byte-identical to what was written. **That closes "written and read" for the client-side evaluator** (`try_fence.go`, which calls the platform's own `RunChecksAction` directly). **It does NOT close "an S6 run citing its fence"**: that fence has not been dispatched through `browser-runner-adapter` in the cluster, because `request_browser_run` (`platform/orchestration/actions/tool_acceptance_actions.go`) resolves its target page by `pages.name`, one function to exactly one page — an assumption true for a tool and false for this component, which is placed on 5 pages across 2 sites. Wiring that dispatch is a separate, still-undecided design choice (extend `request_browser_run` with an opt-in page/site override, or a sibling action) — see `staged_component_build/HANDOFF_2026-07-31c` §4 and its NOTES. So: the PLAN half of this verify-later line is answered; the S6-dispatch half remains open, now for a named and understood reason rather than an unexplored one.
  > **WIRING BUILT 2026-08-02 — see DOC-072.** The design choice above is decided (sibling action) and the code exists, `go build`/`vet`/`test` clean. **Still not dispatched in the cluster** — that is DOC-072's own verify-later, not this one's. This entry's remaining gap narrows to exactly that.
...
- **status:** **BUILT, NOT YET DISPATCHED IN THE CLUSTER.** `go build`/`go vet`/`go test` clean on `platform/orchestration/actions/...` and the full repo, 2026-08-02. This is a code-existence claim, not a run-in-production one — see verify-later.
...
- **the landmine it is built to avoid, not yet proven to avoid:** the same shape DOC-068 itself already names twice in this lane — machinery that reads as generic turning out to encode an assumption specific to what it was first built for (`request_browser_run`'s own page lookup, and separately `prove_fence_can_fail.go`'s mutant list). This action has NOT yet been run against a real placement in the cluster, so its own generality is unproven by the same standard the rest of this lane holds itself to — see verify-later.
- **sources:** `platform/orchestration/actions/tool_acceptance_actions.go` (`RequestComponentBrowserRunAction`, `dispatchBrowserRun`), `platform/orchestration/actions/registry.go` (dispatch entry); `staged_component_build/PLAN_2026-07-30_staged_component_build.md` D9; `staged_component_build/HANDOFF_2026-08-02_continue_here.md` §3 (the decision this closes)
- **relations:** DOC-068 (`subject_type='component'` — this is its remaining verify-later half); TL-036 (the fence + mutation-prover this dispatches, `teaser-reveal-panel`'s 12-check fence already persisted in `doc_plans`); CLC-012 (`teaser-reveal-panel`, the first intended subject)
- **verify-later:** the whole thing — **council submission**, an image roll, a pod-grep proving the built code actually shipped (a roll is not evidence, per fleet-wide practice), and one real dispatch against `teaser-reveal-panel`'s page (`page_id=ebc2c413-61e2-465e-b22b-9aab0167abc9`, `site_id=4851f6fc-71cf-4160-a270-e03d6d3e0732`) with a negative control (a deliberately wrong `page_id`) in the same run. Only once that lands does DOC-068's own verify-later line ("an S6 run citing its fence") actually close.
