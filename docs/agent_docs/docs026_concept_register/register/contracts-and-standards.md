...
- **sources:** platform/orchestration/datahelpers/unified_extractor.go (findFieldRecursive, collectFieldCandidates, extractSingleField); platform/orchestration/datahelpers/action_inputs.go (strict marker, bypass WARN); platform/orchestration/input_contracts/input_mapping.go (`!` on both resolvers); platform/orchestration/datahelpers/resolver_arm_budget_test.go (inner-chain budget); docs/agent_docs/sql_for_agents/417_image_build_handler_asset_id_goes_strict_HOLD.sql; architecture_review/RFC_029 §9
...
- **verify-later:** after the next roll — grep both WARN messages fleet-wide over 48h+ (the Phase 2 precondition); confirm the 417 HOLD migration was applied only post-roll and the child still resolves asset_id (13/13 baseline measured 2026-08-15).
