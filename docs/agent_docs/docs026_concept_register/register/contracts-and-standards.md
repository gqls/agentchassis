...
- **status:** built — committed 2026-08-14 (`d3edb5b89`, 08:44 BST — the session spanned midnight, so figures below are marked 08-13/14), INERT until the next chassis image roll (Go change). Offline detector re-specified in the same commit and already re-run fleet-wide.
...
- **verify-later:** first post-roll run of a step carrying a dotless static on a defaulted field — expect the `Strategy 6: explicit config value beat the spec default` Info line, and expect `--default-shadowed-keys` to stay at 0 dead; whether any `Strategy 6: config value's type differs` Warn appears in the fleet (it should not — no live entry mismatches kinds today)
