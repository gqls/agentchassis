...
- **verify-later:** ~~after the next roll — grep both WARN messages fleet-wide over 48h+~~ **superseded 2026-08-16:** after the roll of the REVISED build, read the window from rows — `SELECT error_code, context->>'field', count(*), min(occurred_at), max(occurred_at) FROM agent_error_log WHERE error_code IN ('RESOLVER_CONFLICTING_CANDIDATES','RESOLVER_MAPPING_BYPASSED') GROUP BY 1,2 ORDER BY 3 DESC;` (48h+, the Phase 2 precondition); confirm the 417 HOLD migration was applied only post-roll and the child still resolves asset_id (13/13 baseline measured 2026-08-15).
