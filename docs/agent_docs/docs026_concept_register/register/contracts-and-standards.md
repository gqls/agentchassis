...
### CTS-058 — Chassis response-envelope status seam (sendWorkflowResponseWithStatus): failures answer with failure statuses
- **status:** deployed — LIVE on the 2026-08-06 chassis build, pod-verified both replicas (`sendWorkflowResponseWithStatus`=3, control=2), induction-proven with a wire-captured before/after (bugs_closed/196 close note)
- **status-evidence:** council-gate APPROVED round 1, corr d1a63089-af5b-41a2-bea1-62259aa5db52; bugs_open/196 (moved to bugs_closed on live verification)
- **what:** Every response the messaging processor sends (platform/messaging/processor.go) is built by one internal routing path, `sendWorkflowResponseWithStatus(ctx, msgCtx, result, status, errInfo)`; `sendWorkflowResponse` is its "complete"/Success:true wrapper, and `sendErrorResponse` / `sendWorkflowFailureResponse` pass a TYPED status — `errors.IsRetryable(err)` → `error_recoverable`, else `error_unrecoverable` — with `Body.Success:false` and populated `Body.Error` (`ErrorInfo{Code, Message, Recoverable}`). Before 2026-08-05 both error senders inherited the success envelope (`CreateResponseContext("complete",100)` + hard-coded `Success:true`), so an awaiting parent recorded child failures as completed steps and continued on junk; the coordinator's error arms (`error_step`, `continue_on_error`, `failWorkflow`) were unreachable from chassis-child failures while remaining exercised by adapters. The legacy body blob `{"error":…, "status":"failed"}` is preserved byte-identically in `Body.Body` for its established readers (handlerReportedFailure — 017's guard, loop_actions, multipage_actions, git_deployer_actions, fixloop_digest_action). LANDMINE: prose/substring retryability classification must NOT be added at this seam — the typed decision is deliberate; the substring seam is bugs_open/197's, and nothing in production constructs Retryable:true yet, so all current failures map to error_unrecoverable by design. Open review question (named at registration per the 07-28 ruling): whether work items whose handler saga now fails honestly (instead of completing-with-junk) need a recovery sweep beyond what adapter-failure sagas already get — 149's queue lane owns that surface.
- **sources:** platform/messaging/processor.go (sendWorkflowResponseWithStatus, sendErrorResponse, sendWorkflowFailureResponse); platform/messaging/processor_response_status_test.go; docs024_key_docs_latest/bugfix_196_failure_stamped_complete/PLAN_2026-08-05_failure_stamped_complete.md
- **relations:** Adapter/response message envelope contract (ADP-001); coordinator status switch (coordinator.go:316-331); bugs_open/195's typed-code-first remedy (validation_drop.go); bugs_open/197 (the retryable-side classifier, deliberately untouched); handlerReportedFailure (bugs_closed/017's compensating guard, retained)
- **verify-later:** first live error_recoverable envelope once anything constructs AsRetryable; whether the 017 guard's catch-rate falls to zero post-roll (its motivating case now fails the parent at response time)

### CTS-059 — Resolver precedence: an explicit config VALUE beats a spec Default (Strategy 6)
- **status:** **LIVE and BEHAVIOURALLY PROVEN on chassis `v1.0.1298`, both replicas, 2026-08-14.** Committed `d3edb5b89`; council round 1's four code objections closed in `14e4333f7`; rounds 1 and 2 both returned REVISE and **round 3 is with the council** (corr `41a01378`) — no `Council-Reviewed:` trailer exists anywhere and none may be written until an approved verdict is read.
- **behavioural proof (the check that was owed, now DONE):** six `Strategy 6: explicit config value beat the spec default` lines (`action_inputs.go:923`) across **both** replicas, 2026-08-14 13:56–14:30Z — `diagnose_persist_fix_plan max_plan_bytes=65536` (default 32768) and `diagnose_council_decide max_rounds=3` (default 2), **two of the original 21 dead-mismatched census entries, so the council reviewing this change was itself running on the config it repaired.** Zero `type differs` and zero `empty on required` Warns in the same window — and that reading is only meaningful because `Strategy 0: Resolved config path` fired throughout as the liveness control. ⚠ **LIMIT, stated so it is not over-read:** both those actions read the value from step config DIRECTLY in their own bodies, so this proves Strategy 6 FIRES and APPLIES the value; it does not show a behaviour change, which is consistent with the zero-net-change claim rather than evidence against it.
- **⚠ HOW NOT TO CHECK IT, learned the hard way:** `kubectl logs --tail=N` on an established pod answers nothing here. `[MEASURED 2026-08-14]` a pod started 08:58:03Z retained **243 lines spanning 92 SECONDS**, and Strategy 0's **pre-existing** Info line was absent from that window too — so a zero measures retention, not behaviour. **Stream `logs -f` on every replica, and put a pre-existing line in the same filter as the liveness control.** This is the `bug_historian` seat's round-1 objection confirmed by measurement while verifying the very change it was raised against, and it is why `--report` exists.
- **deploy proof:** the provenance stamp is `bc39e7bf5`, **PRESENT in both replicas' `/proc/1/exe`** with a later commit (`d11fb2a44`) correctly **ABSENT** as a two-sided control, and `git merge-base --is-ancestor` true for both code commits. That settles that the CODE is running. The BEHAVIOURAL half could not be read: the pod retains **243 log lines spanning 92 SECONDS** on a pod started 08:58:03Z, so the `Strategy 6: explicit config value beat the spec default` Info line is simply out of range — and the control proves the absence is meaningless, because Strategy 0's **pre-existing** Info line is missing from that window too. This is the `bug_historian` seat's round-1 objection confirmed by measurement, while trying to verify the very change it was raised against, and it is why `--report` exists (below).
- **status-evidence:** `scripts/audit-default-shadowed-keys.sh` against the live fleet, before → after on the same 184 decoded agents, measured 2026-08-13/14: `21 dead mismatched, 78 dead matching, 96 conditional` → `0 dead mismatched, 0 dead matching, 96 conditional, 99 live overrides`, exit 1 → exit 0. Demand control (the zero is real, not a blinded detector): one live `max_pages: 60` mutated to the string `"60"` and fed back through the same export → exit 1, `1 dead mismatched`, classed `type_mismatch`, live overrides 99→98.
- **what:** `datahelpers.ExtractActionInputs` writes `spec.Defaults` into its value map before any strategy runs, and every strategy opens with `if _, hasValue := result.Values[field]; hasValue { continue }`. A default IS a value and nothing ever deletes from that map, so for a DEFAULTED field strategies 1/2/3/4/5 and the nested-object block were all unreachable: only a Strategy-0 dotted path that resolved could ever set it. 99 live config entries were therefore dead — 48 dotless statics, 51 non-string literals — 21 of them disagreeing with the default that silently won. Strategy 6 closes it: for a field still holding only its Default, a dotless config SCALAR is applied as a literal and provenance (`Defaulted`) is cleared, and the Strategy 3 deprecated bridge may now beat a Default too. Guards, each with a live measurement behind it: a string containing a dot is a REFERENCE and stays dead (bugs_open/248 finding (a) published 150+ page-visible 404s named `input-data.asset-key.jpg` by reading a path expression as a value; that fix chose the same dot discriminator); composites are still refused; and a kind guard (`datahelpers.LiteralKind`, shared with the detector rather than mirrored) refuses a scalar whose type differs from the Default's, so a config typo cannot hand an action a type its spec ruled out. All 99 live entries pass the kind guard, so it rejects nothing live and exists to close the door.
- **LANDMINE (register-time, per the 2026-07-28 ruling):** the SAME dotless string now means different things either side of a Default. `analysis_field: "repo_analysis"` on a **defaulted** field is a LITERAL (Strategy 6); on a **non-defaulted** field it is a single-segment REFERENCE resolved against collected_data (Strategy 4). That asymmetry is deliberate — a field carrying a Default is a setting with a fallback, which is why all 48 live dotless statics are plainly values their authors typed (`repo_name: 'agentchassis'`, `ref: 'main'`, `country: 'GB'`) — and no live config can depend on the other reading, because Strategy 4 was never reachable for a defaulted field. But it means you cannot answer "is this string a value or a path?" from the config alone: you must look at whether the action's spec defaults that field.
- **Open review question, named at registration:** whether a resolving dotless string on a defaulted field should instead resolve as a collected_data reference (today: literal wins, never resolved). Deliberately NOT taken: it is the one arm that could replace a typed Default with a resolved object of unknown shape, `ActionInputs.Get` returns "" for a non-string, and zero live entries want it. Whoever revisits it owns re-measuring the `*_field` family first — 28 of them read config directly today, so the extractor's answer is currently irrelevant to them and that is what makes the change look free.
- **Other consumers told (owner ruling 2026-07-29 §3):** the four `audit_source` entries this bug's third face exposed were repaired independently by bugs_open/264 (migration 399 + `audit_source` made Required with no Default) before this shipped, so candidate 2's activation set is now empty by construction rather than by luck — recorded because the two lanes' fixes are load-bearing for each other's blast-radius claim.
- **sources:** platform/orchestration/datahelpers/action_inputs.go (Strategy 6, `LiteralKind`, the Strategy 3 bridge's has-value skip); cmd/config-key-audit/defaultshadow.go (re-specified classes + `shadowClassVerdict`); platform/orchestration/actions/deploy_image_asset_purpose_source_test.go (`TestLegacyLogoStep_StaticPurposeBeatsTheDefault`, `TestPurposeFieldBridge_BeatsTheDefault` — both FLIPPED from pinning the defect); bugs_open/231
- **relations:** bugs_open/231 (candidate 2, the whole census); bugs_open/248 finding (b) — `Defaulted`/`WasDefaulted` provenance, which Strategy 6 must clear and whose row-purpose fallback breaks if it does not; bugs_open/248 finding (a) — the dot discriminator this reuses; bugs_open/264 (the `audit_source` instance, fixed separately); CTS-048 (local-step input resolution / key_path); CTS-016 (handler dispatch input-path contract)
- **round 2 (council REVISE, closed in `14e4333f7`):** the guardian's gating objection was that the unreachability proof is a static census, not an enforced invariant — so `TestDefaultBeatsTheRecursiveSearch` now pins it, carrying its own **vacuity control** (the same fixture through a spec with no Defaults must resolve `purpose='logo'`/`max_items=99`, else the test proves nothing). Also: the canonical config key now beats a deprecated alias (Strategy 3 ran first and would otherwise win, the opposite of what a migration is for), both non-defaulted bridge arms are pinned, and `--default-shadowed-keys --report` writes one `doc_notes` row per run so a REJECTED override has a durable surface — the resolver cannot supply one itself, having no DB handle by design. `countDeadMismatched` is the single definition of the exit condition.
- **`--report` HAS NOW PRODUCED A REAL ROW, and is still UNDRIVEN.** Run against the live fleet 2026-08-14: `doc_notes` `subject_key='default-shadowed-keys'`, `source='default-shadowed-keys-check'`, `created_at 16:34:31+00` — 185 agents, 0 dead, 96 conditional, 99 live overrides, exit 0. So the durable surface is real rather than prospective. **But nothing SCHEDULES it**, so between runs a mistyped setting is still refused with no fleet-visible signal — `bug_historian`'s objection is MITIGATED, not closed. No CronJob runs it. The overlay was deliberately NOT shipped: the image must exist first, because this fleet reports `ImagePullBackOff` as a Job still RUNNING and never FAILED — the trap `removed-config-keys-check`'s own `cronjob.yaml` records hitting on its first rollout. Wiring it follows that service line-for-line (`build-`/`push-` makefile pair + base cronjob + production overlay).
- **verify-later:** the one check still owed — **observe `Strategy 6: explicit config value beat the spec default` actually firing.** Do NOT read it from `logs --tail` on an old pod (92-second window, measured); stream `logs -f` on both replicas, or drive a step that carries one of the 99 live overrides. Also expect NO `Strategy 6: config value's type differs` Warn anywhere — no live entry mismatches kinds, so one appearing means new config arrived and is being refused. Post-roll census re-confirmed 2026-08-14: 185 agents, **0 dead**, 96 conditional, 99 live overrides, exit 0.

### CTS-060 — Whole-tree search goes unique-or-nothing, and the `!` strict marker (RFC_029 Phase 1)
- **status:** ~~built, inert until an image roll~~ **UPDATED 2026-08-16:** Phase 1 (log-only) is **LIVE** — chassis v1.0.1303, binary stamped `5e075a6f9`, a descendant of `1806371ef` (probed, two-way control). The **revision** (council REVISE, run `ae2a88a7`: the WARNs are ALSO persisted to `agent_error_log`) is **built, inert until the next chassis roll**. The adopter migration `417_…_HOLD.sql` is deliberately HELD; its binary precondition (the `!` parser) is now met.
- **what:** Two changes to input resolution, both from RFC_029 §9 (owner-delegated ruling 2026-08-15). (1) `findFieldRecursive` (`unified_extractor.go`, the inner chain's whole-tree-search arm) now COLLECTS every match — sorted-key DFS, same depth cap, same infrastructure skip-list — and resolves the shallowest-first winner deterministically. Agreeing candidates behave as today; CONFLICTING candidates in Phase 1 still resolve to the stable winner but emit WARN `aggressive search: conflicting candidates` naming field, every candidate path, and the winner. Phase 2 (a later build, after a 48h+ observation window with the precondition in §9 D2) flips conflicts to resolve NOTHING. (2) An opt-in `!` STRICT marker, the mirror of `?`, on both mapping surfaces: in `ExtractActionInputs` step config (`"field!": "<reference>"`) it means explicit resolution only — never the whole-tree search, never the nested-object fallback, never the deprecated bridge — and a reference that fails to resolve fails the extraction loudly, Defaults included; in `ResolveInputMapping`/`WithItem` it is the loud spelling whose value is the `?`→`!` transition (unmarked already hard-fails there). Also shipped: WARN `aggressive search: explicit single-segment mapping bypassed` measuring the bugfix-213 mapped-field class (observation only), an AST arm budget on `extractSingleField`'s resolution returns (floor 5, ceiling 8, mutation-proven), and the inner chain's arms renamed descriptively (direct-path, input-data-prefix, input-data-map, whole-tree-search, alias) so migration 402's Strategy-number miscitation cannot recur. **(3) Added 2026-08-16 (RFC_029 §10.4, the council-revision):** every occurrence of BOTH WARNs is also reported to a registered sink — `datahelpers.SetResolverFindingRecorder(ResolverFindingRecorder)` (`resolver_findings.go`), **nil by default = log-only**; the chassis registers one in `agentbase.initializeComponents` (a thin wrapper over `orchestration.LogAgentError` → `agenterrors.Write`) so each occurrence becomes an `agent_error_log` row: `error_code` **`RESOLVER_CONFLICTING_CANDIDATES`** / **`RESOLVER_MAPPING_BYPASSED`**, `severity='warning'`, `action='input-resolver'`, pod-level attribution only (`orchestration_id` empty by design; the row's `identity_scope` says so). No dedup — frequency is the population §9's disconfirmation clause needs. This is what makes the 48h+ observation window READABLE after the fact (pod log retention is ~90s, CTS-059); its clock starts at the roll of the revised build. **Scope, per the round-2 architecture seat (approved, advisory 2026-08-16): this sink is Phase-1 instrumentation for these TWO finding types — a second finding type or a second consumer gets a fresh architecture look, not a wave-through as "already established"; and never a second parallel sink.**
- **LANDMINE (register-time, per the 2026-07-28 ruling):** a `!`-suffixed mapping key applied BEFORE the binary that parses it has rolled is read by the old binary as an ordinary field named `field!` — the child then receives no `field` at all and its resolver falls back to the whole-tree search, the exact regression the marker exists to prevent; on the ResolveInputMapping surface the un-suffixed reading is REQUIRED and hard-fails steps whose optionality was doing real work. That is why 417 is a `_HOLD` migration. Full entry in LANDMINES.md (footprint `input_mapping`, `!`).
- **Open review question, named at registration:** RFC_029 §9 D3 named build-dispatch-loop's repair mapping as the `!` marker's second adopter, but that mapping is shared by every dispatched item type (402's own measurement: exactly one of 636+ item types carries spec.asset_id), so `?`→`!` there would hard-fail every non-asset dispatch — recorded as a dated correction on the ruling; the repair path keeps `?` and is covered by the search change instead. Whether the mapped-field (bugfix-213) class needs its own remedy beyond the bypass WARN is Phase 2's question, decided by the observation window's data.
- **sources:** platform/orchestration/datahelpers/unified_extractor.go (findFieldRecursive, collectFieldCandidates, extractSingleField); platform/orchestration/datahelpers/action_inputs.go (strict marker, bypass WARN); platform/orchestration/datahelpers/resolver_findings.go (the recorder sink + the two ErrorCodes) and platform/agentbase/agent.go (`recordResolverFinding`, the chassis registration); platform/orchestration/input_contracts/input_mapping.go (`!` on both resolvers); platform/orchestration/datahelpers/resolver_arm_budget_test.go (inner-chain budget); docs/agent_docs/sql_for_agents/417_image_build_handler_asset_id_goes_strict_HOLD.sql; architecture_review/RFC_029 §9
- **relations:** CTS-059 (the outer chain's Strategy 6 and its vacuity-control test, whose flaky control this work repaired); RFC_028's arm budget (the outer half of D4); WFA-009 (the opt-in-required precedent the `!` shape follows); bugs_closed/248 (both incidents), bugs_closed/213 §D (the mapped-field class)
- **(4) ADDED 2026-08-19 — RFC_029 §10.13, the owner-ruled Phase 2 SEQUENCE (three changes, each council-gated, in order):** **(a) Strategy-0 prune** (`action_inputs.go`, corr `ae0dfb93` APPROVED, LIVE v1.0.1310): a field Strategy 0 resolved via an explicit dotted path is no longer handed to the whole-tree search — behaviour-identical (the merge always discarded exactly those answers), removes the ~28% `work_item_id` conflict class; first 16 h read 0 such rows. **(b) Declared tie-break + sorted unwrap** (`unified_extractor.go`, corr `96ac93e6` APPROVED all-approve, ~~bugs_open~~ **bugs_closed** (2026-08-19)/306 cands 1+2): each `fieldCandidate` carries a `rank` (direct match ≺ `~unwrap` hop ≺ sibling, inherited from its first hop off the root) and the sort reads it; `tryUnwrapMapPatterns` pattern 1 iterates `*_result` keys SORTED (the last unsorted map range on the search path). Behaviour-identical today; ~~inert until a roll~~ **LIVE `v1.0.1315` (stamp `590ca3a20`, probed 2026-08-19)** — plus **306 candidate 3** (corr `e4840008` APPROVED round 1, 090 CONFIRMED `a9a33be9`): the search skips `retry_payload` subtrees (`types.RetryPayloadKey` case in `isInfrastructureKey`; code rode `393f15bfd`, tests `02777cd5f`, `unified_extractor_retrypayload_test.go`, 3 tests mutation-proved) — post-roll window: pbh echo class 45→0 against live demand, `retry_payload` in candidate_paths fleet-wide 0. **(c) `ensureCoreFields` GATE** (corr `07468ec0`, **APPROVED round 2** 2026-08-19 — round 1 REVISE was right: the consumer census was top-level-only and was redone recursively, conclusion held; the skip is now NAMED on the standing log line, `unrequested_page_fields_not_injected`; GUARANTEE CHANGE = RFC_029 amendment): `current_page`/`current_section`/`render_context` are injected ONLY when requested; `domain`/`objective`/`model` stay UNCONDITIONAL (39/10/6 undeclared template consumers — never gate). Consumers enumerated; the sole injection-dependent one (`html-developer-chunked`, dormant) re-declared by **migration 483, APPLIED 2026-08-19 before the gate by design**. Removes the 63% `build-dispatch-loop current_page` class. ~~Inert until a roll~~ **LIVE `v1.0.1315` and proven 2026-08-19: bdl conflict rows post-roll = 0 (both fields), with the instrument alive (pcw rows still writing).** **Residual after (a)+(c): requested-field shape conflicts (e.g. `save_page_sections`' page-object vs name-string) — §10.13 step 4, fixed AT SOURCE, not here; then step 5 flips conflicts→refusal on what is left.** Tests: `action_inputs_prune_test.go` (7), `unified_extractor_tiebreak_test.go` (6), `unified_extractor_corefields_gate_test.go` (5), all mutation-proved both ways. **(d) STEP 4 BUILT 2026-08-19 (commit `1a82225ec` + comment-only `916c8b22b` + growth-guard test `c16836d86`; corr `f3716ebe` **APPROVED round 2** ~20:33Z — round 1 REVISE was about `bugs_open/085` sitting open while closed-in-fact, now `bugs_closed/`; inert until the next chassis roll):** the residual was ONE candidate set (23 rows in the 4 h after `v1.0.1315`, 100% of the class): page-content-writer's `generate_content` requests `current_page` (the page RECORD) and the search also found the page NAME string `build_render_context` filed under `render_context.current_page` / `build_render_context.current_page` — two types, one key. Fixed at the producer, in `v3_site_actions.go`: a declared rename map `renderContextStepContractRenames = {current_page → current_page_name}` (+ `renderContextStepContractKey`) applied symmetrically at the STEP BOUNDARY only — `renderCtxToMap` emits `current_page_name` (and will not re-emit the old key from its ContentData merge); `setRenderContextScalarsFromData` reads `current_page_name` first and tolerates a STRING under the old spelling for pre-roll trees (read side only). **The TEMPLATE key is unchanged — components still read `{{.current_page}}`** (1 live reader, `evidence-chart`); `buildRerenderBaseData` (the restore contract's second producer) and `renderEnvelopeIdentity`'s chain follow. Consumers: 0 live agent definitions name the step-boundary key. 7 tests mutation-proved six ways + a resolver-seam test with a CONTROL (`render_context_step_boundary_resolver_test.go`: real `ExtractFields` over a live-shaped tree → 0 conflicts; pre-rename shape → still conflicts). Done-condition: pcw/`current_page` conflict rows 0 in the post-roll window against live pcw demand → step 5 buildable.
- **verify-later:** ~~after the next roll — grep both WARN messages fleet-wide over 48h+~~ **DONE 2026-08-18 — both halves discharged.** (1) The `!` marker's first adopter is PROVEN: 26/26 asset-deployer children carry a bare `asset_id`, 0 carry a literal `asset_id!`, 0 strict errors (RFC_029 §10.8). (2) The window has run since 2026-08-16 10:41Z and produced its first result: 86% of the conflict population was `bugs_closed/287`, whose fix (WFA-017 + `!` on three `mark_complete` steps) took `field=result` to ZERO across 193 loop runs — the marker's first measured win on someone else's bug. Residual triage + Phase 2: RFC_029 §10.7a/§10.9. Original note retained: **superseded 2026-08-16:** after the roll of the REVISED build, read the window from rows — `SELECT error_code, context->>'field', count(*), min(occurred_at), max(occurred_at) FROM agent_error_log WHERE error_code IN ('RESOLVER_CONFLICTING_CANDIDATES','RESOLVER_MAPPING_BYPASSED') GROUP BY 1,2 ORDER BY 3 DESC;` (48h+, the Phase 2 precondition); confirm the 417 HOLD migration was applied only post-roll and the child still resolves asset_id (13/13 baseline measured 2026-08-15).
