...
- **status:** **LIVE on chassis `v1.0.1298`, both replicas, artefact-proven 2026-08-14** — plus the council's REVISE round 1 closed in code (`14e4333f7`). Committed `d3edb5b89` (08:44 BST; that session spanned midnight, so figures marked 08-13/14 are one session). ⚠ **Live is not the same as fully verified: the resolver's own Strategy 6 log line has NOT been observed firing** — see the deploy-proof note below. Round 2 is with the council under the same correlation.
- **deploy proof, and the half of it that FAILED:** the provenance stamp is `bc39e7bf5`, **PRESENT in both replicas' `/proc/1/exe`** with a later commit (`d11fb2a44`) correctly **ABSENT** as a two-sided control, and `git merge-base --is-ancestor` true for both code commits. That settles that the CODE is running. The BEHAVIOURAL half could not be read: the pod retains **243 log lines spanning 92 SECONDS** on a pod started 08:58:03Z, so the `Strategy 6: explicit config value beat the spec default` Info line is simply out of range — and the control proves the absence is meaningless, because Strategy 0's **pre-existing** Info line is missing from that window too. This is the `bug_historian` seat's round-1 objection confirmed by measurement, while trying to verify the very change it was raised against, and it is why `--report` exists (below).
