# Register — agent-spawning-and-groups

> **covers-through: 2026-07-13** · extraction freeze.
> Subsystems that shipped after this date may be absent from this file
> **entirely** — absence here is not evidence of absence in the platform. See `bugs_open/106`.

_Concept count retired 2026-08-09 — derived, not stored; run the drift pair in `000_concept_index.md`, or read `concept-register-drift-check`'s daily row (DOC-074)._ consolidated from 14 raw extractions (7 unique blocks, each present
twice in the source cluster file due to mechanical duplication in the input),
all from unit U26.

### ASG-001 — Agent spawning (agents as DB records claimed by generic pods)
- **status:** deployed
...
### ASG-002 — Agent groups (reusable multi-agent teams)
- **status:** deployed
...
- **relations:** website-builder group; controlled group evolution; group discovery; HITL content-approval demo agent and group (hitl register)
...
### ASG-004 — Workflow template library, lineage and marketplace
- **status:** abandoned
...
### ASG-005 — Multi-dimensional template classification and semantic search
- **status:** abandoned
...
### ASG-007 — Dynamic prompt improvement loop (Prompt Improvement Agent)
- **status:** aspirational
...
- **relations:** execute_llm_prompt; HITL approval mechanism (hitl register); controlled group evolution; (spiritual ancestor of the current improvement-loop / finetuning flywheel)
