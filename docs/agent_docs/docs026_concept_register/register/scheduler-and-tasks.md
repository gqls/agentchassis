...
- **status:** ~~built, council-submitted (corr `2281fc48`); inert until migration `346` is applied~~ **CORRECTED 2026-08-09 (vigilant_designer lane): LIVE AND TICKING** — all three `site-discovery-rotation-*` task rows are `enabled=t`, hourly, stamping since ~09:49Z (12 quality selections by 20:00Z; read live, not from this line). The stale line cost a session a re-derivation of whether the driver existed; the tasks were firing the whole time.
