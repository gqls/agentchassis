...
- **status:** **LIVE 2026-08-15** (`sql_for_agents/430_detected_item_promoter_task.sql`, ledger-recorded; owner ruling the same day = bugs_open/083 fix candidate 2). First tick due within 15 minutes of apply; verify block's partition assertion held on the live pile at apply time: 70 detected = **66 promotable + 4 held + 0 unroutable**.
...
- **verify-later:** first tick's `promoted` count and pairs (scheduler log line "Pre-query task completed (no message fired)" for this task); the 70-row pile drains within ~an hour and STAYS drained (bugs_open/083's own verify criterion 1); `phantom_internal_link` reaches a non-zero `complete` count for the first time (criterion 2 — none are in the pile today, so this waits for the next re-raise); the 4 held rows stay held until someone canaries the pair.
