...
- **status:** **built, inert until the next chassis roll** (Go change). Committed 2026-08-02; council gate submitted the same day, correlation `e78c62e3-7f01-48f1-b083-924eaccd195a`.
...
- **one deliberate divergence from 081, measured:** "has been live" is `build_status IN ('deployed','needs_rebuild') OR deployed_at IS NOT NULL`, wider than 081's `build_status = 'deployed'`. `bugs_closed/037` is an entire case about `needs_rebuild` falling outside a `='deployed'` guard, and the census on 2026-08-02 says **35 of 46 `needs_rebuild` rows carry a non-null `deployed_at`** — they have shipped and are being served.
...
- **OPEN REVIEW QUESTION (registered per the 2026-07-28/29 ordering-exemption ruling, condition 2):** two arms now return an ERROR on refusal where they previously overwrote a live page (`DeployToolToSiteAction`'s tool page, `CreateReportPageAction`); the two companion-guide arms stay non-fatal and log. Is failing visibly right for the tool arm, or should every arm degrade? And: the constant-role contract is enforced by review, not by a type — an explicit opt-in field per call site is one field away if a reviewer thinks documentation is too thin.
