...
- **status:** **LIVE on chassis `v1.0.1218`** (pod-verified both replicas 2026-07-31, all four strings present against a positive control; pods started 17:59 UTC, after both commits). **NOT yet exercised on a live page build** — the acceptance build is queued and blocked behind the fleet-wide dispatch stall (`bugs_open/029`), so the mechanism is proven present, not proven working.
- **status-evidence:** 7 DB-free unit tests pass, including the safety property (literal path beats both aliases) and the negative control (a fact absent from every store stays missing so `on_missing` governs). Diagnosis loop **CONFIRMED** the root cause first iteration, corr `0f76987c-0fc8-48b9-9c12-c49379560f00`. Council gate submitted as `dd03a73b-eee1-4769-93ed-d8fe79154c19`. **Not** verified against a live page build — that needs the roll.
