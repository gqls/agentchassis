...
- **SECOND CONSUMER, 2026-07-31 (`ecf738002`, council corr `a54172b6-9756-4abc-a9e0-f173ad4de779`, verdict pending): `save_page_sections` — site A of `bugs_open/165`.** The rule is used UNCHANGED; only a consumer was added, so nothing this entry guarantees moved. Three things it establishes that the `code_symbols` call site could not, and that the next converter (sites B and C) should read first:
