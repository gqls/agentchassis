...
### CTXA-025 — Guarded reconciliation prune: a destructive delete must first prove the run saw the corpus
- **status:** **LIVE on chassis v1.0.1218 (2026-07-31), both branches PROVEN by induced fault** — not by a healthy run, which proves only that the floor is inert. Armed at the code default 0.5; the live run reports `prune_floor_from_config: false`, i.e. nothing configures it.
- **status-evidence:** `prune_floor.go` + `prune_floor_test.go` (11 tests, every branch incl. the refusal text and the inclusive boundary); wired at one call site, `IndexCodeSymbolsAction`; package suite green against a clean `git archive HEAD` overlay. **Live induction 2026-07-31:** 400 synthetic `interface` rows at a bogus `commit_sha` -> `prune_status=refused_floor`, `pruned=0`, all 5,392 rows intact, durable `doc_notes` row written; cleaned up and re-run at 87% -> `pruned=5`, index restored exactly to 4,992 rows / 592 paths / one commit. Both replicas pod-grepped with a positive AND a negative control in the same exec. **The demonstration worth keeping: the whole-repo path cohort PASSED at 60% while the per-kind cohort refused at 8% — a single whole-corpus ratio, the obvious design, would have permitted that delete.**
- **what:** The rule (and refusal text) for the "delete everything this run did not re-write" pattern, as pure functions of counts — no SQL, no knowledge of any one table, because the measurement is per-table and the rule is not. A caller supplies **cohorts** (`label`, `confirmed-by-this-run`, `stored`) and gets a verdict plus a sentence that names its own remedy. Cohorts rather than one total because a run that re-confirms 95% of rows can still have lost 100% of one class, which a whole-corpus ratio hides completely; the recommended partition is one cohort per independently-losable class **plus one in a different unit** (distinct paths seen, not rows written — that is a signal about whether the run SAW the corpus rather than about how much it WROTE). **CORRECTED 2026-07-31 by the live induction:** this entry first called the path cohort the *stronger* of the two, "the property that actually fails when a tarball arrives truncated". That was an unmeasured ranking and the first real test REVERSED it — paths **PASSED at 60%** while `kind=interface` refused at 8%. Keep both, because they fail on different inputs; do not repeat the ranking, and note that a single whole-corpus ratio (the obvious design) would have permitted that delete). The floor is **resolvable** step config (`prune_floor_ratio`, 0 disables) and the refusal states that remedy, because a large legitimate deletion is a real event and a guard with no exit gets deleted by whoever it blocks. `>1` clamps to 1 (an unsatisfiable floor refuses every prune for ever while looking like a working guard); a cohort exactly AT the floor passes; a cohort with `stored=0` never refuses, so a class stored for the first time cannot block anything. Reading the ratio from config falls back to the DEFAULT on an unparseable value, never to 0 — a typo must not silently disarm a destructive-operation guard. The caller is expected to **fail closed** when the cohorts cannot be measured (`index_code_symbols` does) and to report the decision, not only take it: `prune_status` distinguishes *nothing to delete* from *we refused to delete*, which want opposite responses.
- **sources:** `bugs_closed/135` (filed 2026-07-28, fixed + closed 2026-07-31); `platform/orchestration/actions/prune_floor.go`; `code_symbols_actions.go` (`codeSymbolPruneCohorts`, `recordPruneRefusal`); `docs024_key_docs_latest/bugfix_135_prune_floor/`; council corr `14239fa4-552f-4821-abaf-ea15ccee4ea5`; the design is the end of the six-round argument on corr `7ba5b8c4` (bug_historian rounds 3–4, guardian round 5), not the start of a new one
- **relations:** CTXA-011/012 (the code index and its prune); CTXA-024 + `bugs_closed/108` (the read side — a thinned index answers "absent" *confidently*, which is why the write side needs a floor); **LOCK-007** (register/locks.md — the sibling shape, landed independently the same week: a shared guard answering "may automation overwrite this?" for *writers*, where this answers "may automation delete the remainder?" for *deleters*. Both derive the guard predicate from the operation's own predicate rather than re-typing it, and both fail closed)
- **LANDMINE:** a green indexing run proves NOTHING about this guard — the floor is inert on a healthy repo by design, so "the index still has 4,992 rows" is not evidence it works. Induce the fault (synthetic rows at a bogus `commit_sha` pushing one cohort below the floor) and watch `prune_status` flip — done 2026-07-31, and it refuted the author's own ranking of the two cohorts, which a green run never would have. And note the guard measures the population the DELETE acts on using the **exact complement** of the DELETE's own predicate (`IS NOT DISTINCT FROM` against `IS DISTINCT FROM`): if you re-spell either side (`=`, `COALESCE`, a NULL-safe rewrite), the guard silently starts judging a different population from the one at risk.
- **open review question — NOW FILED as `bugs_open/165`** (at the direction of the council's `bug_historian` seat, HIGH, corr 14239fa4: leaving the siblings exposed is itself the platform's most repeated pattern, `021_HANDOFF_durable_write_guard_covers_one_path_only`). Three other live call sites share the identical destructive shape and are deliberately NOT converted — `populate_nav_tables_action.go:147` (whole-site `site_nav_items`/`site_nav_groups`), `site_db_actions.go:1474` (`link_registry` per source page), `save_page_sections_action.go:532` (agent-writable `page_components` per page). Each needs its own cohorts *measured*, not assumed, and each is another lane's live territory. Converting them is the next thread's work, not a rider on this one.
- **SECOND CONSUMER, 2026-07-31 — LIVE on chassis v1.0.1223, BOTH BRANCHES INDUCED in production, council APPROVED r1 (`a54172b6-9756-4abc-a9e0-f173ad4de779`, 15 reviewers, 7 advisory): `save_page_sections` — site A of `bugs_open/165`** (`ecf738002` + `bb51f5c6e`).
  - **Live evidence.** Refusal: plan inflated 7→20 on one page, orchestration went **FAILED @ save_sections**, `planned sections 35% (7 of 20)`, durable `site_work_items` row as `needs_human_review`, and **all 7 `page_components` rows byte-identical to baseline — nothing deleted**. Pass: plan restored, `COMPLETED`, both cohorts 100%, numbers reported on the successful save. Both replicas pod-grepped with marker + control in one exec.
  - **THE DEMONSTRATION WORTH KEEPING, and it is the mirror of 135's:** the **PLAN cohort refused while the ROW cohort read 7 of 7 = 100%**. The obvious single-cohort design — measure what you wrote against what is stored — would have waved that run straight through. In 135 the per-kind cohort caught what the whole-repo one missed; here the different-unit cohort caught what the row one missed. Two call sites, two opposite-looking results, same lesson: **the cohort you would have left out is the one that fires.**
  - **A refusal is not always self-healing, and the shared refusal TEXT does not know that.** `Reason()` ends "a later run that sees the whole corpus will prune them" — true for 135 (prune refused, run continues), FALSE for this consumer (whole save refused, nothing prunes later, the page is simply not rebuilt). Known defect, deliberately not fixed inside a bug patch because the clause lives in the shared rule; see `bugs_open/165`. The rule is used UNCHANGED; only a consumer was added, so nothing this entry guarantees moved. Three things it establishes that the `code_symbols` call site could not, and that the next converter (sites B and C) should read first:
  - **Cohorts do not transfer, and the shape the bug file proposed was refuted by measurement.** One cohort per `slot_name` is unusable here: 998 of 1,009 `(page_id, slot_name)` groups hold exactly ONE row, so every per-slot cohort is 1 stored and any legitimate single-section removal scores it 0% — 89 real shrinkages in 4.5 months, each refused. Per-`component_id` is no better (365 of 409 pages have distinct components == rows: the row count in a costume). What works is two page-level cohorts in different units — rows, and the PLAN (`pages.sections`, written by seven other actions and never by this one).
  - **The plan cohort is what breaks the RATCHET,** a failure the `code_symbols` cohorts cannot express: once a truncating writer has cut a page from twelve rows to two, the row cohort reads 2/2 = 100% for ever and the damage becomes the new baseline. The plan still reads 2 of 12. It is also the only cohort that can catch a truncated FIRST build (no rows stored yet — 63 pages currently sit in exactly that state).
  - **A refusal need not be self-healing, and here it is not.** In `index_code_symbols` the delete and the write are separable, so a refusal retains stale rows a later healthy run prunes. In `save_page_sections` they are ONE operation — refusing the delete but performing the insert would write duplicates (`bugs_open/156`) — so the whole save is refused and the cost of a false positive is a failed build step rather than nothing. That asymmetry is why the false-positive rate was measured *before* the floor was chosen: 4 of 2,620 real overwrite transitions below 0.5 (0.15%), and 0 of 238 reachable pages for the plan cohort. Durable surface is `site_work_items` here rather than 135's `doc_notes`, because this writer is site-scoped.
  - **LANDMINE for the next site:** exclude actively-locked rows from any plan-side denominator. A locked slot is never written, so counting it refuses *perfect* rebuilds of exactly the pages a human curated (`idea.uk/index.html`: 6 planned, 4 locked → a flawless rebuild scores 2/6). `site_nav_items` carries the same lock columns, so site B inherits this.
