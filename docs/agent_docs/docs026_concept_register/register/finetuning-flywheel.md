...
- **status:** built + mutation-proven, **NOT live** (needs `make build-thunder-adapter` + a whole-fleet `make release`, which the owner runs; migration 400 not yet applied). Council submission written but **not yet submitted** — the kubeconfig token expired 2026-08-13 18:05Z.
