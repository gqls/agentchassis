...
- **status:** built + mutation-proven, **NOT live** (needs `make build-thunder-adapter` + a whole-fleet `make release`, which the owner runs; migration 397 not yet applied). Council submission written but **not yet submitted** — the kubeconfig token expired 2026-08-13 18:05Z.
- **status-evidence:** `provision_defaults_test.go` asserts `cpu_cores` **as sent to Thunder** across the whole live catalogue (fixture = `GET /v1/specs`, re-measured 2026-08-13). **Proven able to fail:** restoring the old constant fails 7 of 8 cases, the sole survivor being `h100_x1_prototyping` — the one spec that accepts 4, which is the asymmetry that hid the bug. Migration 397 carries a `DO`/`RAISE` block that INDUCES a check violation and also refuses a default above the 600s await.
- **what:** Two mechanisms other lanes can use, both in `internal/adapters/thunder`. (1) **`api.Client.GetSpecs`** + `api.Spec` / `SpecKey(gpu,count,mode)` / `Spec.LowestValidVCPUs()` / `Spec.AcceptsVCPUs(n)` — Thunder's catalogue as the authority on what it will actually sell, replacing a hardcoded `cpu_cores: 4` that **9 of 11 single-GPU specs reject** (`bugs_open/258` defect 1). Consulted only to fill a blank, so an explicit request is never blocked by a `/specs` outage; **refuses rather than falling back**, because falling back to a constant is the defect. (2) **`thunder_config.provision_wait_timeout_seconds`** (migration 397, default 540s, CHECK 60–1800, plus a 30-min ceiling re-enforced in Go) — the wait-for-RUNNING deadline as live config, so a slow-booting box is no longer destroyed by a compiled-in 5 minutes that no measured a6000 boot met.
...
- **migration-independence:** the new column is read via `(to_jsonb(t)->>'provision_wait_timeout_seconds')::int`, not a bare column reference — a bare reference would make `store.LoadConfig`, and so every provision/decommission/reaper run, fail outright wherever 397 is unapplied. An unmigrated database degrades to the compiled-in default instead.
- **sources:** `internal/adapters/thunder/api/types.go` (Spec, SpecsResponse, SpecKey, LowestValidVCPUs, AcceptsVCPUs); `internal/adapters/thunder/api/client.go` (GetSpecs); `internal/adapters/thunder/provision_action.go` (`lowestValidVCPUs`, `resolveWaitTimeout`, `provisionWaitTimeoutCeiling`); `internal/adapters/thunder/store/config.go`; `docs/agent_docs/sql_for_agents/397_thunder_provision_wait_timeout.sql` (+ `_ROLLBACK`); `internal/adapters/thunder/provision_defaults_test.go`
