...
- **status:** deployed (proven end-to-end in prod 2026-08-15)
- **status-evidence:** ~~"Still unproven in prod is one run reaching RUN_SH_DONE with the final adapter.tar.gz durable"~~ **CLOSED 2026-08-15**: a real run (SmolLM2-1.7B, 300 rows, a6000) reached `RUN_SH_DONE` at 17:10:01Z with the final adapter verified durable at the artefact — presigned GET on `finetuning/artefacts/phase0-20260815-1621/adapter.tar.gz` → 206, `Content-Range …/67989958`, gzip magic. Box-side `HTTP 200` upload line corroborates. Evidence: finetuning_uk_service NOTES 2026-08-15 evening. (Checkpoint-cadence path exercised earlier — ckpt-0 in B2 June; this run used SAVE_STEPS=0, final-only, by design.)
...
### FTW-042 — Thunder orphan scan (vendor-truth reconcile + thunder_orphan work items)
- **status:** deployed, first run verified (2026-08-09)
- **status-evidence:** fleet `v1.0.1273` pod-grepped (both chassis replicas + adapter, positive + spelling-negative controls); 342 applied AND ledger-recorded (`--record-only`); first scan COMPLETED 11:42:06Z with counts truthful against same-session independent checks (`vendor_billing:0` = manual `instances/list` `{}`; `db_rows:23/db_live:0` = table read; `clean:true`). ~~built, NOT yet live~~ **Caveats that stay true:** the FILING path has fired against no real orphan (unit-tested to the classification boundary; first real orphan is its live proof), and the first live run FAILED before succeeding — `output_field` nested in step `config` is INERT (must be step-level; LANDMINES 2026-08-09 — the reaper's own seed models the wrong form and its response has stored under the bare step name since it shipped).
- **what:** Closes the orphan gap the reaper structurally cannot see: `provision_action.go` INSERTs `thunder_instances` only AFTER the vendor box is up, so a crash in that window — or any hand-created instance — bills at Thunder invisibly to every check that reads our table (the gap `bugs_open/186` §Related names; the manual net was RUNBOOK §1b). Three pieces: thunder-adapter grows a read-only `list_instances` action (verbatim `GET /instances/list` passthrough — the vendor is the source of truth for the bill); chassis action `dispatch_thunder_list` awaits it; chassis action `reconcile_thunder_instances` classifies (pure, unit-tested: `orphan_no_row` / `orphan_terminal_row` filed as work items severity high; `ghost_row` reported-not-filed — nothing billing, decommission self-heals it; a 30-min `grace_minutes` absorbs the legitimate provision window, but an instance with NO parseable `createdAt` cannot hide behind it). Scheduled every 6h by `thunder-orphan-scan` (no pre_query = always fires; shares `thunder-lifecycle` concurrency group with the reaper so a scan never snapshots a reap mid-flight). **Deliberately read-and-report: no remediation authority** — decommission needs the very row an orphan lacks, and killing vendor boxes off a list comparison is authority this scan refused (the considered auto-pause of `thunder_config` was dropped for the same reason: it would not stop the orphan billing, only block innocent lanes).
- **work-item contract (per the RFC_010 ruling):** `item_type='thunder_orphan'` on the `system.internal` site, `item_key='thunder_orphan:<vendor id>'`, pipeline `maintenance`, severity high, targetless `ON CONFLICT DO NOTHING` against `idx_swi_dedup`. **Sole producer:** `reconcile_thunder_instances`. **No automated consumer** — fleet-wide `detected→triaged` promotion is dead (the finetuning repair thread's finding), so the human path is the product: the Error-level `THUNDER ORPHAN DETECTED` log line, the open-items query in RUNBOOK §1b, and the summary text telling the operator to kill the box at the Thunder console.
- **sources:** internal/adapters/thunder/adapter.go (handleListInstances); platform/orchestration/actions/thunder_list_dispatch.go; platform/orchestration/actions/thunder_reconcile_action.go (+_test); docs/agent_docs/sql_for_agents/342_thunder_orphan_scan.sql
- **relations:** Thunder adapter (FTW-040); reaper + monitor lifecycle (FTW-038); finetuning_uk_service RUNBOOK §1b (the manual check this automates); bugs_open/186 (the drill campaign that exposed the gap)
- **verify-later:** first COMPLETED thunder-orphan-scan orchestration with truthful counts (check against §1b's manual API call the same day — a 0-findings run must prove it LOOKED); the filing path the first time a real orphan appears

### FTW-043 — Provision claim: correlation-keyed idempotency for a side-effecting adapter action
- **status:** built + mutation-proven, **NOT live** (needs `make build-thunder-adapter` + a whole-fleet `make release`, which the owner runs; migration 396 must be applied first). Provisioning stays PAUSED until then.
- **status-evidence:** `provision_idempotency_test.go` replays the real shape (same `correlation_id`, **different** `request_id`) and asserts on **creates at the vendor**, not on rows or errors. **Proven able to fail:** with the refusal branch mutated out, the test reports `CreateInstance called 2 times for one logical request`; restored, it passes. Migration 396 carries its own `DO`/`RAISE` verify block that *induces* a duplicate and fails the COMMIT if the constraint accepts it — a PK named in the catalogue is not evidence that it refuses anything.
- **what:** `thunder_provision_claims` (PK `correlation_id`) + `store.TakeProvisionClaim`, a claim taken **before** the vendor call so a duplicate provision is *unrepresentable* rather than detected after the money is spent. One statement — `INSERT ... ON CONFLICT (correlation_id) DO UPDATE SET attempts = attempts + 1 RETURNING ..., (xmax = 0) AS inserted` — so claim and count cannot race, and Postgres's own `xmax` decides whether this INSERT inserted rather than a count we compute. A held claim returns `ErrProvisionDuplicate`, which the adapter maps to `provision_duplicate` / **`error_unrecoverable`**: marking it recoverable would ask the chassis to retry the very thing that builds the second GPU. **A failed attempt KEEPS its claim** — releasing it would restore the exact loop this closes — so a transient vendor failure now ends the workflow instead of self-healing; bounded retry (`attempts <= N`) is the stated next iteration. A request with **no** `correlation_id` is refused, not provisioned: no key, no dedup, and this path spends money. Second use: the claim row is the durable record of a FAILED provision that `bugs_open/258` defect 3 says does not exist (status + `last_error` + vendor id survive pod-log rotation).
- **why `correlation_id` and not `request_id`, which is the trap:** the chassis retry driver (`retryExpiredAwaitedRequest`, budget `RetryVersion < 3`) re-executes a step whose await passed `timeout_seconds`, and each execution runs the dispatch action afresh — minting a **new** `request_id` (`thunder_provision_dispatch.go:99`). Measured: orchestration `8c5bf926`, 4 awaited rows, **4 distinct `request_id`s**, one `correlation_id`, each sent ~1s after the previous row's `timeout_at`. `correlation_id` is the only stable identifier across the attempts. This also **disproves** Kafka redelivery, which replays identical bytes and would hold `request_id` constant — the mechanism `bugs_open/259` was originally filed under, and corrected in place.
- **why its own table, not a column on `thunder_instances`:** a pre-create claim row there would carry no real vendor id, and `reconcile_thunder_instances` classifies a live row absent at the vendor as a `ghost_row` (`thunder_reconcile_action.go:204-219`) — so every in-flight provision would file a spurious finding against FTW-042. The claims table has no existing readers, so it perturbs nothing.
- **consumer notice (owner ruling 2026-07-29 §3):** the sole producer of `provision_instance` is **gpu-provisioner**, via `dispatch_thunder_provision` (verified by grep over the action registry; no other `agent_definitions` row dispatches it). Its guarantee CHANGES: a second `provision_instance` under an already-attempted correlation is now REFUSED rather than served. That is the point, but a workflow relying on the retry driver to re-provision after a transient failure will now fail fast instead of silently spending.
- **sources:** `docs/agent_docs/sql_for_agents/396_thunder_provision_claims.sql` (+ `_ROLLBACK`); `internal/adapters/thunder/store/claims.go`; `internal/adapters/thunder/provision_action.go` (claim between gate checks and `CreateInstance`); `internal/adapters/thunder/adapter.go` (header plumbing + `provision_duplicate` arm); `internal/adapters/thunder/provision_idempotency_test.go`
- **relations:** `bugs_open/259_…_one_provision_request_builds_several_billable_gpus` (the case, and the corrected mechanism); `bugs_open/258` defect 2 (the slow provision that makes the await expire — this fix stops duplicate *billing*, it does not make an a6000 provision succeed) and defect 3 (the missing audit trail this supplies); FTW-042 (the orphan sweep the table shape deliberately avoids disturbing); FTW-040 (the adapter)
- **verify-later:** first live run after the roll — one correlation, `attempts` > 1 with exactly ONE `created`/`succeeded` claim and at most one instance at the vendor; whether "a failed attempt keeps its claim" proves too strict in practice once 258 defect 2 is fixed

### FTW-044 — Vendor-derived provisioning defaults + a tunable wait deadline
- **status:** **LIVE** 2026-08-15 — thunder-adapter `v1.0.1301` / stamp `0115f2b45` (`236810e4e` an ancestor) **and** migration 400 applied (`provision_wait_timeout_seconds = 540`). Council submitted `d24f9829-0a3f-47a8-bdcb-4b63ced63f1b`. ⚠ **Both halves were needed** — for ~44h the binary was live without the column and silently used the compiled-in 5 minutes, i.e. defect 2 was unfixed in behaviour while looking shipped. Not yet demonstrated by a real provision.
- **status-evidence:** `provision_defaults_test.go` asserts `cpu_cores` **as sent to Thunder** across the whole live catalogue (fixture = `GET /v1/specs`, re-measured 2026-08-13). **Proven able to fail:** restoring the old constant fails 7 of 8 cases, the sole survivor being `h100_x1_prototyping` — the one spec that accepts 4, which is the asymmetry that hid the bug. Migration 400 carries a `DO`/`RAISE` block that INDUCES a check violation and also refuses a default above the 600s await.
- **what:** Two mechanisms other lanes can use, both in `internal/adapters/thunder`. (1) **`api.Client.GetSpecs`** + `api.Spec` / `SpecKey(gpu,count,mode)` / `Spec.LowestValidVCPUs()` / `Spec.AcceptsVCPUs(n)` — Thunder's catalogue as the authority on what it will actually sell, replacing a hardcoded `cpu_cores: 4` that **9 of 11 single-GPU specs reject** (`bugs_open/258` defect 1). Consulted only to fill a blank, so an explicit request is never blocked by a `/specs` outage; **refuses rather than falling back**, because falling back to a constant is the defect. (2) **`thunder_config.provision_wait_timeout_seconds`** (migration 400, default 540s, CHECK 60–1800, plus a 30-min ceiling re-enforced in Go) — the wait-for-RUNNING deadline as live config, so a slow-booting box is no longer destroyed by a compiled-in 5 minutes that no measured a6000 boot met.
- **⚠ the landmine this carries:** the adapter's wait **must stay below the dispatching step's `timeout_seconds`** (600s on `gpu-provisioner`'s `dispatch_provision`). Above it, the failure mode is not a timeout but a *quiet success*: the await expires, the retry driver re-dispatches, FTW-043's claim guard correctly refuses the duplicate, and **the workflow reports FAILED while a real billed instance runs on unwatched**. The adapter cannot see that step's config, so this is a stated invariant (migration header + column COMMENT + a test), not a mechanical one. Mechanising it means either a startup check reading the `gpu-provisioner` row, or moving the await timeout into `thunder_config` too.
- **ordering that made this possible:** defect 2 could not be fixed before `bugs_open/259`. A longer block used to mean *more* duplicate billing, so raising the wait before the claim guard was live would have made things worse. FTW-043 went live in `v1.0.1295` (stamp `69612d692`, confirmed by `git merge-base --is-ancestor`), which is what made this safe.
- **migration-independence:** the new column is read via `(to_jsonb(t)->>'provision_wait_timeout_seconds')::int`, not a bare column reference — a bare reference would make `store.LoadConfig`, and so every provision/decommission/reaper run, fail outright wherever 400 is unapplied. An unmigrated database degrades to the compiled-in default instead.
- **sources:** `internal/adapters/thunder/api/types.go` (Spec, SpecsResponse, SpecKey, LowestValidVCPUs, AcceptsVCPUs); `internal/adapters/thunder/api/client.go` (GetSpecs); `internal/adapters/thunder/provision_action.go` (`lowestValidVCPUs`, `resolveWaitTimeout`, `provisionWaitTimeoutCeiling`); `internal/adapters/thunder/store/config.go`; `docs/agent_docs/sql_for_agents/400_thunder_provision_wait_timeout.sql` (+ `_ROLLBACK`); `internal/adapters/thunder/provision_defaults_test.go`
- **relations:** FTW-043 (the claim guard that had to ship first); `bugs_open/258` defects 1 and 2 (defect 3 was closed as a side effect of FTW-043's claims table); FTW-040 (the adapter); FTW-042 (the orphan sweep whose 30-min grace absorbs the boot window this lengthens)
- **verify-later:** first live provision after the roll — the adapter's own `Resolved vCPU count from Thunder specs` line naming the spec key and options, a6000 reaching `running` without being deleted, and **how long an a6000 actually takes to boot**, which is still unknown (both prior attempts were killed at 5 min while STARTING, so `> 5 min` and nothing more)
