...
- **status:** built, NOT yet live (committed 2026-08-08; inert until the agent-chassis + thunder-adapter images roll and `sql_for_agents/342` is applied — in that order)
- **status-evidence:** code + seed in one commit with this entry; no image roll has shipped it; the seed's own header carries the ordering check.
