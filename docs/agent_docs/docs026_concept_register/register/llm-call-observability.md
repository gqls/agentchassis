...
- **status:** built, unrolled (code + migration committed 2026-08-10; inert until the chassis image is rebuilt AND at least one caller adds the marker to its template)
- **status-evidence:** `platform/aiservice/anthropic.go` (const + split in `generate`), 6 wire-shape tests in `prompt_caching_test.go` (two of them mutation-proven to fail — cache_control on the varying block, and caching applied to unmarked prompts), migration `376_llm_call_log_cache_token_columns.sql` applied live 2026-08-10 with a DO/RAISE verify whose control was induced first. **No caller uses the marker yet** — council-gate's 17 templates are the intended first consumer.
- **what:** A sentinel string `<!--CACHE_BREAKPOINT-->` that a prompt template may contain. The Anthropic client splits on it and sends `[shared prefix (cache_control ephemeral, ttl 1h)][per-call suffix]` instead of one flat user-message string. Absent the marker the request is byte-identical to before, so the ~40 other agents on this seam are structurally unaffected — the unsafe default is OFF, per the RFC_010 owner ruling that new authority on a shared seam ships as an opt-in field rather than a documented contract. Reads back `cache_creation_input_tokens` / `cache_read_input_tokens` (previously discarded) into `options.__usage_*`, and `llm_call_logger.cacheTelemetry` persists them to the two new columns.
...
- **sources:** `platform/aiservice/anthropic.go`; `platform/aiservice/prompt_caching_test.go`; `platform/orchestration/actions/llm_call_logger.go` (`cacheTelemetry`); `docs/agent_docs/sql_for_agents/376_llm_call_log_cache_token_columns.sql`
...
- **verify-later:** whether any caller actually carries the marker (until one does this is dead code); first observed `cache_read_input_tokens > 0` in production; whether LCO-007's pre_query needs updating for the `input_tokens` meaning change
