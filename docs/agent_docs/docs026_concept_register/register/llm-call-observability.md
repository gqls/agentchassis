...
- **status:** built, committed, **NOT LIVE** (Go — inert until the next chassis roll). The migration half of the same lane IS live.
- **status-evidence:** commit `eb137faed` 2026-08-16, council corr `b684a399-bb4d-4b1f-82f0-fe1429ebdceb`. Nine mutations, all killed; two changed the code (see below). Package suite green on a clean `git archive HEAD` tree.
...
- **verify-later:** post-roll, whether the WARN fires in production and on which of the seven caps — that is a live census nobody has run; and whether any of the other six caps turn out to be biting (each is one query to settle, and none has been checked)
