...
- **relations:** component library (TLIB-015); component quality tracking (tool-lifecycle TL-024); Component selector architecture (TLIB-003); **derived capability, the live replacement for content_shape (site-plan-and-reconciler PLAN-053)**
- **verify-later:** ~~selector Go code reading these columns~~ **ANSWERED 2026-08-24 — see below**; non-NULL section_type coverage
- **⚠ AMENDED 2026-08-24 (`bugs_open/381`): this entry's own verify-later is now answered, and the answer is that HALF THIS BLOCK IS DEAD.** `[MEASURED 2026-08-24]` `suitable_site_types` / `suitable_page_types` ARE read and scored (`component_selector.go:254-258, 307-311`), and `semantic_tags` is read widely — but **`content_shape` and `visual_density` have ZERO Go readers repo-wide**, are **omitted from the birth INSERT** (`store_generated_component_action.go:634` writes 19 columns and neither of these), are NULL on **128 of 151** active section rows, and `content_shape` has no CHECK so its live vocabulary has drifted beyond the four values documented above (`series`, `sequence`, `mixed`). Worse than unused: **12 rows declare `structured_list` while their `html_template` contains no list markup at all**, so reading the column tells you a prose component can render a list. The status above stays `deployed` because the ALTER and backfill genuinely happened — but "deployed" describes the SCHEMA, not a live mechanism, and a reader looking for component capability must not build on these two columns. The live answer is `component_expresses(html_template, input_schema)` (PLAN-053, migration 591), which derives capability from the template and schema so it cannot drift. Full trap: `LANDMINES.md`, "content_shape and visual_density look like the component-capability metadata you want".
