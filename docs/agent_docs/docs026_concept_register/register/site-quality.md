...
- **verify-later:** first manual run's spawned pod must receive IMAGE_BUCKET (the grant's real proof — the shared chassis correctly has it UNSET); the before/after discrimination control around the leopardess hero batch (a critic that reports the same before and after a real visual change is measuring nothing); Gemini vs Claude model trial (owner decision says trial both).
