...
- ⚠ **CORRECTED 2026-08-20 — "deep-merges partials so every row is self-contained" was TRUE OF THE DOCUMENT AND FALSE OF THE BRIEF, for four months.** `content_direction` carries a derived `formatted` field which is the ONLY part of the spec the page writer reads (its prompt names `content_direction.formatted` and four other fields; there is no path to the structured keys). That field was computed from the **incoming partial before the merge** and, being a scalar, won the merge outright — so a partial write left the document complete and the brief describing only the newly-written keys. Three live sites were serving a fragment from 2026-04-18 to 2026-08-20, one at 5 of 18 keys. **`bugs_open/327`, diagnosed CONFIRMED by the `090` loop** (`8be5f6e9-…`). **Fixed 2026-08-20** — `formatted` is now built from `merged`, and the renderer sorts its keys so an identical spec renders identically (it used to iterate a Go map, making any brief-to-brief diff meaningless). **The fix is COMMITTED AND INERT until the next chassis roll**, and the entry above should not be read as describing live behaviour until then. Landmines: both traps in `LANDMINES.md`, 2026-08-19.
- **the general lesson this entry is now the worked example of:** a spec store can be perfectly self-consistent and still lie to its consumer, when the consumer reads a DERIVED field rather than the store. "Self-contained and pruning-safe" was a claim about the row; nobody had checked it was a claim about the brief.
- **sources:** 021 full; P1#Site Specification System; docs/agent_docs/sql_for_tables/019_site_specs_page_component_history.sql; docs/agent_docs/sql_for_tables/bk_site_specs.sql; `bugs_open/327`; `platform/orchestration/actions/site_spec_formatted_from_merged_test.go`
- **relations:** doc 028 ownership model (SPEC-004); strategic-vs-plan-time split (doc 030); dream spec / per-item status (SPEC-001); **CQ-025** (`audit_writer_brief.py`, which measures the document-vs-brief gap this correction is about)
- **verify-later:** ~~write_site_spec action~~ read 2026-08-20, and the reading is the correction above; pinned enforcement in writers; backfill completeness for legacy sites; **whether the three fragment briefs get restored, which is a CONTENT decision for each site's lane and not part of the code fix**
