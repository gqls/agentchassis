...
- **status:** built 2026-08-22 and exercised against the live estate (43 normalised codes observed, 0 findings, 32 unruled). Repo half live on merge; the daily CronJob is **phase 2, not built** — until it exists this protects an author who runs it, which is the same state `--optional-explicit-wires` shipped in.
...
- **verify-later:** phase 2 — the daily CronJob (`finding-code-registry-check`, image before overlay, one `doc_notes` row per run including clean ones). Phase 3 — the `unruled` count going down; it is the progress metric and it started at 32.
