...
_Concept count retired 2026-08-09 — derived, not stored; run the drift pair in `000_concept_index.md`, or read `concept-register-drift-check`'s daily row (DOC-074)._ consolidated from 120 raw extractions across units U01, U02, U03,
...
### DBG-075 — the finding-code registry: every `agent_error_log` code declares a disposition, checked against the LIVE TABLE
- **status:** built 2026-08-22 and exercised against the live estate; **phase 2 (the daily CronJob `finding-code-registry-check`, 07:30 UTC) BUILT AND PUSHED 2026-08-23 at `v1.0.1331`, awaiting the owner's fleet release to deploy** — the manifests, image, makefile membership (`RELEASE_IMAGES` + `AGENT_DEPLOY_SERVICES`) and `.dockerignore` un-ignore are all committed. Live reading 2026-08-23 after the owner ratified B1 batch 1: **42 codes observed, 0 findings, 25 unruled (exactly at the cap, lowered 32 → 25 in the ruling commit), 11 registered-but-unobserved, retention parity 0 disagreements**.
- **status-evidence:** `scripts/audit-finding-codes.sh` exits 0 against the live table; mutation-proved both directions — appending `TEST_UNREGISTERED_X` to the observed list yields exactly one `undeclared` finding and exit 1, removing it returns to exit 0; a `consumed` entry repointed at a real file that does not name its code yields `reader-does-not-name-code` while the true reader passes. 14 unit tests + 5 in `platform/orchestration/actions`.
- **what:** `bugs_open/358`. Detectors, guards and audits record what they notice as an `agent_error_log` row under a named `error_code`; most such codes had no automated reader, and migration 466's hourly `database-cleanup` `pre_query` deletes an unresolved row at 30 days. The fix is not a reader per code — some are legitimately human evidence, some are time-boxed instrumentation, and operational plumbing is *correctly* consumed by the generic newest-N diagnostic readers. It is that a code cannot enter the estate with **no declared disposition** and nobody notice. One registry (`docs/agent_docs/docs024_key_docs_latest/architecture_review/finding_code_registry.json`, beside its two sibling acks files) declares each code as `consumed` / `instrumented` / `human-evidence` / `operational`, or `unruled` — a **counted backlog, listed at exit 0**, never an exemption. `cmd/config-key-audit --finding-codes` (mode 16) checks it. Every required field is unsatisfiable by typing, per RFC_029 §10.15's lesson: a `consumed` reader is **verified to contain the code**, an `instrumented` entry carries a `review_by` that expires, a `human-evidence` reason must name the retention window it accepts. Also retires two hand-maintained code rosters that had both gone stale (`discovery_checks_error_log_test.go`, `save_sections_content_data_links_test.go`) — their prefix-disjointness rule is kept and promoted to a property of the whole registry, because the estate has live `LIKE` queries on `tool_crosslink_not_emitted%` and `component_validation_%`.
- **LANDMINE — `agenterrors.go:3` says "The ONE writer against `agent_error_log`" and it is NO LONGER TRUE.** RFC_012 did retire nineteen hand-copied INSERTs into it, but **five paths insert rows as of 2026-08-22** (re-census before quoting — a count goes stale by ADDITION, owner ruling 2026-08-22): the seam; `store_generated_component_action.go:1439` (own INSERT, kept deliberately — a council round's edit-quality and guardian seats objected to consolidating it, reason at `:1428`); `internal/agents/contentcreator/claims_guard.go:184`, which **cannot** use it (`*pgxpool.Pool` at `agent.go:92` against `Write`'s `*sql.DB` — a type-level barrier); `sql_for_agents/214_build_dispatch_watchdog.sql:108`; and `cmd/content-loss-check/main.go:292`. **A check placed at the seam reads clean while four of five writers walk past** — which is why the authority here is `SELECT DISTINCT error_code`, blind to none of them and parsing nothing. Two further traps: **grep the CONSTANT, not just the literal** (`page_build_failure_guard.go:131` binds a const to `$1`, so a literal-grep verdicts its code unread), and codes also reach the table as **positional arguments** to `LogActionError`, invisible to an `ErrorCode:` grep.
- **⚠ `BUILD_DISPATCH_STALLED`'s "closed loop" does not exist live.** `bugs_open/358` §2.2 lists it as one of three codes with an automated reader; migration `214` has **never been applied** (0 rows in `schema_migrations` for `214%`) and no `scheduled_tasks` row named `build-dispatch-watchdog` exists. Both halves live inside that unapplied task's `pre_query`. Its zero row count reads as "quiet" and means "absent" — which is why the registry's registered-minus-observed direction is **reported** rather than dropped, and why it is report-only: from that side a quiet code and a dead mechanism are indistinguishable.
- **sources:** `bugs_open/358_HANDOFF_2026-08-22_...md`; `docs/agent_docs/docs024_key_docs_latest/bugfix_358_unread_finding_codes/PLAN_2026-08-22_unread_finding_codes.md`; council `be1fd678-0836-4f32-90a6-8927b2463fee`; `090` corr `c965bfec-993a-4b2b-88ba-d44549c81df1` (UNVERIFIABLE, scope-not-narrowing — **not** a refutation: the loop's static tier is `.go`-only and index-backed, so the SQL writer was outside its corpus entirely and its `pgxpool` search returned 0 from an index holding no `type` kinds)
- **relations:** DBG-015 (the table as a primary forensic source); RFC_012 (the seam this corrects); RFC_029 (the two `instrumented` resolver codes and the "ack satisfiable by typing" rule); WFA-013 / `optional_key_budget_acks.json` (the acks pattern copied here); `bugs_open/355` + `cmd/content-loss-check` (the worked `consumed` exemplar, and the first user of the `resolved` triple in the table's history)
- **⚠ THE SCHEDULED JOB RUNS `--no-source`, AND THAT IS A DELIBERATE SPLIT, NOT A WEAKENING.** Two arms open the Go file a `consumed` entry names, and **no check image in this estate ships a source tree** — so the naive deploy yields 5 `reader-unreadable` findings and exit 1 EVERY MORNING against a healthy registry (`[MEASURED 2026-08-23]`). They grade the registry against source, both halves change only by commit, so a daily re-run cannot come out differently than at build time; they now run at commit time via `scripts/check-finding-code-registry.sh` (`.githooks/pre-commit`, hook 5), scoped by a reader list **computed from the registry** rather than hand-kept. Before that they had **no automatic runner at all** — `[MEASURED]` the pre-existing hook's `-run 'BudgetCron'` filter runs four tests and never `TestShippedRegistryIsSelfConsistent`, and the control is that naming it directly runs and passes. **Every run STATES which arms it skipped**, because a skip that looks like a pass is this entry's own subject matter.
- **⚠ THE REGISTRY TRAVELS IN THE IMAGE**, deliberately against the `component-source-vocabulary-check` landmine's *"MOUNT, not a COPY, whenever the file is meant to change"* — a mount goes stale INDEFINITELY until a human remembers `apply -k`, a copy only until the next release, and this service is in `RELEASE_IMAGES` so that is hours. The residual is made visible: the run states the registry's **declared-code count** and the binary's **build commit** (`pkg/buildinfo`, which no check image had used before), so a cluster row disagreeing with a local audit IS the staleness. ⚠ A registry-shipping check also needs a `!` un-ignore line in `.dockerignore` — without it the `COPY` fails at build time, which is the loud direction and is why this is a COPY.
- **verify-later:** the deploy itself — the image is pushed but the CronJob does not exist in the cluster until the fleet release runs. Prove it at the artefact: `kubectl get cronjob finding-code-registry-check -o jsonpath='{.spec.jobTemplate.spec.template.spec.containers[0].image}'`, then a MANUAL job (never wait for the schedule) and its pod's `terminated.exitCode`, then the row under `source='finding-code-registry-check'`. Phase 3 — the `unruled` count going down; it is the progress metric, it started at 32 and is 25 as of 2026-08-23.

### DBG-076 — the advisory-findings audit: does `pattern-check.py`'s output ever change behaviour?
- **status:** built 2026-08-22, run against real history, **and it has ANSWERED its question** — see the measurement below. Hand-run (`scripts/audit-advisory-findings.py`); the CronJob is **NOT built and is not a wiring detail** — see verify-later. The question had been open and unanswerable since 2026-08-02.
- **THE MEASUREMENT `[MEASURED 2026-08-22]`, 5,000 commits / 14 days, baseline pinned `7a3a06377`:** 226 findings replayed → 37 decidable, **8 acted on, 29 unacted**, 91 `file_gone`, 98 `undecidable`. **8/37 = 22% overall, and that is the wrong statistic to quote.** One rule supplies 25 of the 29 unacted (`logged-model-output`); **without it follow-through is 8 of 12 = 67%**, and it alone is **0 of 25**. Exact split: `unrepaired-component-write` 3/3 acted (the `bugs_open/362` writers), `unscoped-component-render` 4/4 acted, `dynamic-item-type` 1/1 acted; unacted are `logged-model-output` 25, `runtime-fill-marker` 2, `silent-reply-drop` 1, `unguarded-migration-insert` 1. All 88 `check_bug_file_duplicated` findings are `file_gone` — bug files that moved to `bugs_closed/`, which is the correct outcome, not damage. **Conclusion: the advisory channel is NOT ignored; it is ignored for ONE rule whose premise (visitor-written text in the prompt) does not hold in the package it fires on. That is a scoping defect in that rule, and a rule which should be ignored trains the channel into noise — it needs a ruling, not a mandatory seam.**
- **status-evidence:** pinned demand control runs on **every** invocation, not only under `--self-test`: it replays commit `d795e10f5` and asserts `shared-ledger-not-appended`, so a zero from the sweep always ships with proof the sweep can see. Mutation-proved both ways — repointing the expectation at a kind that cannot fire gives exit 2 and the sweep refuses to run. The `acted` arm is separately demonstrated reachable by a real case: `unrepaired-component-write` fired on the two tool writers from 2026-08-02, `bugs_open/362` fixed them, and both files verify SILENT at HEAD.
- **what:** `RFC_008` closed with no mandatory write seam, but named its own decisive question and nobody had answered it: *"nobody has ever measured whether any of [pattern-check's advisory rules] changes behaviour."* The RFC's reopen trigger was first pointed at `bugs_open/358`'s finding-code registry and **that was wrong** — 358 can measure `agent_error_log` because codes are ROWS; `pattern-check.py` findings print to a terminal and persist nowhere. The insight that makes this cheap: **the findings are RECOMPUTABLE.** `pattern-check.py --commit <sha>` already replays any past commit, so the record does not need to be WRITTEN at commit time — it is reconstructed from git, over the whole of history, today. For each finding: is the condition still true at HEAD (nobody acted) or gone (fixed or allow-listed)?
- **why NOT the obvious design — a durable row written by the pre-commit hook:** that puts a network write (this estate reaches its DB via `kubectl exec`, on a token that expires every three days) inside the hook whose own header says *"a stray non-zero exit here stops the whole fleet committing. Keep it boring"*; and it measures only commits made after it ships, answering the question in weeks rather than now. **Nothing in DBG-076 runs at commit time**; `pattern-check.py` is imported, never modified. The one change to it was hoisting its check roster to a module-level `CHECKS` so both it and the auditor read ONE list.
- **⚠ LANDMINE — a diff-scoped check replayed statically is SILENT, and silence is byte-identical to "someone fixed it".** Most of these checks read the *diff*; against HEAD there is no diff, so a naive auditor marks every one of them `acted` and reports a triumphant, meaningless number. The discriminator is empirical, per run, and needs no hand-classification: take findings whose **file is unchanged** between the finding's commit and HEAD — the condition MUST still hold — and see whether the check still fires. Fires ⇒ state-evaluable, believe its verdicts. Silent ⇒ **diff-only**, and every one of its findings is reported `undecidable`, never `acted`. A check with no unchanged-file case is undecidable by default. The per-check evidence is printed so a reader can see which checks carried the run.
- **⚠ AGE IS A CONFOUND IN ONE DIRECTION AND THE WINDOW IN THE OTHER.** On this tree 400 commits is ~1.5 days (4,967 in 14 days), so a naive sweep reports "0% acted on" about findings nobody has yet had the chance to act on — the tool therefore separates **ripe** (first fired ≥2 days ago) from the rest and refuses to answer at all when no finding is ripe. Conversely a finding that first fired BEFORE the window and was fixed INSIDE it is never replayed, so `acted` **under-counts**; `bugs_open/362` is the worked instance.
- **sources:** `scripts/audit-advisory-findings.py`; `scripts/pattern-check.py` (`CHECKS`); `RFC_008`'s decision record and its reopen trigger 2; `bugs_open/362` (the worked `acted` case, and where the audit's first real finding — a THIRD unrepaired `rendered_html` writer, `create_tool_component_regenerate.go:319` — was routed)
- **relations:** DBG-075 (the sibling question for `agent_error_log`, and the reason this one exists separately: that registry is DB-authoritative because its subjects are rows, this one is git-authoritative because its subjects are not); `bugs_open/358` B2 (the same "is anyone reading this?" shape, different channel); `a-post-fix-zero-needs-a-demand-control` (why the pinned control runs on every invocation); LNK-027 / `check_unrepaired_component_write` (the rule whose follow-through RFC_008 actually cared about)
- **verify-later:** the CronJob. Every daily check in this estate runs `postgres:16-alpine` + a script with **no git clone and no repo**, and this auditor is git-shaped to its core — scheduling it means shipping a clone into an image or mounting the tree, which is a real decision, not wiring. Until then this protects an author who runs it, the same state DBG-075 and `--optional-explicit-wires` shipped in. Second: the `unacted` list is a backlog and its trend is the metric.
