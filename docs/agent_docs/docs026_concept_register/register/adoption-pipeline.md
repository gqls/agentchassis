...
- **status:** built and proven offline over all 27 pages of one site; **first page written to the database 2026-08-02, deploy in flight** — not yet proven at the served bytes, and the Python mirror of the Go assembler is deliberately unfalsified until that diff runs
...
- **verify-later:** the mirror. `assemble_mirror.py` is a second implementation of `assemblePage` and agrees with the harness by construction; its scheduled test is the first page's real rendered output diffed against `<work>/predicted/`. Until that diff is recorded here, treat every "27/27 passes" in this entry as conditional on it.
