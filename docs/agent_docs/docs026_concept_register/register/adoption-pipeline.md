...
_Concept count retired 2026-08-09 — derived, not stored; run the drift pair in `000_concept_index.md`, or read `concept-register-drift-check`'s daily row (DOC-074). It said **37** and the file held **40**._ (ADO-037 added 2026-07-30, postdating the freeze), consolidated from 106 raw extractions (the cluster input file contains
...
- **verify-later:** ~~the per-page sha256 gate … the one unproven link is whether firecrawl's `rawHtml` is byte-verbatim~~ **RESOLVED 2026-07-31, and the answer is NO — see ADO-038.** Measured twice on two sites: loancalculator.co.uk matched on **0 of 27** pages (each +8,900–9,060 B, `nav.js` output baked in) and loanandmortgagecalculator.co.uk on **0 of 41**. So `rawHtml` is the serialised post-JavaScript DOM and the mechanism above is rebuild-safe *only once the bytes are repaired*; the mechanism held 41/41, the byte source failed 41/41. Still open: whether `redirects` ever gains a consumer so verbatim adoption can preserve moved URLs without hand-written meta-refresh stubs (G5).

### ADO-038 — The adoption byte gate: proving (and repairing) an adopted component against the deploy repo
- **status:** built and exercised end-to-end on one site; not wired into any agent — it is a script a human runs
- **status-evidence:** `docs024_key_docs_latest/loanandmortgagecalculator_couk/gate_component_bytes.py`, run against the real loanandmortgagecalculator.co.uk adoption 2026-07-31: **41/41 mismatched**, repaired 41 (`UPDATE 1` ×41, `UPDATE 0` ×0), gate re-run **41/41 byte-exact**, then a released `page_rerender` redeployed with an **empty diffstat**.
- **what:** The missing half of ADO-037. `fidelity=locked` preserves a site *as the crawler saw it*, which is not what the origin serves, so a locked adoption needs a gate between "components written" and "rerender allowed to deploy them". This digests every `page_components.rendered_html` **in Postgres** (`sha256(convert_to(col,'UTF8'))`), compares against the corresponding deploy-repo file resolved through the same rule as `datahelpers.PageFilePathFromURL`, and with `--repair` loads the repo bytes back in (`convert_from(decode(…,'base64'),'UTF8')`) under the shared lock predicate, **counting `RowsAffected`** so a lock-suppressed write cannot pass as success. What the crawl actually did, characterised rather than assumed: every relative `href`/`src` absolutised; `<meta charset>` → `http-equiv`; `&#9776;` decoded; **the skip link turned into an absolute URL that reloads the page instead of jumping down it** (an accessibility regression on all 41 pages); and `mortgages/repayment.html` **+11,432 B** because its script builds a 24-row amortisation table on load and the crawl serialised the result.
- **landmine:** **`content_data.sha256` is NOT a fidelity gate** — `adopt_verbatim` computes it over the bytes it is storing, so it is self-consistent by construction and agrees with itself on a mutated page. **`sha256(text::bytea)` is invalid SQL**; `decode(…,'base64')` already *is* bytea. And **the hold is the hard part, not the gate**: `adopt_verbatim` creates the `page_rerender` items already `status='triaged'` inside the adoption transaction and `build-pipeline-trigger` runs at `interval_seconds=120`, so the window is under two minutes — a poller flipping them to `deferred` caught 41 in one second. `deferred` is chosen because it is **not** in `workItemTerminalStatuses`, so the row keeps its `idx_swi_dedup` slot. **`sites.locked_at` does NOT hold dispatch**, despite `213_dispatch_gate_matches_dispatcher.sql:106` containing `AND s.locked_at IS NULL` — the live `build-pipeline-trigger` row has no such clause.
- **consumers to tell:** any workstream adopting a repo-built site with `fidelity=locked` — which is now the only sanctioned way to adopt a site whose bytes matter. And a **standing consequence for all of them**: after adoption, `page_components` and the deploy repo are two independent writers for the same files (a verbatim `page_rerender` commits `Rerender: <file>` into `gqls/sites`), so any change to a site's builder must be followed by `--repair` or the next rerender silently reverts it.
- **sources:** `gate_component_bytes.py`; `docs024_key_docs_latest/loanandmortgagecalculator_couk/RUNBOOK…md` §9–10; `NOTES…md` (2026-07-31 session 2); `LANDMINES.md` ("after a locked adoption … TWO writers")
- **relations:** ADO-037 (this closes its `verify-later`); `cmd/webdesignport` (same owned-page row shapes, and D1's ruling that a second bespoke *porter* is wrong — a repeatable gate-and-repair is not one); LOCK-007 / `pageComponentAgentWritableSQL` (the predicate the repair reuses rather than re-deriving); `bugs_open/084` (owned pages sit outside the browser-verification ladder, which is why the byte gate is the only check they get)
- **verify-later:** whether this should become an action inside the locked path rather than a script a human remembers to run — the honest argument for wiring it in is that the failure mode is silent and fleet-wide; the argument against is that its byte source is a local repo the platform cannot read (G1, and the read token cannot see `gqls/sites`)

### ADO-039 — Decomposing a verbatim-adopted page into editable prose + a proven tool component
- **status:** LIVE. All 27 pages of loancalculator.co.uk decomposed 2026-08-02 (63 rows: 51 prose + 12 tool, 0 verbatim); 25 rendered and **every one byte-identical to the mirror's prediction**, so the mirror is falsified-and-survived rather than merely consistent
- **status-evidence:** `docs024_key_docs_latest/loancalculator_couk/decompose/` — `verify_assembled.py` reports `static checks pass on all 27 page(s)` and `all 12 calculator(s) reproduce their golden values in the ASSEMBLED page`; a guide page screenshots pixel-identical to production apart from the added footer
- **what:** The exit from ADO-037. Verbatim adoption preserves a site by making every page ONE component holding a complete document, which is safe and completely uneditable — no writer loop, no restyle, no per-section anything. This turns that single row back into ordered `page_components` rows: each block of the original becomes an editable `ported-prose` section, lifted byte-for-byte, and the calculator becomes a real `component_level='tool'` row whose values are proven identical to the original's across three input vectors. **The splitting rule is `decompose_prover.py`'s** (descend to the smallest subtree containing every script-addressed node; siblings become prose), certified there against P1–P6 before any of this was written; `decompose_pages.py` re-walks the same tree — importing it, not forking it — to emit blocks in DOCUMENT ORDER, which the prover discards and which is exactly what `position` needs. Three chrome rows (`site_components` head/header/footer) are a hard prerequisite, because assembly supplies the document wrapper the dissolved page wrapper used to provide.
- **landmine:** **The verbatim flip is the ROW COUNT, not a flag** — `loadVerbatimPageHTML` requires exactly one row, so ADDING a section beside the verbatim row switches the page to assembly and leaves a complete `<!DOCTYPE>` document as one of its sections. Replace in one transaction, never insert alongside. Second landmine: **a rule that reads only INLINE scripts is blind on a page whose logic is in a `<script src>`** — `/index.html` keeps `calculateLoan()` in `global.js`, so the calculator's whole results box decomposed as editable prose and the prover's own P4 passed, because P4 asks about inline scripts and that page's inline script addressed only the inputs. Third: **`sectionHasVisibleContent` silently drops any row with ≤10 visible characters** and returns success, so a short prose block vanishes with nothing reported.
- **consumers to tell:** anything that assumes a `rebuild_policy='owned'` site ships stored bytes — after this, an owned page may ASSEMBLE, so the tool-doc strip, outbound link repair, JSON-LD and component-CSS injection all become live for it; `site_components` for an adopted site stops being decorative the moment the first page decomposes (see ADO-040); the writer loop gains 27 pages of editable prose it previously could not see
- **sources:** `docs024_key_docs_latest/loancalculator_couk/decompose_pages.py`, `decompose/{assemble_mirror,verify_assembled,load_decomposition,load_chrome,update_component}.py`, `decompose/prepare_work.sh`, `chrome/{head,header,footer}.html`; `RUNBOOK_loancalculator_couk.md` §"Proving the decomposition BEFORE writing a row"
- **relations:** ADO-037 (what this reverses, deliberately and per-page); ADO-038 (must pass FIRST — decomposing mutated bytes decomposes the mutation); TL-037/TL-038 (the tool-equivalence gate and `computed_values`, which are what make substituting a calculator defensible rather than hopeful); CMP-`ported-prose` below
- ~~**verify-later:** the mirror … treat every "27/27 passes" as conditional on it.~~ **DISCHARGED 2026-08-02.** `assemble_mirror.py` wrote each page's predicted bytes before any row existed; the Go assembler then served them **byte-identical, same md5** — 25 of 25 rendered pages, including a 5-row page with a tool at position 3 and the structurally hardest tool. Go's `json.Marshal` HTML-escaping and its recursive map-key sort were the two details most likely to diverge and did not. **The offline 27/27 is a result, not a hypothesis.** Separately: driving the 12 live calculators against the 07-31 golden shows 41 diverging keys and **zero changed values** — every one is a control that gained an `id` (appearing as named, vanishing as positional) plus the nav placeholder; and a fresh baseline (`GOLDEN_2026-08-02_decomposed.json`) raises asserted fields **94 → 119** with 0 of the 94 shared keys moving. Still genuinely open: the site remains `rebuild_policy='owned'`, so the improvement loop cannot touch it — flipping that is a separate owner decision now that the pages are in pieces.

### ADO-040 — `ported-prose`: the editable half of a decomposed page
- **status:** created 2026-08-02 (`content_components.function='ported-prose'`, `component_level='section'`); in use on one page, deploy in flight
- **what:** A deliberately minimal section component — `<section class="ported-prose" data-component="ported-prose">{{.content}}</section>` — carrying one `html` field. It exists so a decomposed page's prose is a real, editable, re-renderable row rather than frozen markup, and it is generic rather than site-specific on purpose: **every verbatim-adopted site that later wants to be editable needs exactly this**, and it pairs with `ported-page` as the before/after of the same story. Its `llm_guidance` states the safety property a writer needs: the block contains no form control and no element any script on the page addresses, so rewriting it cannot break a calculator — that property is enforced upstream by the splitting rule, not by the component.
- **landmine:** it deliberately carries **no wrapper geometry and no styling**. The page's container geometry belongs to the PAGE, not to each section — giving every prose section its own `.container` stacks N sets of vertical padding, so the gaps between blocks grow with the section count where the original had zero. Put the geometry in the site's head component (one `main { max-width: … }` rule), which is exact for any number of sections.
- **consumers to tell:** `css_snippets.applies_to` keys on generic function names (`card`, `hero`, `section`, `button`); `ported-prose` matches none of them today, so `collectComponentCSS` contributes nothing — adding a snippet for it would start injecting CSS into every decomposed page on the fleet
- **sources:** `docs024_key_docs_latest/loancalculator_couk/decompose/load_decomposition.py` (`PROSE_SCHEMA`, `ensure_prose_component`)
- **relations:** ADO-039 (what creates and uses it); ADO-037 (`ported-page`, the frozen counterpart)

### ADO-041 — `class_count_delta`: the per-class markup-preservation predicate, lane-agnostic
- **status:** BUILT 2026-08-12 and self-proven; **one caller so far is the CLI itself** — it is a predicate other lanes can import, not a wired-in gate. Do not read this as "markup preservation is enforced fleet-wide"; it is not.
- **status-evidence:** `python3 scripts/class_count_delta.py --selftest` passes including its own **induced failure** (a dissolved `.card` must be reported, or the test fails). On real pages: an identical page is silent, one deleted `class=` is caught, and the sibling lane's `tools/consolidation.html` reports **5 false drops without code-stripping and 0 with** — the control that makes the pass meaningful.
- **what:** `counts()` / `strip_code()` / `delta()` / `check()` over two HTML documents, counting **each class NAME separately**. It exists because the three cheaper measurements everyone reaches for first were all tried on the same page on the same day and **all three certified a page that had lost its calculator's panel** (`bugs_open/263`): a class **SET** diff could not see it (the page has four `card`s and one went), an **AGGREGATE** class-attribute count read 18 before and 18 after (two removals offset by two `ported-prose` additions), and a **byte diff against a prediction** was generated from the same manifest that dropped the wrapper, so for that defect class it is structurally incapable of failing. Only `card: 4 -> 3` says it. Two modes: exact-equality (for a seam where every emitted block is a byte slice of the source, so additions are impossible) and drops-only with an additions allowlist (for an assembling seam).
- **landmine:** **`class=` inside a `<script>` body is not markup, and it cuts both ways.** Counting it produces false drops when a decomposition drops the inline script (measured: 5 on `tools/consolidation.html`), and — worse — it can HIDE a real drop, because a JS template emitting `class="card"` offsets a dissolved `.card` exactly the way `ported-prose` did in 263. This module strips `<script>`, `<style>` and comments from both sides; **the lane gate `gate_wrapper_parity.py` does not** (inert today: zero in-scope pages carry a script inside `div#content`, verified 2026-08-12). Second landmine: **an empty side reports a confident zero, and the error has a direction** — an unmatched scope regex or a failed `git show` always says "no problem here". Both sides are asserted non-empty and an unmatched scope is `NOT CHECKED` (exit 2), never a pass.
- **deliberately NOT included: the allowlist.** What may legitimately disappear is per-lane policy (`container` is compensated on the decomposed calculator sites by the head chrome's `main .container` shim; nothing else is), and a shared allowlist means the first lane to widen it weakens the gate for every other lane. Same reasoning as `scripts/truncation_registry.py`, pointing the same way. **A permitted delta is one for which a named, live compensating rule exists** — not one that looked harmless.
- **consumers to tell:** `bugs_open/253` (the Go per-slot component floor `save_sections_component_floor.go` counts class attributes **in aggregate**, which 263's 18→18 proves can net to zero — this is the per-class form it would need); `copy_quality_two_stage`, whose PLAN already requires "class attributes and component boundaries in == out, counted not eyeballed"; `mortgagecalculator_couk_adoption`, whose port pages have no components and whose faithful bytes exist only in a bucket file of unknown shape; the served-page seam of any decomposition lane, where the prediction diff cannot help
- **sources:** `scripts/class_count_delta.py` (`6427f157f`), `docs024_key_docs_latest/markup_preservation_gate/` (the workstream dir — `HANDOFF_2026-08-13_continue_here.md` is the cold start), `bugs_open/263`, `bugs_open/253` (the aggregate-floor contribution), `WRONG_CALLS.md` 2026-08-12 (the netting-out aggregate, and the empty-read that nearly removed the code-stripping)
- **relations:** ADO-039 (the splitting rule whose wrapper defect this measures — its `keep_widget_wrapper` opt-in is the fix, `71fb31a03`); ADO-038 (the byte gate, which is self-consistent by construction in the same way the prediction diff is); ADO-040 (`ported-prose`, the additions this must not count as compensation)
