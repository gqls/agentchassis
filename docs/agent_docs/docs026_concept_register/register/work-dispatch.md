...
- **open review question:** whether the bug file's own first-ranked candidate — "one triage, one owner", i.e. remove the step from the two children — should still happen. This change makes the ordering irrelevant rather than making one ordering mandatory, so it is no longer load-bearing; but three agents promoting site-wide remains a shape where any of them can consume another's rows.
