...
11 concepts. Ten were consolidated from 22 raw extractions (11 unique blocks, each
...
- **status:** deployed — **UPDATED 2026-08-02, was `built, not live`; added 2026-07-31, post-freeze**
...
- **open question:** RESOLVED in part — the Python builder is now the declared TEST ORACLE and manual fallback, not a second authority. But the parity test compares PARSED structures, so it was blind to the two byte-level ways the writers diverge: Go HTML-escapes `<`/`>`/`&` by default, and sorts map keys. Both shipped; the first live commit rewrote all 119 lines to publish a timestamp. Escaping is fixed (`marshalFeedFile`, `SetEscapeHTML(false)`, commit `59f3c67dd`, **needs a roll**); key order is deliberately NOT pinned, because that cost is paid once per change of writer rather than once per day and pinning it means a second hand-maintained copy of every field list. Still open: if the manual fallback is ever used, the two writers rewrite the whole file after each other.
