...
- **status:** partial — **UPDATED 2026-07-31, was `aspirational`**
- **status-evidence:** The emitter now EXISTS: `render_provocation_feed` + the `provocations` pool (VONC-011, commits 572ae8dc6 / 4c11bb9af, migrations 282+283). Still `partial` and not `deployed`, on two counts measured the same night: the running chassis carries the action **0 times** (`strings /app/agent-chassis | grep -c render_provocation_feed` = 0, with `render_news_section` = 3 as a positive control), and the scheduled row is seeded **disabled** pending that image. The GENERATIVE half — scraping topics into provocations — is still unbuilt; what exists selects and publishes from a hand-authored pool. Prior evidence, still true of the generator: Phase-1 diagnostics confirmed "a clean slate — nothing exists yet" (2026-06-25); FX-4 checkbox never ticked; provocations.json hand-committed as of 2026-07-09; RUNNING_NOTES_minilobby 2026-07-11: "There is no Phase-3 emitter yet; all prior commits to the file were hand-made."
...
- **status:** built, not live — **added 2026-07-31, post-freeze**
- **status-evidence:** Migrations 282 (`provocations` table + the 9 live provocations) and 283 (`provocation-feed-publisher` agent + `provocation-feed-refresh` schedule) applied and recorded; commits `572ae8dc6` (action, 14 tests) and `4c11bb9af` (seed). NOT live: running chassis has `render_provocation_feed` **0 times** against a `render_news_section` positive control of 3, and the scheduled row is seeded `enabled=false` until that image ships. Council submission `6612dc0b-8e03-4039-a8c8-fe4fabaaddeb`.
...
- **open question:** two implementations of one feed now exist (Go action + the Python builder). Parity is asserted by a golden-fixture test from the 9 real rows, but the fixture must be regenerated from BOTH sides together — one refreshed from the code under test only ever agrees with it. Whether the Python builder becomes the test oracle or is retired is undecided.
