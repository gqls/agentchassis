...
- **status-evidence:** `platform/orchestration/actions/diagnose_persist_fix_plan_action.go` — `planValidationRefusal`, `planRefusalNoteKind`, `plan_valid` on the success path; `docs/agent_docs/sql_for_agents/272_feature_designer_plan_repair_loop.sql` dry-run against the live row with `COMMIT`→`ROLLBACK` (all four UPDATEs bit, verification block passed, rolled back). 11 tests in `diagnose_plan_refusal_test.go`, all passing, including four end-to-end through `DiagnosePersistFixPlanAction` itself — the helper-only tests hand `problems` in and so cannot prove the action REACHES the refusal path.
...
- **verify-later:** after the roll, pod-grep `plan_validation_refusal` **with a positive control in the same exec** (a tag bump does not imply a rebuild — `bugs_open/153`), then apply `272` and induce the failing branch: fire the designer at a spec whose natural decomposition repeats one file in a stage, and require BOTH a `fix_plan` artifact for the new correlation AND that the run does not end at `complete_refused`. **A green run proves nothing — the failing branch is the whole bug**, and 099's own "how to verify" says the same in more words.
...
- **status:** **built, self-tested, NOT applied to the fleet.** The `--apply` write was refused by the session's permission classifier; the block therefore exists in git and not in any live prompt. `--verify`/dry-run report all five targets as `NEEDS-BLOCK`.
- **status-evidence:** `scripts/apply-seat-length-budget.py` dry run lists the five targets and prints the block; the classifier was exercised against four real live seats and returned `HAND-WRITTEN` (council-gate/review_architecture, correctly refusing to overwrite a hand-authored block), `NEEDS-BLOCK` ×2, and `MISSING` for a non-existent seat.
