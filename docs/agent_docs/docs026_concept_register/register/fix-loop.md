...
- **status:** **DEPLOYED and PROVEN LIVE 2026-07-31** (chassis `v1.0.1216`/`v1.0.1218`, widening pod-grepped on both replicas with an untouched control; config live via migration `272`). Discharging run `FEATURE_CORR 53fff682`: a plan refused on `"stage s1: 2 edits exceeds the per-stage cap 1"` at 18:08:26 was preserved, recorded, routed to `repair_plan`, and a **repaired plan persisted at 18:09:31 — 3 stages carrying all 3 edits** where the original had 2 stages with a 2-edit stage. The designer split the offending stage and lost **no scope**; the run continued into its council rather than `complete_refused`. All four conditions of the corrected verification held. `bugs_open/099` → `bugs_closed/`.
...
- **SECOND CONSUMER, 2026-08-01: `fix-proposer` is now opted in** (`bugs_open/162`, lane `docs024_key_docs_latest/bugfix_162_fix_proposer_plan_repair/`, migration `sql_for_agents/273_fix_proposer_plan_repair_loop.sql`). The open question two lines above — "whether `fix-proposer` should be opted in is deliberately left for its owning lane" — is **answered**: that lane picked up the ready-to-run guarded migration and applied it, so a structurally-refused fix plan now routes to a repair step and back, capped at one round, instead of being discarded. This is the seam working as designed: the mechanism shipped **opt-in**, which is why it could reach production without changing any other consumer's guarantee, and is also why `fix-proposer` stayed exposed until its own lane acted. Re-running `273` now fails with `ERROR: 273: already applied — fix-proposer already has a check_plan_valid step`, which is the cheapest way to confirm it is on. **Consumers as of 2026-08-01: `feature-designer` (272), `fix-proposer` (273).**
- **the query that answers "who consumes this?", because the obvious ones lie:** `orchestration_states` has **no `agent_type` column**, and `agent_definitions` hides the consumer inside a JSON step, so a census by agent type either errors or silently describes the wrong population. Enumerate the steps instead — `jsonb_each(default_config->'workflow'->'steps')`, reading `config->>'repair_step'` and `config->>'max_repair_attempts'` per step and testing `? 'check_plan_valid'` on the steps object. Full statement in that lane's RUNBOOK §1.
...
- **LANDMINE (three, all silent; the third was a defect IN this entry's own tool, found 2026-07-31 and fixed):**
  (0) **A TRUNCATED call has `output_tokens = NULL`**, with the cut stated only in `error_message`. So the truncation count this instrument SHIPPED with — `count(*) FILTER (WHERE output_tokens >= max_tokens)` — could never match one: it reported **4** fleet-wide where the true count was **94**, and six seats read "0 truncations" while having truncated (one of them 51 times). The same implicit `output_tokens IS NOT NULL` also dropped those rows from the p95/peak, so headroom was computed over the calls that did NOT truncate — blindest on exactly the seats that truncate most. Fixed both halves: count from `error_message ILIKE '%stop_reason=max_tokens%'`, and score a truncated call `frac = 1.0` because it did reach its cap. `NOT success` is not a truncation filter either (it also covers API 400s, context-cancelled, dial timeouts).
- **LANDMINE (the two hit during construction):** (1) **`llm_call_log.agent_type` cannot attribute a review call to its council.** Everything before 2026-07-26 14:54 logged `generic`; from 15:03 the same calls log `council-gate`; **`fix-proposer` has never appeared at all.** `WHERE agent_type='council-gate'` therefore discards 1,798 rows of the same population without erroring. Key on (seat, cap) and report `n_holder` — exact for feature-designer and the experience councils, a lower bound for the fix lane. (2) **The denominator changes inside the window.** `max_tokens` is per call and caps were raised mid-window, so a p95 of `output/max(max_tokens)` mixes populations: it renders `review_editquality` as "95% of a 16000 cap" when the 16000-cap rows peak at 62.9%. Compute the ratio per row and join on the seat's current cap.
- **CORRECTION 2026-07-31 to this entry's own rationale.** "Why it exists" argues that counting truncations "reads ~0 today and never fires, because candidate 3 raised the cap on every seat that had actually truncated". The ~0 was real; **that explanation was false** — truncations read ~0 because the instrument could not see them. The decision to build on the leading indicator was still right, and it was reached through a broken measurement that happened to point the same way. **A right conclusion resting on a wrong measurement is the most durable kind of error, because every check you run on the conclusion passes.**
...
- **status-evidence:** `--verify` reports **10 of 10 APPLIED**; a second `--apply` reports "nothing to do", so idempotence is exercised not claimed; `099_SYNC_gate_roster.py` dry run reports **drift (none)**, which is the check that the mirrored pair was not edited on one council only. Cutovers taken from `agent_definitions.updated_at`, not scrollback: `15:12:49`–`15:12:58` (guardian ×3, improvement_guardian ×2, debug_historian ×2) and `15:39:26`–`15:39:30` (editquality ×3). The classifier was exercised against four real live seats and returned `HAND-WRITTEN` (council-gate/review_architecture, correctly refusing to overwrite a hand-authored block), `NEEDS-BLOCK` ×2, and `MISSING` for a non-existent seat. **BEHAVIOUR CHANGE VERIFIED on `review_editquality` (2026-07-31):** measured by ROUND SPAWN TIME (`orchestration_states.created_at`) against that seat's own cutover, because an orchestration keeps the workflow it loaded at spawn — rounds spawned BEFORE the block: 10 calls, peak 15,721 = **98.3%** of cap, mean 9,848; rounds spawned AFTER: 8 calls, peak 8,793 = **55.0%**, mean 6,569. Same seat, same afternoon, **same cap** — editquality was already at 16000, so the budget ALONE did this, which separates the two halves the `review_architecture` result could not (there the cap raise and the budget shipped together). The small-sample objection cuts the wrong way: a maximum grows with n, and the arm with MORE calls is the one with the higher peak. **NOT verified for the other three seats** — `guardian`/`improvement_guardian`/`debug_historian` have ZERO pre-cutover calls in the window, so no control arm. Their post-cutover peaks (77.6/37.5/70.8%) sit below their 14-day historical peaks (99.2/96.6/99.8%), but **a max over 14 calls is not comparable to a max over 278** — recorded as a non-finding, not a result.
...
### FIX-060 — Notes-first output reorder for seats whose REMIT makes notes load-bearing (2026-07-31 addition)
- **status:** **deployed**, one seat (`feature-designer/review_architecture`); idempotent, re-run reports `ALREADY`.
- **status-evidence:** all three `review_architecture` seats now report an identical key order — `reviewer, verdict, notes, objections, missing, checks, code_checks` — queried from the live rows; snapshot taken before the write; row count asserted at exactly 1.
- **what:** `scripts/reorder-seat-notes-first.py`. Moves `notes` to third in a seat's output schema by **parsing the schema line as JSON and rebuilding it with the desired key order**, then replacing that exact line in the prompt. Not regex surgery: the line is syntactically valid JSON even though it is a template (`"problem": "..."`, `"edit": 1`), and its `notes` value contains `\n\n`, `|` and angle brackets that a pattern would eventually mangle. Asserts the line appears exactly once before replacing.
- **why it exists:** `bugs_open/138` candidate 4. Truncation eats the tail, so a seat whose remit puts something load-bearing in `notes` loses exactly the field that makes it measurable — for `review_architecture` that is the mandated `ARCHITECTURE_SIGNAL`, and when it truncated the result was indistinguishable from "this seat is noise, retire it". fix-proposer and council-gate were fixed by hand 2026-07-29; feature-designer was reached by nothing, because 099 mirrors only those two councils.
- **sources:** bugs_open/138_HANDOFF_2026-07-29_…md (2026-07-31 entries); docs024_key_docs_latest/bugfix_138_degraded_gates/{NOTES, HANDOFF_2026-07-31_continue_here}.md
- **relations:** FIX-059 (the length budget applier — a sibling, and the one that generalises; this one deliberately does NOT); FIX-054 (the architecture seat); 099_SYNC_gate_roster (mirrors two councils of six, which is why the third was stale)
- **LANDMINE:** **this is NOT a rule to apply broadly, and the measurement says so.** Moving `notes` forward pushes `objections` into the tail, and objections survive 80% of truncations while carrying both the severities the gate reads and the content the proposer revises against. `notes` survives 6.7%. For 49 of 51 seats the current order already sacrifices the cheapest field — **the seat's own remit decides, not a schema rule.** Related refutation: `severity` is last inside each objection and looks like the same defect one level down; 0 of 2,713 stored objections lack one.
- **open review question:** whether any other seat's remit qualifies. `guardian` is the candidate — its prompt requires the contained alternative in `notes` on a veto — but there are **15 vetoes all-history, 0 degraded, 0 with empty notes**, so it is a real risk with zero observed instances. Left alone; recorded so the next reader does not re-derive it.
