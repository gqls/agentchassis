...
- **status:** built 2026-08-12, NOT yet live (inert until the next fleet roll — Go change). The scrape path it rides on IS live and was verified before building.
- **status-evidence:** Unit-tested and **mutation-verified** (`platform/observability/db_pool_stats_test.go`): stubbing out the registration makes the size assertion fail with `go_sql_max_open_connections absent — the pool's size is still unreadable`, and restoring it passes. The READER was checked first, because a metric nobody scrapes is this platform's own documented failure mode (`bugs_open/040`): PodMonitor `ai-persona-system/agent-chassis` selects `app in (agent-chassis, dynamic-agent)` and targets `9090/metrics`; Prometheus reports both chassis pods **`health: "up"`** on v1.0.1291 and already holds **19 `ai_persona_*` series**. Note the pod declares only containerPort 8080 — the numeric `targetPort: 9090` is what makes the scrape work, so do not "tidy" that away.
