...
_Concept count retired 2026-08-09 — derived, not stored; run the drift pair in `000_concept_index.md`, or read `concept-register-drift-check`'s daily row (DOC-074)._ consolidated from 114 raw extractions across units U01, U02, U03, U04,
...
- **status:** partial
- **status-evidence:** The routing half was fixed by `bugs_closed/060` (chassis v1.0.1167): a successful resolution now records the resolved type. The FAILURE half stayed broken until `bugs_open/239` (2026-08-10): a dispatch that could not be resolved fell through to the pod's own workflow and filed `owner_agent_type='generic'`, so the one column an operator checks to tell a substituted run from a real one said `generic` on exactly the runs where it mattered. This entry's own suggested fix shape — "selectWorkflow sets owner_agent_type to the resolved type" — is now implemented for both arms: on refusal the FAILED row carries the REQUESTED type (see SYS-090). ~~Committed 2026-08-10, awaiting a fleet roll~~ → LIVE and pod-proven 2026-08-11: refusal on v1.0.1284, the FAILED row on v1.0.1286 (bugs_open/239 closing section).
- **original status-evidence:** "orchestration_states rows where the generic agent routed to a different workflow still show owner_agent_type = 'generic'" (2026-04-20, P3).
...
- **status:** deployed
- **status-evidence:** Described as the running framework ("deploys as a scalable Kubernetes deployment, 3 replicas in production"); ~~the HITL agent definition (2025-11-03) still references image `docker.io/aqls/agent-chassis:v1.0.407`~~ — **CORRECTED 2026-08-12: that was true when written and is false now.** The row still exists (`simple-content-writer-with-approval`, created 2025-11-03, active) and its `image_tag` is **`v1.0.1290`** — the live tag. It was never re-pointed by hand: **all 187 live `agent_definitions` rows carry `v1.0.1290`**, uniformly, so the release rewrites them [the uniformity is MEASURED; the rewriting mechanism is INFERRED from it]. **So an image tag read off an agent row can never be durable evidence of anything but the date you read it** — it expires on the next release, and this citation was 883 versions behind. Caught by `scripts/report-register-version-lag.py` (DOC-077), settled by one query with a control (187 rows carry a tag, so the zero for `v1.0.407` is absence, not a blind predicate).
...
### SYS-090 — Fail-closed dispatch resolution: an unresolvable `orchestrate` request refuses instead of running the consumer's own workflow
- **status:** LIVE and PROVEN IN FULL on v1.0.1286 (2026-08-11): the refusal (proven on v1.0.1284) AND the FAILED orchestration row, whose sqlDB-guard no-op was fixed at `209917d15` and pod-proven the same day — corr `cc7bd91a`: `no-such-agent-239 | FAILED | …agent_type_unresolved…`, where the same query returned 0 rows over all prior history
- **status-evidence:** Pod-proven both directions on both replicas, 2026-08-11: `strings /app/agent-chassis | grep -c DISPATCH_FAIL_CLOSED` = 8 (was 0 on v1.0.1280), negative control `grep -c "didnt find it basically"` = 0 (was 1). Behaviourally: 5 of 5 byte-identical dispatches naming a non-existent agent failed closed deterministically, a fragmented heredoc failed with `parse_failure`, ZERO orchestrations were created by any of the six (pre-fix: six COMPLETED `generic` no-ops), and 22 natural fleet dispatches since the roll all resolved to their requested type with zero collateral failures. ⚠ **CAVEAT, found by that verification:** `recordDispatchFailureState` guarded on `p.sqlDB`, which is populated only when `DATABASE_URL` is set — and it is NOT set on the chassis — so the FAILED `orchestration_states` row this entry and SYS-014 both describe has never been written in production. Fixed in the tree (`db := p.db` fallback) with a regression test on the production shape; ~~rides the next roll. Until then, the queryable trace is `chassis_intake_events.last_error LIKE 'DISPATCH_FAIL_CLOSED%'`, not the orchestration row~~ → **RESOLVED 2026-08-11, same day: the fix (`209917d15`) is in v1.0.1286 by stamp ancestry (`c3b424c8e`), and one unknown-agent dispatch (corr `cc7bd91a`) wrote the FAILED row owned by the requested type — baseline 0 rows all-history before. The orchestration row is now the queryable trace this entry always claimed.** Original evidence: `bugs_open/239` root-cause + closing-verification sections; unit tests `platform/messaging/processor_dispatch_resolution_test.go` (reproduction mutation-verified: reverting the refusal yields `resolved to "generic" (source "own_default")`), `platform/discovery/find_by_type_guards_test.go`, `platform/agentbase/intake_dispatch_disposition_test.go`. ⚠ The `strings`-based pod checks quoted above are the RETIRED recipe (CLAUDE.md 2026-08-11) — they were real measurements on v1.0.1284 but the replacement is the provenance stamp + `git merge-base --is-ancestor`.
- **what:** `MessageProcessor.selectWorkflow` now returns `(workflowSelection{Plan, RunAgentType, Source}, error)` and REFUSES an orchestration action it cannot resolve, instead of falling through to the consuming pod's own `DefaultConfig["workflow"]`. Two typed codes in `platform/errors` carry the disposition: `DISPATCH_UNRESOLVABLE` (terminal — reasons `parse_failure` | `agent_type_unresolved` | `workflow_missing`; on `NonRetryablePermanentCodes`) and `DISPATCH_LOOKUP_UNAVAILABLE` (transient, built `AsRetryable`, so a database fault is re-attempted rather than mistaken for an unknown agent). A refusal leaves two traces the old silent path never left: a FAILED `orchestration_states` row whose `owner_agent_type` is the type that was REQUESTED (SYS-014's own suggested fix shape), and the bug-196 failure envelope on the responses topic. The intake worker pool marks a terminally-refused event `failed` (`DISPATCH_FAIL_CLOSED …` in `last_error`) rather than `done`; a transient one is left for the existing attempts<3 re-pop, with both claim-holders RELEASING the dedupe claim (`StateRepository.ReleaseMessageClaim`) so a re-attempt cannot lose the race against its own earlier try. The message body is now parsed ONCE per message (`MessageContext.Body()`, memoised) — `selectWorkflow` and `executeWorkflow` previously ran independent parses and could disagree about the same bytes, i.e. run one agent's workflow and file it under another's name.
- **what changed for OTHER consumers (the guarantee, stated because this is a shared seam):** a malformed or unresolvable `orchestrate` message used to COMPLETE having done nothing; it now FAILS, loudly and durably. Named consumers of the seam: `kafka-scheduler` (`cmd/scheduler/main.go` `fireTrigger` — 9,433 messages in an 8-day census, all naming `agent_type` explicitly, including the `generic` ticks whose real workflow IS the no-op; unaffected because they RESOLVE), `call_agent`/`spawn_agent` envelopes carrying an inline `config.workflow` (711 in the same window — Priority 1, untouched), the council-gate trigger scripts (165, all naming `council-gate`), the `report-dispatch`/`diagnose-dispatch` loops, and every operator following a `HANDOFF`/`RUNBOOK` drive-loop recipe. Dedicated and spawned pods, whose own type IS the target and whose messages name nothing, keep the own-default path and now log `DISPATCH_OWN_DEFAULT`. The only measured traffic that begins to fail is the 48 unparseable bodies in that window — every one a kcat fragment from a hand-dispatch.
- **landmine:** `kcat -P` sends ONE MESSAGE PER LINE of stdin, so a multi-line envelope becomes N invalid-JSON fragments wearing the same headers — the trigger that produced this bug, and still a trap for the sender even after the chassis stops hiding it. Full entry in `LANDMINES.md` (footprint: kcat, `system.agent.*.requests`, `scripts/initial_messages/`). Second: `platform/discovery` `FindByType` gained the `deleted_at IS NULL` + `is_snapshot` guards every sibling loader already had — an active snapshot is stored at version+1000 and OUTRANKED the live definition, so this seam could dispatch to a snapshot.
- **open review question:** the refusal is deliberately scoped to `isOrchestrationAction` — non-orchestration actions keep the old own-default behaviour. Whether Priority 3 has any remaining legitimate consumer for orchestration actions that name nothing (beyond dedicated/spawned pods, whose job topics the intake census cannot see) is NOT established; the `DISPATCH_OWN_DEFAULT` warn line exists to make that population countable per pod before anyone narrows it further. **OWNER 2026-08-12: leave it to accumulate real fleet data; revisit with the measured population, not before.** ~~Related: three MORE dead `p.sqlDB` guards in the same file are filed as `bugs_open/259` (the `three_processor_paths` slug — 259 is an AMBIGUOUS number, a second unrelated 259 was filed the same day; resolve by slug).~~ → **RESOLVED IN THE TREE 2026-08-14 (`bugs_open/259`, `three_processor_paths` slug — 259 is an AMBIGUOUS number, a second unrelated 259 was filed the same day; resolve by slug): `p.sqlDB` NO LONGER EXISTS. `MessageProcessor` has ONE handle, `p.db`,** so the fallback this entry's fix introduced (`db := p.db; if db == nil { db = p.sqlDB }`) has collapsed to a single read and the class cannot recur — there is no second handle to guard on. Deleted with it: the child-workflow completion check (inert — both branches `return nil`), `sendWorkflowSuccessResponse` and its `sendWorkflowResponse` wrapper (zero callers, and with them the only fleet-wide `{"status": "completed"}` placeholder), the processor's copy of the `bugs_open/003` F3 two-phase dedupe claim (redundant — `agentbase` runs the same claim, and the same release-on-transient-dispatch rule, on a live handle), the unread `stateRepo` field built from the nil handle, and the callerless `createSQLDB`. ~~⚠ **Inert until a fleet roll** (Go change): the guards are gone from the tree, not yet from production.~~ → **LIVE AND VERIFIED on v1.0.1298, 2026-08-14; `bugs_closed/259`.** Council APPROVED (`0ff072ef`). Deploy proven at the artefact by the **absence** of two deleted log literals on both pods with a kept literal as the present-control (the `build provenance` line had already scrolled, and a sha probe is the wrong tool for a deletion since the release stamps a later commit than the fix). Behaviour re-verified afterwards: a dispatch naming `no-such-agent-259-postroll` still produced the FAILED row owned by the **requested** type (corr `8234e56e`, 13:55:58Z), and `processed_messages` shows agentbase's claim writing continuously across the roll with no loss of writers. The third check — `recordDroppedValidationError` — is INCONCLUSIVE **by demand, not by failure**: that writer fires ~2/day and its last row predates the roll by 6¼ hours, so zero since is uninformative; its edit is provably behaviour-identical (the old code always fell through to the very handle the new code reads) and its premise is what the FAILED row measured.
- **sources:** bugs_open/239 (root cause + fix + verification plan); LANDMINES.md 2026-08-10 kcat entry; WRONG_CALLS.md 2026-08-10; 016b §9 (corrected entry)
- **relations:** SYS-014 (owner_agent_type observability — this implements its stated fix shape for the failure arm); SYS-035 (the generic orchestrate envelope); bugs_closed/060 (the success arm); bugs_closed/074 and bugs_closed/030 (earlier encounters with the same fallthrough); bugs_open/201 (a different silent no-op, inside a real workflow)
- **verify-later:** DONE 2026-08-11 — the five checks passed on v1.0.1284, and the sixth (the FAILED orchestration row, which those five found to be a no-op) passed on v1.0.1286; evidence in bugs_open/239's closing-verification section

### SYS-091 — Database pool instrumentation: `go_sql_*` metrics for every pool `agentbase` opens
- **status:** **LIVE, SCRAPED AND PROVEN END-TO-END on v1.0.1295 (2026-08-13).** `[MEASURED 2026-08-13 14:07Z]` 2 of 35 active Prometheus targets are `agent-chassis-*` pods, both `health:"up"` scraping `http://<podIP>:9090/metrics`, and the chassis-filtered query returns two series reading `go_sql_max_open_connections{db_name="clients_db"} 12`. Contention counters read on the chassis for the first time: `wait_count_total 0`, `wait_duration_seconds_total 0`, `open_connections 2`, `in_use 1`. ~~LIVE in the binary since v1.0.1293 (2026-08-12) and serving correctly — but NOT scraped on the chassis until the next roll applies the manifest fix below.~~ ~~built 2026-08-12, NOT yet live (inert until the next fleet roll — Go change). The scrape path it rides on IS live and was verified before building.~~
- **status-evidence:** Unit-tested and **mutation-verified** (`platform/observability/db_pool_stats_test.go`): stubbing out the registration makes the size assertion fail with `go_sql_max_open_connections absent — the pool's size is still unreadable`, and restoring it passes. `[MEASURED 2026-08-12, v1.0.1293]` both chassis pods serve all nine `go_sql_*` series on `:9090`, reading `go_sql_max_open_connections{db_name="clients_db"} 12` — matching `CHASSIS_DB_MAX_OPEN_CONNS=12`. The Go half is proven.
- **⚠ CORRECTED 2026-08-12 (dispatch/pool lane, D2's post-roll check) — the READER CHECK BELOW WAS WRONG, AND IT WAS WRONG IN THE DIRECTION THAT READS AS SUCCESS.** ~~The READER was checked first, because a metric nobody scrapes is this platform's own documented failure mode (`bugs_open/040`): PodMonitor `ai-persona-system/agent-chassis` selects `app in (agent-chassis, dynamic-agent)` and targets `9090/metrics`; Prometheus reports both chassis pods **`health: "up"`** on v1.0.1291 and already holds **19 `ai_persona_*` series**. Note the pod declares only containerPort 8080 — the numeric `targetPort: 9090` is what makes the scrape work, so do not "tidy" that away.~~ The selector and the endpoint are stated correctly; the two conclusions drawn from them are both false. (1) **No chassis pod was ever a target** — `[MEASURED 2026-08-12]` 0 of **141** active targets across every scrape pool had a pod name matching `agent-chassis-*`. The pods read as `health:"up"` because the `job` label is the **PodMonitor's name**, so all ~108 spawned-agent targets carry `job="ai-persona-system/agent-chassis"` and look like the chassis. (2) The numeric `targetPort` is not what makes the scrape work — it compiles to `keep __meta_kubernetes_pod_container_port_number == "9090"`, which keys on the port a pod **DECLARES**. The spawned pods declare 9090 (`spawn_actions.go`); this Deployment declared only 8080, so it passed the label arm and was dropped at the port arm. **Cost:** an instrument built, council-APPROVED and shipped specifically to answer `bugs_closed/246`'s "you cannot observe the pool at 12" was serving that exact reading, unscraped, on both pods. **Fixed** at `deployments/kustomize/services/agent-chassis/base/deployment.yaml`, which now declares `9090 name: metrics` — ~~inert until the next roll applies it~~ **applied and confirmed on v1.0.1295, 2026-08-13**: both chassis pods now declare `8080,9090` and both are scraped. The disconfirming result (an empty vector on the chassis-filtered query) was available throughout and is what the same query returned the day before, so this check could have come out either way. Full trap in `LANDMINES.md` ("a PodMonitor's numeric `targetPort` keys on the port a pod DECLARES").
- **what:** `observability.RegisterDBPoolStats(reg, db, dbName)` wraps the stdlib `collectors.NewDBStatsCollector`, called by `agentbase` at the one place the pool is opened (`agent.go`, beside the `SetMaxOpenConns` it describes). Emits `go_sql_max_open_connections` (the CONFIGURED size — so "is the fleet running the 12 the operator asked for?" becomes a reading, not an inference from an env var), `go_sql_wait_count_total` and `go_sql_wait_duration_seconds_total` (contention), plus open/in-use/idle and the three close-reason counters. Registration is idempotent (`AlreadyRegisteredError` → nil); a nil `*sql.DB` is refused with an error rather than deferred to a panic on the first scrape — `agentbase` leaves its handle nil whenever `DatabaseURL` is empty, so that shape is reachable, not theoretical.
- **why the stdlib collector, not hand-rolled `ai_persona_*` gauges:** it would be nine bespoke gauges to rename metrics that dashboards and alert rules already recognise — against the owner's standing bloat/streamlining rider (RFC_023 ruling, 2026-08-12) and CLAUDE.md's reuse-before-new-machinery.
- **landmine:** the wait counters are **CUMULATIVE since process start**, so a raw value spanning a roll describes two different processes — read them as a `rate()`. And the first readings are a **BASELINE, not a result**: the 4→12 pool change went live with v1.0.1288 at 17:13 UTC 2026-08-11, so no comparable "before" exists and early numbers are not evidence that `bugs_closed/246`'s fix changed anything.
- **what this does NOT see:** pgbouncer's server-side queue (`cl_waiting`/`maxwait` via `SHOW POOLS`). `db.Stats()` is the Go client side only. Both instruments are wanted; the pgbouncer half is blocked on the `pgbouncer-userlist` secret's `pgbouncer_admin` line (owner holds the credential; the Terraform half is wired at `aee444a35`).
- **sources:** `bugs_closed/246` (the defect that had no instrument, and its D2 follow-up); `bugfix_246_shared_pool_ownership/` standing five; `bugs_open/040` (why the reader had to be checked first)
- **relations:** SYS-090 (same lane, fail-closed dispatch); `bugs_closed/246` (pool ownership); `bugs_open/040` (the metrics endpoint this rides on)

### SYS-092 — Kafka PRODUCE outcomes are counted, and a structurally invalid dial is refused under its own label
- **status:** **LIVE on `agent-chassis` v1.0.1322 since 2026-08-21 16:54Z** (round-1 form; round 2's cardinality and label fixes are committed and ride the NEXT roll). Proven at the binary with both controls — `ai_persona_kafka_produce_total` PRESENT and `refusing dial to structurally invalid address` PRESENT on the running pod, a pre-existing positive control PRESENT, a nonsense negative control ABSENT, and a literal committed *after* the build ABSENT (so the probe demonstrably discriminates). **And collecting:** `sum by (outcome) (max_over_time(ai_persona_kafka_produce_total[1h]))` returned `ok = 99` within the first hour — which is the DEMAND CONTROL, not a nice-to-have: a zero there would mean the instrument is broken, not the fleet clean. Council round 1 REVISE (both code objections upheld and fixed); round 2 resubmitted on the same correlation. **Instrumentation and structural hygiene only — no behaviour change on any successful path**, which is deliberate: `bugs_open/040`'s round 1 was VETOED for bundling an unmeasured fleet-wide dial-timeout cut with its instrumentation, and that lesson is the shape of this round.
- **what:** two pieces on the two sides of one connection.
  1. **`ai_persona_kafka_produce_total{topic_class, outcome}`** (`platform/observability/metrics.go`), incremented in `KafkaProducer.Produce` (`platform/kafka/producer.go`) on both the success and failure branches. `classifyProduceErr` gives the bounded vocabulary — `ok` / `client_no_leader` / `broker_no_leader` / `too_large` / `timeout` / `canceled` / `network` / `other` — mirroring `classifyDialErr`'s discipline. **The wrapped error text at the return is byte-identical**, so every log search and every needle in `platform/errors`' transient classifier still matches.
  2. **`instrumentedDialFunc` refuses an address whose host is empty**, before dialling, under a new `empty_host` outcome, with **no duration observation** (nothing was dialled; a ~0s sample would drag the latency distribution this bug is read from). The broker label stays `""` so the series is continuous with the events already recorded under it.
- **WHY A COUNTER HAD TO EXIST: the dial counters see a CONNECTION fail; nothing had ever seen a WRITE fail.** `[MEASURED 2026-08-21]` over the retained `agent_error_log` window (07-22..08-21): **63** "Kafka write errors" plus **40** "topic partition has no leader" rows across **93 distinct orchestrations**, recurring most days since 08-10 — on `complete_workflow` (11), `process_message` (10), `orchestrate` (9) and ~20 `call_agent` dispatch steps. Every one of those is work that had already been done, lost or misreported at the moment of reporting it, and application logs were the only witness — pod-lifetime, on ephemeral Jobs.
- **⚠ `topic_class`, NEVER `topic` — this is a cardinality guard, not a style choice.** Reply topics are minted per job (`job.<correlation>-<orchestration>-<step>.responses`) and this cluster has carried **~25,000 topics at once**. A raw topic label on a counter that fires on every produce in the fleet is an unbounded series explosion in Prometheus, which is a worse outage than the one being measured. `topicClass` collapses `job.*` → `job`, `system.agent.*` → `system.agent`, and every other `system.*` topic to its FAMILY drawn from a **closed set**, with anything unknown becoming `system.other`. **Every arm returns a FIXED string — never a substring of its input.** Pinned by `TestTopicClassIsBoundedAcrossManyJobTopics` (5,000 distinct job topics → exactly ONE label) and `TestSystemTopicsCollapseToAClosedFamilySet`.

  > **CORRECTED 2026-08-21 (round 2), and the correction is the entry's most useful line.** The first cut of this arm was `case strings.HasPrefix(topic, "system."): return topic` — **the author stated the no-substring rule and broke it in the very next arm**, then flagged it in the submission's *risks* for a reviewer to confirm rather than resolving it. The council's `editquality` seat objected HIGH and was right. Measured afterwards rather than conceded in the abstract: `[MEASURED 2026-08-21, live cluster]` of **937** `system.*` topics, **859** are caught by the `system.agent` arm and **78 would have reached that arm as distinct label values** — with the two biggest residue families, `system.errors.<agent-type>` (18) and `system.responses.<agent-type>` (17), **growing with every new agent type**. Bounded in practice, unbounded in principle, which is not a guard. Adding a family is now a compile-time edit to `knownSystemTopicFamilies` that a reviewer can see, not a runtime consequence of somebody naming a topic.
- **the finding that makes no-leader worth TWO labels, not one:** `client_no_leader` and `broker_no_leader` are kept apart because the two errors behave **oppositely** inside kafka-go, and that difference is the whole diagnostic value — a reader must be able to tell *exhausted immediately* from *retried and still failed*. `[CORRECTED 2026-08-21 round 2: the first cut collapsed them into one `no_leader`; the council's editquality seat objected MEDIUM and was right.]` The detail: **kafka-go will never retry it.** The writer loops up to `MaxAttempts` (default 10) with backoff, but breaks after attempt 1 when `!isTemporary(err) && !isTransientNetworkError(err)` (`writer.go`), and `protocol.ErrNoLeader` — the client-side `"topic partition has no leader"` — is a **bare string type with no `Temporary()` method** (verified in the module cache, `protocol/error.go`: `func (e Error) Error() string` is its only method). That is the observed **`Kafka write errors (1/1)`** fingerprint: one message, one attempt, one error, on a condition usually over in seconds. The broker-side code-5 `kafka.LeaderNotAvailable` **is** Temporary and has a different text, which is why the typed check and the substring check both earn their place rather than one being redundant.
- **the `empty_host` guard, and what it is FOR:** `net.SplitHostPort(":9092")` **succeeds** with `host=""` — it is not a malformed address, it is a valid one naming the local machine. So such a dial goes to the pod's own loopback, where nothing listens, and returns `ECONNREFUSED` **instantly**; a single pod racks up hundreds per burst. `[MEASURED 2026-08-21]` **94,419** `refused` over 7 days, of which **85,887 carry an EMPTY broker label** — which is `brokerLabel`'s output for `host=""`, so that is not a missing label, it is a dial to `":9092"`. `topic_manager.go`'s `getController` was one producer and is fixed (`controllerAddress`, live v1.0.1291) — **and the bursts RECURRED after it**, so at least one more producer exists and it is **not in this repository**: kafka-go's consumer-group coordinator lookup builds `net.JoinHostPort(out.Coordinator.Host, …)` straight from a FindCoordinator response with no validation (`consumergroup.go`), and every `Reader` in the fleet dials through this dialer. Vendored code cannot be guarded upstream; this function is the one choke point **below** all of it. `[INFERRED as the live mechanism — the burst pods are GC'd, so the armed Warn-log discriminator cannot be read retrospectively.]`
- **status-evidence:** mutation-proven, each mutation applied and its kill set recorded. Round 2 adds two: restoring `return topic` on the `system.*` arm fails `TestSystemTopicsCollapseToAClosedFamilySet` and the per-job bound test; collapsing the two no-leader labels fails the classifier table. **A** key the guard on a parse failure instead of an empty host → the guard test AND its unparseable-address control both fail. **B** fold `empty_host` into `refused` → the guard test fails on the label-split assertion. **C** `topicClass` returns its input → both cardinality tests fail. ⚠ The guard test asserts on the **error text**, not merely that an error came back: on a machine where something happens to listen on `localhost:9090` the dial would otherwise succeed, and only the guard produces that string — so the test discriminates rather than passing by accident. Two controls sit beside it (an unparseable address, and a populated host against a real local listener).
- **sources:** `platform/observability/metrics.go` (`KafkaProduceTotal`); `platform/kafka/producer.go` (`classifyProduceErr`, `topicClass`, the `Produce` increment); `platform/kafka/dialer.go` (`dialOutcomeEmptyHost`, `instrumentedDialFunc`); `platform/kafka/produce_outcome_test.go`, `platform/kafka/dialer_test.go`; `deployments/kustomize/services/agent-chassis/base/kustomization.yaml` (the PodMonitor wiring, below); `bugs_open/040`.
- **⚠ the PodMonitor wiring drew a MEDIUM minimality objection and is deliberately NOT reverted.** The `editquality` seat's point is fair — infra wiring is orthogonal to the kafka mechanism this entry is about, and it should have been its own commit. It stays because it was an **explicit owner decision** taken in the session that shipped it, and because forward-only forbids an amend on a shared branch. Read it as owner-directed scope, not as scope-creep that slipped through.
- **also in this change — the PodMonitor is finally IN THE BUILD.** `base/podmonitor.yaml` was hand-applied 2026-07-26 and sat **unreferenced by any kustomization for 26 days**: live, committed, and reconciled by nothing, so an `apply -k` would not have recreated it after a deletion and file-vs-live drift was silent in both directions. Wired into `resources:` on the owner's decision 2026-08-21, after verifying the file matches the live object field for field; `kubectl diff -k` shows only the overlay's own `environment`/`region` labels being added, and `spec.selector` is untouched, so Prometheus discovery is unaffected. The file's "NOT YET APPLIED" banner — true when written, false for most of its life — is corrected at source.
- **relations:** **SYS-091** and `bugs_open/040` §8e (the three-layer metrics fix this completes on the produce side); the `ai_persona_kafka_dial_total` family in the same file. **DOES NOT CLOSE `bugs_open/040`**: the `timeout` residual (**146** in 7 days, prod-0 dominant) is undiagnosed and §4.2's node-pinned probes are still unexercised; the `refused` mechanism is `[INFERRED]`, not confirmed. **NOT bundled, deliberately:** a retry on the produce path is behaviour and ships opt-in in its own round; extending `/metrics` to the 13 adapter Deployments (`bugs_open/040` §8b) is its own round again.
- **verify-later:** **(1) In the binary after the next roll**, per SERVICE (`bugs_open/249`): the build-provenance stamp plus `git merge-base --is-ancestor`. Greppable literals: `ai_persona_kafka_produce_total` and `refusing dial to structurally invalid address`. **(2) The produce counter, with its own demand control:** `sum by (outcome, topic_class) (max_over_time(ai_persona_kafka_produce_total[24h]))` — **`ok` ≫ 0 within minutes IS the control**; if `ok` is zero the instrument is broken, not the fleet clean. ⚠ `max_over_time`, never `increase()` — the pods carrying this are ephemeral Jobs whose counters are frequently born at their final value, and `increase()` over samples that are all `1` is `0` by definition (`bugs_open/040` §10's trap). **(3) The discriminator, and this is the point of the guard:** on the next burst, `sum(max_over_time(ai_persona_kafka_dial_total{outcome="empty_host"}[48h])) > 0` confirms the remaining producer is library-internal, since `getController` has been structurally unable to emit such a dial since v1.0.1291. **The disconfirming result, named in advance: `refused` with an empty broker label must now be STRUCTURALLY ZERO — a non-zero means a third producer exists outside the instrumented dial path**, and that is where to look next. **(4) The PodMonitor:** `kubectl -n monitoring exec prometheus-… -- wget -qO- '…/api/v1/query?query=go_sql_max_open_connections{pod=~"agent-chassis-.*"}'` — two chassis pods reading 12; an empty vector is the disconfirming result.

### SYS-093 — an OPT-IN bounded produce retry on the reply lane, plus the two classifier needles that stop a seconds-long broker blip terminating finished work
- **status:** **BUILT, INERT UNTIL THE NEXT CHASSIS ROLL.** Committed 2026-08-21 with mutation cover; submitted to the council gate. **Behaviour, and it ships OPT-IN with the unsafe side OFF** (owner ruling 2026-08-02 §2) — adopted at four named sites, with the five other `DeliverReply` callers byte-identical to today and a test pinning that.
- **what:** three pieces answering one measured failure.
  1. **`platform/kafka/produce_retry.go`** — `RetryPolicy`, `DefaultReplyRetryPolicy` (4 attempts, 500ms→4s jittered), `ProduceWithRetry`, and `WithRetry(policy)` as a variadic `DeliverOption` on `DeliverReply`. Deterministic errors are never retried (validation refusal, too-large, context-cancelled). **The last attempt's error is returned VERBATIM** — every caller wraps it in its own message and `platform/errors`' classifier matches needles inside them, so a wrapping layer here would silently change what all of that sees.
  2. **Four adopters, each saying so at its own call site** where a reviewer of the CALLER can see it: `CompleteWorkflowAction` (`workflow_actions.go`), `notifyParentOfFailure` (`coordinator.go` — closing the asymmetry with `notifyParentOfSuccess`, which has been on the shared reply seam since `bugs_open/133` while this stayed a bare fire-once log-and-drop), `notifyParentOfSuccess` (adds `WithRetry` to its existing `DeliverReply`), and `sendWorkflowResponseWithStatus` (`platform/messaging/processor.go` — the single produce exit serving both the success path and the two processor failure senders).
  3. **Two needles admitted to `TransientErrorNeedles`** (`platform/errors/transient_failure.go`): `"kafka write error"` and `"no leader"`.
- **WHY, and it is a census not a hunch.** `[MEASURED 2026-08-21]` over `agent_error_log`'s retained window (07-22..08-21): **63** rows matching "Kafka write error" plus **40** matching "has no leader", across **93 distinct orchestrations**, recurring most days since 08-10. The worst shape is a workflow whose every substantive step has already committed failing at its terminal `complete_workflow` send — the work is done and the head row reads FAILED. And **none of the nine pre-existing needles matched either string**, so every one of those classified `error_unrecoverable` and terminated work permanently, on a condition usually over in seconds.
- **and kafka-go does NOT cover it, which is the fact that makes this not a duplicate of the library.** Its writer loops to `MaxAttempts` (default 10) with backoff but breaks after **one** attempt on `!isTemporary(err) && !isTransientNetworkError(err)`, and `protocol.ErrNoLeader` is a bare string type with no `Temporary()` method. `Kafka write errors (1/1)` is that fingerprint — and the `(1/1)` is *messages-failed / messages-sent*, not attempts. Full trap in `LANDMINES.md`.
- **⚠ `"write message to kafka"` was DELIBERATELY NOT ADMITTED.** It is our own producer's wrapper on **every** write failure, including the deterministic oversize refusal and the validation refusal — admitting it would reclassify permanent failures as retryable, which is exactly the defect `bugs_open/274` spent 12 days on. The two admitted needles cover all 103 measured rows without it, and a **control test** asserts both deterministic refusals still classify TERMINAL.
- **⚠ what opting in accepts:** a retry after a **lost ack** duplicates the reply. kafka-go v0.4.47 has no idempotent producer, so it cannot be prevented at this layer; it is absorbed downstream by the parent's two-phase `ClaimAwaitedRequest` and by `processed_messages`. **So only adopt this where the consumer is a parent orchestration with that dedupe** — which every current adopter is. Worst-case latency is bounded and worth stating: ~44s (4 × the 10s `WriteTimeout` plus backoff) before a reply is finally reported undeliverable. That is a long time to hold a step, and the right trade against losing a completed workflow; it is also why this is not a default inside `Produce`, which serves hot dispatch paths too.
- **status-evidence:** mutation-proven, each mutation applied and its kill set recorded rather than assumed. Deterministic-stop removed → two tests fail. Error wrapped instead of returned verbatim → the text-contract test fails. Jitter cap: **a test caught a real defect in my own code** — `rand.Int63n(delay) + delay/2` spans `[delay/2, 1.5×delay)` and therefore *breaks* `MaxDelay` by up to 50%; fixed to jitter within `[delay/2, delay]`. ⚠ **And an honest negative: the default-OFF pin does NOT discriminate the flag.** Removing the `cfg.retry` check alone survives, because the zero-value `RetryPolicy`'s `Attempts: 0` is clamped to 1 — two mechanisms in series. That is a real belt-and-braces property; the test pins the observable promise ("no option, one attempt"), the clamp is pinned separately, and the mutation that does fail it (defaulting `retry: true` **with** a non-zero policy) is named in the test. Recorded rather than left as a mutation claim nobody applied.
- **sources:** `platform/kafka/produce_retry.go`, `platform/kafka/reply_delivery.go` (`DeliverOption`), `platform/kafka/produce_retry_test.go`; `platform/orchestration/actions/workflow_actions.go`, `platform/orchestration/coordinator.go` (`notifyParentOfFailure`, `notifyParentOfSuccess`), `platform/messaging/processor.go`; `platform/errors/transient_failure.go` + `platform/messaging/retryable_transient_test.go`; `platform/observability/metrics.go` (`KafkaProduceRetryRecoveries`); `bugs_open/040`.
- **relations:** **SYS-092** is the instrumentation half and shipped first — read it for the produce counter this round's recoveries counter sits beside. **`bugs_open/133`**'s `DeliverReply` is the seam extended here, and its "the caller's existing retry path stays in charge" comment was TRUE and named a caller that does not exist on this path (`failWorkflow` has no retry) — that sentence is now qualified at source. **`bugs_open/274`** is why deterministic errors are excluded and why the third needle was refused. **DOES NOT CLOSE `bugs_open/040`**: the `timeout` residual (146/7d) is undiagnosed and the `refused` mechanism is `[INFERRED]`.
- **⚠ CONSUMERS, told rather than merely measured (owner ruling 2026-07-29 §3).** `DeliverReply`'s five existing callers — `agentbase/agent.go`, `orchestration/helpers.go`, `coordinator.go`, `adapters/webscrape/adapter.go`, `batch_handler.go` — are **unchanged** and need do nothing. What changed about the shared guarantee, for everyone: *a produce failure classified "Kafka write error" or "no leader" is now RETRYABLE where it was terminal, so a child that hits one may be redispatched by its parent (capped at `retry_version >= 3`) instead of failing permanently.* Anything relying on those strings being terminal must say so now.
- **verify-later:** **(1) In the binary after the next roll**, per SERVICE: the provenance stamp plus `merge-base`. Greppable literals: `kafka produce succeeded after retry` and `ai_persona_kafka_produce_retry_recoveries_total`. **(2) The DIRECT positive evidence, which is the point of the recoveries counter:** `sum(max_over_time(ai_persona_kafka_produce_retry_recoveries_total[7d])) > 0` says a produce that would have been a failure was in fact recovered. **A falling failure count alone cannot establish this** — quiet weather produces the same number — so pair it with the demand control `sum(max_over_time(ai_persona_kafka_produce_total{outcome=~"no_leader|network|timeout"}[7d]))` non-zero over the same window. Both zero means nothing happened, not that anything was fixed. **(3) The behavioural claim:** the weekly `agent_error_log` count of `error_message ILIKE '%Kafka write error%' OR '%has no leader%'` should fall against the 103-row/month baseline recorded here. **(4) The disconfirming result, named in advance:** a rise in duplicate replies — `SELECT count(*) FROM awaited_requests WHERE status='processed'` growing out of step with dispatches, or `DUPLICATE_SKIPPED` log volume rising — would mean the lost-ack duplication is not being absorbed as assumed, and the adopter set is wrong.

### SYS-094 — The two-door public-exposure pattern: the STANDING pattern for any internet door into the cluster (owner ruling 2026-08-25, RFC_054 Q1)
- **status:** ruled and live — both existing doors already conform; declared the standing pattern by the owner 2026-08-25, closing RFC_054 Q1–Q3.
- **what:** Every internet-reachable door into the cluster is built ONE way, and a new door either cites this entry or files an RFC first — that is the ruling, not a preference. The pattern: (1) a hostname-scoped nginx vhost on the box, listening on loopback only; (2) exact-path exposure — named locations only, everything else 404s at the box; (3) ingress ONLY via the cloudflared tunnel dialling out (no inbound ports; ufw default-deny behind it); (4) an edge gate matched to the audience — Cloudflare Access for the owner's console, credential-in-link token + token-shape regex + edge rate limit for customer links; (5) the WireGuard leg fenced by networkpolicy to exact service:port; (6) ZERO Ingress objects in the cluster (**0** as of the 2026-08-24 census). Worked examples: `admin.apis.uk` (`webdesign_uk_build_service/box/admin.apis.uk.nginx`) and `links.webdesign.uk` (`box/links.webdesign.uk.nginx`). The architecture seat's per-door review condition (stated at the `admin.apis.uk` round) converts to reviewing DEVIATIONS from this entry.
- **the two companion rulings from the same review (owner, 2026-08-25):** RFC_054 **Q2** — the delivery-only listener in core-manager IS to be built (own plan + council round): a second HTTP port serving ONLY the delivery routes, with the box and the wg fence pointed at it, so a box misconfiguration can expose nothing but delivery routes and the fence drops the admin port. Until it lands, the nginx location regex on the box is the last line, stated here rather than hidden. **Q3** — the review trigger for door N+1 is this entry plus the header line in every `box/*.nginx` / `box/*.cloudflared-ingress.yml`; `/stripe/webhook`'s door was reviewed by RFC_054 itself (HMAC over raw bytes; honest 503 until Stripe keys land), so the shopfront unparking owes no further review.
- **sources:** `docs024_key_docs_latest/architecture_review/RFC_054_public_cluster_exposure_boundary_review.md` (the review + §5 rulings); `docs024_key_docs_latest/webdesign_uk_build_service/box/*.nginx` (the worked vhosts); `docs024_key_docs_latest/web_admin_console/HANDOFF_2026-08-24b_continue_here.md` §3.3–§3.10 (the census the review was filed on); LANDMINES "A CLOUDFLARE PAGE RULE PUT THERE FOR PARKING…" (the trap the /c/ move + this pattern close).
- **relations:** public-api (httpguard — the in-binary edge-trust sibling; its `CloudflareTunnel()` peer gate is the same trust boundary one layer down) · site-chatbot (the WireGuard leg's first proven consumer) · the Q2 delivery-only listener — **BUILT 2026-08-25, registered as SYS-095**.

### SYS-095 — The delivery-only listener: core-manager serves customer routes on a SECOND port, so the admin port carries nothing publicly reachable (RFC_054 Q2, owner ruling 2026-08-25)
- **status:** ~~built 2026-08-25, Go INERT until the next core-manager roll~~ **LIVE END-TO-END 2026-08-26**: rolled ~11:55Z (pod stamp `e7f1045fd`, ancestry + control), verified at the pod (containment triple — `:8090` admin path 404 · `:8088` same path 401 not-404 control · `:8090` `/c/<shape>` 200 reach control), box vhost applied ~13:2xZ by the webdesign lane (old vhost backed up), and **outside-verified the same afternoon** (GET/POST `/c/<43-shape>` 200 from the internet · `/c/x` 404 · suffix `/c/<43>/confirm` 404 both verbs — the landmine's named control). Council trail `25cd3044` still advisory-open (round 4).
- **what:** `core-manager` now starts a second HTTP listener carrying **only** the customer delivery routes (`GET`/`POST /c/:token`, and `/d/` when it is built). The delivery routes were **moved, not copied** — they are mounted nowhere else — and `links.webdesign.uk.nginx` proxies to `:8090` instead of `:8088`. Opt-in via `server.delivery_port` (`SERVICE_SERVER_DELIVERY_PORT`, set only in the production overlay), **unsafe default OFF**: unset means no listener AND no customer route served anywhere, never a fallback onto the admin port. Enforced rather than intended — `assertNoDeliveryRoutes` walks the main router's real route table at construction and **refuses to build the server** if any `/c/` or `/d/` path is mounted there, so a future re-mount crash-loops instead of quietly re-opening the hole.
- **what it actually buys, and what it does not:** before this, the only thing between the internet and the API holding every site's data was an anchored `location` regex in nginx **on a box** — widening it by one character would have exposed the whole admin API with nothing in the binary refusing. Now widening it exposes `/c/` and `/d/` and stops. ⚠ **This does NOT make the wg fence the containment.** Port 8088 stays on the fence because the chat bot's facts relay (`/api/v1/site-facts/:domain`) needs it, so a NEW box vhost written straight to `:8088` would still reach the admin API. Making the fence the control means moving the facts relay to a box-facing listener too and dropping 8088 — a change touching a live customer-facing bot, deliberately out of scope of the ruling. **Stated here so the next reader inherits the residual rather than the reassurance.**
- **landmine (cost a real defect in this build):** `SERVICE_SERVER_DELIVERY_PORT` is set ONLY in the overlay and named in no service's YAML — and viper's `AutomaticEnv` **does not reach a key absent from both the config file and the defaults**, because `Unmarshal` populates from viper's known key set. Without the explicit `BindEnv` in `platform/config/loader.go`, the overlay reads perfectly, the listener never starts, and every customer link 404s at the box with no error anywhere in the cluster. Caught by writing the test first: `TestDeliveryPortIsReadFromTheEnvironment` **failed before the bind was added**. Any future env-only config key on this loader has the same trap.
- **verification (the one that counts is from OUTSIDE):** the box's anchored regex means a request that works from inside the cluster can be dead on the only path a customer takes, with no log line anywhere (LANDMINES, footprint `links.webdesign.uk.nginx`). The containment check is `curl core-manager:8090/api/v1/admin/work-items` → 404 **paired with the same path against :8088** → not-404; the 404 alone passes just as well against a typo'd hostname.
- **sources:** `docs024_key_docs_latest/architecture_review/RFC_054_public_cluster_exposure_boundary_review.md` §2 Q2 + §5 (the ruling); `docs024_key_docs_latest/site_delivery_and_editor/PLAN_2026-08-25_delivery_only_listener.md` (design, ordering, verification table); `internal/core-manager/api/server.go` (`assertNoDeliveryRoutes`, `newDeliveryEngine`, `newDeliveryServer`); `internal/core-manager/api/delivery_listener_test.go` (four mutation-proved guards); `platform/config/loader.go` (the BindEnv and why); `deployments/kustomize/services/wireguard/base/networkpolicy-wireguard-egress.yaml`; `docs024_key_docs_latest/webdesign_uk_build_service/box/links.webdesign.uk.nginx`.
- **UPDATE 2026-08-26 — two adversarial-review findings fixed (`67f1794f9`):** (1) **the pod could stay Ready with a dead customer door** — both listener goroutines' `cancel()` calls were DEAD CODE (main blocked on a bare `<-sigCh`; nothing consumed the cancellation), so a bind failure logged once and the box 502'd every customer link with nothing unhealthy anywhere; the shutdown path is now `select{sigCh, ctx.Done()}`, making both cancels real. The round-2 council answer had asserted the opposite. (2) **the box-shape guard vouched for `/d/`** while the vhost has no `/d/` location — split into `boxServablePrefixes` (`/c/` only; registering `/d/:token` now refuses to start core-manager until the vhost block and the list entry land in one commit) vs `deliveryRoutePrefixes` (admin-router guard, keeps `/d/`). Both mutations killed.
- **relations:** SYS-094 (the two-door pattern this is the Q2 companion to — its "unbuilt as of 2026-08-25" note is closed by this entry) · DGH-011 (the ZIP deliverable whose `/d/` download link is the next route to mount here) · DGH-014 (the token shape these routes redeem) · `bugs_open/245` (why no standing service holds object-store keys — which is why `/d/` will be a DB→302 and needs no credentials on this listener).
