...
- **status:** partial
- **status-evidence:** The routing half was fixed by `bugs_closed/060` (chassis v1.0.1167): a successful resolution now records the resolved type. The FAILURE half stayed broken until `bugs_open/239` (2026-08-10): a dispatch that could not be resolved fell through to the pod's own workflow and filed `owner_agent_type='generic'`, so the one column an operator checks to tell a substituted run from a real one said `generic` on exactly the runs where it mattered. This entry's own suggested fix shape — "selectWorkflow sets owner_agent_type to the resolved type" — is now implemented for both arms: on refusal the FAILED row carries the REQUESTED type (see SYS-090). Committed 2026-08-10, awaiting a fleet roll.
...
- **verify-later:** after the next fleet roll, run the five checks in bugs_open/239's "How to verify once it is LIVE"
