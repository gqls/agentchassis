...
- **open review question:** the refusal is deliberately scoped to `isOrchestrationAction` — non-orchestration actions keep the old own-default behaviour. Whether Priority 3 has any remaining legitimate consumer for orchestration actions that name nothing (beyond dedicated/spawned pods, whose job topics the intake census cannot see) is NOT established; the `DISPATCH_OWN_DEFAULT` warn line exists to make that population countable per pod before anyone narrows it further.
