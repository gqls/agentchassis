...
- **relations:** public-api (httpguard — the in-binary edge-trust sibling; its `CloudflareTunnel()` peer gate is the same trust boundary one layer down) · site-chatbot (the WireGuard leg's first proven consumer) · the Q2 delivery-only listener (unbuilt as of 2026-08-25 — register it when it ships).
