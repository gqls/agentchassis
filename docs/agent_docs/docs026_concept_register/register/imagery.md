...
- **status:** built, committed 2026-07-31 (`3b812161b` + `d671fb2b2`) — **inert until the next chassis roll**; its only consumer is check_undeployed_assets, itself reachable only via `design-discovery-agent` ← `improvement-sweep`, disabled since 2026-05-02 (bugs_open/083). So: built and unit-tested, NOT exercised in production.
- **status-evidence:** commits above; 8 mutation-verified guards in `check_undeployed_assets_test.go` + 2 in `platform/storage/brand_head_paths_test.go`; fleet-wide simulation against live prod recorded in docs024 `bugfix_142_undeployed_asset_detector/NOTES` §9.
