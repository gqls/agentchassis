...
- **status:** built, committed 2026-07-31 (`9f0a8ec5b`), **inert until the next chassis roll**
...
- **enforcement:** `TestNoRawRuntimeFillMarkerTestOutsideThisPackagesPredicate` reads the source tree and fails the build for any raw marker test outside `runtime_fill.go`, so a ninth copy cannot appear unnamed. It does not judge WHICH scope a site should use — it forces the choice through a named predicate. LANDMINE: its first version matched only the `Contains` family and missed `regexp.MustCompile`, reporting the tree clean while a call site sat in it; verify a new detector's first result by the method it replaces, not by itself. One allow-listed site (`rerender_single_page_action`) keeps a raw regexp because its `(?i)` makes it the tree's only case-insensitive marker test — an unresolved divergence, not an oversight.
- **verified call-site manifest (literal grep, 2026-07-31):** Go predicate-form NINE — 5 element-scoped, 3 whole-input by stated decision, 1 raw-by-allow-list — plus FOUR SQL-side copies (`check_required_fields_missing`, `check_component_standards`, `check_component_template_corrupted`, `check_empty_sections`' query) invisible to a `--include=*.go` grep, all asking the section question.
