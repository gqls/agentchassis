...
26 concepts (LNK-024, LNK-025, LNK-026 added post-freeze; LNK-025 and LNK-026 2026-07-31), consolidated from 56 raw extractions (28 unique blocks, each mechanically duplicated once in the cluster input file — see note in styling-render-pipeline.md) across units U01, U02, U05, U21, U23, U24d, U24f, U25.
...
- **status:** built, **council-APPROVED 2026-07-31 round 4** (9 approve / 5 advisory, none high; corr `4465f655-c6c6-49b4-a9b8-4ca7a5f647df`), **inert until the next chassis roll**
- **status-evidence:** Go changes are inert until an image is rebuilt and rolled. Verify at the pod with `strings /app/agent-chassis | grep -c RuntimeFillSpans` plus a positive control in the same exec. Measured before commit against live artefacts: assembled `vonc.com/index` (48,956 B) — old whole-input test exempts 100%, `RuntimeFillSpans` returns 2 spans covering 6,172 B (12.6%); assembled `vonc.com/provocations-index` (7,684 B) — 1 span, 1,400 B, 0 findings, byte-identical output.
