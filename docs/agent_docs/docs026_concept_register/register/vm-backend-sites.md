...
- **status:** partial
- **status-evidence:** the TOOL half is LIVE 2026-08-14 — migration `sql_for_agents/406_tool_suggester_requires_backend_gate.sql` gates tool-suggester's `load_library_tools` on `sites.deploy_config->'capabilities' ? 'backend'`, verified with a disagreeing pair (webdesign.uk offers `chat-input-box`; static control `ai-agent-orchestration.com` does not). Tag census at that date: 2 rows (`intent-probe` section-level, `chat-input-box` tool-level); capability census: 3 sites (webdesign.uk, noted.co.uk, relojistas.com). The SECTION half — plan D5's `load_components` NOT-tagged filter, the only thing guarding section-level `intent-probe` from generic planning — and the audit check comparing placed `requires-*` tags against site capabilities both remain UNBUILT, tracked as `bugs_open/276` (filed 2026-08-14 at the council's direction, REVISE round on corr c78ed496).
...
- **status:** deployed — first `--apply` succeeded 2026-08-02 late (idea.uk, Hetzner → alexis/leah; login/info/update/verify all 1000; the `.uk` parent published in ~2 minutes). The `host:create` fallback stayed unexercised: Cloudflare's host objects already existed registry-side
- **status-evidence:** live run output in `idea_uk_vm_site/RUNNING_NOTES` §X.39; two pre-auth traps found and fixed on the way: Nominet refusals are UNFRAMED text (now surfaced, not a bare traceback), and dual-stack connect prefers IPv6, which a v4-only allow-list refuses — client now pins AF_INET
