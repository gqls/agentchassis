...
- **status:** DB half LIVE (mig 344 applied + probe-verified 2026-08-08, trigger enabled, ledger empty, zero digests backfilled — deliberate); Go half committed, inert until the next chassis image rolls. Council corr `cffbfec4-3bec-4577-8844-d17c546ded3e` (submitted alongside the commit).
...
- **open review:** (a) WARN-and-proceed vs HOLD on divergence was decided as WARN (069 locks are the hold shape) — the council round may say otherwise; (b) `page_component_history` archives `content_data` only — should the page side gain a `rendered_html` archive the same way (STY-025's clobber class is the page-side sibling of 226)? (c) no ledger pruning shipped (~57 rows × ~4-8KB per full wave).
