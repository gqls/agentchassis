...
- **status:** partial
- **status-evidence:** Guard confirmed LIVE in the running pod as of 2026-07-16 (`v1.0.1123`) — grepped binary symbols `missingRequiredLLMFields`=2, `"escalating page to writer instead of blanking"`=1, `escalateRerenderToWriter`=4 (`aaa_fails_to_mend/004_HANDOFF_image_landing_blanks_article_body.md`). The broader structural fail-loud guard (`RenderComponentAction`, `v3_site_actions.go`) is committed to HEAD on branch `085_debug_and_feature_loops` but not yet in a deployed chassis image (`article-body-json-envelope` session notes, 2026-07-16).
- **what:** Go's `text/template` runs with `Option("missingkey=zero")` (`call_agent.go:1152`), so any template referencing a key absent from its data map — e.g. `{{.content}}` when a section's `content_data` holds a raw, never-unwrapped LLM JSON envelope instead of a top-level `content` key — renders **empty silently, no error**. Two independent guards now exist for two different call paths: (1) `rerender_page_sections_action.go` ~174-207 checks `missingRequiredLLMFields` before a scoped section re-render and escalates the whole page to the writer (`escalateRerenderToWriter`) instead of overwriting good HTML with a blank shell — this is what's live in prod; (2) `RenderComponentAction` in `v3_site_actions.go` (~line 1633) refuses to render at all when a component's `input_schema`-required content field is missing, logging "required content field(s) missing — refusing to render an empty section" — a broader, path-independent fail-loud, code-complete but awaiting the next chassis image. This is the same failure family as PBP-019/STY-004/STY-019's `sectionHasVisibleContent` empty-shell drop and the CLC-003 field-contract guard (tool-library.md) — a recurring platform pattern (schema says required, renderer says silently empty) rather than a one-off bug, and explicitly named as sharing a class with a "product-page defect" (`HANDOFF_2026-07-14_empty_product_sections.md`).
- **sources:** aaa_fails_to_mend/004_HANDOFF_image_landing_blanks_article_body.md (full, esp. §3, §7 code map); aaa_fails_to_mend/HANDOFF_2026-07-14_article_body_json_envelope.md; call_agent.go:1152; rerender_page_sections_action.go:174-207,488-520; v3_site_actions.go:1633
- **relations:** PBP-019 (sectionHasVisibleContent assembler filter); STY-004 (pre-store validation + empty-section filter); STY-019 (visible-content filter, same defence family); CLC-003 (component-lifecycle field-contract guard, tool-library.md — same class, different mechanism); the article-body-json-envelope incident's actual root cause (LLM output-token truncation, `max_tokens` 2000→8000, a SEPARATE and now-resolved issue from this structural rendering pattern — see that workstream for the data-recovery side)
- **verify-later:** confirm `RenderComponentAction`'s fail-loud path (`v3_site_actions.go` ~1633) ships in the next chassis image and is verified live in the pod, the same way the first guard was; whether any OTHER template-rendering call site besides `rerender_page_sections` and `RenderComponentAction` still has an unguarded `missingkey=zero` path
