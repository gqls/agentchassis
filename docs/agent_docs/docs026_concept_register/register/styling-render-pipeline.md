...
- **open review:** (a) no pruning shipped — DELETE-arm volume projected ~0.9MB/day worst case on a 29MB table; (b) loudness for section_editor/create_report_page overwrites (archive-only this round); (c) the save-path double-archive (app snapshot row + trigger row) accepted and stated; (d) whether the app snapshot's COALESCE artefact-embedding should be retired now the trigger owns recovery.
