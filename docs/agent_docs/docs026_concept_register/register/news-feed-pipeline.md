...
_Concept count retired 2026-08-09 — derived, not stored; run the drift pair in `000_concept_index.md`, or read `concept-register-drift-check`'s daily row (DOC-074)._ consolidated from 44 raw extractions (22 unique blocks, each duplicated
...
### NEWS-020 — Editorial features: the worked pattern for a multi-source, chart-backed analysis page
- **status:** deployed (one live instance, robot-hands.com; the PATTERN is reusable fleet-wide, the STORY selection is manual)
- **status-evidence:** `/insights/robot-demand-step-change.html` live 2026-08-19, verified at the served artefact (24.5 KB, 11 chart bars all resolving through registered facts, 5 per-observation citations, claimscan 0 findings across 6 components). Its `/insights/` section index live 2026-08-20 and top-nav eligible. Seeds: `sql_for_agents/491` (substrate), `492` (page), `493` (section index). Diagnosis run `3802fb10-c34f-4eff-9914-b2959c723bd5` (CONFIRMED) established the gap this pattern works around.
- **what:** A repeatable way to publish an editorial feature — a page about one current news story, built from the site's OWN feed where several channels cover the same story, with the background charted from cited facts. Six sections, **all existing components, nothing new built**: `hero` → `generic-text-block` (the analysis) → `evidence-timeseries` (the background series) → `evidence-chart` (magnitudes) → `generic-text-block` (the source cluster, linked out, title+link only) → `call-to-action`. Rows are **locked `permanent` with `rendered_html` written in the same statement** and the page is `rebuild_policy='owned'`, so no generic rebuild path can touch authored copy (the Thames `252`/`266` pattern). `content_data` copies the evidence register **exactly** — no display keys — and `rendered_html` is produced by executing each LIVE component template through `executeGoTemplate`'s semantics (`call_agent.go:1170`, `missingkey=zero`, `<no value>` stripped). Deployed by **assemble-only** `page-rerender` (no reason argument), which stitches stored `rendered_html` and never puts a model in the path.
  **Discoverability:** the section hub is `page_type='section-index'` at `/insights/index.html` — the ONLY way an `/insights/` page reaches the primary nav, because the child-URL bar exempts section indexes by `page_type` (`populate_nav_tables_action.go:377-384`); feature pages stay `in_header=false` and are reached from the hub. **Cross-linking back from news** uses `content_feed_items.published_page_id`, stamped on the items a feature drew on — the column's designed purpose, unwritten fleet-wide until this pattern (0 → 8 rows, 2026-08-20).
  **Lifecycle (owner ruling 2026-08-20):** indefinite retention at one stable URL, updated in place; retirement is deliberate de-listing, never deletion; **refresh cadence is set per FACT, not per page** (a series moves on its source's publication schedule, prose only on a narrative change), with tracker/feature/explainer tiers naming the dominant fact class.
- **why it matters:** it is the first thing on this platform that turns the news feed into original analysis rather than a headline list, and it does so **without new platform capability** — the honesty properties come for free from the chart components (a plotted point cannot carry its own number; a series observation carries its own citation, never the parent's). It is also the first live consumer of `evidence-timeseries` outside oufe, and the first non-zero `published_page_id` in the fleet's history.
- **landmines:** the feed's own render path **already detects same-story clusters and DISCARDS them** (`queryresolve/news_items.go` `newsTopicalTokens:185` / `capNewsItemsPerTool:222`) because the display requirement is that no one story dominates — reuse the heuristic, never that call site. `WriteFeedItemsAction`'s advertised "dedup" is **exact-URL-per-site only**, not cross-source. A `section-index` only reaches the header while the site is under `max_header_items` (default 8). Both traps are in `LANDMINES.md`.
- **sources:** `docs024_key_docs_latest/news_editorial_features/` (the standing five + `DESIGN_2026-08-19_starting_point.md`); `sql_for_agents/491`–`493`
- **relations:** NEWS-005/NEWS-006/NEWS-010 (the aspirational designs this partially realises — the publishing gap is now closed for hand-authored features, still open for automated ones); VIZ-001/VIZ-002/VIZ-003 (the chart substrate it consumes); CLM-001 (the claims gate); `features_open/001` (the pooled, per-site-angle projection of the same idea, still unbuilt and gated on pooling)
- **verify-later:** whether a second site adopts it; whether `published_page_id` gains a live reader (the `analysis_url` half of the news JSON is designed, not built); whether the hand-authored `/insights/` listing earns a `rebuild_insights_listing` sibling of `rebuild_blog_listing` (which queries `page_type='blog-post'` only)
