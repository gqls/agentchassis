...
- **status:** fully committed (`65c1984d0` keys+policy, `7a066dba1` writer guard, `b36163fb3` reconciler, `038211dd8` the REVISE response). **ALL THREE site-level switches default OFF**, so the chassis can roll with this and no site's behaviour changes until its structure spec opts in. Council `56e13695-17cb-48ec-bc6b-0371fde8b717` — REVISE round 1, resubmitted round 2.
...
- **verify-later:** the first replan of an opted-in site — `PLAN_PAGE_IDENTITY_SNAPPED` rows present, no new `planned`+never-deployed twin in `pages`, and the `owned_page_review:tool-*` refile loop on fundamentallyai stopping. Until then the counters are zero because nothing has run, which is **not** evidence either way.
