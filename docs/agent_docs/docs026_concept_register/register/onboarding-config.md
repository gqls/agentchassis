...
_Concept count retired 2026-08-09 — derived, not stored; run the drift pair in `000_concept_index.md`, or read `concept-register-drift-check`'s daily row (DOC-074)._ consolidated from 50 raw extractions (25 unique blocks, each present
...
### ONB-023 — Published contact is an opt-in intake answer, separate from the billing address
- **status:** built 2026-08-31, **INERT until the next chassis roll**; the producer half (`published_contact` on the box wire) is inert until the intake chat starts sending it.
- **status-evidence:** committed `162877051`; `Council-Submitted: 2026df60-b91e-4f1c-8425-a3f6f14e6309` (verdict pending — not an approved verdict). No migration and no backfill: verified 2026-08-31 that the seed had run exactly once fleet-wide and 0 current `site_specs` rows carried the address.
- **what:** `sites.email` is now **only** the address the SITE PUBLISHES. The address a customer PAID with is delivery-only and lives in `build_queue.direction.customer_email`, where `collect_external_orders` already put it and where the delivery dispatch already takes it from. `seedCustomerIdentity` (`seed_build_queue_action.go`) reads a new OPTIONAL direction key, `published_contact` (today `{email}`, shape open), and writes `sites.email` and the `contact` evidence fact **only** from it. Absent the key the site publishes no contact at all — the safe side, and the OWNER RULING of 2026-08-31 for the class. `collect_external_orders` passes `published_contact` through only when the box actually sends it, so an absence stays an absence rather than becoming an empty object a reader could mistake for an answer.
  **THE DEFECT THIS CLOSES (bugs_open/420, order-intake slug — the number is ambiguous, resolve by slug).** One column carried two facts with different consent, so on the first paid build the payer's address became the public footer contact on all 13 deployed pages. **A billing address is not consent to publish.** The subtler half: the seed also minted "Enquiries reach «payer»." as a **renderable registered evidence fact**, which meant deleting the address from the column and the pages could not make it stay gone — a rebuild consuming the register could legitimately re-emit it, validated clean. That is not hypothetical; it fired on order 1, established by elimination (the briefing spec written twelve minutes after seeding carried the address while the identity spec's contact block was all-null and the brief prose did not contain it).
  **Why `business_name` is still seeded and `contact` is not:** the business name is constitutive — no page can exist without naming the business, so giving it to a site-building order is inseparable from consenting to publish it. The email is severable: the site is fully buildable without it, and the intake never asked. The distinction is the SCOPE of the attestation, not its truth.
  Two render paths that synthesised `info@<domain>` when the column was empty were **removed** in the same commit: post-fix an empty column is the correct and common state, and fabricating an address nobody owns would make "the site publishes no contact" quietly false.
- **sources:** `platform/orchestration/actions/seed_build_queue_action.go` (`seedCustomerIdentity`, and its header's fourth boundary); `collect_external_orders_action.go` (`boxBriefOrder.PublishedContact`); `section_editor_actions.go`, `multipage_actions.go` (synthesis removals); `seed_customer_identity_test.go`; `bugs_open/420` (order-intake slug).
- **relations:** PAY-010 (order-intake collection — its "NOT YET (1) P5 seeding is unbuilt" line is now stale twice over: P5 shipped, and this entry changes what it seeds); council trail `7e3dd082` (the original P5 round — **none** of its boundaries is undone here; the change is to WHAT may be seeded, never to when or how); bugs_open/111 (the contact-block gates that make absence render as absence); bugs_open/326 (re-seeding as the canonical build retry — see the trap below).
- **⚠ TRAP, live until the roll:** a site whose `sites.email` has been cleared by hand can be **REFILLED** by re-seeding, because `build_queue.direction.customer_email` is durable and the pre-fix binding is fill-only-if-empty. Do not re-seed boxingonline.com before this rolls.
- **A guard obtained for free:** `validate_page_content` treats `sites.email` as the one licensed ("official") address. Post-fix the column no longer holds the payer address, so any residual occurrence of it on a page is **FLAGGED** by the existing hallucinated-email check instead of licensed.
- **verify-later:** the order-2 rehearsal — an order whose payer email differs from any contact the brief asks to publish; assert ZERO occurrences of the payer email across every page enumerated from `pages WHERE deployed` (served-page probe, with a demand control, NOT a DB sweep — four independent sweeps each read clean during the incident while a fifth store still served the address), no evidence fact rendering it, and the delivery email still reaching the payer.
