...
- **status:** built, calibrated, and BASELINED with mutation-proven growth detection (2026-08-03/04); NOT yet standing — a CARRIER is the only thing owed (a Go binary needs an image, unlike the mounted Python lint)
...
- **verify-later:** the CronJob wiring (image + overlay); whether the baseline/ratchet mode gets built or the census stays a by-hand read; whether `{{else}}`-branch fields (`empty_state_text`) get their own probe arm (list-empty rather than field-absent — the current positive control correctly refuses to vouch for them)
