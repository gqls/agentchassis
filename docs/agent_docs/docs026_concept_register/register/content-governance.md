...
- **status:** built and exercised; advisory, run by hand (not scheduled, not wired to any agent)
...
- **verify-later:** whether this belongs in a scheduled sweep rather than by-hand invocation; whether `scoreComponent` should refuse a fact-shaped `{{else}}` at BIRTH (`store_generated_component`), which would close the door at the write path rather than after the fact.
