...
- **council:** APPROVED, correlation `0008d215-7fc1-4a7c-a249-bce9ace27ef2`, round 2 (round 1 REVISE was a hollow test assertion — `mock.ExpectationsWereMet()` with zero expectations proves nothing about whether a query ran; fixed by asserting 401-not-500 and a mutation-proven positive control). Round-2 advisory (non-gating) answered here so the next reader has it: there is **no existing external-read route group** in core-manager to reuse (only `/health` and `/agents/bootstrap` sit outside AuthMiddleware — this reuses that pattern, which is the only precedent), and **tools-api cannot host this** — it is the island VM, not in this cluster, and has no path to `clients_db`; the tools-api/SAAS-001 reference is to the shared isolation *principle* (keep the DB perimeter closed), not reusable code.
...
### CHAT-011 — Chat brief submission: the bot COMMITS the brief (submit_brief tool, box order store, collection endpoints)
- **status:** deployed (box binary `c32a5121a` rolled 2026-08-26 night; a live probe conversation submitted a real brief through the public edge and received reference BR-8D2MA3, then acked as the probe)
- **what:** The webdesign.uk chat bot's brief-maker now closes its own loop: on the visitor's clear yes it calls a `submit_brief` tool (one tool round per request, mutation-proven), the box stores the brief in `orders.json` (store.go's atomic pattern) with a minted **BR-XXXXXX order reference** (unambiguous alphabet; deliberately NOT the voucher WD- prefix), and relays the reference to the visitor — a stored submission's reference reaches them even if the follow-up model call dies. Nothing is copy-pasted into an email any more. Collection contract per P4 §8: `GET /internal/orders` (uncollected, oldest first) + `POST /internal/orders/ack` (idempotent; re-ack keeps the original CollectedAt), bearer `ORDERS_API_TOKEN` (503 when unset, said loudly at startup), served over the PUBLIC edge via two exact-match nginx locations — **measured 2026-08-26: cluster pods have NO route to the box's wg0, the tunnel only carries box→cluster flows**, so HTTPS+bearer is the only transport that exists (P4 §2's own words). The conduct was loosened the same evening at the owner's direction: questions fit what the site IS (content/publication/affiliate-with-feed are normal work), a pasted description is taken as the brief rather than re-interviewed, and "where the content comes from" joined the elicitation list. Caps: 3 briefs/conversation, 40–10,000 chars, message 5000/body 16KB (pasted briefs), max_tokens 2048.
- **sources:** `box/chat-service/orders.go` (+`orders_http.go`, `orders_test.go`); `chat.go` (tool round), `claude.go` (tool wire support), `facts.go` (conduct); `box/webdesign.uk.nginx` (the two `/internal/` locations); commits `dbd6b9774` + `c32a5121a`
- **relations:** CHAT-010 (same box service, same facts frame); PAY-010 (the cluster half that collects these); billing external_reference (migration 659) — the join the owner ruled 2026-08-26: reference, never the brief
- **verify-later:** first REAL customer submission end to end; the box's ORDERS_API_TOKEN ↔ cluster WEBDESIGN_BOX_ORDERS_TOKEN pairing surviving a fleet release (terraform 047 carries it from 2026-08-26)
