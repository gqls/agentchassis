...
- **relations:** propagates the guard `collectPageSections` was given by council round `3ed2b792` and `scanBlogArticles` by round `170147b4` — both bug_historian, both required rather than volunteered · extends the Go source-sensor ratchet idiom of `TestNoDynamicallyConstructedItemTypes` (WFA/work-item minting), `TestEveryCheckProducedItemTypeIsClassified` and `TestFindingCodeScanEveryWriteIsRegistered` · sibling seams in the same pattern file: `bugs_open/384` (instance 1), `bugs_open/404` (instance 2, candidate 0 unclaimed), `bugs_open/399` (the "detectable rather than unrepresentable" argument one seam along)
- **verify-later / STATED RESIDUALS, so this entry is not read as wider coverage than it has:** (1) the **41 sites outside `platform/orchestration/actions`** are ADVISORY-only — no blocking cover; (2) **CONTENT loss is a different axis and is NOT covered** — `loadStoredSections`' own `_ = json.Unmarshal(cdJSON, &s.contentData)` keeps the row and empties it on corrupt JSON, so `offered == kept` and no count guard can see it; (3) the ratchet tracks the **shape**, so it cannot tell whether an existing count is actually acted on; (4) classifier parity between the two readers is verified but **not mechanically enforced** — no test invokes the Python classifier from Go; (5) the baseline can still be hand-lowered in the same commit that deletes a guard — a review surface, not a mechanical one. **The 207 figure is a census and will go stale by ADDITION:** re-run before quoting it — `git log --since=2026-08-26 --diff-filter=A -- platform/ internal/ pkg/ cmd/`.
