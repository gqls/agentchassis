...
- **status:** the table itself is **deployed** (migrations `465`/`466`, FK-enforced since `466`); the retention semantics below are **live since `566`** (applied 2026-08-23); the two guards in `589` are **written, council-submitted and NOT YET APPLIED** as of 2026-08-24 — do not read this entry as describing a constraint that exists. Update the status here when `589` applies.
...
- **verify-later:** whether `589` actually applied (`SELECT conname FROM pg_constraint WHERE conrelid='orchestration_status_vocabulary'::regclass` — expect `chk_status_not_terminal_and_pausable`), and whether arm 3 carries `AND NOT is_pausable`; also re-run the two health queries in the `LANDMINES.md` entry **with their row-count demand control**, since both return empty when the table is healthy and an empty or renamed table reads identically
