...
- **verify-later:** whether a live bundle has yet taken the whole-file arm — `SELECT count(*) FROM diagnosis_artifacts WHERE kind='bundle' AND body LIKE '%NO next_scope can render this path%'` (expect 0 until a chassis roll); and whether the 6-run cap_only figure goes to 0 after it
