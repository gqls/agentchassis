...
### BIZ-031 — Offer-track discovery checks: premise_incomplete + revenue_shape (B3), and the needs_strategy producer set
- **status:** ~~built + tested; inert until an image roll AND the check names land in quality-discovery-agent's checks array. Neither check has ever run.~~ **LIVE + WITNESSED, observe-only (2026-08-09 evening, same lane):** image v1.0.1276 pod-verified on both replicas (positive + negative grep), migration 361 added both names to the checks array (snapshot 54df4b7b), and both checks RAN on dartsonline.com, webdesign.uk and gaswholesalers.com via SCH-025 oneshot envelopes — zero `checks_failed`, zero `checks_unregistered`. Truthful silences hand-verified on the first two (dartsonline's conversion path positively present). Findings remain observe-only: items born `detected`, no triage carrier. Council trail: round 2 REVISE gates on the OWED `schema_migrations` ledger rows (358/359/361, owner-blocked), not on the mechanism.
- **what:** The first mechanical enforcement of BIZ-001's revenue-shape doctrine on LIVE SITES (the council's `review_mission` seat enforces it on platform code only). `check_premise_incomplete` (site-level): a DEPLOYED site (pages count — never `sites.status`, which lies) with no current `strategy` aspect or no `revenue_models.primary_model` files `needs_strategy` → domain-strategist; safe ONLY because the B2 refresh gate (migration 341 slug `domain_strategist_refresh_safe…`, 2026-08-08) completes without chaining a rebuild on deployed sites. `check_revenue_shape` branches on the site's OWN recorded primary_model: saas_tools/display_advertising → service-selling CTA lexicon in anchor/button TEXT (never prose) files `revenue_shape_cta` → component-template-fixer per page, chrome hits are residue → ONE capability_gap (fork-first is A4.4, unbuilt); lead_generation/direct_business → missing nav-reachable form page files `missing_conversion_path` → content-gap-planner; affiliate → capability_gap (no machinery); **sponsored_listings and any model with no case → ONE `capability_gap` of the new kind `GapRuleMissing` naming the model (see WII-014). ~~deliberate silence (no rule stated)~~ — CORRECTED 2026-08-11: v1 returned an empty result there, which is indistinguishable downstream from "examined and clean", and the council's `bug_historian` seat called it a swallow in round 3. A model is examined by HAVING A CASE; there is deliberately no Go-side list of known models to drift out of step with the strategist prompt that mints them (six as of 2026-08-11).** Both retract narrowly on positive observation (RFC_010); both new item types carry fail-closed verifiers re-running the SAME predicate, and are excluded from the claimed-item-timeout sweep (declared in 220 + live via migration 358, applied 2026-08-09).
- **needs_strategy PRODUCER SET (owner ruling 2026-08-02 / RFC_010 §1 — this statement is the ruling's condition):** (1) `vertical-exemplar-researcher` (greenfield builds, the original producer); (2) `check_premise_incomplete` (deployed estate, this entry). **Shared item_key shape: `strategy_<domain>`** — matched exactly by both so `idx_swi_dedup` holds ONE open strategy request per site whichever producer files first; the check's retraction closes either producer's open item on positively-observed completeness. Handler for both: domain-strategist.
- **sources:** platform/orchestration/actions/discovery_checks/check_premise_incomplete.go, check_revenue_shape.go (+ _test.go each); docs/agent_docs/sql_for_agents/358_revenue_shape_claim_timeout_exclusions.sql; features_open/030 §5.2–5.3; vigilant_designer_offer_analysis/PLAN_2026-08-02 §B3
- **relations:** BIZ-001 (the doctrine); B1 (site-review premise load, migration 340 slug `site_review_agent_loads_the_premise`) and B2 (the gate this check's safety depends on); CQ-019 (literal_markdown — the producer-set precedent); remit.go capability_gap shape; [[bugfix-149-checker-layer-queue]] (fatal unregistered names)
- **verify-later:** quality-discovery-agent checks array (are they named yet?); first observe-only run's findings vs the 10× direct_business population measured 2026-08-08; whether the greenfield arm negative control (needs_briefing on a fresh build) has been witnessed

### BIZ-032 — `offer-analyser` (B4): the offer/benefit auditor, and `site_specs` aspect `offer_ordering` — the estate's first ranked reader-priority artefact
- **status:** **LIVE + PROVEN END TO END, 2026-08-14** (config only, no image roll — all six actions were already registered). Migration `408_offer_analyser_agent.sql`, applied and ledger-recorded the same evening. Two live runs, both COMPLETED: gaswholesalers.com (`afe600a9-36c2-4056-bccb-d88692d8d02a`, 58s) and leopardessconsulting.co.uk (the degraded arm). ~~**Not yet wired into `improvement-loop`** — it runs only when hand-dispatched, deliberately, so the due-gate wiring is a separate migration.~~ **ENROLLED 2026-08-15, on the owner's call:** migration `409_improvement_loop_calls_the_offer_analyser.sql` (held as `_HOLD` since 08-14, renamed the day the owner said go) applied + ledger-recorded — every sweep's audit-due branch now runs `call_site_review → spawn_offer_analyser → call_offer_analyser → record_audit_pass`, error arm rejoining the sweep so one auditor cannot strand it. Single-owner audit clean after (promotion still solely `triage_findings`, migration 286). **`offer_ordering` has ZERO automated consumers today**; its first named consumer is the `copy_quality_two_stage` lane, which asked for exactly this artefact in `CONTRIB_2026-08-12` and does not yet read it.
- **what:** One LLM pass over a site's own recorded premise + its reachable page surface, producing **two** outputs. (1) **`site_specs` aspect `offer_ordering`** — a RANKED "what is this site's reader trying to do, so what should a page lead with?": keys `reader_goal`, `lead_with[]` (≤6, each `rank`/`point`/`why`/`from_field`/`differentiated`), `avoid_leading_with[]`, `inputs_missing[]`, `degraded`, `primary_model`, `spec_version`. Per SITE, never per page. It exists because B2 restored four premise fields (`satisfaction_condition`, `value_proposition`, `trust_threshold`, `recurring_value` — on 22 of 22 sites as of 2026-08-14) that are four prose paragraphs a human can read and **no pass can sort by**; the ORDERING is the thing that did not exist anywhere. (2) **work items under `audit_source='offer-analysis'`** where the live surface disagrees with that ordering or with the site's recorded revenue model — existing item types, existing handlers, no new routing (the lane's rule: a detector ships with its drain or not at all, `bugs_open/115`).
- **THE CLOSED VOCABULARY IS A `category` VOCABULARY, and that is a finding about `write_audit_findings`, not a style choice:** it classifies deterministically in Go from `category` + whether the named page exists. The `work_item_type` field that `site-review-agent`'s prompt asks for **is read by nothing.** B4's seven allowed values and their live routes: `gap` → needs_content_page/page-build-handler (page exists) or needs_content_planning/content-gap-planner (it does not) · `content`/`differentiation`/`structure` → content_rewrite/page-build-handler · `cta` → cta_improvement/component-template-fixer · `nav_restructure` → nav_restructure/component-template-fixer · `tone` → tone_shift/page-build-handler. An off-vocabulary value does not fail — it mints `audit_finding_<x>` aimed at content-gap-planner (6 such rows exist fleet-wide, 5 still `detected`).
- **⚠ LANDMINE — `findings_field` MUST address the ARRAY, never the object above it.** `write_audit_findings`'s parse switch handles a JSON string and a JSON array and has **no case for a JSON object** (`bugs_open/272`, filed 2026-08-14). `site-review-agent` asks its LLM for an object and points `findings_field` at it, and has therefore filed **zero** work items ever — no row anywhere carries `audit_source='site-review'`. B4 returns an object too (it has two outputs) but points `findings_field` at `offer_analysis.result.findings`. A migration guard asserts that string, because the failure mode is silence indistinguishable from a clean site. **If 272's Go fix later lands, do NOT "simplify" this back to the object path.**
- **⚠ LANDMINE — the degraded verdict is computed in SQL, and a re-run's omitted key SURVIVES.** `load_premise` returns `premise_fields_missing` (which of the four premise fields are empty on THIS site) and the prompt copies it verbatim; leopardessconsulting.co.uk reads `[recurring_value]`, every other site `[]`, so it discriminates. Separately, `write_site_spec` **deep-merges** (`siteSpecDeepMerge`, `site_spec_actions.go:513` — maps recurse, arrays replace), so a key the model forgets keeps the previous run's value **while looking current**; the prompt therefore demands every `ordering` key every run, including empty arrays and `false`. Freshness is the ROW's (`created_at`/`is_current`), never a timestamp in the payload.
- **⚠ LANDMINE — the offer surface is page METADATA, not page CONTENT, so a page-level verdict may be a HYPOTHESIS.** `load_offer_surface` passes name, type, nav membership, title and meta description, and not one word of what a page says. On the first run, 3 of 5 findings were grounded in what the surface really holds (a generic title; four service pages `not-in-nav`; two pages with no meta description); **2 were inferences about page bodies, which the model said so itself in the finding text.** Do not read B4's page-level verdicts as observations. v2 fix named, not built: add a bounded head-of-hero excerpt per page (the surface is already 14.9KB at webdesign.co.uk's 101 pages).
- **⚠ LANDMINE — the page-reachability predicate is an INLINED COPY of Go.** The surface uses the LINKABILITY floor (`status='active'` + `NOT (deployed_at IS NULL AND build_status='planned')`), which is the SQL of `datahelpers.PageMayBeLinkedPredicateFor` — deliberately NOT `PageHasShippedPredicateFor`, which would drop 11 pages that serve HTTP 200 (measured fleet-wide 2026-08-09, `links.go:297-317`). A config-only agent cannot call the helper, so **if that floor changes in Go, this string does not follow it.**
- **honesty ceiling (features_open/030 §4):** this platform collects **no engagement data**, so the analyser grades artefact-against-premise and nothing else; the prompt forbids "users want…" outright and a migration guard asserts the constraint is still in the prompt (that guard caught a real case-mismatch on the first trial run — `LIKE` is case-sensitive — so it is demonstrably not vacuous). Its inputs are also **unverified prose**: nothing on this estate claim-checks a `site_specs` row, so a false premise sentence is graded as truth (`bugs_open/161`'s shape one layer up). The owner ruled on 2026-08-14 that the claims audit be extended to cover `site_specs`; until then this ceiling stands.
- **sources:** docs/agent_docs/sql_for_agents/408_offer_analyser_agent.sql (+ _ROLLBACK); features_open/030 §5.4 and §7; vigilant_designer_offer_analysis/PLAN_2026-08-02 §B4 + decision log 2026-08-14; NOTES 2026-08-14 evening (predictions, results, both limits); copy_quality_two_stage/CONTRIB_2026-08-12_the_ordering_input_you_want_is_already_in_site_specs.md (the consumer's ask)
- **relations:** BIZ-001 (the revenue-shape doctrine it grades against); BIZ-031 (B3's checks — same lane, same premise substrate, `revenue_models.primary_model` is the field both read); B1 (`site-review-agent`'s premise load — the nearest live thing, and the one whose findings path is broken by 272); WII-014 (`GapRuleMissing` — the same "a silence reads as a clean bill" lesson this entry's degraded field applies to prose); [[bugfix-238-regeneration-drops-resolver-keys]] (the merge-vs-replace family); RFC_010 §1 (why a second producer on existing item types needs no architecture round, provided the producer set and key shape are stated — see below)
- **`audit_source='offer-analysis'` PRODUCER STATEMENT (RFC_010 §1's condition):** B4 is a new producer on five EXISTING item types (`content_rewrite`, `needs_content_page`, `nav_restructure`, `cta_improvement`, and `needs_content_planning` when a page name is not on the list). Dedup keys are minted by `write_audit_findings` and **carry the audit_source as their first segment** — `offer-analysis_<item_type>_<page_name>_<site_id>` — so B4's items can never collide with `design-audit`'s or any other producer's on the same page and type, and each producer's row deduplicates only against itself.
- **UPDATE 2026-08-15 (owning lane's review pass) — two claims above went stale within a day, and one defect of our own was fixed.** (1) ~~`bugs_open/272`~~ → **`bugs_closed/272`, fixed AND live on v1.0.1301** — the object shape now parses (run `b2c82a25`: 5 findings → 5 items under `audit_source='site-review'`, the first such rows ever). B4's array-path config **stays as-is**: belt-and-braces, and config must not assume a binary version. (2) The off-vocabulary minting is **fixed in code by `bugs_open/279`** (commit `d6d56e540`: unknown category → `capability_gap`, deferred, roadmap-surfaced; CI test guards the closed set) — **not live until the next chassis roll**, and B4's closed category vocabulary stays correct practice regardless: a `capability_gap` is still a finding that reached no handler. (3) Migration `421` (applied + recorded 2026-08-15) removed 10 doubled apostrophes from the live prompt — a dollar-quoting authoring slip in 408 (`''` inside `$prompt$` is two literal characters, not an escape); found by review, not by a symptom; both live runs had tolerated it.
- **verify-later:** ~~whether `improvement-loop` wiring has landed~~ (landed 2026-08-15, migration 409 — what is now unwitnessed is the first SWEEP-driven B4 run and the ordering rows growing past 2 of 22); whether `copy_quality_two_stage` reads `offer_ordering` (a first automated consumer changes this from an artefact to a contract); whether the 5 gaswholesalers items survive triage and what their handlers do with the two inferential ones (all 10 items still `detected`/unclaimed as of 2026-08-15 afternoon); whether the `why`-clause behavioural leak recurs across more runs than the two measured
