...
- **status:** built + tested (2026-08-09, vigilant_designer_offer_analysis B3); **inert until an image roll AND the check names land in quality-discovery-agent's checks array** (fatal-if-unregistered since bugfix_149 B4; IMP-016 observe-only first). Neither check has ever run.
