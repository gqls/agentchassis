...
def ratchet_name(line: str) -> str:
    """The workstream name a ratchet line accepts, stripped of its annotation.

    A line is written by hand as often as by --update-ratchet, and sessions
    annotate the ones they decide are one-offs:

        bugfix_161_register_ratifies    # one-off: SQL + repair script for ...
        docs/agent_docs/docs024_key_docs_latest/bugfix_134_optional_marker — single-bug fix lane

    Until 2026-08-04 the whole line was the key, so an annotated line matched
    nothing and its subsystem was reported as NEW on every run: 12 of the 53
    lines carried an annotation, and 12 of the 17 names in that day's "NEW
    since the ratchet" list were already accepted backlog. A ratchet that
    re-reports what it has accepted is the wallpaper this check exists to
    avoid, so the annotation — the reasoning, the most valuable part of the
    file — must not cost suppression.
    """
    name = line.split("#", 1)[0]
    name = re.split(r"\s+—\s+", name, maxsplit=1)[0]
    name = name.strip().rstrip("/")
    return name.rsplit("/", 1)[-1]  # tolerate a path-form line


def read_ratchet() -> dict:
    """name -> full line, so --update-ratchet can carry annotations forward."""
...
def main() -> int:
    ...
    if args.update_ratchet:
        # Carry each surviving name's existing line forward verbatim: the
        # annotation ("one-off, because …") is the reasoning a later session
        # needs, and regenerating bare names silently deletes all of it.
        kept = [ratchet.get(n, n) for n, _ in sorted(uncovered)]
        RATCHET.write_text(
            ...
            "# An inline `# …` annotation is preserved across regenerations and does\n"
            "# NOT stop the line suppressing its subsystem.\n"
