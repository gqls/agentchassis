-- ROLLBACK for 335 — restores the 2026-08-07 hand-written parking CTE (the
-- bugfix_205 version: park-at-5 literals inline) and removes the shared
-- mechanism. Does NOT restore the pre-205 unconditional reset — for that
-- emergency-only path see
-- bugfix_205_poison_pill_reaper/ROLLBACK_2026-08-07_reaper_pre_query.sql.
BEGIN;
...
DROP FUNCTION IF EXISTS business_intel.reap_stale_collection_tasks();
DROP TABLE IF EXISTS reaper_policies;

DO $do$
BEGIN
  IF (SELECT count(*) FROM scheduled_tasks
       WHERE name='stale-orchestration-reaper'
         AND pre_query LIKE '%UPDATE business_intel.collection_tasks%'
         AND pre_query NOT LIKE '%reap_stale_collection_tasks%') <> 1 THEN
    RAISE EXCEPTION 'pre_query not restored to the inline parking CTE';
  END IF;
END
$do$;
