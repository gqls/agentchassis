...
--
-- ✅ HOLD DISCHARGED AND APPLIED 2026-08-26 — the `_HOLD` suffix is off.
--
-- Both conditions were met and each was PROBED, not assumed:
--
--   1. ROLLED. Image v1.0.1345 carries check_archived_page_still_serving.go.
--   2. CAPABILITY, with a positive AND a negative control in the same query:
--      service_binary_capabilities kind='discovery_check' —
--        archived_page_still_serving  present on BOTH live chassis pods
--        site_unreachable             present (positive control)
--        …_NOTREAL                    0 rows (negative control)
--      ⚠ AND THE FLEET WAS MIXED WHEN FIRST CHECKED: over a 45-minute window,
--      95 of 428 pods carried it. That reading is a TRAP — those are short-lived
--      SPAWNED pods accumulated across the roll, not the two long-lived
--      deployment pods that actually run availability-discovery-agent. Narrowing
--      the window to 10 minutes gave 153 of 153 carrying it, zero without. **Ask
--      the window, not just the count**: a wide window on a spawning service
--      mixes pre- and post-roll pods and reads as a half-finished rollout.
--
-- Applied by hand 2026-08-26. Post-apply the checks array reads 3:
