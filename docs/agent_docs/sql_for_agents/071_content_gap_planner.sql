...
-- ============================================================================
-- Fix 1: content-gap-planner prompt edits (Defect 1 — duplicate surfaces)
-- ============================================================================
-- Applies:
--   1a. new_page example: remove hardcoded "generic-text-block"
--   1b. add_to_page example: "faq-section" -> "faq"
--   1c. add section-selection rules (no-pairing + use function name)
--
-- Writes the full corrected prompt_template back via jsonb_set. The new
-- value is dollar-quoted ($PROMPT$ ... $PROMPT$) so inner quotes/newlines
-- need no escaping. loadAgentDefinition reads per-spawn (no cache), so this
-- takes effect on the next content-gap-planner run. No chassis rebuild.
--
-- SAFETY: take a backup of the current value first (below), and review the
-- RETURNING output before committing if running inside a transaction.
-- ============================================================================

-- 0. Backup current prompt to a scratch table (optional but recommended)
...
If the gap needs its own page (e.g. a dedicated FAQ page, a pricing page), recommend creating one. Specify the page name, title, purpose, and sections from the Available list.
    ...
    ### C) Update site spec
         ...
         WHERE type = 'content-gap-planner'
             ...
             left(default_config #>> '{workflow,steps,plan_gaps,config,prompt_template}', 80) AS prompt_head;
