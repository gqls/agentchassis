...
--
-- The binary was probed on BOTH live chassis pods before applying, with controls
-- in the same exec: `header_slots` present (3), positive control `site_unreachable`
-- present (7), negative control `zzz_not_a_real_symbol_404` absent (0).
--
-- Applied by hand 2026-08-26. Post-apply, verified at the live object:
--   ai-agent-orchestration.com :: slots=9 cap=9 siblings_kept=2
-- The sibling count is the load-bearing half — `locale` and `analytics` survived
-- the merge, so the write set ONE path and did not replace `data`.
--
-- ⚠ STILL OWED: the nav rebuild. A declaration does nothing until
-- populate_nav_tables next runs for the site, and the served header is stale
-- until the re-render items it files have drained. See the bug file's §E.
--
