...
BEGIN
  ...
  -- the banned set: five patterns, each with a reason
  SELECT count(*) INTO ncorr FROM site_specs s2, jsonb_array_elements(s2.data->'banned_claims') b
   WHERE s2.site_id = '8ff093d5-1f19-453b-9439-a10379bbcd76' AND s2.aspect = 'evidence_base' AND s2.is_current
     AND length(b->>'pattern') > 5 AND length(b->>'reason') > 20;
  IF ncorr <> 5 THEN
