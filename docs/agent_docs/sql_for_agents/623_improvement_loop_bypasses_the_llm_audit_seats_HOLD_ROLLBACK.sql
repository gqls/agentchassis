-- 623_..._HOLD_ROLLBACK.sql - put the four LLM audit seats back on the loop's path.
...
BEGIN
    IF NOT EXISTS (
        ...
        RAISE EXCEPTION '623 ROLLBACK: not applied';
    END IF;
END $probe$;
BEGIN;
SELECT snapshot_agent('improvement-loop', '623_..._ROLLBACK: pre-restore');
...
DO $v$ BEGIN
    IF NOT EXISTS (SELECT 1 FROM agent_definitions WHERE type='improvement-loop' AND is_active AND COALESCE(is_snapshot,false)=false AND deleted_at IS NULL
        ...
        RAISE EXCEPTION '623 ROLLBACK verify: edge not restored';
    END IF;
END $v$;
COMMIT;
