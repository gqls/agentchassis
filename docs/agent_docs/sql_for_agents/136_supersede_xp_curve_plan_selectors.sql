...
-- Guard 1: the interaction check asserts `#tableWrap tr`. Accept EITHER
--   (a) a <table> statically inside #tableWrap, OR
--   (b) evidence the script renders rows into it: the wrapper is filled via
--       innerHTML, the <table>/<tr> markup lives after <script>.
-- Criteria are RUNTIME assertions (Tier-4 reads the rendered DOM), so a
-- JS-built table is legitimate and simply invisible to a static scan. Verified
-- for this component: id="tableWrap" at 7066, <script> at 7116, <table> at
-- 14384 -> the wrapper is `<div id="tableWrap"></div>` and JS fills it.
--
-- NB: strpos/substr, NOT a regex. Postgres ARE caps bounded repetition at 255
-- (RE_DUP_MAX), so `.{0,300}` raises "invalid repetition count(s)".
DO $$
DECLARE h text; p int; static_ok boolean; dynamic_ok boolean;
BEGIN
    ...
    IF p = 0 THEN
        RAISE EXCEPTION 'anchor id="tableWrap" not present in html_template — do not assert it';
    END IF;

    static_ok  := strpos(substr(h, p, 400), '<table') > 0;
    dynamic_ok := (h LIKE '%tableWrap%innerHTML%')
                  AND strpos(h, '<tr') > 0
                  AND strpos(h, '<table') > strpos(h, '<script');

    IF NOT (static_ok OR dynamic_ok) THEN
        RAISE EXCEPTION 'no <table> inside #tableWrap and no evidence the script renders rows into it — rename the check to curve-switch-EDIT and re-run';
    END IF;

    RAISE NOTICE 'tableWrap anchor verified (static=%, dynamic=%)', static_ok, dynamic_ok;
END $$;
...
tune base XP and growth, read per-level costs in a table beside a cumulative-XP
canvas chart. Client-side only.

## Source spec
Function `tool-xp-curve-designer` on gamesdesign.co.uk; inputs: curve type, base
XP, growth factor, max level; outputs: formula, stats row, per-level table,
cumulative canvas chart.
...
- **On every `input`/`change`:** `#formulaBox` shows the formula; `#statRow` the
  summary stats; the script re-renders a table into `#tableWrap` (an empty div
  in the template) with one row per level; `#xpChart` redraws on 2D canvas
  (area, line, dots ≤25 levels, grid).
...
- v1 (machine-written, 2026-07-09) asserted a table-body id and described a
  stats-strip id that the component never defines. Real: the rows are rendered
  by script into `#tableWrap`, and the stats element is `#statRow`.
- `#tableWrap tr` is a RUNTIME selector — the wrapper is empty in the template.
  Static checks must validate a selector's ANCHOR (`#tableWrap`), not its whole
  path, and must never delete a check merely because the DOM is built by JS.
