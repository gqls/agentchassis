...
-- DRAFT 2026-07-04; RE-DRAFTED 2026-07-06 (guideline pass): error_step added
-- INSIDE the step''s config — 001 §16: the coordinator reads
-- step.Config["error_step"]; step-level error_step is silently ignored. A
-- doc_notes write failure now routes to complete, so the caller still receives
-- the diagnosis (result_from "diagnosis" reads emit''s output, not persist''s).
-- Renumber 0NN. Requires 0NN_doc_plans_and_notes.sql applied and
-- persist_diagnosis_note deployed (it is, per Stage 2).
--
...
SET default_config = jsonb_set(
        jsonb_set(
            ...
            '{
                ...
                "config": {"error_step": "complete"},
                "next_step": "complete",
                "description": "Persist the diagnosis as a NOTES entry when the run has an explicit subject (skips otherwise). Never a fix; must never fail the diagnosis — error routes to complete.",
