-- 340 — allow subject_type='decision' on doc_notes (RFC_015 stage 1)
--
...
BEGIN
  ...
  IF found_def IS NULL THEN
    RAISE EXCEPTION 'doc_notes_subject_type_check absent — table not in the shape 340 expects; read it before applying';
  END IF;

  IF found_def LIKE '%decision%' THEN
    RAISE EXCEPTION 'doc_notes_subject_type_check already allows decision — 340 already applied; record with --record-only';
  END IF;

  IF found_def NOT LIKE '%component%' OR found_def NOT LIKE '%landmine%' THEN
    RAISE EXCEPTION 'constraint lacks component/landmine — vocabulary drifted from what 340 was written against';
  END IF;
END $$;
