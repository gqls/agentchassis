-- 646 ROLLBACK — return request_render_audit to the deterministic prefix.
--
...
BEGIN
  ...
  IF v_has <> 0 THEN
    RAISE EXCEPTION '646 rollback: rotate_coverage is still present — aborting rather than leaving the flag on with no table';
  END IF;
END $$;
