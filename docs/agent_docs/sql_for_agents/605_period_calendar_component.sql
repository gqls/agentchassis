...
BEGIN
  ...
  IF position($i${{.section_title}}$i$ in tpl) > 0 AND position($g${{if .section_title}$g$ in tpl) = 0 THEN
    RAISE EXCEPTION '605 VERIFY: {{.section_title}} is interpolated with NO {{if .section_title}} guard anywhere — an absent key renders the literal <no value> onto a live page';
  END IF;
  IF position($i${{.label}}$i$ in tpl) > 0 AND position($g${{if .label}$g$ in tpl) = 0 THEN
    RAISE EXCEPTION '605 VERIFY: {{.label}} is interpolated with NO {{if .label}} guard anywhere — an absent key renders the literal <no value> onto a live page';
  END IF;
  IF position($i${{.focus}}$i$ in tpl) > 0 AND position($g${{if .focus}$g$ in tpl) = 0 THEN
    RAISE EXCEPTION '605 VERIFY: {{.focus}} is interpolated with NO {{if .focus}} guard anywhere — an absent key renders the literal <no value> onto a live page';
  END IF;
