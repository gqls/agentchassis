-- 624_..._HOLD_ROLLBACK.sql - undo Phase 3: restore improvement-loop from 620's own
...
DO $probe$ BEGIN
    IF NOT EXISTS (SELECT 1 FROM agent_definitions WHERE type='reader-experience-auditor' AND deleted_at IS NULL) THEN
        RAISE EXCEPTION '624 ROLLBACK: not applied';
    END IF;
END $probe$;
BEGIN;
SELECT snapshot_agent('improvement-loop', '624_..._ROLLBACK: pre-restore');
DO $u$
...
BEGIN
    ...
    SELECT default_config INTO v_snap FROM agent_definitions_backup
     WHERE type='improvement-loop'
       AND snapshot_reason LIKE '624_acceptance_council_seats_record_only_and_seat_failures_are_rows_HOLD: pre-update%'
     ORDER BY snapshot_taken_at DESC LIMIT 1;
    IF v_snap IS NULL THEN RAISE EXCEPTION '624 ROLLBACK: the pre-update snapshot of improvement-loop was not found - restore by hand from the snapshot_agent row'; END IF;
    ...
    IF (SELECT count(*) FROM jsonb_object_keys((SELECT default_config #> '{workflow,steps}' FROM agent_definitions WHERE type='improvement-loop' AND is_active AND COALESCE(is_snapshot,false)=false AND deleted_at IS NULL))) <> 31 THEN
        RAISE EXCEPTION '624 ROLLBACK verify: improvement-loop does not have 31 steps after restore';
    END IF;
    RAISE NOTICE '624 ROLLBACK: done';
END $u$;
COMMIT;
