...
-- nothing on the web.
--
-- HELD, THEN RELEASED. The owner held this behind the Gemini thread finalising
-- the house voice, and that landed 2026-07-27 19:35 (voice_style_block in
-- agent_default_configs, platform/voicestyle, content-creator v1.0.1178). The
-- composed sentence below was rewritten against those rules, which is the whole
-- reason the hold existed: writing generic marketing copy onto six client sites
-- and then rewriting it a second time is worse than waiting.
--
-- What changed and why. The first draft read "Use our free interactive X in your
-- browser, with a companion guide explaining how it works." Against the house
-- voice that fails twice: "free interactive" is word-weight the claim does not
-- earn, and "explaining how it works" is a vague gesture where the rules ask for
-- the named action. It also gave the reader no reason to care, which the voice
-- block calls out directly.
--
-- ============================================================================
...
SELECT s.domain,
       ...
       'An interactive '
         || regexp_replace(regexp_replace(COALESCE(NULLIF(p.title, ''), p.name), '\s*\|.*$', ''), '^UK ', '')
         || ', free to run in the browser. The companion guide sets out the method '
         || 'behind it, so you can check the working.'         AS composed
FROM pages p
...
-- STEP 1 CAUGHT A REAL DEFECT, which is why it is not optional: pages.title
-- carries the nav suffix (" | Tools"), so the first draft of this expression
-- would have published "An interactive AI Agent ROI Estimator | Tools, free to
-- run in the browser." on 11 of the 17. The inner regexp_replace strips
-- everything from the first pipe. Re-verified: all 17 now read cleanly at
-- 142-171 characters.
--
...
--
-- UPDATE pages p
--    SET meta_description = 'An interactive '
--          || regexp_replace(regexp_replace(COALESCE(NULLIF(p.title, ''), p.name), '\s*\|.*$', ''), '^UK ', '')
--          || ', free to run in the browser. The companion guide sets out the method '
