...
--
-- NOT per-agent-instance (unlike agent_memory).
-- Shared resource — any agent can read via rag_lookup, any can write via rag_index.
--
-- Embedding column is vector(768) for nomic-embed-text.
-- If you change to a model with different dimensions, ALTER the column and rebuild the index.
...
CREATE TABLE IF NOT EXISTS knowledge_base (
    ...
    -- Classification
    collection VARCHAR(100) NOT NULL,  -- e.g. 'industry_sites', 'veterinary', 'energy_wholesale'
