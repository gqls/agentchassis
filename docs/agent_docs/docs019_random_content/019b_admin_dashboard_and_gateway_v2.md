...
**Work Items** — fetches `/api/v1/admin/work-items` with filters for status, site, and item type. Split-pane layout: item list on the left (40%), detail panel on the right (58%) when selected. Items are tagged with badges: purple "checkpoint" for checkpoint items, pink "needs input" for other review items.
...
### 3. Standard Review Items

All other `needs_human_review` items that aren't checkpoints or placeholder content.

**What the admin sees:**
- Heading: "Item Detail"
- Read-only spec in a collapsible JSON view
- Buttons: **Retry**, **Resolve**

### Common Actions (all item types)

| Action | What it does | Available when |
|--------|-------------|----------------|
| Retry | Resets to `triaged`, clears error and attempts | `failed`, `blocked` |
...
The dashboard uses `IMAGE_TAG` like all other services. The deploy step automatically updates the kustomize overlay tag.
...
make build-dashboard IMAGE_TAG=v1.0.878

# Push
make push-dashboard IMAGE_TAG=v1.0.878

# Deploy (updates kustomize tag, applies, waits for rollout)
make deploy-dashboard IMAGE_TAG=v1.0.878

# Full release (build + push + deploy)
make release-dashboard IMAGE_TAG=v1.0.878
...
docker build --no-cache -t docker.io/aqls/admin-dashboard:v1.0.878 \
...
**UI shows old version after deploy:** Docker cached an old layer. Rebuild with `--no-cache`. Hard-refresh browser with `Ctrl+Shift+R`.
