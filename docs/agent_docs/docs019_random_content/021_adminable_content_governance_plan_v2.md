...
| `write_audit_findings` | `write_audit_findings_action.go` | Skip findings where target component is locked |
| `fix_component_template` | `fix_component_template_action.go` | Soft check: skip if `locked_by IN ('admin', 'admin-removed', 'checkpoint')`, proceed if `deploy` or NULL |
| `page-build-handler` content writes | Various | Same soft check before overwriting `rendered_html` |
| `content-quality-audit` | Audit checks | Exclude locked components from audit scope |

Discovery agents (rows 1-2) do a hard gate — no work items created for locked components. Execution agents (rows 3-4) do a soft check that catches stale pre-lock work items but allows admin-initiated work (which can include `"force": true` in the spec).
...
### How locks interact with editing and task execution
...
The lock gates different things at different stages:

**Discovery and audit agents** check the lock before *creating* work items. The `AND pc.locked_at IS NULL` clause in their queries means: "this component is locked, don't suggest changes for it." No work items are created for locked components.

**Execution agents** (page-build-handler, rerender-pages, template fixers) do *not* check the lock when *processing* a work item they've been assigned. The work item is an explicit instruction — either from an admin action in the dashboard, or from the dispatch loop. The authorization happened at creation time.

This means there is no race condition when an admin edits a locked component:

1. Component is locked (`locked_by = 'admin'`)
2. Audit sweep runs → sees lock → creates no work items for it
3. Admin edits content in dashboard → API updates `content_data`, refreshes `locked_at`
4. API also creates a `page_rerender` work item
5. Rerender agent claims the item, reads all components on the page (including the locked one with new content), assembles the full page HTML, commits to git, deploys
6. Component stays locked throughout — no window for agents to interfere

The rerender agent doesn't *modify* the component — it *reads* `rendered_html` from all components and stitches them together with header and footer. The lock is irrelevant to it because it's a read operation.

This separation keeps things simple: locks prevent unsolicited changes (from audits and improvement sweeps), but don't block solicited changes (from admin edits or explicitly created work items).
...
### Discovery / Audit agents (check lock before creating work items)
...
| Content quality audit | Excludes from audit scope | Audits as normal |
...
Agents processing work items don't refuse locked components outright — the work item is the authorisation. But agents that *write directly to a component* do a soft check to catch stale work items from before the lock was applied.

| Agent Action | Locked Component | Unlocked Component |
|-------------|-----------------|-------------------|
| Page rerender (reads all components) | Reads normally, includes in page | Reads normally |
| Content rewriter (admin-created item) | Writes new content, lock stays | Writes new content |
| Template fixer (stale item from pre-lock) | Logs warning, skips, marks item `skipped` | Applies fix |
| Page-build-handler (writes rendered_html) | Checks `locked_by`: skips if `admin`, proceeds if `deploy` | Builds as instructed |

The check for write-path agents is: if `locked_by IN ('admin', 'admin-removed', 'checkpoint')`, skip. If `locked_by = 'deploy'` or `NULL`, proceed. This catches stale items without blocking admin-initiated work. Admin-created items can include `"force": true` in the spec to bypass even the soft check.
...
**Locks are additive, not destructive.** Locking a component doesn't remove it from the page or hide it from view. It just means "don't change this unsolicited." Agents can still read locked components for context (e.g. understanding the page's tone) — they just can't modify them or create work items about them. Editing a locked component does not require unlocking it. The lock stays on during and after the edit. Unlocking is a separate, deliberate action meaning "agents can improve this again."
