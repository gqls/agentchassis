...
**Work items created:** N × `add_tool` → `tool-deployer` (library) or `tool-generator` (novel)
...
- **Cross-linking tool pages and companion guides** — the guide references the tool, but the tool page doesn't link back to the guide. Both are discoverable independently (nav entry for tool, guide via blog listing) but there's no automatic bidirectional linking.
...
→ page-build-handler writes hero/intro/CTA sections
