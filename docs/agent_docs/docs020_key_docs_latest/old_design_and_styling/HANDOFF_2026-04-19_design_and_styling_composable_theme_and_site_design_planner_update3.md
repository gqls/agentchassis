...
6. **`install_site_composition_action.go`** — TO DO. Transactional wrapper: css_themes + style_collections INSERT + UPDATE sites.style_collection_id + write `resolved_composition` spec. The three resolvers already insert palette/typography/layout references; install ties them together.
...
| # | File | Contents |
|---|---|---|
| 4.6 | install_site_composition_action.go | Transactional wrapper, writes resolved_composition spec |
...
Deliverable 4.6: `install_site_composition_action.go`. The transactional wrapper that ties the three resolvers together — creates `css_themes` + `style_collections` rows with the palette/layout/typography FKs already resolved, updates `sites.style_collection_id`, and writes the `resolved_composition` spec.

Design question to resolve before writing:
- Currently palette + typography resolvers each open their own transaction and insert rows. If `install_site_composition` fails after they've committed, we have orphan palette/typography rows. Options: (a) accept orphans as low-cost (they're already isolated from sites until the FK is set), (b) restructure so resolvers return intent-only and install does all inserts in one transaction (larger refactor), (c) install has cleanup-on-failure logic. Decision for next turn.

Deliverable 5 (agent definition SQL) follows once install's shape is settled.
