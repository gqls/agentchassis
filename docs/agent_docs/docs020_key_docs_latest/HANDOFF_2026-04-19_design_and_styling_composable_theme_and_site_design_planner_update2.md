...
Five actions to write, in order:
...
2. **`resolve_composition_layout.go`** — TO DO. Tag-overlap scoring against layouts.industry_tags, alphabetical tiebreak, threshold-or-fallback, returns candidates list.
3. **`resolve_composition_typography.go`** — TO DO. Font-family exact match, layout default, fallback to sans-modern.
4. **`resolve_composition_palette.go`** — TO DO. Source priority cascade (fingerprint → mission → design_intent → archetype default).
5. **`install_site_composition.go`** — TO DO. Transactional INSERT palette + typography_set + css_themes + style_collections + UPDATE sites, then writes `resolved_composition` spec.
...
| # | File | Contents |
|---|---|---|
| 4.2 | resolve_composition_layout.go | Tag-overlap scoring |
| 4.3 | resolve_composition_typography.go | Font-match + fallback |
| 4.4 | resolve_composition_palette.go | Source priority cascade |
| 4.5 | install_site_composition.go | Transactional install + spec write |
| 5 | agent_site_design_planner.sql | agent_definitions INSERT |
| 6 | wire_needs_composition_item.sql + apply_adoption_plan patch | Pipeline integration |
| 7 | render_css_from_spec_action.go patch | Hard-fail tightening |
...
Continue Deliverable 4: write `resolve_composition_layout.go`. It's the meatiest of the resolvers — tag-overlap scoring against `layouts.industry_tags`. After layout, typography and palette are each much smaller. install_site_composition is the transaction wrapper that ties everything together.

Then Deliverable 5 (agent definition) follows naturally from the action signatures. Then wiring. Then the tightening and sweep.
