...
- [x] Prefix patch written 2026-04-21: `PATCH_rag_actions_nomic_prefixes.md` + `rag_actions_prefix_patch.go.txt`. Three surgical edits to `platform/orchestration/actions/rag_actions.go` — new `applyNomicPrefix` helper, one-line change in `RAGLookupAction`, one-line change in `RAGIndexAction`. Model-scoped (only triggers for `nomic-embed-*`), guards against double-prefix, logs `prefix_applied` for observability. Awaiting build + deploy.
- [ ] Apply nomic task prefix patch: commit, push, rebuild chassis, deploy new tag.
- [ ] Post-deploy verification: re-run `rag-test-agent`, confirm logs show `prefix_applied=true`.
- [ ] Log in production whether embeddings were prefixed (add `prefix_applied` to logs) so we can spot regressions. *— handled by patch above.*
