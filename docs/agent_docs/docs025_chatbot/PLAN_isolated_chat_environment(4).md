...
### Billing also largely exists — in the auth service (corrected again after reading it)

The auth service (separate service, own DB, JWT, gateway/proxy to the chassis)
already has more of this than expected:

- A **`subscriptions`** table and model: `user_id`, `tier`, `status`
  (`active`/`trialing`/`past_due`/`canceled`/`expired`), start/end/trial/cancelled
  dates, `payment_method`, and **`stripe_customer_id` / `stripe_subscription_id`**
  — so **Stripe is the assumed provider already**.
- **Tier definitions** (`free`/`basic`/`premium`/`enterprise`) with prices and
  limits, plus per-user **usage stats**, and a Stripe **`CheckoutSession`** type.
- The **JWT already carries `client_id` *and* `tier`** (with `user_id`, `email`,
  `role`, `permissions`). So entitlement is *already propagated in the token*.
- The shared key across both planes is **`client_id`**: auth (`Claims.ClientID`,
  `Project.ClientID`) ↔ chassis (`networks.client_id → clients.id`), with
  `clients.external_id` the back-reference.

So billing/identity is **mostly reuse**, not new. What this leaves genuinely new
or needing change, all modest:

1. **Reframe tiers around the right product nouns.** Existing limits are
   personas/projects/content_items (the old product). The site product wants
   sites/domains, builds-per-period, maintenance on/off, chat on/off. New tier
   *definitions*, not new infrastructure.
2. **Scoping + a one-off path.** The subscription is per-`user` and recurring;
   sites are per-`client`. Operator-at-scale + per-domain sell wants entitlement
   per **client** (recurring) **plus** a one-off path for the **$5-per-site**
   build (a credit/charge, not a subscription). The recurring half exists; the
   one-off half is a small addition.
3. **Provider abstraction for pluggability.** Current code is Stripe-shaped at the
   schema level (`stripe_*` columns). To honour "pluggable billing," generalise
   behind the subscription service's interface (`provider`,
   `provider_customer_id`, `provider_subscription_id`); Stripe stays the first
   impl. Minor schema generalisation.
4. **Confirm/complete Stripe wiring.** Models, table, and CRUD exist; live
   checkout-session creation and **webhooks** were not evident in what was read —
   verify before relying on them.

### Isolation bonus — entitlement travels in the JWT

Because the token already carries `client_id` + `tier`, the isolated satellite
can **validate the JWT and read tier/client without any call back to core or the
auth service** — a stateless fast path that keeps the boundary clean. Only the
one-off $5 credit needs a lookup; the recurring tier check is in the token.
