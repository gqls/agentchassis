...
## 8. Open questions / risks (with current stances)
...
- **Is maintained currency enough?** The base models keep improving and may close
  ...
  stay ahead. *Stance: accepted — we keep current deliberately, and the watchlist
  warrants **its own small recurring workflow** (a research workflow that scans
  for new model/tool/image-gen capabilities and updates the menu, triggering a
  re-run of ideation across domains).* 
- **Maintenance cost** of the capability watchlist and the multi-model ensemble —
  the ongoing price of the moat. *Stance: acknowledged — monitor it.*
- **Hosted defensibility** is low on static assets (§5); the product may not clear
  its own bar. *Stance: keep looking to improve it; pivot if we find something
  better, decided at the time.*
- **Pricing** — idea.uk is a higher-value result (a verified report + prototype),
  not a £2 day-pass. *Stance: start **cost-plus** (price to cover model/research
  cost plus a margin) until we've built something of real value, then revisit.*
- **Verification depth vs cost** — deeper verification is better but costs more
  per run. *Stance: acknowledged — find the level that keeps a run affordable.*
- **Scope creep into the build product** — the build bridge (§5.5) blurs idea.uk
  into the build-as-a-service plan. *Stance: boundaries kept deliberate and
  early.*
- **Sale-readiness** — idea.uk may be sold as a working site (§2). *Stance: keep
  its workflows/actions identifiable and minimal as we build, so it can be
  stripped down or moved to a minimal framework later; final structure deferred.*
