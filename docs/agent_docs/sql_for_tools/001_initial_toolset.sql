-- 065_tool_library_migration.sql
--
-- Adds fork-on-deploy support for content_components and seeds the
-- canonical tool library.
--
-- Fork model:
--   Library tool:  component_level = 'tool', forked_from IS NULL
--   Site fork:     component_level = 'tool', forked_from = <library_tool_id>
--
-- When deploying a tool to a site, the deployer copies the content_component
-- row (new ID, forked_from = original). The site's page_components references
-- the fork. Changes to the library tool do NOT cascade to existing forks.
--
-- To update all sites: create per-site work items (the improvement-loop way).
...
-- ============================================================
-- Each tool stores: <style> + <main> + <script>
-- The site head/header/footer are injected by compile_page_sections.
-- CSS uses var(--color-*) so brand comes from the site stylesheet.
--
-- render_mode = 'standalone' means no template substitution needed.
-- The html_template IS the final output (aside from head/header/footer).
...
INSERT INTO content_components (
    ...
    semantic_tags, html_template, input_schema, forked_from
                   ...
                   <div style="display: grid; grid-template-columns: 1fr 1fr; gap: 2rem;">
                       <div>
                           ...
                           <p style="font-size: 0.9rem;">
                               Seeing a higher conversion rate in Version B is great, but it might be random noise. This tool calculates the <strong>Z-Score</strong> to tell you if the difference is statistically real.
                           </p>
                       </div>
                       <div>
                           <h3 style="font-size: 1rem; color: var(--color-accent); margin-bottom: 0.5rem;">Confidence Level (95%)</h3>
                           <p style="font-size: 0.9rem;">
                               We use the standard 95% confidence interval (Z &gt; 1.96). This means there is less than a 5% chance your result happened by accident.
                           </p>
                       </div>
                   </div>
               </div>
           ...
           <script>
               const abInputs = document.querySelectorAll(".input-card input");
               function abCalc() {
                   const vA = parseInt(document.getElementById("visA").value);
                   const cA = parseInt(document.getElementById("convA").value);
                   const vB = parseInt(document.getElementById("visB").value);
                   const cB = parseInt(document.getElementById("convB").value);
                   ...
                   const se = Math.sqrt((pA * (1 - pA) / vA) + (pB * (1 - pB) / vB));
                   const z = (pB - pA) / se;
               ...
               abInputs.forEach(function(i) { i.addEventListener("input", abCalc); });
               abCalc();
           </script>',
             '{}'::jsonb,
             NULL
         )
    ON CONFLICT (function) WHERE function = 'tool-ab-test-calculator' DO NOTHING;
...
INSERT INTO content_components (
    ...
    semantic_tags, html_template, input_schema, forked_from
             ...
             '<style>
               ...
               .entropy-warning {
                   ...
                   display: none; font-size: 0.9rem; line-height: 1.5;
               }
           </style>
           ...
           <main class="container" style="padding-top: var(--space-lg, 3rem);">
               ...
               <div style="max-width: 600px; margin: 0 auto;">
                   ...
                   <p style="text-align: center; font-size: 0.85rem; color: #888; margin-top: 1.5rem;">
                       *Crack time assumes a high-end GPU cluster at 100 billion guesses/second.
                   </p>
               </div>
           </main>
    ...
    ON CONFLICT (function) WHERE function = 'tool-password-entropy' DO NOTHING;
...
INSERT INTO content_components (
    ...
    semantic_tags, html_template, input_schema, forked_from
             ...
             false,
             true,
             'Generate favicons from emoji or uploaded images. Live browser tab preview, ICO + PNG export, installation code snippet. All client-side.',
             '["generator", "favicon", "design", "icon", "webdev"]'::jsonb,
             -- NOTE: This template is large. Seeded separately in 065b if preferred.
             '',
             '{}'::jsonb,
             NULL
         )
    ON CONFLICT (function) WHERE function = 'tool-favicon-generator' DO NOTHING;


-- ----- Bayesian Ranking Calculator -----
INSERT INTO content_components (
    name, display_name, function, category, component_level, render_mode,
    is_dark_section, is_active, description,
    semantic_tags, html_template, input_schema, forked_from
) VALUES (
             'tool-bayesian-ranking',
             'Bayesian Ranking Calculator',
             'tool-bayesian-ranking',
             'tool-calculator',
             'tool',
             ...
             false,
             true,
             'Demonstrates why naive star ratings are misleading. Calculates Bayesian average scores that account for review count (confidence). Interactive comparison of two products.',
             '["calculator", "statistics", "rating", "ranking", "ecommerce", "reviews"]'::jsonb,
             '',
             '{}'::jsonb,
             NULL
         )
    ON CONFLICT (function) WHERE function = 'tool-bayesian-ranking' DO NOTHING;
...
INSERT INTO content_components (
    ...
    semantic_tags, html_template, input_schema, forked_from
             ...
             false,
             true,
             'Visual clip-path polygon editor. Drag points to shape elements, preset shapes (triangle, hexagon, pentagon, ribbon), CSS output with copy button. All client-side.',
             '["generator", "css", "clip-path", "design", "webdev", "visual"]'::jsonb,
             '',
             '{}'::jsonb,
             NULL
         )
    ON CONFLICT (function) WHERE function = 'tool-clip-path-builder' DO NOTHING;
...
INSERT INTO content_components (
    ...
    semantic_tags, html_template, input_schema, forked_from
             ...
             false,
             true,
             'Client-side meme creator. Upload image, add top/bottom text with Impact font, toggle black/white text, adjustable size, JPEG download. Privacy-first — no server upload.',
             '["generator", "meme", "image", "social-media", "fun"]'::jsonb,
             '',
             '{}'::jsonb,
             NULL
         )
    ON CONFLICT (function) WHERE function = 'tool-meme-generator' DO NOTHING;
...
INSERT INTO content_components (
    ...
    semantic_tags, html_template, input_schema, forked_from
             ...
             false,
             true,
             'Midjourney-style prompt builder. Toggle lighting, camera lens, render engine, style tags. Outputs combined prompt with aspect ratio and stylize parameters. Copy to clipboard.',
             '["generator", "ai", "prompt", "midjourney", "creative", "photography"]'::jsonb,
             '',
             '{}'::jsonb,
             NULL
         )
    ON CONFLICT (function) WHERE function = 'tool-prompt-architect' DO NOTHING;
...
INSERT INTO content_components (
    ...
    semantic_tags, html_template, input_schema, forked_from
             ...
             false,
             true,
             'Client-side background removal. Magic wand (flood-fill with adjustable tolerance) and manual eraser brush. Undo history, checkerboard transparency preview, PNG export. No server upload.',
             '["generator", "image", "background-removal", "photo-editing", "design"]'::jsonb,
             '',
             '{}'::jsonb,
             NULL
         )
    ON CONFLICT (function) WHERE function = 'tool-bg-remover' DO NOTHING;
...
SELECT function, display_name, category,
       ...
       CASE WHEN html_template = '' THEN 'NEEDS_TEMPLATE' ELSE 'OK' END as template_status
