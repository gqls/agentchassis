...
-- SNAPSHOT (rollback safety): full backup of the row being changed, taken inside
-- the txn BEFORE the UPDATE so it captures the pre-change html_template/input_schema
-- (a CREATE TABLE AS SELECT in-txn sees the old values). Rollback is at the foot of
-- this file. NAMEDATALEN=63: the backup table name is kept short.
CREATE TABLE IF NOT EXISTS content_components_bak_gamelist_tierd AS
SELECT * FROM content_components WHERE name = 'game-list_pre_037';
...
-- ============================================================================
-- ROLLBACK (component change) — restore the changed columns from the snapshot:
--   UPDATE content_components c
--   SET html_template = b.html_template,
--       input_schema  = b.input_schema,
--       render_mode   = b.render_mode,
--       updated_at    = NOW()
--   FROM content_components_bak_gamelist_tierd b
--   WHERE c.id = b.id;
-- Drop the snapshot once satisfied: DROP TABLE content_components_bak_gamelist_tierd;
-- ============================================================================
...
--      CREATE TABLE IF NOT EXISTS pages_bak_gamelist_rebuild AS
...
--      FROM pages_bak_gamelist_rebuild b WHERE p.id = b.id;
-- ============================================================================