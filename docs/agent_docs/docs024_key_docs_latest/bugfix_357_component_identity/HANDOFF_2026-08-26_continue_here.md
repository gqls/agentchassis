...
> ## ⚠⚠ UPDATED 2026-08-26 09:35Z — **PRECONDITION 4 HAS FAILED. DO NOT RUN 578.**
>
> Credit came back at 08:58Z and the tests ran. **The control PASSED** (a non-adopted page
> rebuilds end to end, `save_ran = t`, `COMPLETED`) and **the canary on the adopted page failed
> for the third time** — same hour, same conditions, only the page differing. Chasing that
> found the mechanism end to end:
>
> 1. `plan_sections` plans the page's one section onto **`adopted-fragment`** — the component
>    whose own seeded description reads *"Not for authoring: nothing should ever plan a page
>    section onto this component."* Nothing enforces that sentence.
> 2. `CompilePageSectionsAction` keeps only sections whose rendered `html != ""` and got
>    **0 of 1**, returning `skipped: true`, `reason: "no sections defined for page"`,
>    `page_body` length 0. So `page_content_0.response.page_html` never exists.
> 3. `assemble_page` looks for it and **never returns** — two inverse fallbacks in
>    `extractFieldValue` recurse forever, `fatal error: stack overflow`, container `exitCode: 2`,
>    **12,654 of 12,654 log lines identical**. Filed as **`bugs_open/408`**.
>
> **So an adopted row does not survive a rebuild — the rebuild crashes the pod.** 578 would bind
> **22 more live pages** to that same component. On this evidence each would then wedge its next
> rebuild, converting 22 mislabelled-but-rebuildable pages into 22 unrebuildable ones — **worse
> than the defect 357 describes.** Precondition 4 earned its place today.
>
> **[MEASURED]** steps 1–3. **[INFERRED]** that step 2's zero comes from `{{.body}}` rendering
> empty on a fresh write — the code keeps only sections whose html is non-empty, but **I have
> not observed the empty render itself.**
>
> **Three rebuilds on cv1 gave three different failure points, and that is the state of the
> evidence:** `request-index` (2 normal rows) **COMPLETED cleanly**; `index` (1
> `adopted-fragment` row) crashed as above; `how-it-works-index` (0 rows, normal components)
> failed earlier at `render_component` on an unrelated LLM schema mismatch. So the rival
> explanation — *cv1's specs are too thin to generate content* — is **weak**: one page built
> completely and another generated content and got as far as rendering it. **That leaves the
> `adopted-fragment` binding as the better-supported cause BY ELIMINATION, not by direct
> observation.**
>
> **The decisive test, for whoever picks this up:** re-run the canary on `index` and capture the
> **page-content-writer child orchestration by correlation while it is still alive** — its
> per-section output shows the adopted section's rendered html directly. The writer
> orchestration from my run had already been reaped when I went looking. It costs one pod crash
> and a 4-hour wedged run (`bugs_open/408`), which is why I stopped short of it.
>
> **The 578 recommendation does not depend on which way that goes: do not run it while the
> cause is open.** If it is the binding, 578 creates 22 unrebuildable pages. If it is something
> else, we still do not know what breaks the one adopted page we have.
...
| Phase 3 — repair the 22 | **NOT RUN, and must NOT be — precondition 4 FAILED 2026-08-26** (see banner) |
