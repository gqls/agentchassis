...
> empty on a fresh write (the code keeps only non-empty renders; I did not observe the empty
> render itself). **That distinction decides whether PHASE 2 itself needs a fix** — if an
> adopted-fragment section can never be regenerated by the build path, adoption is creating
> pages that cannot be rebuilt, and no repair should re-type rows onto it until that is
> answered. **That is the next session's first question.**
