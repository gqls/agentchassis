...
| | item | state |
|---|---|---|
| 1 | **📅 2026-08-16 — price the discovery-rotation ramp.** Calls AND tokens (baseline ~248k input tok/h idle; driven sweep ~806k/h). Queries at the foot of `sql_for_agents/395_enable_quality_discovery_rotation_slow_ramp.sql` | **PRICED 2026-08-15: ≈0 LLM calls/tokens** — discovery checks are LLM-free by code and by a measured full 22-site pass (08-09/10, 774 items filed, zero `llm_call_log` rows under any spelling). The real cost is rows, not tokens. **And the ramp has NOT started**: the 08-09/10 stamps hold every site un-due until 08-16 ~09:49Z. Evidence + tomorrow's check: NOTES 2026-08-15 entry. Residual: re-run 395's watch queries ~08-17 after a handful of sites |
