...
**(a) ✅ RULED 2026-08-19: FLOW A** — *"most times we will have a brief"*. **This does not mean a
human writes every brief.** It means the brief is always present, and it has three producers
under one contract: **human** (tens), **generated by a new brief-writer step** (thousands),
**third-party** from webdesign.uk customers (*"all sorts of quality and completeness"*). A thin
customer brief and a bare domain are the same problem — an incomplete brief brought up to
contract before dispatch — so the third-party case costs nothing extra. It plugs in without
touching anything downstream: `mission_brief` is already a `site_specs` aspect and
`domain-submitter` already writes it. Design, with four open questions:
**`PLAN_2026-08-19_one_flow_three_brief_sources.md`**. Three things it establishes:
- the brief-writer needs the **capability catalogue** as input, or it promises things we cannot
  build — `games` has no mechanism, and `capability_gap` already sits at **42 raised / 41
  deferred**;
- **the config-enabled HITL step the owner wants already exists**: `site_work_items.approval_mode`
  is honoured by the dispatcher (`load_work_item_actions.go:709` — a non-`auto` item is not
  dispatched until `status='approved'`). ⚠ **All 10,311 rows are `auto`; it has never once been
  exercised.** Prove both halves before designing on it;
- at 2,000 domains **per-brief review does not scale** — review-on-collision does, which argues
  for reviving the binding collision check (b) just deferred.
...
## 3c. 🏛 THE REGULATED-IDENTITY GATE — one prompt paragraph, five verified gaps

Owner, 2026-08-19: *"for now we reject a request until they write to us by email with proof."*

**What is live:** migration **`464`** → register **`CGV-032`** — a prose rule in
`domain-research-classifier`'s prompt: regulated business models (lending, credit broking,
"compare and apply", eligibility checkers routing into applications, debt advice, mortgage
arranging, insurance distribution, investment advice, payment services, claims management,
funeral plans) are *"not on the menu at all"* unless a mission brief explicitly asks. Proven by a
real A/B on `loanzy.uk` (run 1 "Lender Lead Facilitation" → run 2 "Borrowing Guides, Rights and
Regulations Overview").

**Five gaps, each verified independently 2026-08-19, not taken from the search report:**
1. **No Go code guards this anywhere** — no `*_guard.go`, no discovery check. One paragraph of
   prose in one prompt is the entire control.
2. **No fleet-wide banned-claim pattern for regulatory status.** `claims_global.go` holds 24
   patterns, all about accuracy overclaims; **zero** matches for fca/regulated/authorised/broker.
   **A new site is born with no protection against "We are FCA authorised and regulated".**
3. **Exactly 2 sites** carry a per-site pattern that would block it, both hand-seeded
   (verified by querying every current `evidence_base`).
4. **The rule is on the classifier ONLY** — verified: present in that agent's live config and no
   other. Bypass or re-run it and nothing downstream re-asserts it. This is the hole P11 named.
5. **The positive control is owed** — that the classifier *declines* is proven; that a brief which
   legitimately *asks* still works has never been tested.

Also: `build-briefing-agent` already writes *"FCA authorisation number — not yet known"* into its
gaps list and has no authority to stop anything, **because a gap is a note**.

**No proof mechanism exists of any kind.** The current test is "does the brief say so" — free
text a third-party customer can simply write. **Cheapest useful step, independent of the rest:
promote a regulated-status pattern into the FLEET-WIDE banned set** — one entry, protects all
~17 live sites instead of 2, and backstops every path where the classifier's prompt is bypassed.
It stops the *claim*, not the *positioning*, so it is a backstop and not the policy.

## 3d. 🌐 THE DOMAIN ESTATE — inventory blocked, classification built

Owner asked for the Nominet inventory, parked-vs-live separation, people's-name extraction, and
**~50 domains reserved as register-free test domains**.

- **Inventory is BLOCKED on one owner-run command.** Nominet is EPP over TLS, not a token API;
  credentials are complete (the TAG missing since 08-04 is present); the client is staged at
  `scripts/domains/epp.pl` and copied into `postgres-clients-0`. A session's permission
  classifier refuses to pipe a credentials file into a process. Recipe + traps:
  **`RUNBOOK_domain_inventory_and_classification.md`**. ⚠ **The EPP greeting is served to any
  IP — only `LOGIN_CODE=1000` proves the allowlist.**
- **Classification needs no credentials** and is built + validated:
  `scripts/domains/classify_nameservers.py`. Measured on the 152: **124 parked on Dan.com**, 6
  Cloudflare, 5 other marketplaces, **17 answering nothing (11 SERVFAIL, 6 NXDOMAIN)**.
  ⚠ **`NXDOMAIN` on a `.uk` name can mean the registration lapsed** — six portfolio domains are
  listed as owned and do not resolve. Check before planning a build on any of them.
  ⚠ **No Clook nameservers appear in the 152**, so that pattern is unverified — a class that has
  never matched is not evidence of absence.
- **People's-name extractor** built with three buckets:
  `scripts/domains/extract_person_name_domains.py`. Its first control run **failed** —
  `jamesbrown.co.uk`/`davidsmith.me.uk` came out MAYBE because common surnames are dictionary
  words — fixed with a surname list checked first. 0 NAME on the finance portfolio, which is the
  correct answer and proves it does not over-fire.
- **Reserved test domains: NONE YET, and not for want of trying.** All 152 portfolio domains are
  already named in the register (**0 eligible, 152 excluded**), and most parked ones are **twins
  of propositions we serve** — an unregistered test build on a twin competes with the real site,
  which is exactly what the register prevents. The set must come from the wider estate. Mechanism
  and picker: **`RESERVED_test_domains.md`** + `scripts/domains/pick_test_domains.py`.
  **The load-bearing design point: a reserved domain must be marked reserved**, because once the
  registry covers everything, "no entry" otherwise reads as "nobody got to it yet" and the next
  gap-filler destroys the control.
