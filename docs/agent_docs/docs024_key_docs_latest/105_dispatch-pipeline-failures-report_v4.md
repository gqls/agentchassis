...
| P5 | No feedback loop between section data requests and planning — stale requests block forever, no auto-close when resolved | Feedback loop bug | 12 vonc.com sections + all future builds | **UPDATED** — `plan_sections_action.go`: revised from pre-check to plan-then-reconcile. Ready sections auto-close stale data requests; deferred sections create new requests (skip duplicates) |
...
| P9 | Audit gap findings routed to `content_rewrite` on existing pages | Routing bug | 2 validation-failed rewrites + all future gap findings | **FIX WRITTEN** — `write_audit_findings_action.go`: Rule 4 routes `gap` findings on existing pages to `needs_content_page` (rebuild), not `content_rewrite` |
| P10 | Auditors produce recommendations (opinions) that pipeline tries to auto-fix as if they were bugs | Architectural — classification gap | ~10 items + ongoing false positives | **DEFERRED** — see `design-note-recommendation-specialists.md`. Proposed three-way classification (bug/recommendation/gap) with specialist agents + per-site approval mode. ~1 week project. |
...
| `plan_sections_action.go` | P5: plan-then-reconcile — auto-close stale data requests for ready sections, create requests for deferred, skip duplicates | ~80 |
