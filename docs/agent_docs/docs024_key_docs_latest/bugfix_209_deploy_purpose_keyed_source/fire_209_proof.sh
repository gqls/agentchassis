...
if [ "$MODE" = "pageflow" ]; then
  ...
  # 2026-08-22: reviewed_brief now comes from a load_site step (mirroring the
  # pageflow arm) — the old mapping read input_data.reviewed_brief, which this
  # script never supplied, and the STRICT input_mapping hard-failed the run
  # before site-work-orchestrator ever spawned ("source path
  # 'input_data.reviewed_brief' not found"). The swo arm had never been run.
  JSON='{"headers":{"correlation_id":"'$CORRELATION_ID'","orchestration_id":"'$ORCHESTRATION_ID'","request_id":"'$REQUEST_ID'","message_id":"'$MESSAGE_ID'","message_type":"request","client_id":"'$CLIENT_ID'","action":"process","sender":{"agent_id":"cli-user","agent_type":"cli","pod_name":"cli"},"timestamp":"'$TIMESTAMP'"},"config":{"workflow":{"start_step":"load_site","steps":{"load_site":{"action":"ensure_site_record","config":{},"description":"Load site record with content_data from database","output_field":"site_record","next_step":"spawn_orchestrator"},"spawn_orchestrator":{"action":"spawn_agent","config":{"role":"site_orchestrator","agent_type":"site-work-orchestrator"},"output_field":"orchestrator","next_step":"call_orchestrator","description":"Spawn site work orchestrator"},"call_orchestrator":{"action":"call_agent","config":{"target_role":"site_orchestrator","input_mapping":{"domain":"input_data.domain","mode":"input_data.mode","site_id":"input_data.site_id","reviewed_brief":"site_record.content_data"},"timeout_seconds":1800},"output_field":"orchestrator_result","next_step":"complete"},"complete":{"action":"complete_workflow","config":{"output_fields":["orchestrator_result"]},"description":"Build complete"}}}},"input_data":{"domain":"'$DOMAIN'","mode":"build","site_id":"'$SITE_ID'"}}'
fi
