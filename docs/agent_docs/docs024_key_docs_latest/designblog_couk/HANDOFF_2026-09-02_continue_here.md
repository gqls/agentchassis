...
| Orphaned-hero class CLOSURE — **repair wave UNOWNED** | components (asked 09-03 to claim or name the driver; census in 114) | 721 PROVEN mechanically 09-03 (field lands, 5→23 rows) but exactly ONE page visibly improved; 61 essentially untouched. **The only remaining gap is ownership of the repair wave**: page-scoped reason-carrying rerenders (`section_data_resolved`/`image_landed`) across the affected pages + SERVED-bytes verification. Neither ambient wave closes it (ordinary rerenders redeploy stored HTML; GTM wave is `stale_chrome`). 7th component (teaser-reveal-panel) rightly excluded → 425 fix-candidate 4. ⚠ every post-721 count measures the REPAIRED population — 721's own "5 of 158" is already 28 by addition |
