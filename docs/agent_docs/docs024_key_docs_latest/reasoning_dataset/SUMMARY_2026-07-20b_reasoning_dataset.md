# SUMMARY — reasoning dataset, as at 2026-07-20b (the go/no-go answered)

*Snapshot. Current state only — chronology lives in `README_where_we_are.md`
(plain prose) and `NOTES_reasoning_dataset.md` (technical). Written to be read
aloud.*

**The milestone that earns this snapshot:** the quality report landed, so the
blocking question — is this corpus worth a training run — now has a measured
answer rather than an estimate. `SUMMARY_2026-07-20_reasoning_dataset.md`, taken
this morning, still lists that report as pending work. That is what the position
looked like then and it stays that way; the series is the record.

> This thread broke the never-overwrite rule earlier today by rewriting the
> morning file in place. It was restored verbatim from commit `4b8d2bca0` and
> this snapshot took its own `b`-suffixed name, per CLAUDE.md's convention.

Series: `SUMMARY_reasoning_dataset_2026-07-18.md` (planning) →
`SUMMARY_2026-07-19_reasoning_dataset.md` (submissions under review) →
`SUMMARY_2026-07-20_reasoning_dataset.md` (bugs filed, A assigned) → **this one**.

*Next snapshot only when the five headings would genuinely differ again — most
likely when the council lane is mined or the volume decision is taken. Not
tomorrow by default.*
