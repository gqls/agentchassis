...
compare sections to the *page* and never to each other, and
`grep -rn "HAVING count(*) > 1" platform/ internal/ pkg/ scripts/` returns **nothing
fleet-wide**. A page shipped rendering itself twice and was found two days later by a
human running a census by hand. **A whole class of defect — "this table holds two of
something that should be unique" — had no detector of any kind.**
...
| 156 | **A page can hold content-identical duplicate `page_components` rows, render itself twice, and nothing in the platform detects it.** `vonc.com/about.html` shipped 12 rows that were 6 identical pairs (same `rendered_html`, `content_data` AND `component_id`), so a visitor read the whole page twice. **Every persisted source correctly says 6** — `site_plan_sections` (is_current), `pages.sections`, one `pages` row; only `page_components` doubled. **One write pass, not two**: 12 rows in 93 ms, positions **1..12 distinct**, `created_at` monotonic with `position` — which *refutes* the concurrent-save race (two writers each number 1..6, giving two rows per position), and adjacency (1,1,2,2,3,3) refutes a whole-loop re-run. So: one loop over a list that already held 12 entries. **[UNRECOVERABLE]** the producer — the list lived in `collected_data` and both `orchestration_states` and `site_work_items` for that window have aged out; `save_page_sections`, `CompilePageSections` and `loop_actions` each append once per item, so evidence reaches the loop's *item list* and stops. **The durable defect is detection**: no unique constraint, `content_hash` empty on all 12 rows, the save's five refusing guards all compare sections to the *page* and never to each other, and `grep -rn "HAVING count(*) > 1"` over `platform/ internal/ pkg/ scripts/` returns nothing fleet-wide. ⚠ **Do NOT fix with a unique index on `(page_id, slot_name)`** — fleet census: 17 duplicate groups, **11 legitimate** (`generic-text-block` 2–3× per page on five sites, differing content). Discriminator is content identity, not slot repetition. Fix candidates ordered by what closes the door: (1) dedup guard on `(slot_name, md5(content_data))` at the save — needs no root cause; (2) a `check_duplicate_sections` discovery check (wire it into an agent, per `149`); (3) populate `content_hash`; (4) hunt the producer | **OPEN, unowned** (detection gap). Filed 2026-07-30 answering HANDOFF D's first-move question in the `gauntlet_dead_cta` lane; **distinct from `151`** (near-duplicate copy across siblings — different cause, different fix; vonc's `151` instance went in separately at `55fe5eaf9`). Live instance **still doubled** — the six `DELETE`s + renumber were refused by the permission classifier, prepared and awaiting an owner call. See §9 *"The ROW NUMBERING distinguishes written-twice from written-once-from-a-doubled-list"* |
