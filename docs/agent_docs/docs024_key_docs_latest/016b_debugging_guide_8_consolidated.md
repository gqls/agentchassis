...
| 170 | **The style-collection chrome PIN applies no eligibility predicate at all.** `RenderHeader`/`RenderFooter` consult `coll.HeaderComponentID` **before** the by-function branch and dereference it with `GetComponentByID` — `WHERE id = $1 LIMIT 1`, no `is_active`, no `forked_from`, no `component_level`. Not a weaker predicate than `ResolveChromeComponent`: *no* predicate. So a pin bypasses both `bugs_closed/118` and `bugs_closed/167`. Live 2026-07-31: **three DEPLOYED sites** (ai-agent-orchestration.com, finetuning.uk, gaswholesalers.com) render `header-professional-dark`, `is_active=false`, on every page build | **OPEN, unowned**, filed 2026-07-31 by the 167 lane. This is **118's class on a FOURTH path** (deactivated component as chrome), not 167's — all four live pins are `component_level='site'`. 118's census counted code that *selects from a pool*, and a pin is not a selection, which is why it was never counted. Not fixed with 167 because repointing moves three live sites from a 3,637-char header to a 2,551-char one — a fleet-visible markup change, the one thing 118 established must not hide inside a zero-visible-change fix. **A fail-loud guard SHIPPED with 167** (`reportIneligibleChromePin`, ERROR log, changes nothing that renders — a test pins that, so making it repair goes red) after the council's `bug_historian` seat gated on the exposure being unguarded. ⚠ **LANDMINE for whoever fixes it: `forked_from IS NULL` is correct for pool selection and WRONG for a pin** — pinning a site to its own fork is the intended use. Measured over all four live pins, the pool predicate and the pin predicate disagree on exactly one row, `leopardessconsulting.co.uk`, which is the *correct* one; copying the pool predicate makes the detector's first output a false positive | filed 2026-07-31 |
