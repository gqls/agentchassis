...
| 031 | A wrong entry in the concept register (`styling-render-pipeline.md:389` + 2 `.buckets` copies) claims scoped page-rerender "SKIPS pages whose content hash is unchanged". No such code exists and `git log -S` shows it never did — but a council seat quoted it as "the pipeline's own contract" and blocked a correct plan at HIGH severity, costing a full round | filed 2026-07-19; **owned by another thread** — do not start a parallel fix |
