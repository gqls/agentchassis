...
| 199 | **`extractContentWithFallbacks` can hand a transport envelope to RENDER context, where `190`'s storage guard cannot see it** — when the LLM fell to the text path, `result` is a *string*, so the "unwrap `.result`" branch does not fire and the last-resort branch can return the whole envelope map. The render gate (`missingRequiredLLMFields`) catches this only for components declaring required `source:"llm"` fields; one with an **empty or optional-only** schema declares nothing — the same hole shape as `missingkey=zero` | **OPEN, filed 2026-08-04 at the council's explicit request.** The `bug_historian` seat objected that naming this path in `190`'s rationale and calling it "separable" is *"a silent decision"*, and that a tracked bug is the correct disposition. **`[UNMEASURED]`** — whether any live component has an empty/optional-only input schema, and whether the path has ever fired, is NOT measured; the census is the first task and decides whether this is a bug or a note. Deliberately NOT folded into `190`: a change to shared render-resolution behaviour has a different blast radius from a storage guard, and bundling it would be `bugs_closed/124`'s REJECTED shape. `190`'s `isLLMTransportEnvelope` predicate is directly reusable | filed 2026-08-04 by the `190` lane |
