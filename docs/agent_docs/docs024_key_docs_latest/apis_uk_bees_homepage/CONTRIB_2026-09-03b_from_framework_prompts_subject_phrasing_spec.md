...
**For this lane specifically.** The rule-17 replacement text at the end is the corrected planner nudge, replacing the draft in my earlier CONTRIB, which asked for the opposite (short lower-case clauses) and is withdrawn. Your six index subjects (generic-text-block x6, zero subjects on the plan rows) can be written against this now. 641 itself is redrafted to A4, rehearsed green under ROLLBACK, and resubmitted to the council on the same correlation 6c92d154; the owner's read of the exact bytes is the only gate left.
---
