...
- The fleet is on **agent-chassis `v1.0.1174`**, rolled at **15:11:15 UTC**
  (verified: `kubectl -n ai-persona-system get deploy agent-chassis -o jsonpath=...`
  plus the pod `startTime`). The binary was built at 14:58 UTC and contains every
  Go commit through `e96d42226` (14:52 UTC). **A lot of threads were parked
  waiting for exactly this roll** — those are marked "roll-unblocked" below.
...
### The page writer is armed to fail across the whole estate tonight

`bugs_open/112`, filed today by the Gemini thread's own triage sweep, **OPEN and
unowned**. At 14:34 UTC the `page-content-writer` agent was switched to Gemini in
the live database. Spawned agent pods do not inherit the main deployment's
secrets — their environment is an explicit allow-list built in Go
(`platform/orchestration/actions/spawn_actions.go:2440-2518`), and it injects
`ANTHROPIC_API_KEY` and `GROK_API_KEY` but **has no `GEMINI_API_KEY` block at
all**. So the writer will now fail at client construction, the step has no
`error_step`, and the failure takes down the whole page build — every section of
every page.

It has not fired yet only because no page build has reached the writer since the
flip (`llm_call_log` has zero Gemini rows). The bug file predicts the first
casualty is the scheduled `model-directory-publish` at **~20:25 UTC tonight**.

**Do first:** the bug file's own interim mitigation, which needs no code and is
live the moment it is written — revert `page-content-writer.generate_content` to
`anthropic` / `claude-sonnet-4-6` in the DB. That un-arms the failure without
touching the Gemini client fix, which is correct and stays live. The proper fix
(ten lines in `spawn_actions.go`, mirroring the `ANTHROPIC_API_KEY` block) is Go
and needs a build and a roll.

**But it reverses another thread's deliberate work**, so it is also an owner call
in §C. Note the Gemini thread's own cost measurement argues for the revert
independently: Gemini spends ~1,815 thinking tokens per section against Claude's
zero — roughly ten times the billable output per section, across the estate.

**Open:** `/home/ant/projects/agentchassis/bugs_open/112_HANDOFF_2026-07-27_spawned_pods_get_no_gemini_api_key_so_the_writer_flip_cannot_work.md`
...
**Do first:** apply seeds **207 → 209 → 210** in that order, but re-seed **208**'s
`base_url` first — the committed value `/api/gripper/v1` sits outside the island
Caddy allow-list and would 404 every tick, silently, for ever. It must be
`/api/v1/tools/gripper`.
**Open:** `docs/agent_docs/docs024_key_docs_latest/robot_hands_gripper_dossier/DESIGN_2026-07-24_gripper_dossier_pilot.md`
...
**Do first:** fire the revalidator with its shipped `dry_run=true` against the 370
parked items, and read `result.revalidation` and `resolution_path` before letting
it close anything.
...
**Do first:** `bugs_open/083` — add the one-line log that captures the upstream
error in the two `tools-api` handlers that currently throw it away, then
**rebuild the island** (RUNBOOK §5) or the fix is inert.
...
**Blocked by:** `bugs_open/112` (§0 — the writer physically cannot get an API key
in a spawned pod) and `bugs_open/029` (hung spawns have halted builds fleet-wide
since 19 July). Also needs an owner call on which live page may be mutated (§C).
...
19:49 UTC tick.
**Do first (free):** `SELECT count(*) FROM content_feed_items cfi JOIN sites s ON
s.id=cfi.site_id WHERE s.domain='webdesign.co.uk';` — confirm items landed before
touching anything else. If the 19:49 tick also failed, this is blocked on 029.
...
**40 open files in `/bugs_open/`, 90 closed, as of 16:05 UTC today** (`ls -1
bugs_open/*.md | wc -l`). The old version of this file carried a bug table; it is
...
**The diagnosis queue is empty** (`SELECT ... WHERE item_type='needs_diagnosis'
AND status='awaiting_diagnosis'` returns nothing; 19 complete, 4 cancelled, 4
failed). So nothing is sitting waiting for a diagnosis run right now.
