...
**Structural fix.** Put `110` and `111` into the canonical migrations directory in number order, so a directory-based migrate applies the full ordered set instead of stopping at the last filed migration (`109`). Hand-applied one-offs that live outside that directory get reverted whenever the deploy re-runs the directory — which is the mechanism behind this whole incident.
