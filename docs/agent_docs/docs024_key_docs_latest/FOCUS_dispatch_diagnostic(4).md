...
## Verification status (2026-05-15 end of session)

**Phase 2G + 2H verified end-to-end at scale.** Seven hero items for robot-hands.com processed through the full chain — discovery, audit, triage, dispatch, image-build-handler, store_asset, deploy_image_asset, mark_work_item_complete — without manual intervention beyond initial unsticking. All seven assets committed to git within ~5 minutes.

**One item still outstanding:** the icon (`icon_cross_technology`) is in a corrupted state machine combination (`status=triaged` with stale `claimed_at`). Two attempts remaining. The icon_dimensions_patch is reportedly deployed but unverified — see `TODO_imagery_followups.md` items 1-5.

**Findings during verification, captured as architectural questions Q1-Q4 below:**
- Q1 — discovery emissions auto-triage (still open)
- Q2 — scheduled audit runs (still open)
- Q3 — dispatch architecture and the NOT EXISTS blocker (researched in depth this session)
- Q4 — what the `pipeline` field is for (researched, decision recorded, implementation pending)

**Concrete next steps recorded in `TODO_imagery_followups.md`** — single reference for all open items, organised by area with severity ratings.
