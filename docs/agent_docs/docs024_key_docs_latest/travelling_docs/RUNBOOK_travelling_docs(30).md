...
**Last updated:** 2026-07-09 (rev 33 — TASK 3 PROVEN: the system wrote its own first PLAN, with real selectors. New incident: index_plan hangs (rag_index embeds per chunk with no deadline) — bypass drafted; env-prefix bit again)
...
**Position as of 2026-07-09: TASK 3 PROVEN ✓ (auto-PLAN written) · index_plan HANG — apply the bypass, then Task 4 / the game**
...
**Confirm the review with three cheap checks (paste):**
```sql
-- a) provenance + size of the auto-PLAN
SELECT subject_key, source, is_current, length(body) AS body_len,
       body LIKE '%```criteria%' AS has_fence, created_at
FROM doc_plans WHERE source = 'tool-generator' ORDER BY created_at DESC LIMIT 3;

-- b) the composer did NOT invent selectors — they exist in the component HTML
SELECT html_template LIKE '%id="curveType"%'   AS has_curvetype,
       html_template LIKE '%id="xpTableBody"%' AS has_xptablebody
FROM content_components WHERE function = 'tool-xp-curve-designer';

-- c) did any chunk reach the KB before the hang?
...
`rag_index` chunks the PLAN then calls `GenerateEmbedding` **per chunk** against
`ollama-adapter…:11434`. Embedding *errors* are non-fatal by design ("store
without embeddings"); a **stall is not an error**, and no deadline exists on that
call. `config.error_step` routes errors, so it never fired. Banked as 016b v8:
**error containment does not protect against a hang.**

*Evidence to confirm (the pod is still alive — grab it before the reaper):*
```bash
kubectl -n ai-persona-system get pods | grep -iE 'chassis|ollama'
kubectl -n ai-persona-system get svc  | grep -i ollama
P=$(kubectl -n ai-persona-system get pods -o name | grep chassis | head -1)
kubectl -n ai-persona-system logs "$P" --tail=2000 | grep -E '46cd5299|rag_index'
```
Expect `rag_index: chunked content` and then silence.

*Stopgap (DB-only, reversible) — `drafts/0NN_bypass_index_plan_until_embed_timeout.sql`:*
re-points `write_plan.next_step` to `complete`, leaving `index_plan` defined,
unreachable, and annotated with why. Nothing is lost — the PLAN is persisted by
`write_plan` **before** `index_plan`, and the KB copy is derived.
*Structural fix (next chassis build):* `context.WithTimeout` around
`GenerateEmbedding` (or `http.Client{Timeout}` in `aiservice.OllamaClient`), so a
stall degrades into the non-fatal path the action already has; then re-enable
`index_plan` with the one-line UPDATE in the file tail.
...
**Next steps, in order:**
1. Apply the bypass migration (1 snapshot, 2x UPDATE 1, guard, COMMIT); verify
   `write_next = complete`.
2. Paste the three confirmation queries above (closes the composer review).
3. Then the game: `tool-recreation-handler` on the economy-simulator page —
   Task-4 proof (first `fix`-category NOTES row), now that its note template no
   longer reaches unverified keys.
