...
**Created:** 2026-07-04
**Last updated:** 2026-07-06 (rev 19 — snapshot mechanism switched to the EXISTING snapshot_agent() function (side-table draft superseded un-applied); 3b.3 VERIFIED; 3b.4 attempt-1 subjectless — env vars not exported; script banner hardened)
...
**Position as of 2026-07-06 (late): 3b.3 VERIFIED — snapshots + positive run pending**
Stages 0–2 ✅ · 3a ✅ · 3b.1 ✅ · 3b.2 APPLIED ✓ · 3b.3 ✅ (mapping paths exact;
both contracts t/t) ·
**▶ NEXT, in order:**
**(1) retroactive snapshots via the EXISTING function** (side-table draft
superseded un-applied):
```sql
SELECT snapshot_agent('diagnose-agent',        '2026-07-06 retroactive: post-3b state (arc updates 1-4 pre-dated the rule)');
SELECT snapshot_agent('diagnose-orchestrator', '2026-07-06 retroactive: post-3b state (arc updates 1-4 pre-dated the rule)');
SELECT type, version, is_snapshot, deleted_at IS NOT NULL AS soft_deleted, created_at
FROM agent_definitions
WHERE type IN ('diagnose-agent','diagnose-orchestrator') ORDER BY type, version;
```
(paste the inspect — it reveals the convention; `\sf snapshot_agent(text,text)`
optional) ·
**(2) 3b.4 positive run — env vars on the SAME line (or exported):**
...
(banner MUST show a `Subject:` line; runs `03bebec9…`/`324456a9…` were
subjectless skip-regressions) ·
...
Until its write-location is confirmed (`\sf snapshot_agent(text,text)` or the
post-call inspect in §0 NEXT), FUTURE current-row selectors add
`AND is_snapshot IS NOT TRUE` — harmless if snapshots live elsewhere,
load-bearing if they are higher-version in-table rows. The side-table draft
