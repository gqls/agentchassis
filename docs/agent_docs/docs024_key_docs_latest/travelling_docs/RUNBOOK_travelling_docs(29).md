...
**Last updated:** 2026-07-09 (rev 32 — compose_plan diagnosed: TEMPLATE_FIELD_ERROR on `.generated_html.result` (text outputs arrive as bare strings). Fix drafted + three Task-4 note templates hardened with the proven `| toJSON` form)
...
**Position as of 2026-07-09: tool creation RESTORED ✓ · PLAN-write fault FOUND + fix drafted → apply, then re-run 085 with a NEW function name**
...
### ▶ WHERE WE ARE (2026-07-09): tool creation restored; PLAN-write fault found

**Closed:** the schema drift (`0NN_add_component_provenance.sql` applied;
`varchar(100)`/`varchar(255)`, mirrored — the orchestration id is varchar, not
uuid). Pre-flight on the three unchecked tables: 0 missing columns, both
`ON CONFLICT` unique indexes present. Run `f3bec131…` **created the tool**:
component `f70cce61…` (`component_level=tool`, active, real HTML — `save_tool`
hard-fails on empty `html_content` and enforces the tool-doc header gate), tool
page, guide page, nav.

**Found:** `agent_error_log` (orchestration `036cd3bd…`, 11:41:36) —
> `step compose_plan failed: … execute_llm_prompt: failed to render prompt template: … at <.generated_html.result>: can't evaluate field result in type interface {}` · `TEMPLATE_FIELD_ERROR`

The template I wrote was wrong, not the action. **The rule (proven, not
assumed):** `output_format: "text"` hands the prompt template the **bare
string** → use `{{.generated_html}}`; `output_format: "json"` hands it a map →
use `{{.tool_analysis.result | toJSON}}` (the form already live in
`recreate_tool`). Action **config** paths are a different resolver and keep
`.result` (`save_tool`'s `html_content: "generated_html.result"` resolved fine
in the same run). Corroboration: `{{.site_record.domain}}` renders fine in that
same template, so map traversal was never the issue.

**Containment worked as designed:** the run still finished at `complete` and
the tool exists — a docs failure did not fail tool creation. Task 3 is
**half-proven**: routing correct, PLAN body not produced.

**Fix — `drafts/0NN_fix_prompt_template_field_paths.sql`** (four snapshots, one
transaction, idempotent string replaces):
1. `tool-generator.compose_plan` — the blocker: `{{.generated_html.result}}` →
   `{{.generated_html}}`.
2. `tool-recreation-handler.compose_note` — **same class of bug, caught before
   it fired:** my Task-4 template reached `{{.tool_analysis.result.tool_name}}`
   / `.tool_type` / `.purpose`, keys never verified → replaced with the proven
   `{{.tool_analysis.result | toJSON}}` dump; `{{.deploy_result | toJSON}}`.
3. `component-template-fixer.compose_note` — `{{.input_data | toJSON}}`,
   `{{.fix_result | toJSON}}`.
4. `tool-improver.compose_note` — `{{.update_result | toJSON}}`.

**Your steps:**
```bash
psql "$CLIENTS_DB_URL" -f drafts/0NN_fix_prompt_template_field_paths.sql   # 4 snapshot NOTICEs, 4x UPDATE 1, DO, COMMIT
# then re-run with a NEW function name (tool-xp-curve-designer now occupies the
# unique index: function WHERE component_level='tool' AND forked_from IS NULL AND is_active):
...
```
Then paste the `doc_plans` proof select **and the PLAN body** (the one-time
composer review). That closes Task 3.

**Two schema traps banked (016b v7):**
- `agent_error_log.orchestration_id` is **`text`** — do not cast the literal to
  `uuid` (`operator does not exist: text = uuid`). The §0-REF query is corrected.
- **Provenance stamps the chassis, not the agent:** the new component shows
  `source_agent_type = generic` (pods are `agent-chassis-*`). So the planned
  `source_agent` fallback to `ExecutionContext.Sender.AgentType` is **dropped** —
  it would ship the same emptiness under a new name. The config-declared
  `plan_source` / `note_source` are the reliable provenance, and the doc actions
  already carry them.

**Backfill note:** `tool-xp-curve-designer` (created today) has no PLAN — it
predates the working hook. Leave it; a later improve/recreate run can write one.

**Canonical migration still parked:**
`docs/agent_docs/docs019_…/NNN_add_component_provenance.sql` — `cat` + diff it
against what we applied, then move it into the migrations directory with a real
number.
