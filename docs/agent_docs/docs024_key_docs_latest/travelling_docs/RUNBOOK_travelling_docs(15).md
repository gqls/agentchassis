...
**Created:** 2026-07-04
**Last updated:** 2026-07-06 (rev 16 — STAGE 3a CLOSED: gate verified in state form (ProcessingHistory persist_note→complete + COMPLETED child + 0-count); 3b.1 fetch queries live; capture-window substitute rule recorded)
...
**Position as of 2026-07-06 (later): STAGE 3a CLOSED ✓**
Stages 0–2 ✅ · 3a ✅ (wiring + routed anchorless degrade + gate VERIFIED — the
evidence trio is in the Stage 3 Status) ·
**▶ NEXT: Stage 3b — run the §3b.1 fetch queries, paste the JSON; the two
wiring migrations get drafted against it** ·
...
**3b — make it write (NOW LIVE as the next step).**

**3b.1 — fetch first (run + paste back):**
```sql
SELECT jsonb_pretty(default_config #> '{workflow,steps,call_diagnoser}')
FROM agent_definitions
WHERE type = 'diagnose-orchestrator' AND deleted_at IS NULL
ORDER BY version DESC LIMIT 1;

SELECT type, jsonb_pretty(input_contract) AS contract
FROM agent_definitions
WHERE type IN ('diagnose-agent','diagnose-orchestrator') AND deleted_at IS NULL
ORDER BY type, version DESC;
```

**3b.2 — the two edits (drafted against the paste):** thread the subject through:
