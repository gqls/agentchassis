...
**Created:** 2026-07-04
**Last updated:** 2026-07-07 (rev 26 — TASK 4 migration DRAFTED from the fetched workflows (6 note steps, both fixer branches, recreation's ×10 error_step corrections, timeouts 240/480); TASK 5 opened: gamesdesign dogfood — economy-simulator root cause found (influx uses slider INDEX; Players pinned to the Gold axis))
...
**Position as of 2026-07-07 (later): TASK 4 DRAFTED — apply next · TASK 5 (gamesdesign dogfood) opened**
...
**TASK 4 (MIGRATION DRAFTED) — the fix agents write NOTES at every fix.**
...
*Your part:* renumber + apply (expect THREE snapshot NOTICEs, five UPDATE 1,
DO, COMMIT), then run the verify at the file tail (three rows, `has_compose`/
`has_append` true, timeouts 240/480/2400, `no_step_level_error` true) — paste
it.
...
*Sequencing:* (1) apply Task 4 → (2) run the component lookup below and paste
→ (3) I script the `improve_tool` work item for the fix (the system then
writes its own first fix NOTES entry) + the tool-generator trigger for a NEW
gamesdesign tool (Task-3 proof).
```sql
...
```
(If the join names miss, `\d pages` / `\d sites` first — schema before SQL.)
