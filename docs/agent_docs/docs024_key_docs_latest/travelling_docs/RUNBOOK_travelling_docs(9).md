...
**Created:** 2026-07-04
**Last updated:** 2026-07-06 (rev 10 — Stage-3a migration APPLIED + shape verified; helper tests 6/6 PASS; diagnosis trigger + gate checks added)
...
**Position as of 2026-07-06 (later):**
Stage 0 ✅ · Stage 1 ✅ · Stage 2 ✅ (+ helper tests 6/6 PASS) ·
Stage 3a wiring **APPLIED + shape VERIFIED** ·
**▶ NEXT: run the diagnosis trigger (§3a-trigger) → gate verification** ·
3b + Stages 4–6 pending. Optional pilot PLAN: unblocked now (see after Stage 3).
...
1. Spawned pod + token: `kubectl -n ai-persona-system get pods -l
   agent_type=diagnose-agent --sort-by=.metadata.creationTimestamp | tail -3`;
...
2. Follow by correlation id: `kubectl -n ai-persona-system logs -f -l
   agent_type=diagnose-agent --tail=500 | grep $CORRELATION_ID` — markers:
