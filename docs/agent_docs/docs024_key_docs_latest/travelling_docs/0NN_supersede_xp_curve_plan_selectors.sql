...
BEGIN
    IF NOT EXISTS (
        ...
        WHERE function = 'tool-xp-curve-designer'
          ...
          AND is_active
          AND html_template ~ 'id="tableWrap".{0,300}<table'
    ) THEN
        RAISE EXCEPTION 'no <table> found within 300 chars after id="tableWrap" — do not assert #tableWrap tr; mark the check -EDIT instead';
    END IF;
END $$;
