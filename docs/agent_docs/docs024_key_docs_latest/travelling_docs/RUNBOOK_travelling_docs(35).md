...
**Last updated:** 2026-07-09 (rev 40 — resolved: `#tableWrap` is an EMPTY div filled by JS, so `#tableWrap tr` is a valid runtime selector. Guard relaxed to accept dynamic evidence. Stage-5 rule: validate a selector's ANCHOR, not its path)
...
**Position as of 2026-07-09: TASK 3 PROVEN ✓ · supersede READY (guard v3) — apply with `-f`, then the game (Task-4 proof)**
...
**Apply:**
```sql
ROLLBACK;   -- the prompt still shows clients_db=!#
```
```bash
psql "$CLIENTS_DB_URL" -f drafts/0NN_supersede_xp_curve_plan_selectors.sql
```
Expect: `NOTICE: tableWrap anchor verified (static=f, dynamic=t)`, guard 2
silent, `UPDATE 1`, `INSERT 0 1` ×2, guard 3, `COMMIT`.
