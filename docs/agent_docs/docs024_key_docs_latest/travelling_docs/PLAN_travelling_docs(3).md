...
**Created:** 2026-07-04
**Last updated:** 2026-07-04 (rev 4 — acceptance-criteria location decided; Tier 0 completeness + Tier 4 headless runner active; Phase A drafts written)
**Status:** design agreed; Phase A drafted (migration + four actions), pending live-DB verifications and wiring. This document is the spec.
...
**Phase A (drafted 2026-07-04 — pending verifications + wiring)**
1. Formats locked (above; criteria block v0 in the runner PLAN).
2. Migration drafted: `drafts/0NN_doc_plans_and_notes.sql` — **apply only after
   `drafts/verify_before_migration.sql` returns clean** (collision + live
   pipeline values).
3. Actions drafted: `write_doc_plan`, `append_doc_note`, `load_doc_context`
   (with criteria extraction), `persist_diagnosis_note` — registry.go entries in
   each file's header; workflow wiring migrations are the next step.
4. Wire: `create_tool_component` → PLAN; fix agents → note-append last step;
   diagnosis agent → persist step; `rag_index` after writes.
5. Tier-2 contract-presence check fed by `criteria_json`.
...
- Live-DB verifications (`drafts/verify_before_migration.sql`) — collision +
  `site_work_items.pipeline` value set (column is unconstrained text, default
  `'build'`; no CHECK in the dump).
