...
**Last updated:** 2026-07-09 (rev 31 — drift fix PROVEN: tool + pages + nav created, run COMPLETED at `complete`. But 0 PLAN rows: a doc step errored into its containment target. Culprit is compose_plan or write_plan by elimination — agent_error_log read queued)
...
**Position as of 2026-07-09: tool creation RESTORED ✓ · Task-3 half-proven (no PLAN written) — read `agent_error_log` for orchestration `036cd3bd…`**
...
### ▶ WHERE WE ARE (2026-07-09): drift fixed, tool created, PLAN not written

**Closed:** the schema drift. `drafts/0NN_add_component_provenance.sql` applied
(`source_agent_type varchar(100)`, `source_orchestration_id varchar(255)`,
mirrored — the orchestration id is **varchar, not uuid**, which is exactly why
the dynamic `format_type` mirror was used instead of a guess). Pre-flight on the
three unchecked tables: **0 missing columns**, both `ON CONFLICT` unique indexes
present. Re-run `f3bec131…` / orchestration `036cd3bd…` then **created the tool**:
component `f70cce61…` (`component_level=tool`, active), page
`/tools/tool-xp-curve-designer.html`, guide page, nav. `save_tool` works.

**Open:** the run finished at **`complete`** (not `complete_error`) with **0
`doc_plans` rows**. Read that shape carefully — it means `save_tool` succeeded
and one of the three doc steps **errored into its containment target**
(`config.error_step: "complete"`), which is the designed behaviour: docs never
fail tool creation. So Task 3 is **half-proven** — the wiring routes correctly,
the PLAN itself was not produced.

**By elimination the culprit is `compose_plan` or `write_plan`:** an `index_plan`
failure would have left the `doc_plans` row behind (it runs *after* the write).
Config keys were checked against the action source and match
(`plan_body_field`, `plan_source`, `subject_type`, `subject_key_field`;
`collection`, `content_field`), and `docResolveSubject` uses
`ExtractNestedFieldString`, which accepts the three-level
`input_data.spec.function` path. So: no guessing — read the error log.

**Next reads (paste all three):**
```sql
-- 1. DECISIVE — which step failed and why (by orchestration, never site+time):
SELECT occurred_at, agent_type, step_name, action, error_code, severity,
       left(error_message, 400) AS err, context
FROM agent_error_log
WHERE orchestration_id = '036cd3bd-7a0f-419d-83e5-947838b85e0e'::uuid
ORDER BY occurred_at;

-- 2. First row ever stamped with the new provenance columns:
SELECT function, created_from, source_agent_type, source_orchestration_id
FROM content_components WHERE function = 'tool-xp-curve-designer';

-- 3. Corroboration (expect 0 — index_plan is downstream of write_plan):
SELECT count(*) AS tool_docs_rows FROM knowledge_base WHERE collection = 'tool_docs';
```

**Watch item from query 2:** the earlier `agent_error_log` row showed
`agent_type = generic` on pod `agent-chassis-…` — `tool-generator` runs on the
generic chassis. If `source_agent_type` stamps as `generic` rather than
`tool-generator`, then `ExecutionContext.Sender.AgentType` is a poor provenance
source too, and the planned `source_agent` fallback for the doc actions must be
rethought (config-declared `source` is already the reliable field).

**Mitigation in pocket** (only if the error proves to be call size/timeout on
`compose_plan`, whose input carries the whole generated HTML): truncate the HTML
inside the prompt template with `{{printf "%.4000s" .generated_html.result}}` —
a workflow-only change, no Go build.

**Canonical migration is parked, not missing:**
`docs/agent_docs/docs019_documentation_audit_autonomous_build_and_operate/NNN_add_component_provenance.sql`
— written, never renumbered into the migrations path (the only `git grep` hit
for the name is the Go comment). Follow-ups: `cat` + diff against what we
applied (it may add indexes/backfill); move it into the migrations directory
with a real number.

**Bonus finding (backlog now precise):** `write_doc_plan_action.go` and
`append_doc_note_action.go` set `sourceAgent = params.Headers["agent_type"]`,
absent in that step context — hence the empty `source_agent` on our first NOTES
row. See the watch item above before choosing the fallback source.
