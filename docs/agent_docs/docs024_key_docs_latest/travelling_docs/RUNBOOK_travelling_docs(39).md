...
**Last updated:** 2026-07-16 (rev 47 — **P3 screenshots LIVE + PROVEN on v1.0.1125**; proof run 1 caught `idx_swi_dedup` counting `cancelled` as open (cancelled items silently blocked all re-raising) → **migration 157**, applied out of band; next free migration number **158**. Prior rev 46: P3 built — durable `s3://` URIs in notes, `{uri, view_url}` in item specs, presigned URLs never in doc_notes; idle-adapter Kafka ERROR spam fixed chassis-wide. Prior rev 45 line below.)
