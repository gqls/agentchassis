...
**Last updated:** 2026-07-16 (rev 46 — **P3 screenshots-on-failure BUILT, deploy-gated** on the next chassis + adapter images (+ adapter storage env): failing Tier-4 runs photograph the live page full-page, evidence lands as a durable `s3://` URI in the note's new `Evidence:` line and as `{uri, view_url}` in the item specs — presigned URLs never enter doc_notes. Idle-adapter Kafka ERROR-log spam fixed chassis-wide. Prior rev 45 line below.)
