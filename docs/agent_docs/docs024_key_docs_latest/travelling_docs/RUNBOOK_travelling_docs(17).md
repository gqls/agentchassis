...
**Created:** 2026-07-04
**Last updated:** 2026-07-06 (rev 18 — SNAPSHOT-BEFORE-UPDATE standing rule adopted (retroactive snapshot migration drafted); 3b.2 APPLIED; interim run 03bebec9 is SUBJECTLESS (regression, not the 3b.4 test); uuid-placeholder note)
...
**Position as of 2026-07-06 (evening): 3b APPLIED — snapshot, verify, positive run pending**
Stages 0–2 ✅ · 3a ✅ · 3b.1 ✅ · 3b.2 APPLIED ✓ (pre-dates the new snapshot
rule — retroactive snapshot drafted) ·
**▶ NEXT, in order: (1) apply `drafts/0NN_agent_definition_snapshots.sql`
(standing rule) · (2) run the 3b.3 verify selects · (3) 3b.4 positive run WITH
the env vars:**
`SUBJECT_TYPE=pipeline SUBJECT_KEY=build ./drafts/084_TRIGGER_diagnose_v1.sh
"smoke: subject-carrying diagnosis — first machine-written NOTES row"` ·
Interim run `03bebec9…` (16:20) is SUBJECTLESS — will SKIP (free post-3b
regression of the gate, NOT the 3b.4 test) ·
Stages 4–6 pending · Pilot PLAN: unblocked.
...
**Snapshot before updating (STANDING RULE 2026-07-06)** — prepend to every
agent-updating migration:
```sql
INSERT INTO agent_definition_snapshots
SELECT ad.*, now(), '<migration filename>: pre-update'
FROM agent_definitions ad
WHERE ad.type = '<type>' AND ad.deleted_at IS NULL;
```
Restore = UPDATE the live row from the snapshot (join on `id` +
`snapshot_taken_at`). Table + retroactive snapshot of the two agents this arc
updated: `drafts/0NN_agent_definition_snapshots.sql` (includes the inspection
query for the existing `is_snapshot` convention — converge on it if live).
...
Applied BEFORE the snapshot standing rule — apply the retroactive snapshot
first thing: `drafts/0NN_agent_definition_snapshots.sql`. File:
...
**3b.3 — verify shape** *(NOT yet run — do this before 3b.4)*:
```sql
...
All green → **3b CLOSED**: the travelling-docs pipeline is live end-to-end for
diagnoses; update the §0 position line.
...
- **Snapshot before updating `agent_definitions`** (standing rule 2026-07-06 —
  §0-REF snippet). The four pre-rule updates in this arc have a retroactive
  snapshot + per-migration rollback comments.
