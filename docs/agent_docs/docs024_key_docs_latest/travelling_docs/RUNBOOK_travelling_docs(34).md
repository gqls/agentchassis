...
**Last updated:** 2026-07-09 (rev 39 — guard 1 REFUSED the supersede: no `<table>` near `#tableWrap`. Design consequence: criteria are RUNTIME assertions; static validation can only confirm, never refute, dynamically-rendered selectors)
...
**Position as of 2026-07-09: TASK 3 PROVEN ✓ · supersede BLOCKED by its own guard (good) — inspect the HTML, then finalise · then the game (Task-4 proof)**
...
id="tableWrap"`. Correct behaviour: my replacement selector `#tableWrap tr` was
itself unverified, and the guard stopped me writing a second unverifiable
assertion while repairing the first. Nothing was written.

**Design consequence (this changes Stage 5):** acceptance criteria are **runtime**
assertions — the Tier-4 runner evaluates them against the *rendered DOM*, not
against `html_template`. If this tool builds its rows in JS (likely: the `<tbody>`
we matched may live inside a template string in the `<script>` block), then
`#tableWrap tr` can be valid at runtime and absent from the static HTML. So:
- static validation can **confirm** a selector, never **refute** one;
- selectors it cannot find are **runtime-only**, to be marked (e.g. `-EDIT`, or a
  `"runtime": true` flag) and left for Tier-4 — never silently deleted;
- a selector that appears NOWHERE (statically or in the JS) is the real error —
  that is what `#xpTableBody` and `#statsStrip` were.

**Inspect before finalising (paste both):**
```sql
SELECT length(html_template) AS len,
       strpos(html_template, 'id="tableWrap"')     AS p_wrap,
       strpos(html_template, '<table')             AS p_table,
       strpos(html_template, '<tbody')             AS p_tbody,
       strpos(html_template, '<script')            AS p_script,
       html_template LIKE '%tableWrap%innerHTML%'  AS wrap_filled_by_js,
       html_template LIKE '%<tr%'                  AS has_tr_markup
FROM content_components WHERE function = 'tool-xp-curve-designer' AND is_active;

SELECT substr(html_template, strpos(html_template, 'id="tableWrap"'), 300) AS after_wrap
FROM content_components WHERE function = 'tool-xp-curve-designer' AND is_active;
```
Reading: `p_table > p_script` ⇒ the table is JS-built ⇒ keep the check, relax
guard 1 to accept "JS fills `#tableWrap`" as evidence. `p_table = 0` and no `<tr`
⇒ rows are not a table at all ⇒ retarget the check (or mark it `-EDIT`).

**Stage 5 (Tier-2 static) requirement, refined:** at PLAN-write time, check each
criteria selector against the component's `html_template` **and its inline JS**.
Found statically ⇒ confirmed. Found only in the script (or not at all, but the
element is created dynamically) ⇒ mark **runtime-only**, leave to Tier-4. Found
nowhere at all ⇒ drop the check or mark it `-EDIT`. Same logic belongs in
`tool-auditor`. The composer's two invented ids fall in the last bucket; today's
`#tableWrap tr` may fall in the middle one.
