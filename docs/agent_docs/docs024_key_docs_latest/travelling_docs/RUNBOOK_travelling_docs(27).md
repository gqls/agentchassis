...
**Last updated:** 2026-07-08 (rev 29 — BLOCKER DIAGNOSED from agent_error_log: generate_tool_html SUCCEEDED; save_tool/create_tool_component failed on schema drift (missing content_components.source_agent_type). Migration drafted; re-run 085 after applying)
...
**Position as of 2026-07-08: blocker ROOT-CAUSED (schema drift) — apply `drafts/0NN_add_component_provenance.sql`, then re-run 085**
...
### ▶ BLOCKER ROOT-CAUSED (2026-07-08) — schema drift in `create_tool_component`

**It was NOT the LLM step.** `agent_error_log` (orchestration `9f93a988…`,
16:14:44) names the real failure:
> `step save_tool failed: … create_tool_component: … column "source_agent_type" of relation "content_components" does not exist (SQLSTATE 42703)`

`generate_tool_html` **succeeded**; `save_tool` failed one step later. The
120s polling had merely sampled `current_step` during generation — the last
observed step is where the run WAS, never where it FAILED (016b v6).

**Cause:** the binary shipped ahead of its migration. `create_tool_component_action.go`
says so above its own INSERT: *"source_* = creation provenance
(NNN_add_component_provenance.sql), mirroring knowledge_base's pair … apply
that migration before this binary deploys."* Production `content_components`
has neither `source_agent_type` nor `source_orchestration_id`;
`knowledge_base` has both.
**Latent, not new:** last `generated` tool component = **2026-05-16**, so no
tool has been created in ~2 months; `component-creator` inserts a different
column set and kept working. Our proof run was simply the first caller.
**0 PLAN rows remains correct** — `complete_error` bypasses `compose_plan`;
Task 3 is unproven, not broken.

**Fix — 3 steps:**
1. *Reuse first* — is the canonical migration in the repo?
```bash
find ~/projects/agentchassis -name '*provenance*'
git -C ~/projects/agentchassis grep -l add_component_provenance
```
   If found, apply THAT file (diff against the draft). If the named file was
   never written, apply the draft and commit it so the code reference resolves.
2. Apply `drafts/0NN_add_component_provenance.sql` — mirrors `knowledge_base`'s
   column types dynamically (no guessing), additive + nullable + idempotent, no
   `snapshot_agent()` (data table, not `agent_definitions`). Verify:
```sql
SELECT column_name, data_type, is_nullable FROM information_schema.columns
WHERE table_name='content_components' AND column_name LIKE 'source_%' ORDER BY column_name;
```
3. Re-run the proof — the function name is still free (the failed run inserted
   nothing; the component INSERT was its first statement):
```bash
./drafts/085_TRIGGER_toolgen_gamesdesign_v1.sh
```
   Expect component + page + nav, then `compose_plan → write_plan → index_plan`
   → a `doc_plans` row with `source='tool-generator'` (**Task-3 proof**), and
   the component stamped `source_agent_type='tool-generator'`.

**No second drift:** every other INSERT in that action (`pages`,
`page_components`, `site_work_items`) was checked column-by-column against the
bundle's production schema and matches.
...
which is absent in that step context — hence the empty `source_agent` on our
first NOTES row. `create_tool_component` uses
`params.ExecutionContext.Sender.AgentType` and it populates. Fix in the next
chassis build: fall back to the execution context (same for `rag_actions.go`).
