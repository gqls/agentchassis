...
**Created:** 2026-07-04
**Last updated:** 2026-07-07 (rev 23 — PILOT PLAN SEEDED ✓ (first tool PLAN live; Stage-5 precondition met); duplicates explained by the unique index's real predicate; plain-terms mini-glossary added; Task 3 reworded — awaiting an explicit yes)
...
**Position as of 2026-07-07 (12:32): STAGE 3 CLOSED ✓ · PILOT PLAN SEEDED ✓ (Stage-5 precondition met)**
...
**TASK 3 (AWAITING YOUR YES) — automate PLAN-writing at tool creation.**
*In plain terms:* PLANs are currently written by hand (you just did the first
one). This task changes the `tool-generator` agent so EVERY future tool it
creates also writes its own PLAN row automatically — same shape as your pilot,
machine-authored at creation.
*Your part if you approve (3 small steps, ≈10 min):*
1. `SELECT snapshot_agent('tool-generator', '<migration file>: pre-update');`
2. apply the one SQL file I produce;
3. (optional proof) create one test tool and watch a `doc_plans` row appear.
*What the file will contain:* a `compose_plan` step (an LLM drafts the PLAN
body, criteria included, from the tool spec + the generated HTML) → a
`write_doc_plan` step (`config.error_step → complete` — a docs failure can
never fail tool creation) → and, under the correct-while-touching rule (see
the mini-glossary), the three currently-inert step-level `error_step`s in this
workflow move into `config`.
*To proceed:* reply **"yes, draft the tool-generator migration"** (this is
what the earlier shorthand "go B" meant).
