...
**Created:** 2026-07-04
**Last updated:** 2026-07-07 (rev 27 — TASK 4 APPLIED (3 snapshots, guard passed = shape verified); lookup surprise: the game body is NOT a page-linked component — discovery queries queued; 085 toolgen trigger ready (Task-3 proof on gamesdesign))
...
**Position as of 2026-07-07 (evening): TASK 4 APPLIED ✓ · TASK 5 in discovery · 085 proof trigger READY**
...
*Lookup result (2026-07-07):* the game PAGE exists
(`game-economy-simulator`, site `e33263f4…`) but the only page-linked
components are a shared `hero` + a guide text block — **the game body is NOT a
page-linked content_component**, so `tool-improver.load_tool` (which requires
a `component_id`) cannot reach it yet. Where the 32KB body lives needs
DISCOVERY, not assumption. Run these and paste:
```sql
-- Q1: everything linked to the game page, no name filter
SELECT cc.id::text AS component_id, cc.function, cc.component_level, cc.is_active,
       cc.forked_from IS NOT NULL AS is_fork, length(cc.html_template) AS html_len
FROM content_components cc
JOIN page_components pc ON pc.component_id = cc.id
JOIN pages p ON pc.page_id = p.id
WHERE p.site_id = 'e33263f4-74f8-494f-b191-546845dbbddf'::uuid
  AND p.name = 'game-economy-simulator'
ORDER BY cc.function;

-- Q2: any economy-ish component sitewide, INCLUDING unlinked rows
SELECT id::text AS component_id, function, component_level, is_active,
       forked_from IS NOT NULL AS is_fork, length(html_template) AS html_len
FROM content_components
WHERE site_id = 'e33263f4-74f8-494f-b191-546845dbbddf'::uuid
  AND (function ILIKE '%econom%' OR display_name ILIKE '%econom%'
       OR html_template ILIKE '%Economy Stabilization Sim%')
ORDER BY function;
```
If BOTH come back empty: `\dt *section*` (the recreation path saves via
`save_page_sections` — the body may live as sections, not a component), and
paste that. The `improve_tool` item gets scripted the moment the body has an
address.
```sql
SELECT cc.id::text AS component_id, cc.function, cc.is_active,
       p.url, p.name AS page_name, p.site_id::text AS site_id
FROM content_components cc
JOIN page_components pc ON pc.component_id = cc.id
JOIN pages p ON pc.page_id = p.id
JOIN sites s ON p.site_id = s.id
WHERE s.domain = 'gamesdesign.co.uk'
  AND (p.url ILIKE '%economy%' OR cc.function ILIKE '%econom%');
```
**Task-3 PROOF trigger READY:** `drafts/085_TRIGGER_toolgen_gamesdesign_v1.sh`
— creates ONE new tool (`tool-xp-curve-designer` by default, overridable via
env) on gamesdesign through the full pipeline. REAL side effects: component +
page + nav on the live site, plus the Task-3 `doc_plans` row. Run it whenever
ready:
```bash
./drafts/085_TRIGGER_toolgen_gamesdesign_v1.sh
```
Paste back: the status by correlation, the `doc_plans` proof select (2), and
the PLAN body (4) for the one-time composer review.
