...
**Created:** 2026-07-04
**Last updated:** 2026-07-04 (rev 6 — Stage 2 complete on production; Stage-3 step fragment prepared; pilot PLAN path added; newcomer intro added)
...
**3.1 — Fetch the current definitions (run and paste back):**
```sql
...
**3.3 — Apply the drafted migration**, then trigger ONE diagnosis run with an
explicit subject in input (`subject_type`: `tool`|`pipeline`, `subject_key`,
e.g. `pipeline`/`build` or a known tool function).
...
- **tool-generator / `create_tool_component` path → PLAN at creation.** Needs a
  ...
  `doc_plan_body`): compose from material in hand (spec slice, delivery
  mechanism, deliberate decisions, initial criteria block). Then `rag_index`
  (`collection='tool_docs'`).
- **Fix agents (`tool-improver`, `component-template-fixer`,
  `update_component_html` path) → `append_doc_note` as the LAST step.**
