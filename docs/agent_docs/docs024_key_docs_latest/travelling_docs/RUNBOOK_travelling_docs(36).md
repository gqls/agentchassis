...
**Last updated:** 2026-07-09 (rev 42 — supersede VERIFIED. Recreation fetch: `spec` is undeclared in its input_contract; `site_plan_sections` is site-plan structure, not HTML; recreation writes no component → its NOTES subject should be pipeline/build, a Task-4 wiring correction)
...
**Position as of 2026-07-09: TASK 3 PROVEN ✓ · PLAN corrected ✓ (v1 retired, v2 current, 1 correction note) · TASK 5: two fixes needed before the trigger**
...
**TASK 5 (ACTIVE) — fix the economy simulator through the system (Task-4 proof).**
...
*Three reads to finish the picture:*
```sql
SELECT column_name, data_type FROM information_schema.columns
WHERE table_name = 'pages' AND column_name IN ('sections','status','build_status');

SELECT name, status, build_status,
       pg_typeof(sections) AS sections_type,
       length(sections::text) AS sections_bytes
FROM pages
WHERE site_id = 'e33263f4-74f8-494f-b191-546845dbbddf'::uuid
  AND name IN ('game-economy-simulator','tool-xp-curve-designer');

\dt *research*
```
```bash
grep -rn "load_existing_content" ~/projects/agentchassis/platform/orchestration/actions/*.go | head -3
grep -rniE '"(mode|rawHtml|raw_html|markdown)"' ~/projects/agentchassis/platform/orchestration/actions/load_existing_content*.go | head -20
```
`mode`'s accepted values decide whether the recreation can read the original game
HTML from the adoption crawl. If it cannot, `load_existing_content` error-routes
to `load_related_context` (contained, by design) and the LLM would rebuild from
the spec alone — acceptable, but it would lose the game's existing behaviour, so
prefer feeding it the real content.
