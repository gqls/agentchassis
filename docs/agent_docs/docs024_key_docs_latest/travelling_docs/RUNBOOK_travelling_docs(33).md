...
**Last updated:** 2026-07-09 (rev 38 — aborted transactions are sticky: `ROLLBACK;` then run the file with `-f`/`\i`, not by pasting. Supersede file hardened with a leading ROLLBACK)
...
```sql
ROLLBACK;   -- if the prompt shows clients_db=!#
```
```bash
psql "$CLIENTS_DB_URL" -f drafts/0NN_supersede_xp_curve_plan_selectors.sql
```
The file now opens with its own defensive `ROLLBACK;` too. Expect: 2 guards
