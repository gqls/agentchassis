...
*v8 (2026-07-09): EXECUTING_STEP forever usually means the worker died (OOMKilled), not that the step is slow — corrected from an initial "stall" reading after the pod's Last State and healthy embedder probes came in.*
...
### `EXECUTING_STEP` forever usually means the WORKER DIED, not that the step is slow

**Symptom.** A step sits in `EXECUTING_STEP` far longer than the workflow's
`timeout_seconds`; `agent_error_log` is silent; `config.error_step` never fires.
It looks exactly like a hang.

**Mechanism.** `orchestration_states` is written BY the worker. If the worker
process dies — OOMKill, node eviction, panic — nothing writes the failure, and
the row is frozen at its last state. `since_s` then measures **time since the
pod died**, not time spent in the step. `config.error_step` routes *errors*
raised by an action; a killed process raises nothing. Worked example: a step
began at 13:08:01, the pod's `Last State` showed `Terminated / OOMKilled /
Exit Code 137` at 13:08:24, and the row still read `EXECUTING_STEP` 44 minutes
later.

**Triage order (before blaming the action):**
```bash
kubectl -n <ns> get pods | grep <agent>                       # RESTARTS column
kubectl -n <ns> describe pod <pod> | grep -A6 'Last State'    # OOMKilled? Exit 137?
kubectl -n <ns> logs <pod> --previous --tail=3000 | grep <orchestration_id>
```
`RESTARTS 1 (Nm ago)` next to a stuck run is the tell. Post-restart logs will not
contain the run — use `--previous`. Probe suspected-stalled dependencies with a
BOUND (`curl -m 5`) before assuming they hang; a healthy dependency rules the
stall hypothesis out.

**Related but distinct — genuine stalls.** An action making a network call with
no `context.WithTimeout` / `http.Client{Timeout}` can hang indefinitely, and
"non-fatal on error" handling does not cover it (a stall raises no error). Both
faults deserve fixing; distinguish them by the pod's `Last State`, not by the
orchestration row.

**Memory suspects when a chassis OOMs.** Watch for state that grows per step:
`collected_data` accumulating large blobs (site specs, LLM outputs) and —
observed here — `__raw_message__` nested inside `__raw_message__`; plus
full-params logging at Info on every action (`DEBUGaa: params sent to action
handler` dumps the entire `CollectedData`). Measure before theorising:
```sql
SELECT pg_size_pretty(length(collected_data::text)::bigint) AS collected_size,
       (length(collected_data::text) - length(replace(collected_data::text,'__raw_message__','')))
         / length('__raw_message__') AS raw_message_copies
FROM orchestration_states WHERE correlation_id = '<uuid>'::uuid;
```

Category tags: `oomkilled`, `orphaned-orchestration`, `executing-step-forever`,
`missing-deadline`, `collected-data-growth`, `previous-logs`.

### Postgres regex: bounded repetition is capped at 255

**Symptom.** `ERROR: invalid regular expression: invalid repetition count(s)` on
a pattern like `html_template ~ 'id="x".{0,300}<table'`.

**Mechanism.** Postgres ARE enforces `RE_DUP_MAX = 255` for `{m,n}` bounds.
Anything above 255 is rejected at parse time — inside a `DO` block this aborts
the transaction (`clients_db=!#`), so **`ROLLBACK;` before retrying**.

**Fix.** Either drop to `{0,255}`, or — better for containment checks — say what
you mean with `strpos`/`substr`:
```sql
p := strpos(h, 'id="tableWrap"');
IF p = 0 OR strpos(substr(h, p, 400), '<table') = 0 THEN RAISE EXCEPTION '...'; END IF;
```
No engine limits, and the intent reads plainly. Silver lining: the failure fired
inside `BEGIN`, so nothing was written — guards that abort early are working as
designed.

**The aborted transaction is sticky.** After ANY error inside a transaction,
psql shows `clients_db=!#` and answers every further statement with
`current transaction is aborted, commands ignored until end of transaction
block` — including the `BEGIN` of your next attempt, so a re-run appears to fail
for a different reason. `ROLLBACK;` is the only way out. Two habits:
- open migration files with a defensive `ROLLBACK;` (a clean session merely warns
  "no transaction in progress");
- run files with `psql -f <file>` or `\i <file>` rather than pasting — pasting
  mangles comment lines and dollar-quoted bodies, and the mangled text is what
  ends up in the paste you later try to debug.

Category tags: `postgres-regex`, `re-dup-max`, `aborted-transaction`,
`sticky-abort`, `psql-paste-mangling`, `guard-design`.
