...
**Created:** 2026-07-04
**Last updated:** 2026-07-07 (rev 25 — TASK 3 APPLIED + SHAPE-VERIFIED (snapshot 1bca62f6; UPDATE ×3; verify quadruple green) — tools now write their PLAN at birth, proof pending; TASK 4 fetch opened (fix-agents NOTES hook))
...
**Position as of 2026-07-07 (later): STAGE 3 ✓ · PILOT ✓ · TASK 3 APPLIED + VERIFIED ✓ — proof pending · TASK 4 fetch next**
...
**TASK 4 (NEXT) — the fix agents write NOTES at every fix (~2 min fetch now).**
*In plain terms:* creation now writes the PLAN; this task makes every FIX
append a NOTES entry (symptom → root cause → fix → verification + tags) as the
tail of the three fix agents' workflows. After it, tools get a PLAN at birth
and a NOTES entry at every change — the loop this design was built for.
*Your part now — run and paste (one query, three workflows):*
```sql
SELECT type, jsonb_pretty(default_config->'workflow') AS workflow
FROM agent_definitions
WHERE type IN ('tool-improver','component-template-fixer','tool-recreation-handler')
  AND deleted_at IS NULL
ORDER BY type, version DESC;
```
*Then I draft:* ONE snapshot-prefixed migration (three `snapshot_agent` calls,
three targeted updates) adding a `compose_note` + `append_doc_note` pair as
each workflow's tail (`config.error_step` → each terminal step — notes never
fail a fix), with the tag strategy and body inputs decided against each
workflow's real collected data, the loop-substep corollary checked per
workflow, and correct-while-touching applied to `tool-recreation-handler`'s
known step-LEVEL `error_step`s (`tool-auditor`'s are Stage 5's concern, not
this migration).
