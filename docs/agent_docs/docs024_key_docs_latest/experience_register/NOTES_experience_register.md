...
## 2026-07-29b — four rounds in, and a defect the council found in the VALIDATOR

Rounds 3 and 4 both came back REVISE, and both times the [high] objections were the council
catching something real rather than disagreeing with a judgement. Worth separating what moved from
what will not move by editing.

**What the council found that I had got wrong.**
- My reason for deferring `feed_loads` described `strings.Contains(html, ch.Path)` as though it
  were inherent to `asset_loads`. It is a choice, not a law. **The true reason is stronger:
  63 `doc_plans` fences already use `asset_loads`, so making it fetch changes what the type MEANS
  for all of them — RFC-scope under the ruling made hours earlier.** A better reason, found by
  being corrected.
- `inert_row_has_no_activation_handler` could be partly proxied by the **existing** `interaction`
  type — click the inert row, assert the container survives. Real reuse I had not considered.
  Added, bounded to what it actually catches (navigation, not an in-place state change).
- The `contract` clause and the `description` still restated `no-inert-control` in prose, in the
  two fields the round-2 sweep of `states` did not reach.

**`[FINDING — platform, not this entry]` The validator can produce a self-contradicting deferral
record.** `deferral_honesty [low]` flagged a `deferred_checks` row whose reason says *"every part
of it is executable today — remove the marker if the platform has caught up"* with `"field": ""`.
That text is generated by `experience_criteria.go`, not authored by me: it is the branch for a
check marked `_unsupported` whose type IS executable. The branch exists for a good reason (a stray
marker must not silently uncount a real check) but it has **no vocabulary for the case that
actually occurs**: the TYPE runs and this INSTANCE would assert nothing — `selector_count` with no
threshold, because no checker reads one. That is IMPLEMENTED-vs-SATISFIABLE again, this time
inside the validator's own reporting. Not fixed here; it is platform code and wants its own gated
change.

**What will NOT move by editing, and should stop being re-litigated each round.**
`deferral_honesty` has now objected three rounds running that the central clause — the JS-bound
handler, the "looks inert but isn't" case — is 100% on the deferred side. **It is, and it will
stay there until a browser runs the register's checks**, because this component renders its rows
CLIENT-SIDE: every clause about a live row is Tier 4 by physics, not by choice. The same seat also
notes the five tier-4 deferrals share one boilerplate reason and are "one missing capability, not
five" — which is exactly right and is the same fact stated from the other side.

> **So the honest conclusion after four rounds: this entry cannot be approved until bind + verify
> run at Tier 4.** Further revisions can only improve its prose. Anyone picking this up should
> wire the browser tier rather than open round five — and should treat a fifth round that changes
> only wording as evidence they are polishing, not converging.
