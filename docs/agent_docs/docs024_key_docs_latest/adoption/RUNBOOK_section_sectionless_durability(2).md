...
## 4c. Fix A — guard `complete_work_item` (prerequisite for S2)

S2 only works if the dispatch loop doesn't overwrite the flag S2 sets. The
build-dispatch-loop's `mark_complete` step calls `complete_work_item` on every
successful handler saga, and `complete_error` is a success-labelled
`complete_workflow` — so an unguarded `complete_work_item` re-stamps a
handler-set `needs_human_review` back to `complete`. (Confirmed mechanism: the
2026-06-06 skinner-box retry hit `complete_error` on a sectionless page and ended
`status='complete'` with no components.)

Code change (Go, `load_work_item_actions.go` → `CompleteWorkItemAction`): the
`UPDATE` gains `AND status NOT IN ('needs_human_review','failed','unresolved','rejected','wont_fix','verified','blocked')`,
and returns `completed = rows>0`, logging when skipped. No-op on the normal
`claimed → complete` path; preserves deliberate flags. File:
`load_work_item_actions.go` (outputs) → place at `./platform/orchestration/actions/`.
Brace/paren/bracket balance verified.

**Deploy order:** Fix A must ship with (or before) S2 — S2's flag is ineffective
without it. Both are part of the same chassis image roll (§2).

**Verify:** trigger a build on a sectionless page with no same-role sibling
(role with one page). Expect the item to end `needs_human_review`, not `complete`.
With Fix A absent it would end `complete` (the clobber).

## Rollback
- Code: redeploy the previous chassis image (revert all three files: the
  `load_page_sections_from_spec`, `check_sectionless_pages`, and
  `load_work_item_actions` changes). All additive; no data migration.
