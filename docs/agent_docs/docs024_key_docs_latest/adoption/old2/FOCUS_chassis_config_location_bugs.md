# FOCUS — Chassis Config Location and Resolution Bugs

Date: 2026-05-18 (revised — added error_step finding)
Status: max_tokens workaround applied for site-adoption-agent; error_step fix deployed via Path A (struct field + reader fallback); temperature TODO outstanding

Originally about LLM config resolution. Expanded to cover a wider pattern: **the chassis reads several configurable fields from locations that don't match where workflow authors put them in agent definitions**. Three instances documented here so far; pattern likely extends further.

The three instances:

| Field | Workflow author puts it | Chassis reads from | Result |
|---|---|---|---|
| `max_tokens` | sibling of `ai_service` OR inside `ai_service` | top-level `ai_service` only (with shadowing) | Step-level dead if agent has top-level ai_service |
| `temperature` | inside `ai_service` OR step config | top of `default_config` only | All non-top-level placements dead |
| `error_step` | step level (parallel to `next_step`) | inside `step.Config` | All step-level placements dead (pre-fix) |

Common shape: author intuition vs chassis-reader location are misaligned. The chassis lacks a coherent convention for which control fields live at the step level vs inside config. `next_step` is at step level; `error_step` (pre-fix) was inside config. `max_tokens` sometimes inside ai_service, sometimes alongside it. Worth flagging for any future config-shape standardisation work.
...
# Instance 3 — `error_step` routing was silently dead

Discovered 2026-05-18 while debugging the failed gamesdesign.co.uk re-adoption.

## What was wrong

Agent definitions place `error_step` at the **step level**, sibling of `config`, parallel to `next_step`:

```json
"fetch_primary_css": {
    "action": "firecrawl_scrape",
    "config": { "url_field": "...", "scrape_config": {...} },
    "next_step": "enrich_fingerprint",
    "error_step": "analyze_site"      ← step level
}
```

The chassis (`coordinator.go` lines 2943 and 3151) read it from `step.Config["error_step"]` — **inside** `config`. Because the `Step` struct had no `ErrorStep` field, JSON unmarshalling silently dropped the value during workflow plan loading. Every step-level `error_step` was dead config.

## Symptom

When a step timed out or errored, the workflow failed entirely instead of routing to its configured fallback. For site-adoption-agent, this turned a transient firecrawl CSS timeout (`fetch_primary_css`) into a fatal adoption failure — the configured `error_step: analyze_site` was never read, so the whole orchestration died with the parent stuck at `call_adopter` instead of falling through to analyze_site with a partial design fingerprint.

## Blast radius (audited 2026-05-18)

62 dead `error_step` settings across 18 agents:

| agent | dead error_step count |
|---|---|
| image-build-handler | 12 |
| tool-recreation-handler | 10 |
| site-adoption-agent | 7 |
| improvement-loop | 6 |
| content-gap-planner | 5 |
| css-patch-agent | 4 |
| tool-generator | 3 |
| internal-linker, component-creator, tool-auditor, design-audit-agent | 2 each |
| webdesign-agent, site-review-agent, blog-content-planner, tool-deployer, spec-updater, page-content-writer, tool-suggester | 1 each |

Every one of these was a graceful-degradation path the agent author had intentionally configured but the chassis never honored.

## Decision: Path A (code-only fix)

Two paths considered:

- **Path A** — add `ErrorStep` field to the `Step` struct, patch `coordinator.go` to read `step.ErrorStep` first with fallback to `step.Config["error_step"]`. Single deploy fixes all 62 dead paths.
- **Path B** — data migration moving `error_step` into config for all 18 agents. More targeted but requires per-agent SQL.

Path A chosen because:
- Smaller change set (one struct field + two function patches)
- Recovers all 62 dead paths in one deploy with no per-agent migration
- Step-level placement matches author intuition (parallel to `next_step`) and is the natural reading of the workflow JSON
- Config-level placement still works via fallback (preserves the chassis docstring example for backward compatibility)

## Code changes (applied 2026-05-18)

### 1. `pkg/models/contracts.go` — `Step` struct

Added `ErrorStep` field next to `NextStep`:
...
type Step struct {
    ...
    ErrorStep       string                 `json:"error_step,omitempty"`  // NEW
...
### 2. `coordinator.go` `routeToErrorStepOrFail` (line ~3151)
...
### 3. `coordinator.go` `handleUnrecoverableError` (line ~2941)

Same dual-lookup pattern. This function does extra cleanup (deletes from AwaitedRequests, marks await complete) so the lookup is inlined rather than delegated:
...
The wider audit query continues to return the 18 agents — the data hasn't moved. But after deploy, the chassis reads `step.ErrorStep` so those values now take effect:

```sql
SELECT type, COUNT(*) AS step_level_error_steps_now_live
FROM agent_definitions,
     LATERAL jsonb_each(default_config->'workflow'->'steps') AS step_key
WHERE deleted_at IS NULL AND is_active = true
  AND step_key.value ? 'error_step'
  AND NOT (step_key.value->'config' ? 'error_step')
GROUP BY type
ORDER BY step_level_error_steps_now_live DESC;
```

Counts unchanged — meaning matters changes. To verify the chassis actually routes through these paths after deploy, trigger a step failure on any agent and confirm orchestration ends up at the configured `error_step` rather than `status = FAILED`.

## site-adoption-agent surgical patch (2026-05-18, becomes redundant)

Earlier in this debugging cycle, `fetch_primary_css.error_step` was moved into config via SQL UPDATE to unblock the adoption before the code patch was ready. With Path A deployed, that SQL move becomes redundant — the step-level placement would have worked too. No need to revert; the field is read from config first only because of the writer convention difference, but both locations now work.

## Future cleanup (optional, not blocking)

A future migration could move all 62 step-level `error_step` settings into config for symmetry with the chassis docstring example. Not required — Path A's fallback chain handles both. The decision can wait until there's a reason to touch the agent definitions anyway (other config changes, etc.).

---

## What changed from the previous version of this doc

**2026-05-18 (third revision):**
- Title and scope expanded: was "LLM Config Resolution Order Has Shadowing Bugs", now "Chassis Config Location and Resolution Bugs". The doc now covers three instances of the same family of bugs: max_tokens (shadowing), temperature (single read path), error_step (struct field missing).
- Added Instance 3 — error_step routing was silently dead across 18 agents (62 paths)
- Documented Path A code fix: struct field added to `Step`, dual-lookup in `routeToErrorStepOrFail` and `handleUnrecoverableError`
- Noted the site-adoption-agent surgical SQL patch becomes redundant after Path A deploys
...
- `pkg/models/contracts.go` `Step` struct — `ErrorStep` field added 2026-05-18
- `coordinator.go` `routeToErrorStepOrFail` (line ~3151), `handleUnrecoverableError` (line ~2941) — both patched with step-level + config fallback

**Documents:**
- Original diagnosis: 2026-05-17 → 2026-05-18
- Symptom debug guide entry: section 6.6 of `016_debugging_guide_v2_13_.md`
...
**SQL artefacts:**
- site-adoption-agent surgical move of `fetch_primary_css.error_step` into config (2026-05-18, becomes redundant after Path A deploy)
- Top-level `ai_service.max_tokens=16000` patch (2026-05-18)
