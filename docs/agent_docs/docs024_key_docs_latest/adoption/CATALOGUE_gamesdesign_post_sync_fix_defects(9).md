...
**Group 1 — link & list resolution [ours; highest leverage; ROOT CAUSE CONFIRMED 2026-06-04 — two causes].** Reads confirm: game-list is the legacy numbered-flat anti-pattern (LLM-fabricated, not Tier D); guide-list is Tier D but queries `pages_where_type:guide` while guides are `page_type=blog_post` (→ empty); tool-list works (Tier D + tools=`tool`). `game` page_type now exists on this site (classifier updated since FOCUS_component_schema_patterns.md), so game-list will resolve once Tier D. cta_urls are empty because their `site_specs.*` paths are unpopulated with no fallback. Fix: (1) game-list→Tier-D `query.pages_where_type:game`; (2) guide-list vocabulary (query `blog_post` band-aid vs `guide` first-class type structural); (3) cta_url fallbacks; (4) hero CTAs separate. Architecture (queryresolve + Step-3 anti-fabrication) already built — per-component schema/template work.  **PROGRESS 2026-06-04:** game-list Tier-D rewrite delivered (`migration_game_list_tier_d.sql`, validated) — items-array `query.pages_where_type:game`, tool-list parity, cta_url fallback; data change, live on commit, then rebuild pages with `sections @> ["game-list"]`. Guide fix = structural route chosen (classifier `guide` type + re-type + `/blog/`↔`/guides/`), pending classifier. cta_url fallbacks for tool/guide deferred pending fallback-mechanism check. **Original framing below:** Re-confirms Families B, C, D on the post-A1 build. Diagnostic contrast: the **tool-list resolves real pages** (`href="/tools/<slug>/index.html"`) while the hero CTAs and the game-list emit static placeholders. Symptoms: hero "Launch a Tool"→`/contact.html`, "Browse All Tools"→`/services.html` (nonexistent) on every hub (B); the tool-list's own "Browse All Tools" `href=""` (B); game-list `Play Now` `href=""`, `img src=""`, and a fabricated/duplicated set — Jelly Invaders appears twice (C); guides hub has hero only, no guide-list (D). Fix hypothesis: make game-list/guide-list use the tool-list's real-page resolution, and rewrite hero CTAs to real targets. → next, after Group 2.
...
*Fix (located 2026-06-04 — the auto-complete is a SQL `pre_query`, not Go):* **(A)** the completion logic lives in the **`claimed-item-timeout` scheduled task's `pre_query`**, and `migration_claimed_item_timeout_evidence_check.sql` (already authored) is Option A — it gates completion on *page-specific* deploy evidence (`deployed_at > claimed_at`), replacing the old loose site-level `updated_at` check that false-completed the homepage. Its `reset` CTE is also the stale-claim reset (= Lever C). The homepage was false-completed under the OLD evidence on 06-03, so **the migration was not applied then — verify and apply it.** Hardening (DONE — `migration_claimed_item_timeout_evidence_v2.sql`): the `needs_content_page` branch now credits by `page_components` existence + `updated_at > claimed_at` recency (the artifact it produces), not `deployed_at` (a later item's output) — which also decouples it from the untrustworthy `deployed` flag. **(B) DONE — `v3_site_actions.go` patch delivered (`v3_site_actions_optionB.patch`):** `UpdatePageStatusAction` now calls `pageHasComponents(pageID)` before marking `deployed`; if 0 rendered components it refuses and flips to `needs_rebuild` (clearing the stamp) so the reconciler rebuilds rather than skips. Fail-open on a check error. The A1 stamp on the has-components path is untouched. **(A) APPLIED 2026-06-04** — `migration_claimed_item_timeout_evidence_v2.sql` is live (verified: task `enabled=t`, 120s, new pre_query present). *Immediate unstick (DONE 2026-06-04):* `UPDATE pages SET build_status='needs_rebuild', built_from_plan_version=NULL WHERE … name='index'` → UPDATE 1 (recurs if a response is lost again until A is applied).
