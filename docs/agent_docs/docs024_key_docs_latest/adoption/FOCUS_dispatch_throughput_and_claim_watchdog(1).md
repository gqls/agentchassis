...
4. **git-adapter cross-site race (guardrail for A):** add retry-on-non-fast-forward in `updateRef`. **Possible real instance observed (2026-06-03):** the missing root `index.html` on gamesdesign — `deployed`+`stamped` in DB, no file in repo — fits a silent non-fast-forward loss exactly; under diagnosis as catalogue A4 / Group 2. If confirmed, this race is not just latent but actively dropping deploys, raising its priority.
