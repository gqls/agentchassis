...
> **REVISED 2026-06-04 — DO NOT BUILD THIS. The reset already exists.** The `claimed-item-timeout` scheduled task's `pre_query` already contains the stale-claim reset: its `reset` CTE handles claims past 40 min with `attempt_count+1`, `status = CASE WHEN attempt_count+1 >= max_attempts THEN 'failed' ELSE 'triaged'`, clearing `claimed_by`/`claimed_at` — exactly the `reset_stale_claims` design below. Building a separate `claim-watchdog` task would duplicate it. The same task also has an *evidence-gated completion* branch (Mode 1 of the silent-completion family); `migration_claimed_item_timeout_evidence_check.sql` tightens that branch (page-specific `deployed_at` evidence vs the old loose site-level `updated_at`). So **Lever C is effectively already implemented**; the remaining actions are: (a) **verify the migration is applied** (the homepage was false-completed under the old loose evidence on 06-03, so it was NOT applied then) and apply it; (b) optionally **tune the 40-min threshold** down toward ~25 min if faster unfreeze is wanted (still safe — above the 1200s `call_handler`); (c) proceed to the **guardrails** (timeout alignment, fairness `ORDER BY`, git-adapter retry), which are the real remaining dispatch work. The design below is retained only as the rationale/spec that the existing `reset` CTE already satisfies.
