...
> **2026-06-05 (later) — supersedes the "convergence active on the first pass" claim in this section: the convergence has been INERT, not merely first-pass-gated.** A clean re-adoption proved `reconcilePlanWithRealised` has never run for any site. `ValidateSitePlanAction` reads `existing_pages` with `ev.([]interface{})`, but `load_existing_pages` runs via `query_database`, which returns `[]map[string]interface{}` — a type that does NOT satisfy that assertion in Go. So `existingPages` was always empty and the reconcile early-returned **silently** (no log on the empty case). Symptom: gamesdesign produced the same bare-sibling plan (5 bare `*-basics`/etc., zero `guide-*`) on both the June-3 and June-5 builds despite `adoption_locked` being true and `existing_pages` populated for `plan_site`. Consequence: the entire doc-029 Phase-1 convergence (and the item-stem dedup + the union sections-carry, which sit downstream of it) has been dead since deploy. **Fix** in `v3_site_actions.go`: `ValidateSitePlanAction` now accepts both `[]interface{}` and `[]map[string]interface{}` (converting the latter), plus an `existing pages loaded for convergence` count log so an empty set can never fail silently again. **VERIFIED RESOLVED on a clean run (2026-06-05 17:26Z, corr 6381cb13).** A fresh adoption from a cleared DB produced a converged plan: `site_plan_pages` for gamesdesign held `guide-economy-basics … guide-skinner-box` all as `role=guide` (5), with ZERO bare siblings (`economy-basics`/`fairness-in-rng`/`p2p-architecture`/`rng-design`/`skinner-box` absent) — i.e. Pass A unioned the adopted guides into the plan and Pass C2 dropped the bare-sibling duplicates, both firing for the first time. The type fix engaged `reconcilePlanWithRealised` exactly as intended. The statements below now describe ACTUAL, verified behaviour. (Note: separate content/render-layer defects surfaced on this same run — `guide-list` rendered empty on the guides hub *and* the root landing, and every hero emitted placeholder CTAs to `/contact.html` + `/services.html`. Neither is a convergence defect; both are tracked separately.)
