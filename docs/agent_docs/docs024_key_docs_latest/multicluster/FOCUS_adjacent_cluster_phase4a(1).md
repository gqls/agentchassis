...
Don't change the existing `pgbouncer-clients` Service (it's still in-cluster ClusterIP for primary's agents). Add a **second** Service that targets the same Pods but is `type: NodePort`:
...
kind: Service
metadata:
  name: pgbouncer-clients-external
  namespace: ai-persona-system
  labels:
    app: pgbouncer-clients
    purpose: cross-cluster
spec:
  type: NodePort
  selector:
    app: pgbouncer-clients          # same selector as in-cluster Service
  ports:
    - name: pgbouncer
      port: 6432
      targetPort: 6432
...
Why a second Service instead of changing the first: in-cluster consumers continue to use `pgbouncer-clients.ai-persona-system.svc.cluster.local:6432` exactly as today. Zero risk to existing traffic. The cross-cluster path is opt-in.
