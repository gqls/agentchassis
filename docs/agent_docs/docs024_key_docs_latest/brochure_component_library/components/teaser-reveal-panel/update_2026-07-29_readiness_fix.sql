...
(function () {
  ...
  // THE BUG THIS FIXES: snippets.js is included as a plain, non-deferred
  // <script src> in <head>, which executes and blocks HTML parsing at that
  // point -- before the panel markup later in <body> exists. Querying the DOM
  // immediately found zero panels and the whole file quietly did nothing, on
  // every page, since the very first version (verified: querying for panels
  // synchronously at parse time returns none; hero-card-carousel's own
  // snippet already guards against exactly this with the same readyState
  // check, which this file lacked). Wait for DOMContentLoaded when the
  // document is still loading; otherwise (e.g. dynamically inserted script)
