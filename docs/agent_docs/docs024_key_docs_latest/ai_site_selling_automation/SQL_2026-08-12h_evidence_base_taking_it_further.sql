-- FILE: SQL_2026-08-12h_evidence_base_taking_it_further.sql
--
...
-- change the premise later — so the question is OPEN, and the register's own
-- governing rule covers exactly this case: where something is not settled the
-- page says nothing about it, because silence is always publishable and a
-- plausible-sounding invention never is. The fact therefore licenses ONE thing:
-- the outward pointer.
--
-- The writer_block addition states both halves of that, because a writer told
-- only "do not offer our time" will helpfully write "we never do this" instead,
-- which forecloses a decision the owner has explicitly left open. Two
-- prohibitions, opposite directions, one paragraph.
--
-- NOT DONE HERE, and it needs the owner: the actual NAMES. Naming a service
-- means adding it to `allowed_entities` (currently webdesign.uk, Stripe, the
...
$json$::jsonb,
       'owner_ruling',
       'Adds taking_it_further (owner 2026-08-12): building on the site is not a '
       'service offered here; the customer owns the files and may take them '
       'anywhere; third-party options may be listed. Paid follow-on work is '
       'explicitly UNSETTLED, so the copy must neither offer it nor rule it out. '
       'Named services still owed — naming one requires allowed_entities.',
       true,
...
DO $$
DECLARE n_facts int; is_pinned bool; has_fact bool; wb text;
BEGIN
  SELECT jsonb_array_length(data->'facts'), COALESCE(pinned,false),
         data->'facts' @> '[{"id":"taking_it_further"}]'::jsonb, data->>'writer_block'
    INTO n_facts, is_pinned, has_fact, wb
    FROM site_specs
   WHERE site_id='1fcfa4f3-ec80-4010-878b-b971cd46711f' AND aspect='evidence_base' AND is_current;

  IF n_facts <> 14 THEN RAISE EXCEPTION 'facts: expected 14, got %', n_facts; END IF;
  IF NOT is_pinned THEN RAISE EXCEPTION 'pinned not inherited'; END IF;
  IF NOT has_fact THEN RAISE EXCEPTION 'taking_it_further missing'; END IF;

  -- BOTH prohibitions must be in the wire, not just the one that feels urgent.
  IF wb NOT LIKE '%never offer our time%' THEN
    RAISE EXCEPTION 'writer_block does not forbid offering our time';
  END IF;
  IF wb NOT LIKE '%never state that we would never do it either%' THEN
    RAISE EXCEPTION 'writer_block does not forbid foreclosing the open decision';
  END IF;
END $$;
