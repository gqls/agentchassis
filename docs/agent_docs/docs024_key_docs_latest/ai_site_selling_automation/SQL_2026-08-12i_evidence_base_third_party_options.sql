-- FILE: SQL_2026-08-12i_evidence_base_third_party_options.sql
--
-- webdesign.uk register: the named third-party services, by category, that the
-- site may point customers at. Owner, 2026-08-12: list third-party services for
-- customers who want to take their sites further; quality matters more than
-- price; not the design agencies everyone else promotes.
--
-- CHOSEN AROUND WHAT OUR CUSTOMER ACTUALLY HOLDS — a ZIP of plain static HTML,
-- hosted by them — rather than around what is easiest to monetise:
--   hosting        Cloudflare Pages, Netlify   (static files, free tier)
--   statistics     Fathom Analytics, Plausible (privacy-first, cookie-free)
--   contact forms  Formspree, Basin            (accept a post from a static page)
--
-- THE AFFILIATE PICTURE, MEASURED RATHER THAN ASSUMED, because the owner asked
-- for existing mechanisms he could apply to:
--   Fathom      HAS one — 25% recurring, $10 off the referred customer, 30-day
--               cookie. CONDITION: standalone affiliate accounts were removed,
--               so it now requires an active Fathom subscription of your own.
--   Plausible   HAS NOT, on principle: "never spent a cent on advertising,
--               affiliates or paid endorsements".
--   Formspree, Basin   no programme found.
--   CloudCannon runs a partner programme (agency discount + referrals) rather
--               than an affiliate commission, and is git-repo shaped, which our
--               plain-HTML deliverable is not — so it is NOT listed.
--
-- THE FINDING THE OWNER SHOULD SEE: in this space the quality bar and the
-- affiliate bar mostly point in OPPOSITE directions. The best of these refuses
-- affiliates as a matter of principle. Optimising the list for commission would
-- drag it toward the mediocre, which is the exact outcome the owner asked to
-- avoid.
--
-- DELIBERATELY NOT LISTED: anywhere to find a PERSON. Every directory the
-- research surfaced is what the owner explicitly rejected — Toptal, Behance,
-- Dribbble, and Working Not Working, which Fiverr owns. Webflow and Framer
-- expert directories fail for a second reason: those people would REBUILD the
-- site on their platform rather than extend the files we hand over. Silence is
-- publishable and a padded list is not, so the category is left empty pending
-- the owner's steer. This is the category he cared most about.
--
-- The writer_block addition bounds how a named service may be described: no
-- ranking, no superlative, no price we do not charge, no promise about what any
-- of them will do. We point at a door; we do not vouch for the room.
...
$json$::jsonb,
       'owner_ruling',
       'Adds third_party_options (owner 2026-08-12) and the six services to '
       'allowed_entities. Affiliate state measured, not assumed: Fathom has a '
       '25% recurring programme but now requires an active subscription; '
       'Plausible refuses affiliates on principle; the forms services have none. '
       'The find-a-person category is deliberately EMPTY — every directory found '
       'is one the owner rejected, and the platform expert directories would '
       'rebuild the site rather than extend it.',
       true,
...
DO $$
DECLARE n_facts int; n_ents int; is_pinned bool; wb text;
BEGIN
  SELECT jsonb_array_length(data->'facts'), jsonb_array_length(data->'allowed_entities'),
         COALESCE(pinned,false), data->>'writer_block'
    INTO n_facts, n_ents, is_pinned, wb
    FROM site_specs
   WHERE site_id='1fcfa4f3-ec80-4010-878b-b971cd46711f' AND aspect='evidence_base' AND is_current;

  IF n_facts <> 15 THEN RAISE EXCEPTION 'facts: expected 15, got %', n_facts; END IF;
  IF NOT is_pinned THEN RAISE EXCEPTION 'pinned not inherited'; END IF;

  -- A named company that is not an allowed entity is exactly what the claims
  -- layer exists to catch. Naming six services and forgetting the list would
  -- make every mention a finding.
  IF n_ents <> 10 THEN
    RAISE EXCEPTION 'allowed_entities: expected 10, got % — a named service would read as an unsupported entity', n_ents;
  END IF;

  IF wb NOT LIKE '%pointing at a door%' THEN
    RAISE EXCEPTION 'writer_block does not bound how a named service may be described';
  END IF;
END $$;
