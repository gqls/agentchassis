-- FILE: SQL_2026-08-13a_writer_block_names_the_services.sql
--
-- webdesign.uk register: put the six third-party service NAMES into
-- writer_block's own text.
--
-- THE DEFECT THIS FIXES IS MINE, and it is this lane's own 2026-08-09 lesson
-- recurring: **facts[] is bookkeeping; writer_block is the wire. A fact not
-- copied into writer_block does not exist for the writer.** SQL_2026-08-12i put
-- the six names into facts[] and allowed_entities, and writer_block referred to
-- them as "in fact `third_party_options`" — a data structure the writer never
-- receives. writer_block's own whitelist paragraph then instructed: if a claim
-- is not in this list, "rewrite around it or leave it out". Asked twice
-- (gapfill_faq, gapfill2_faq) to name the services, the writer refused twice
-- and wrote around them. **The writer was not disobedient; it was obedient to
-- the stricter rule.** Both "failed" rounds were this spec defect.
...
$json$::jsonb,
       'owner_ruling',
       'Puts the six service names INTO writer_block text (was: a reference to '
       'fact third_party_options, which the writer never sees — the 2026-08-09 '
       'facts-vs-wire lesson recurring). No fact or ban changes.',
       true,
       'ai-site-selling-automation-2026-08-13',
...
DO $$
DECLARE wb text; is_pinned bool;
BEGIN
  SELECT data->>'writer_block', COALESCE(pinned,false) INTO wb, is_pinned
    FROM site_specs
  ...
  IF wb NOT LIKE '%Netlify%' OR wb NOT LIKE '%Plausible%' OR wb NOT LIKE '%Basin%' THEN
    RAISE EXCEPTION 'the names did not reach the wire';
  END IF;
END $$;
