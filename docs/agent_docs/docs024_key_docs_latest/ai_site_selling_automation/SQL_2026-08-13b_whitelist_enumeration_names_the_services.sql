-- FILE: SQL_2026-08-13b_whitelist_enumeration_names_the_services.sql
--
-- Root cause of THREE failed rounds, read from the rendered prompt
-- (llm_call_log 9c7735b8): the six service names were IN the prompt (fix
-- 2026-08-13a) — but writer_block's governing sentence, "What you may state as
-- fact is … AND NOTHING ELSE: [enumerated list]", enumerated every fact EXCEPT
-- them. Two instructions conflicted; the writer obeyed the stricter one, all
-- three rounds. The permission sentence was advisory; the enumeration is the
-- law. This adds the six to the enumeration itself.
...
$json$::jsonb,
 'owner_ruling',
 'Adds the six third-party services to writer_block''s whitelist ENUMERATION '
 '("and nothing else" list). Diagnosed from the rendered prompt: the names were '
 'present but outside the governing list, so the writer wrote around them three '
 'rounds running.',
...
BEGIN
  ...
  IF wb NOT LIKE '%Formspree, Basin)%' THEN
    RAISE EXCEPTION 'the six are still outside the whitelist enumeration';
  END IF;
END $$;
COMMIT;
