-- FILE: SQL_2026-08-13c_writer_block_services_imperative.sql
--
-- The REAL cause of four failed rounds, found by probing the rendered prompts:
-- `content_rewrite.spec.content_guidance` HAS NO READER. Four writers
-- (apply_gap_plan, tool_content_item, create_tool_component, deploy_tool),
-- zero readers, grep-proven at HEAD; and all four rounds' guidance strings are
-- absent from every page-content-writer prompt (llm_call_log probes, rounds 3
-- and 4). The writer only ever received writer_block + the existing section;
-- edit_live preserves, so with no instruction to ADD, it correctly added
-- nothing. Rounds 1–3's diagnosed spec defects were real but incidental.
--
-- The fix uses the one channel proven to drive behaviour: writer_block itself.
-- The permission sentence becomes an IMPERATIVE — the FAQ's taking-it-further
-- answer must name the six, as standing content. This also survives every
-- future rewrite with no per-item guidance, which suits content that is part
-- of the offer rather than a one-off edit.
...
$json$::jsonb,
 'owner_ruling',
 'writer_block: naming the six services in the taking-it-further answer becomes '
 'an IMPERATIVE (standing content), because content_guidance has no reader and '
 'permission alone moved nothing in four rounds.',
 true, 'ai-site-selling-automation-2026-08-13', COALESCE(pinned,false)
FROM site_specs
WHERE site_id='1fcfa4f3-ec80-4010-878b-b971cd46711f' AND aspect='evidence_base' AND is_current=false
...
BEGIN
  ...
  IF wb NOT LIKE '%standing content, not an option%' THEN
    RAISE EXCEPTION 'the imperative did not reach the wire';
  END IF;
END $$;
COMMIT;
