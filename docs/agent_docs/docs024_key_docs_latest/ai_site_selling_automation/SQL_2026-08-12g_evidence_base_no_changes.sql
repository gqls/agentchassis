-- FILE: SQL_2026-08-12g_evidence_base_no_changes.sql
--
-- webdesign.uk register: apply owner ruling PLAN §1d. Three changes, one edit.
--
-- 1. RETIRE `changes_included_one_set`. The owner retired the included set of
--    changes on 2026-08-12 evening. The term shipped to all five pages that
--    same afternoon, so it is currently LIVE AND FALSE on every one of them —
--    a true claim made false by a ruling four hours after it went out, which is
--    the mirror image of the stale-claim problem this lane spent the day on.
--
-- 2. ADD `yours_to_change`, the owner's own point and the reason the hard term
--    is fair rather than merely harsh: the customer gets the site's files and
--    can change anything; editing something that already works is far less work
--    than starting from a blank page. **WE do not change it; YOU can.** It is
--    not a softener bolted onto a bad term — it is the better half of the deal,
--    and writer_block says so and fixes the order (hard term first, then this).
--
-- 3. ARM DETECTION on the retired wording, because correcting the register does
--    not remove the claim from the page (bugs_open/161) and because this lane
--    has now had the same class of defect twice in one day.
--
-- MEASURED BEFORE WRITING, and it could have come out otherwise: over the 25
-- live components, the current register finds 14 and this one finds 19 — the
-- five extra are exactly one per live page. An independent DB query
-- (`rendered_html ~* '(one|1|a) set of changes'`) returns the SAME five
-- components. Two methods, same answer, no misses and no false positives.
--
-- ROLLBACK: the superseded row is one UPDATE from current.
...
$json$::jsonb,
       'owner_ruling',
       'Owner ruling PLAN §1d (2026-08-12 evening): no changes included at £149, '
       'take it or leave it. Retires changes_included_one_set, which was live on '
       'all five pages. Adds yours_to_change — the files are the customer''s to '
       'edit, and editing beats starting from a blank page — which is what makes '
       'the hard term fair. Two new banned patterns arm detection on both the '
       'two-rounds and one-set wordings and on any offer of our time.',
       true,
...
DO $$
DECLARE n int; n_facts int; n_bans int; is_pinned bool; has_new bool; has_old bool;
BEGIN
  ...
  IF n <> 1 THEN RAISE EXCEPTION 'expected 1 current register, found %', n; END IF;

  SELECT jsonb_array_length(data->'facts'), jsonb_array_length(data->'banned_claims'),
         COALESCE(pinned,false),
         data->'facts' @> '[{"id":"yours_to_change"}]'::jsonb,
         data->'facts' @> '[{"id":"changes_included_one_set"}]'::jsonb
    INTO n_facts, n_bans, is_pinned, has_new, has_old
    FROM site_specs
   WHERE site_id='1fcfa4f3-ec80-4010-878b-b971cd46711f' AND aspect='evidence_base' AND is_current;

  IF n_facts <> 13 THEN RAISE EXCEPTION 'facts: expected 13, got %', n_facts; END IF;
  IF n_bans  <> 27 THEN RAISE EXCEPTION 'bans: expected 27, got %', n_bans; END IF;
  IF NOT is_pinned THEN RAISE EXCEPTION 'pinned not inherited'; END IF;
  IF NOT has_new THEN RAISE EXCEPTION 'fact yours_to_change is missing — the hard term would stand alone'; END IF;
  IF has_old THEN RAISE EXCEPTION 'the retired changes_included_one_set fact survived'; END IF;
END $$;
