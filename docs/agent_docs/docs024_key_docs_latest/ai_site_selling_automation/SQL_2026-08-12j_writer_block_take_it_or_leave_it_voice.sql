-- FILE: SQL_2026-08-12j_writer_block_take_it_or_leave_it_voice.sql
--
-- webdesign.uk: the take-it-or-leave-it REGISTER, written into writer_block.
-- Owner, 2026-08-12: make the copy really scrappy — "well written but sort of
-- saying hard that this is the deal" — because stating the terms hard is the
-- only way £149 works.
--
-- WHY writer_block AND NOT content_direction, which is where a voice would
-- normally live: `content_direction` is being actively edited by the fleet
-- voice lane (`claude-ideauk-copy-20260812`), whose session was live four
-- minutes before this was written. Two sessions writing one spec is the
-- same-file collision no pathspec commit can prevent. `writer_block` is this
-- lane's own key, and it is also the one text the writer demonstrably reads as
-- its facts section — this lane learned on 2026-08-09 that facts[] is
-- bookkeeping and writer_block is the wire. So the voice goes where it will
-- actually be obeyed, and nothing of theirs is touched.
--
-- TWO GUARDS AGAINST THE OBVIOUS MISREADINGS, both stated because a writer
-- given "be scrappy" will reliably produce one of them:
--   * NOT self-deprecating. We are not admitting to a cheap service, we are
--     describing a deliberate one. A sentence that sounds like an apology has
--     it exactly backwards.
--   * NOT scruffy WRITING. The words are the one thing on this site that must
--     be immaculate. The OFFER is stripped back; the CRAFT never is. This is
--     the owner's own rule — utmost quality in the product, as if delivering
--     for a corporate — applied to the prose.
--
-- The register it aims for: someone behind a trade counter who knows what he is
-- selling and is not going to talk you into it. Unhurried, unbothered, faintly
-- amused, unapologetic. Confidence, never aggression — the flatness IS the
-- confidence.
...
$json$::jsonb,
       'owner_ruling',
       'The take-it-or-leave-it register (owner 2026-08-12). Written into '
       'writer_block rather than content_direction because the fleet voice lane '
       'was live on that spec. Guards both misreadings: not self-deprecating, '
       'and not scruffy writing — the offer is stripped back, the craft is not.',
       true,
...
DO $$
DECLARE wb text; n_facts int; is_pinned bool;
BEGIN
  SELECT data->>'writer_block', jsonb_array_length(data->'facts'), COALESCE(pinned,false)
    INTO wb, n_facts, is_pinned
    FROM site_specs
   WHERE site_id='1fcfa4f3-ec80-4010-878b-b971cd46711f' AND aspect='evidence_base' AND is_current;

  IF n_facts <> 15 THEN RAISE EXCEPTION 'facts changed: expected 15, got %', n_facts; END IF;
  IF NOT is_pinned THEN RAISE EXCEPTION 'pinned not inherited'; END IF;

  -- Both guards must be in the wire. A voice instruction that says only "be
  -- hard" reliably produces either an apology or bad prose, and this lane has
  -- no second chance at a five-page rewrite.
  IF wb NOT LIKE '%not self-deprecating%' THEN
    RAISE EXCEPTION 'writer_block does not guard against the apologetic reading';
  END IF;
  IF wb NOT LIKE '%not scruffy WRITING%' THEN
    RAISE EXCEPTION 'writer_block does not guard the craft — the offer is stripped back, the prose is not';
  END IF;
END $$;
