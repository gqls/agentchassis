...
- [ ] **T1 — Adoption → tool-pipeline handoff (M2 fix, owns the gap).** When adoption classifies a source page as `page_type='tool'`, emit an `add_tool`/`evaluate_tools` work item so `tool-generator` produces a widget. Adoption *emits*; tool-generator *owns generation*. No widget logic inside adoption.
...
- **2026-05-26 (step 3)** — Ran queries D/E/F. **Gating resolved: M2, not M1.** No tool component exists for the site (D=0), tool pipeline never ran (E = adoption recreate only), no clobber occurred (F=0 history). §2.4 puzzle resolved: sections came from the adoption crawl via the recreate path, not `site_plan`. Read `load_page_sections_from_spec` source (confirms §2.4 sync). Recorded responsibility analysis (overlap on identity + gap on widget ownership). Added tasks T1–T4 (§5b), including T3 canonicalisation of `create_tool_component`/`deploy_tool` page identity (the naming divergence). Wrote debugging-guide addendum. Next: decide T1 vs T3 ordering with current code in hand.
