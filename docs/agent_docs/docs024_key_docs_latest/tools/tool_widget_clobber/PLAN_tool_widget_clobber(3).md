...
Concrete, ordered. None implemented yet; pull current revisions first (adoption code is moving in parallel).

- [ ] **T1 — Fix the adoption interactivity misroute (M2 fix). REFRAMED (§2.6): the handoff already exists.** The route to `tool-recreation-handler` is gated on `len(page.Features) > 0`; for gamesdesign tool pages `Features` was empty, so they went to `page-build-handler`. Prime suspect: `buildPageFeatureMap` keys by the raw `fm["page"]` while the routing loop looks up the **canonicalised** `pageName` (tools gain a `tool-` prefix) → guaranteed miss for tool pages. Fix: canonicalise the feature key in `buildPageFeatureMap` (or match raw-name before canonicalisation), so detected tools route to `tool-recreation-handler`. **Confirm b1 vs b2 first** (query G): if adoption emitted no `interactive_features` at all (b2), the detection prompt is the target instead. Also confirm `tool-recreation-handler` is `active` with a current image.
- [ ] **T2 — Post-adoption detection check (safety net).** Discovery check: `page_type='tool'` page with zero `component_level='tool'` `page_components` → emit `add_tool`. Also closes the `check_tool_health` INNER-JOIN blind spot.
...
-- G) DECIDES b1 vs b2: did adoption emit interactive_features, and with what page keys?
--    keys without a 'tool-' prefix (e.g. 'drop-rate-simulator') => b1 (key desync vs canonicalised lookup).
--    no rows                                                    => b2 (detection emitted nothing).
--    (confirm the JSON column name with \d research_results — may be `results`/`data`/`analysis`.)
SELECT jsonb_path_query(rr.results, '$.interactive_features[*].page') AS feature_page_key
...
- **2026-05-26 (step 4)** — Verified the "no agent owns widget generation" claim before acting; **it was false.** `apply_adoption_plan` routes interactive pages (`len(page.Features) > 0`) to **`tool-recreation-handler`** (a real agent that LLM-recreates the widget, gates on `check_tool_completeness`, deploys via `page-rerender`). The cause is a **misroute**: gamesdesign tool pages had empty `Features` → static `page-build-handler` path. Prime suspect: `buildPageFeatureMap` keys by raw `fm["page"]` while the loop looks up the canonicalised `pageName` (tools gain `tool-` prefix) → guaranteed miss (§2.6). T1 reframed from "build a handoff" to "fix the misroute / key desync". Added query G to decide b1 (key desync) vs b2 (no features emitted). Also noted `check_missing_tools` exists as a separate site-level `evaluate_tools` trigger (not the cause here). Next: run G; confirm `tool-recreation-handler` active.
