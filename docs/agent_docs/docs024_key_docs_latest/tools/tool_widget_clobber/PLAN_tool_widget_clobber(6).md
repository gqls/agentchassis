...
- [ ] **T1 — Fix the adoption interactivity misroute (M2 fix). FULLY CONFIRMED + FIX DRAFTED (§2.7, step 6).** Queries H/I confirmed: tools stored bare (`drop-rate-simulator`/`tool`), games prefixed (`game-…`/`game`); all five games routed `needs_tool_recreation → tool-recreation-handler` (complete), zero tools did. Fix: replace `buildPageFeatureMap` (apply_adoption_plan_action.go:721–742) to key by **canonical** name via `datahelpers.CanonicalisePage`, using `plan["pages"]` to resolve each feature's role. `adoptedPage.Features` (line 551) and `contentData["interactive_features"]` (line 525) both read `pageFeatures[pageName]` (canonical), so the one change fixes routing + attachment. One local rename noted (`pageName`→`rawPage` inside the function). `tool-recreation-handler` untouched; reuses existing helper; no import changes expected (confirm on current revision). Minimal alt (change the two lookups to `rawName`) rejected — reintroduces a raw join space.
- [ ] **T1b — Backfill existing gamesdesign tool pages.** The T1 fix is forward-looking; the six already-built static tool pages won't self-heal. Emit `needs_tool_recreation` for them (or re-run adoption). T2 (detection check) is the clean general mechanism.
- [ ] **T4 sequencing input — do the games' widgets survive?** Games went through `tool-recreation-handler` and completed. Check whether the live game pages currently carry a `component_level='tool'` component (widget intact) or were later clobbered by a rebuild. Intact → M1 not biting in practice (T4 lower priority); wiped → T4 moves up.
...
- **2026-05-26 (step 6)** — Ran H + I. **Fully confirmed.** H: tools bare/`tool`, games prefixed/`game`. I: all five games routed `needs_tool_recreation → tool-recreation-handler` (complete); zero tools. Read current `apply_adoption_plan_action.go`: `buildPageFeatureMap` at 721–742 keys raw; `adoptedPage.Features` (551) and `contentData` (525) both read `pageFeatures[pageName]` (canonical) — so one change fixes both. Drafted the canonical-keying replacement (reuses `CanonicalisePage`, one noted local rename). Added T1b (backfill existing pages — they won't self-heal) and a T4-sequencing check (do the games' widgets survive a later rebuild?).
