...
- [ ] **T1 — Fix the adoption interactivity misroute (M2 fix). CONFIRMED b1 (§2.7): prefix desync in `buildPageFeatureMap`.** The route to `tool-recreation-handler` is gated on `len(page.Features) > 0`; tools get empty `Features` because the feature map is keyed by raw `fm["page"]` (`drop-rate-simulator`) while the loop looks up the canonical `pageName` (`tool-drop-rate-simulator`). Fix, self-contained in `buildPageFeatureMap` (already receives `plan`): resolve each feature's role from `plan["pages"]` and key the map by the **canonical** name via `CanonicalisePage`. No signature change; reuses the existing helper; `tool-recreation-handler` untouched. Confirm exact `raw_name`/`raw_type` (§7 query 1) before writing the key transform; confirm games routed to `tool-recreation-handler` (§7 query 2) as the mechanism check. Also confirm `tool-recreation-handler` is `active` with a current image.
