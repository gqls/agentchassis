...
- **Commercial model (Section 13):** operator-primary (thousands of domains),
  vendor-optional (sell a domain + its backend; rarely the whole framework).
  Separability is a *saleability* property, not just blast-radius. Operating
  thousands ≠ thousands of clusters — isolate at the *satellite*, partition for
  sale at the *domain*. Honour now: `owner_id` on sites, an entitlement seam at
  build-submission and maintenance-run, a **pluggable billing adapter (Stripe
  first)**, credential parameterization, and a build-tier cost profile.
...
## 13. Commercial model, entitlement, and separability

Recorded because one requirement here is load-bearing and expensive to retrofit:
sites (and sometimes the whole framework) get **sold**. Separability is therefore
not only a blast-radius property — it is what makes the product saleable.

### Resolved direction

- **Operator-primary, vendor-optional.** Operate **thousands of domains**
  yourself; some have chat capability. When a domain is bought, optionally sell
  its **backend** too. (This resolves the earlier operator-vs-vendor fork: it is
  both, with operating as the default.)
- **Two sale granularities:**
  - *Per-domain (+ backend)* — the common, high-volume case: hand a single
    domain's site + its dynamic backend (chat, data, functions, entitlement) to a
    buyer.
  - *Whole-instance / framework* — the rare, large case: sell the SaaS unit
    itself (e.g. `design.co.uk`-as-a-business, including build capability).

### Operating thousands ≠ thousands of clusters

Two different units of granularity, and conflating them would either bankrupt the
ops budget or kill the sell option:

- **Unit of blast-radius isolation = the satellite** (vs core). One or a few.
- **Unit of separability-for-sale = the domain**, *partitioned within* a
  satellite's stores. Operate many domains on one/few satellites with strict
  per-domain partitioning (keyed on `site_id`/`domain`) so any single domain can
  be **lifted out** at sale time. Per-domain infrastructure is **not** required;
  per-domain *partition + extractability + swappable credentials* is.

So the "build separable units even though you operate several" guidance becomes:
not separate clusters per domain, but clean per-domain partitioning plus the
ability to extract one domain's artifact + data + re-pointed credentials.

### Seams to honour now (cheap now, expensive to retrofit)

1. **Ownership on site rows.** Site rows born with an `owner_id`; entitlement is
   keyed on owner; sell-on = re-point `owner_id` + credentials + billing. (Under
   Y-copy the satellite's `sites` needs this column — ties to Section 6.)
2. **Entitlement seam at two points**, both calling the billing/entitlement
   *abstraction* (never Stripe directly):
   - *Build-submission / intake* — before the orchestrator spends a penny, check
     allowed/paid (free-tier quota remaining, or paid). Routes to paywall if not.
   - *Maintenance-run* — before `maintenance_queue` work runs on a hosted site,
     check the subscription is active; skip/pause if lapsed.
3. **Pluggable billing adapter.** An interface (check entitlement, record/verify
   payment, handle webhook) with **Stripe as the first implementation**,
   swappable to other providers or a buyer's choice. Same adapter discipline as
   the edge / LLM / storage abstractions already in the design.
4. **Credential parameterization.** No hardcoded keys anywhere; LLM, storage, and
   billing credentials all come from config/secrets that swap at handover. The
   per-domain chat already uses a per-deployment edge secret; extend the
   discipline platform-wide so a sold unit can run on the buyer's accounts.
5. **Build-tier / cost profile.** A flag on the build (`saas_cheap` vs
   `portfolio`) driving model selection (Haiku / local Ollama, batched,
   heavy template/component reuse) so a $5 build has margin, and "next day" lets
   builds batch onto cheaper/off-peak capacity. Uses the existing step/agent-level
   LLM config mechanism; cheap to add as a parameter, annoying to retrofit if
   model choices are baked per agent.
6. **Snapshot-able building blocks.** For a *whole-instance* sale, the component/
   tool library, style collections, and knowledge_base patterns must snapshot
   *into* the sold unit (or be licensed as an update feed — a deferred commercial
   fork). A *per-domain* sale transfers only the built artifact + backend data and
   needs none of this unless the buyer wants to keep rebuilding.

### How each pricing model maps to a seam (none need building now)

| Model | Seam it needs |
|---|---|
| $5/site, next-day | Entitlement gate at build-submission + `saas_cheap` build-tier + batch scheduling |
| First-site-free | Same gate with a free quota; still needs identity (email verify) — free is a cost/abuse magnet |
| Maintenance subscription | Entitlement gate at maintenance-run |
| Sell domain + backend | `owner_id` re-point + credential/billing swap; per-domain partition extract |
| Sell whole framework | Building-block snapshot + credential parameterization + whole-instance extract |

### Ownership already exists — what is and isn't new (corrected after reading the schema)

The platform already has an ownership hierarchy: **`clients` → `networks` →
`sites`** (`sites.network_id → networks.client_id → clients.id`, all FK'd). So
ownership is **reuse, not net-new**, which corrects the earlier "net-new identity
layer" note. Specifics that matter:

- **Owner = `client`.** `clients.external_id` (unique) is the natural hook to map
  to an external billing/identity id (e.g. a Stripe customer); `clients.settings`
  and `networks.settings` are jsonb with room for plan/entitlement state.
- **`owner_id` on sites is unnecessary** — ownership is transitive via
  `network_id`. **Per-domain sell-on = re-parent the site's `network_id`** to the
  buyer's network/client and swap credentials. The hierarchy already supports
  moving a site between owners.

**What is genuinely new:** entitlement/subscription *state* (a table FK'd to
`clients`/`networks`, or fields in their `settings`), the pluggable billing
adapter mapping `clients.external_id` ↔ provider, and the two gate checks.

### Billing is *scaffolded, not wired* — corrected again after reading the implementation

Earlier I wrote "billing largely exists." Reading the actual
`subscription/{models,repository,service,handlers}.go` corrects that: the **data
model** exists, but there is **no working payment integration**. What's real vs
aspirational:

Real (reusable scaffold):
- A `subscriptions` table/model (`user_id`, `tier`, `status`
  active/trialing/past_due/canceled/expired, dates, `payment_method`,
  `stripe_customer_id`, `stripe_subscription_id`), a `subscription_tiers` table,
  tier constants (`free`/`basic`/`premium`/`enterprise`), admin CRUD handlers, and
  a `CheckoutSession` *type*.
- The **JWT carries `client_id` + `tier`**, and `client_id` is the shared key to
  the chassis (`networks.client_id → clients.id`, `clients.external_id` the
  back-reference). Propagation is genuinely there.

Aspirational / not implemented (verified):
- **No Stripe integration at all.** `service.go` imports no Stripe SDK.
  `CreateSubscription` is a bare DB insert that sets `status = active` with **no
  payment step**; nothing creates a real checkout session; **there is no webhook
  handler.** The `CheckoutSession` type is never constructed.
- **Quota enforcement is non-functional.** `GetUsageStats` returns hardcoded
  zeros ("returning mock data"), so `CheckQuota` always passes (0 < max).
- **The module looks unexercised.** `repository.go` mixes `?` (MySQL-style)
  placeholders in most queries with `$1` (Postgres-style) in `ListAll` — it
  cannot all run against one database, a strong sign it has never been used in
  anger.
- `Create` doesn't even persist the `stripe_*` columns it selects.

**Security consequence for the gate:** today `status = active` means "a row
exists," not "payment cleared." A gate that trusts `subscription.status` (or the
JWT `tier`) is only as trustworthy as whoever can mint subscriptions — which,
until webhooks drive status, is unverified. The build-submission gate must key
off a **webhook-verified** status, which does not exist yet.

So the corrected "genuinely new" for billing is **larger** than last turn:

1. **Build the actual provider integration** (checkout/payment-intent creation +
   **webhooks** that drive `status` and write the provider ids + real usage
   queries to replace the mock). This is net-new, not "confirm/complete."
2. **Do it behind a provider interface from the start.** Because nothing calls
   Stripe yet, there is no retrofit cost — `provider` + `provider_customer_id` +
   `provider_subscription_id` generalises the `stripe_*` columns; Stripe is impl
   #1. This is the cleanest moment to honour "pluggable billing."
3. **Reframe tiers** around site product nouns (sites, builds-per-period,
   maintenance on/off, chat on/off) instead of personas/projects/content.
4. **Scope to `client` + add a one-off path.** Subscriptions are per-`user` and
   recurring; sites are per-`client`; the **$5-per-site** build is a one-off
   credit/charge, not a subscription. Per-client recurring + one-off credit are
   both additions.

Net: the *scaffold and propagation* are reuse; the *payment processing, webhook
truth, working quota, client-scoping, one-off path, and site tiers* are new. The
adapter discipline you want is free here precisely because the integration was
never built.

### Isolation bonus — entitlement travels in the JWT (with a caveat)

Because the token carries `client_id` + `tier`, the isolated satellite can
**validate the JWT and read tier/client without calling core or the auth
service** — a stateless fast path that keeps the boundary clean. Caveat from
above: the tier in the token is only trustworthy once subscription status is
webhook-verified. Until then it asserts "a subscription row says so," not
"payment cleared." The one-off $5 credit needs a lookup regardless.

**Where the gates actually sit (placed against real workflows):**

- **Build-submission gate — reuse the existing `approval_mode` model.**
  `site_work_items` already has `approval_mode` (`auto`/`hitl`/`eval`) where
  `hitl` holds an item in `pending_review` until released to `approved`; the
  dispatch loop already respects this. An entitlement gate is the same shape:
  hold the first expensive item (`needs_site_plan`, or `needs_briefing` if you
  want to charge before the brief) in a `pending_entitlement` state until the
  billing/entitlement check clears, then release. Nothing in the dispatch loop
  changes. *Product choice:* gate before the brief (pay to start) vs after the
  brief, before planning (free conversational qualification, pay to build —
  aligns with first-site-free and the chat funnel). Same mechanism either way.
- **Maintenance-run gate — filter the heartbeat site-selection queries.** The
  selection queries are the single insertion point: `build-pipeline-trigger`
  ("sites with triaged items"), `improvement-loop` ("least-recently-audited site,
  queue depth < 20"), `content-feed-trigger` ("news-feed sites"). Add an
  entitlement filter (join site → network → client, require active subscription)
  so lapsed/free sites drop out of ongoing maintenance. At thousands of domains
  this is the **cost-control valve**, not just a paywall — it earns its place in
  the pure-operator case before anything is ever sold. Filter at selection, not
  per item.
- **Build-tier economics already have a rail.** The batch architecture's rule —
  scheduled→batch (50% cheaper, ~24h), user-initiated→sync, chosen at
  workflow-config time — is exactly the `saas_cheap` vs `portfolio` split. A
  "next-day $5" build rides the batch/cheap-model path; a watched portfolio build
  stays sync. So the cost profile is partly a config choice on existing rails,
  not new machinery.

**Design the seam now** (entitlement abstraction + the two check points +
entitlement state on client/network); build billing depth later.

### Defers safely (while the unit stays separable)

Actual Stripe integration depth; franchise-vs-outright licensing for whole-frame
sales; any recursion of instances-selling-instances. None of these constrain the
build as long as separability and the seams above are honoured.
