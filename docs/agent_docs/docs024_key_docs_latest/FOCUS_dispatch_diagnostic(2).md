...
**Immediate unblock:** UPDATE-d the 7 stuck items to `pipeline='build'`. Items started claiming on the next tick.
...
**Research into what `pipeline` is for:**
...
| Audit's triage step config | `target_domain: "build"` — different terminology but plausibly the value that gets written to `pipeline` when triage promotes detected → triaged |
| `design-dispatch-loop` | Does not exist. The infrastructure is per-pipeline-dispatcher but only one dispatcher is wired up |

**What the field appears to be for, based on evidence:** a coarse routing label that lets multiple dispatchers consume different categories of work — analogous to how Kafka consumer groups partition work. The shape is built for it (each dispatcher has its own `item_pipeline` config; agent_definitions support different dispatchers); the wiring isn't. Only `pipeline='build'` items ever get dispatched today.
...
**Three architectural options, in increasing scope:**

A) **Drop `pipeline` entirely. Single dispatcher claims all triaged items.** Simplest. Risk: a flood of expensive image generation work could starve cheaper content work because all handlers compete for the same 5 dispatch slots. Mitigated by per-handler concurrency limits OR by priority weighting.

B) **Keep `pipeline` but make it derived from `handler_agent`.** A small mapping table (`handler_pipelines`) or a computed-column. Discovery checks no longer declare pipeline; it's inferred. Bug we hit becomes impossible by construction. Multiple dispatchers can still exist if needed for scheduling isolation.

C) **Keep `pipeline` as-is but enforce alignment between it and `handler_agent`.** A CHECK constraint or validation in the INSERT path that refuses mismatched pairs. Cheapest if multi-pipeline architecture is genuinely wanted; preserves the existing shape.

**User's intuition:** "fix handlers should choose what they fix based on what they can fix." This aligns with option A or B — handler_agent IS the source of truth for what gets handled by whom; pipeline is redundant. The strongest argument for keeping pipeline at all is operational isolation (a build dispatcher and a design dispatcher could have different cadences, priorities, error tolerances). Whether that benefit is real today is the question.

**Recommendation, not yet implemented:** start with option B. It captures the operational-isolation latent capability AND eliminates the consistency bug. Concrete steps would be:
1. Add a `pipeline` column derivation (either at write time via a trigger or via a small lookup table).
2. Update discovery checks (including `unfulfilled_imagery_plan`) to NOT set pipeline directly.
3. Verify with a fresh run on a clean site.

Or start with option A if simplification is preferred — let one dispatcher own the queue, retire `pipeline` as a real field, drop the index filter. Smaller code change, more aggressive simplification.

**Open until decided.** No urgency — the immediate unblock (UPDATE to `pipeline='build'`) keeps the loop closing. Worth bottoming out before the next discovery check is written, to avoid baking the same bug into Phase 4/5/6 emissions.
