...
if [ -n "${ORCH:-}" ]; then
  ...
  echo "   (grep EVERY chassis replica, and know the log rotates within MINUTES under"
  echo "    load — a clean grep is NOT a negative. The DURABLE witness is the audit"
  echo "    table: the takeover CAS is the only version-preserving writer, so:)"
  echo "   SELECT changed_at, old_version, new_version FROM orchestration_state_audit"
  echo "     WHERE orchestration_id='\$ORCH' AND old_version=new_version;"
  echo "   -- expect: your manual stamp, then the CAS moments before the step advances"
  echo "   PASS = the takeover CAS fingerprint appears AND the orchestration advances"
  echo "          past its current step with ONE execution (no ~3-min retry series):"
