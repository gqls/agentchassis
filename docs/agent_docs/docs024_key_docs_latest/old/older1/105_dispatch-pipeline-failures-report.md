...
| # | Issue | Impact | Items unblocked |
|---|-------|--------|-----------------|
| P1 | `component_id` nil bug in `load_tool` step | Pipeline bug | 5 `improve_tool` items |
| P2 | Fork tool unique constraint on `content_components` | Pipeline bug | 1 `add_tool` item |
| P3 | vonc.com serving raw CSS instead of page content | Upstream data/template | 3 `wont_fix` + potentially 10 `needs_section_data` |
| P4 | Stale item handling — 15+ items sitting 48h+ with no progress | Process gap | ~15 items |
| P5 | Timeout tuning for large payloads | Pipeline config | 2 timed-out items |
| P6 | Surface validation blocker details on `content_rewrite` failures | Visibility | 5 items in `needs_human_review` |
| P7 | Triage `needs_section_data` — auto-fill vs client input | Content gap | 25 items |
...
**Root cause:** The `fork tool` operation is trying to insert a new `content_components` row but hitting a unique constraint. Forking tools is a legitimate operation (fork-and-improve evolution), so the fork logic itself is buggy. Likely the fork isn't generating a new unique slug/key, or it's reusing the source component's identifier instead of creating a derivative one. Needs a fix in the `fork tool` database operation — probably needs to append a version suffix or generate a new ID rather than carrying over the original's key.
