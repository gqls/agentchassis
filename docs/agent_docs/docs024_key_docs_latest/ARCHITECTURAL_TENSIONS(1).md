...
- **Two role-normalisers** (`normaliseRole` vs `normalisePageType`) — latent
  divergence; they agree today only by coincidence of inputs.
...
- A single role-normalisation function and a single `CanonicalisePage`, called by
  whoever needs canonical identity, never two implementations.
...
Observed. The smallest move toward it, which also directly fixes the 2026-05-23
incident: make `CanonicalisePage` **identity-preserving on already-canonical
input** (a page named `games-index` with role section-index must round-trip to
`games-index`, not be re-derived from a stripped slug), and collapse the two
role-normalisers into one. This is the companion to the Tension #1 `ValidateRoles`
change scoped alongside this catalogue.
