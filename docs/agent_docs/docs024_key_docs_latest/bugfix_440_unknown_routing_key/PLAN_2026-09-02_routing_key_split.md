...
| 1b | creator stamps `routing_reason` alongside `reason` for in-vocabulary values (`create_rerender_items_action.go`); additive, gate still reads `reason` | council gate | AFTER the 404 lane has read its r4 verdict — same file; CONTRIB posted first |
