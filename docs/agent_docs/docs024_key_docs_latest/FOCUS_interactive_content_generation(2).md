...
- **`executeJavascript` actions** let us run arbitrary JS in the page
  before scraping and capture return values — e.g.
  `document.querySelectorAll('script')` mapped to source URLs and
  textContent. Returns land in `actions.javascriptReturns`.
...
plain rawHtml + external-fetch pattern.
