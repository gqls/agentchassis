...
**`resolve_composition_layout`** — thin wrapper over the shared `resolveLayoutByTags` helper (also used by `fork_theme_from_site`). Matches `classification.industry_tags` against `layouts.industry_tags` for active non-fallback layouts, picks the best-scored match. On fallback (no match crosses the score threshold), it queues a `needs_new_layout_candidate` item with `status: 'needs_human_review'` and an EMPTY `handler_agent` — the dispatch loop skips status `needs_human_review`, so nothing tries to claim it and no "blocked" flip happens; humans pick it up when they choose to. (Until 2026-08-17 this named `handler_agent: 'hitl-review'`, an agent that never existed — retired by `bugs_open/291`; the empty handler is the canonical parking idiom, migration 217.)
...
| Layout tag match below threshold | resolve_composition_layout uses default layout + queues `needs_new_layout_candidate` (status needs_human_review, no handler — the hitl-review name was retired by bugs_open/291). | Human reviews the queued candidate, decides whether to add a new layout to the library. Composition already installed with fallback layout. |
...
**~~`hitl-review` handler~~ — RETIRED 2026-08-17 (`bugs_open/291`), never built.** It was written here as a deferred roadmap item; tool-auditor's config then named it as a live handler with no `status` key, so review items were born dispatchable and bled to `blocked` (14 rows over 3 days before the fix). `needs_human_review` items now park with an EMPTY handler (migration 217 idiom); their working exit is the admin confirm endpoint (`confirm_work_item_handler.go` — a confirmed `spec.check='tool_auditor'` item files an `improve_tool` follow-up). A future review surface is `bugs_open/033`'s problem, and whatever it is, it keys on the STATUS, not a handler name.
