...
page-build-handler by design. So (a) a bare `page_rerender` would reproduce the phantom CTA
(it doesn't re-resolve CTAs) — the "repoint to page_rerender" idea is dropped; (b) the clobber
