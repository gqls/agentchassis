...
**LLM cleared (confirmed 2026-05-26 from `response_text`).** The 16:27 `plan_site`
output contained `game-auto-battler` and **not** `tool-auto-battler` or
`tool-game-auto-battler` (`out_kept_game_prefix=t, out_renamed_to_tool=f,
out_double_prefixed=f`; `output_tokens=4388`, so no truncation). So the LLM emitted
the games faithfully — both downstream names are post-LLM artefacts. That resolves
confirmation (a) below: it is not the model.

The open question is now narrow: **two downstream steps disagree about the same
page.** `write_site_plan` stored `tool-auto-battler` in `site_plan_pages`;
`sync_pages_to_db` wrote `tool-game-auto-battler` to `pages`. To settle it without
guessing (item 19): read (a) the `page_type` the LLM assigned the games in
`response_text` — the field that routes the `CanonicalisePage` branch — and
(b) the canonicalisation in `WriteSitePlanAction` vs `sync_pages_to_db`, i.e. why
one strips-then-prefixes (`game-auto-battler` → `auto-battler` → `tool-auto-battler`)
while the other prefixes the un-stripped name (`game-auto-battler` →
`tool-game-auto-battler`). Do not assume that strip-vs-no-strip split until the
two code paths confirm it. The build was still in flight when observed (reconciler
`needs_page` items all `triaged`, tool recreations completing), so the final
duplicate/orphan set may shift — re-run the planned-vs-realised query after it
drains before designing against it.
