...
## Path C — work breakdown

The full Path C work is split into six incremental steps, each
independently verifiable. After each step we pause and check the
output before moving on.

### C1 — extract_interactive_fingerprint (new action)

**Status:** in progress, 2026-05-15.

**Decision:** new file, not extending the design extractor. Reasons:
design and interactive are separate concerns with separate downstream
consumers (webdesign-agent vs planner/classifier/future generators);
the design extractor's output is a stable contract that shouldn't be
mutated; additive change is safer than expanding an existing contract.

**Step Zero done:** no existing action, agent, or spec aspect covers
interactive extraction from crawled HTML. The only hit on relevant
search terms (`scriptTagRe` in `check_tool_health.go`) audits our own
deployed tools — output-side, not input-side. No overlap.

**Filename:** `platform/orchestration/actions/extract_interactive_fingerprint_action.go`

**Action name:** `extract_interactive_fingerprint`. Category
`"analysis"`. IsLocal `true`. Same shape as the design extractor.

**Helper reuse:** `fpResolveURL` and `fpDedup` from the design
extractor are called directly (same package — no need to copy or
re-export).

**Input extraction:** uses `datahelpers.ExtractActionInputs` with an
`ActionInputSpec` (newer convention from `001_development_guide`,
matching what new code should use even though the design extractor
sibling pre-dates this convention).

**Inputs:**
- `crawl_field` (optional, default `"crawl_result"`) — where to find
  the crawl pages in collected_data. Same default as the design
  extractor.

**What it extracts per crawled page:**
- `<canvas>` element count plus per-element id/width/height/parent
- Inline `<script>` block count plus first 8KB of text content per
  block (truncated so the fingerprint doesn't bloat for analytics
  bundles)
- External `<script src="...">` URLs, resolved against the page URL
- Inline event handler attributes (`onclick`, `onsubmit`, `oninput`,
  `onchange`, `onload`) plus element type
- `<form>`, `<input>`, `<select>`, `<textarea>` counts
- Library/API signals via regex on inline script text:
  `requestAnimationFrame`, `getContext('2d')` or `getContext('webgl')`,
  `fetch(`, `WebSocket`, `addEventListener`, jQuery `$(`, Three.js
  `THREE.`, Phaser `Phaser.`, p5 `setup()`+`draw()`, React `React.`,
  Vue `Vue.`
- A `type_hint` heuristic per page: `calculator` (form + inputs + script
  with event listeners), `game_or_animation` (canvas + animation loop),
  `interactive_widget` (script + event handlers, no form), `static`
  (none of the above)

**What it aggregates across pages:**
- Total counts (pages with canvas, pages with inline scripts, pages
  with external scripts)
- Deduplicated list of external_js_urls for the workflow's fetch loop
- Library_signals frequency map
...
```json
{
  ...
  "meta": {
    "pages_analyzed": 12,
    "inline_script_blocks": 23,
    "external_script_count": 4,
    "canvas_pages_count": 3,
    "interactive_pages_count": 7
  },
  "per_page": [
    { "url": "...", "type_hint": "calculator", "canvas_count": 0,
      "inline_script_count": 2, "inline_script_samples": ["..."],
      "external_scripts": ["..."], "event_handler_count": 8,
      "form_count": 1, "input_count": 5,
      "signals": ["addEventListener", "form_with_inputs"] },
    ...
  ],
  "external_js_urls": ["https://gamedesign.uk/js/calculator-common.js"],
  "library_signals": {
...
**Out of scope for C1:**
- External JS fetching (C3)
- LLM brief generation (C5)
- Spec writing to `interactive_reference` / `interactive_intent` (C6)
- Full AST parsing or token analysis beyond regex signals (out of
  scope entirely — too complex for the value)
- Type classification beyond simple heuristics — the LLM downstream
  does the real classification

**Verification:** after C1 lands, dry-run the action against a fresh
collected_data shape (manually or via a one-off workflow step) and
check the fingerprint structure looks sensible. No integration with
adoption workflow yet — that's C2.

### C2 — wire into adoption workflow

Add an `extract_interactive_fingerprint` step to the adoption
orchestrator workflow, parallel to `extract_design_fingerprint`. Both
consume the same crawl_result and run independently. Collect the
output but don't act on it yet. Readopt gamesdesign.co.uk to verify
the extractor runs end-to-end against real source data.

### C3 — external JS fetch loop

Add a workflow step that iterates `external_js_urls` from the
fingerprint and calls `firecrawl_scrape` on each. Same pattern as the
existing CSS fetch loop. Output is `js_scrape_result_<n>` per URL.

### C4 — enrich_fingerprint_with_js (new action)

Parallel to `enrich_fingerprint_with_css`. Reads each fetched JS
file's content from the scrape results, runs the same library-signal
regex bank against the fetched source, merges into the per-page or
aggregate fingerprint. External scripts that reveal libraries
previously invisible (because the inline script just imported them)
get surfaced here.

### C5 — generate_interactive_intent (new LLM step)

LLM reads the enriched fingerprint and produces a semantic brief per
interactive element: what it is, what it does, what would be needed
to recreate it (formula complexity, mechanics, framework
requirements), feasibility marker (`producible_now`, `producible_simpler`,
`blocked`).

### C6 — apply_adoption_plan extended

Write `interactive_reference` (concrete fingerprint) and
`interactive_intent` (LLM brief) to site_specs. Same pattern as the
existing design_reference / design_intent writes.

## Path C — work breakdown (older sequencing kept for context)
