...
**Status:** **✅ SHIPPED AND LIVE 2026-08-25 on chassis `v1.0.1339`** — commits `52958897f` + `fa0b513f1` + the pre-roll acronym fix `35f452a0e`; council `3ed2b792` **APPROVED round 1** (`64d852b4` submitted for the acronym fix). Proven at the artefact with a behavioural demand control, not just a binary probe: a synthetic unsupported claim in live `protocol-tracker` copy is FLAGGED in `hero` and `call-to-action` and SILENT in `protocol-tracker-listing`. **The mechanism works as designed; what remains open is question 1's second half only** (whether the declaration should move from a Go map into a `content_components` column as the list grows). Originally: **BUILT 2026-08-25 — commit `52958897f`, inert until the next chassis roll.** The owner ruled "finish the jobs properly" on 2026-08-25 and the component-grain mechanism is implemented: `thirdPartyDataComponents` keyed on the component's registered function, `ClaimSurface.ComponentFunction` threaded through all five construction points, and the build gate scanning per section where `sections_metadata` allows. All three tracker page types are **out** of `editorialPageTypes` again. Measured: 16 findings, zero on any tracker page (identical suppression to the interim), with hero and CTA scanned again; corpus-level mutation proof returns exactly the original 36/20 when the new map is emptied; and the block-equivalence question in §2 below was answered **before** the code was written (4 pages / 21 components, exact multiset match). **What remains open for the architecture track is question 1's second half only** — whether the declaration should move from a Go map into a `content_components` column once the list grows, and what the bar for a new member is when the author is not also the person measuring. Originally raised 2026-08-24 by the `bugs_open/364` lane, at the explicit request of the
