...
The palette-band mapping uses `var(--color-on-primary, var(--color-background))`.
...
```
If a real token exists under a different spelling, adjust the three `on-primary`
strings in the `paintPaletteBand` case (grep `on-primary` in the file).
