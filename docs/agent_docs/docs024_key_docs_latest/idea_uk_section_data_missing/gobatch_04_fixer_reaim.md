...
## Pre-deploy confirmation (one SELECT)
The palette-band mapping below uses `var(--color-on-primary, var(--color-background))`.
Confirm the on-colour token's actual name in your palettes before building; if it is
e.g. `--color-primary-text`, adjust the three strings in Edit G3's `paintPaletteBand`
case (grep `on-primary` in the NEW code):
```sql
SELECT id, name,
       (css_variables::text LIKE '%on-primary%')     AS has_on_primary,
       (css_variables::text LIKE '%primary-text%')   AS has_primary_text
FROM palettes WHERE is_active = true LIMIT 5;
-- (adjust table/column names per \d palettes if they differ)
```
