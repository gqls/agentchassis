...
**Next, in order:**
1. **W4b — data-only chrome (read staged: `w4b_00_before.sql`).** Code fact (render_site_components_action.go:345–430): `renderAndStoreSiteComponent` joins the PINNED `site_components.component_id` with **no `is_active` filter**, and without `force_rerender` it SKIPS non-empty slots — so the **repoint must precede the force_rerender**, or it re-renders the old dark chrome from the inactive templates. Status: ids in hand; **`w4b_01_repoint.sql` prepared (repoint only, guarded on the old ids; rendered_html untouched so there is no chrome-less window)**; head row stays pinned. The forced-re-render trigger is chosen from `w4b_02_read_triggers.sql` (each agent's render step config + real work-item spec shapes) — crafted against real shapes, not guessed.
