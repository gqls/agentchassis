...
## 2026-07-06 — EXPIRY THREAD CLOSED ON CONTENT; SLICE 2+F DEPLOYED; SLICE 3 DELIVERED
- **w9_06: both pages t/f/f** (index 13:10, tools 13:21 — local illustrations, zero presigned/B2 residue). With Edit F deployed, recurrence prevention is live → **the presigned-expiry thread closes.** Trailing watch only: the tools ITEM sits claimed with retained error while its render is green (the claim-retry class) — expect it to complete; if it lands `failed`, read the error text.
- **Slices 2 + F deployed** (user-confirmed): scheme-aware fallback chrome, the Debug tidy, and the deploy-time localisation are in the running image.
- **Slice 3 delivered — `gobatch_04_fixer_reaim.md`:** Edit G replaces `processComponentCSS`'s is_dark_section branch with a painting classifier + `--section-*` declaration rewriter (pair → pair references; ink → ink; palette/hex band → on-colour family; ambient → declarations removed), keeping the signature (param ignored, call sites unchanged), the strip machinery, and the contrast safety gate. Edit G2 deletes the injector trio (removing a stray logger.Debug at :647). Edit G3 adds the classifier/rewriter (new identifiers listed per the rule). Selection queries confirmed flag-free — untouched. **Pre-deploy: the one-SELECT on-colour token confirmation** (adjust `--color-on-primary` if the palettes spell it differently).
- Remaining spec work = **slice 4**: creator prompt rewrite (locate via the staged agent_definitions query), the 003 contract rewrite, and the flag_page_image_rebuild section-scope extension. Then the tail decision (the re-aimed fixer is its natural vehicle), planned pages, hygiene.

## 2026-07-06 (later) — SLICE 3 APPLIED TO FILE; instruments corrected
- **The patched `fix_forced_text_colours_action.go` is delivered** (692 → 739 lines): Edits G/G2/G3 applied positionally (function start → column-0 brace, no whitespace risk). Structural checks all green: old branch gone, injector trio gone (incl. the :647 Debug), classifier + rewriter in, param kept-ignored, zero logger.Debug, imports still satisfied. Brace balance verified with the CORRECTED instrument (literals/comments stripped: 102/102 → 105/105; the naive count false-failed on regex character classes — owned). No Go toolchain in the container → `go build ./...` in the repo is the compile gate.
- **Pre-deploy confirmation corrected in gobatch_04:** my `palettes.css_variables` guess was wrong (the user's error proved it); the real source per `loadSitePalette` is `style_collections.color_palette` via `sites.style_collection_id`. Zero-guess check added (grep the emitted tokens on a deployed page). Absence of an on-colour token does NOT block the deploy — the fallback chain resolves to `--color-background`.
- User SQL side-answer: `site_specs.aspect` is text → `jsonb_pretty(aspect::jsonb)`.
- Deploy path: drop the patched file into the repo, `go build ./...`, ride the next image. First live run stays the tail decision's vehicle (manual spawn, non-idea.uk site, read `details`).

═══════════════════════════════════════════════════════════════════════════════
# ▶ WHERE WE ARE + WHAT TO RUN NEXT (2026-07-06, later) — supersedes everything above
═══════════════════════════════════════════════════════════════════════════════

## State
1. **Scheme → components (the original P0): CLOSED 2026-07-03.** All nine deployed-grep checks passed; idea.uk resolved light renders light.
2. **Brief-explanation: CLOSED 2026-07-04.** Section + illustration render on index and tools; the two `needs_section_data` escalations self-closed.
3. **Presigned-URL expiry: CLOSED 2026-07-06.** Renders verified t/f/f both pages; the 18-row backfill flipped every asset URL local; **Edit F is deployed**, so future deploys record local URLs — no recurrence class remains. One trailing watch below.
4. **Go batch:** slice 1 (plan_sections Edits A+B) deployed; slice 2 (fallback chrome C/D + Debug tidy E) deployed; Edit F deployed. **Slice 3 (the fixer re-aim) is DELIVERED AS A PATCHED FILE** — `fix_forced_text_colours_action.go` in outputs, structurally verified — awaiting the on-colour check, compile, and a ride on the next image.
5. **Slice 4 input RECEIVED (today):** the full component-creator definition (prompt included) — nothing more to run for it; the spec is my next deliverable on your word.

## YOUR NEXT ACTIONS
```bash
PSQL="kubectl -n ai-persona-system exec -i postgres-clients-0 -- psql -U clients_user -d clients_db"
```
**1. Trailing watch (seconds):** the tools rebuild item that was claimed-with-error while its render was already green:
```bash
$PSQL -c "SELECT item_key, status, attempt_count FROM site_work_items WHERE site_id=(SELECT id FROM sites WHERE domain='idea.uk') AND created_by='w9_localise_rebuild' ORDER BY item_key;"
```
Expect both `complete`. If tools shows `failed`, paste `left(error,240)` and we read it — the renders are already correct either way.

**2. The on-colour token check** (decides the spelling of three strings in the patched fixer; **absence is fine** — the chain degrades to `--color-background`):
```bash
# zero-guess, on a fresh pull of the deployed index.html:
grep -o '\-\-color-[a-z-]*' index.html | sort -u     # look for on-primary / primary-text
```
or DB-side:
```bash
$PSQL -c "SELECT sc.id, sc.name, (sc.color_palette::text LIKE '%on-primary%') AS has_on_primary, (sc.color_palette::text LIKE '%primary-text%') AS has_primary_text FROM style_collections sc JOIN sites s ON s.style_collection_id=sc.id WHERE s.domain='idea.uk';"
```
If a token exists under a different spelling: adjust the three `on-primary` strings in the patched file (`grep -n 'on-primary' fix_forced_text_colours_action.go`). Otherwise ship as-is.

**3. Ship slice 3:** copy `fix_forced_text_colours_action.go` from outputs over the repo's copy, then:
```bash
go build ./...
# ride the next chassis image/rollout — sharing an image with slice 4's code later is fine;
# nothing depends on slice 3 shipping first except the fixer-driven library tail.
```
Do NOT point the improvement loop or a manual spawn at the re-aimed fixer yet — its first live run is deliberately the library-tail decision's vehicle (manual spawn at a NON-idea.uk site, read the `details` JSON).

**4. Say "spec slice 4"** — I then produce, from the definition you pasted: (4a) the creator-prompt rewrite as an `UPDATE agent_definitions` file (contract items 5/6/7 re-written to the paired-variable standard: paint from pairs/surface/on-colour, declare `--section-*` only as references, `is_dark_section` demoted to metadata, image fields optional-with-gate); (4b) the 003 contract replacement text; (4c) the `flag_page_image_rebuild` section-scope extension as an anchored Go edit (file already on disk).

## Then — decisions, not scripts
- **Library tail** (~10 surface-painting declarers + ~17 band-class, non-idea.uk): hand-needles vs the re-aimed fixer — decide after slice 3's first supervised run.
- **Planned pages**: build or unlink news-index / guides-index / tool-audience-check.
- **Hygiene list**: `site_work_items.updated_at` frozen; legacy `active`/`system` statuses; "Rerender:" commit message on full builds; claim-timeout retries; the legacy pinned head component; scope_ref-by-ordinal drift; site-wide hero = hero-about's image everywhere (per-page heroes need un-shadowing — later); what ran 07-01; orphan `illustration-specimen-report.jpg`; component-creator definition `updated_at` bumped today 13:22 (most economically the deploy re-registering definitions — verify only if it recurs unexplained).

## 2026-07-06 (evening) — actions 1+2 CLOSED; the patched fixer finalised
- **Action 1 (trailing watch): CLOSED** — both w9 rebuild items `complete` (tools on one retry).
- **Action 2 (on-colour): RESOLVED** — the deployed page defines nothing itself (definitions live in `/assets/css/styles.css` — page-side grep sees only consumption; instrument note); consumption is decisive: `--color-primary-text` ×18, `on-primary` ×0, and the creator contract's item 7 names `--color-primary-text`. **The patched fixer's three strings switched to `var(--color-primary-text, var(--color-background))`** (verified: 0 on-primary remain). One-line ship check added: grep `styles.css` for a `--color-primary-text:` DEFINITION; if absent → slice-4 remit (the contract promises a token the stylesheet may omit), not a blocker.
- Token-list residue noted for hygiene: `--color-hero-title`/`--color-hero-subtitle` consumed ×1 each (legacy base-head rules).
- **Remaining actions: (3) ship the patched fixer** (copy over repo file → `go build ./...` → next image; no early fixer runs) **and (4) say "spec slice 4"** (creator prompt + 003 + flag extension — all inputs in hand).

## 2026-07-06 (late) — SLICE 4 DELIVERED (4a prompt / 4b 003 / 4c flag); slice 3 shipping
- **Inputs closed by the :root paste:** `--color-primary-text: #ffffff` IS defined → the fixer chain resolves white-on-primary; ship-check answered before running. Extras noted (surface-alt, hairline, code-bg, callout pair) → folded into 4a's item-7 list. The paste is the fallback layer ("palette vars override") → the CTA pair's absence there concludes nothing.
- **4a (`slice4a_creator_prompt.sql` + rollback):** four needle replaces inside the prompt — item 5's dark line → the consumer chain; item 6's literal block → SECTION PAINTING (a/b/c/d, references only, is_dark_section demoted to catalogue metadata in-text); item 7 gains the pair + the real extended vocabulary; Tier C gains the IMAGE FIELDS RULE (conditional DESCRIBED, not shown — the prompt is itself Go-template-rendered; literal if-syntax would execute and vanish). Gate booleans + guarded idempotent UPDATE + RETURNING; backup command in-file.
- **4b (`slice4b_003_contract.md`):** item 6 → the painting contract + new 6b image-fields rule; the literal example (~490–496) → a pair-referenced example. Consumer-side narrative untouched.
- **4c (`gobatch_05_flag_section_scope.md`):** anchored edit at :105–110 — section scope_refs prefix-split to the page and fall through to the existing emit path; `strings` import check noted; cosmetic header/step-description follow-ups optional.
- **Filename caution:** the deploy note said `fix_forced_text_colors_action.go` (US) — the delivered patch is `..._colours_...`; confirm the patched content landed in the repo's actual single filename.
- **Run order:** backup → `$PSQL < slice4a_creator_prompt.sql` (gate t/t/t/t/f → UPDATE 1 t/t/t/f); apply 4b to the repo doc; apply 4c + build (rides slice 3's image or the next). Then the queue: supervised first fixer run = the tail decision; planned pages; hygiene.

═══════════════════════════════════════════════════════════════════════════════
# ▶ WHERE WE ARE + WHAT TO RUN NEXT (2026-07-06, night) — supersedes everything above
═══════════════════════════════════════════════════════════════════════════════

## State
- **Threads closed:** the P0 (07-03), brief-explanation (07-04), presigned expiry (07-06).
- **Deployed:** slices 1 (plan_sections A+B), 2 (fallback chrome C/D + Debug tidy E), Edit F (deploy-time localisation). **Slice 3 (the re-aimed fixer): deploying per your note** — confirm it built.
- **Slice 4a: LANDED** (gate t/t/t/t/f → UPDATE 1 t/t/t/f) — the creator prompt now teaches the painting rules, the image-fields rule, the extended vocabulary; `is_dark_section` demoted to metadata in-text.
- **Slice 4b: DELIVERED as a patched doc** — `003_contracts_and_standards_7_.md` in outputs (item 6 → painting contract; new 6b image-fields; both literal examples → reference form; heading/lead/narrative de-dark-keyed; zero producer-side dark framing remains).
- **Slice 4c: PENDING APPLY** — `gobatch_05_flag_section_scope.md` now carries Edit H (the scope mapping) AND Edit H2 (the header comment); `slice4c_step_description.sql` staged for after the deploy.
- **Named check (evidence so far benign):** deploys bump `agent_definitions.updated_at` without overwriting `default_config` (the old prompt survived today's deploy). Residual risk = a future re-SEED from repo config reverting 4a — Step C below.

## THE STEPS
```bash
PSQL="kubectl -n ai-persona-system exec -i postgres-clients-0 -- psql -U clients_user -d clients_db"
```
**STEP A — repo changes (one commit, one image):**
1. Copy `outputs/003_contracts_and_standards_7_.md` over the repo's 003 doc.
2. Apply `gobatch_05_flag_section_scope.md` — Edit H (:105–110) and Edit H2 (:17–24) to `flag_page_image_rebuild_action.go`; add `strings` to imports if absent.
3. Confirm the patched fixer landed in the repo's ONE real filename (your note said `colors`, the delivered file says `colours` — no parallel files).
4. `go build ./...`
5. Image + rollout (carries the fixer too if slice 3 has not shipped yet).

**STEP B — after the deploy:**
```bash
$PSQL < slice4c_step_description.sql    # expect UPDATE 1; RETURNING begins 'Imagery landed:'
```

**STEP C — re-seed risk (reduced):** the user confirms component-creator is a dynamically spawned/registered agent, not a YAML-seeded one, and today's deploy bumped `updated_at` without overwriting `default_config` (4a's edit survived it). So the DB row is authoritative and no repo seed file will revert it. The only residual: if agent-definition registration ever runs an upsert that rewrites `default_config` from an in-code template, 4a would revert. One grep in the chassis repo settles it — search for a literal fragment of the OLD prompt in Go sources:
```bash
grep -rn "DARK SECTIONS (if the section has a dark background)" --include='*.go' .
```
A hit = an in-code prompt template exists; mirror 4a's four changes there too. No hit = nothing in code can revert the DB row; done.

**STEP D — the supervised fixer first-run (the library-tail vehicle):** manual spawn per 016b's harness at a **non-idea.uk** site; read the returned `details` JSON (declarations rewritten per class, colours stripped, skips). On that evidence: hand-needles vs the fixer for the ~10 surface-painting declarers + ~17 band-class components.

**STEP E — decisions:** planned pages (build or unlink news-index / guides-index / tool-audience-check); the hygiene list (unchanged from the 07-06 morning section).

═══════════════════════════════════════════════════════════════════════════════
# ▶ STEP D + PLANNED PAGES — written out clearly (2026-07-06, night+)
═══════════════════════════════════════════════════════════════════════════════
This section supersedes any earlier position notes and is written as prose with the
commands separated, per request.

## First, one read that sizes both jobs
Both the fixer trial and the three page builds depend on the current shape of two
sites, so run this before either. It is read-only and schema-first (it `\d`s pages
before selecting):

```bash
PSQL="kubectl -n ai-persona-system exec -i postgres-clients-0 -- psql -U clients_user -d clients_db"
$PSQL < stepD_and_pages_reads.sql
```

Paste the four blocks back. Block A shows what dartsonline's fresh build produced and
which of its components carry literal `--section-*` or hardcoded text colours — that is
the fixer's "before" picture. Block B shows whether idea.uk's planned pages
(news-index, guides-index, tool-audience-check) already have stub `pages` rows or must
be created from nothing. Block C shows how idea.uk's already-deployed tool page is
shaped, which is the template the audience-check build copies.

## STEP D — the supervised first run of the re-aimed fixer
The point of Step D is to watch the re-aimed `fix_forced_text_colours` act on a real
site once, under supervision, before it is ever allowed near the improvement loop or a
second site. dartsonline is the right specimen: freshly built from the submit-domain
trigger, not idea.uk, and disposable — you can wipe and resubmit it if a run makes a
mess. Its `site_id` is `5fe8785b-223d-41a3-88ee-c07187622381`.

The sequence is: confirm the deployed pod carries the re-aimed fixer; capture the
before-state (Block A of the read above); spawn the fixer against dartsonline through
the same manual harness described in 016b; read the returned `details` JSON rather than
trusting the render; then re-read the components to see what changed. The `details`
payload is the real output to judge — it reports, per component, which `--section-*`
declarations were rewritten to references (and to which class: pair, ink, palette-band,
ambient), how many literal text colours were stripped, and which components were skipped
by the contrast gate. A correct run rewrites the literal declarers to references, strips
hardcoded text hex on text elements, and leaves already-referencing components and
genuinely ambient sections untouched.

For the manual spawn, use the harness in 016b (the debugging guide) exactly as written
there — it constructs the needs-item and the kcat envelope with the correct headers and
responses_topic. dartsonline's trigger, if you prefer to drive it from a work item, is a
`needs_*` row whose handler is the fixer's agent type; the read above plus 016b give the
exact item shape. After the run:

```bash
# find the fixer pod that handled it:
kubectl -n ai-persona-system get pods --sort-by=.metadata.creationTimestamp | tail -5
kubectl -n ai-persona-system logs <fixer-pod> --tail=120

# then re-read dartsonline's components (same query as Block A) and diff by eye against the before:
$PSQL < stepD_and_pages_reads.sql
```

The decision that comes out of Step D is the library-tail choice: if the `details`
show clean, correct rewrites on dartsonline's varied components, the re-aimed fixer is
the vehicle for the ~10 remaining surface-painting declarers and ~17 band-class
components elsewhere; if the `details` show misclassifications, the classifier's
regexes get tuned against the cases that missed before any broad run. Either way, do not
point the improvement loop at the fixer until this single supervised run is judged.

If a run does make a mess of dartsonline, it is disposable: delete its rows and
resubmit from the trigger. Hold off on the exact delete statements until we write them
together against the schema — a site spans several tables (sites, pages,
page_components, site_specs, site_work_items, assets, site_plan_imagery) and the clean
way is one guarded cascade, not ad-hoc deletes.

## PLANNED PAGES — build news-index, guides-index, and the audience-check tool
These three are referenced by navigation but were never built, so their nav links 404.
Building them is additive and carries no regression risk to the eight deployed pages.
The right mechanism depends on Block B of the read: if idea.uk's planned pages already
exist as `pages` rows in a non-deployed state, the build is an emit of `needs_page`
items for them (the same shape W6 used) and the handler renders and deploys each; if the
rows do not exist, they must be created first — and the structural way to create them is
through the planner path that owns page creation, not a hand-INSERT into `pages`, so
that the nav, the plan, and the page row stay consistent.

news-index and guides-index are content-index pages: they list news articles and guides
respectively. If the site has a blog/news listing mechanism (the rerender workflow
referenced `rebuild_blog_listing`), these may be index components fed by a query rather
than hand-authored pages — Block B's navigation/plan data will show whether they are
meant to be query-backed index pages or static ones. The audience-check page is a tool
page: Block C shows idea.uk already deployed `/tools/audience-check`, so there is an
existing tool page and component to copy the shape from; the build is most likely a
`needs_page` (or the planner's tool-page path) that points at that tool component.

Because the correct build path forks on what Block B and C return, the concrete
INSERT/emit files come next turn, written against the actual rows. What is safe to state
now is the shape: additive, one work-item emit per page through the handler that already
builds pages, reusing the tool component that already exists for audience-check, and
routing news/guides through the listing mechanism if that is what the plan intends.

## STEP C reminder (re-seed risk) — now a single grep
Per the reduced Step C above: one `grep -rn "DARK SECTIONS (if the section has a dark
background)" --include='*.go' .` in the chassis repo. A hit means an in-code prompt
template exists and must mirror 4a; no hit means the DB row cannot be reverted by a
deploy.
