...
**▶ POSITION (2026-07-03, W3c read):** Fixed set: footer, CTA, hero. Variants RECLASSIFIED (hardcoded legacy-hex gradient, not paint-nothing; guide amended with the gradient-hex regex gap). **W6 gate list concrete: hero-about + hero-contact (→W3d, one update for all five variants) then brief-explanation + about-content (→W3e dumps staged), plus a literal-scan of idea.uk's six unflagged functions.** After W3e: repoint site_components + force_rerender (data-only) → W6 rebuild + deployed grep — the Go code batch (fallbacks/prompt/fixer/003) is off idea.uk's critical path and ships as one deploy later. Awaiting: w3d_00/01/02 + w3e_00 output. Open items: what ran 07-01; planned-but-unbuilt pages in nav; hand-vs-fixer decision for the non-idea.uk hazard tail.
