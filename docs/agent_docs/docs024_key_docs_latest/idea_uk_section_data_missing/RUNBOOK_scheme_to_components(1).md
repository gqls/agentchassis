...
```sql
-- E2a: what spec rows does idea.uk's site plan hold, and how big? (Confirm column
--      names from \d site_specs in the bundle; spec_data may be large.)
SELECT spec_type, length(spec_data::text) AS bytes, created_at
FROM site_specs
WHERE site_id = '1244516d-014d-421c-88c6-090bb1e9552a'
ORDER BY created_at DESC;
-- E2b: then extract ONLY the sections array of the plan spec (fill in <plan_type>
--      from E2a) and look for any per-section style / contrast / scheme hint:
--   SELECT jsonb_pretty(spec_data->'sections')
--   FROM site_specs
--   WHERE site_id = '1244516d-014d-421c-88c6-090bb1e9552a' AND spec_type = '<plan_type>'
