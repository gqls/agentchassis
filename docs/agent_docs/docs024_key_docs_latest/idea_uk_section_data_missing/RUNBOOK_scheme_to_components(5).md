...
**▶ CURRENT POSITION (2026-07-02):** Bundles 1–3 read; Checks 1 and 2 run on the cluster (results below). The scheme→variable pipeline is verified correct end-to-end (core palette + chrome bg/text vars all flip with scheme; renderer section defaults key on palette luminance). The active `site-header` is already variable-driven (no fix needed); the active `site-footer` is half-migrated (light bg var + self-declared white text = unreadable on light — a live library bug); hero/CTA paint dark bands from palette vars with white-text assumptions (whether that is a bug or a design choice is the open question); blast radius 37/84 self-declarers + 15 hex backgrounds. **The earlier "Q4 settled = self-declare nothing" position is superseded** — stripping self-declarations from self-painted dark bands would break them on light sites unless renderer or layout supplies the context, so the decision is now between four architectures (see `HANDOFF_scheme_to_components_for_claude_code.md` §Decision). **Next: (1) the user answers the gating design question — may a light site carry dark hero/CTA bands, or must light mean fully light; (2) run CHECK 3 (staleness/provenance/self-declarer split) before concluding why deployed idea.uk is dark; (3) the invariant no-regret fixes (footer text, fallback chrome, creator prompt) are safe to implement under any architecture.**
