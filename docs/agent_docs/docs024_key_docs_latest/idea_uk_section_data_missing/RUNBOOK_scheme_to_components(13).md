...
**▶ POSITION (2026-07-03):** W1, W2a, W2b, W3a, W1b, **W3b (hero → ink model)** all COMPLETE — fixed-template set is now footer + CTA + hero, all inert until re-render. Gate false alarm on W3b owned (my expected counts were mis-derived; guide's needle-gate rule refined). **Next read: `w3c_00_before.sql`** — the five hero-* variant templates + **idea.uk's per-page component inventory, which GATES W6** (any hazard/unconverted function on idea.uk's pages must be fixed pre-rebuild). Then: variant fixes, the hand-vs-fixer decision for the remaining ~12 surface-painters (W5), W4 chrome path, W6 rebuild + deployed grep. Open items: what ran 07-01; planned-but-unbuilt pages in nav.
