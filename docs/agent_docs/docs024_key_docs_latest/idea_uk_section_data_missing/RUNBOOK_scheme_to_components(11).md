...
**▶ POSITION (2026-07-03, calls received):** W1, W2a, W2b COMPLETE. **Calls: hero = (c) single-hue color-mix gradient; batch = YES.** w3_00 gate passed (10 needles, 4/4); **imageless heroes are the COMMON case (80/114 + all 26 hero-* renders)** — (c) design recorded (§HERO (c) DESIGN: per-branch --hero-ink; layered solid+gradient background mixing 15% toward the ink; inverse button). **W3a (CTA conversion) + W1b (five-layout batch) prepared — awaiting execution + paste-back** (§W3a + W1b EXECUTION). Then: W3b hero files, then the hero-* variant hazard batch (26 live imageless renders). Debugging guide updated → `016b_debugging_guide_7.md` (v4: two-assembly-paths pattern, SQL pitfalls, sites.status). Open items: what ran 07-01; planned-but-unbuilt pages in nav.
