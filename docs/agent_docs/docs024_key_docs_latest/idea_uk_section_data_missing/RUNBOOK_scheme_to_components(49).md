...
**STEP C — the seed check (five minutes):** in the chassis repo:
```bash
grep -rl "component-creator" configs/ deployments/ --include='*.y*ml' --include='*.json' 2>/dev/null
```
If agent definitions are seeded from files, mirror 4a's prompt change and the step description there so a future deploy cannot revert them. If nothing seeds them, note that and move on.
