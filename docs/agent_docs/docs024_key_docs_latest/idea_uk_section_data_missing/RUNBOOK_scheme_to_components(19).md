...
**Next, in order:**
1. **W4b — data-only chrome (read staged: `w4b_00_before.sql`).** Code fact (render_site_components_action.go:345–430): `renderAndStoreSiteComponent` joins the PINNED `site_components.component_id` with **no `is_active` filter**, and without `force_rerender` it SKIPS non-empty slots — so the **repoint must precede the force_rerender**, or it re-renders the old dark chrome from the inactive templates. Status: **REPOINT DONE**; workflow fully digested (spec.function/component_id unused; gate exact; side-effects = forced chrome render + JS bundle + full-site page rerender + deploy + mark deployed). **`w4b_04_trigger_item.sql` prepared** (schema-first, check-first dedup, truthful provenance) + `w4b_05_verify_chrome.sql` (item lifecycle, stored-chrome booleans, per-page drain). After verification: the deployed intermediate = light chrome over old sections; then W6.
