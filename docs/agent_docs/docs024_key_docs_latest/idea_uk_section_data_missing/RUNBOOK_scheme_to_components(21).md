...
**Next, in order:**
1. **W4b — data-only chrome (read staged: `w4b_00_before.sql`).** Code fact (render_site_components_action.go:345–430): `renderAndStoreSiteComponent` joins the PINNED `site_components.component_id` with **no `is_active` filter**, and without `force_rerender` it SKIPS non-empty slots — so the **repoint must precede the force_rerender**, or it re-renders the old dark chrome from the inactive templates. Status: **W6 verified DB-side** — fixed templates confirmed in the rebuilt stored sections (fossil + old-white gone everywhere); retry failures were claim timeouts (dispatch infra). **One discrepancy gates the symptom-close: the rebuilt index lost its brief-explanation section** (tools to check) — `w6_04_check_dropped_section.sql` distinguishes never-planned vs resolution-failure vs the 016 drop hazard. Run w6_04 + the deployed grep (now incl. the brief-explanation data-component count); the close call is made on both together. Then: the Go code batch, the tail decision, the planned-pages choice, the hygiene list.
