...
═══════════════════════════════════════════════════════════════════════
## CHECKPOINT (Tt) — applied code verified correct; build path exonerated; forward probe staged
═══════════════════════════════════════════════════════════════════════
- **Witness read (uploaded plan_sections_action.go):** Edit B = the spec exactly (full loop, key+kind-alias, defer Close, Warn paths); Edit A at :1188; 0× logger.Debug (their swap was here). Loop-variant DEAD. content_data fallback = gap-fill, hero/logo only → clobber DEAD. pageName: one caller (:617), uniform for both pages; tools works ⇒ plumbing fine. **My "hero resolves ⇒ pageName correct" owned as unsound** (fallback can supply hero) — replaced by the uniformity argument.
- Provenance (w8_05 wide): 17:26 manual-recovery needs_rerender (brief-explanation-scoped) → 17:27:58 TWO page_rerender items (third item_type; handler page-rerender) → index 17:30 / tools 18:12 renders sit in the OVERLAP (build-duration story AND rerender story both fit) → not concluded; forward test supersedes forensics. manual-recovery's nature = optional context from the user.
- **w8_07 staged**: \d page_components + one clean index build + has_image AND stored content_data illustration_url — three-way interpretation (close / template-side / resolver-side→temp log).
idea.uk live VM untouched.

═══════════════════════════════════════════════════════════════════════
## CHECKPOINT (Tu) — probe blindness owned; index resolved all along; presigned-URL expiry found
═══════════════════════════════════════════════════════════════════════
- **Owned:** has_image probed the asset-KEY string; objects are UUID-named → blind. Stored illustration_url PRESENT on index's 17:30 render AND the clean 13:02:10 probe build (same presigned URL, signed 13:25:06 07-03 = the storage moment; assets.url literally holds the presigned string). **Edit B likely fine on index throughout; the two-day "miss" = probe artefact.** Tools' blind-pattern match unexplained → 8.1 image regions.
- **NEW STRUCTURAL:** presigned URL, Expires=604800 → the rendered image DIES 2026-07-10. Heroes deploy to local /assets/images/ (a localisation step section illustrations don't get). 8.2 compares url forms; batch list gains: localise section illustrations at image-deploy (or store permanent URLs) — the real remaining W7c work.
- Probe build was FAST (~10 min) vs yesterday's 82/124-min tails — pod health variance noted.
- w8_08 staged (corrected booleans + regions + url forms). idea.uk live VM untouched.

═══════════════════════════════════════════════════════════════════════
## CHECKPOINT (Tv) — BRIEF-EXPLANATION CLOSED; hero-exposure check staged
═══════════════════════════════════════════════════════════════════════
- **CLOSED on content:** corrected probe t/t both pages — illustration renders on index AND tools; the saga = my blind key-string probe (owned in runbook); Edit B fine throughout. Escalations self-closed earlier. Loose end (tools' blind match) moot.
- duration column: integer, NULL here; expiry lives only in the URL string; cast pattern noted (`created_at + make_interval(secs => duration)`).
- **Successor thread OPEN: presigned-URL expiry/localisation.** 8.2 shows hero_home's assets.url in the SAME B2 form (06-21 → expired ~06-28 if presigned) and W6 re-resolved heroes via a.url → current hero renders may hold dead URLs. w8_09 staged (renders local/B2/presigned + stored hero URLs). Fix direction (batch): localise at image-deploy into /assets/images/ + resolver hands local path (as the historically-deployed heroes suggest), or permanent URLs. Decide on 9.x evidence.
- Then: back to the main line — apply slice 2, spec slices 3–4, the tail, planned pages, hygiene.
idea.uk live VM untouched.

═══════════════════════════════════════════════════════════════════════
## CHECKPOINT (Tw) — heroes SAFE (local paths render); exposure = 2 illustrations, dies 07-10
═══════════════════════════════════════════════════════════════════════
- 9.2: all five hero assets.url presigned, signed 06-21 → expired ~06-28. 9.1: every current main-hero render LOCAL (/assets/images/), incl. today's fresh build → dormant dead URLs, nothing renders them → **heroes-broken concern closed benign**. Variant heroes f/f/f = by design.
- **Contrast = the mechanism clue:** same build renders hero local + illustration B2 → hero_url likely sources content_data.hero_url (local, written at 06-21 image deploys, gap-filled) NOT site_assets.hero. w9_00 staged: 0.1 hero_url declared source; 0.2 image-build-handler deploy-step config; + repo ls/git log for whether 07-03 illustration files already landed (→ path-handoff-only fix).
- Exposure: illustration_home/_tools URLs die 2026-07-10 → localisation batch item priority THIS WEEK. Fix shape post-read: deploy records local paths for all kinds where the resolver reads them, or Edit B maps a localised column.
- Main line still queued behind this read: apply slice 2; spec slices 3–4; tail; planned pages; hygiene.
idea.uk live VM untouched.

═══════════════════════════════════════════════════════════════════════
## CHECKPOINT (Tx) — localisation mechanism named; fix = assets.url→local at deploy
═══════════════════════════════════════════════════════════════════════
- Mechanism: ContentData-priority merge (component_library:736–739) + legacy sites.content_data.hero_url written at the 06-21 image deploys (plan_sections:155 comment = witness). Heroes never touch plan_sections for their image. **Side-obs flagged:** per-page hero assets possibly unconsumed (hero schema has no site_assets.hero field) → 1.3 library check; if empty, they expired unused (site-wide hero serves all pages).
- **Fix settled:** deploy writes repo file for section kinds + sets assets.url to the LOCAL path (B2 ref → separate column if wanted); Edit B untouched; two existing rows = guarded flip + one rebuild post-files. Sizing: repo ls/git log (owed ×3) + w9_01 (step names, deploy-step config, hero-consumer check).
- Deadline unchanged: current illustration URLs die 2026-07-10.
- Queue behind this: apply slice 2; spec slice 3 (fixer re-aim) + slice 4 (creator prompt + 003, incl. image-fields-optional + the flag scope extension); tail; planned pages; hygiene.
idea.uk live VM untouched.

═══════════════════════════════════════════════════════════════════════
## CHECKPOINT (Ty) — deploy = asset-deployer sub-agent; hero shadow hypothesis; stakes widened
═══════════════════════════════════════════════════════════════════════
- 25 steps read: imagery lane = check_item_type_imagery → spawn/call_imagery_gen → store_imagery_asset → brand check → spawn/call_asset_deployer → flag_rebuild → complete. Deploy behaviour lives in the ASSET-DEPLOYER agent (my deploy_asset anchor missed — owned). w9_02 reads it (type-name listing first).
- **1.3: hero consumes site_assets.hero** → side-observation overturned; field name ≠ hero_url → hypothesis `background_image`, presigned-filled, SHADOWED by legacy content_data.hero_url via `or` + ContentData priority. 2.2 names it; 2.3 checks both keys + presigned in index's stored hero data. **Consequence: legacy-less sites render presigned heroes → expiry bites heroes too → deployer fix scope = ALL kinds.**
- Repo ls/git log still the sizing fact (4th mention, gently). Deadline 2026-07-10 unchanged.
idea.uk live VM untouched.

═══════════════════════════════════════════════════════════════════════
## CHECKPOINT (Tz) — deployer commits to git; shadow corrected to site-level or-order; purpose-naming risk
═══════════════════════════════════════════════════════════════════════
- Deployer: deploy_asset = download S3 → optimise by purpose → COMMIT TO GIT (files exist in repo) → the fix = URL handoff only (deploy_asset writes local path to assets.url post-commit; S3 ref preserved per \d) + backfill flip + one rebuild.
- **Shadow corrected (2.3 disproved section-level form):** hero_url lives at SITE level (sites.content_data — :278 witness), merged beneath section data; template `or` picks it over the presigned background_image. Legacy-less sites → presigned heroes render → expiry bites; field's use_fallback covers only the asset-missing case.
- **Purpose-naming collision risk:** legacy `hero.jpg` singular + "optimize by purpose" → committed names may be purpose-based → the two illustrations may have overwritten each other. Repo ls/git log = the decisive fact (filenames + collision) — asked plainly, 5th time.
- w9_03 staged (\d assets + presigned inventory). After the ls: write the deploy_asset change spec + the backfill flip + rebuild. Deadline 2026-07-10.
idea.uk live VM untouched.

═══════════════════════════════════════════════════════════════════════
## CHECKPOINT (Ua) — localisation fix written; deadline met by data-side alone
═══════════════════════════════════════════════════════════════════════
- Repo listing: key-named files (`_`→`-`, `.jpg`), both illustrations committed 07-03 → collision CLOSED benign; convention mechanical. Orphan specimen file + missing hero.jpg/last-write hero_url wrinkle banked (per-page heroes = later improvement; hero flips render-neutral under the shadow).
- \d assets: storage_path/provider/filename columns → S3 object path preserved in the flip; updated_at trigger present. All 18 rows presigned (16 dormant-expired, 2 live).
- **Written:** w9_04 (guarded 18-row flip, idempotent, preserve-then-localise, verify 0/18) + w9_05 (index+tools rebuild + local booleans) — **07-10 deadline met data-side**; gobatch_03 (deploy_asset records local url post-commit; SQL contract given; exact anchoring on file upload). Rides the batch image with slice 2.
- Queue: run w9_04/05 → apply slice 2 + gobatch_03 → spec slices 3 (fixer re-aim) + 4 (creator prompt + 003 + flag scope extension) → tail → planned pages → hygiene.
idea.uk live VM untouched.

═══════════════════════════════════════════════════════════════════════
## CHECKPOINT (Ub) — backfill 18/0/18; Edit F anchored; hero_url mechanism closed
═══════════════════════════════════════════════════════════════════════
- w9_04 landed exactly (18 flipped, 0 presigned left, S3 paths preserved) → deadline defused data-side; w9_05 pair draining (pasted verify = pre-drain snapshot, expected f/t/t; re-run after).
- **Edit F written against real bytes** (gobatch_03 updated): post-commit UPDATE assets (preserve-then-localise), best-effort with Warn/Info, four new scoped locals noted (assetIDStr/assetUUID/parseErr/execErr), skip when asset_id absent (extend by site+asset_key if real runs log the skip). Naming convention confirmed in code (:185–194, Phase 2E).
- **hero_url archaeology CLOSED from the file's comment:** purpose+"_url" → CollectedData → input_mapping → BuildRenderContext; last-write-wins per purpose → site-wide hero currently = hero-about's image everywhere. Banked (per-page heroes later; or-order/shadow unchanged).
- Next: w9_05 verify post-drain → apply Edits C/D/E (slice 2) + F (slice 3a) in one image → spec slices 3 (fixer re-aim) + 4 (creator prompt + 003 + flag scope) → tail → planned pages → hygiene.
idea.uk live VM untouched.

═══════════════════════════════════════════════════════════════════════
## CHECKPOINT (Uc) — consolidated WHERE-WE-ARE + next-actions written to the runbook
═══════════════════════════════════════════════════════════════════════
- Runbook gained the superseding section (three threads' state; numbered user actions with exact scripts; my spec queue; the decisions/hygiene tail). `w9_06_verify_only.sql` created so the drain-watch doesn't re-run the insert file.
- User's path: (1) w9_06 until t/f/f both pages (+ optional deployed grep); (2) apply Edits C/D/E/F, go build, one deploy; (3) say the word → I spec slices 3 and 4 (creator-prompt locate query staged in the runbook).
idea.uk live VM untouched.

═══════════════════════════════════════════════════════════════════════
## CHECKPOINT (Ud) — expiry thread CLOSED; slice 2+F live; slice 3 (fixer re-aim) delivered
═══════════════════════════════════════════════════════════════════════
- w9_06: t/f/f both pages (07-06 renders) → localisation verified on content; Edit F deployed → recurrence prevention live → THREAD CLOSED. Tools item claimed-with-error while render green = claim-retry class, trailing watch only.
- Slice 3 written against real bytes (gobatch_04): Edit G whole-function re-aim (classifier + declaration rewriter; isDark param kept-ignored; contrast gate retained for the strip; contractAdded repurposed as rewrite counter — name kept, meaning noted); Edit G2 deletes ensureSectionContractInHTML/injectSectionContract/sectionContractRe (+ the :647 Debug); Edit G3 new classifier/rewriter code (new identifiers listed). Queries confirmed flag-free → untouched. Pre-deploy on-colour token SELECT included (adjust three strings if palettes use primary-text spelling).
- 3b coupling noted (fix_hardcoded's hex→primary stays coherent; ordering unchanged). First live run = the tail decision's vehicle (manual spawn on a non-idea.uk site first).
- Next: slice 4 (creator prompt via the staged locate query + 003 rewrite + flag scope extension).
idea.uk live VM untouched.

═══════════════════════════════════════════════════════════════════════
## CHECKPOINT (Ue) — slice 3 patched into the file; two instruments corrected
═══════════════════════════════════════════════════════════════════════
- Patched file delivered (739L): G positional replace; G2 trio deleted (cut_func walks back doc comments; the removed injectSectionContract carried the :647 Debug — asserted); G3 inserted at the old sectionContractRe site. Checks green; brace instrument corrected (strip literals/comments first: 102/102 → 105/105 — naive count false-fails on `[{;\s]`-class regexes, owned).
- gobatch_04's confirmation section corrected: real palette source = style_collections.color_palette (from loadSitePalette's own query); zero-guess emitted-token grep added; absence = graceful fallback to --color-background, non-blocking.
- aspect::jsonb cast answered (site_specs.aspect is text). The 5fe8785b site poke noted without speculation.
- Remaining: user compiles + ships the patched fixer with the next image; slice 4 spec (creator prompt locate → rewrite; 003 contract; flag scope extension); then tail/planned-pages/hygiene.
idea.uk live VM untouched.

═══════════════════════════════════════════════════════════════════════
## CHECKPOINT (Uf) — consolidated 07-06 position written; creator prompt received
═══════════════════════════════════════════════════════════════════════
- Runbook gained the superseding WHERE-WE-ARE (07-06): three threads closed + batch state (slices 1/2/F deployed; 3 = patched file awaiting on-colour check→compile→image; 4 = input received, spec on request). Numbered actions: trailing tools-item watch; on-colour token check (grep or style_collections SQL — absence fine); ship slice 3 (no early fixer runs — first run = tail vehicle); say "spec slice 4".
- Creator definition digested for 4a: contract item 6 = the literal dark block to replace; item 7 lacks pair/on-colour tokens; is_dark_section in output to demote to metadata; Tier C image fields to gain optional-with-gate. Definition updated_at bumped today 13:22 — economical read: the deploy's re-registration; hygiene-listed, not chased.
idea.uk live VM untouched.

═══════════════════════════════════════════════════════════════════════
## CHECKPOINT (Ug) — on-colour resolved from consumption evidence; fixer finalised
═══════════════════════════════════════════════════════════════════════
- Trailing watch closed (both complete; tools retried ×1). On-colour: page defines NOTHING (styles.css holds definitions — instrument limitation owned in runbook); consumption 18×primary-text / 0×on-primary + contract item 7 → three strings switched to primary-text spelling in the patched file (0 on-primary remain, re-verified). Ship check added (styles.css definition grep → slice-4 remit if absent). hero-title/subtitle ×1 residue → hygiene.
- User's remaining actions: ship slice 3; say "spec slice 4".
idea.uk live VM untouched.

═══════════════════════════════════════════════════════════════════════
## CHECKPOINT (Uh) — slice 4 delivered in three parts; :root closed the last input
═══════════════════════════════════════════════════════════════════════
- :root paste: primary-text DEFINED (#ffffff) → fixer chain correct; extras → item-7 extension; fallback-layer comment → no CTA-absence conclusion. Filename caution raised (colors vs colours — one repo file).
- 4a: four-needle prompt UPDATE (+rollback, +backup cmd) — painting rules replace the literal dark block; metadata demotion in-text; image-fields rule with the templating hazard handled (described, not shown). 4b: 003 item 6 + 6b + example swap (needle discipline note for the doc). 4c: flag prefix-split at :105 (strings import check; cosmetic follow-ups listed).
- Slice 3 deploying per the user. Next after 4a/b/c land: supervised fixer first-run (tail vehicle) → tail decision → planned pages → hygiene.
idea.uk live VM untouched.

═══════════════════════════════════════════════════════════════════════
## CHECKPOINT (Ui) — 4a landed; 003 patched end-to-end; cosmetics written; steps A–E labelled
═══════════════════════════════════════════════════════════════════════
- 4a evidence: gate t/t/t/t/f → UPDATE 1 t/t/t/f. Deploy-vs-definitions: old prompt survived today's deploy → re-registration bumps updated_at only; re-seed risk = Step C.
- 003 patched in FOUR passes (item 6 + 6b; example 1 → pair references; the second literal block found by the doc-wide check → palette-band reference form + class rename; heading/lead/narrative sweep — one historical mention neutralised, zero producer dark-keying remains). Delivered to outputs.
- Cosmetics written: Edit H2 (full header paragraph, exact bytes) appended to gobatch_05; slice4c_step_description.sql (jsonb_set path, guarded, run post-deploy).
- Runbook: superseding night section with STEPS A–E (repo commit incl. filename caution → post-deploy SQL → seed check → supervised fixer run → decisions).
idea.uk live VM untouched.

═══════════════════════════════════════════════════════════════════════
## CHECKPOINT (Uj) — 4c step desc landed; Step D + planned pages written as prose; Step C reduced
═══════════════════════════════════════════════════════════════════════
- 4c step description UPDATE 1 (RETURNING confirms 'Imagery landed:'). Cosmetic drift closed.
- User corrections absorbed: component-creator is dynamically spawned (Step C reduced to ONE go-source grep for the old prompt fragment — DB row authoritative, survived today's deploy). Planned pages: user wants all three built.
- Wrote a clear STEP D + PLANNED PAGES section (prose + separated bash): one schema-first read (stepD_and_pages_reads.sql — dartsonline before-state + literal-declarer scan; idea.uk planned-page stub existence + nav/plan source; existing tool-page shape) sizes both. Step D = supervised single fixer run on dartsonline (5fe8785b), judge the details JSON (per-class rewrites/strips/skips), THEN the tail decision; disposable-site delete deferred to a guarded cascade written against schema. Planned pages = additive needs_page emits per page via the existing handler, forked on Block B/C (stub rows? query-backed index vs static? reuse the deployed audience-check tool component) — concrete files next turn on the rows.
- The "mangled tail" was truncated DISPLAY, not file corruption (the W4b region reads intact); the clean rewrite lives in the new superseding section regardless.
idea.uk + dartsonline live rows untouched this turn.

═══════════════════════════════════════════════════════════════════════
## CHECKPOINT (Uk) — sizing read digested; planned pages exist; nav/plan re-read (0-rows was query fault)
═══════════════════════════════════════════════════════════════════════
- dartsonline: 16 components, ALL literal_section_var=f (built post-creator/fixer work), 2 product-grid literal_text_hex=t → THIN fixer specimen (tests strip + classify, barely the rewrite path). Judge details accordingly; disposable + safe-to-prove regardless.
- idea.uk planned pages EXIST as rows: news-index/guides-index (section-index), tool-audience-check (tool), all planned/active → build = W6-shaped needs_page emit vs existing rows (additive, no creation, no regression).
- 0-ROWS DISCIPLINE: nav/plan aspect guess (navigation/site_plan/composition) wrong-needled → 0 rows is query fault not absence. tool-audience-check is a BARE row (no components; nothing deployed to copy). Corrected read stepE_pages_plan_read.sql: real aspects (E.1) + planned rows' sections field (E.2, \d first) + site_plan_sections per-page rows for the three (E.3) → decides pure-emit vs planner-populate.
- Structural preference restated: if plan rows are missing, the planner path (build-site-planner) populates them, not a hand-INSERT — keeps nav/plan/page consistent.
- dartsonline delete deferred to a guarded cascade (7 tables) written against schema when needed.
idea.uk + dartsonline rows untouched this turn.

═══════════════════════════════════════════════════════════════════════
## CHECKPOINT (Ul) — planned pages = a PLANNING gap (empty sections, no plan rows), not a build gap
═══════════════════════════════════════════════════════════════════════
- E.3 decisive (needle proven by 3 tools rows): the three planned pages have NO site_plan_sections rows; E.2: their pages.sections = []. Catalogued, never composed.
- E.1: composition owned by site-design-planner (resolved_composition, refreshed 06-26 + per-page plans). → W6 needs_page emit is WRONG (would build an empty-sections page). Structural route = run the planner over the three, not hand-write plans (guessing component lists = planner's job; hand-INSERT drifts plan/nav/page).
- stepE_planner_read.sql staged: P.1 steps + consumed item type; P.2 config/page-selection; P.3 \d site_specs + resolved_composition contents (site-level all-pages vs six); P.4 planner work-item history (re-invocation shape). Build files next turn from these.
- Two-phase shape anticipated: planner composes the three (fills site_plan_sections) → THEN W6 needs_page builds+deploys them.
idea.uk + dartsonline rows untouched this turn.

═══════════════════════════════════════════════════════════════════════
## CHECKPOINT (Um) — context reset to architecture + working rules; scope re-anchored
═══════════════════════════════════════════════════════════════════════
System purpose (restated by user): intelligently PLAN and BUILD multipage websites from a domain name — targeted design/content per vertical, eventually parsing best-in-class exemplars (reasoning from why they work, not copying), adding tools/blog/news/infographics from wider-world reasoning. Close agent/message logging (headers+body); agent-creation guidelines doc in repo; distinct low-overlap agent responsibilities; sub-agents allowed for e.g. research-before-content.

Working rules re-anchored (superseding any drift): every agent is an orchestrator owning a workflow of ≥1 steps that call ACTIONS. Children respond to the PARENT's responses_topic, not their own. Keep WORKFLOWS SIMPLE; complexity lives in Go ACTION code. Do NOT create sub-workflows in SQL — spawn sub-agents with their own workflows (clean logs, separate responsibilities). Keep workflow var names in sync with what actions expect; don't rename current-code identifiers except deliberately + noted. No logger.Debug. Reuse/alter existing funcs+structs before creating new. Schema-first before SQL. Structural over quick fixes. 0-rows: check the query before trusting. Reasonable step sizes. No summary docs unless asked; brief end-summaries fine. Don't call fixes final; no "perfect/critical/excellent"; no congratulation. k8s ns ai-persona-system; kafka ns kafka (cluster personae-kafka-cluster...). Deploy = github → actions → backblaze s3.

Scope re-anchored: the delivered/deployed colour+imagery+localisation work stands. The ONLY open build item = 3 catalogued-but-uncomposed pages on idea.uk (news-index, guides-index, tool-audience-check): pages rows exist, page_type+nav set, but sections=[] and no site_plan_sections rows → never composed, hence nav 404s. NOT a planner breakage (the tested route built 6 pages correctly) — these are the un-built tail. Fix via the EXISTING path: compose (populate site_plan_sections) then W6 needs_page build — using the planner's own trigger, not hand-written plans. Reading site-design-planner's definition to find which existing lever composes already-catalogued pages.

═══════════════════════════════════════════════════════════════════════
## CHECKPOINT (Un) — composition mechanism read; re-plan is the designed, safe path
═══════════════════════════════════════════════════════════════════════
Flow established from code (write_site_plan_action.go + v3_site_actions.go):
- Composition = build-site-planner (needs_site_plan) → write_site_plan: supersedes current site_plans, inserts fresh plan, writes site_plan_pages + site_plan_sections for the pages in the plan output. Whole-plan, LLM-driven — not an append.
- The 3 pages have pages rows + nav but no site_plan_sections because the LLM plan behind the current site_plans row didn't include them (forward-looking catalogue, per the submit trigger note).
- KEY: normaliseRealisedToPlanPage (v3_site_actions.go:4383) exists so a re-plan LOADS realised pages (load_existing_pages), converts them to plan-page shape CARRYING their sections, and UNIONS with the LLM proposal — its own comment: without carrying sections the upsert would clobber built pages. So re-running the planner is SAFE by design: 6 built pages keep composition, 3 catalogued pages get composed. No hand-written site_plan_sections (planner's job; would drift plan tables).
- Structural fix = emit needs_site_plan for idea.uk → planner re-runs (union) → cascade (needs_composition/needs_design) → needs_page builds+deploys the 3. 
- Reading before emit (stepF_replan_read.sql): F.1 planner steps + consumed item; F.2 load_existing_pages/write_site_plan step configs + input_mapping; F.3 \d site_work_items + historical needs_site_plan shape to copy; F.4 confirm the 3 pages carry nav intent so load_existing_pages surfaces them.
- Reuse note: no new code — the tested build-site-planner route + existing W6 needs_page emit. Nothing renamed.
idea.uk + dartsonline rows untouched this turn.

═══════════════════════════════════════════════════════════════════════
## CHECKPOINT (Uo) — handoff produced; context paragraph produced; spam question opened
═══════════════════════════════════════════════════════════════════════
- Produced HANDOFF_claude_code_continue.md (detailed, for Claude Code) + CONTEXT_paragraph_for_new_chat.md (short) as separate deliverables. Both capture: three closed threads (P0 scheme→components 07-03, brief-explanation 07-04, presigned expiry 07-06); Go batch slices 1/2/F deployed, slice 3 fixer re-aim + slice 4 creator-prompt(landed)/003(patched)/flag-scope(4c) delivered; the ONE open build item (3 catalogued-but-uncomposed pages → re-plan via build-site-planner, safe by normaliseRealisedToPlanPage union) + Step D supervised fixer trial (dartsonline, disposable) + hygiene list.
- NEW WORKSTREAM (user): idea.uk contact form is receiving spam report-requests (fake orders like ord_1783948426211007948, test/test@test.com). User wants (a) to remove existing spam and (b) an IP block list. Opened schema-first: need the table holding form submissions + whether IP is captured. Staged spam_read.sql. This is a NEW area — not part of the colour/imagery/pages work; logged separately.
idea.uk + dartsonline rows untouched this turn.

═══════════════════════════════════════════════════════════════════════
## CHECKPOINT (Up) — spam workstream: data IS reachable (corrected); handoff written
═══════════════════════════════════════════════════════════════════════
- CORRECTION owned: my "form data is on a third-party VM, out of reach" was too hasty. The VM runbook shows nginx reverse-proxies the reserved tool paths — the /request intake among them — to the chassis Go process on 127.0.0.1:8080. So submissions post THROUGH the chassis handler and are written to a reachable store; my clients_db table-name search (order/lead/contact/submission/...) just missed the actual table name — a search miss, not absence. User confirmed "we have all the data here."
- HANDOFF_spam_and_ip_blocklist.md written (self-contained): find the intake table by DATA (search the ord_... id across tables) + by reading the /request Go handler; \d it for spam fields + whether it persists client IP (X-Forwarded-For via nginx); then guarded DELETE (SELECT-first) for cleanup and — only if IP is persisted — a blocklist table + handler check, else honeypot/timing/CAPTCHA at the handler. To be explored next chat.
- This is separate from the chassis colour/imagery/page handoff.
idea.uk live VM + chassis rows untouched this turn.
