...
**▶ POSITION (2026-07-03):** W1, W2a, W2b, **W3a (CTA follows the pair), W1b (five-layout batch)** all COMPLETE — trigger observed bumping layouts.updated_at in anger. **W3b (hero → ink model) prepared** (§W3b EXECUTION: position()-gated, twelve replaces with multi-line needles) — awaiting execution + paste-back. Then: the hero-* variant hazard batch (26 live imageless renders), and the hand-needles-vs-re-aimed-fixer decision for the remaining ~12 surface-painting hazards (W5). Later: W4 chrome path (fallbacks + repoint + force_rerender), W6 idea.uk rebuild + deployed-grep verification. Open items: what ran 07-01; planned-but-unbuilt pages in nav.
