...
**▶ POSITION (2026-07-02, late):** W1 COMPLETE. **W2a COMPLETE — the active site-footer follows the footer pair** (UPDATE 1; RETURNING verified; visual verify via `w2_02_verify_fixed.sql` — original errored on Postgres's 255 regex-bound cap). **site-head PARKED** (placed nowhere). W2b prepared (layouts.updated_at trigger, reuse-gated). Awaiting: w2_02_fixed + w2b_00/01 output. Then **W3 — CTA conversion** (pair values in hand; hero gradient-vs-solid decision at prep; NB the pair conversion also fixes a latent white-on-cyan hero failure on tool-portal-dark). Open items: idea.uk's sites.status (absent from active count — read in w2b_00a); what ran 07-01; the 5-layout contrast curation batch (awaiting approval); planned-but-unbuilt pages in nav.
