...
**Next, in order:**
1. **W4b — data-only chrome (read staged: `w4b_00_before.sql`).** Code fact (render_site_components_action.go:345–430): `renderAndStoreSiteComponent` joins the PINNED `site_components.component_id` with **no `is_active` filter**, and without `force_rerender` it SKIPS non-empty slots — so the **repoint must precede the force_rerender**, or it re-renders the old dark chrome from the inactive templates. Status: **W6 REBUILD COMPLETE** (six items drained; pages redeployed 19:18–22:45; three items retried before completing — error read staged). **Final verification staged: `w6_03_final_verify.sql` (DB-side: error class + rebuilt stored sections, fossil inverted) + the deployed grep on a fresh index.html.** On a clean grep, the thread's original symptom — light-resolved idea.uk rendering dark — is closed; remaining work: the Go code batch (scheme-aware fallbacks, creator prompt, fixer re-aim, 003 edits, Debug tidy) as one deploy, the non-idea.uk hazard/band tail decision, the planned-pages choice, and the small hygiene list.
