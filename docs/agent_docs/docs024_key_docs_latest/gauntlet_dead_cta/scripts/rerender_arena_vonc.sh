#!/usr/bin/env bash
# rerender_arena_vonc.sh — assemble-only rerender + deploy of vonc.com/tools/arena.
#
...
PAGE_ID="d2c8a925-1dca-44b6-a866-4561905a87a8"   # tool-arena, /tools/arena/index.html
DOMAIN="vonc.com"
...
echo "========================================="
echo "Rerender tool-arena (assemble-only + deploy)  ${DOMAIN}"
echo "  SAVE: CORRELATION_ID=${CORRELATION_ID}"
echo "========================================="

kubectl -n kafka run "kcat-arena-$(date +%s)-$RANDOM" --rm --restart=Never \
