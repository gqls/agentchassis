...
(function () {
  ...
  function buildVerdictCard(permalink) {
    ...
    // RFC_020 §5.4 — the scope of the ruling travels WITH the artefact.
    //
    // The card is the thing that gets screenshotted and posted, and it outlives
    // both the link and the page's own wording. So the qualification has to be
    // drawn onto the image itself: "the judge ruled: opponent wins" on its own
    // is a sentence a stranger can read as a finding of fact about whatever the
    // round was about. It is not one — the judge scores how the case was made.
    //
    // It is a fixed string, not a field, so no round can be missing it, and it
    // is drawn AFTER the auto-fit has already reserved its space (see FOOT) so
    // it can never be the thing that overlaps.
    // The em dash is written as an escape, not as a literal character: every
    // string this file DRAWS does the same (the middle dot on the header and
    // address lines, the em dash in the share copy) while the comments use
    // literal characters. The delivery path is base64 through psql into a
    // canvas fillText, and the escape is the form nothing downstream can
    // re-encode. Asserted by check 5 in verify_card_2026-08-10.py.
    var SCOPE = "The judge rates how well the case was argued \u2014 not whether it is true.";
    ...
    // FOOT reserve the header, the ruling line, the scope line and the address.
    // Measured against the DRAWN layout, not a raw character budget — labels and
    // chrome take vertical space the prose then cannot have, and a 32px draft
    // that was "inside" a 737-character budget still overlapped its own ruling
    // line.
    //
    // FOOT went 130 -> 172 when the scope line was added (2026-08-10): the four
    // footer baselines below are all measured back from H, so the reserve moves
    // by as much as the topmost of them did, keeping the same 18px between where
    // the prose stops (H - FOOT) and where the rule is painted (H - 154).
    //
    // > CORRECTED 2026-08-10, before this shipped: an earlier draft of this
    // > comment said that gap "is what stops a full-length round touching its
    // > own ruling". It is not, and the loop below is why. heightAt/size adapt
    // > to whatever USABLE is, so the reserve does not prevent an overlap — it
    // > is ABSORBED, and what it actually buys is TYPE SIZE. On the real
    // > measured round: 26px before this change, 24px after. FOOT only becomes
    // > load-bearing past the loop's own floor of 12px, which realistic input
    // > never reaches (the challenge is engine-written at ~305 chars; a
    // > 2000-char defence — the input cap — still fits at 13px and does not
    // > collide). Caught by the positive control in
    // > verify_card_2026-08-10.py, which put FOOT back to 130 expecting a
    // > collision and got a clean card at BOTH values. Logged in WRONG_CALLS.md.
    // > If you shrink this reserve, the thing you break is legibility, not
    // > layout — check 9 asserts >= 20px so that trade is made against a number.
    var TOP = 112, FOOT = 172, USABLE = H - TOP - FOOT;
    function heightAt(f) {
    ...
    x.fillStyle = "#f59e0b";
    x.fillRect(L, H - 154, 120, 6);
    x.fillStyle = "#ffffff";
    x.font = "800 34px system-ui, sans-serif";
    x.fillText("The judge ruled: " + verdict + ".", L, H - 104);

    // Shrink-to-fit rather than wrap. A second line here would run into the
    // address beneath it, and the failure would be silent — canvas clips
    // nothing and reports nothing. Stepping the size down keeps it one line
    // whatever the platform substitutes for system-ui; at 18px the string
    // measures well inside MAXW on every font tested, so the loop is a
    // guard against font substitution, not the normal path.
    var sf = 20;
    x.font = "600 " + sf + "px system-ui, sans-serif";
    while (sf > 13 && x.measureText(SCOPE).width > MAXW) {
      sf--;
      x.font = "600 " + sf + "px system-ui, sans-serif";
    }
    // NOT #fbbf24, the block-label amber. Computed against this card's own
    // #6d28d9 it is 4.26:1, and the two lines that already use it are 20px/700
    // labels — large text, where 3:1 is the floor and it passes. This line is
    // the one sentence on the card that a reader must not miss, and the
    // shrink-to-fit above can take it to 13px, where 4.5:1 applies and 4.26
    // fails. #fde68a is the same hue lighter and computes 5.71:1, so it holds
    // at every size the loop can choose. Both figures are arithmetic from the
    // WCAG formula over known constants (the card background is a literal in
    // this function, not an inherited value) — check 6 in
    // verify_card_2026-08-10.py recomputes them from the drawn pixels.
    x.fillStyle = "#fde68a";
    x.fillText(SCOPE, L, H - 64);
