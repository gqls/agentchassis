...
| **Migration `619_…_HOLD`** (bypass the 4 LLM seats, one edge) | written + rehearsed (apply and apply-then-rollback in rolled-back txns); **NOT applied — the owner's word is the gate** (PLAN §5 Phase 1). |
| **Migration `620_…_HOLD`** (acceptance + reader seats, record mode on 6 seats, 11 seat-failure rows, `check_seats_ran`) | written + rehearsed both ways; **NOT applied — needs the roll AND the RFC verdict AND `-v FILING_MODE_SHIPPED=<verified sha>`**. |
| **RFC_056** + council submission (Go + both HOLDs) | submitted, corr **`d1342f2a-fcfc-4fb1-b28d-cf3ee0ad492a`** — check the verdict before acting on 620. Addendum answers the vigilant lane's parked-verdict objection. |
...
1. **Owner applies 618** (command in `bugs_open/376` §11f) → verify 15 steps + `check_exemplar_floor`
   ...
   route unblocks.
2. **Owner's word on Phase 1** → apply `619_…_HOLD`, then `UPDATE scheduled_tasks SET enabled=true
...
4. **After the next chassis roll**: prove the roll carries `c440d5c5e`
   (`git merge-base --is-ancestor c440d5c5e <stamp>`), then 620 becomes appliable (its probe
   demands the sha).
