...
- **OPEN — why the keys diverged (RUNBOOK R3a).** R3 confirmed the five render an identical content-free shell with
  old-key `content_data`. `component_versions` has only the 15:06 snapshot, so this action hadn't regenerated the
  component before. So the divergence is either a prior/out-of-band schema change (manual/migration) or a standing
  **writer-vs-schema mismatch** (the writer emits keys the schema never declared). Compare the pre-15:06 snapshot's
  `input_schema->'fields'` to the live fields and to `content_data`'s keys to decide — that determines whether the
  fix lands in `StoreGeneratedComponentAction` (migrate keys / preserve names on regen) or in the writer/schema.
