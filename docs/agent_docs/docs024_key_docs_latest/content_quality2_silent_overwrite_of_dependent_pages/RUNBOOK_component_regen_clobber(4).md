...
triggered rerender is assemble-only, the bytes in those rows are the **pre-15:06** render, preserved — and since
the keys matched before 15:06, that render was good. So one current-state question remains (R3a sub-check): are
the five still showing the old good render — breakage **latent** until the next real re-render — or an empty one?
The single confirmed empty-and-dropped *symptom* was gamesdesign's index, since removed in an unrelated rebuild,
so it is not in the affected set below. Recovery, where needed, is a `content_data` key re-map plus a section
re-render that **must carry `reason=section_data_resolved`** (a bare `needs_rerender` is assemble-only and just
re-ships the shell). The remaining steps confirm scope, back up, recover, and verify.
...
- [x] **R3a** — When/why the binding broke — RESOLVED (15:06 regen rename, `content_data` not migrated); run the content-presence sub-check for broken-now vs latent
...
`render_has_*` TRUE → rows still hold the good render; breakage is **latent** (live pages fine now, but the next
real re-render empties them). FALSE → **empty now**; the five are live-broken and need recovery. The fix and the
future risk are identical either way.
