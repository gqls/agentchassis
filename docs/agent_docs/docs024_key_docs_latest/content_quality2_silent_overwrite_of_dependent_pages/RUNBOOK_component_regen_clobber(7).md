...
- [ ] **F1-prompt** — Companion: component-creator `generate_template` must preserve existing field names (so names-preserving regens succeed)
- [ ] **F2** — Test the guard on a throwaway component (rename/drop rejected; addition + names-preserving restyle allowed)
- [ ] **F3** — Propagate `reason=section_data_resolved` (re-add to spec + make `create_rerender_items` carry it) — needs that action's code
...
## Step F3 — Propagate the section-rerender reason (needs the `create_rerender_items` action code)

So a legitimate restyle regen (and recovery) can drive an actual section re-render rather than an assemble-only
re-ship. Two coupled changes: (a) re-add `"reason": "section_data_resolved"` to `createRerenderWorkItem`'s spec in
`store_generated_component_action.go`; (b) make `rerender-pages` propagate `spec.reason` onto the per-page
`page_rerender` items its `create_rerender_items` step creates. (b) is a workflow change (add
`"reason": "input_data.spec.reason"` to that step's config) **if** the `create_rerender_items` action reads a
`reason` config key and stamps it on the items — otherwise a small Go change to that action. Need the
`create_rerender_items` action source (`grep -rn "func CreateRerenderItems\|\"create_rerender_items\"" platform/`)
to choose and write it precisely, and to confirm it wouldn't over-scope (a site-wide `section_data_resolved` would
re-render every page's sections, not just the dependents).
...
1. Create a test component with a dependent page that has populated `content_data`. Regenerate it so the new
   schema **renames** a retained field. Assert it is **rejected** with the field-contract message (and an
   `agent_error_log` row), and the existing component/dependents are untouched.
2. With the companion prompt change in place (names preserved), regenerate so only the template markup changes;
   assert the store **succeeds** and leaves dependents' `content_data` intact (their section markup refreshes via
   F3 or a rebuild — not asserted here unless F3 has shipped).
3. Regression: a regeneration that does **not** rename/drop retained fields still succeeds and leaves dependents
   intact; a pure field **addition** is allowed.

Done-when: a rename/drop regeneration is rejected loudly, a names-preserving restyle succeeds and re-renders
dependents with content, and additions still work.
