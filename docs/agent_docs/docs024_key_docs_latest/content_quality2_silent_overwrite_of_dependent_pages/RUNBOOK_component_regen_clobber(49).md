...
## YOU ARE HERE → Part D, Step D2b (prevention: canonical-token prompt rule + store WARN lint)
D2a VERIFIED in production 2026-07-06 (gamesdesign stylesheet shows the full 11-name compatibility-aliases :root block). Then robot-hands/vonc self-heal-or-bridge, Part C residue, Part E flags.
...
- [x] **D2a — VERIFIED in production 2026-07-06.** Adapted the proven 076 webdesign trigger for gamesdesign (site_id e33263f4… verified) → full chain ran (spawn→call→analyze_design→render_css_from_spec→git_commit→Actions→Backblaze) → curl shows the trailing compatibility-aliases :root block with all 11 names (layout defined none). Vocabulary drift now self-heals on every design pass. Details: Fleet self-rolled to v1.0.1098 (18:55). `needs_design` producer = **build-site-planner** only (4 items, spec={}); trigger via the REAL path, not a hand-INSERT. gamesdesign curl empty = no design pass since deploy (stale stylesheet, expected). NEXT: read build-site-planner config → pick its design path OR a direct standalone webdesign-agent spawn (workflow start_step check_site_context loads from input_data.domain — runs on a domain alone, exercises render_css_from_spec in isolation) → curl gamesdesign styles.css for the alias block. Vehicle: gamesdesign (test site — no palette-shift
  risk). Steps: (1) confirm webdesign-agent image_tag > v1.0.1096; (2) read a `needs_design` item's shape (never
  guess) → clone for gamesdesign; (3) after the run: curl gamesdesign styles.css for the trailing
  "compatibility aliases" :root block + log grep for `appending compatibility token aliases` /
  `token_alias_length`. PASS = the bridge block present with the layout's missing names. Patch history: (`render_css_from_spec_action.go` in outputs + alias tests + diff).
  Discovery: the `:root` lives in DB layout templates (`comp.LayoutTemplate`, forkable) — so the patch is IN GO,
  post-render, as **step 11** mirroring step 10's `buildSectionDefaults` pattern: `buildTokenAliases(renderedCSS)`
  appends ONLY names not already defined (`"<name>:"` match — var() usages and sibling names don't count),
  idempotent, per-name so layout-defined values always win; one added zap field (`token_alias_length`) on the
  existing log; NO renames. TO SHIP: heads-up to the parallel chats (webdesign agent touched 13:22, v1.0.1096) →
  build → Actions → image bump → each site self-heals on its next design pass. Original draft block for reference:
  ```css
...
- [~] **D2b — prevention (IN PROGRESS 2026-07-06).** Reuse-first per the guidelines: extend the EXISTING
  component-template contract + validation fn, not new mechanisms. Three parts:
  - **D2b-1 contract rule** ✔ DRAFTED — rule 11 for the "New component checklist" (003_contracts §template-rules):
    templates reference ONLY canonical tokens + the buildTokenAliases aliases; invent no new var(--…). (Doc is
    read-only in /mnt/project → delivered as a paste-in patch block in chat.)
  - **D2b-2 prompt** ⏳ — make the generating agent's prompt enforce rule 11 (the cause-closing piece). 21 agents
    match the CSS ILIKE; narrowing to component-creator / component-template-fixer / section-editor /
    landing-page-builder via a prompt dump (issued) before editing IN PLACE (workflow-jsonb, immediate). Don't
    guess the agent.
  - **D2b-3 lint** ✔ CODE — `AuditTemplateTokens` + `canonicalCSSTokens` allowlist appended to
    component_validation.go (WARN-only, never rejects; reuses the contract-enforcement file), + 7 table tests,
    gofmt-clean. PENDING: wire the call where `ValidateComponentTemplate` is already invoked
    (`StoreGeneratedComponentAction`) — needs that action file. Ships on the next build.
  Original plain description: WHY: D2a rescues variable names we have
  already catalogued, but the LLM that writes component templates invents names with no constraint, so it keeps
  minting new ones (--hero-ink was the newest). WHAT: add a canonical-token rule to the component-creator's prompt
  (the prompt-in-agent-config we edited for F1-prompt) so template CSS may reference ONLY the sanctioned design
  tokens + the bridge aliases — no new orphans appear. Optional: a store-time check that logs a WARN when a
  template references an unknown `var(--…)` (detection net, not a blocker — same shape as the F8 fallback lint).
  HOW: read the component-creator agent config + the guideline section on prompt/agent changes → workflow-jsonb
  edit (immediate, no rebuild); optional lint is a small Go addition. Low risk, cause-side, complements D2a.
- [ ] **D-sites — heal robot-hands + vonc now, or let them self-heal.** WHY: D2a only takes effect on a site's
  NEXT webdesign pass; these two still serve pre-fix stylesheets, so their newer-vocabulary sections render
  dark-on-dark (gripper-detail's blank look). ROUTES: (a) do nothing — they self-heal on their next design pass;
  (b) fix now WITHOUT re-rolling their palette via a one-off manual commit of just the bridge `:root` block to each
  site's styles.css (identical CSS, no LLM). CAUTION: forcing a full webdesign pass re-runs analyze_design (fresh
  LLM spec) → palette can shift — hence route (b) for an immediate, palette-preserving fix. Work: a decision, then
  nothing or two small commits.


## Part E — Flags & hygiene (deferred structural hardening; independent, any order)
Root-cause weaknesses logged during the investigation and parked so they didn't derail the active fix. Each is a
read-first, reuse-first change of its own. Suggested first: F8 mitigation (the structural lesson of this episode).

- [ ] **F8 mitigation** — WHY: the whole gauntlet spread happened because site-specific copy was baked into a
  SHARED component (fallback values + llm_guidance), and the store guard checks field NAMES only. WHAT: a
  guard/lint so shared-component fallbacks and llm_guidance must be site-neutral (reject, or WARN at store time).
  Highest-value — it prevents the class of bug this episode was about.
- [ ] **F5** — WHY: the store guard rejects RENAMED retained fields but not ADDED required fallback-less fields,
  which strand renderability (the `cta_url`-required-with-no-site-spec facet that blocked recovery). WHAT: extend
  the guard to also reject (or force optional/fallbacked) an added required field on a regeneration.
- [ ] **F6** — WHY: the store guard's NOT EXISTS status list and the `idx_swi_dedup` predicate disagree on
  `unresolved` (index treats it terminal; guard doesn't), and `itemsCreated++` isn't gated on RowsAffected. WHAT:
  align the two status lists; gate the counter on rows actually affected.
- [ ] **F7 (residual)** — WHY: `update_component_html` swaps a template without validating placeholder⇄schema
  agreement (its snapshot INSERT is already fixed). WHAT: add that validation on template swaps.
- [ ] **F4 (softened)** — WHY/WHAT: advise (store-side) when a regen FORKS a duplicate function row instead of
  matching an existing one; low urgency (evidence is the F2-3b fork; the duplicate hero rows were old manual seeds).
- [ ] **Hygiene** — 40 stale `unresolved` page_rerender items (2026-05-01) to clear; loose dispatch item-status
  semantics documented across the investigation (complete-at-dispatch, errors-in-complete, status-change without a
  timestamp bump, parent-topic-vanished noise) — worth a pass when convenient.
