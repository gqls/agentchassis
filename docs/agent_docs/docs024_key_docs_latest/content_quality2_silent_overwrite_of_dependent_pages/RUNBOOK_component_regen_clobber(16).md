...
- [ ] **F2** — Test: Tier 1 unit ✔ · Tier 3a keep ✔ (two regens, `{body,eyebrow,heading}` preserved, templates genuinely changed — md5) · **Tier 3b redo pending** (first attempt forked a duplicate — LLM chose function `simple-intro-band`; redo fixture pins the function) · then cleanup of ALL zzz test rows incl. the fork `80222fc1`
- [ ] **F1-prompt pin** — apply `F1prompt2_pin_function.sql`: pin the existing `function` inside the regeneration rule (loader already returns it) — closes regen-vs-create nondeterminism at generation time
...
UPDATE. **Redo after the first attempt forked:** the fixture's `function` must be the one the LLM will actually
choose — use `simple-intro-band` (demonstrated choice for this description) and pin it in the description too.
Clean up ALL earlier zzz rows first (cleanup SQL below — it also removes the forked duplicate `80222fc1`, whose
`section_type` is `zzz-fg-reject`).
```sql
INSERT INTO content_components
  (name, function, section_type, component_level, is_active, forked_from, html_template, input_schema, created_from)
...
INSERT INTO site_work_items (site_id, source, pipeline, item_type, severity, summary,
                             priority, handler_agent, status, created_by, spec, item_key)
VALUES ('<TEST_SITE_UUID>', 'f2-test', 'build', 'needs_new_component', 'medium', 'F2 reject test (redo)',
        50, 'component-creator', 'triaged', 'f2-test',
