...
- [ ] **F1-prompt** — Companion (two parts): (a) load the existing component's field names before `generate_template` (component-creator generates blind today); (b) prompt rule to reuse them verbatim — applied as a snapshot-first, drift-checked anchored migration (mind the 072 nested-prompt trap)
