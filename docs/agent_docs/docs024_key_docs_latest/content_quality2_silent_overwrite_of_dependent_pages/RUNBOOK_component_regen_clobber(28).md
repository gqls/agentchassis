...
- [ ] **R5b** — 2/5 recovered + verified. Pending three are behind TWO STUCK `claimed` children (gripper-detail page item claimed 12:58, never ran; ai-agent-orch trigger claimed 12:57, no page items created). Not swallowed — dedup index confirmed: `UNIQUE (site_id,item_key) WHERE status NOT IN (complete,verified,rejected,wont_fix,failed,unresolved)`. Unstick: confirm no active child (pods + dispatch logs) → re-triage the two → R6b. vonc.com/index = healthy 6th dependent (ongoing platform build)
...
- [ ] **7. ← NEXT: unstick the two stalled `claimed` items** (gripper-detail page item, claimed 12:58:24;
      ai-agent-orch trigger, claimed 12:57:33 with no page items created).
- [ ] **8. Final verify (R6b) → Step R6** (live deploys) → closeout.
