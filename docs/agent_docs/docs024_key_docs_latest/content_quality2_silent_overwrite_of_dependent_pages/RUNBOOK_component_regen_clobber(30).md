...
- [ ] **R6c — gripper-detail LIVE PAGE EMPTY (new symptom, 2026-07-03).** The system-stats SECTION is rendered in
  the DB (R6b pass), so the gap is page-level: (a) the page's OTHER sections empty/unrendered, (b) `pages.sections`
  empty → assembly stitched nothing, (c) assemble/deploy leg after `save_sections` didn't ship, or (d) nested
  `/entities/` artifact-path handling in git→Actions→Backblaze. Distinguish with: all-sections listing
  (`page_components` for the page: slot_name, function, build_status, len), the page row
  (`build_status, last_built_at, deployed_at, rendered_header/footer lens, jsonb_array_length(sections)`), and
  `curl -s -o /dev/null -w '%{http_code} %{size_download}' https://robot-hands.com/entities/gripper-detail.html`
  (+ `| head -60`). DB-whole + recent deploy timestamps → inspect the git artifact; other sections tiny/missing →
  a different empty-band instance or a never-fully-built page; `sections=0` → assembly had nothing.
...
Throughout, the component id `fdd92ad4-521a-4602-89cf-7ee1a66c10f1` is `system-stats`, and the example page is
...
```sql
SELECT jsonb_pretty(input_schema) FROM content_components WHERE id = 'fdd92ad4-521a-4602-89cf-7ee1a66c10f1';
...
WHERE id = 'fdd92ad4-521a-4602-89cf-7ee1a66c10f1'
ORDER BY 1;
```
...
WHERE component_id = 'fdd92ad4-521a-4602-89cf-7ee1a66c10f1'
...
UUID `fdd92ad4-521a-4602-89cf-7ee1a66c10f1`; the example page is `robot-hands.com` /
`gripper-detail`.)
...
SELECT jsonb_object_keys(input_schema) FROM content_components WHERE id = 'fdd92ad4-521a-4602-89cf-7ee1a66c10f1' ORDER BY 1;
...
FROM page_components
WHERE page_id = (SELECT p.id FROM pages p JOIN sites s ON s.id = p.site_id
                 WHERE s.domain = 'robot-hands.com' AND p.name = 'gripper-detail')
  AND component_id = 'fdd92ad4-521a-4602-89cf-7ee1a66c10f1'
ORDER BY 1;
...
WHERE pc.component_id = 'fdd92ad4-521a-4602-89cf-7ee1a66c10f1' ORDER BY s.domain, p.name;
...
VALUES ('e33263f4-74f8-494f-b191-546845dbbddf'  /* gamesdesign.co.uk — the test site */, 'f2-test', 'build', 'needs_new_component', 'medium', 'F2 keep test',
...
INSERT INTO site_work_items (site_id, source, pipeline, item_type, severity, summary,
                             priority, handler_agent, status, created_by, spec, item_key)
VALUES ('e33263f4-74f8-494f-b191-546845dbbddf'  /* gamesdesign.co.uk — the test site */, 'f2-test', 'build', 'needs_new_component', 'medium', 'F2 reject test (redo 3)',
        50, 'component-creator', 'triaged', 'f2-test',
...
CREATE TABLE IF NOT EXISTS _bak_compregen_component AS
  SELECT *, now() AS _bak_at FROM content_components WHERE id = 'fdd92ad4-521a-4602-89cf-7ee1a66c10f1';

CREATE TABLE IF NOT EXISTS _bak_compregen_pagecomps AS
  SELECT *, now() AS _bak_at FROM page_components WHERE component_id = 'fdd92ad4-521a-4602-89cf-7ee1a66c10f1';
```
...
## Step R5b — Unblock section readiness and finish the recovery (step-by-step)

Constants for this recovery (no fill-ins needed):
- Component: `fdd92ad4-521a-4602-89cf-7ee1a66c10f1` (`system-stats`)
- Sites/pages: leopardessconsulting.co.uk (index), robot-hands.com (index, gripper-detail),
  ai-agent-orchestration.com (index, case-study-kafka-consumer-group-remediation)
- Chosen CTA URL: `/contact.html` (verified to exist as a page on all three sites)
- Stale/empty render fingerprint: `md5 = 462c7c4501b8137d06a0f4c3e7592772`, `len = 7369`

Progress:
- [x] **1. Diagnose** — sections CARRIED because `cta_url` (`required:true`, `source: site_specs.cta.primary_url`,
      no fallback) had no `cta` spec on any site (both shapes checked, 0 rows).
- [x] **2. Branch decision** — template has no `{{if}}` blocks (`has_if_blocks=f`) → Branch B (per-site real spec).
- [x] **3. Verify CTA URLs are real pages** — all three sites have `contact` at exactly `/contact.html` (active).
- [x] **4. Insert the specs** — done (`INSERT 0 3`).
- [x] **5. Retrigger** — done 2026-07-03 12:57:33 (`INSERT 0 3`).
- [x] **6. Partial verify** — 2/5 recovered: leopardess/index (md5 6227e777…, 13:02:32) and robot-hands/index
      (md5 5477b4a6…, 12:59:35); needles true, cta baked. vonc.com/index appeared as a healthy 6th dependent
      (ongoing platform build — not ours).
- [x] **7. Resolved WITHOUT re-triage** — ai-agent-orch's child ran late (13:42:16: both scoped page items
      created + completed); gripper-detail's item flipped `claimed`→`complete` with the SAME `updated_at`
      (status change didn't bump the timestamp — 5th loose-semantics sighting). 7c skipped (would match 0 rows).
      Caveat: item `complete` ≠ render done — step 8 is the truth, especially for gripper-detail.
- [x] **8. Final verify (R6b) — PASS 2026-07-03** — all five recovery rows off the stale md5 with distinct
      md5s/lengths, fresh timestamps (gripper-detail rendered late at 13:44:31; ai-agent-orch pair 13:43/13:45),
      needles true, cta baked. vonc.com/index healthy sixth. **Recovery complete at the DB level.**

### Step 7 — unstick the two stalled items

NOTE (post-redeploy): if the agents have been redeployed/restarted since 12:58, no pre-restart child can still be
working these items — once 7a confirms they are still `claimed`, the re-triage in 7c is safe.

**7a. Confirm they are still stuck:**
```sql
SELECT s.domain, w.item_type, coalesce(w.spec->>'page_name', w.spec->>'function') AS what,
       w.status, w.updated_at
FROM site_work_items w JOIN sites s ON s.id = w.site_id
WHERE (w.item_key = 'recover_system_stats:fdd92ad4-521a-4602-89cf-7ee1a66c10f1'
       OR (w.item_type = 'page_rerender' AND w.spec->>'reason' = 'section_data_resolved'))
  AND w.updated_at > '2026-07-03 12:57'
ORDER BY w.updated_at;
```
Read: `claimed` rows for `gripper-detail` and/or the `ai-agent-orchestration.com` trigger = still stuck → 7c.
`complete` with the page rows still stale = run 7b logs. Fresh renders already landed = skip to step 8.
RESULT 2026-07-03 ~13:45: all rows `complete`; ai-agent-orch page items ran at 13:42 → skipped 7c, moved to step 8.
Contingency if gripper-detail alone stays stale in step 8 (its transient pod's logs are gone):
```sql
UPDATE site_work_items SET status='triaged', updated_at=NOW()
WHERE item_type='page_rerender' AND spec->>'page_name'='gripper-detail'
  AND spec->>'reason'='section_data_resolved';
```

**7b. (context, optional) pods and dispatch-loop view:**
```bash
kubectl -n ai-persona-system get pods | grep -E 'rerender-pages|page-rerender|build-dispatch'
kubectl -n ai-persona-system logs -l agent-type=build-dispatch-loop --tail=800 \
  | grep -iE 'recover_system_stats|gripper-detail|ai-agent-orchestration' | tail -30
```

**7c. Re-triage the stuck items** (safe post-restart; the dispatch loop only claims `triaged`):
```sql
UPDATE site_work_items SET status = 'triaged', updated_at = NOW()
WHERE status = 'claimed'
  AND ( item_key = 'recover_system_stats:fdd92ad4-521a-4602-89cf-7ee1a66c10f1'
     OR (item_type = 'page_rerender' AND spec->>'page_name' = 'gripper-detail'
         AND spec->>'reason' = 'section_data_resolved') );
-- expect UPDATE 2 (fewer if one progressed in the meantime)
```
The re-triaged trigger re-runs the whole rerender-pages pass for ai-agent-orchestration.com (blog rebuild →
get_pages → scoped items); the gripper-detail page item just gets re-claimed directly.

**7d. Watch the chain run** (repeat 7a in psql with `\watch 60`, or):
```bash
kubectl -n ai-persona-system logs -l agent-type=rerender-pages --tail=1000 \
  | grep -iE 'scoping to component dependents|items_created'
kubectl -n ai-persona-system logs -l agent-type=page-rerender --tail=2000 \
  | grep -iE 'rerender_page_sections|not ready|carrying|rerendered='
```
Expect `scoping to component dependents … dependent_pages=2` for ai-agent-orchestration.com, then per-page
section re-renders with `carried=0`.

### Step 8 — final verify (R6b)
...
ORDER BY s.domain, p.name;
```
PASS: every row on the three recovery sites has `md5 <> '462c7c4501b8137d06a0f4c3e7592772'`, a fresh `updated_at`,
`has_stat1_label` + `has_headline` = t, `cta_href_empty` = f, `cta_url_baked` = t. (vonc.com/index rides along as
the healthy sixth dependent.) Then Step R6: confirm the bands on the live deploys (git/Backblaze).

## F5 (flagged, not built) — regen ADDING a required field is also a contract change

The guard (F1) blocks rename/remove of retained fields. This incident shows additions can strand too: a regen
that adds a `required` field with no fallback (Tier-C here — `site_specs.*` the sites don't have; an
`llm`-required addition would silent-empty instead) leaves dependents unable to re-render. Candidate extension:
the field-contract guard also rejects (or forces optional/fallbacked) any ADDED `required` field on a
regeneration. Flagged for later alongside F4.

## Step R6 — Verify (corrected needles + trigger diagnostics)

Do not read verification early: C1's own bulk UPDATE stamps all five `updated_at` identically — a render is proven
by a **changed md5**, `has_headline`, and `build_status` transition, not by the timestamp. And short numeric values
("70") are unusable needles — 2-char substrings false-positive against CSS. Use labels/headlines.

**R6-diagnostics (if reason-stamped `page_rerender` items are missing for a site):**
```sql
-- D1. The three recovery triggers: processed? failed?
SELECT s.domain, w.status, w.error, w.updated_at
FROM site_work_items w JOIN sites s ON s.id = w.site_id
WHERE w.item_key = 'recover_system_stats:fdd92ad4-521a-4602-89cf-7ee1a66c10f1'
ORDER BY s.domain;

-- D2. OPEN page_rerender items on the sites — dedup blockers (create_rerender_items uses ON CONFLICT DO NOTHING
-- keyed on page_rerender_<name>_<siteid>; a pre-existing OPEN item silently swallows the new reason-stamped one.
-- Known accounting flaw: itemsCreated++ runs even on a swallowed conflict — logs can overcount items_created.)
SELECT s.domain, w.spec->>'page_name' AS page, w.status, w.spec->>'reason' AS reason, w.updated_at
FROM site_work_items w JOIN sites s ON s.id = w.site_id
WHERE w.item_type = 'page_rerender'
  AND w.status NOT IN ('complete','verified','rejected','wont_fix','failed')
ORDER BY s.domain, page;

-- D3. Page statuses (get_pages_for_rerender picks only 'deployed'/'active')
SELECT s.domain, p.name, p.status
FROM pages p JOIN sites s ON s.id = p.site_id
WHERE p.id IN (SELECT page_id FROM page_components WHERE component_id = 'fdd92ad4-521a-4602-89cf-7ee1a66c10f1')
ORDER BY 1, 2;
```
```bash
kubectl -n ai-persona-system logs -l agent-type=rerender-pages --tail=1000 \
  | grep -iE "scoping to component dependents|items_created|No pages found|has_pages"
```

**D4 — "deployed but byte-identical" signature (section re-render ran but CARRIED).** If the five rows show fresh
per-page `updated_at`, `build_status='deployed'`, yet the SAME stale md5: that is the `rerender_page_sections`
carry path — the gate matched and the action ran, but a section failed `planSection` readiness (a **required
non-LLM field couldn't resolve**) so its stored (empty) HTML was re-emitted and `save_sections` wrote it back.
Prime candidate here: `cta_url` (`source: site_specs.cta.primary_url, required: true`, no fallback) with no
`cta.primary_url` in the site's specs → permanently not-ready → carried forever. Decide with the page-rerender
logs (the action logs "not ready, carrying stored HTML" / "escalating" / "done … rerendered=N carried=M"):
```bash
kubectl -n ai-persona-system logs -l agent-type=page-rerender --tail=3000 \
  | grep -iE "rerender_page_sections|not ready|carrying|escalat|rerendered"
```
and check the spec presence (site_specs is per-aspect: site_id, aspect, data jsonb, is_current):
```sql
SELECT s.domain, sp.aspect, sp.data->>'primary_url' AS primary_url
FROM site_specs sp JOIN sites s ON s.id = sp.site_id
WHERE sp.is_current AND sp.aspect = 'cta' AND s.domain IN ('leopardessconsulting.co.uk','robot-hands.com','ai-agent-orchestration.com');
SELECT s.domain, sp.aspect, jsonb_pretty(sp.data->'cta') AS cta_obj
FROM site_specs sp JOIN sites s ON s.id = sp.site_id
WHERE sp.is_current AND sp.data ? 'cta' AND s.domain IN ('leopardessconsulting.co.uk','robot-hands.com','ai-agent-orchestration.com');
```
Remedies branch on the finding: populate a real `cta.primary_url` per site, or relax the shared component's
`cta_url` (required:false / add a fallback) — the latter is a co-managed shared-component write (R7 + snapshot
discipline). Note: if the logs show the carry path, F3 itself worked end-to-end (reason propagated, gate matched,
section re-render executed) — the blocker is one layer deeper, in section readiness.
