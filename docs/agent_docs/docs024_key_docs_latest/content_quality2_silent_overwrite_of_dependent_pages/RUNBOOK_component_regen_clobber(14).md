...
- [ ] **F2** — Test: Tier 1 unit ✔ · Tier 3a keep ✔ (two regens, `{body,eyebrow,heading}` preserved, no rejections) · **Tier 3b reject pending** (inactive `zzz-fg-reject`, expect `agent_error_log` rejection naming `zzz_*`, component untouched, 0 version rows) · then cleanup of both test components
