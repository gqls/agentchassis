...
```
2. Insert the specs (the `(site_id, aspect) WHERE is_current` slot is free — verified 0 rows):
```sql
...
```
3. Re-trigger the five (the same C2 `needs_rerender` inserts; prior items are `complete`, excluded by the dedup
   guard — expect `INSERT 0 3`).
4. R6b verify: `has_stat1_label`/`has_headline` true ×5, five DISTINCT md5s, `cta_href_empty` false. The resolver
