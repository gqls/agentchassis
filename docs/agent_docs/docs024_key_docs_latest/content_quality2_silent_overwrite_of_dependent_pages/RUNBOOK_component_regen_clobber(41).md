...
- [ ] **R6e — F8 Steps 1–3 EXECUTED 2026-07-03** (snapshot v2 ✔ · neutralize ✔ · verify ✔ stats=llm/CTAs neutral · backup 3 ✔ · strip UPDATE 3 ✔ · scoped triggers INSERT 0 2 ✔ · other chat informed). A–D READ 2026-07-04: matchmatrix was NOT skipped — fanned out and ERRORED (error text to fetch); auto-escalations
  ...
  content_components — Admin Content Brief block optional/absent. Mechanism closed with one carrier. NEXT: full
  llm_guidance dump (\o /tmp/guidance.txt — sweep for vonc concepts beyond the word + updated_at for the lock) →
  snapshot v3 → jsonb_set ONLY the llm_guidance keys (structure preserved, product removed) → writer re-passes
  (gripper-selection-guide; idea.uk coordinated; matchmatrix verify). F8 lint must cover llm_guidance — the
