...
**Next, in order:**
1. **W4b — data-only chrome (read staged: `w4b_00_before.sql`).** Code fact (render_site_components_action.go:345–430): `renderAndStoreSiteComponent` joins the PINNED `site_components.component_id` with **no `is_active` filter**, and without `force_rerender` it SKIPS non-empty slots — so the **repoint must precede the force_rerender**, or it re-renders the old dark chrome from the inactive templates. Status: **REPOINT DONE** (header/footer → active fixed components; head pinned; stale HTML still serving). Trigger identified: **rerender-pages v6** (`force_rerender: true`, gated on `spec.refresh_site_components` — ours sets true). `w4b_03_read_rerender_config.sql` staged: full workflow config + real item metadata, so the trigger insert is crafted from actual shapes.
