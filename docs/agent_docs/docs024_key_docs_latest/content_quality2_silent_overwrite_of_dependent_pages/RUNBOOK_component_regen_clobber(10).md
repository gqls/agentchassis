...
- [ ] **F1-prompt Part 2** — Prompt rule written: `F1prompt_component_creator_preserve_field_names.sql` (top-level prompt, dormant `{{if .existing_field_names}}` block; snapshot-first, drift-checked). Safe to apply now
- [ ] **F1-prompt Part 1** — Feed `existing_field_names` to `generate_template` (needs `content_components` schema to choose: pre-generation lookup by section_type vs store-driven retry)
