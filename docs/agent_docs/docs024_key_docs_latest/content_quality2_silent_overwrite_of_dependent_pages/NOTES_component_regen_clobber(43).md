...
### 9ba. THIRD F8 CARRIER FOUND: per-field `llm_guidance` carries vonc's pitch with literal examples — mechanism closed

Locator (a): five llm fields carry gauntlet-flavoured llm_guidance — badge_label ("…feels live and game-like,
e.g. …"), description ("…what the daily game is — a provoc…"), heading (guidance containing the em-tag instruction
+ a gauntlet example), step_3_sentence ("…the Gauntlet reveals h…"), step_3_title ("…e.g. 'The Gauntlet Score[s the
Room]'" — the EXACT string the writer reproduced on BOTH sites). Writer prompt renders per-field guidance ⇒ every
writer pass on any site was instructed to write vonc's product — explains idea.uk 16:27 AND gripper-selection-guide
18:04 with ONE carrier, post-fallback-fix (which never touched llm_guidance). (b): NO content_brief column on
content_components (\d confirms) — the prompt's Admin Content Brief block is optional/absent; the earlier
some_schemas content_brief hit belongs to another table (plan/section side — user's site_plans instinct plausible;
not needed for this fix). User's "more local" guess was right.

NEXT (before writing): full-guidance dump issued (\o /tmp/guidance.txt: all llm_guidance values — sweep for vonc
concepts beyond the literal word: provocation/players/scoring/daily game — + current updated_at for the optimistic
lock; our Step-2 NOW() is the expected value). Then: snapshot v3 → per-field jsonb_set on ONLY the llm_guidance
keys (site-agnostic wording, structural guidance preserved: word counts, <em> instruction; other attrs untouched)
→ writer re-passes (gripper-selection-guide rebuild; idea.uk coordinated; matchmatrix rebuild verify — band still
12:46). F8 MITIGATION LIST GROWS: store-time lint must cover llm_guidance alongside fallbacks — guidance is the
highest-leverage contamination surface (actively instructs every future writer pass).

### 9bb. Full guidance dump: ALL ELEVEN llm fields carry vonc's product spec (product name = "Spark"); v3 + full neutralization issued

The gauntlet-LIKE filter undercounted: every llm field's llm_guidance is vonc's spec — heading: "communicate what
SPARK is in an arena-voice tone, e.g. 'Every day a new <em>Gauntlet</em> begins'"; illustration_alt: "Spark's daily
game arena"; steps 1–3: provocation drops / stake your position / the Gauntlet scores the room; description: "what
the daily game is". Lock value clean: updated_at 2026-07-03 17:17:38.725719 = our own Step-2 write. Issued:
snapshot v3 → CTE jsonb_object_agg patch replacing ONLY llm_guidance on the 11 fields (structural guidance kept:
word counts, <em> rule; product removed; other attrs untouched) with optimistic lock → verify strpos sweep for
gauntlet/spark/provocation (expect f|f|f) → capture the escalated needs_page spec shape (robot-hands, latest 2)
for cloning. \o clarified with the user: it writes inside the pod; host-side capture = kubectl exec … psql -c >
file. Remaining queue after f|f|f: clone needs_page for gripper-selection-guide (current copy gauntletised) +
how-it-works; notify other chat re idea.uk writer re-pass; verify matchmatrix band (still 12:46?). F8 lint scope
confirmed three-surface: fallbacks, merged content, llm_guidance (the strongest).

### 9bc. Runbook rewritten as a clean action document (requested); jsonb_object_agg explained

User asked: write the snapshot SQL into the runbook, clean it up with clear step position + check code per step.
Rewrote wholesale (old version kept as RUNBOOK_pre_cleanup_backup.md): YOU ARE HERE banner (Part C Step 6),
constants/conventions (incl. the kubectl -c > file capture pattern and the v1.0.1094 fleet caution), Part A/B
compacted to closed summaries with standing verify queries, Part C = the active F8 remediation as steps 1–12 with
SQL + expected + CHECK blocks at each step (v3 snapshot SQL and the 11-field CTE patch written in, with an inline
explanation of the jsonb_each → || patch → jsonb_object_agg → jsonb_set pipeline), Part D = R6f, Part E = flags +
hygiene. Also answered jsonb_object_agg in chat: the aggregate inverse of jsonb_each — folds (key,value) rows back
into a single jsonb object.

### 9bd. Steps 6–8 PASSED (2026-07-06, lock held across the idle days); component fully clean; Step 9 concretised

v3 snapshot INSERT 0 1 (versions 1/2/3 verified); the 11-field patch UPDATE 1 — the 17:17:38 lock held from 07-03
to 07-06 (nothing touched the component); verify f|f|f + the 11 neutral guidance strings. Component now clean of
all three F8 carriers. Step 8 shape: handler_agent=page-build-handler, item_key=needs_page:<page>,
spec={reason: content_data_backfill, page_name}. Step 11 folded early: matchmatrix band still 2026-07-01 12:46
with its needs_page 'complete' at 17:31 ⇒ rebuild never ran (complete-at-dispatch, 7th sighting). Step 9
concretised in the runbook: 9a re-triage matchmatrix + gripper-selection-guide (terminal→triaged; no open dups so
the dedup index is safe); 9b fresh needs_page:how-it-works MIRRORED column-for-column from gripper's row
(SELECT-based insert — no guessed columns) with the NOT EXISTS guard; CHECK = three fresh band stamps,
gauntletised=f. Then Step 10 (idea.uk via other chat) and Step 12 (final sweep: f except vonc; then live eyeballs +
backup drops). RUNBOOK_pre_cleanup_backup.md presented on request.

### 9be. Step 9a/9b executed (UPDATE 2, INSERT 0 1); progress query added to the runbook

The pre-run CHECK correctly shows the before-state (gripper 18:04 gauntletised, how-it-works 17:37 clean-but-old-
contract, matchmatrix 12:46). Added a PROGRESS query to Step 9: latest needs_page item per page via LEFT JOIN
LATERAL (avoids fan-out from older items) paired with the band stamp + a band_rebuilt boolean (> 2026-07-06 11:50)
+ gauntletised — with a reading guide that treats item status as untrustworthy (dispatch stamps) and the band stamp
as the proof; includes the stuck-claimed re-triage condition. PASS = 3× band_rebuilt t / gauntletised f. Remaining
after PASS: Step 10 idea.uk (other chat), Step 12 final sweep + live eyeballs + backup drops.

### 9bf. Step 9 progress: gripper-selection-guide + how-it-works REBUILT CLEAN; matchmatrix = SECOND no-op rebuild — page_type suspect

Progress read (13:15): gripper-selection-guide band 12:59:13 gauntletised=f (the last contaminated robot-hands page,
rebuilt under neutral guidance) + how-it-works 13:01:32 f — both items still 'claimed' (render/deploy legs or
status lag; band stamp is the proof). matchmatrix: both re-triaged items claimed in the SAME sweep (identical
item_stamp 12:45:15.963444 — batch claim), sibling's identical spec worked, yet matchmatrix went complete again
with the band untouched (07-01 12:46), no error ⇒ per-PAGE difference; prime suspect = page_type (MatchMatrix is a
robot-hands TOOL — the page-build-handler's content_data_backfill flow plausibly branches/skips tool pages,
completing without action; query D checked status only, never page_type). Issued: page_type read ×3 + matchmatrix
band cd_keys/len + (if tool-ish) the page-build-handler config dump via kubectl -c > file. FRAMING: matchmatrix is
NOT contaminated (f throughout) — re-triaged only for old-architecture content; if the skip is by design, the call
is leave-the-benign-old-band vs route through the tool-page flow — decide with the page_type fact, no forced third
re-triage.

### 9bg. STEP-12 SWEEP PASSES — contamination cleared board-wide; page_type falsified; matchmatrix parked-pending-check

Sweep 2026-07-06 ~13:25: f on EVERY brief-explanation dependent except vonc (its own copy) — idea.uk regenerated
clean TODAY 13:10:35/13:21:15 (other chat's pass or their pipeline; Step 10 done either way), both rebuilt
robot-hands pages hold. F8 contamination objective MET: three carriers neutralized at the component, every affected
page regenerated or stripped. page_type FALSIFIED (all three 'content', owned) — matchmatrix's double no-op isn't a
tool-page branch; with gripper ALSO having had empty cd and rebuilding fine, remaining discriminators are thin.
Reclassified matchmatrix as NON-contamination hygiene: issued jsonb_array_length(pages.sections) discriminator ×3
(0-vs->0 would indicate a planning-data gap — handler had nothing to write) + live eyeball; recommendation = park
unless ugly, no third re-triage. Runbook: steps 9/10 ticked, Step 12 = closeout (matchmatrix decision, live
eyeballs incl. the outstanding ai-agent case-study page, backup drops), banner updated. Remaining open work after
closeout: Part D (R6f styles.css vocabulary) + Part E flags.

### 9bh. matchmatrix DECIDED (sections=0 planning gap, legacy — parked); R6f CONFIRMED on the fresh rebuild; Part D promoted with D1/D2

Discriminator: matchmatrix sections_listed=0 vs siblings 9/5 ⇒ the page-build-handler iterates the planned section
list and had nothing to write — planning-data gap on a legacy page (repo: matchmatrix.html 2 months old) — PARKED
(options later: planner reconcile/adopt or retire). Both rebuilds fully deployed (deployed_at 13:40/13:44; repo
commit 54 min ago). gripperselectionguide.txt (uploaded, read): 55,633B artifact, :root=0, styles.css linked,
consumed vars dominated by the UNDEFINED vocabulary — --border-radius ×9, --section-border/-text/-text-muted/
-heading, --spacing-section, --color-heading, --container-max-width, --shadow, + NEW --hero-ink ×12 from the fresh
hero template ⇒ **R6f confirmed as the "renders badly" mechanism; fresh rebuilds render WORSE (more new-vocabulary
templates)** — Part D promoted to the active user-visible problem. Plan: D1 manual validation (site-wide consumed
list from DB regexp_matches + properly-greped defined list from /tmp/st.css; alias :root block drafted,
palette-mapped; one manual commit to the site repo; PASS = two pages themed on hard refresh; will be overwritten by
the next design pass — D2 makes it stick). D2 codify: post-pass in the styles.css write path (fix_harcoded_colours
precedent) + feed consumed vocabulary into webdesign generation; needs webdesign-agent config + storage_actions.go;
apply to vonc too. Part C residue: live eyeballs + the two backup DROPs.

### 9bi. Corrected diff (--section-* IS defined — my lossy grep owned); styles.css is a DETERMINISTIC Go template → D2 redesigned

Proper defined-list capture (39 names): the --section-* family IS defined — my earlier grep -A30 truncated exactly
there (the stray --section- fragment was the clue). Real gap = 11 names in two patterns: SYNONYMS (--border-radius/
--shadow/--spacing-section/--container-max-width vs --radius/--shadow-md/--section-pad-y/--container-max; legacy
--primary-color/--secondary-color/--accent-color) and ORPHANS (--hero-ink, --color-heading, --color-white,
--color-error). webdesignagent.txt (read): the flow is analyze_design (LLM → design-spec JSON: color_scheme/
typography/spacing) → **render_css_from_spec — "Render CSS from design spec using Go template (deterministic, no
LLM)"** → git_commit styles.css → spawn site-asset-renderer. So the defined vocabulary lives in ONE Go template =
the single home for the generic fix; storage_actions' three styles.css writes are the OLD builder extract paths
(html-developer/generic) — NOT this flow, don't patch there. D2 redesigned: D2a alias block in the Go CSS template
(list drafted, canonical-mapped); D2b component-creator prompt canonical-token rule + optional store WARN lint;
D2c rollout = next design pass per site (CAUTION: re-run = fresh LLM spec → palette can shift unless pinned via
design_intent/design_reference) with the manual bridge commit as the palette-preserving interim for robot-hands/
vonc. NEEDED: render_css_from_spec action source (grep issued). COORDINATION FLAG: webdesign-agent row updated
TODAY 13:22 on v1.0.1096 — parallel chats actively on this agent; re-read at patch time. User direction honoured:
generic fix, not per-site.

### 9bj. D2a patch staged: buildTokenAliases as step 11 of RenderCSSFromSpecAction (+ tests + diff)

Discovery in the action: the :root vocabulary lives in DB LAYOUT TEMPLATES (comp.LayoutTemplate, parsed at L182;
themes forkable via fork_theme_from_site) — patching layouts would be per-row and drift-prone. The file also
contains the exact precedent: step 10 buildSectionDefaults = "renderer-enforced" token guarantees (which is WHY
--section-* showed as defined). Patch = step 11 sibling: buildTokenAliases(renderedCSS, logger) after
sectionDefaults, appending a trailing :root block with ONLY the missing names — definition detection =
strings.Contains(css, name+":") so var() usages ("--shadow)") and sibling names ("--shadow-md:") don't count;
idempotent (second pass sees own definitions → ""); package-level tokenAliases table (11 names, ordered,
palette-safe values: synonyms→canonical var() refs, orphans→color-text/literals); one zap field added to the
existing rendered log (token_alias_length); helper logs appended names at Info. NO variable renames. gofmt clean
(action + test). Files: render_css_from_spec_action.go (patched full file), render_css_from_spec_alias_test.go
(6 table tests: empty-css-appends-all, defined-skipped, all-defined-empty, usage-not-definition,
sibling-not-definition, idempotency), PATCH_render_css_token_aliases.diff. TO SHIP: coordinate (webdesign agent
touched TODAY 13:22, fleet v1.0.1096) → build → Actions → image bump; sites self-heal on their next design pass;
robot-hands/vonc interim = the manual bridge commit if wanted before then. D2b (creator-prompt canonical-token
rule + store WARN lint) remains.

### 9bk. D2a deployed to production; verification plan staged (gamesdesign vehicle)

User confirms the Go code is deployed. Verification: trigger a webdesign pass on gamesdesign (test site — fresh
LLM design spec is harmless there; robot-hands/vonc keep the palette-shift caution) via a needs_design work item —
shape-read query issued first (never guess a spec), clone drafted after. Post-run checks: curl the committed
styles.css for the trailing compatibility-aliases :root block; opportunistic pod-log grep for the new Info line
("appending compatibility token aliases", token_alias_length field). Also: confirm agent_definitions image_tag
moved past v1.0.1096. Remaining Part D after verify: D2b (component-creator canonical-token prompt rule — F1prompt
migration pattern — + store-time WARN lint) and the robot-hands/vonc choice (natural-pass self-heal vs
palette-preserving manual bridge commit). Part C residue unchanged (live eyeballs, backup DROPs); Part E flags
unchanged.

### 9bl. Verify vehicle: build-site-planner is the sole needs_design producer; use the real path or standalone webdesign-agent

Re-confirmed the operating agreement (reuse-first, schema-before-SQL, structural-first, thin workflows/Go complexity,
var-name discipline, no logger.Debug, ai-persona-system/kafka namespaces, GitHub→Actions→Backblaze, orchestrator-
responds-to-parent-topic, no summary docs). Reading guideline sections before agent/workflow/action changes
(dev-guide constraint-values + orchestration read this turn). Facts: needs_design produced ONLY by
build-site-planner (4 items, latest 12:39, spec={}, handler self-loads) → conforming trigger = the real path, not a
manual INSERT (dev-guide: verify-at-point-of-use / reuse the real mechanism; avoids dedup/constraint surprises).
gamesdesign curl empty = no design pass since the D2a deploy (stylesheet still last month's; matches repo) — NOT a
patch failure. webdesign-agent now v1.0.1098 (18:55, parallel chats). Issued: build-site-planner default_config +
input_contract read — to choose (a) its design-only entry if any (whole-site plan would be too heavy, may
regenerate gamesdesign content) vs (b) a direct standalone webdesign-agent spawn (start_step check_site_context →
load_site_context from input_data.domain/site_id → exercises render_css_from_spec alone = the changed code). Then
curl gamesdesign styles.css | grep 'compatibility aliases' + opportunistic pod-log Info line.

### 9bm. D2a verify vehicle chosen: adapt the proven 076 webdesign trigger for gamesdesign (reuse, not invent)

Read the two uploaded triggers from disk (docs empty again). 076_webdesign_trigger = self-contained CLI workflow:
spawn_agent(webdesign-agent, role webdesigner) → call_agent input_mapping {site_id:input_data.site_id,
domain:input_data.domain} timeout 240 → complete; posts to system.agent.generic.requests with the full header set
and responses_topic=system.agent.generic.responses. This runs render_css_from_spec (our step 11) WITHOUT a full
plan — the narrow vehicle. 081_build_site_planner = the heavier orchestrate path (whole plan) — not needed.
Adapting 076: swap ONLY site identity to gamesdesign; every header/topic/workflow kept verbatim (no var-name
drift). Guideline verify-at-point-of-use → confirm gamesdesign site_id via SELECT (memory had
e33263f4-… but verifying) before filling SITE_ID. CAUTION logged: webdesign-agent start_step check_site_context →
analyze_design RE-RUNS the LLM design step → rewrites gamesdesign's stylesheet from a fresh spec (fine on the test
site; the check is purely whether the trailing compatibility-aliases :root block appears). Monitor grep:
RenderCSSFromSpec|compatibility token aliases|token_alias_length|git_commit. Verify: curl gamesdesign styles.css
?cb=2 | grep -A14 'compatibility aliases'. PASS = alias block present listing the layout's undefined names.

### 9bn. D2a VERIFIED in production — step 11 emits the alias block on a real gamesdesign pass

Adapted 076 trigger (site_id e33263f4… verified via SELECT per verify-at-point-of-use) published to
system.agent.generic.requests (corr d8d4cdd4…, orch 39d1144c…); kcat produced+exited clean. curl gamesdesign
styles.css (cache-busted) returns the trailing "/* renderer-enforced compatibility aliases (component vocabulary
bridge) */ :root { … }" with ALL 11 names present → tool-portal-dark layout defined none, so buildTokenAliases
emitted the full set (the definition-detection + full-set-on-empty path both exercised live). Full deploy chain
confirmed working end-to-end (spawn_agent → call_agent → analyze_design → render_css_from_spec[step 11] →
git_commit → Actions → Backblaze → served). R6f drift now SELF-HEALS on every webdesign pass; gripper-detail's
dark-on-dark mechanism closed at source for future passes. NOTE: the pass re-ran analyze_design (fresh LLM spec) on
gamesdesign — fine (test site). Remaining Part D: D2b (component-creator canonical-token prompt rule — F1prompt
migration pattern — + store-time WARN lint on unknown var(--…) names, F8's sibling) and the robot-hands/vonc choice
(next natural pass self-heals; or the palette-preserving manual bridge commit for an immediate fix). Part C residue
(live eyeballs incl. the outstanding ai-agent case-study page; two backup DROPs) and Part E flags unchanged.

### 9bo. Runbook clarified (requested): D2b/D-sites/Part-E flags rewritten with plain WHY/WHAT/HOW

Rewrote the shorthand into plain descriptions per request: D2b (prevention — canonical-token prompt rule to stop
new orphan tokens at the LLM source; why D2a alone is insufficient; optional store WARN lint), D-sites (heal
robot-hands/vonc now via a palette-preserving manual bridge commit vs let them self-heal on next pass; the
analyze_design palette-shift caution spelled out), and Part E flags each given WHY (the root-cause weakness it
addresses, tied to what happened this episode) + WHAT (the change). Folded the old D2c rollout note into D-sites.
No code/db change this turn — documentation clarity only. Options laid out for the user to choose the next thread;
suggested order D2b → D-sites → Part E (F8 first).

### 9bp. D2b reshaped reuse-first from the guidelines (contracts already own the rule set + a validation fn)

Read dev-guide §1 (reuse-first agent-creation checklist; line 71 = the default_config ILIKE method for finding
which agent owns behaviour) + contracts §template-rules. FINDINGS that reshape D2b: contracts already define
(a) a numbered "New component checklist" (1–10, the LLM/human-facing template rules) and (b) three validation
functions incl. **ValidateComponentTemplate(function, htmlTemplate)** (currently checks data-component). So D2b must
EXTEND these, not add a parallel prompt rule / new store check. Subtlety: checklist item 6 already says dark
sections MUST SET --section-* vars; R6f is the reciprocal gap — templates CONSUME tokens the theme never DEFINES —
so D2b adds "may only CONSUME sanctioned tokens" (D2a's alias list now guarantees they exist). Not a duplicate.

D2b = three conforming edits: (1) contracts doc — add rule 11 "CSS custom properties: reference only canonical
tokens + sanctioned aliases; invent no new var(--…)" (draftable now, it's the source of truth the prompt mirrors);
(2) the component-writing agent's prompt — extend its template-rules block in place (workflow-jsonb, immediate) —
but FIRST identify the agent via `SELECT type FROM agent_definitions WHERE default_config::text ILIKE
'%data-component%' OR '%html_template%'` (StoreGeneratedComponentAction persists the LLM output → the generator is
upstream; don't assume the type name), then dump its prompt step; (3) extend ValidateComponentTemplate to WARN on
unknown var(--…) tokens (reuse, not a new store check) — needs the file (grep -rln ValidateComponentTemplate
platform/ → likely contracts.go). No variable renames. Issued: the ILIKE identify query + a request for the
ValidateComponentTemplate file.

### 9bq. D2b in progress: lint coded (AuditTemplateTokens, warn-only) + tests; contract rule 11 drafted; prompt edit pending agent-identify

Read component_validation.go: self-contained contract-enforcement file (ValidateComponentFunction/Template,
Normalize*), header lists call sites. Reuse point confirmed. Added AuditTemplateTokens(function, htmlTemplate,
logger) + canonicalCSSTokens map + cssVarRefRe — WARN-ONLY (returns unknown var(--…) names first-seen deduped,
never errors; logs one Warn when logger!=nil and unknowns exist). Rationale for warn-not-error: ValidateComponent-
Template returns error at persistence points; making unknown tokens fatal would block legit saves as vocabulary
evolves — detection net, not gate. allowlist = theme-defined vocabulary (from the 39-name defined list) + the 11
buildTokenAliases aliases, kept in sync with D2a as the canonical source. gofmt-clean; 7 table tests (canonical
pass, aliases accepted, unknown detection, dedup+order, empty, nil-logger no-panic, whitespace/fallback var forms;
regex var\\(\\s*(--[a-z0-9-]+) verified against 'var( --x )' and 'var(--x, fb)'). PENDING call-site wiring in
StoreGeneratedComponentAction (where ValidateComponentTemplate already runs) — needs that file.

D2b-1 contract rule 11 DRAFTED (canonical tokens + aliases; no new var(--…); references RenderCSSFromSpecAction/
buildTokenAliases/AuditTemplateTokens) — delivered as a paste-in patch (003_contracts is read-only in /mnt/project).
D2b-2 prompt: 21 agents matched the CSS ILIKE — issued a scoped prompt dump (component-creator, component-template-
fixer, section-editor, landing-page-builder) to identify the CSS-generating step before editing in place; won't
guess. robot-hands EXCLUDED from D-sites (another thread rebuilding it) — D-sites is vonc-only now. Files:
component_validation.go (patched), component_validation_token_audit_test.go.
