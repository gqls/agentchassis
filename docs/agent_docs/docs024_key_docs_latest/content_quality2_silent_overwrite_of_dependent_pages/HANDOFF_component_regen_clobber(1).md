...
- **Five affected instances** (all `component_id` = `fdd92ad4`, all `build_status` = `pending`,
  `component_version_id` = NULL, all `rendered_html` empty). Their `updated_at` of 15:06:12.956 is the
  `build_status='pending'` UPDATE from `update_component_html` (≈16ms after the component's own update) — the
  empty render itself was a **separate** write (see §4 / RUNBOOK R3a), so don't read that one timestamp as the
  moment of emptying. Page-id prefixes: `b2edae00`, `0747e2fc`, `9baea9f9` (each an `index` page on a
  **different** site), `77f10b13` (`case-study-kafka-consumer-group-remediation`), `11364960` (`gripper-detail`).
  Drive enumeration off `component_id`, not the prefixes (RUNBOOK R2).
...
- `rerender_page_sections_action.go` → `RerenderPageSectionsAction` — the no-LLM re-render. It loads stored
  ...
  renamed template.
- **OPEN — which render step wrote the blanks.** `update_component_html` doesn't render; `rerender_page_sections`
  carries / is gated. Prime suspects: `component-creator`'s own dependent re-render (likely via
  `RenderComponentAction`, the canonical render-and-persist path that `rerender_page_sections`'s comments
  reference) and whatever consumes `build_status='pending'`. Pin it (RUNBOOK R3a) before fixing.
- `platform/orchestration/actions/component_selector.go` — the "component has empty `input_schema` → needs
  regeneration with content fields" trigger logic, plus `IncrementUsageCount`, `SelectComponentByType`.
