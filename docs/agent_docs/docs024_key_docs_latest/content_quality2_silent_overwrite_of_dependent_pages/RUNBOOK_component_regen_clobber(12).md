...
### Tier 2 — Rejection path (integration, test DB)

A Go integration test against a throwaway component that drives `StoreGeneratedComponentAction` with a
`generated_template` whose `input_schema` **renames** a retained field. Set up: insert a throwaway
`content_components` row (below); build `ActionParams` with the generated_template under the step's collected-data
key. Assert: the call returns an error containing `regeneration removes/renames`; an `agent_error_log` row is
written (via `recordValidationRejection`); and the component row is **unchanged** (`updated_at`, `input_schema`,
`html_template` identical; no new `component_versions` row). This is the deterministic proof of the fail-safe —
preferred over trying to force a rename through the LLM end-to-end.

### Tier 3 — End-to-end smoke on a throwaway component

Setup (namespaced `zzz-fieldguard-test`, trivially cleaned up):
```sql
INSERT INTO content_components
  (name, function, section_type, component_level, is_active, forked_from, html_template, input_schema, created_from)
VALUES ('zzz-fieldguard-test','zzz-fieldguard-test','zzz-fieldguard-test','section', true, NULL,
  '<style>.zzz-fieldguard-test-section{padding:2rem}</style><section class="zzz-fieldguard-test-section" data-component="zzz-fieldguard-test"><p class="eyebrow">{{placeholder "eyebrow"}}</p><h2>{{placeholder "heading"}}</h2><p>{{placeholder "body"}}</p></section>',
  ...
  'manual');
```
(Optional dependent, to also prove non-stranding + later F3 scoping: insert a throwaway page + a `page_components`
row for this component with `content_data` populated on `eyebrow`/`heading`/`body`.)

Trigger a regeneration — insert a `needs_new_component` work item for this section_type (needs a valid `site_id`;
`status='triaged'` so the dispatch loop claims it):
```sql
...
VALUES ('<TEST_SITE_UUID>', 'f2-test', 'build', 'needs_new_component', 'medium',
        'F2 regen test', 50, 'component-creator', 'triaged', 'f2-test',
        '{"section_type":"zzz-fieldguard-test","description":"test stats band","site_type":"test"}'::jsonb,
        'f2_regen_zzz-fieldguard-test');
```

Observe (happy path — names preserved, guard passes):
```bash
kubectl -n ai-persona-system logs -l app=agent-component-creator --tail=800 \
  | grep -iE "load_existing_component|field-name preservation|REGENERATION FIELD-NAME|regeneration removes/renames"
```
- `load_existing_component` logs "found existing component — requesting field-name preservation" (`field_count=3`).
- the rendered prompt contains the REGENERATION FIELD-NAME RULE with `eyebrow, heading, body` (check pod logs /
  `llm_call_log`).
```sql
-- fields preserved after regen?
SELECT array_agg(k ORDER BY k)
FROM content_components c, jsonb_object_keys(c.input_schema->'fields') k
WHERE c.section_type='zzz-fieldguard-test' AND c.forked_from IS NULL;
-- expect eyebrow, heading, body still present (possibly + new fields)

-- no field-contract rejection on the happy path
SELECT created_at, error_message FROM agent_error_log
WHERE error_message LIKE '%regeneration removes/renames%' ORDER BY created_at DESC LIMIT 5;
```

Cleanup:
```sql
DELETE FROM component_versions WHERE component_id IN (SELECT id FROM content_components WHERE section_type='zzz-fieldguard-test');
DELETE FROM page_components   WHERE component_id IN (SELECT id FROM content_components WHERE section_type='zzz-fieldguard-test');
DELETE FROM content_components WHERE section_type='zzz-fieldguard-test';
DELETE FROM site_work_items    WHERE spec->>'section_type'='zzz-fieldguard-test';
```

Done-when: Tier 1 green; Tier 2 shows the rename rejected with an `agent_error_log` row and the component untouched;
Tier 3 shows `load_existing_component` feeding the names, the prompt carrying the rule, the regen preserving
`eyebrow/heading/body`, and no rejection on the happy path.
