...
## Step R5b — Unblock section readiness (the required `cta_url` with no site spec)

Established 2026-07-02: the five re-renders ran and CARRIED because `cta_url` (`required: true`,
`source: site_specs.cta.primary_url`, no fallback) cannot resolve — **no site has a `cta` spec** (both shapes
checked, 0 rows). This is a second facet of the regen bug (F5 below): the regeneration ADDED a required Tier-C
field the dependent sites can't satisfy, so even correctly re-keyed content can't re-render.

Decide the remedy with one read (also re-checks freshness before a component write):
```sql
SELECT updated_at,
       (html_template ~ '\{\{\s*if\b') AS has_if_blocks,
       substring(html_template FROM GREATEST(position('cta' IN html_template) - 250, 1) FOR 700) AS cta_region
FROM content_components WHERE id = '<COMPONENT_UUID>';
```

**Branch A — the CTA block is `{{if .cta_url}}…{{end}}`-wrapped:** flip the requirement off; the section renders
cleanly without the CTA. Backup first, then:
```sql
CREATE TABLE IF NOT EXISTS _bak_compregen_component AS
  SELECT *, now() AS _bak_at FROM content_components WHERE id = '<COMPONENT_UUID>';

UPDATE content_components
SET input_schema = jsonb_set(input_schema, '{fields,cta_url,required}', 'false'::jsonb),
    updated_at = NOW()
WHERE id = '<COMPONENT_UUID>';
```
Then re-trigger the five (the same C2 `needs_rerender` inserts — the completed items don't block re-insertion,
`complete` is excluded by the dedup guard) and verify with R6b.

**Branch B — the CTA is a bare substitution (no `{{if}}`) — CONFIRMED 2026-07-02 (`has_if_blocks=f`).**
Relaxing `required` would ship a dead styled `<a href="">` button (the `.stats-cta` class), and no single fallback
URL is correct across sites (phantom-CTA lesson: never invent URLs). Convention read from
`plan_sections_action.go`: `ensureSpecs` loads `SELECT aspect, data FROM site_specs WHERE site_id=$1 AND
is_current=true` into `specs[aspect]`; `resolveSpecPath("cta.primary_url")` reads `specs["cta"]["primary_url"]`;
and in `planSection`, required + no fallback + unresolvable = defer (deterministic). Sequence:

1. Pick a real CTA target per site (pages.url is the link format):
```sql
SELECT s.domain, p.name, p.url, p.page_type, p.status, p.nav_order
FROM pages p JOIN sites s ON s.id = p.site_id
WHERE s.domain IN ('<D1>','<D2>','<D3>')
ORDER BY s.domain, p.nav_order, p.name;
```
2. **Verify the chosen URLs exist as pages first** (the resolver does NOT validate them — a wrong value renders a
   404 button and bakes into `rendered_html` + `content_data`, costing another render pass to fix). If a spec was
   inserted with an unverified URL, correct it with an UPDATE on the `cta` aspect row (`jsonb_set(data,
   '{primary_url}', …)`) — the `(site_id,'cta')` current slot is occupied after the insert.
3. Insert the specs (the `(site_id, aspect) WHERE is_current` slot is free — verified 0 rows):
```sql
INSERT INTO site_specs (site_id, aspect, data, source, created_by, is_current)
SELECT s.id, 'cta', jsonb_build_object('primary_url', v.url),
       'manual-recovery', 'manual-recovery', true
FROM (VALUES ('<D1>','<URL1>'), ('<D2>','<URL2>'), ('<D3>','<URL3>')) AS v(domain, url)
JOIN sites s ON s.domain = v.domain;  -- expect INSERT 0 3
```
4. Re-trigger the five (the same C2 `needs_rerender` inserts; prior items are `complete`, excluded by the dedup
   guard — expect `INSERT 0 3`).
5. R6b verify: `has_stat1_label`/`has_headline` true ×5, five DISTINCT md5s, `cta_href_empty` false. The resolver
   also persists the resolved `cta_url` into each row's `content_data` (stored ⊕ resolved merge) — expected.
...
WHERE w.item_key = 'recover_system_stats:<COMPONENT_UUID>'
ORDER BY s.domain;
...
FROM pages p JOIN sites s ON s.id = p.site_id
WHERE p.id IN (SELECT page_id FROM page_components WHERE component_id = '<COMPONENT_UUID>')
ORDER BY 1, 2;
```
```bash
...
WHERE sp.is_current AND sp.aspect = 'cta' AND s.domain IN ('<D1>','<D2>','<D3>');
SELECT s.domain, sp.aspect, jsonb_pretty(sp.data->'cta') AS cta_obj
FROM site_specs sp JOIN sites s ON s.id = sp.site_id
WHERE sp.is_current AND sp.data ? 'cta' AND s.domain IN ('<D1>','<D2>','<D3>');
```
