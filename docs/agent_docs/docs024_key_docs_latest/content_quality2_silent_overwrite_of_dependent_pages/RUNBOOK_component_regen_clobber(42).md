...
_Action document. History/detail: NOTES_component_regen_clobber.md (§1–9bb) and HANDOFF. Updated 2026-07-04._

## YOU ARE HERE → Part C, Step 6 (snapshot v3), then Steps 7–12
Parts A and B are complete and verified. Part C steps 1–5 are done. Part D (theming) and Part E (flags) queue behind.
...
- [ ] **Step 6 ← NEXT — snapshot v3** (before touching the schema again):
  ```sql
...
- [ ] **Step 7 — neutralize llm_guidance ×11** (lock on `2026-07-03 17:17:38.725719+00` = our Step-2 write):
  ```sql
...
- [ ] **Step 8 — capture the escalated `needs_page` spec shape** (never guess a spec):
  ```sql
...
- [ ] **Step 9 — clone rebuilds** for `gripper-selection-guide` (current copy is gauntletised) and `how-it-works`
  (old-architecture content). SQL to be written FROM the Step-8 shape (item_key/spec/handler mirrored; dedup
  NOT EXISTS as in Step 4; fresh item_key suffix since prior ones are terminal).
...
- [ ] **Step 11 — matchmatrix verify** (its rebuild was dispatched but unproven — band stamp decides):
  ```sql
  SELECT pc.updated_at AS band_stamp, w.status AS needs_page_status, w.updated_at AS item_stamp
  FROM page_components pc
  JOIN content_components cc ON cc.id=pc.component_id
  JOIN pages p ON p.id=pc.page_id JOIN sites s ON s.id=p.site_id
  LEFT JOIN site_work_items w ON w.site_id=s.id AND w.item_type='needs_page'
        AND w.spec->>'page_name'='matchmatrix'
  WHERE s.domain='robot-hands.com' AND p.name='matchmatrix' AND cc.function='brief-explanation';
  -- band_stamp still 2026-07-01 12:46 = rebuild never ran → re-triage the needs_page item (status='triaged')
  ```
