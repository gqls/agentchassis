...
- [ ] **R5b** — Unblock readiness: `cta_url` required + no `cta` site spec anywhere → CARRY loop. One read decides Branch A (`{{if}}`-wrapped → required:false) vs Branch B (bare → per-site `cta` spec, resolver convention first); then re-trigger + R6b
...
**Branch B — the CTA is a bare substitution (no `{{if}}`):** relaxing `required` would ship a dead
`<a href="">` button, and no single fallback URL is correct across all sites sharing the component (the
phantom-CTA lesson: never invent URLs). Go the content-correct route instead: insert a real `cta` spec per site —
but FIRST read the resolver's exact `site_specs.*` convention (aspect naming + data shape) from
`plan_sections_action.go` (`newSourceResolver` / the `site_specs.` resolution path) before writing any aspect
rows. Do not guess the aspect convention.
