...
while the `needs_rerender` it raises is assemble-only. Concrete change set (patch in this folder:
`F1_store_generated_component_action.patch`; `gofmt -e` clean):
...
1. **Field-contract guard (the fix) — reject a regeneration that renames/removes a retained field.** Added to the
   ...
   stranding dependents. New fields are allowed (additions don't trigger it). New helper `schemaFieldSet` (mirrors
   `deriveRenderMode`'s `fields` parse) and a `sort` import. Comparing old-schema↔new-schema is a proxy for "what
   dependents have" (content_data is written to match the schema); a comment notes the exact alternative
   (query dependents' `content_data` keys) if ever needed.
2. **Rerender reason.** `createRerenderWorkItem`'s spec now carries `"reason": "section_data_resolved"` so a
   legitimate restyle (field names preserved) actually re-renders dependents' sections from their `content_data`
   against the new template, rather than the assemble-only re-ship. **Dependency:** this only takes effect if
   `rerender-pages` propagates `spec.reason` onto the per-page `page_rerender` items — verify/align its
   `create_rerender_items` step (a one-line companion change there if it currently drops the reason).
...
whose config/image changes (`snapshot_agent`).
...
2. With the companion prompt change in place (names preserved), regenerate so only the template markup changes;
   assert the store succeeds, the triggered re-render carries `reason=section_data_resolved`, and the dependent's
