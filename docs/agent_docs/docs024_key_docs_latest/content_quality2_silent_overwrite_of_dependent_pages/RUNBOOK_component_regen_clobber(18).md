...
regenerating the template. F2 is complete (Tier 1 unit, 3a keep, 3b reject). What's left: F2 test-row cleanup;
deploy **F3** (patches ready: F3a `create_rerender_items` scoped reason, F3b store reason re-add — Go first, then
the F3c `rerender-pages` config SQL); then recover the five (R4 backup → R5 → R6). Until F3 ships, recovery does
**not** route the section re-render through `rerender-pages` (it drops the reason): use a full `needs_page` rebuild,
or re-key + a directly-inserted `page_rerender` carrying the reason.
...
- [ ] **F2-cleanup** — remove the 3b fixture + all `f2_reject_*` items (SQL in the Tier 3b redo block, steps 0–1)
...
- [ ] **F3** — Code APPLIED in repo (verified 2026-07-02: F3a byte-identical, F3b exact hunk, both gofmt-clean). Pending: ensure a build made AFTER these changes is deployed (v1.0.1086 provenance uncertain — rebuild + bump if in doubt), then apply F3c (`rerender-pages` config SQL — pasted row confirms `reason`/`component_id` still absent from the `create_rerender_items` step config)
