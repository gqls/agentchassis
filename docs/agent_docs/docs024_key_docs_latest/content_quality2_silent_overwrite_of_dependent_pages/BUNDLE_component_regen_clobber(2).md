...
- **`<file>:StoreGeneratedComponentAction`** — TO ADD. `store_generated_component` (component-creator's store step)
  maps to `StoreGeneratedComponentAction`; not in this bundle or the uploaded files. It is the **prime suspect for
  the rename write** (it stores the regenerated template/schema). Locate and add it:
  `grep -rn "func StoreGeneratedComponentAction" ~/projects/agentchassis`, then `-scope <file>:StoreGeneratedComponentAction`.
