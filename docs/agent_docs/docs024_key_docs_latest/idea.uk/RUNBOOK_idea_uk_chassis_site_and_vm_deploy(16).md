...
**Route 1 — fresh staging rebuild (recommended; also validates the migration end-to-end).** Submit idea.uk
to a staging `site_id` (Phase 1 below). The migrated classifier runs and now writes the structured blocks,
composition reads them, and you get a clean parchment build to look at — the existing site and its
composition left untouched. Confirm the classifier output on the staging site:
...
"SELECT data->'palette'->'reference_values' AS palette,
 ...
 FROM site_specs WHERE site_id='‹staging_site_id›' AND aspect='design_intent' AND is_current;"
```
...
Reset + re-resolve — inspect the composition tables first; these deletes are destructive and FK-bound,
so do **not** delete blind:

```bash
kubectl exec -n ai-persona-system postgres-clients-0 -- psql -U clients_user -d clients_db -c "\d css_themes"
kubectl exec -n ai-persona-system postgres-clients-0 -- psql -U clients_user -d clients_db -c "\d style_collections"
...
"SELECT item_type, handler_agent, status, item_key, priority
 FROM site_work_items WHERE item_type IN ('needs_composition','needs_design') ORDER BY created_at DESC LIMIT 5;"
# Then, once the FK relationships are clear from the \d output: clear sites.style_collection_id, remove this
# site's css_theme / style_collection rows, and enqueue needs_composition (+ needs_design) mirroring the rows
# above (item_key unique per idx_swi_dedup). Fill these in from \d before running.
```
