...
**23. A rubric that doesn't score the *consequence* of being wrong will rank dangerous candidates the same as safe ones — score operator-side risk separately, gate it, and surface what got dropped loudly.** Concrete instance (2026-05-28): the v2 ideation method (`idea_uk_method_v0.md`) scored agritec/SFI single-farm assessment Def 4 / Will 4 / Build 3 / Reuse 2 / Dur 4 — sum 17, **test-now**, a confident recommendation to build it. The score sum is fitness (how attractive is the opportunity), and the rubric had no dimension for *what it would cost the customer if our output were wrong*. A wrong action stack in a £49 report could cost a farmer £5k–50k of lost grant money; the rubric couldn't distinguish that exposure from a same-sum candidate where the worst case was wasted time. The product was paused on operator instinct, not on the rubric, which is the wrong end to catch it from — instinct doesn't scale to other verticals (finance, medical adjacency, legal-adjacent). The fix was to add a sixth factor — **Risk to the operator**, 1–5, 5 = safest, scoring the *consequence* of being wrong, not the probability: 5 = pure analysis, customer decides, no loss beyond fee paid; 4 = minor, refunds make whole; 3 = meaningful decisions, customer can verify citations, PII recommended; 2 = high-stakes / regulated-adjacent, needs human review + PII + reviewed T&Cs before build; 1 = regulated profession (medical / legal / FCA financial advice) — do not build without qualifications. Crucially, Risk is **NOT added to the sum** (sum is fitness; Risk is hazard; mixing them hides exposure) and **NOT in the gate** (the Def ≥ 3 AND Will ≥ 3 gate is fitness-only). Instead: **Risk = 1 is dropped automatically** and listed in a separate "Dropped for operator risk" section in the output so the operator sees what got killed for risk vs what failed the gate; **Risk ≤ 2 still advances** if it clears the gate but is flagged "needs liability work before building" and the candidate's `cheapest_test` is required to say "validate demand first; PII + T&Cs reviewed by a UK solicitor required before any build"; **Risk acts as a tiebreaker** in the rank (prefer safer at equal fitness, but doesn't outweigh fitness). The generalisable lesson: when a scoring system recommends actions for an operator who carries downstream exposure for being wrong, the operator's risk must be a separate scored dimension. Fitness scores cannot capture hazard, and a high-fitness/high-hazard candidate looks identical to a high-fitness/low-hazard one without it — so the dangerous one gets recommended with the same confidence. The follow-on disciplines: (a) when extending a rubric, check whether the new use case introduces a new failure mode the existing factors can't see — adding the case to the rubric, even with hindsight, is cheap; relying on instinct to catch it is fragile; (b) surface what got dropped, separately and visibly — a silent filter that just removes Risk = 1 candidates hides the fact that the method considered them, and the operator loses an audit signal; (c) keep hazard out of fitness sums — averaging them obscures both. Implementation: `idea-go/prompts.go` `scorePrompt` (rubric + JSON field), `idea-go/engine.go` `scored` struct + score-step pipeline + `render` + `riskNote`, `idea_method_runner.py` Python parity, `idea_uk_method_v0.md` (the method doc, v3, with the Risk rubric and rules), `idea_method_prompt.md` (the single-shot prompt). What the "needs liability work" flag in the output is asking the operator to follow up on lives in `LIABILITY_AND_TERMS.md` (risk analysis, mitigations, draft T&Cs). The Phase C swap in `DEVELOPMENT_RUNBOOK.md` (SFI single-farm assessment paused; SFI26 Diff Alerts substituted, Risk 4) is the first product decision the new column actually changed.

**24. A hosted/server tool that auto-injects a helper tool collides if you also declare that helper — check what a server tool pulls in before adding your own.** Concrete instance (2026-06-04): enabling `web_search_20260209` (the v2 web-search tool, with dynamic filtering) and *also* declaring `code_execution_20250522` in the same `tools` array returned `400 invalid_request_error: "Auto-injecting tools would conflict with existing tool names: ['code_execution']. Each tool name must be unique."` The v2 web-search tool injects its own `code_execution` tool server-side to filter results before they enter the context window — that injection *is* the dynamic-filtering mechanism. The docs phrase it as the v2 search "requires the code execution tool to be enabled," which reads like "add it yourself"; in fact the search tool enables it for you. The fix was to pass only `web_search_20260209` and remove the explicit `code_execution_20250522` — the filtering still runs (and is still free, as it's the search tool's own machinery). Discipline: when a hosted/server tool documents a dependency on another tool, assume it may inject that dependency itself and verify before adding it to your `tools` array — the 400's message naming the exact conflicting tool (`['code_execution']`) is the tell. The same will apply to any future server tool that composes other server tools.

**25. Extended thinking has different request shapes across model generations — a call helper that hard-codes one model's parameter shape breaks the moment a step points at another family.** Concrete instance (2026-06-04): after bumping the verify step to `claude-opus-4-8`, the call 400'd with `"thinking.type.enabled" is not supported for this model. Use "thinking.type.adaptive" and "output_config.effort" to control thinking behavior.` Opus 4.7 and 4.8 (and Mythos) dropped *manual* extended-thinking budgets (`thinking:{type:"enabled",budget_tokens:N}`) in favour of *adaptive* thinking (`thinking:{type:"adaptive"}` + `output_config:{effort:"low|medium|high|xhigh|max"}`); the manual form now 400s on those models. Sonnet 4.6 still accepts the manual form — which is precisely why the cut step (Sonnet 4.6) had just succeeded on the *same* `callClaudeOpts` code path that the verify step (Opus 4.8) rejected in the very same run. The single helper had assumed every model took `enabled`+`budget_tokens`. The fix: branch on the model string — a `usesAdaptiveThinking()` predicate returns true for `opus-4-7`/`opus-4-8`/`mythos` (emit adaptive + `output_config.effort`) and false otherwise (emit manual budget, clamped to `< max_tokens` with room left for output). Disciplines: (a) the same *capability* is not the same *wire format* across model generations — when you swap a model string, re-check that model's request-shape constraints, because "a one-line model swap" is rarely one line when thinking or sampling parameters are in play; (b) the diagnostic tell here was sharp and worth recognising — two steps running the "same" thinking code diverged *purely by which model they targeted*, a strong signal the parameter is model-conditional, so look at per-model API differences before suspecting your own logic; (c) carry-over constraints in the same family bite quietly: Opus 4.7/4.8 also reject non-default `temperature`/`top_p`/`top_k` (400), so a helper that always set those would fail on Opus while passing on Sonnet for the same structural reason. Adjacent to #16 (don't change a value earlier evidence proved correct) but distinct — here the value (`thinking.type: enabled`) was genuinely correct for one model and genuinely wrong for another, in the same codebase at the same moment, decided only by the model field.

**26. A long agentic call (extended thinking + a server-side tool loop) holds the connection for minutes before any response headers — the HTTP client timeout must allow for the worst-case step, not the average.** Concrete instance (2026-06-04): with the verify step on Opus 4.8 at `xhigh` effort plus up to 6 web searches, the call failed with `Post ".../v1/messages": context deadline exceeded (Client.Timeout exceeded while awaiting headers)`. The previous two failures on this same step were fast 400s (rejected immediately); this one was the opposite — the request was *accepted* and the server held the connection open while it thought, searched, and thought again, sending no response headers until done. The shared `http.Client` had `Timeout: 180s`, shorter than the operation. Fix: raised to 900s. Disciplines: (a) "while awaiting headers" on a non-streaming request means the server is still working, not that the network is broken — distinguish a timeout-while-working from a connectivity error before touching retries; (b) thinking effort and tool-loop depth multiply latency — `xhigh` + N searches can run for many minutes, so size the timeout for the slowest step; (c) the durable fix for very long calls is streaming — Anthropic recommends it for large thinking budgets because incremental SSE events keep the connection alive and also dodge intermediary proxy/load-balancer timeouts that a raised client timeout alone won't help with; a raised timeout is the pragmatic stopgap, streaming is the real answer; (d) one global client timeout sized for the slowest call is simplest, at the cost of slow failure on a genuinely hung fast call — per-request contexts are the refinement if that ever matters.

**27. Confirm the exact current request shape of any model or hosted-tool API from live docs before coding against it — these shapes drift between generations, and a remembered shape is a guess.** This session (2026-06-04) produced two back-to-back 400s on the same step from coding against API shapes that had changed under a model-string bump: the auto-injected `code_execution` collision (item 24) and the `thinking.type.enabled` rejection on Opus 4.8 (item 25) — the latter needing the adaptive-thinking shape (`thinking:{type:"adaptive"}` + `output_config:{effort}`) looked up before it could be written correctly. The pattern: an API surface that was correct for an earlier model or tool version is silently wrong for a newer one, and memory/training data lags the published docs. Discipline, now standing: before writing or changing code that calls a model or hosted-tool API — especially right after a model-string bump — verify the current request shape (thinking config, tool declarations, sampling params, `output_config`/effort) against the live docs for *that specific model*, and prefer one confirming search over shipping a remembered shape. The cost asymmetry is stark: one doc check versus a failed run, an error re-read, a code change, and a second round-trip — and against real API spend each round-trip costs money, not just time. (Items 24, 25, and the adaptive-thinking lookup are the worked examples; item 5 — "verify the new code is actually live" — is the deployment-side cousin of the same "don't assume, confirm" reflex.)
...
## 11. idea.uk standalone service — page-serving and deploy gotchas

These apply to the **idea.uk** product (the `idea-go/` Go service on the Hetzner box), not the chassis pipeline above. It is API-first with a few server-rendered HTML pages, deployed by `setup.sh` behind nginx + Let's Encrypt.

**Missing page — 404 on a path that is linked from the site (e.g. `/terms`, `/refund-policy`, `/privacy`, and originally `/`).**
Root cause: the service registers each path explicitly on an `http.ServeMux` in `routes()`. The `/` handler is a catch-all that returns 404 for anything that is not exactly `/`. A link to a path with no `mux.HandleFunc` returns Go's bare `404 page not found`.
Symptom: browser shows plain-text "404 page not found" on a linked URL while `/health` and the API endpoints work.
Fix: every served path needs a handler. The landing page is embedded (`//go:embed page.html`) and served at `/`; the policy pages have their own handlers rendering through the `page()` wrapper.
Check: `grep mux.HandleFunc service.go` is the complete list of served paths — cross-check every `href="/..."` in `page.html` against it. Before assuming a code bug, `curl -s -o /dev/null -w '%{http_code}' https://idea.uk/<path>`; a 404 coming from the Go service (not an nginx page) means there is no route.

**Missing design — a page renders as unstyled black-on-white text.**
Root cause: `writeHTML` emits a bare HTML fragment with no `<head>`/CSS. That is correct for content injected into the already-styled landing page (the taster result is placed into `#taster-result` via `innerHTML`), but wrong for any endpoint the browser NAVIGATES to as a full page — form POST targets (`/subscribe`, `/request`) and the Stripe redirects (`/order/success`, `/order/cancel`). Those show the fragment as the whole document, so it is unstyled.
Symptom: an endpoint returns the right words but looks nothing like the rest of the site.
Fix: wrap full-page responses in `a.page(title, body)` (brand CSS, fonts, header bar, footer with the contact email). Rule of thumb: full navigation → wrap; fragment injected into an existing page → leave bare.
Note: `.taster-result` is `white-space: pre-wrap` (so the engine's line breaks survive). Any HTML block injected there that carries its own markup (e.g. the £29 upsell) must reset `white-space: normal`, or the source newlines render as blank lines.

**Placeholders showing literally on the live page (`CONTACT_EMAIL`, `MONTH_SLOTS`).**
Root cause: `page.html` ships with those tokens; serving the raw embedded bytes shows them verbatim.
Fix: the service templates the page once at startup (`NewApp` runs a `strings.NewReplacer` over `pageHTML` into `a.landingHTML`) and `home` serves the filled copy. `CONTACT_EMAIL` falls back to `OPERATOR_EMAIL`; `MONTH_SLOTS` to "a limited number of". Set both in `/etc/idea/idea.env`.

**Deploy gotchas (setup.sh / systemd / Let's Encrypt):**
- **`set -e` can leave a half-configured box.** If certbot fails (DNS not propagated yet, or a bad email), the script aborts before nginx/the service are fully up. setup.sh now makes certbot failure non-fatal (box still gets ufw/fail2ban and the service on HTTP), so a TLS problem no longer leaves you with nothing to debug.
- **systemd `EnvironmentFile` keeps inline comments.** `PORT=8080  # comment` puts the comment into the value → invalid port → restart loop → nginx 502. Symptom: crash loop and a 502 right after editing the env file. Keep comments on their own lines; setup.sh also strips inline comments defensively.
- **Let's Encrypt rejects `@example.com`.** A placeholder registration email makes certbot fail (and, with `set -e`, aborts setup). Use a real address; setup.sh guards the placeholder up front.
- **Replacing a running binary gives `text file busy`.** Overwriting `/opt/idea/idea` in place while the service runs fails. scp to a temp name and `mv -f` over it (atomic rename), then `systemctl restart idea`. `setup.sh MODE=update` does this for you.
