...
1. **Coordinator `output_fields`-only + size-cap stub since 14 Jan (commit 06a8c6ef), UNCHANGED since —
   git-confirmed 2026-06-17. SIZE is the trigger.** `git log -L` on `extractWorkflowResult` /
   `extractWorkflowResultWithSizeLimit` / `extractMinimalResult` shows only two commits ever touch them:
   **11394b22 (11 Jan)** created `extractWorkflowResult` dumping all non-internal `collected_data`; **06a8c6ef
   (14 Jan, "made extract function respect output fields")** added, in one commit, the `output_fields` (plural)
   read + the `skipPatterns` fallback + `MaxResultSizeBytes=900000` + `extractWorkflowResultWithSizeLimit`
   (trims TOP-LEVEL strings >10k only; comment concedes it doesn't recurse) + `extractMinimalResult`
   (`status:"completed"` stub) + switched `notifyParentOfSuccess` to the size-limited variant. No change since;
   the 06-05→06-16 window query returned nothing → **no ~06-13 coordinator change** (phase2g 06-13 touched
   writer/planner/image-build-handler, not these functions).
   **CORRECTS last turn's reading** ("coordinator changed ~06-13 / restore singular handling"): the coordinator
   has **never** honoured singular `output_field` (plural-only since its 2nd commit), so there is nothing to
   "restore". The writer's `complete` was always singular (backups Mar–Jun) → it has ALWAYS hit the fallback
   dump. 06-06 gamesdesign worked because that page's dump came in **under 900k and carried the `page_content`
   key** the parent reads (`page_content_` with the underscore is in `skipPatterns`; `page_content` is not) —
   not because the coordinator honoured singular. The 01-16 logs confirm: page-content-writer dumping 46–147k,
   `field_count` 39–57, all under the cap, passing. A big multi-section page's dump clears 900k → trimmer can't
   shrink nested → `extractMinimalResult` stub → 0 sections → empty page. So **size is the trigger**; the
   singular key is necessary-but-not-sufficient (matches the bucket-audit finding). The 14-Jan date in the
   original gamesdesign note was right.
   **User decision (phase5 notes, migration 104):** do NOT change `extractWorkflowResult` to honour singular;
   `output_fields` (plural) is the standard; fix the AGENT to declare plural (104 proved it — clean
   `field_count` 4, no dump). Treated as governing.
   **Fix — two separate jobs.** (A) *Framework oversize-safety (structural, do regardless; gamesdesign notes
   §51/§52):* in `extractWorkflowResultWithSizeLimit`/`extractMinimalResult`, never return `status:"completed"`
   when the result was minimised — surface a **failure** (route to `notifyParentOfFailure` / `is_error`) — and
   recurse the trimmer into maps/arrays or persist the artefact + return a reference. Touches only the >900k
   path; leaves `extractWorkflowResult`'s plural contract as-is (honours the phase5 decision); low-risk for the
   clean `output_fields` agents (their results never reach the cap). (B) *Writer plural contract = the targeted
   unblock,* IF idea.uk's index is the size-stub: (i) its `needs_page:index` FAILED on **claim-timeout
   (attempts exhausted)** = the *hang* mode, not the stub→false-complete mode → confirm via the parent
   orchestration's `collected_data.page_content.response` (stub message vs genuine hang) BEFORE touching the
   writer; (ii) pick the plural fields from the parent's `save_page_sections` input_mapping + the compile output
   — notes warned `output_fields:["page_content"]` may nest wrongly (need `response.sections_metadata`, so may
   require hoisting inner fields rather than `["page_content"]`). Guardrail: do NOT raise `MaxResultSizeBytes`.
