...
- **Adoption (gamesdesign):** the adoption `generate_design_intent` step writes a `design_intent` that carries a
  **structured `palette.reference_values`** block (and the crawl gives a structured `font_family`). Result:
  ...
  did **not** win — the `design_intent` slot did. CONFIRMED why (2026-06-19, from `resolve_composition_palette.go`
  + `resolve_composition_helpers.go`): cascade slot 1 calls `extractReferenceValuesFromSpec(design_reference,
  "palette")`, which reads `design_reference.palette.reference_values` — a key the fingerprint never writes (it
  stores colours under `suggested_mapping` / `css_variables` / `colors`). So slot 1 is **dead**; the palette
  always comes from `design_intent.palette.reference_values` (slot 3), which adoption's `generate_design_intent`
  populates. Same dead-slot applies to typography slot 1 (`design_reference.typography.reference_values` vs the
  fingerprint's `typography.fonts` / `suggested_mapping.font_family`). Optional hardening: point slot 1 at
  `suggested_mapping`/`css_variables` so an adopted `design_reference` can drive palette/typography directly.)
