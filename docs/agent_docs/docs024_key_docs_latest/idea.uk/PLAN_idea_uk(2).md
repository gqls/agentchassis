...
4. **Build the idea.uk fake-door** (this is where we are now). A static landing
   ...
   fulfilled **manually** in the early phase using the method we already have.
   The page captures real payment intent — the test of willingness-to-pay that
   every estimate above has been waiting on.
