...
How direction enters — structural, not a classifier change:
- **`mission_brief` + `roadmap_brief`.** The classifier's prompt treats `mission_brief` as the PRIMARY
  source ("derive identity/tone/design from it; research validates"), explicitly outranking the scrape.
  Seed these with where idea.uk is going, re-run the front of the pipeline, and content/positioning move
  forward instead of mirroring the current site.
- **Author the `design_intent` deliberately** (grounded in the frontend-design skill) rather than leaving
  it to the classifier's inference — this is how we avoid reproducing the parchment/Fraunces look and get
  a distinctive, leading design that expresses the direction.
- **Tidy the duplicate spec rows first** so the planner reads the new direction, not a stale snapshot.
...
Next once scope is set: draft `mission_brief` + `roadmap_brief` → author `design_intent` (read the skill)
→ tidy duplicate specs → let the planner run on the forward direction → then the VM deploy (Phase 2).
