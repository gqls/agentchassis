...
// payLinkText / payLinkHTML build the "ready to pay" email. The buyer may not
// immediately recognise the sender, so it opens by saying what this is and what
// they asked for, then gives the pay link and the delivery + refund promise.
func payLinkText(name, business, link string, price int) string {
	return fmt.Sprintf("Hi %s,\n\n"+
		"This is idea.uk. You asked us to find AI product ideas for %s. We've looked into it and we can "+
		"do a useful job, so your report is ready to start.\n\n"+
		"To go ahead, pay £%d here:\n%s\n\n"+
		"As soon as you've paid we'll email you the report — within 72 hours, and usually much sooner. "+
		"A person reads it before it goes out. If it doesn't turn up anything worth acting on, email us "+
		"within 14 days and we'll refund you in full.\n\n"+
		"Questions? Just reply to this email.",
		name, business, price, link)
}

func payLinkHTML(name, business, link string, price int) string {
	esc := html.EscapeString
	return fmt.Sprintf(`<div style="background:#eceff3;padding:24px 12px;font-family:'Helvetica Neue',Helvetica,Arial,sans-serif;font-size:16px;line-height:1.7;color:#36424f">`+
		`<div style="max-width:560px;margin:0 auto;background:#ffffff;border:1px solid #e2e6ec;border-radius:10px;padding:28px 30px">`+
		`<div style="font-family:Georgia,'Times New Roman',serif;font-size:20px;font-weight:bold;color:#15243d">idea<span style="color:#b08a3e">.</span>uk</div>`+
		`<div style="height:3px;width:46px;background:#b08a3e;margin:10px 0 18px"></div>`+
		`<h1 style="font-family:Georgia,'Times New Roman',serif;font-size:23px;line-height:1.25;color:#15243d;margin:0 0 12px">Your report is ready to start</h1>`+
		`<p style="margin:0 0 14px">Hi %s — this is idea.uk. You asked us to find AI product ideas for <strong>%s</strong>. We've looked into it and we can do a useful job, so your report is ready.</p>`+
		`<p style="margin:0 0 22px"><a href="%s" style="display:inline-block;background:#15243d;color:#ffffff;text-decoration:none;font-weight:bold;padding:13px 22px;border-radius:6px">Pay £%d and start my report</a></p>`+
		`<p style="margin:0 0 14px">As soon as you've paid we'll email you the report — within 72 hours, and usually much sooner. A person reads it before it goes out.</p>`+
		`<p style="margin:0 0 18px;color:#707b88;font-size:15px">If it doesn't turn up anything worth acting on, email us within 14 days and we'll refund you in full.</p>`+
		`<p style="border-top:1px solid #e2e6ec;padding-top:14px;margin:0;color:#707b88;font-size:14px">Questions? Just reply to this email. &middot; idea.uk, by leopardess.uk</p>`+
		`</div></div>`,
		esc(name), esc(business), esc(link), price)
}
...
// b64Body base64-encodes a body part and wraps it at 76 characters per line, so
// no line approaches the SMTP 998-octet limit (which was folding long HTML lines
// mid-tag). Safe for UTF-8 content of any line length.
...
func sendOne(operatorEmail, to, subject, text, htmlBody string) {
	...
	if htmlBody == "" {
		...
		// pick the richest part they can render. Both parts are base64-encoded so no
		// line can exceed the SMTP 998-octet limit (long unbroken HTML lines were
		// being folded mid-tag by a mail server, corrupting the markup).
		boundary := fmt.Sprintf("idea-bnd-%d", time.Now().UnixNano())
		fmt.Fprintf(&msg, "Content-Type: multipart/alternative; boundary=\"%s\"\r\n\r\n", boundary)
		fmt.Fprintf(&msg, "--%s\r\nContent-Type: text/plain; charset=\"UTF-8\"\r\nContent-Transfer-Encoding: base64\r\n\r\n%s\r\n", boundary, b64Body(text))
		fmt.Fprintf(&msg, "--%s\r\nContent-Type: text/html; charset=\"UTF-8\"\r\nContent-Transfer-Encoding: base64\r\n\r\n%s\r\n", boundary, b64Body(htmlBody))
		fmt.Fprintf(&msg, "--%s--\r\n", boundary)
	}
