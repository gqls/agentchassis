...
LETSENCRYPT_EMAIL="${LETSENCRYPT_EMAIL:-}"   # required for full mode; not needed for MODE=update
MODE="${MODE:-full}"
# Validate the email only in full mode (update just swaps the binary). Catch the
# obvious placeholder up front rather than failing minutes later at certbot.
if [[ "$MODE" != "update" ]]; then
  : "${LETSENCRYPT_EMAIL:?set LETSENCRYPT_EMAIL for cert registration (full mode)}"
...
# Optional space-separated IPs to EXEMPT from nginx rate limiting (done the
# correct way via geo+map, unlike the old `if ($remote_addr) { break; }` which
# was a no-op). Empty = no exemption (fine for idea.uk: operator endpoints are
# key-gated and /audience-check is rate-limited in the app).
WHITELIST_IPS="${WHITELIST_IPS:-}"

log() { echo "[setup] $*"; }
have() { command -v "$1" >/dev/null 2>&1; }

# Emit the nginx http-context rate-limit preamble. With WHITELIST_IPS set, uses
# the canonical geo+map pattern so listed IPs map to an empty key (= not counted
# = genuinely exempt). Without it, a plain per-IP zone.
rate_limit_preamble() {
  if [[ -n "$WHITELIST_IPS" ]]; then
    ...
    echo '    0 "";'
    echo "}"
    echo "limit_req_zone \$limit_key zone=idea_rl:10m rate=10r/s;"
  else
    echo "limit_req_zone \$binary_remote_addr zone=idea_rl:10m rate=10r/s;"
  fi
}
...
# systemd EnvironmentFile does NOT strip inline comments: a "# ..." after a value
# becomes part of the value. This once made PORT="8080  # ..." → invalid port →
# the service crash-looped behind a 502. Flag it here, loudly, not via the loop.
if grep -nqE '^[A-Za-z_][A-Za-z0-9_]*=.*[[:space:]]#' "$ENV_FILE"; then
  log "WARNING: $ENV_FILE has inline comments after values — systemd includes them IN the value and the service may fail (e.g. an invalid PORT). Offending lines:"
  grep -nE '^[A-Za-z_][A-Za-z0-9_]*=.*[[:space:]]#' "$ENV_FILE" | sed 's/^/[setup]   /'
  log "Fix: put comments on their own lines, or run:  sed -i 's/[[:space:]]*#.*\$//' $ENV_FILE"
fi
...
if [[ ! -d "/etc/letsencrypt/live/$DOMAIN" ]]; then
  ...
  certbot certonly --webroot -w "$ACME_WEBROOT" -d "$DOMAIN" \
    ...
    || log "WARNING: certbot failed — staying on HTTP for now. Fix the cause (DNS, email, port 80 reachability) and re-run; the service still starts on HTTP below, and a successful re-run upgrades it to HTTPS."
else
...
# ── nginx log rotation (size-based; improves on the daily default) ──────────────
