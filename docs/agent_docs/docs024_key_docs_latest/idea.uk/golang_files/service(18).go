...
func (a *App) handleRequest(w http.ResponseWriter, r *http.Request) {
	...
	a.deliver(a.cfg.OperatorEmail, fmt.Sprintf("[idea.uk] New report request %s", id),
		fmt.Sprintf("New idea.uk report request.\n\nRequester: %s (%s)\nBusiness: %s\nAudience: %s\nNotes: %s\n\nOrder id: %s\nConfirm or decline this order id via the operator step.",
			name, email, business, audience, notes, id))
...
func (a *App) confirm(w http.ResponseWriter, r *http.Request) {
	...
	if a.cfg.ReviewBeforePay {
		...
		writeJSON(w, 200, map[string]any{"status": "running",
			"note": "engine started — you'll get the draft to review, then /approve to bill or /decline"})
		return
	}
