...
func (a *App) fulfil(id string) {
	...
	if a.cfg.AutoDeliver {
		...
		a.deliver(a.cfg.OperatorEmail, fmt.Sprintf("[idea.uk] REVIEW %s (%s)", id, o.Domain),
			fmt.Sprintf("Paid order ready for review before sending to %s.\n\n--- DRAFT REPORT ---\n\n%s", o.Email, report))
	}
}
...
func (a *App) subscribe(w http.ResponseWriter, r *http.Request) {
	...
	writeHTML(w, "<h1>Thanks</h1><p>We'll send the next worked example.</p>")
}
...
func (a *App) handleRequest(w http.ResponseWriter, r *http.Request) {
	...
	a.deliver(a.cfg.OperatorEmail, fmt.Sprintf("[idea.uk] NEW REQUEST %s (%s)", id, business),
		fmt.Sprintf("From: %s <%s>\nBusiness: %s\nAudience: %s\nNotes: %s\n\n"+
			"Confirm: POST /confirm {\"order_id\":\"%s\"}\nDecline: POST /decline {\"order_id\":\"%s\"}",
			name, email, business, audience, notes, id, id))
	writeHTML(w, "<h1>Request received</h1><p>We'll reply within 24 hours with a "+
...
func (a *App) confirm(w http.ResponseWriter, r *http.Request) {
	...
	writeJSON(w, 200, map[string]any{"status": "awaiting_payment", "checkout_url": checkoutURL})
}
...
func (a *App) webhook(w http.ResponseWriter, r *http.Request) {
	...
	case "awaiting_payment":
		...
		a.dispatch(func() { a.fulfil(evt.OrderID) })
...
func (a *App) orderSuccess(w http.ResponseWriter, r *http.Request) {
	...
	writeHTML(w, "<h1>Payment received</h1><p>Your report is being prepared and will "+
		"arrive by email within 72 hours. If we can't find anything worth acting on, you're refunded.</p>")
}

func (a *App) orderCancel(w http.ResponseWriter, r *http.Request) {
	writeHTML(w, "<h1>Order cancelled</h1><p>No charge was made.</p>")
}
...
func makeDeliver(operatorEmail string) func(to, subject, body string) {
	return func(to, subject, body string) {
		...
		from := env("SMTP_FROM", operatorEmail)
