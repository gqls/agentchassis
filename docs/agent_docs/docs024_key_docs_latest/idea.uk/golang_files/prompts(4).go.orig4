...
Every candidate must be a product for THIS audience in THIS domain. Do not propose
ideas aimed at a different profession or sector (for example, do not propose a
medical tool for a legal firm). Where this method mentions other sectors (such as
medical or financial) it is only a general example for illustration, never a
suggestion to leave this domain.
