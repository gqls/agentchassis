...
- [ ] **Layout picks ignore light/dark character** — `resolveLayoutByTags` (`fork_theme_composition.go`, now on disk) matches on `category` + `industry_tags` only, so idea.uk's light-editorial intent landed on `tool-portal-dark` (dark, hardcoded-blue header/footer). Even with a correct parchment palette the page won't read right until layout selection weighs the `design_intent` scheme/character. Ready to draft (structural fix, the harder of the two).
...
### Stage A — Validate idea.uk's palette (the immediate visible check)

idea.uk is a fresh build with a `design_intent`, so once that spec carries the structured palette the
cascade resolves it directly — this does **not** require the Go hardening (that only adds the
`design_reference` fallback for adopted sites). The migration only affects the **next** classifier run,
so idea.uk's existing prose-only row still needs either a re-run or a direct patch.

**A1 — inspect (read-only) before touching anything:**

```bash
# confirm the JSONB column name and the current-version flag on site_specs
kubectl exec -n ai-persona-system postgres-clients-0 -- \
  psql -U clients_user -d clients_db -c "\d site_specs"

# read idea.uk's current design_intent (use the JSONB column name \d just showed, e.g. spec_data)
kubectl exec -n ai-persona-system postgres-clients-0 -- psql -U clients_user -d clients_db -c \
"SELECT aspect, is_current, ‹jsonb_col› FROM site_specs
 WHERE site_id='97ed2f64-65ca-4b67-8a98-dfd8195a0d3a' AND aspect='design_intent' AND is_current;"

# composition state — is one already installed? (decides whether a re-resolve is destructive)
kubectl exec -n ai-persona-system postgres-clients-0 -- psql -U clients_user -d clients_db -c \
"SELECT id, style_collection_id FROM sites WHERE id='97ed2f64-65ca-4b67-8a98-dfd8195a0d3a';"
```

**A2 — get parchment into `design_intent`.** Two routes:

- **(cleaner) fresh staging rebuild** — submit idea.uk through the pipeline to a staging `site_id`
  (Phase 1 below). The full path runs with the migration in place and writes the spec the correct,
  versioned way; nothing on the existing row or composition has to be unpicked. Preferred for validation.

- **(in place) patch the current `design_intent` row** — least invasive on the existing site; preserves
  the single-current-row invariant by mutating the JSONB only. Back up first:

```bash
# backup the current design_intent JSONB to a table (idempotent name per run)
kubectl exec -n ai-persona-system postgres-clients-0 -- psql -U clients_user -d clients_db -c \
"CREATE TABLE IF NOT EXISTS bak_idea_design_intent_20260620 AS
 SELECT * FROM site_specs
 WHERE site_id='97ed2f64-65ca-4b67-8a98-dfd8195a0d3a' AND aspect='design_intent' AND is_current;"

# patch palette + typography into the JSONB (replace ‹jsonb_col› with the real column from \d).
# values: ink primary, rust the SOLE accent, parchment bg/surface — blue-as-primary is banned by the brief.
kubectl exec -n ai-persona-system postgres-clients-0 -- psql -U clients_user -d clients_db -c \
"UPDATE site_specs
 SET ‹jsonb_col› = jsonb_set(jsonb_set(
       ‹jsonb_col›,
       '{palette,reference_values}',
       '{\"primary\":\"#1A1816\",\"secondary\":\"#1A1816\",\"accent\":\"#A8391A\",\"background\":\"#EFE7D6\",\"surface\":\"#EFE7D6\",\"text\":\"#1A1816\",\"text_muted\":\"#5A554C\",\"border\":\"#1A1816\"}'::jsonb, true),
       '{typography,reference_values}',
       '{\"font_family\":\"'\''IBM Plex Sans'\'', system-ui, sans-serif\",\"heading_font\":\"'\''Fraunces'\'', Georgia, serif\"}'::jsonb, true)
 WHERE site_id='97ed2f64-65ca-4b67-8a98-dfd8195a0d3a' AND aspect='design_intent' AND is_current;"
```

**A3 — re-resolve the composition.** If `style_collection_id` from A1 is **NULL**, a `needs_composition`
work item is enough. If it's **set** (idea.uk has already built), `install_site_composition` refuses to
overwrite, so a bare requeue no-ops — you'd have to clear `sites.style_collection_id` and the site's
`css_theme` / `style_collection` rows first, which is destructive. This is why the fresh-staging route is
cleaner. If requeuing, first inspect how composition/design items are shaped, then enqueue:

```bash
# see the columns and how composition/design items look for any site
kubectl exec -n ai-persona-system postgres-clients-0 -- psql -U clients_user -d clients_db -c "\d site_work_items"
kubectl exec -n ai-persona-system postgres-clients-0 -- psql -U clients_user -d clients_db -c \
"SELECT item_type, handler_agent, status, item_key, priority
 FROM site_work_items WHERE item_type IN ('needs_composition','needs_design')
 ORDER BY created_at DESC LIMIT 5;"
# then enqueue a needs_composition row for idea.uk mirroring those columns (item_key unique per idx_swi_dedup).
```

**A4 — watch it run, then verify:**

```bash
kubectl -n ai-persona-system logs -f deploy/‹worker› \
  | grep -iE 'composition|palette|typography|design'

# verify the spec took
kubectl exec -n ai-persona-system postgres-clients-0 -- psql -U clients_user -d clients_db -c \
"SELECT ‹jsonb_col›->'palette'->'reference_values' AS palette
 FROM site_specs WHERE site_id='97ed2f64-65ca-4b67-8a98-dfd8195a0d3a' AND aspect='design_intent' AND is_current;"

# verify the rendered result (schema-independent): the deployed CSS should carry parchment, not blue
curl -s ‹deployed styles.css URL› | grep -iE '#efe7d6|#1a1816|#a8391a'   # expect these
curl -s ‹deployed styles.css URL› | grep -iE '#[0-9a-f]*(2563eb|3b82f6|1e3a8a)'  # expect nothing (blues)
```
