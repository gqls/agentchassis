...
> **2026-06-11 update — real-world lesson from idea.uk.** Sending a site's transactional mail via a
> shared cPanel relay (Clook → MailChannels) failed: MailChannels content-filtered leopardess.uk's
> legitimate outbound (`550 5.7.1 [CS]`). idea.uk now sends via **AWS SES** with the site's own
> DKIM identity. So when this design lands in the chassis, the email-provisioner should set up a
> **dedicated transactional sender (SES or equivalent) with per-domain DKIM**, not route through a
> shared relay. Identity scheme (deterministic per-domain addresses) stands; the *sending pipe* is
> the part this proved out. See `HANDOFF.md` / `running_notes.md`.
...
## Learning 2026-06-06 — prefer per-site forwarders over a server catch-all

Real-world check: an inbound test to `idea-uk@leopardess.uk` was rejected by the operator
domain's own mail server with "No Such User Here". The operator domain is a normal local
mail domain (it has a real mailbox for SMTP auth), so the server delivers known mailboxes and
routes everything else to cPanel's **Default Address**, which defaults to *fail*. A server
catch-all therefore needs the Default Address explicitly set to forward — and forwarding a
catch-all to an **external** inbox is restricted or discouraged on many hosts because it
causes backscatter (forwarding spam sent to random@domain → the server's IP gets blocked).

**Refinement:** the inbound model is **a specific forwarder per published site**, not a
wildcard catch-all. When a site is published, create one forwarder
`<encoded-domain>@operator → operator inbox` and record it on the site's `email` aspect
(`provisioned: true`). This is exactly the email-provisioner agent's job, and it means we
only ever forward addresses that exist (no backscatter, no reliance on host catch-all policy).
The deterministic encoding still defines each site's address; provisioning creates the
forwarder for it. A server catch-all remains an optional convenience for the long tail *if*
the host allows external default-address forwarding, but it is not the design's basis.

---

## Operational note — 2026-06-10 (what idea.uk's go-live taught us)

Two realities the framework's email design must account for, learned deploying idea.uk on a
Hetzner box sending through Clook (cPanel + MailChannels):

1. **A cloud box usually can't use outbound SMTP 25/465.** Hetzner blocks 25/465/2525 on the
   box; only **587 (submission)** was open. Any per-site or operator mail sent direct from a
   cloud box must use 587 (STARTTLS) — or, better, an HTTPS email API on 443 that sidesteps
   SMTP-port blocks entirely. Don't assume 465/SMTPS works from a VM.
2. **Shared-host relays content-filter outbound.** Clook relays via MailChannels, which blocked
   a legitimate transactional notification as "Spam Content" (`550 5.7.1 [CS]`). For a service
   that must reliably deliver transactional mail (pay-links, receipts, reports), a shared relay's
   spam filter is a liability. The framework should plan a **dedicated transactional sender** with
   the operator domain authenticated (SPF/DKIM/DMARC), rather than relying on a shared cPanel
   relay — and keep transactional bodies clean (no embedded `From:`-style header lines, no raw
   JSON), which is what tripped the filter.

The deterministic per-site address scheme and the per-site-forwarder design below are unchanged;
this note concerns the **sending** path, which is the part that bit us. The catch-all remains the
manual-phase inbound convenience.
