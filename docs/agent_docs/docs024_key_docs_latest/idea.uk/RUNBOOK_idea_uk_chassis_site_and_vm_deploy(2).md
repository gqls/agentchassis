...
- **Not blank-slate.** It still scrapes the live idea.uk (the Phase 1 caveat), so the read is informed by
  the current landing page. For a truly name-only read you'd keep the scrape from reaching the live tool.
...
Still genuinely open (these gate confidence that the fresh front segment flows end-to-end):
- **`domain-strategist`** — its definition. The classifier hands off to it (`needs_strategy`); confirm it
  then creates the `needs_site_plan` (or equivalent) that reaches `build-site-planner`, and whether it
  writes a `briefing` / `strategy` spec.
- **Work-item triage / dispatch** — `build-pipeline-trigger` only dispatches items at status `triaged` /
  `approved`. What moves a freshly-created `needs_domain_research` / `needs_strategy` to that status — an
  auto-triage step, or a manual/HITL approval? Adoption-built sites *do* get built, so the mechanism
  exists; the question is only whether the fresh entry feeds it the same way. `build-dispatch-loop`'s
  definition would settle it.
