...
## Going live with Stripe, and refunds (2026-06-11)

Full procedure (expands the brief "Stripe go-live" note above, which it supersedes). The code already
switches from FakeProvider to Stripe when BOTH `STRIPE_SECRET_KEY` and `STRIPE_WEBHOOK_SECRET` are set
— no code change to go live. `orderSuccess` is correct for a real redirect (no `fake` param → it just
shows "Payment received"; the webhook does delivery).
...
2. `/etc/idea/idea.env`: `STRIPE_SECRET_KEY=sk_test_…`, `STRIPE_WEBHOOK_SECRET=whsec_…` →
   `systemctl restart idea`. Startup log must stop saying "No Stripe keys".
...
5. `/etc/idea/idea.env`: `STRIPE_SECRET_KEY=sk_live_…`, `STRIPE_WEBHOOK_SECRET=whsec_…` (the live one) →
   restart.
