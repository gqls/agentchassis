...
**▶ CURRENT POSITION (2026-06-26):** **Composition + stylesheet are correct; the page layer is not.** Steps 1–6 done: idea.uk is on `tool-portal-light` + parchment, and the deployed `styles.css` (commit `05ef817`) is verified — exactly `tool-portal-light`, parchment palette, NO LLM drift. **But Step 6 found the deployed pages still render dark** — the page HTML is unchanged from the 2026-06-21 build and bakes per-component inline `<style>` (dark gradient header, dark stock-photo hero with a navy `--accent-color`/`#0f3460` button bug, dark CTA, dark footer) that overrides the stylesheet. A `styles.css` swap can't flip already-built pages. **NEXT: Step 7 — rebuild the pages on the light layout, but investigate first** whether page-build component selection is scheme-aware and whether those components hardcode dark, so a rebuild actually produces a light site rather than the same dark chrome.
...
### ◀ Step 7 — rebuild the pages on the light layout  *(DO THIS NEXT — investigate before triggering)*
A page rebuild only helps if the build selects light / palette-deferring components for a light layout; if it re-assembles the same gradient-header + dark-hero components it reproduces the same dark chrome, and the `--accent-color`/`#0f3460` fallback bug rides along regardless. So **before** triggering a rebuild, establish:
1. **Is page-build component selection scheme/layout-aware?** Read the page-build agent definition (`SELECT type, agent_category, jsonb_pretty(default_config) FROM agent_definitions WHERE type = '<page-build agent>';` — likely `page-builder` / the handler behind `page_rerender`) and how it picks header/hero/cta/footer. If a light layout would still get the gradient/dark-overlay components, that selection is the structural gap.
2. **Do those components hardcode dark, or defer to the palette?** Inspect the component templates for the dark header/hero/cta/footer. The fix is for them to use `--color-*` vars (and the correct names — `--color-accent`, not `--accent-color`) and a light header/typographic hero for a light scheme, the way `tool-portal-light`'s own `.site-header`/`.hero-section` rules already do.

Decide from that whether (a) a scheme-aware rebuild fixes it, or (b) the components need structural work first (preferred over patching one page). Then trigger the rebuild and re-verify the deployed pages read light/parchment with no stray blue.
