...
3. **Cut the generic ones.** **[diff-model]** Critique each against **the specific
   free alternative the audience would actually use** — not "a free model in the
   abstract" but the concrete thing they'd otherwise do (e.g. *describe the idea to
   Bolt themselves*, *ask ChatGPT*, *use the builder's own planning step*). If that
   free alternative gets them most of the way, drop it. Be ruthless — most
   candidates should die here. (v1 lesson: the abstract "could free AI do this"
   was too soft; name the real substitute.)
...
**Rank** advancing candidates by the sum of all five factors.
