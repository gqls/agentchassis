...
- **Preferred fix (framework):** generate the aspirational direction — a dedicated `vision` aspect —
  **before the classifier runs**, driven by "lead the vertical with the best content for users / build
  beyond the best examples". The classifier then derives forward content *and* design from it (it already
  prioritises the mission slot above the scrape), so every site is aspirational by default and idea.uk is
  the first beneficiary. (If design is really decided by the downstream design agents rather than the
  classifier's draft, the vision feeds those instead — confirm via their defs.)
- **Open wiring decisions (lean):** (1) dedicated `vision` aspect vs generating into `mission_brief` —
  lean dedicated (provenance + distinct responsibilities); (2) a dedicated generator agent vs extending
  `domain-strategist` — lean dedicated (strategist is post-classifier + monetisation-framed); (3) ordering
  — the generator must precede the classifier (`domain-submitter` → `needs_vision` → generator →
  `needs_domain_research`).
- **Verify before building (schema / reuse first):** `SELECT DISTINCT aspect FROM site_specs` + any CHECK
  constraint on `aspect`; how `build-site-planner` weights `strategy` vs `mission_brief`; and the
  `brand-designer` / `visual-designer` / `site-design-planner` + `build-briefing-agent` defs (so aspiration
  reaches the design, not just the words).
...
Next: settle the three wiring decisions + share the design-agent defs → spec the `vision` generator
(simple workflow, reasoning in the LLM step, new aspect, wired before the classifier; check schema first)
→ content + design then flow forward by default → tidy duplicate specs → planner → VM deploy (Phase 2).
