...
**▶ CURRENT POSITION (2026-06-25) — read this first.** The matcher code is LIVE in production (both `fork_theme_composition.go` + `resolve_composition_layout_action.go` deployed), so the step-3 code gate is cleared. A step-2 run left an OPEN, uncommitted transaction (psql prompt `clients_db=*#`): the detach did `UPDATE 1` but the four deletes and the spec-supersede each did 0 rows, and `COMMIT` did not execute (it was concatenated onto the `\echo` line by the paste). 0-row deletes most likely mean idea.uk's composition rows were already removed, but that is not yet confirmed, and the open transaction holds a lock on the `sites` row. **Do this, in order:**
1. End the transaction safely: type `ROLLBACK;` (one word). Undoes only the redundant detach, releases the lock, returns to the committed state. Do NOT `COMMIT` (could leave a half-cleared state).
2. Run `reresolve_idea_uk_02b_check_state.sql` as a FILE (`kubectl exec -i … < file`) to read the committed state.
3. Decide from 02b's output:
   - detached (NULL) + all four composition counts 0 + current resolved_composition 0 → **step 2 already done → go to step 3.**
   - `style_collection_id` = 54c5a076… + counts 1 + resolved_composition 1 → **step 2 has not taken → re-run `reresolve_idea_uk_02_detach_and_clear.sql` as a FILE** (never pasted), then re-run 02b.
4. When 02b confirms detached + rows gone + spec superseded, run step 3 (`reresolve_idea_uk_03_trigger.sh`) then step 4.

_Why the deletes may have shown 0: if an earlier step-2 already committed, a re-run finds the rows gone (DELETE 0) and the spec already superseded (UPDATE 0), while the detach still reports UPDATE 1 because the `sites` row matches by id even when `style_collection_id` is already NULL. 02b distinguishes that from a genuine non-match._

**Progress (2026-06-22):** layouts backed up (`bak_layouts_pre_scheme_20260621`, 17 rows) and `migration_layouts_scheme_and_light_tool_portal.sql` applied (ALTER + tool-portal-dark=dark + soft-editorial=light + `tool-portal-light` inserted, scheme light). Step 1 backups created (`bak_*_idea_20260621`: 1 site / 9 specs / 1 sc / 1 theme / 1 palette / 1 typo) and `reresolve_idea_uk_01b_inspect_only.sql` PASSED: current chain confirmed on `tool-portal-dark` (dark); the **four uniqueness checks are all 0**; `palette-idea-uk` + `typography-idea-uk` both have `source_site_id = idea.uk` (so the step-2 guarded deletes remove them); `tool-portal-light` present and active. (Step 2 has since been attempted — see ▶ CURRENT POSITION above.) Do NOT re-run step 1 or the layouts backup (the `CREATE TABLE bak_*` would collide). Remaining gate: deploy the matcher code before step 3 (below). **Update 2026-06-25:** a first re-trigger failed at `ensure_site_record` (`domain not found in input_data`) because the trigger passed only `site_id`; the trigger now passes `domain` too. Step 2 re-pointed at the freshly-made `bak_*_idea_20260625` set (the `_20260621` set also still exists, identical). No mutation occurred on the failed attempt.
...
2. **`reresolve_idea_uk_02_detach_and_clear.sql` — un-installs idea.uk's current (dark) composition so the planner can install a fresh one.** This is the only mutating SQL in the re-resolve. Run it only after step 1's four uniqueness counts were all 0. In one transaction it:
   - sets `sites.style_collection_id = NULL` (detaches the site from its installed composition);
   - deletes idea.uk's four composition rows in FK-safe order — its `style_collection`, then its `css_theme`, then its `palette` and `typography_set` (the last two only where `source_site_id = idea.uk`, so a shared/seed row can't be hit) — which also frees the unique names `theme-/palette-/typography-idea-uk` so the re-install recreates them cleanly;
   - marks the old `resolved_composition` spec `is_current = false`, because `install_site_composition` refuses to overwrite a composition it still thinks is installed.

   It touches nothing else: not idea.uk's `sites` row, not its other specs, not the live £29 tool. After it commits, idea.uk has **no installed composition** until step 3 re-runs the planner — expected and harmless (the build deploys to B2, which DNS doesn't point at). Reversible from the `bak_*_idea_20260625` tables (re-insert the rows, re-set `sites.style_collection_id`, flip the old spec back to `is_current = true`). It does **not** need the matcher code deployed — only step 3 does — so it is safe to run before the image is rolled.
3. **GATE — deploy the matcher code first.** The merged `fork_theme_composition.go` + `resolve_composition_layout_action.go` must be built into the chassis image AND `site-design-planner` rolled to it before this step; otherwise the re-trigger spawns the OLD scheme-blind matcher and re-picks `tool-portal-dark`. Step 2 needs no code and is safe anytime; only this step does. Then `reresolve_idea_uk_03_trigger.sh` — orchestrates `site-design-planner` with `input_data {site_id, domain}` (envelope identical to 082/080c). **`input_data` MUST include `domain`:** the planner's first step `ensure_site_record` loads the site by domain and fails with `domain not found in input_data` if only `site_id` is passed (confirmed 2026-06-25; the trigger file now passes both). The planner re-runs composition (now scheme-aware → `tool-portal-light`) and emits its `needs_design` handoff, which the build-dispatch-loop routes to `webdesign-agent` to re-render `styles.css`. If design doesn't advance, re-run the trigger with `AGENT="webdesign-agent"`.
