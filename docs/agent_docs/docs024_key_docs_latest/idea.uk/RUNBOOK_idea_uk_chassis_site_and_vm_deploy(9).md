...
1. **Coordinator dropped `output_field` handling ~06-13 (CONCLUDED 2026-06-17 from backup history).
   Coordinator-side fix.** Backup survey: `page-content-writer`'s `complete` has been `output_field:
   "page_content"` (singular), **never** `output_fields`, in every backup that has it — 2026-03-23, 04-02,
   04-21, and 06-10. So (a) it is **not** a 06-13 writer regression (the key was always singular), and (b) it
   is **not** the 14-Jan coordinator change (the 06-06 build worked with that same singular writer, so the
   coordinator was still honouring `output_field` after 14 Jan). The only fit: the writer always emitted
   singular `output_field`; the coordinator **used to honour it** (return `collected_data["page_content"]`
   flat = the `{page_html,…,sections_metadata}` the parent read on 06-06) and **stopped between 06-06 and
   06-15** — now reading `output_fields` (plural) only, ignoring the singular key, falling into dump → size →
   stub. The ~06-13 "phase2g" cluster (writer/planner/image-build-handler all touched; the 06-13 `pre_phase2g`
   backups) is the likely window. `output_field` is the *documented* convention (001); `output_fields` /
   `extractWorkflowResult` / the 900k cap are undocumented (not even in `035`). **Fix (structural,
   coordinator-side — no writer edit):** restore the coordinator honouring `output_field` (singular) as a
   return key (return that key's value flat, as pre-06-13) **and** harden the oversize path to fail loudly /
   pass by reference, never a `status:"completed"` stub. Fixes `page-content-writer` + the 3 other
   `output_field`-only agents at once; the 100 `output_fields` agents are unaffected. Do NOT raise
   `MaxResultSizeBytes`. **Confirm + locate the exact change:** `coordinator.go` git history on
   `extractWorkflowResult` / `MaxResultSizeBytes` / `extractMinimalResult` — the ~06-13 commit that switched to
   `output_fields`-only and added the size-stub fallback shows what to restore and what to harden.
