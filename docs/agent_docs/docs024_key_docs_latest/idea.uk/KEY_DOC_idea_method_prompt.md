...
STEP 5 — SCORE survivors, 1–5 each (5 always better/safer):
  ...
  - Durability: 1 next model release erodes it · 3 holds a while · 5 model progress
    doesn't erode it.
  - Risk to operator (CONSEQUENCE of being wrong, not probability):
    1 regulated profession (medical/legal/FCA — do not build without proper
    qualifications) · 2 high-stakes financial or quasi-regulated (needs human
    review + PII + reviewed T&Cs before build) · 3 meaningful decisions, customer
    can verify our citations (PII recommended) · 4 minor consequences, refunds
    make customers whole · 5 pure analysis, customer decides, no plausible loss
    beyond fee paid.
  GATE: advance only if Defensibility ≥3 AND Willingness ≥3. Flag Durability ≤2 as
  "short-lived". Risk is NOT in the sum (sum = Def+Will+Build+Reuse+Dur out of 25)
  and NOT in the gate, but:
    - Risk = 1 is DROPPED (regulated profession territory), surface separately.
    - Risk ≤ 2 advances but flagged "needs liability work before building"; the
      cheapest_test for these MUST require "validate demand first; PII insurance
      in force and T&Cs reviewed by a UK solicitor before any build."
  For each advancing candidate give the cheapest demand test and mark "test_now"
  (Buildability ≥4 or cheap test, AND Risk ≥3) vs "consider".

STEP 6 — OUTPUT. Rank advancing candidates by score sum (Risk as tiebreaker —
  prefer safer at equal fitness). For each: title, idea (capability × asset →
  what it does), the verified findings, the five fitness scores + sum, Operator
  risk score with its short label, the cheapest test, and test-now/consider.
  Show the "needs liability work" flag where Risk ≤ 2. Then list what was dropped
  for failing Def/Will, and SEPARATELY list what was dropped for Risk = 1. If
  nothing advances, say so plainly and give the route to a better audience, a
  new asset, or a different monetisation.
