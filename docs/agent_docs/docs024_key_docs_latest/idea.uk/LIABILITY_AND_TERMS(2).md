...
**First-line filter — the method's Risk column (added 2026-05-28).** Since the
agritec/SFI catch, the ideation method scores every advancing candidate on
**Risk to the operator** (1–5, 5 = safe). The rules:

- **Risk = 1** (regulated profession — medical, legal, FCA financial advice)
  is **dropped automatically** by the method. It surfaces in a separate
  "Dropped for operator risk" section so the operator sees what got killed
  for risk vs what failed the Def/Will gate.
- **Risk ≤ 2** (high-stakes financial/regulatory adjacency) **still advances
  if it passes the Def/Will gate**, but flagged as "⚠ needs liability work
  before building" — the candidate's cheapest_test must require validating
  demand first and getting PII insurance + reviewed T&Cs in place before any
  build.
- **Risk ≥ 3** can move through the normal demand-test path.

This document is what the "needs liability work" flag points at — practical
package of legal, operational, and technical mitigations for any candidate the
method scores Risk ≤ 2. The single-farm SFI assessment was the first product
to be paused on this basis; the SFI26 Diff Alerts product replaced it as the
first vertical tool (Risk ≈ 4).
