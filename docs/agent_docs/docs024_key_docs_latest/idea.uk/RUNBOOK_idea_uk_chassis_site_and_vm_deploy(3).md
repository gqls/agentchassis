...
Still genuinely open (this is the last thing gating confidence that the fresh front segment flows):
- **Work-item status / triage** — `build-pipeline-trigger`'s `find_dispatchable_site` only picks sites
  with items at status `triaged` / `approved`, and `build-dispatch-loop` then claims and runs each item's
  `handler_agent`. What sets a freshly created item (`needs_domain_research`, `needs_strategy`,
  `needs_briefing`, …) to a dispatchable status — a default on `create_work_item`, or a triage/approval
  step? Adoption-built sites *do* get built, so the mechanism works; the only question is whether the
  fresh entry feeds it identically. The work-item table's status default / any triage agent would settle
  it. (Not needed for the Phase 0 classifier-only read.)
