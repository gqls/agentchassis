...
**▶ CURRENT POSITION (2026-06-25):** Prereqs done (migration applied; matcher code LIVE in production). **Steps 1, 1b, 2 are COMPLETE** — `reresolve_idea_uk_02b_check_state.sql` confirmed idea.uk is detached (`style_collection_id` NULL), its four composition rows are deleted, and the `resolved_composition` spec is superseded (backups intact). **NEXT: Step 3 (re-trigger), then Step 4 (verify).**
...
### ◀ Step 3 — re-trigger the planner  *(DO THIS NEXT)*
```
bash $DDIR/reresolve_idea_uk_03_trigger.sh
```
Orchestrates `site-design-planner` with `input_data {site_id, domain}`. **`domain` is required** — the planner's first step `ensure_site_record` loads the site by domain and fails with `domain not found in input_data` if only `site_id` is passed. The planner re-runs composition (now scheme-aware → should pick `tool-portal-light`) and emits `needs_design`, which the build-dispatch-loop routes to `webdesign-agent` to re-render `styles.css`. Monitor (the script prints the correlation_id):
```
$PSQL -c "SELECT status, current_step, error FROM orchestration_states WHERE correlation_id='<printed-id>'::uuid;"
```
Expect it to walk: ensure_site_record → validate_composition_inputs → check_ready → resolve_composition_layout → resolve_composition_typography → resolve_composition_palette → install_site_composition → complete. If `needs_design` doesn't advance to a render, re-run the script with `AGENT="webdesign-agent"`.

### Step 4 — verify
```
$PSQL < $DDIR/reresolve_idea_uk_04_verify.sql
```
Expect `layout_name = tool-portal-light` (scheme light), a fresh `resolved_composition` with `palette_source = design_intent_values` and the parchment values, and **no** `needs_new_layout_candidate` queued this run. Then confirm (outside SQL) that the B2 build re-rendered and the page reads light/parchment.
