...
### The finetuning lane's timing hypothesis — TESTED AND REFUTED, and the refutation gives a better mechanism

**Their hypothesis** (offered as one, explicitly not asserted): the 88-vs-35 split might be
re-render **timing** rather than a difference in the loss — a page rendered since the key went
serves regenerated bytes without the value (visible damage), one not yet re-rendered keeps old
bytes that still carry it (invisible). **If true, the 88 would not be safe but PENDING, each
turning into a 35 at its next re-render, and the visibly-broken count would grow with no new
loss.** That would have changed what the fix owes already-damaged rows, so it was worth testing
immediately.

**`[MEASURED 2026-09-04]` It does not hold.** Comparing each row's last write
(`page_components.updated_at`) against its page's render stamp
(`GREATEST(deployed_at, last_built_at)`), over the null-safe 123:

| arm | rows | page rendered AFTER the row write | rendered before / with |
|---|---|---|---|
| html still paints an asset | **88** | **87** | 1 |
| html paints nothing | **35** | **33** | 2 |

**Render-after-write is near-universal in BOTH arms (120 of 123), so it cannot be what separates
them.** Their caveat is carried and it is a real limit on this test: `updated_at` is the row's own
last write, **not the moment the key vanished**, so this is a proxy — but a proxy that comes out
identical on both arms refutes a hypothesis that requires them to differ.

**What the refutation implies, and it is a better account than timing.** If a page was rendered
*after* its row already lacked the key, and the component's `rendered_html` still paints
`hero-home.jpg`/`hero.jpg`, then **the value is being supplied at RENDER time and not persisted
back into `content_data`**. That splits 114's population into two mechanically different states:

- **88 rows — resolved-at-render, unpersisted.** Something (the site-scope brand hero, route 3 of
  `ensureAssets`) resolves at render and reaches the HTML, but the stored row is never updated.
  The page looks fine and shows the *generic* image. **Damage: the stored row is wrong, and the
  page shows the site's shared hero instead of its own.**
- **35 rows — nothing resolves at all.** No value at render, none stored, nothing painted.
  **Damage: visible to a visitor.** The finetuning lane's four are here — they measured
  `rendered_html` carrying neither the page's own content-hero nor the `hero.jpg` fallback, with a
  working positive control on the same site. **That is why a human reported one of those four
  pages and none of the 88.**

⚠ **This changes the fix's acceptance test, not its design.** A carry-forward has nothing to
carry on either arm; the fallback branch is what fills them. But it means **the 88 must be
verified at the STORED ROW, not at the page** — the page already looks acceptable, so an
artefact-only check would read them as fixed before anything changed. Recorded because it is
exactly the shape that made this bug invisible for a month.

> **CORRECTED 2026-09-04 — a check I published in this file two hours ago is HALF INERT, and the
> lane I published it to caught it.** I wrote that the cheap check for attributing a commit to a
> lane is `git show --format='%an %ae' -s <sha>` **plus** `who-owns.py`. The first half
> discriminates **nothing on this tree**: `[MEASURED 2026-09-04, by the 475 lane]`
> `git log --since=1.day --format='%an' | sort | uniq -c` → **998 commits, 998 `cqls`.** One git
> identity, every session. So the author field can never name a lane, and I prescribed it as if
> it could — in the same entry where I was correcting myself for guessing a lane from a subject
> line. **What actually discriminates: the commit-message PREFIX (lanes self-label — `475:`,
