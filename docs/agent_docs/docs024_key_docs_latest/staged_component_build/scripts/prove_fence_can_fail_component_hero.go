...
// prove_fence_can_fail_component_hero.go — the S2 mutation discipline,
// applied to fence_component_hero.json.
//
...
// table) and swaps in this SECTION COMPONENT's own mutants. `hero` carries no JS at all,
...
// USAGE
//
//	go run docs/agent_docs/docs024_key_docs_latest/staged_component_build/scripts/prove_fence_can_fail_component_hero.go \
...
var mutants = []mutant{
	...
	{
		name:       "the hero loses the identity acceptance resolves it by",
		from:       `<section class="hero" data-component="hero"`,
		to:         `<section class="hero" data-component="hero-renamed"`,
		expectFail: []string{"hero-renders"},
	},
	{
		// bugs_open/157's class: present in the DOM, rendered too small to see.
		name:       "the hero collapses below visible size (10x10, overflow hidden)",
		from:       "</body>",
		to:         `<style>section.hero[data-component="hero"]{min-height:0!important;width:10px!important;height:10px!important;overflow:hidden!important;padding:0!important}</style></body>`,
		expectFail: []string{"hero-has-visible-area"},
	},
	{
		// The exact live defect class this tranche found on tool-gas-unit-converter:
		// full structure, empty text slots. An empty h1 is what a hero rendered
		// against missing content_data serves.
		name:       "the headline renders empty",
		from:       `<h1>What It Costs to Work With Us</h1>`,
		to:         `<h1></h1>`,
...
func main() {
	if len(os.Args) != 3 {
		fmt.Fprintln(os.Stderr, "usage: prove_fence_can_fail_component_hero.go <criteria.json> <live-url>")
		os.Exit(2)
	}
	...
	fmt.Printf("=== proving the fence can fail (component-hero) ===\n")
