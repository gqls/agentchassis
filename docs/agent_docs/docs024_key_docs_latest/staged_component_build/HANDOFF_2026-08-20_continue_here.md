...
| class | state |
|---|---|
| `bdl` / `commit_sha` | 🟡 **handler side ESSENTIALLY DONE, 2026-08-21 ~14:0xZ.** Re-scoped the "~16 configs" down to the **real** population: `handler_agent` is dispatched dynamically from `site_work_items.handler_agent` (a top-level column), and of ~34 distinct handler types, only **6 direct + 1 dependent** ever reach a `git_commit` — eleven agents with their own `git_commit` step have **zero** occurrences as a live handler value and are irrelevant to this class. **5 of 6 direct handlers APPLIED + verified live**: 519 page-rerender (dominant, 4,810/7d — verified end-to-end on real post-apply traffic), 521 rerender-pages, 522 css-patch-agent (REVISE round 1 → resubmitted → APPROVED → applied), 523 section-editor, 527 webdesign-agent, 528 nav-updater. **534 page-build-handler** (the one dependent case — no `git_commit` of its own, reaches one only by calling page-rerender) is submitted, awaiting verdict, guarded to refuse unless 519 is live. **After 534 lands, this row's config half is COMPLETE** and the one-line `"commit_sha?"` wire on `mark_complete` is `staged-component-build`'s to build — ordering is the OPPOSITE of 516's: the wire must land AFTER the standardisation or it silently drops the sha for any unconverted handler (moot once 534 lands, since that's the last one). Full path-per-agent table + the sketch-abbreviation and negative-control lessons: NOTES `## 2026-08-21 (~13:5xZ)`. |
