...
| class | state |
|---|---|
| `bdl` / `commit_sha` | 🟢 **HANDLER SIDE COMPLETE, 2026-08-21 ~14:2xZ.** All **9 real handlers** standardised, applied, verified live at the config: 519 page-rerender (dominant, 4,810/7d — verified end-to-end on real traffic), 521 rerender-pages, 522 css-patch-agent (REVISE r1 → resubmitted → APPROVED), 523 section-editor, 527 webdesign-agent, 528 nav-updater, 534 page-build-handler (dependent on 519, guarded in SQL), **535 asset-deployer + 536 image-build-handler** (dependent on 535) — these last two found by `staged-component-build`'s empirical cross-check against `site_work_items.result`, which ALSO flagged `tool-generator` as a **false positive**: its one `commit_sha` hit traced to a sibling loop iteration's `section-editor` commit, cross-contaminated by the exact whole-tree-search defect this bug is about — not a real gap, not built. The real population turned out to be dynamic (`handler_agent` dispatched per-item, not a fixed set), so "the real population" should be re-measured, not assumed, if picked up again. **Remaining: the one-line `"commit_sha?"` wire on `mark_complete`, `staged-component-build`'s, now redesigned around the tool-generator lesson** (guard keys on which handler's own tree CAN produce a commit, not which items historically recorded one — immune to the contamination rather than dependent on it being clean first). Full path-per-agent table + every lesson (sketch-abbreviation, negative-control false-positive, the tool-generator trace): NOTES `## 2026-08-21`, several entries from `~13:5xZ` through `~14:2xZ`. |
