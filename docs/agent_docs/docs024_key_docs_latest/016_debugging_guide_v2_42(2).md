...
### 6.2 Database capabilities — a captured `\df` and `\dx`

§6.1 tells you to *run* `\df` before hand-rolling SQL. This section *captures the answers* so they are discoverable without re-deriving, and adds the extension list (`\dx`) — what the database can do, not just what tables it has. The live `\df` / `\dx` are canonical; treat the blocks below as a scaffold to refresh whenever functions or extensions change, and as the answer to "what helpers/extensions exist" rather than re-querying each time.

**Helper functions — confirmed** (refresh from `\df`):

| Function | Arguments | Returns | Use |
|---|---|---|---|
| `snapshot_agent` | `p_agent_type text` | `uuid` | Back up an agent's `default_config` before patching. Run before ANY `agent_definitions` change (§6.1). |
| `snapshot_agent` | `p_agent_type text, p_reason text DEFAULT NULL` | `uuid` | Same, with a human-readable reason on the snapshot. |
| `revert_agent` | `p_agent_type text` | — | Restore the most recent unrestored snapshot for the agent (§6.1). |

```
-- Run `\df` in psql against the agent DB and replace this block with the actual output.
--  Schema |      Name      | Result data type |          Argument data types          | Type
-- --------+----------------+------------------+---------------------------------------+------
--  public | snapshot_agent | uuid             | p_agent_type text                     | func
--  public | snapshot_agent | uuid             | p_agent_type text, p_reason text DEF… | func
--  public | revert_agent   | …                | p_agent_type text                     | func
--  …
```

**Extensions — what's installed** (refresh from `\dx`). **pgvector is confirmed installed and in use:** `knowledge_base.embedding` is `vector(768)` with an IVFFlat cosine index (`idx_kb_embedding … vector_cosine_ops`) plus a companion trigram index on `content` — so the database already does both semantic and lexical retrieval, and `knowledge_base` is the store the RAG path (`rag_actions.go`) reads. (`\df` also lists `sparsevec`, the matching artefact.) Confirm versions with `\dx`; `\dT *vector*` lists its types. Any embedding storage should reuse this — pgvector columns + the cosine/trigram indexes — not in-Go cosine (migration plan A5 / B4).

```
-- Run `\dx` in psql and replace this block with the actual output.
--                 List of installed extensions
--    Name   | Version |   Schema   |              Description
-- ----------+---------+------------+----------------------------------------
--  vector   | …       | public     | vector data type and similarity search   <- confirms pgvector
--  …
```

Keeping this current is a habit, not a one-off: capture `\df`/`\dx` here whenever a migration adds a function or extension. The context tool is meant to fold these same two captures into the bundle it assembles for DB-touching tasks (so generation sees available helpers/extensions automatically) — see the migration plan; this section is the human-readable copy of that capture.
