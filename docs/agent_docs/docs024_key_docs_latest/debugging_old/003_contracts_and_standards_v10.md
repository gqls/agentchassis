...
The original thunder-adapter `buildResponseHeaders` — **fresh `request_id` via `uuid.New()` plus plain `Produce`** — is the anti-pattern. Any adapter still doing either of those should be migrated to the favoured pattern. Checklist when auditing an adapter's `sendSuccessResponse`/`sendErrorResponse`:
1. Is `request_id` the incoming one, or a new UUID? (must be incoming)
2. Is `message_id` set? (must be)
3. Is it `ProduceWithValidation`? (must be)
