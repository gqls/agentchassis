...
## 6.6 LLM step config is partially ignored — known structural bug

`ExecuteLLMPromptAction` reads `max_tokens` and `temperature` from the agent's top-level config only, never from the step's own config inside `workflow.steps.<step>.config`. Settings like `max_tokens: 32000` at step level are dead config — they appear in the database when you query the agent definition, but the chassis doesn't read them. Tracked in `FOCUS_step_level_llm_config_ignored.md`.

**Symptom.** A step that should produce a long output produces a truncated one. `llm_call_log.output_tokens = 2048` (the hardcoded Anthropic fallback) regardless of what the step config says. JSON outputs are cut mid-object and the downstream validator fails or quietly drops the truncated tail.

**Diagnose with:**

```sql
-- See what max_tokens were actually used
SELECT created_at, agent_type, step_name, model,
       input_tokens, output_tokens, success,
       LEFT(COALESCE(error_message, ''), 200) AS err
FROM llm_call_log
WHERE agent_type = '<agent>' AND step_name = '<step>'
ORDER BY created_at DESC LIMIT 5;
-- If output_tokens = 2048 (exactly), suspect the cap is hitting the hardcoded fallback.

-- See what the step config actually says
SELECT default_config->'workflow'->'steps'->'<step>'->'config'->'max_tokens' AS step_max_tokens,
       default_config->'ai_service'->'max_tokens' AS top_level_max_tokens
FROM agent_definitions
WHERE type = '<agent>' AND deleted_at IS NULL AND is_active = true;
-- step_max_tokens is read by NOTHING today.
-- Only top_level_max_tokens is respected.
```

**Workaround.** Add the desired `max_tokens` to the agent's top-level `ai_service` object. Single value applies to all LLM steps in the agent — wasteful for steps with naturally bounded output, but unblocks the truncating step. SQL pattern (snapshot first):

```sql
SELECT snapshot_agent('<agent>', 'set top-level max_tokens as workaround');
...
**Same bug applies to `temperature`.** Step-level `temperature: 0.2` is also ignored. To force a deterministic step today, add `temperature` to the top-level `ai_service`. The Anthropic API's default (~1.0) is being used everywhere step-level temperature wasn't backfilled into agent-level.

**Same bug applies to whole `ai_service` overrides.** If an agent has a top-level `ai_service`, every step's own `ai_service` override is silently ignored. This breaks the model-swap pattern documented in `023_llm_quality_testing.md` — its SQL writes a new `ai_service` into `workflow.steps.<step>.config.ai_service` but the chassis keeps using the agent's top-level value. Verify with:
...
Any row returned is a step whose `ai_service` override is currently dead config.

**The fix path** (structural, not yet applied): extend `ExecuteLLMPromptAction` to read step config first for `max_tokens`, `temperature`, AND `ai_service`, then fall back to agent config, then to top-level ai_service. See the focus doc for code, line numbers, and per-step audit query to run when the structural fix lands.
