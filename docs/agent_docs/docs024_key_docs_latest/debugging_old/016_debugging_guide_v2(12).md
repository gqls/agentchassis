...
`llm_call_log.temperature` is universally NULL across all calls in the last 14 days, even for the agents (`component-creator=0.4`, `content-creator-*`, `copywriter`, `reasoning=0.2`, `researcher=0.3`, etc.) that have a top-level temperature set. We can't tell from the log whether temperature is reaching the API or not. Two possibilities:
...
Diagnosis without code change: compare output style of `component-creator` (top temp 0.4) vs `content-creator-cta` (0.8). If they look identically variable, the chassis isn't applying temperature. If component-creator looks more structured, temperature is being applied but not logged.
