...
### Adoption faithfulness: convergence preserves by URL/name, residual divergence is `page_type`-driven
...
**Both trace to the `page_type` adoption assigned, via `CanonicalisePage`
(`datahelpers/page_canonical.go`).** That function builds the URL differently per
role, and the convergence in `ValidateSitePlanAction` preserves adopted pages
largely by **exact URL match** (Pass B). When adoption's URL and the planner's
canonical URL agree, preservation works; when they diverge, it doesn't.

- **`game` / `tool` roles strip the prefix into a section path:**
  `CanonicalisePage(game, "game-auto-battler")` → `/games/auto-battler/index.html`
  (no `game-` in the URL). The planner emits the same canonical URL, so Pass B
  matches and snaps the name back to `game-auto-battler`. Preserved. ✓

- **`blog_post` role keeps the full slug verbatim:**
  `CanonicalisePage(blog_post, "guide-rng-design")` →
  `/blog/guide-rng-design.html` (the `guide-` is baked into the URL). There is a
  dedicated `guide` role that *would* strip it (→ `/guides/rng-design/index.html`),
  but the adopted guides are typed **`blog-post`, not `guide`**, so they never
  reach it. The planner emits `/blog/rng-design.html` (no `guide-`), which does
  **not** match the adopted URL → Pass B misses → the de-prefixed LLM name wins.

- **`games-index` / `tools-index` are typed `content`, not a section-index
  role.** `CanonicalisePage`'s section-index branch only fires for
  `section_index` / `blog_index` / `entity_directory` (`isSectionIndexRole`), so a
  `content`-typed `games-index` gets a flat `/games-index.html` URL instead of
  `/games/index.html`. The same type starvation defeats the convergence's
  collision pass (Pass C), whose `sectionStemOf` requires a section-index type —
  so the LLM's flat `games` isn't recognised as colliding with the adopted
  `games-index`, and isn't dropped. `guides-index` is exempt because it was typed
  `blog-index`.

**Root cause (single, upstream).** The `page_type` assigned at adoption time
(`analyze_site` / `apply_adoption_plan`) determines all of this:
`games-index`/`tools-index` should carry a section-index type, and guides should
carry `guide` (or be left as `blog_post` only if the URL convention is made
consistent). The convergence is doing its job for the types whose canonical URL
the planner reproduces; it can't preserve by URL when adoption and planner build
different URLs for the same logical page. This is the same signal-starvation
described in `FOCUS_planner_ignores_adopted_state.md` (Mechanism 2), now isolated
to specific page types.

**Correction to earlier session notes.** Interim diagnoses in this conversation
claimed the convergence "didn't fire" or that "the planner diverged wholesale."
The `site_plan_pages` evidence corrected that: the convergence preserved all 12
games+tools by adopted identity. The residual is narrow and type-specific, not a
wholesale failure. (See the tempo trap above — those wrong calls came from
reading work-item names instead of the plan table.)

**Open sub-question (verify before fixing).** Pass A unions adopted pages by
*name*; in principle it should have re-added `guide-rng-design` even when Pass B
missed. It didn't appear in the plan, which suggests either a downstream
canonicalisation collapse (both `guide-rng-design` and the LLM's `rng-design`
canonicalising to the same name and deduping on the `(plan_id, name)` unique
index) or that the LLM emitted the prefixed names directly. Resolve by reading
the `plan_site` `response_text` in `llm_call_log` for the run — what the LLM
emitted per page — before changing convergence or canonicaliser code.

**Fix direction (not yet applied).** Correct adoption's `page_type` assignment so
section indexes and guides get types whose `CanonicalisePage` URL matches the
planner's canonical output; OR harden the convergence to match by slug-stem
rather than exact URL. Decide after the `response_text` check settles which.
