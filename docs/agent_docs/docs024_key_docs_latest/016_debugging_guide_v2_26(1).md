...
**Patch drafted (2026-05-27, NOT yet deployed).** Three edits in `site_db_actions.go`, all inside `SyncPagesToDBAction` and `upsertPage`:
1. `SyncPagesToDBAction` normalisation loop now builds `[]datahelpers.LLMPlannedPage` and runs `datahelpers.ValidateRoles` over the whole set before the per-page `CanonicalisePage` — a near-verbatim mirror of `write_site_plan_action.go:235–278`, reusing the in-package `firstNonEmptyField`/`firstNonEmpty` helpers. `ValidateRoles` preserves order, so `validated[i]` aligns with `pages[i]`. Logs `CorrectedFromRole` when a role changes, matching the plan-writer.
2. `SyncPagesToDBAction` resolves the current plan id once (`SELECT id FROM site_plans WHERE site_id=$1 AND is_current=true`) and threads it into `upsertPage`.
3. `upsertPage` gains a `planID uuid.NullUUID` param, writes it to a new `built_from_plan_version` `$12` column, and on conflict does `built_from_plan_version = COALESCE(EXCLUDED…, pages…)`. Greenfield callers with no plan pass an invalid `NullUUID` → column stays NULL → behaviour unchanged (this is what keeps the change safe for the plan-less callers, the constraint that drove option 2).

Caveats recorded with the patch, to settle at build time (not in this sandbox — no Go toolchain): run `gofmt`/`go vet`/package build; the plan-id lookup calls `params.DB.QueryRowContext` directly (matches `reconcile_site_plan_action.go`, so `ActionParams.DB` is `*sql.DB` in practice — but `upsertPage`'s type-switch also tolerates `*pgxpool.Pool`, so if any caller passes a pgx pool the lookup would need the `interface{}` path). The existing ON CONFLICT `deployed → needs_rebuild` flip was deliberately left untouched (separate re-plan-triggers-rebuild behaviour, not part of this regression). Files: `site_db_actions.go`, `site_db_actions.patch`.
