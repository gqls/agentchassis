...
| 2D | Drop old `(site_id, purpose)` unique constraint | n/a | **✅ today** | **n/a (constraint removal)** | **partial (multi-image enabled at schema level)** |
| 2E | Variant path through image-build-handler | **delivered today, not yet deployed** | **delivered today, not yet applied** | — | — |
