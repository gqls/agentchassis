...
**Severity:** HIGH (elevated 2026-05-18 — now blocking productivity nearly every dispatch session).
...
**Incidence:** 5 confirmed incidents in this verification cycle alone (2 on 2026-05-15, 2 on 2026-05-18, plus the icon state-machine corruption). Pattern is now the dominant time-cost in operating the system.
...
**Concrete shape:** add a scheduled_tasks row that runs every 5 minutes:
```sql
...
```
15 lines of SQL. Eliminates an entire blocker class. Bumping this to a "do this next" priority.
