...
| 2B–D | Multi-image readiness (asset_key, drop constraint) | — | — | — | — |
...
1. **Phase 2B/C/D — multi-image readiness.** Phase 2A locking shipped today.
   The remaining structural work for assets is `asset_key` column +
   relaxed `(site_id, purpose)` unique constraint + parameterised
   `image-build-handler` deploy paths. Once these land, the 5 hero
   variants become routable.
...
4. **Consider whether the hero variants should keep firing**. They currently
   produce flag-only items every time discovery runs. Once Phase 2B/C/D
   lands, they become routable; before that, they're recorded but produce
