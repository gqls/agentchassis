...
Phase 2G is complete by design, partially applied operationally. All SQL migrations applied. Two Go-side artefacts (the `flattenImageryBlock` path fix and the new `check_unfulfilled_imagery_plan.go`) are staged but await chassis rebuild + rollout. Once those ship, 2G is closed end-to-end. Phase 2H scope confirmed 2026-05-13; design in progress, code awaits adapter source. Phases 3, 4, 5, 6 of the outer plan not started.

### Application status (2026-05-13)
...
| `flattenImageryBlock_patch.go` | Go | ⚠️ staged | Replace lines 768-847 of `write_site_plan_action.go`. Chassis rebuild + rollout required. Without this, `site_plan_imagery` rows don't populate even when planner emits the block. |
| `check_unfulfilled_imagery_plan.go` | Go | ⚠️ staged | New file in `platform/orchestration/actions/discovery_checks/`. Chassis rebuild + rollout required. Without this, `unfulfilled_imagery_plan` is registered but the check class isn't compiled in. |
| `phase_2g_step5_image_build_handler_needs_imagery.sql` | SQL | ❓ unconfirmed | Adds `needs_imagery` branch to image-build-handler workflow. |
| `phase_2g_step4_register_check.sql` | SQL | ✅ | `unfulfilled_imagery_plan` appears in `design-discovery-agent.run_checks` list. |
| `site_plans_unique_current_index.sql` | SQL | ❓ unconfirmed | Adds revert function + history view (partial index was already present). |

To confirm operational state, run the three-line UNION query in the status doc.
...
| 2G.4 — `check_unfulfilled_imagery_plan` discovery check | 📦 designed, ready to deploy | — |
| 2G.5 — image-build-handler `needs_imagery` branch | 📦 designed, ready to deploy | — |
