...
2. Address item 23 (icon visual quality). Likely path: prompt-engineering fix first (cheapest), then consider whether icons need a different generation pipeline entirely.
3. Address item 22 (icon purpose field) — small Go fix, can be done in parallel with item 23 investigation.
4. Address item 21 (services hero coverage) — decide whether services warrants a hero image, and if so, get the planner to emit it.
5. Once robot-hands.com renders correctly end-to-end with appropriate imagery, THEN trigger finetuning.uk for parallel verification.
6. After both sites verified, switch focus to architectural items (6 — watchdog for stuck claims, 12 — asset→page rerender coupling).
