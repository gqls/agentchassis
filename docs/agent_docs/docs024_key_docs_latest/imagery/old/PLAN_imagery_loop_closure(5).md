...
Phases 0, 1, 2A–2F, and 2G steps 1–3 are delivered and verified. Step 3 required two corrective patches (max_tokens bump + `flattenImageryBlock` path fix) discovered during verification on robot-hands.com. Steps 4 and 5 are designed and packaged; 6 not started. Phases 2H, 3, 4, 5, 6 of the outer plan not started.
...
| 2G.6 — `check_legacy_image_prompts_aspect` age-out | — not started | — |
...
6. **Age-out check `check_legacy_image_prompts_aspect`** — detects sites still being served by the legacy `image_prompts` aspect; emits guidance. Written but not registered until 1–5 are stable across multiple sites.
