...
## Update 2026-05-27 — do we still need a handler?

Traced the resolution machinery as it stands today (not as assumed when this doc and the 2026-05-06 supersession were written):

- `plan_sections` resolves `query.*`-sourced fields via the `queryresolve` package (`actions/queryresolve/queryresolve.go`). `queryresolve.Resolve` dispatches by query name. **Only `pages_where_type:<type>` is implemented** — it returns the standard list-item shape (`name`/`title`/`url`/`meta_description`/`nav_label`) for pages of that `page_type`, filtered `status IN ('active','deployed')`.
- The vocabulary comment in the same file also names `pages_under_section:<section>`, but it is **not in the dispatch switch** — it hits `default → unknown query name → error`, falls through, and the field defers.
- `needs_section_data` items are emitted by `createDeferredItems` at `status='needs_human_review'` with empty `handler_agent`. Lifecycle is managed by `loadOpenSectionDataRequests` + `closeResolvedDataRequest`: on a *later* `plan_sections` run, if the field now resolves, the open item auto-closes. So a deferred item only clears if something re-plans the page after the data exists.

**Why the guide-list duplication / stuck items happen.** A guide-list whose `items` field is sourced from the unimplemented `pages_under_section`, or from `pages_where_type` before the listed pages reached `active`/`deployed`, defers. The same list on index and guides-index defers independently, and nothing re-plans those pages once the guides exist — so they sit at `needs_human_review`.

**Decision: a full LLM handler agent (and the `directory-builder` agent) is not needed for the query-resolvable cases.** The machinery already resolves list data from a query; the gaps are (1) a missing query and (2) no re-attempt after upstream data lands. Two pieces:

1. **Implement `pages_under_section:<section>` in `queryresolve`** — a near-clone of `resolvePagesWhereType`, filtering by parent section/page rather than `page_type`. Then guide-list items resolve mechanically.
2. **Add a section-data reconciler — a resolver, not an agent.** Input `site_id` (+ optional `page_name`); load open `needs_section_data` items; for each, read `spec.missing` and re-attempt resolution through the existing `queryresolve`/`sourceResolver` (the data may exist now); on success, write the resolved values where the renderer reads and close via `closeResolvedDataRequest`, then flag the page for re-render; on genuine failure, leave it at `needs_human_review`. Reuses `queryresolve`, the resolver, `closeResolvedDataRequest`, `insertWorkItem`. Can run as a loop discovery check or a build step.

**What stays HITL.** Genuinely-human data — leadership team, pricing tiers, case studies, contact details — has no query. Those items remain `needs_human_review` and are supplied via the API, exactly as the original doc describes. The reconciler distinguishes the two by `spec.missing[].source` — a `query.*` source is resolvable, a human/spec source is not — and does not touch the human ones.

This refines, but does not contradict, the 2026-05-06 supersession: list data is still query-sourced; the new point is that closing the loop needs a small reconciler, not a new agent.
