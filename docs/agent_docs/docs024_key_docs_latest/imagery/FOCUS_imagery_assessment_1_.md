# FOCUS — Imagery: Where We Are Now
...
### 5.1 Per-page hero resolution — gap and decision (2026-05-27)

Observed on gamesdesign.co.uk after a full adoption build: the pipeline generated and committed per-page assets (`hero-home.jpg`, `hero-games.jpg`, `logo.jpg`, `icon-*.jpg`) and the `needs_imagery` items completed, but the deployed pages render `url('/assets/images/hero.jpg')` — never produced — on every page, and the header shows a text mark, not `logo.jpg`. The assets are correct; the wiring from asset to component is the gap.

Root cause is three things stacked:

1. **The section resolver was not page-aware.** `plan_sections`' `sourceResolver.ensureAssets` mapped a single site-wide `content_data["hero_url"]` to `assets["hero"]`. `store_asset` writes `content_data["<purpose>_url"]` keyed by purpose, and every page hero is purpose `hero`, so heroes overwrite each other (last-write-wins). The per-page keys this doc lists in §5 (`hero_home_url`, `hero_about_url`, …) are recognised by `BuildRenderContextAction` but are not what `plan_sections` reads, and only `hero_home` is generated anyway.
2. **Timing.** `needs_imagery` runs asynchronously after the page first renders, so at first render `site_assets.hero` was unresolved, `on_missing: use_fallback` fired, and the static `/assets/images/hero.jpg` baked into `page_components.rendered_html`.
3. **The terminal rerender reassembles, it does not re-resolve.** `needs_rerender` and the colour/CSS fixers regex-patch stored `rendered_html`; they do not re-run `plan_sections`. So the baked fallback survives even once the asset exists.

**Decision.** Make `plan_sections`' `ensureAssets` page-aware: resolve this page's hero from `site_plan_imagery` (`scope='page'`, `scope_ref=pageName`, `kind='hero'`) joined to `assets` (`spi.key = a.asset_key`, `a.url` is the web path), and the site logo from the site-scope row, keeping `content_data` as gap-fill for legacy/adopted sites. Couple it with a re-render trigger: a terminal step in image-build-handler (`flag_page_image_rebuild`) that, for page-scoped imagery, flags the page `needs_rebuild` and emits `needs_page` → `page-build-handler` at priority 99 so the page re-resolves *through* `plan_sections` after its asset lands. Delivered 2026-05-27 as the edited `plan_sections_action.go` and new `flag_page_image_rebuild_action.go`.

**Two open items this leaves:**

- **Logo is out of scope of that fix.** The header is a site component rendered by `render_site_components`, not by `plan_sections`, so a page rebuild does not touch it. Resolving `site_assets.logo` in the header render path is a separate follow-up.
- **Reconcile the two resolution paths.** `BuildRenderContextAction` (the per-variant `hero_*_url` merge described above) and `plan_sections`' `site_assets` resolution both exist. Before trusting the section-resolver fix, confirm which path the deployed hero actually comes through, and confirm the hero component declares its background as a `site_assets.hero` field with `/assets/images/hero.jpg` as its `use_fallback` (in `content_components.input_schema`) rather than hardcoding the path in `html_template` — if hardcoded, the fix moves to the template.
