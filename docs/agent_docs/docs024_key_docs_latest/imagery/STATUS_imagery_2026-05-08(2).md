...
| 2C | `store_asset` writes `asset_key`; switch ON CONFLICT | — | — | — | — |
...
1. **Phase 2C — `store_asset` writes `asset_key`, switch ON CONFLICT.**
   Go-side change to `v3_site_actions.go`. Accept optional `asset_key`
   config (default to `purpose`). Update INSERT and ON CONFLICT clause
   from `(site_id, purpose)` to `(site_id, asset_key)`. Backward-compatible:
   existing callers don't pass asset_key, so default-to-purpose preserves
   their behaviour exactly. Phase 2B delivered today is the prerequisite.

2. **Phase 2D — drop old `(site_id, purpose)` unique constraint.** After
   Phase 2C is bedded in (a few generations through the loop with no
   issues), drop the old index. Single statement migration. After this,
   the multi-image case is unblocked: `image-build-handler`'s deploy
   paths can be parameterised by purpose (Phase 2D' / 2E if we want a
   distinct phase for that), and the 5 hero variants become routable.
...
5. **Consider whether the hero variants should keep firing**. They currently
   produce flag-only items every time discovery runs. Once Phase 2C+D
