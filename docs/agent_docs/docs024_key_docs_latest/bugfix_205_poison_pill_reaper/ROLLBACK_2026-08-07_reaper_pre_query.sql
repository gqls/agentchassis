-- ROLLBACK 2026-08-07 — restore the stale-orchestration-reaper pre_query to its
-- pre-205 text (generated mechanically from BACKUP_2026-08-07_reaper_pre_query.sql;
-- do not hand-edit the quoted block). Restores unconditional resets: only for use
-- if parking misbehaves. DO/RAISE so a miss aborts the COMMIT.
BEGIN;
DO $do$
DECLARE n int;
BEGIN
...
$PQ$, updated_at = now()
  ...
  IF n <> 1 THEN RAISE EXCEPTION 'expected exactly 1 row, got %', n; END IF;
END
$do$;
DO $do$
BEGIN
  IF (SELECT count(*) FROM scheduled_tasks
       WHERE name='stale-orchestration-reaper'
         AND pre_query NOT LIKE '%tasks_parked%') <> 1 THEN
    RAISE EXCEPTION 'pre_query still carries the parking branch after rollback';
  END IF;
END
$do$;
COMMIT;
