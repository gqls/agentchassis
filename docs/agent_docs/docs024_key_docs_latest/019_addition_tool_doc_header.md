...
removes the block where `rendered_html` is produced, so the public page carries
none of it. That's why it can be richer than a one-liner — and why general
...
| Render/deploy | `StripToolDocHeader` runs in the `rendered_html` path for every component (cheap no-op when absent). `tool_health`'s `no_script` check still passes — only the comment block is removed. |
