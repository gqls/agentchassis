...
## 5. Deploy on update
"Commit is deploy", target swapped: the **VM-sites repo's** Action fires on each
commit, ships the rebuilt static files (and the engine binary when it changes) to
the box, and restarts the service. The terminal build item is unchanged and
target-agnostic — it just commits to the site's repo; the repo's Action does the
VM-specific work. Start with one VM repo → one Action → one box; add per-domain
box routing only when relocation is needed.
