...
*v2_31 (2026-06-05): added the adoption-convergence clobber and no-op failure patterns (§9), and guide page_type URL/list specifics (§6.5). See FOCUS_adoption_faithfulness_via_locks for the full subsystem state.*
...
**Why.** `reconcilePlanWithRealised` gates on `adoption_locked`, which `load_existing_pages` (054) computes **only** via the first-plan branch: `NOT EXISTS (an is_current plan for this site)`. So it is `true` only when no current plan exists at build time. It ends up inert when: (1) a current `site_plan` already exists — any re-plan of an existing site (the 90-day re-plan directive branch was designed but NOT deployed); (2) the query predates 054 and doesn't emit `adoption_locked`; (3) `existing_pages` doesn't reach the `validate_plan` step's `CollectedData`, so `ValidateSitePlanAction` sees an empty set and returns early.
...
```
Log: `grep 'reconciled with adoption-locked pages'` → all-zero `unioned_in/dropped_collision/snapped_rename` == no-op.
