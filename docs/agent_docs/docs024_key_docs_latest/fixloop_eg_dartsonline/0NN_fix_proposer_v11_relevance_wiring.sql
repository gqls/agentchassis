...
--
-- v11 (2026-07-17): RELEVANCE FILTER WIRING (no new seat). Inserts a
-- select_panel step (action select_review_panel) after persist_plan and a
-- conditional gate before each of the 4 advisory seats (bug_historian,
-- reuse_agent, guidelines, tooling_provenance), so each runs ONLY when the
-- fix's edited files/diagnosis match its footprint; edit-quality + guardian
-- stay always-on. A skipped seat's review field is absent, which
-- council_decide now tolerates as an abstention (the fleet-wide Go change in
-- the chassis image that carries select_review_panel).
-- >> DEPLOY ORDER: apply this ONLY AFTER the chassis image carrying the
--    select_review_panel action is live in the pod
--    (strings /app/agent-chassis | grep -c SelectReviewPanelAction >= 1) --
--    a workflow naming an unregistered action fails at that step. <<
-- Design: docs026_concept_register/DESIGN_relevance_filter.md sec 7.
--
-- v10 (2026-07-17): adds a sixth council reviewer, review_tooling_provenance --
-- ADVISORY (approve|object, no veto). Candidate #10 from the concept register's
-- "ten more" list, grounded in CTXK-015 (the single most-rediscovered concept,
-- 11 sources: cmd/bundle/contextkit investigation lore + the resolve-from-registry
-- trap) and DOC-010 (travelling PLAN/NOTES in doc_plans/doc_notes). Lens: does
-- the fix use the platform's own investigation + documentation machinery or
-- reinvent/work around it? A SPECIALIST seat -- narrow relevance -- so it is a
-- prime candidate for the relevance filter (DESIGN_relevance_filter.md); its
-- footprint is already in that config. Applied always-on for now only because
-- the filter's chassis image isn't deployed yet (another thread leads that);
-- it auto-gates the moment the filter lands. Full design:
-- docs026_concept_register/PILOT_tooling_provenance_reviewer.md.
--
-- v9 (2026-07-17): CORRECTNESS FIX (no new seat). run_checks.check_fields
-- listed only editquality + guardian, so the three advisory seats
-- (bug_historian, reuse_agent, guidelines) could request read-only SQL checks
-- in their output that were NEVER executed or fed back on a revise round.
-- Extended check_fields to all five reviewers. diagnose_run_checks tolerates
-- absent/empty check lists (confirmed: action comment "Absent or empty fields
-- are fine"), so approving reviewers with no checks contribute nothing. Gap
-- caught by the council-gate thread's review of the reuse-agent pilot; its own
-- gate seed already ran all seats' checks. Pure SQL, no image.
--
-- v8 (2026-07-17): adds a fifth council reviewer, review_guidelines --
-- ADVISORY (approve|object, no veto). Two lenses per FIX-036: (1) adherence
-- to documented conventions/contracts (wrapper-orchestrator, idx_swi_dedup,
-- truthful provenance, declared contracts, schema-source tiers); (2) "did the
-- guideline fall short" -- a guideline-GAP (the rule itself is wrong, not the
-- fix) leans SIDE-TASK per FIX-016/FIX-042: it goes in notes with an APPROVE,
-- NOT an objection, so a correct fix isn't forced to revise for exposing a bad
-- rule. Last always-on seat before the relevance-filter (see
-- PLAN_concept_register.md); further specialist seats gate behind it. Full
-- design: docs026_concept_register/PILOT_guidelines_agent_reviewer.md.
--
...
SELECT snapshot_agent('fix-proposer', 'pre-update: v11 — relevance filter wiring (select_panel + 4 gates)')
...
'```'
        ...
        'select_panel', jsonb_build_object(
          ...
          'description', 'Stage-3 relevance filter: deterministic (no-LLM) step that decides which OPTIONAL reviewer seats are relevant to THIS fix, from the edited file paths + diagnosis text vs a config footprint map. Emits panel.run_<seat> booleans the gates below check. Skipped seats abstain; council_decide tolerates absent fields.',
          'output_field', 'panel',
          'next_step', 'review_editquality',
          'config', jsonb_build_object(
            'plan_field', 'plan_persisted',
            'extra_text_fields', jsonb_build_array('diagnosis_row.conclusion'),
        ...
        'gate_bug_historian', jsonb_build_object(
          'action', 'conditional',
          'description', 'Relevance gate: run the bug-historian only if the fix touches a rebuild/rerender/render path; else skip (it abstains).',
          'config', jsonb_build_object(
        ...
        'gate_reuse_agent', jsonb_build_object(
          'action', 'conditional',
          'description', 'Relevance gate: run the reuse-agent only if the fix adds new code/tables/migrations; else skip.',
          'config', jsonb_build_object(
        ...
        'gate_guidelines', jsonb_build_object(
          'action', 'conditional',
          'description', 'Relevance gate: run the guidelines reviewer only if the fix touches contracts/work-items/agent-defs/schemas; else skip.',
          'config', jsonb_build_object(
        ...
        'gate_tooling_provenance', jsonb_build_object(
          'action', 'conditional',
          'description', 'Relevance gate: run the tooling & provenance reviewer only if the fix touches investigation/doc tooling; else skip.',
          'config', jsonb_build_object(
...
'You judge two things about this fix plan: (1) does it FOLLOW the platform''s documented conventions and contracts; (2) where it appears not to, is that because the PLAN is wrong, or because the RULE is? You change nothing; you judge.' || chr(10) || chr(10) ||
...
'{"reviewer": "guidelines", "verdict": "approve|object", "objections": [{"edit": 1, "problem": "names the specific rule violated", "severity": "low|medium|high"}], "missing": [], "checks": [{"sql": "SELECT ...", "why": "what this settles"}], "notes": "any guideline-gap goes HERE (approve + note), not in objections"}'
        ...
        'review_tooling_provenance', jsonb_build_object(
          'action', 'execute_llm_prompt',
          'description', 'Council reviewer 1.95 -- tooling & provenance: does this fix use the platform''s own investigation (cmd/bundle/contextkit) and documentation (doc_plans/doc_notes travelling docs) machinery, or reinvent/work around it? Advisory only (no veto -- see PILOT_tooling_provenance_reviewer.md). Candidate #10, grounded in CTXK-015 (most-rediscovered concept, 11 sources) + DOC-010.',
...
'You judge one thing: does this fix use the platform''s own investigation and documentation machinery, or reinvent / work around it? You change nothing; you judge.' || chr(10) || chr(10) ||
'## The platform''s own machinery (use it; don''t reinvent it)' || chr(10) ||
'- INVESTIGATION: cmd/bundle / contextkit is the platform''s tool for reading a change''s real supported method before touching it -- the standing rule is "that''s a code question -> a bundle." Actions resolve from the REGISTRY (key -> Handler symbol -> function), NEVER by filename convention (execute_llm_prompt lives in ai_actions.go; some actions lack the _action suffix) -- a fix that assumes a file name from an action key is repeating a documented recurring mistake.' || chr(10) ||
'- TRAVELLING DOCS: every tool/pipeline/agent carries a living PLAN + NOTES in Postgres (doc_plans / doc_notes, keyed by subject_type + subject_key). Fixes are supposed to load the subject''s prior decisions before changing it and leave a NOTES entry -- so the next fix builds on this one instead of re-deriving lost context. (The fix loop itself uses exactly this pattern via diagnosis_artifacts / doc_notes.)' || chr(10) ||
'- DOC HYGIENE tooling (dedup / thin_versions / archiving) already exists.' || chr(10) || chr(10) ||
'Judge the plan: (a) does it add new ad-hoc context-gathering / bundling / source-parsing code where cmd/bundle/contextkit or an existing action already does it; (b) does it touch a tool/pipeline that has a travelling PLAN/NOTES without accounting for it; (c) does any edit resolve an action/handler by filename convention rather than the registry; (d) does it reinvent existing doc/context tooling. If none apply (most fixes touch no tooling at all), approve.' || chr(10) || chr(10) ||
...
'{"reviewer": "guardian", "verdict": "approve|object|veto", "objections": [{"edit": 1, "problem": "...", "severity": "low|medium|high"}], "missing": [], "checks": [{"sql": "SELECT ...", "why": "what this settles"}], "notes": "..."}'
        ...
        'repropose', jsonb_build_object(
          ...
          'config', jsonb_build_object(
            ...
            'temperature', 0.0,
            'input_fields', jsonb_build_array('diagnosis_row', 'plan_persisted', 'review_editquality', 'review_bug_historian', 'review_reuse_agent', 'review_guidelines', 'review_tooling_provenance', 'review_guardian', 'check_results'),
            'output_format', 'json',
            'prompt_template',
...
'## Guidelines reviewer said (advisory, no veto; a guideline-gap is a note not an objection)' || chr(10) || '{{.review_guidelines.result}}' || chr(10) || chr(10) ||
'## Tooling & provenance reviewer said (advisory, no veto)' || chr(10) || '{{.review_tooling_provenance.result}}' || chr(10) || chr(10) ||
