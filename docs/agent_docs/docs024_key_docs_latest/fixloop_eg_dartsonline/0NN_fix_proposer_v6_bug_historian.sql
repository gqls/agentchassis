...
SELECT snapshot_agent('fix-proposer', 'pre-update: v6 — adding review_bug_historian advisory council seat')
...
'{"reviewer": "guardian", "verdict": "approve|object|veto", "objections": [{"edit": 1, "problem": "...", "severity": "low|medium|high"}], "missing": [], "checks": [{"sql": "SELECT ...", "why": "what this settles"}], "notes": "..."}'
        ...
        'repropose', jsonb_build_object(
          ...
          'config', jsonb_build_object(
            ...
            'temperature', 0.0,
            'input_fields', jsonb_build_array('diagnosis_row', 'plan_persisted', 'review_editquality', 'review_bug_historian', 'review_guardian', 'check_results'),
            'output_format', 'json',
            'prompt_template',
