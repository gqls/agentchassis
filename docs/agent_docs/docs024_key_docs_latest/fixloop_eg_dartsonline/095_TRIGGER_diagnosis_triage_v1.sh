#!/usr/bin/env bash
# 095_TRIGGER_diagnosis_triage_v1.sh — fire the triage router (manual, v1; ships dry_run).
...
TARGET_AGENT_TYPE='diagnosis-triage'
...
ORCH_NAME="triage-v1-$(date +%H%M%S)"
...
kubectl -n kafka run -i --rm "kcat-triage-$(date +%s)" \
