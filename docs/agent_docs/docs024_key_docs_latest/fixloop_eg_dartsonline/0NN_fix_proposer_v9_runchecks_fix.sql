...
SELECT snapshot_agent('fix-proposer', 'pre-update: v9 — run_checks.check_fields fix (advisory seats'' checks were never executed)')
...
'{"reviewer": "guardian", "verdict": "approve|object|veto", "objections": [{"edit": 1, "problem": "...", "severity": "low|medium|high"}], "missing": [], "checks": [{"sql": "SELECT ...", "why": "what this settles"}], "notes": "..."}'
        ...
        'repropose', jsonb_build_object(
          ...
          'config', jsonb_build_object(
            ...
            'temperature', 0.0,
            'input_fields', jsonb_build_array('diagnosis_row', 'plan_persisted', 'review_editquality', 'review_bug_historian', 'review_reuse_agent', 'review_guidelines', 'review_guardian', 'check_results'),
            'output_format', 'json',
            'prompt_template',
