...
-- CORRECTED 2026-07-20 (bugs_closed/031): this bullet previously asserted a content-hash
-- skip in scoped mode. No such code exists and never did; the live rows were patched the
-- same day (PATCH_render_guardian_031_content_hash.sql + 099 sync). Corrected here too so
-- a replay of this seed cannot resurrect the false claim.
'- TWO RERENDER MODES, DIFFERENT SKIP SEMANTICS: scoped mode (spec.reason = image_landed / section_data_resolved) re-renders each stored section from its html_template + stored content_data. There is NO content-hash skip (bugs_closed/031: that claim was never true of the code). Its real bail-outs (rerender_page_sections_action.go): page-level -- skipped when the page has no stored components, escalated-to-writer when any section''s stored content_data is absent or missing a required llm field; neither writes nor deploys. Section-level -- stored HTML carried when the component cannot be loaded, its plan is not ready, or html_template is empty. Assemble mode (page_id, no reason) re-embeds chrome unconditionally. A fix routing chrome-only changes through scoped mode can silently miss exactly the pages that bail out. When judging a rerender claim, cite the code path, not a register entry -- the register is documentation, not the implementation.' || chr(10) ||
...
'Judge the plan: (a) does any edit add a render path that can silently drop/blank/skip content instead of failing loud or escalating; (b) does it route chrome/header/footer changes through scoped rerender (whose page-level bail-outs can leave pages undeployed) instead of assemble mode; (c) does it break the data-runtime-fill exemption or edit rendered artifacts instead of source templates; (d) does it hardcode colours / bypass the var() inheritance chain, or remove a validation layer. If the fix does not touch rendering/assembly/styling, approve.' || chr(10) || chr(10) ||
