...
'5a. WHOLE-CONFIG vs SURGICAL — the seed shape depends on whether the row EXISTS. Creating a NEW agent definition: a full INSERT (the house template) is correct. EDITING an EXISTING live agent definition (adding/changing steps or step config): the seed MUST be a SURGICAL UPDATE using jsonb_set on ONLY the specific paths it changes — NEVER a whole-column replacement of default_config, and never an upsert that reconstructs the whole workflow. Reason (learned the hard way on this platform, twice): these rows are CO-EDITED by concurrent threads; a whole-column rewrite silently drops any step or key that was not in your partial view of the live row, with no error and no failed work item. You cannot enumerate the live row exhaustively — your schema view is partial and your verification queries truncate — so a shape that is safe only if your view was complete is the wrong shape. Surgical jsonb_set leaves every unaudited step byte-identical BY CONSTRUCTION rather than by a post-apply diff that runs after the damage.' || chr(10) ||
