...
'5. SEEDS ARE FILES: anything DB-side (a new agent definition, a workflow config) ships as a .sql seed FILE in the PR — operation "add", artifact_role "seed" — NEVER as a live change. Every seed needs a post_merge_checklist entry, and image_deploy must come strictly before any seed_apply (a seed naming an unregistered action fails at runtime). A seed applied via ON CONFLICT (type, version) DO UPDATE already ACTIVATES the new definition atomically — do NOT add a separate stage or checklist step to "activate" or "bump the version"; applying the seed IS the activation.' || chr(10) ||
