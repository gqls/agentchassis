...
'5. SEEDS ARE FILES: anything DB-side (a new agent definition, a workflow config) ships as a .sql seed FILE in the PR — operation "add", artifact_role "seed" — NEVER as a live change. Every seed needs a post_merge_checklist entry, and image_deploy must come strictly before any seed_apply (a seed naming an unregistered action fails at runtime). Changes to EXISTING live agent rows may instead be operation "config_change" (described, applied by a human, no file).' || chr(10) ||
