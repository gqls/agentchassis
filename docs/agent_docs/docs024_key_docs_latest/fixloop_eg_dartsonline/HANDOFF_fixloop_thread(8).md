...
⚠ REGENERATE: the first BUNDLE_fixloop_F0.md was built WITHOUT -psql, so it
carries code+docs only — no schema, no runtime evidence, no capability facts.
For the guides pilot the DB half is where the answer probably lives (the
pages/site_work_items rows and the gamesdesign differential). Rebuild with
-psql, and note: ONE quoted argument, NO -it/-t (output is captured; a TTY
breaks it). cmd/bundle's skip message + usage text were patched to say so
(cmd/bundle/main.go in this thread's outputs — needs gofmt + build).
...
go run ./cmd/bundle \
  ...
  -schema-tables pages,page_components,site_work_items,site_specs,sites,agent_definitions \
...
2026-07-07). BUNDLE_fixloop_F0.md is GENERATED and verified (199,579B, all
scopes resolved) — carry it into the project folder. ★ THE PILOT IS CONFIRMED (2026-07-07): **dartsonline published a Guides nav
link and a blank /guides/index.html, while gamesdesign.co.uk — same platform
— has working guides, games and tools, and gaswholesalers.com has a live news
feed.** The system linked to something it did not build: a broken route, not
a missing feature. Full symptom string, the five-criteria justification, the
standing hypothesis (reconcile_site_plan's routing table omits "guide";
nav derives from the plan, not the built set), the mandatory pre-check
queries and the gamesdesign DIFFERENTIAL are in the runbook's ★ F0 PILOT
section. Two earlier candidates were rejected as absent-decision/absent-
connection findings and moved to RUNBOOK_builder_route.md items 6 and 7 —
that triage history is itself the worthiness test working. Imagery remains
another chat's. First action: slice **F0.1** with
