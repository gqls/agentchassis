...
**Step 1 — choose the singular handling. RESOLVED → flatten (see draft).**

- *A-flatten (chosen)* — coordinator returns the single field's CONTENTS as the
  body (flat). Matches all three live parents + site-work-orchestrator's planner
  reads + model-trainer's reads + dev-guide 1081. No consumer/save edits.
  Fragility addressed separately by the RENAME (singular `result_from` /
  `output_field`; plural `multiple_output_fields` / `output_fields`), not by
  forcing singular==plural. Drafted in `resolveResultSpec_draft.go`.
- *A-nest (rejected)* — `output_field` as one-element nested list. Would leave
  site-work-orchestrator (reads `site_plan.response.needs_images` etc.) AND
  model-trainer (reads `preparation_result.dataset_uri`) BROKEN, plus require the
  page-build-handler path edits. Wrong for the real consumers.
- *B (config)* — superseded by the chassis fix; would need per-agent path edits.

**Claims VERIFIED against live workflows (was "to verify"):** flatten breaks none
of the four and is corrective for more than two —
  - writer → page-build-handler/page-rebuild/site-work-orchestrator: FIXED (flat
    page_html/sections_metadata reads resolve).
  - site-planner → site-work-orchestrator: FIXED (flatten lifts `needs_images`,
    `image_prompts`, `style_collection`, `pages` to `site_plan.response.*`; the
    earlier "unconsumed" was a `validated_plan`-string-search artifact).
  - training-data-preparer → model-trainer: corrective (flat matches the reads +
    the producer's own comment).
  - thunder-reaper: `reaper_summary` never set → flatten yields metadata-only; no
    awaiting consumer → safe.
  - content-reviewer (auto_eval): blind now and under flatten (reads dump keys);
    HITL improves; follow-up repoint to sections_metadata. Not a regression.
