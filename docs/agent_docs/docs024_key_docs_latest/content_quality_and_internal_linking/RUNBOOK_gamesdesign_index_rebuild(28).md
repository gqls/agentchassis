...
> **Step checkboxes:** each step heading below carries a `[ ]`. Change it to `[x]` when that step is done, so progress is visible at a glance.
>
> **Interactive checklist:** `RUNBOOK_gamesdesign_checklist.html` mirrors these parts/steps as tickable boxes with per-part and overall progress (open it in a browser; ticks are saved locally). Seeded to the 2026-06-24 status.
...
Status: **cause confirmed (2026-06-22); fix WRITTEN (2026-06-24, see P4.3).** A different failure from a silent
...
> **UPDATE (2026-06-24) — fix WRITTEN (patched `save_page_sections_action.go`); apply via P4.3.**
> Correction to item 2 below: **Layer 2 is implemented in `save_page_sections` (carry-forward of
> existing interactive sections), not in `load_existing_content` / `plan_sections`.** Reading the
> source showed the tool's interactive `<canvas>`/JS markup exists *only* as
> `page_components.rendered_html`; the planning path traffics in section-*name* skeletons and
> LLM-regeneratable content and cannot reconstruct the tool — so preservation must happen at the
> save path. **Both layers (guard + carry-forward) and the `source_item_id` stamp are in that one
> patch.** The numbered detail below is retained as the reasoning trail.
...
Recommended order: 1 (protect) → 2 (fix). The earlier "repoint `link_resolution_rebuild` →
`page_rerender`" plan is **ruled out** — P4.2 confirmed `page_rerender` doesn't re-resolve the
schema-sourced CTA, so it would preserve the tool but reproduce the phantom. Layer 3 survives only
as a theoretical option (extend the re-render to re-run CTA resolution), not worth it once Layer 2
lands.
...
**P4.2 RESULT (2026-06-22) — re-render preserves sections, does NOT re-resolve the CTA.**
Item `398dedc2` completed clean; both sections re-rendered (`updated_at` → 17:08:23), hero and
generic-text both intact → `page_rerender` is confirmed **non-destructive**. But `cta_url` stayed
`/contact.html` (no flip to `/contact/index.html`) → `page_rerender` does **NOT** re-resolve the
schema-sourced hero CTA. So **routing `link_resolution_rebuild` → `page_rerender` is ruled out** as
a link fix. Clobber fix is firmly **Layer 1 (guard) + Layer 2 (preserve interactive sections
through the full rebuild)**, keeping `link_resolution_rebuild` on `page-build-handler` and making
it non-destructive. Layer 3 (repoint) is dead unless the re-render is extended to re-run CTA
resolution — not worth it once Layer 2 lands.

**Separate thread — phantom hero CTAs (CONFIRMED in deployed HTML 2026-06-22).** The deployed
`guide-economy-basics` hero has TWO phantom CTAs: primary `cta_url=/contact.html` ("Read the Full
Guide") and secondary `secondary_cta_url=/services.html` ("Browse Economy Guides"). Real contact
is `/contact/index.html` and there is NO services page — and the page's own FOOTER correctly links
`/contact/index.html`, so the bug is specifically the hero CTA resolution, not site-wide. These two
values map exactly to the hero component's schema sources (`cta_url` ← `pages.contact`,
`secondary_cta_url` ← `pages.services`), which appear to produce a constructed `/{area}.html` (wrong
format for contact; fabricated for a non-existent services page). The intent-aware
`resolve_internal_links` (which should point these at real content hubs by nav_order, excluding
contact) is NOT overriding them.

History caveat / retraction: `page_component_history` for this page shows ONLY the P4.2 write
(06-22) — NO 06-14 row — so I can NOT claim the §5 full rebuild produced/kept the phantom (unlike
pathfinding, which had 06-14 rows; history was capturing then). The §5 rebuild likely didn't re-save
this hero. The phantom is probably an earlier build's value no rebuild has corrected.

Two-part bug: **(A)** the `pages.<area>` source resolver produces wrong/fabricated URLs (correctness
bug); **(B)** `resolve_internal_links` isn't running on / winning over these heroes, so the generic
brochure-style schema defaults (contact/services) leak through. **(B) is the structural fix** (real,
intent-appropriate links — the "well-linked sites" goal); (A) is a source-resolver correctness fix.

DIAGNOSTIC RESULT (2026-06-22): **hubs exist** — section-index pages: games-index
(`/games/index.html`, nav 4), about (5), contact (6), tools (100), guides (100), all active. Run
through `resolve_internal_links`' `chooseCTATargets` for a guide page: exclude about/contact →
games/tools/guides remain → primary = games-index (`/games/index.html`, lowest nav_order), secondary
= guides-index (`/guides/index.html`). So IF the resolver ran/won the hero CTAs would be those REAL
links — not the deployed `/contact.html` + `/services.html`. The resolver is NOT failing for lack of
hubs; its output isn't reaching the hero. (The `unresolved_cta` query result was garbled — a shell
`grep` was pasted into psql, leaving a `-#` continuation — so it's not captured; but with hubs present
the resolver wouldn't emit `unresolved_cta` anyway, so re-running is low-value.)

This narrows the bug to **wiring/precedence**, a code question: is `resolve_internal_links` even
called by page-content-writer, and if so does the generic schema field resolution
(`cta_url`←`pages.contact`, `secondary_cta_url`←`pages.services`) overwrite it?

RESULT (2026-06-22): the resolver IS wired — `ILIKE '%resolve_internal_links%'` on
page-content-writer's config returned **false**, but that's a wrong probe: the step list has
`prepare_link_context` / `spawn_link_resolver` / `resolve_links`, i.e. page-content-writer **spawns
the internal-link-resolver as a sub-agent** (the action lives in that agent's config, not the
writer's). (Don't-trust-one-query: the false ILIKE is a probe artifact, not absence.) Flow:
`… process_sections_loop (LLM) → prepare_link_context → spawn_link_resolver → resolve_links →
compile_page`. So the resolver runs after content gen, augments each CTA section's `resolved_data`
with a real hub, and returns `sections_ready`. Yet the deployed hero shows the schema defaults
(`/contact.html` + `/services.html`), not the hubs the resolver would compute
(`/games/index.html` + `/guides/index.html`). So its augmented output isn't reaching the rendered
hero. Two candidate mechanisms (both guideline-warned): (1) the spawned agent's `sections_ready`
isn't threaded back into what `compile_page` renders — output-mapping / workflow-var-name mismatch,
or the sub-agent responds to its own topic not the parent's; (2) the resolver's `resolved_data` is
overwritten at render by the schema-source resolution (merge order favours the schema value); (3) the
spawn target has no matching `agent_definitions` row → spawn no-ops.

NEXT (trace the data flow + confirm the sub-agent):
```sql
-- A: full configs of the link + compile steps — does the resolver's sections_ready reach compile_page?
SELECT key AS step, jsonb_pretty(value) AS config
FROM agent_definitions ad, jsonb_each(ad.default_config #> '{workflow,steps}')
WHERE ad.type = 'page-content-writer'
  AND key IN ('prepare_link_context','spawn_link_resolver','resolve_links','compile_page','build_render_context')
ORDER BY step;

-- B: which agent is spawned, does it exist, and does it call the resolver action?
SELECT type, (default_config::text ILIKE '%resolve_internal_links%') AS calls_resolver_action
FROM agent_definitions WHERE type ILIKE '%link%' OR type ILIKE '%resolver%';
```
Then read `render_component` / the field resolver for merge precedence. Structural fix **reuses the
existing resolver** (it computes the correct links); likely realign a variable name so the resolver's
`sections_ready` is what `compile_page` renders, or fix merge precedence. NEITHER Layer 1 nor Layer 2
fixes this.

PATTERN PRECEDENT (render_site_components_action.go, read 2026-06-22): the SAME `/contact.html`
phantom was already fixed for SITE components. It captures the real contact page URL from nav items
(`GetNavItems` → real `pages.url`) for the header CTA "instead of the hardcoded phantom /contact.html"
(lines 141–157), fixed `/privacy.html`/`/terms.html` legal phantoms the same way (159–171), and gates
to no-button when the page is absent (226–233) — which is why the deployed footer/header link
`/contact/index.html` correctly. The phantom's source is the deprecated `loadNavItems`/`loadFooterNavItems`
`COALESCE(url, '/' || name || '.html')` construction; the hero's `cta_url` (schema source `pages.contact`)
is producing that same `/{name}.html` value. CONVERGENCE: `resolve_internal_links` ALREADY embodies the
correct pattern (real-page-only via `validPages.Contains`, gate if no hub) and would set a real hub
(`/games/index.html`); it just isn't reaching the rendered hero. So the page-side fix is the SAME as the
resolver-data-flow fix — get the resolver's `sections_ready` to drive the render — and it's reuse, not new
URL logic (the resolver is already correct, like render_site_components now is). The orchestration check
above decides where the output is lost (fallback vs render override).

ORCHESTRATION RESULT (2026-06-23) — CONFIRMED two stacked bugs, both config-level (NOT topic, NOT
render override, NOT no-hubs):
- page-content-writer runs: `got_sections_ready_at_expected_path = f` on EVERY run; `resolved_links`
  is shaped `{"response":{"input_data":{"site_id":…,"sections":[…]}}}` — i.e. `select_sections` reads
  `resolved_links.sections_ready` but (a) there's no `sections_ready` there and (b) the envelope nests
  under `.response`; what's present is the resolver's INPUT echoed (`response.input_data.sections`), not
  its augmented output. → `select_sections` falls back to `input_data.section_plan.sections_ready` (the
  un-augmented plan with the schema `/contact.html`). So it's the FALLBACK branch, not a render override
  (render_component is exonerated).
- internal-link-resolver runs: all COMPLETED but `final_result` is **empty**; `responses_topic` is the
  parent's spawn topic and the parent DID receive a response — so respond-to-parent wiring is fine. The
  resolver just isn't surfacing its `resolve_internal_links` output.

TWO STACKED BUGS (both known families): (1) **resolver doesn't surface its action output** —
`final_result` empty ⇒ the internal-link-resolver's `complete_workflow` isn't emitting the
`{sections_ready, unresolved}` the action returns (Part-1 result-contract / `output_field` family); (2)
**`select_sections` reads the wrong path** — `resolved_links.sections_ready` vs the nested
`resolved_links.response.…` the call_agent envelope produces (workflow-variable/path sync). Both are
workflow-config fixes (no new Go, reuses the resolver which already computes correct links).

PIN BOTH:
```sql
-- A: the resolver's own workflow — why is final_result empty? does complete_workflow surface sections_ready?
SELECT jsonb_pretty(default_config #> '{workflow}') AS workflow
FROM agent_definitions WHERE type = 'internal-link-resolver';

-- B: full resolved_links envelope for one run — the exact path the agent's result lands at
SELECT jsonb_pretty(collected_data #> '{resolved_links}') AS resolved_links_full
FROM orchestration_states
WHERE owner_agent_type = 'page-content-writer' AND status = 'COMPLETED'
  AND collected_data #> '{resolved_links}' IS NOT NULL
ORDER BY created_at DESC LIMIT 1;
```
Fix: align the resolver's complete output to emit `sections_ready`, and point `select_sections`' first
path at the real location (per B) — then the hero CTA becomes the real hub (`/games/index.html`) the
resolver already computes. NEITHER Layer 1 nor Layer 2 (Part 4 clobber) touches this.

RESOLVER WORKFLOW (2026-06-23) — config looks CORRECT, so the empty `final_result` is a
result-extraction puzzle: `resolve_links` runs `resolve_internal_links` (output_field `link_resolution`),
then `complete` maps `config.output = {sections_ready: link_resolution.sections_ready, unresolved:
link_resolution.unresolved}`. That should yield `final_result = {sections_ready,…}`, yet it's empty on
every run. Break is between "action produced output" and "complete surfaced it". TWO candidates: (1)
**the resolver's chassis predates the Part-1 result-contract fix** — Part 1 is what taught
`complete_workflow` to honour the `output`/`result_mapping` mapping form (pre-fix: only plural
`output_fields`; everything else fell back to a dump/stub). If `internal-link-resolver`'s `image_tag`
wasn't bumped to the Part-1 chassis, `config.output` is ignored → empty `final_result` (the echoed
`input_data` envelope the parent saw fits the pre-fix fallback). (2) the action isn't populating
`link_resolution` (e.g. `input_data.sections` not resolving). DECISIVE QUERY:
```sql
SELECT orchestration_id, status,
       (collected_data #> '{link_resolution,sections_ready}' IS NOT NULL) AS action_produced_sections,
       jsonb_pretty(collected_data #> '{link_resolution}') AS link_resolution,
       jsonb_pretty(final_result)                          AS final_result
FROM orchestration_states
WHERE owner_agent_type = 'internal-link-resolver' AND status = 'COMPLETED'
ORDER BY created_at DESC LIMIT 1;

SELECT type, image_tag, updated_at
FROM agent_definitions WHERE type IN ('internal-link-resolver','page-content-writer');
```
If action_produced_sections=true but final_result empty → result-extraction layer → check image_tag vs
the Part-1 chassis (fix = redeploy this agent on it; reuse, not new code). If link_resolution empty → the
action isn't getting its sections. Bug 2 (select_sections reads `resolved_links.sections_ready` vs the
nested `resolved_links.response.…`) stands regardless — corrected envelope query: `#> '{resolved_links}'`
(array path; key is `resolved_links`, the output_field, not the action name).

ROOT CAUSE CONFIRMED (2026-06-23) — path mismatch; chassis hypothesis RETRACTED.
- Both `internal-link-resolver` and `page-content-writer` are on the SAME image `v1.0.1070` → NOT a
  Part-1-not-deployed/version issue. Retract the chassis hypothesis.
- Full envelope: `resolved_links = {response:{input_data, resolve_links, link_resolution}, response_status,…}`.
  The resolver's augmented sections ARE present — at `resolved_links.response.link_resolution.sections_ready`
  (and `.response.resolve_links.sections_ready`). But `select_sections` reads `resolved_links.sections_ready`
  (top level) → null → fallback to `input_data.section_plan.sections_ready` (un-augmented → schema phantom).
  So it's the FALLBACK branch via a PATH mismatch. The earlier empty `final_result` is explained: the reply
  to the parent is built from the resolver's collected_data (so `link_resolution` is present), not from
  `final_result` — data flows despite empty `final_result`. So "bug 1" is a cleanliness oddity, not data-loss.

FIX (workflow-only, DB, immediate, no redeploy) — repoint `select_sections`. **Variable-path change:**
`select_sections.config.fields.sections_ready[0]`: `resolved_links.sections_ready` →
`resolved_links.response.link_resolution.sections_ready` (fallback to `input_data.section_plan.sections_ready`
kept).
```sql
...
```
Takes effect on the next FULL build of a page (bare `page_rerender` won't re-run the resolver). For
gamesdesign `guide-economy-basics` (hero = generic `hero`, which the resolver matches), a rebuild then sets
`cta_url` to a real hub (`/games/index.html`) and the phantom clears.

TWO SEPARATE FOLLOW-UPS (not part of the path fix):
1. **Resolver function matching is narrow.** `resolve_internal_links`' `ctaFieldNames` matches only exact
   `"hero"`/`"call-to-action"`. Component variants with `source:"renderer"` CTA fields (`hero-about`,
   `platform-comparison`, `gauntlet-cta`, …) are skipped → their CTAs never resolve even after the path fix.
   Broader coverage gap across templates.
2. **Resolver returns its whole collected_data** (echoed input incl. every component's full HTML — huge
   reply) and `final_result` is empty. Make it return a lean `{sections_ready, unresolved}`; then
   `select_sections` simplifies to `resolved_links.response.sections_ready`. Needs the `complete_workflow`
   result-spec (`output` mapping vs `output_field`) behaviour confirmed first.

---

## P4.3 — apply the written fix: build, deploy, verify  `[ ]`

The fix is written: a patched `save_page_sections_action.go` carrying all three changes (Layer 2
carry-forward + Layer 1 interactivity guard + `source_item_id` stamp) — all in `save_page_sections`,
the only place that holds the tool's rendered markup. This is a Go change, so it needs the full
deploy cycle, plus one workflow-config change for the `source_item_id` stamp to populate.

**What the patch does**
- **Layer 2 (carry-forward), before the content-regression guard.** Selects existing deployed
  interactive sections (`rendered_html ILIKE '%<canvas%' OR '%game-container%' OR '%tool-page%'`).
  Per section: same slot present and still interactive → keep the rebuild's version; same slot but
  rebuilt as non-interactive (the pathfinding hero-as-text case) → keep the existing interactive
  markup in place; slot dropped → re-append the tool. Runs *before* Layer 1, so a successful
  carry-forward means Layer 1 never fires.
- **Layer 1 (interactivity guard), after the content-regression guard.** If the existing deployed
  page is interactive and the incoming set is not → return `interactivity regression blocked…`.
  Safety net for when Layer 2 can't preserve (e.g. the preload query fails): the existing good page
  stays and you get a loud error. Helper `sectionHTMLIsInteractive(html)` carries the shared signal.
- **`source_item_id`.** The history snapshot INSERT now stamps `source_item_id = $2` from a
  config-driven `work_item_id_field`; nil → SQL NULL, so no behaviour change until the workflow sets
  the field.

**Step 1 — workflow config (immediate, no redeploy).** `[ ]` For `source_item_id` to populate, add
`work_item_id_field` to the `save_sections` step on every agent that saves sections, pointing at the
`collected_data` path of the driving work-item id (the same value `plan_sections` already receives
via its `work_item_id` input — confirm the exact path from an orchestration `collected_data` dump):

```sql
SELECT snapshot_agent('page-build-handler', 'pre work_item_id_field for source_item_id stamping');
UPDATE agent_definitions
SET default_config = jsonb_set(
      default_config,
      '{workflow,steps,save_sections,config,work_item_id_field}',
      '"<collected_data.path.to.work_item_id>"'::jsonb)
WHERE type = 'page-build-handler';
-- repeat for any other agent with a save_sections step
```

**Step 2 — build and deploy the Go change.** `[ ]` `go build ./...` (the sandbox lacked a Go toolchain, so
build in-repo), then GitHub → Backblaze → new chassis image. Back up and bump `image_tag` on every
agent that runs `save_page_sections`:

```sql
SELECT snapshot_agent('page-build-handler', 'pre interactivity guard + carry-forward');
SELECT snapshot_agent('page-rerender',      'pre interactivity guard + carry-forward');
-- + any other builder whose workflow has a save_sections step
UPDATE agent_definitions SET image_tag = '<new tag>'
WHERE type IN ('page-build-handler','page-rerender');   -- add the others
```

Layer 1 fires only on pages that are *currently* interactive, so guides/index and other plain pages
are unaffected. Interaction with Part 2: a `page_rerender` that would produce non-interactive sections
for an interactive page now blocks loudly instead of clobbering; a NULL-`content_data` tool section
that escalates to `needs_page` now hits Layer 2 and is preserved (the escalation loop is broken).

**Step 3 — re-create the lost game and verify.** `[ ]` Re-run the tool-recreation for `game-pathfinding`
(and fix its `needs_page:game-pathfinding` mis-key per Part 3). Then confirm the page is interactive
again and that a subsequent rebuild leaves it that way:

```sql
-- pathfinding should be interactive again (expect has_interactive_markup = t)
SELECT p.name, p.url, p.build_status,
       bool_or(pc.rendered_html ILIKE '%<canvas%'
            OR pc.rendered_html ILIKE '%game-container%'
            OR pc.rendered_html ILIKE '%tool-page%') AS has_interactive_markup
FROM pages p JOIN page_components pc ON pc.page_id = p.id
WHERE p.id = '56af8679-1f7d-4da6-b148-f5727b16693d'
GROUP BY p.name, p.url, p.build_status;

-- attribution check: overwrites now carry source_item_id (no longer NULL) once Step 1 is set
SELECT source, source_item_id, created_at
FROM page_component_history
WHERE page_id = '56af8679-1f7d-4da6-b148-f5727b16693d'
ORDER BY created_at DESC LIMIT 5;
```

Regression check: drive a full rebuild of pathfinding, then re-run the first query — it should
*still* show `has_interactive_markup = t` (Layer 2 preserved the tool). If a rebuild produced
non-interactive content, the save should have been blocked — look for the guard/carry-forward log
lines in the handler/rerender pod:

```
kubectl -n ai-persona-system logs <page-build-handler or page-rerender pod> \
  | grep -E 'INTERACTIVITY REGRESSION BLOCKED|Layer 2'
```

**Rollback.** Step 1 (workflow-only): `revert_agent('page-build-handler')` (etc.). Step 2 (Go change):
revert `image_tag` to the prior chassis tag on the bumped agents — note the save path then returns to
silent overwrite, so re-protect promptly. The patched file is `save_page_sections_action.go` (this
session's output).

---

## Part 5 — `system-stats` section silently dropped (content ↔ template key mismatch)

Status: **cause confirmed 2026-06-24.** The live index deploys 4 of its 5 sections — `system-stats`
is in `page_components` (7,369 bytes) but renders with **zero** visible text, so the assembler's
visible-content filter (`rerender_single_page` → `getPageSections` → `sectionHasVisibleContent`, the
≤10-char rule) correctly drops it. Root cause: the component template
(`content_components` `fdd92ad4-521a-4602-89cf-7ee1a66c10f1`) binds placeholders by exact key —
`{{.eyebrow_label}}`, `{{.section_headline}}`, `{{.stat1_value}}`, `{{.stat1_label}}`, … — but the
stored `content_data` uses a different scheme — `eyebrow`, `heading`, `stat_1_number`, `stat_1_label`,
… — with **no shared keys**, so every field resolves empty. `component-creator` updated the component
at 15:06 (after the 14:26 build); it is used on **22** pages, all losing the band the same way. The
filter is right; the fix is to realign the two sides.

### P5.1 — trigger a full content rebuild (NOT a bare rerender)  `[ ]`
A `page_rerender` reuses the stored, mis-keyed `content_data`, so it won't help — you need
`page-build-handler` → `page-content-writer` to regenerate `content_data` against the current
component schemas. Re-open the work item that originally built the index (`0b8114c1`, ran 14:13) so
the dispatch loop re-runs it. The dispatchable status in this system is **`triaged`** (not `pending`) —
that is the value `build-dispatch-loop` claims:

```sql
UPDATE site_work_items
SET status = 'triaged', claimed_by = NULL, claimed_at = NULL,
    completed_at = NULL, attempt_count = 0, error = NULL
WHERE id = '0b8114c1-3caa-48da-8041-ad59c4cd89b8';
-- if it errors on a unique constraint, a rebuild is already queued (idx_swi_dedup over non-terminal rows)
```
(A full rebuild regenerates ALL of the index's sections' content, not only `system-stats`. Confirmed
2026-06-24: this re-open moved the item to `claimed` by `build-dispatch-loop` — the rebuild is running.)

### P5.2 — confirm it claimed and completed  `[ ]`
```sql
SELECT id, status, claimed_by, attempt_count||'/'||max_attempts AS attempts,
       completed_at, left(error,200) AS error
FROM site_work_items WHERE id = '0b8114c1-3caa-48da-8041-ad59c4cd89b8';
```

### P5.3 — decisive re-check: keys realigned + renders?  `[ ]`
```sql
SELECT slot_name,
       (content_data ? 'stat1_value')   AS has_template_key,   -- expect t after a good rebuild
       (content_data ? 'stat_1_number') AS has_old_key,         -- expect f
       length(regexp_replace(regexp_replace(regexp_replace(regexp_replace(regexp_replace(
         rendered_html,'<style[^>]*>.*?</style>','','gi'),
         '<script[^>]*>.*?</script>','','gi'),
         '<[^>]+>','','g'),'&[a-zA-Z#0-9]+;','','g'),'\s+','','g')) AS approx_visible_len,
       updated_at
FROM page_components
WHERE page_id = '6e988cc4-4898-4021-aa5e-2ab0271f9b75' AND slot_name = 'system-stats';
```
- `has_template_key=t`, `has_old_key=f`, `approx_visible_len>10` → writer now emits schema-matching
  keys; `system-stats` renders. Remedy the fleet: rebuild the other 21 pages using `fdd92ad4`, and make
  a component schema/template change trigger a rebuild of dependent pages.
- keys unchanged (`has_old_key` still `t`) → the writer is **not** emitting the component
  `input_schema` field names; structural fix = the content-generator ↔ component-schema binding (emit
  exactly the field names the template references).

### P5.4 — confirm the deployed page  `[x]`
Re-fetch the live `/index.html`. **Result (2026-06-24 18:37):** the page now has **4** sections — `hero`,
`tool-list`, `guide-list`, `game-list` — and **no** `data-component="system-stats"`.

### Result (2026-06-24 18:37) — the rebuild DROPPED the band, it did not re-key it  `[x]`
WI `0b8114c1` completed 18:37:45 (no error). The P5.3 re-check returned **0 rows** for
`slot_name='system-stats'` — there is no longer a system-stats row for the index; the live page confirms 4
sections. So the empty-band symptom is resolved by **removal**, with two consequences:
- the key-mismatch fix is **unverified** (the band wasn't planned, so the writer never had to emit the
  template's keys);
- the **durable** component bug persists — `content_components` `fdd92ad4` still mismatches, and ≤21 other
  pages using it still deploy the band empty.
Also: the 14:13 build planned 5 sections incl. system-stats; re-running the **same** work item at 18:37
planned 4 → `plan_sections` is non-deterministic, or its stored section list changed. The per-page rebuild
route has now demonstrated it can silently drop a section — so the durable fix should be at the **component**
level, not per-page.

### P5.5 — outcome (2026-06-24): concluded  `[x]`
The fleet + version checks resolved this without a component edit:
- **Only 5 live instances** use `fdd92ad4` (`usage_count=22` is a stale counter), and **none is gamesdesign** —
  three other-site `index` pages, a Kafka case-study page, and a `gripper-detail` page. All 5 carry the writer keys
  and render empty; all were re-rendered in one batch at `15:06:12.956` when the component changed.
- **No revert target.** `component_versions` holds exactly one version — `version_number 1`, the 15:06
  `component-creator:regen`, `change_source=manual` — i.e. the regeneration itself, a mixed snapshot, not a clean
  earlier definition. The live component is coherent (template-scheme, 22/22 synced).
- **Manual regen → likely the other chat.** Components appear to be hand-managed outside this chat, so editing the
  shared `fdd92ad4` would risk a tug-of-war and has no clean revert anyway.

**Decision:** gamesdesign's system-stats is **closed by removal** (band dropped at 18:36; site not among the 5). Leave
the shared component alone. The 5 empties and the **durable platform bug** — `component-creator` renaming a component's
fields and re-rendering dependents from un-migrated `content_data` (writer `stat_1_number` vs regenerated component
`stat1_value`) — are other sites' pages and a platform concern to **flag**, not fix from here. Pivot gamesdesign to its
open threads (Part 4 deploy, phantom-CTA `select_sections` fix, `rerender-pages` `page_id`).
...
| 4 — interactive page clobber | **Cause confirmed; fix WRITTEN** (both layers in `save_page_sections` + `source_item_id`) | Set `work_item_id_field`; `go build`+deploy+bump `image_tag`; re-create `game-pathfinding` | Part 4 / **P4.3** |
| 5 — `system-stats` dropped | **Concluded (closed-by-removal)** — gamesdesign index dropped the band; the 5 broken instances are all other sites; no revert target (1 manual `component-creator` version); shared component is co-managed | leave the shared component; **flag** the platform bug (regen empties dependents); gamesdesign needs nothing here | **Part 5 / P5.5** |
| (related) phantom CTA resolution | Surfaced by P4.2 — hero `cta_url`=`/contact.html` survives rebuilds | Confirm via `page_component_history`; read the `pages.<area>` field resolver | Part 4 / P4.2 result |

Suggested order: **finish P2.4–P2.7**; **apply the phantom-CTA `select_sections` path fix**
(workflow-only, then full-rebuild `guide-economy-basics` — see the phantom-CTA thread above);
**apply Part 3** as its own commit; and **deploy the Part 4 fix via P4.3** (set `work_item_id_field`,
`go build` + bump `image_tag`, then re-create the pathfinding game). The Part 4 code is written —
what remains is the build/deploy and the game re-creation.
