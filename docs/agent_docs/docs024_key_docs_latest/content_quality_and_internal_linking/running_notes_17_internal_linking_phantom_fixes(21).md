...
## Writer child output shape — no monolithic page_html; it emits per-section (2026-06-15)
- CONTENT REGRESSION BLOCKED is NOT in the collected logs (all agents) — rolled. So the log-line path to new_text_chars is closed; must measure from orchestration_states.
- 472eed7d top-level keys confirm it IS the writer child (select_sections, spawn_link_resolver, resolved_links, process_sections_loop, section_output_0..4, generated_content_0..4, sections_for_render, render_context, compile_page, link_context). 
- KEY FINDING: there is NO page_html / assembled-HTML key. The writer emits PER-SECTION outputs (section_output_0..4) + sections_for_render, NOT a single page_content.response.page_html. That's why every produced_html_len query returned null/empty — not wrong nesting, the field doesn't exist on this path. The regression guard sums s.HTML across the sections it's handed (the same per-section pieces).
- So new_text_chars must be measured by summing stripped text across section_output_0..4 (or sections_for_render / compile_page). Exact query is in the handoff. STILL the one decisive unmeasured number.
- ALSO note for the new chat: page-build-handler's save_sections reads sections_metadata_field = page_content.response.sections_metadata. If the writer child (472eed7d) has no page_content.response.*, the HANDLER's save may be getting its sections from compile_page / a different field — worth confirming the handler's actual input vs what the writer emits (possible metadata-path mismatch feeding the guard a wrong/empty section set → which would ALSO make new_text small). This is a NEW thread the handoff must carry.

## REGRESSION GUARD RULED OUT — writer produced 33k chars, not thin (2026-06-15)
- Measured new content: section_output_0..4 stripped lengths = 3032 + 7955 + 8101 + 7449 + 6798 = **33,335 chars**. Threshold was existing/4 = 5760. New (33k) >> 5760 (and > existing 23041). The content-regression guard CANNOT have fired — the new content is abundant.
- So the LEADING hypothesis (guard blocking thin content) is FALSIFIED. The writer worked fine; content is not thin.
- SURVIVING explanation: the METADATA-PATH MISMATCH (was the "new thread", now promoted to lead). save_page_sections reads sections from `page_content.response.sections_metadata` (primary) or HTML fallback. The writer emits content in `section_output_0..4` / `sections_for_render`, NOT in `page_content.response.sections_metadata`. If save's configured field is empty, it finds no sections via its path and writes nothing.
- BUT an honest CONTRADICTION to resolve first: save's "no sections found" early-return INCLUDES page_id, yet _diag1 showed EMPTY result_page_id. The empty page_id is what pointed at the guard's `return nil,err` — but the guard is now ruled out. So either (a) my mapping of which return yields empty page_id is wrong, or (b) the `result`/page_id column I queried isn't populated from the action's return the way I assumed (maybe `result` is set by the handler/dispatch, not the action's return map). RESOLVE THIS FIRST in the new chat — the two clues (empty page_id ⇒ guard; 33k content ⇒ not guard) currently conflict.
- DECISIVE NEXT READS (new chat):
  1. Inspect what save_page_sections actually RECEIVED on index's run: does collected_data have `page_content.response.sections_metadata`? Is it empty/missing? (The writer child 472eed7d top-keys had `page_content` but its `.response` had no keys — strongly suggests sections_metadata is NOT where save looks.)
  2. Find where the writer's per-section output is meant to bridge to save's expected `sections_metadata` — is there a compile_page / assembly step that should populate page_content.response.sections_metadata from section_output_*? Did it run? (472eed7d has a `compile_page` key — inspect it.)
  3. Resolve the page_id contradiction: read how site_work_items.result is populated for a page-build-handler run (action return vs handler-set) to know what an empty result_page_id actually implies.
- NET: not thin content, not the guard. The save is getting no sections via its configured metadata path while the writer DID produce 5 sections elsewhere → a wiring/path mismatch between writer output and save input is the leading cause. Plus a page_id-signature contradiction to resolve.
