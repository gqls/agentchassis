...
| **(new) index missing `system-stats`** | **ROOT CAUSE** — `content_data` keys (`eyebrow`/`heading`/`stat_1_number`…) share **zero** keys with the template (`eyebrow_label`/`section_headline`/`stat1_value`…), so every placeholder renders empty; component rewritten 15:06 today; `usage_count=22` | full content rebuild of index (NOT bare rerender), re-check keys: match → rebuild the 22 dependents + trigger-on-schema-change; still mismatched → bind the writer to the component `input_schema` |
...
| **(new) `rerender_single_page` invalid page_id** | `rerender-site` is the only caller mapping `current_page.page_id` (others use `page_record.id`=UUID); its run dump returned **0 rows — not decisive** (states likely pruned; morning errors ~3h old) | `GROUP BY owner_agent_type` to check it ran + retention; if pruned → read `get_pages_for_rerender` statically (what is each page's `page_id`?). Lower priority than `system-stats` |
