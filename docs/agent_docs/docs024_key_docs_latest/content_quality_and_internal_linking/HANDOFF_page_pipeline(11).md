...
| **Part 4** — interactive page rebuilt as plain content | **DONE** — patched `save_page_sections` (Layers 1+2) deployed on `v1.0.1077`; `game-pathfinding` A* tool re-created (interactive ~20KB, deployed 2026-06-26) and now protected from re-clobber | none — optional later: set `work_item_id_field` for history traceability; delete stale `z_context/check_phantom_internal_links.go` |
| **Phantom hero CTA** | **FIX APPLIED** 2026-06-26 — `select_sections.sections_ready` repointed to `resolved_links.response.link_resolution.sections_ready` (snapshot `5946a27b`, `revert_agent` available) | **only open step:** `guide-economy-basics` build (`e26cd02f`) re-opened — once it completes, confirm `has_phantom_cta=false` on the hero; if still true, widen resolver `ctaFieldNames` for hero variants |
| **`rerender_single_page` invalid page_id** | **RESOLVED — not a pipeline bug** — every bad-`page_id` `page_rerender` row is a *manual* debugging item omitting `page_id`; the automated `create_rerender_items` path produces clean UUIDs throughout (06-24 "31/18" culprits pruned — inferred manual) | optional: retire the 3 stale `failed` manual rows (`wont_fix`) |
...
**To close the current work — one required step:**
1. **Verify phantom-CTA.** `guide-economy-basics` build `e26cd02f` is re-opened; once it completes, run the
   `has_phantom_cta` check on `page_id=8ed97fd2-…` — `false` on the hero closes it. If still `true` after a clean
   build, widen resolver `ctaFieldNames` (hero variants).

**Optional cosmetic now:**
- Retire the 3 stale `failed` manual `page_rerender` rows (`3e4fcdfb` / `f75380d7` / `26deb09c` → `wont_fix`).

**Already closed this session (no action):** Part 4 (deployed; A* game restored + protected); phantom-CTA fix
applied; `rerender_single_page` invalid page_id (not a pipeline bug); `system-stats` (concluded — closed-by-removal
for gamesdesign; shared component out of scope).

**Tracked for a future session (not part of this thread):**
- **Part 2:** P2.4–P2.7 (real image-landed flow, `section_data_resolved`, NULL `content_data` escalation, plain
  `page_rerender` backward-compat).
- **Part 3:** apply the `workItemKey` builder + adoption diff, build, bump image, run P3.4 — *after* Part 2 verifies.
- **Cross-site platform bug:** `component-creator` regen renames a component's fields and re-renders dependents from
  un-migrated `content_data` (empties them); affects other sites' `fdd92ad4` pages — flag to the platform / other chat.
- **Minor:** widen resolver `ctaFieldNames`; make the resolver return a lean result; set Part 4 `work_item_id_field`;
  delete stale `z_context/check_phantom_internal_links.go`.

Part 2 and Part 3 are the only substantive open pipeline work; Part 3 gates on Part 2.
