...
**Separate finding to confirm — the phantom may survive a FULL rebuild too.** `guide-economy-basics`
had a §5 `link_resolution_rebuild` full rebuild (`508c0ccb`, 06-14 19:16), yet its hero `cta_url`
is still `/contact.html` (real contact is `/contact/index.html`). If the full rebuild also fails
to fix it, NEITHER path fixes phantom CTAs and the §5 batch's goal is unmet — a distinct
**phantom-CTA-resolution** bug (the hero `cta_url` source `pages.contact` appears to produce
`/contact.html`, not the contact page's real URL). Separate from the clobber, and possibly more
important for the site-build goal. Confirm before concluding:
```sql
-- what cta_url did each historical write to this hero store? did the 06-14 full rebuild store the phantom?
SELECT created_at, source, source_item_id,
       content_data->>'slot_name' AS slot,
       content_data->>'cta_url'   AS cta_url
FROM page_component_history
WHERE page_id='8ed97fd2-2e33-4bdb-a24e-0f3badaca382'
ORDER BY created_at;
```
Then read the field resolver for `source: pages.<area>` (does it read the contact page's
`pages.url`, or construct `/{area}.html`?) and whether `resolve_internal_links` runs on these
heroes (it should set a hub, not contact). NEITHER Layer 1 nor Layer 2 fixes this CTA bug — they
make the rebuild safe; the phantom CTA needs its own fix.
