...
NEXT DIAGNOSTICS (read-only first):
```sql
-- did resolve_internal_links run but find no eligible hubs? (it emits unresolved_cta when it can't)
SELECT page_name, section_name, spec->>'missing' AS missing, created_at
FROM site_work_items
WHERE item_type='unresolved_cta' AND site_id='e33263f4-74f8-494f-b191-546845dbbddf'
ORDER BY created_at DESC LIMIT 20;

-- are there section-index hubs for the resolver to choose? (it needs page_type='section-index')
SELECT name, url, page_type, nav_order, status
FROM pages
WHERE site_id='e33263f4-74f8-494f-b191-546845dbbddf' AND page_type='section-index'
ORDER BY nav_order;
```
Then read the `source: pages.<area>` field resolver (constructs `/{area}.html` vs reads `pages.url`?)
and page-content-writer's wiring of `resolve_internal_links` (does it run; does schema field
resolution override its output?). NEITHER Layer 1 nor Layer 2 fixes this — the phantom CTA needs
its own fix.
