...
## Part 14 — doc triage (old design docs) + DOCS_FILES finalized; hero work is NOT greenfield

User supplied the repo doc listing + a batch of April design/adoption docs ("relevant but may be stale"). Triage:

**Reframing finding (most important):** the directory listing shows `HANDOFF_2026-06-02_hero_resolver_and_section_data_reconciler.md` — a **June** handoff. So the lead item (hero CTAs) is **NOT greenfield**: there's an existing hero resolver + section-data reconciler (`reconcile_section_data_action.go`). The next task must START by reading that handoff and the resolver code, then find why it emits wrong CTAs — not build resolution from scratch. Also `PLAN_design-note-recommendation-specialists.md` notes `component-template-fixer` "already handles CTA fixes"; a phantom/wrong-destination link is a *bug* finding. Don't have the June-02 handoff content yet — **need it uploaded** (it's the authority for the lead item).

**Grep result:** none of the April design docs mention `cta_url`/`index_url`/`Browse All`/a hero resolver — they're about design *fidelity* (palette/fonts/layout/header/footer reproduction), not the CTA-URL bug. So:
- **Directly relevant (fidelity background, verify current):** `FOCUS_design_and_styling_adoption_problems.md` (canonical adoption design-defect list → maps to CATALOGUE header/footer/layout items), `FOCUS_design_and_styling_adoption_WORK_PLAN_v2.md` (design_reference vs design_intent, webdesign three-way, fingerprint, site-design-planner), `PLAN_design-note-recommendation-specialists.md` (bug/recommendation/gap routing + component-template-fixer/identity-advisor reuse).
- **Background (CSS theme system):** `PHASE_4_4_cleanup_summary.md` (palettes/layouts/typography_sets composition; fork_theme_from_site Phase 5 adoption-fork pending), the fingerprint-build handoffs (04-12/04-14), CSS-var extraction (likely done).
- **Component path:** `026_component_regeneration_flow(1).md`, `FOCUS_llm_reliability_for_component_generation.md`, April component-linking handoffs (likely superseded by June hero resolver).
- All April → high stale-risk; the fingerprint/design_intent pipeline + composition refactor were built since.

**DOCS_FILES finalized** in `package_content_quality_debug.sh` with confirmed names: `001_development_guide(3)`, `002`, `003`, **`016_debugging_guide_v2.md`** (no `_NN` suffix — corrected from the v2_35 guess), `CATALOGUE_…(9)` under `adoption/`, the June-02 hero-resolver handoff, this session's docs, the fidelity-background set, `019/020`, `026_component_regeneration`. Added a hero-CTA-resolution code group to MODULE_FILES (`reconcile_section_data_action.go` + a TODO for the hero-resolver action name, to confirm from the June-02 handoff). `bash -n` clean.

## Part 15 — internal-linking subsystem mapped; two FOCUS docs produced; notes continue in 16

Read the June-02 hero/section-data handoff + `multipage_actions.go`/`site_db_actions.go`/`queryresolve.go`. Key corrections/findings: the June-02 "hero resolver" is hero IMAGE resolution (not CTA URLs); the phantom `/services.html` traces to a hardcoded fallback nav in `multipage_actions.go` (~310-318) injected when nav resolution is empty; existing linking machinery to reuse = `link_registry` (`ExtractAndSyncLinksAction`), `buildNavigationFromPages`/`GetNavigationStructure`, `fixAnchorLinks`, `queryresolve` (`pages_where_type`/`pages_under_section`), `upsertPage` (slug/url/nav_label/nav_order), `*_index_url` specs (unpopulated → empty Browse-All hrefs); `med_url_discovery_action.go` is a business_intel pet-med price scraper, NOT relevant.

Produced `FOCUS_internal_linking.md` and `FOCUS_content_quality.md` (grounded in the above). Updated the collector → `package_content_and_linking_debug` scope: added `multipage_actions.go`/`flag_page_image_rebuild_action.go`/`reconcile_section_data_action.go` to MODULE_FILES, added `link_registry`/`site_areas` schema + a phantom-link query (link_registry vs pages) + the hero field-vs-template query + nav-of-record query to live capture, and bundled both FOCUS docs + `running_notes_15(9)` + `running_notes_16` + the June-02 handoff into DOCS_FILES so the next chat carries full context. `bash -n` clean.

**Live running notes continue in `running_notes_16_content_quality_and_internal_linking.md`.** This file (15) is closed.
