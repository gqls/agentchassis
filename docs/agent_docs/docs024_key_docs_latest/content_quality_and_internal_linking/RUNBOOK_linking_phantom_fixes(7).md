...
PIPELINE — correction to an earlier note: the check emitting `pipeline="content"` for page_component findings is CORRECT, not a bug. Earlier "fix content→build" was WRONG — do not make that change. (The §5 manual inserts used `"build"` because they target `page-build-handler` directly; discovery routing is per-surface and survives — see below.)

PREREQUISITE CODE-READ — RESOLVED (2026-06-14, `run_discovery_checks_action.go`): per-finding routing SURVIVES. `insertWorkItem` maps `pipeline: wi.Pipeline` and `handlerAgent: wi.HandlerAgent` directly from each returned `WorkItemSpec`; the config `check_pipeline` only sets `dctx.Pipeline` (a shared-context default the check never consults). So `routeBySurface`'s per-surface values are written verbatim: site_component→`nav-link-fixer`/`build`, page_component→`page-build-handler`/`content`. **The check is correct as written — enable with NO code change.** (Checks return `WorkItemSpec`s; the action inserts them inside its own tx — my check reads via `dctx.DB`, which is the right pattern for an audit of the live rows.)

Steps (gate cleared, ready when you choose):
1. `snapshot_agent('completeness-discovery-agent', 'enable phantom_internal_links observe-only')`.
2. `jsonb_set`-append `"phantom_internal_links"` to `default_config.workflow.steps.run_checks.config.checks`.
3. Run once via kcat (§3 orchestrate pattern, `agent_type='completeness-discovery-agent'`) against gamesdesign.
4. Inspect: `SELECT item_type, surface, count(*) FROM site_work_items WHERE created_by='completeness-discovery-agent' AND status='detected' GROUP BY 1,2;` — ~empty on the post-§5 site = end-to-end proof through the real check. Findings stay `detected` (unclaimable) until you triage; sweep stays off.
