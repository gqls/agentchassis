...
| **(new) index missing `system-stats`** | **CONFIRMED render gap** — `content_data` is full (real stats) but the component template emits 0 text; assembler correctly drops the text-empty shell | inspect the `system-stats` component template (`content_components` `fdd92ad4`) vs the flat `stat_N_*` `content_data` keys; fix the template to interpolate them |
...
| **(new) `rerender_single_page` invalid page_id** | culprit = **`rerender-site`** — its `rerender_loop` maps `page_id: current_page.page_id` over `get_pages_for_rerender.pages`; the other two callers use `page_record.id` (UUID, fine) | dump a recent `rerender-site` run's `pages_for_rerender.pages` `page_id`s — names/urls → `get_pages_for_rerender` mis-keys; clean UUIDs → loop `current_page` resolution |
