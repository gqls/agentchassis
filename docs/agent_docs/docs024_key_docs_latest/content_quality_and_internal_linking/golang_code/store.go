...
// store.go — persistence for the traffic-probe. Forked from idea.uk's store.go:
...
// AddVisit counts one page load for a host (denominator for intent-per-1k-visits).
...
// AddEvent records one intent event under its host.
