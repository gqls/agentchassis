...
### P5.5 — revised plan  `[ ]`
1. **Confirm structure** — `page_components` columns + current index rows (slot_name, component_id,
   updated_at, visible_len). Guards the 0-rows (renamed slot?) and shows whether the other 4 rewrote at 18:37.
2. **Fleet check (the real bug)** — across pages still using `fdd92ad4`, do any carry the template's keys
   (`stat1_value`) or do all carry the writer's keys (`stat_1_number`)? All writer-keys → align the component
   template ↔ writer output at the component level (repairs every page at once).
3. **Decide on the index** — pin system-stats back (find the `plan_sections` section-list source) or accept
   the 4-section page.
...
| 5 — `system-stats` dropped | **Cause confirmed** — content_data keys ≠ template keys → empty render → dropped (22 pages) | full content rebuild of index (re-open WI `0b8114c1`), re-check keys; then writer-binding fix or fleet rebuild | **Part 5 / P5.1–P5.4** |
