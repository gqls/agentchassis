...
**Next diagnostic (to design the fix):** dump the routing-relevant step configs so we can see
whether there's a links-only / preserve-existing branch that's bypassed, or the writer always
runs:
```sql
...
**Fix direction (reuses Part 2):** route link maintenance through a re-render-class path
that re-resolves link fields and *preserves* sections, instead of the full writer. Open
question that the config above informs: are internal links resolved fields (then repointing
works directly, like Part 2) or baked into LLM prose (then a dedicated links-only rewrite is
needed that still preserves interactive sections)?
