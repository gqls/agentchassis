...
# Part 4 — interactive page rebuilt as content (game/tool dropped): investigation

Status: **investigation, not a fix** (surfaced 2026-06-22). Different failure from a
silent no-op: the page rebuilt successfully but as the *wrong kind* of page — the
interactive component was lost. A `page_rerender` cannot fix it (it re-renders the
sections that exist; the game isn't among them), so this needs a build once we know what
dropped it. Do not queue a `page_rerender` for these pages expecting a game to appear.

Established (for gamesdesign `game-pathfinding`, page 56af8679-…,
`/games/pathfinding/index.html`):
- The page has 2 sections — `hero` + `generic-text-block` (the *fallback* component for
  unmatched sections). No interactive component. Sections last written 2026-06-14 20:07:52.
- The interactive build (`needs_tool_recreation` → tool-recreation-handler) completed
  2026-06-05, before that rewrite — and was mis-keyed `needs_page:game-pathfinding` (a
  live instance of the Part 3 / Bug 2 mis-key).
- A `link_resolution_rebuild` for the page, **handled by `page-build-handler`**, completed
  2026-06-14 20:08:37 — coincident with the rewrite. (Carries error "Claim timed out —
  handler pod likely died" yet status `complete`, which is itself worth a look.)

Hypothesis (confirm, don't assume): a links-only maintenance rebuild routed through the
full content builder (`page-build-handler`) regenerated the page as a standard content
layout and dropped the interactive component. The fallback text block is the tell that
the rebuild couldn't match the game section.

Diagnostics, in order:
1. **History** — did a game section exist before 06-14 and get replaced?
   ```sql
   SELECT position, slot_name, component_id, build_status,
          length(rendered_html) AS html_len, created_at, updated_at
...
2. **Orphan check** — does a pathfinding game component still exist in `content_components`?
   ```sql
   SELECT id, name, function, section_type, render_mode,
          length(js_content) AS js_len, component_level, is_active, updated_at
   FROM content_components
   WHERE name ILIKE '%pathfind%' OR function ILIKE '%pathfind%'
      OR name ILIKE '%astar%' OR name ILIKE '%a-star%'
      OR description ILIKE '%pathfind%'
   ORDER BY updated_at DESC;
   ```
3. **Handler behaviour** — what `page-build-handler` does on `item_type =
   link_resolution_rebuild`: full page rebuild, or links-only? (its `default_config` +
   the `46c2da91` orchestration).

Remediation (TBD, after 1–3): re-running the tool-recreation regenerates and reattaches
the game, but if a content/link rebuild dropped it, it will be dropped again next time one
fires. The structural fix is to stop links-only (and content) rebuilds from destroying
interactive sections — either `page-build-handler` preserves existing interactive
components on rebuild, or `link_resolution_rebuild` doesn't route through the full content
builder. Interacts with Part 3 (the same page carries the `needs_page:` mis-key on its
