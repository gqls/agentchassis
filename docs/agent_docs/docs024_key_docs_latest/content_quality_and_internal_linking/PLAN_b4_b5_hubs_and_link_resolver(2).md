...
> **Status (2026-06-10).** B4/B5 is written and ready for the batch deploy — `section_index_for.go` (verb), `b4_b5_hub_links_schema.sql`, `b4_b5_hub_links_template_gate.sql`. The chosen option below is (c). Step 3: architecture confirmed (build-time resolution via `internal-link-resolver` spawned by `page-content-writer`); `resolve_internal_links_action.go` is written (section-augmenting) and corrected against 001/003 (see `running_notes_17` + `016_debugging_guide_v2_45`). Remaining: the agent_definitions row, the `page-content-writer` wiring, `routeBySurface` → `needs_rebuild`, and `unresolved_cta` emission. Agent-modeling facts now known: workflow in `default_config.workflow`, `agent_category=specialist`, contracts + image fields required; rebuild via `build_status='needs_rebuild'` → `page-rebuild`.
