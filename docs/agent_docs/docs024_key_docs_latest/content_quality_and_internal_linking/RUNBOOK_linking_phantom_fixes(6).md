...
HOME AGENT (confirmed 2026-06-14): add it to **`completeness-discovery-agent`** (`check_domain: "content"`), NOT quality/design. It's a content-integrity check, same family as that agent's `empty_sections` / `unlinked_page_components` / `unresolved_sections`. (The discovery agents are domain-scoped: `quality-discovery-agent`=build, `design-discovery-agent`=design, `completeness-discovery-agent`=content.)

PIPELINE — correction to an earlier note: the check emitting `pipeline="content"` for page_component findings is CORRECT, not a bug. `completeness-discovery-agent` runs with `check_domain: "content"`; content-domain findings route through the content path. (The §5 manual inserts used `"build"` because they target `page-build-handler` directly; discovery-routed content findings are `"content"` by the agent's convention. Earlier "fix content→build" was WRONG — do not make that change.)

PREREQUISITE CODE-READ before enabling (the one real open question): `run_discovery_checks` passes `check_domain` to every check, but `check_phantom_internal_links` sets its OWN per-surface `WorkItemSpec.Pipeline` (`build` for site_component, `content` for page_component) in `routeBySurface`. Confirm whether `run_discovery_checks` honours a check's returned `Pipeline`/handler or STAMPS `check_domain` over it. If it stamps, the site_component→`nav-link-fixer`/`build` route is lost (everything becomes `content`) and the per-surface routing must move out of the check (e.g. split into two checks, or route by surface downstream). If per-finding override survives, enable as-is. Read the `run_discovery_checks` action + one existing check's `WorkItemSpec` construction (e.g. `check_empty_sections`) to settle it.

Then: snapshot `completeness-discovery-agent` (`snapshot_agent('completeness-discovery-agent', ...)`), `jsonb_set`-append `"phantom_internal_links"` to `default_config.workflow.steps.run_checks.config.checks`, and run it once via kcat (the §3 orchestrate pattern, `agent_type='completeness-discovery-agent'`) against gamesdesign. Inspect: `SELECT item_type, surface, count(*) FROM site_work_items WHERE created_by='completeness-discovery-agent' AND status='detected' GROUP BY 1,2;` — on the post-§5 site this should be ~empty (the proof the batch worked end-to-end through the real check, not just the manual audit query). Findings stay `detected` until you choose to triage; sweep stays off.
