...
the dispatch loop re-runs it (set `status` to whatever dispatchable/unclaimed items use — commonly
`pending`):

```sql
-- what status do dispatchable (unclaimed) items use?
SELECT status, count(*) FROM site_work_items WHERE claimed_by IS NULL GROUP BY status;
...
```
(A full rebuild regenerates ALL of the index's sections' content, not only `system-stats`.)
