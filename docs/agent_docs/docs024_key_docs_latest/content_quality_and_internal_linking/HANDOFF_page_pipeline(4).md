...
| **(new) index missing `system-stats`** | **CONFIRMED** — section in `page_components` but `visible_len=0`; assembler correctly drops a text-empty shell | upstream content gap: inspect `system-stats` `content_data` (empty → never generated; populated → render/template gap), then populate it |
...
| **(new) `rerender_single_page` invalid page_id** | narrowed — `page-rerender`'s `render_page` calls it correctly; the bad `page_id` (len 31/18) is set by its **parent loop** (spawns `page-rerender` per page) | find the parent (`...LIKE '%page-rerender%' AND type<>'page-rerender'`), inspect its loop `items_field` + `page_id` mapping (likely iterating names/urls, not `pages.id`) |
