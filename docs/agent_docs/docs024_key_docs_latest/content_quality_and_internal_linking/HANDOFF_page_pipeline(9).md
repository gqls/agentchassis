...
| **index `system-stats` (rebuilt 18:37)** | rebuild **dropped** the band (0 rows; deployed index = 4 sections, no stats) — symptom gone by **removal**, not repair; key-mismatch **unverified**; durable component bug persists on ≤21 other `fdd92ad4` pages; `plan_sections` non-deterministic (5→4 sections from the same WI) | confirm structure; **fleet-check** `fdd92ad4` pages' `content_data` keys → align component template ↔ writer keys at the **component** level (not per-page rebuild); decide pin-vs-accept on the index |
