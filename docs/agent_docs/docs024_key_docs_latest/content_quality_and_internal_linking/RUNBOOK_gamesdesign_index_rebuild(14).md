...
- A bare `page_rerender` would NOT fix these links: the build-resolved `cta_url` is persisted
  in `content_data`, and the re-render re-resolves only query/hero-image fields (via
  `planSection`), not CTAs — so it would reproduce the phantom. (Confirm: `SELECT
  content_data->>'cta_url' FROM page_components WHERE page_id='8ed97fd2-…' AND
  slot_name='hero';` → expect `/contact.html`.)
...
`link_resolution_rebuild` → `page_rerender`" plan is dropped** — it neither re-resolves CTAs
nor addresses non-link rebuild triggers.
...
short: links are build-resolved CTA fields (a bare `page_rerender` won't fix them), and the
clobber affects *any* full rebuild — so the fix is the interactivity guard (Layer 1) +
preserving interactive sections through the rebuild (Layer 2), not a routing repoint. The two
guards (`source_item_id`; interactivity-aware save) and re-creating `game-pathfinding` follow
regardless.
