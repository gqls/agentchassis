...
## June-02 actions — registry DONE, reconciler still needs a host
Both June-02 local actions are now registered in `registry.go`: `flag_page_image_rebuild` (was already present, lines ~914–919) and `reconcile_section_data` (entry added — `Handler: ReconcileSectionDataAction, Category: "site", IsLocal: true`). So both are callable.
- **Open item:** `reconcile_section_data` **still has no host — registering it makes it callable, but nothing calls it yet.** Per `HANDOFF_2026-06-02_hero_resolver_and_section_data_reconciler.md`, pick one host and wire the step: a periodic loop discovery check, or a post-build finalize step in the rerender path (plan-time is too early — the listed pages don't exist yet). The action is invocation-agnostic: given `site_id` it scans open `needs_section_data`, re-runs `query.*` sources, and emits `needs_page` for now-resolvable pages; `plan_sections` closes the item on re-render via `closeResolvedDataRequest`. Until hosted, query-resolvable `needs_section_data` items still sit at `needs_human_review`.
...
1. **Hero CTAs wrong site-wide (highest value — every page).** Every hero links both buttons to `/contact.html` and `/services.html`; CTA text↔destination mismatch (e.g. "Browse Tools" → `/contact.html`); **`/services.html` is a phantom page** (no such page). **NOT greenfield** — there is an existing hero resolver: read `HANDOFF_2026-06-02_hero_resolver_and_section_data_reconciler.md` FIRST, then find the hero-resolver action + `reconcile_section_data_action.go` and determine why the resolver emits wrong CTAs (stale `*_index_url` specs? fallback to `/contact.html`/`/services.html` defaults? text/url decoupled?). Also note `component-template-fixer` (existing agent) "already handles CTA fixes" per `PLAN_design-note-recommendation-specialists.md` — a phantom/wrong-destination link is a *bug* finding (→ `content_rewrite`/component-template-fixer), not a recommendation. STEP ZERO across hero resolver + component-template-fixer before writing.
...
## Design-fidelity background (older docs — relevant but verify; pipeline evolved since April)
The adoption design subsystem (why an adopted site should reproduce the original's look) is documented in older docs that still frame the *design-fidelity* defects behind the CATALOGUE polish items (header genericised, footer genericised, layout flattened, palette/fonts):
- `FOCUS_design_and_styling_adoption_problems.md` — canonical list of adoption design-fidelity defects (maps to several CATALOGUE items; April — many may be resolved by the fingerprint/design_intent pipeline + the palettes/layouts/typography_sets composition refactor; cross-check current state).
- `FOCUS_design_and_styling_adoption_WORK_PLAN_v2.md` — the design subsystem map: `design_reference` (extracted values) vs `design_intent` (semantic), webdesign-agent three-way priority, fingerprint, site-design-planner (Phase 3, status unknown), requirement-driven components (Phase 4).
- `PHASE_4_4_cleanup_summary.md` — CSS theme composition (`palettes`/`layouts`/`typography_sets` via FKs); `fork_theme_from_site` Phase 5 (adoption fork) was broken/pending — verify.
- Component path: `026_component_regeneration_flow(1).md`, `FOCUS_llm_reliability_for_component_generation.md`, and the April component-linking handoffs (likely superseded by the June hero resolver).
These are NOT about the hero-CTA-URL bug specifically (none mention `cta_url`/`index_url`) — that's the June-02 hero resolver. Treat the April docs as fidelity background, stale-risk high.
