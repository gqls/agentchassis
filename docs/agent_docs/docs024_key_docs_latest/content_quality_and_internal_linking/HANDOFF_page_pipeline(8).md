...
| **(new) index missing `system-stats`** | **ROOT CAUSE** — `content_data` keys (`eyebrow`/`heading`/`stat_1_number`…) share **zero** keys with the template (`eyebrow_label`/`section_headline`/`stat1_value`…), so every placeholder renders empty; component rewritten 15:06 today; `usage_count=22` | re-open WI `0b8114c1` for a full content rebuild (Runbook **P5**), re-check keys: match → rebuild the 22 dependents + trigger-on-schema-change; still mismatched → bind the writer to the component `input_schema` |
