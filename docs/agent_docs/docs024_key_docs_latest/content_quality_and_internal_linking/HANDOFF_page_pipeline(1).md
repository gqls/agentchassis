...
| **Part 2** — no-LLM re-render path | **DEPLOYED** 2026-06-21; verified for `image_landed` only | run P2.4–P2.7; confirm live index shows 5 sections |
| **Part 3** — `item_key` canonicalization | **CODE PREPARED**, not applied | apply *after* Part 2 verifies |
| **Part 4** — interactive page rebuilt as plain content | **CAUSE CONFIRMED**; fix spec pinned; gated on source | run clobber bundle (§11), then write Layer 1 guard + Layer 2 preserve-sections |
...
**Designed fix (spec PINNED 2026-06-24; not yet implemented — needs source, see §Bundles).**
Interactivity is detected from the `page_components.rendered_html` column via
`ILIKE '%<canvas%' OR '%game-container%' OR '%tool-page%'` (there is **no** stored
`has_interactive_markup` column — it was computed in the blast-radius sweep).
- **Layer 1 — extend the existing content-regression guard** in `save_page_sections` (do NOT add a new
  guard). That guard already holds old+new `rendered_html` and computes a prose-text regression (the
  "content regression blocked: N chars vs M" errors); it waved the game through because 18KB game → 2KB
  text *grew* the prose while losing the markup/JS. Add a parallel check on the same `rendered_html`
  reusing the sweep signal (`<canvas` / `game-container` / `tool-page`, page-level `bool_or`): if the page
  is currently interactive and the incoming set is not → block + escalate (`mark_needs_review`), same as
  the text guard. Note: `data-component` alone is NOT a signal (every section carries it).
- **Layer 2 — preserve non-spec interactive sections through rebuild.** The clobber happens because the
  rebuild plans from the spec (`load_spec_sections` = `load_page_sections_from_spec` → `plan_sections`) and
  the tool is a `page_component` not in the spec. Merge existing interactive `page_components` not
  represented in the spec into the planned set (in `load_page_sections_from_spec` or `plan_sections`) so
  the DELETE+INSERT keeps them. Keeps CTA re-resolution via the existing writer; no routing change.
- **Layer 3 (re-route through `page_rerender`) is RULED OUT** — P4.2 showed it preserves sections but does
  NOT re-resolve CTAs.
- **Source needed:** `save_page_sections_action.go` (L1), `plan_sections_action.go` +
  `load_page_sections_from_spec` (L2), `registry.go` — gather via the clobber bundle (§Bundles).
- Plus: stamp `source_item_id` into `page_component_history` on the save path; re-create the
  `game-pathfinding` A* tool after the fix (and fix its `needs_page:game-pathfinding` mis-key per Part 3);
  delete the stale duplicate `z_context/check_phantom_internal_links.go` (the live `platform/...` copy is
  correct).
