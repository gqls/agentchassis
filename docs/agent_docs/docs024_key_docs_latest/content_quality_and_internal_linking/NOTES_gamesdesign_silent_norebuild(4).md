...
long-standing config/coordinator mismatch. No fix applied yet; the exact returned
shape the fix must produce is pending the open 06-06 anomaly (below). Supersedes
`PREAMBLE_gamesdesign_diagnosis_handoff.md` (its "generation shortfall" framing
is falsified). A generalised entry for this failure mode is in
...
**Step 0 — map live vs legacy wiring (GATES everything).** Inconsistent consumers
exist: `page-build-handler` reads the writer's response FLAT
(`page_content.response.sections_metadata`/`.page_html`/`.skipped`);
`content-reviewer` reads it as the DUMP (`page_content.processed_sections.results`,
`page_content.compile_page.page_html`) — fields only present in the fallback dump.
So ANY fix that bounds the writer's result risks breaking content-reviewer unless
it's off the live path. Run the agent_type-reference query (who calls
page-content-writer / content-reviewer / site-planner / pageflow-builder) to learn
which build path is current (dispatch → page-build-handler) vs legacy
(pageflow-builder → site-planner → content-reviewer). If pageflow-builder is dead,
site-planner and content-reviewer are off-path and the writer's only live consumer
is page-build-handler — which collapses the risk.

**Step 1 — choose the singular handling. Options (verify, don't assume):**

- *A-flatten* — coordinator returns the single `output_field`'s contents as the
  body (flat). Matches existing flat consumers (page-build-handler) and the
  documented contract (dev-guide 1081) → no consumer/save edits. But keeps a
  shape difference between singular and plural → re-introduces the naming
  fragility unless the trigger is renamed (see result_from).
- *A-nest* — treat `output_field: "x"` as one-element `output_fields: ["x"]`
  (nest under the name). UNIFIES singular = plural semantics → removes the
  shape-fork fragility (the user's preferred direction). COST: produces the
  NESTED shape, so the writer's consumers must move to nested paths (the four
  page-build-handler paths) AND `model-trainer`'s flat reads of
  `preparation_result.dataset_uri`/`.training_run_id` BREAK (it would need
  `preparation_result.preparation_result.…` or training-data-preparer reverted).
  So A-nest = clean semantics + B-level consumer churn + a model-trainer fix.
- *result_from* — explicit new key `result_from: "x"` → body = contents of x
  (flatten), distinct non-confusable name. Gets A-flatten's no-churn benefit AND
  removes fragility (trigger is an explicit key, not "singular present"). Migrate
  the 4 singular agents `output_field` → `result_from`. Consumers unchanged.
- *B (config)* — writer `output_fields: ["page_content"]` (nested) + repoint
  page-build-handler's four paths. Per-agent; repeats for each singular agent.

**Claims to verify before choosing (do not take on trust):** the "A-flatten
breaks none of the four, fixes two" net. Per agent: writer (flat consumers →
fixed); training-data-preparer/model-trainer (model-trainer reads flat → flatten
corrective, nest breaks); site-planner (no consumer of `validated_plan` — confirm
via wiring query, likely legacy); thunder-reaper (`reaper_summary` never set →
flatten yields metadata-only; confirm no awaiting consumer reads its response).

**Step 2 — harden the size path (code, separate, affects all agents).**
...
`status:"completed"` when output was dropped — surface a failure. This also fixes
Fault 2's silent-success half.
