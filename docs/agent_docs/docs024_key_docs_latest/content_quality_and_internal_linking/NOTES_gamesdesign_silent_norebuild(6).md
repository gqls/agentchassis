...
long-standing config/coordinator mismatch. FIX IMPLEMENTED (pending deploy +
verify): `extractWorkflowResult` rewritten to use a centralised `resolveResultSpec`
...
**Step 1 — SHIPPED (pending deploy).** `extractWorkflowResult` body replaced to
...
the one function (verified). Pre-merge: grep the full orchestration package for
existing `setIfAbsent`/`toStringSlice`/`toStringMap`/`getMapKeys` to avoid
redeclaration (none in coordinator.go/processor.go).
...
**Deploy + verify.** Rebuild the chassis image, roll the agents to the new tag
(image_tag is currently `v1.0.1063` across these agents). Then rebuild
gamesdesign index and check: `page_content.response.sections_metadata` arrives
populated; `page_components` rendered_html timestamps advance past 2026-06-06;
the content-regression guard passes on real content; the `page_rerender` work
item only completes on a real save. Watch logs for `resolveResultSpec: resolved
result contract mode=flatten matched_key=output_field` on the writer, and the
deprecation Warns (a census of which agents still use old key names → drives the
rename migration).

**Follow-ups (separate commits, not shipped here):**
  1. Size-path hardening: `extractWorkflowResultWithSizeLimit` must never return
     `status:"completed"` when output was dropped — surface a failure; recurse the
     trimmer or persist+reference. Fixes Fault 2's silent-success half. Flatten
     bounds the writer (~81k) so the stub no longer fires for it, but the fallback
     and any huge single field can still stub silently.
