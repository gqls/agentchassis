...
| **(new) index missing `system-stats`** | live page (12 min post-deploy) has 4 of 5 sections | suspect = assembler `getPageSections` >10-char visible filter; run Q1 (still in `page_components`?) + Q2 (visible_len) — fork: builder-drop / content-empty / filter-false-positive |
...
| **(new) `rerender_single_page` invalid page_id** | seen 2026-06-24 — `render_page` (in a spawned child) fails `invalid page_id: invalid UUID length: 31`/`18`; the action is correct (rejects a non-UUID) | a render loop passes a name/slug/path as `page_id`; find the agent whose `default_config` has both `render_page` + `rerender_single_page`, inspect its loop `items_field` + `page_id` mapping |
...
2. **Deploy the Go change:** `go build ./...` → GitHub → Backblaze → new chassis image → `snapshot_agent`
   then bump `image_tag` on agents that run `save_page_sections` (`page-build-handler`, `page-rerender`, any
   builder with a `save_sections` step). Layer 1 fires only on interactive pages → guides/index unaffected.
