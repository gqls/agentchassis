...
1. **`needs_page` vs `needs_content_page`** — two types, same `page-build-handler`,
   same effective work. Share one dedup namespace, keep separate, or unify the types?
   The apply steps below assume **keep separate, prefix == type** (conservative; no
   behaviour change beyond fixing the drop). Unifying changes the key-builder and adds
   a type migration.
...
2. **Code — repoint adoption** (`apply_adoption_plan_action.go`, ~line 655): tool items
   → `needs_tool_recreation:<name>`; content items → `needs_content_page:<name>` (was
   `needs_page:<name>` for both). This is the Bug 2 fix.
