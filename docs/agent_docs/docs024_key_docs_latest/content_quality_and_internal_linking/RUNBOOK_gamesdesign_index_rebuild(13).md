...
**Fix direction — a routing change (not a handler-branch fix).** `link_resolution_rebuild`
is links-only maintenance and should not run the writer at all. Two routes; the choice turns
on how internal links are represented:
- *links resolved at render time* → route `link_resolution_rebuild` to the **Part 2
  `page_rerender` path**: it re-renders every existing section from stored `content_data`
  (preserving the tool) and re-resolves fields — fixes the links and keeps the tool, reusing
  shipped+verified machinery.
- *links baked into LLM prose* → a **links-only rewrite over stored content** that preserves
  sections. There is already an `internal-link-resolver` / `resolve_internal_links_action.go`
  (016 §v2_45) doing links-only resolution, so the question may reduce to *why the linking §5
  batch created `link_resolution_rebuild` → page-build-handler instead of routing to that
  resolver.*

**Next diagnostic:** determine link representation — inspect a content section's stored
`rendered_html` / `content_data` (literal `<a href="/...">` vs a resolved token) and read
`resolve_internal_links_action.go` + wherever the §5 batch sets `handler_agent` for these
items. That decides which route applies and where the one-line repoint goes.
...
**Outcome — the Part 4 routing decision:**
- render-resolved links → repoint `link_resolution_rebuild` to the `page_rerender` path (reuses Part 2; preserves the tool).
- prose-baked links + an in-place resolver → repoint to `internal-link-resolver`.
- prose-baked links + no in-place resolver → build a links-only rewrite that preserves sections.
Then, regardless of route: stamp `source_item_id` into `page_component_history` (save path),
add the interactivity-aware save guard, and re-run the tool-recreation for `game-pathfinding`
(see Part 3 — fix its `needs_page:` mis-key in the same pass).
...
| 4 — interactive page clobber | Cause confirmed; fix = a routing change | **Run the link-representation diagnostic** | **P4.1 (above)** |
