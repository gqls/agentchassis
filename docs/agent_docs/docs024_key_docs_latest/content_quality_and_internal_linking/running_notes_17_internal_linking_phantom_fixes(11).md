...
- Cause is NOT page-specific: same "claim timed out / pod died" hit index (x1), games-index (x3). Points at dispatch claim-timeout window or page-build-handler pod resourcing under load (21 rebuilds at once), not content. FIX: re-queued both as `_retry2`. WATCH: if retries also time out → investigate claim-timeout/pod capacity, not the pages. If they land → it was load.
