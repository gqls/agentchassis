...
(`cta_url`←`pages.contact`, `secondary_cta_url`←`pages.services`) overwrite it? Next:
```sql
-- is the resolver wired into page-content-writer at all? (page-content-writer is its only caller)
SELECT type, (default_config::text ILIKE '%resolve_internal_links%') AS references_resolver
FROM agent_definitions WHERE type = 'page-content-writer';

-- top-level steps (section processing is a nested loop sub_workflow, per doc 005)
SELECT jsonb_object_keys(default_config #> '{workflow,steps}') AS step
FROM agent_definitions WHERE type = 'page-content-writer';
```
If `references_resolver = false`, the resolver exists but is never called → that's the bug (fix: wire
it into the section loop). If true, drill into the loop's sub_workflow for step order, and read
`render_component` / the field resolver for precedence (does the resolver's `resolved_data` survive, or
does the schema-source resolution overwrite `cta_url`?). Structural fix **reuses the existing
resolver** — it already computes the correct links; the work is making it run and win, not new code.
NEITHER Layer 1 nor Layer 2 fixes this.
