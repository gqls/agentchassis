...
### Part 14m — retire applied; docs corrected; awaiting clean run (2026-06-05)
RETIRE applied: migration_retire_current_plan_for_reconverge.sql -> 337fb25d is_current=false, current_plans_remaining=0. (User will instead CLEAR DB + re-adopt for a true from-scratch run, which also removes the orphan bare pages — cleaner than retire+rebuild.)
DOCS corrected (not claiming resolution — pending a clean run):
 - FOCUS_adoption_faithfulness_via_locks.md: added a note superseding the "convergence active on first pass" claim — convergence has been INERT since deploy due to the []map[string]interface{} vs []interface{} type mismatch; fix written, NOT yet verified.
 - 016_debugging_guide_v2_31.md: no-op pattern now lists the type mismatch as cause (0 — confirmed) + the new `existing pages loaded for convergence` count-log diagnostic; changelog amended.
NOTE on log (user paste): scheduled build-pipeline-trigger; call_dispatch -> build-dispatch-loop timed out at 300s, retried (retry_version 1, produced OK). Dispatch/throughput area (slow-deploy), SEPARATE from convergence; retry recovered.
STATUS: NOT resolved until a clean run shows existing_pages>0 + unioned_in>0 + converged plan. Test plan issued (deploy updated v3 FIRST -> clear DB -> re-adopt -> watch existing_pages count log -> verify plan/pages -> render).

### Part 14n — CLEAN RUN: convergence RESOLVED; new content/link defects surfaced (2026-06-05)
Re-adoption corr 6381cb13 (17:26Z), clean DB + updated v3 (type fix) deployed. PLAN QUERY: guide-economy-basics..guide-skinner-box all role=guide (5), ZERO bare siblings. CONVERGED. The []map[string]interface{} vs []interface{} type fix in ValidateSitePlanAction engaged reconcile (Pass A union + Pass C2 dedup) for the first time. Tools hub rendered tool-list with 6 cards; tools+games working. => the convergence/duplicate-page root cause (chased June3 -> June5 12:51 -> June5 17:26) is RESOLVED on a clean run. Pass C2 + carry confirmed live & effective.
DOC STATUS: can now flip FOCUS + debugging-guide notes from "NOT yet verified" to verified-resolved on the 17:26 run.
NEW DEFECTS (all SEPARATE from convergence — content/render layer):
 1. guides-index HUB has NO guide-list: deployed guides/index.html = hero only, no list. tools-index rendered [hero, tool-list] (6 cards), guides-index did not. Odd one out -> check guides-index.sections vs tools/games in the converged plan. (Possible: LLM gave guides-index [hero] only, OR union/carry didn't include guide-list, OR guide-list present but empty.)
 2. HERO CTAs wrong site-wide: every page hero links to /contact.html and /services.html regardless of context; text-destination mismatch (tools hero "Browse Tools" -> /contact.html; games hero "browse tools" wrong label+link); /services.html is a phantom page. Hero component cta link resolution issue.
 3. GUIDE copy tool-flavoured: guide heroes/CTAs describe simulators ("Launch P2P Simulator", "test a live Star Topology system"). Guide content-direction issue (source pairs guides w/ demos). p2p guide DID get real content (WebRTC generic-text-block) so guides have substance, just tool-framed.
 4. POLISH (Group-3): tool-list cta-btn href="" (empty, cta_url fallback); source-brand suffix in card titles ("TTK Calculator | GameDesign.uk", "- GameDesign.uk"); one empty tool desc (TTK card desc blank); empty footer brand-tagline + footer-contact mailto/phone.
NEXT: diagnose #1 (hub sections query) + #2 (hero cta links) as priorities; #3/#4 content polish after. None block the convergence closure.

### Part 14o — guides hub: guide-list deferred (timing) + reconcile_section_data not wired (2026-06-05)
> **[SUPERSEDED by Part 14p, 2026-06-06] The "guides built late / timing" diagnosis below was WRONG.** A query-sourced list field can never defer a section (the resolver returns a non-nil empty slice; plan_sections stores it and continues), so the empty guide list was never the trigger. The real cause was guide-list's `cta_url` field being `required=true` with no `on_missing`, tripping a missing `skip_field` case in plan_sections. reconcile_section_data is also a red herring for this case. Read 14p; the text below is retained only to show the (incorrect) reasoning path.
DEFECT #1 (guides hub empty) ROOT CAUSE — narrowed, one confirm pending.
Evidence chain:
 - pages: guides-index sections = ["hero","guide-list"]; root index = ["hero","tool-list","guide-list","game-list","system-stats"]; tools-index ["hero","tool-list"]; games-index ["hero","game-list"]. So guide-list IS planned on both the hub and the landing — NOT a planner/structure gap.
 - guide rows: all 5 page_type='guide', status='active' (build_status: economy-basics/fairness-in-rng/p2p-architecture=deployed; rng-design=needs_rebuild; skinner-box=planned). => they SATISFY the resolver predicate. DISPROVES the page_type/status hypothesis (would have been wrong to edit statuses).
 - resolvePagesWhereType (chassis queryresolve.go): SELECT name,title,url,meta_description,nav_label FROM pages WHERE site_id=$1 AND page_type=$2 AND status IN('active','deployed') ORDER BY COALESCE(nav_order,100),name LIMIT $3. For type='guide' this returns all 5. Resolver is CORRECT. tool-list (same resolver, type='tool') returns 6 -> resolver path works.
 - TIMING is the cause: queryresolve.Resolve is called inline from plan_sections_action.go at COMPOSITION time (not rerender). reconcile_section_data_action.go header states: plan_sections DEFERS a list section to needs_section_data (site_work_items, item_type='needs_section_data', status needs_human_review) when its query-sourced field resolves EMPTY because the listed pages don't exist yet; "those items sit forever because nothing re-plans the page after the data lands." The 2 guides still needs_rebuild/planned confirm guides were built LATE — when guides-index + root index were composed, guide pages were absent -> guide-list resolved 0 -> deferred -> section omitted from rendered HTML (hence hero-only hub). tools/games built earlier -> their lists resolved fine.
 - reconcile_section_data_action.go (uploads; registered "reconcile_section_data", Category "site", IsLocal) is the DESIGNED loop-closer: for each open needs_section_data item whose missing[].source are ALL query.*, it re-runs queryresolve.Resolve now; if data exists, emits needs_page -> page-build-handler re-renders via plan_sections -> field resolves -> closeResolvedDataRequest auto-closes. It does NOT close items itself (re-render through plan_sections is the single source of truth). BUT its header says it is NOT WIRED into any agent ("WIRING (recommended): ... pick the host agent and add a step ... Not wired here").
CONFIRM QUERY (pending): SELECT id,status, spec->>'page_name', spec->>'section_name', spec->'missing' FROM site_work_items WHERE site_id=(...) AND item_type='needs_section_data'. Expect guides-index + index, section guide-list, missing source query.pages_where_type:guide. The source string also distinguishes pure-timing (source=:guide, expected) from a stale-type wiring bug (source=:blog-post etc.).
STRUCTURAL FIX (preferred over a one-off manual re-render, per standing instruction): wire reconcile_section_data into the finalize/rerender path that runs AFTER pages are built (its header: running at plan time is too early). Need to identify the host agent workflow + add the step with site_id; then a re-render of guides-index + index closes the loop and guide-list populates. NEXT: confirm query, then find the host agent for the wiring.
NOTE: same mechanism explains the root landing's missing guides; one wiring fixes both.

### Part 14p — guides hub: REAL cause = guide-list cta_url required=true tripping plan_sections' missing skip_field case (RESOLVED 2026-06-06)
Corrects 14o. Confirmed cause + fix + verification.

ROOT CAUSE (code + data confirmed):
 - plan_sections field resolution (chassis production_agent-chassis-actions-current_context.txt ~line 44340): for a `query.*` field the resolver returns a NON-NIL empty slice on zero rows; plan_sections checks `value != nil`, stores it, and `continue`s. => a list/query field NEVER sets shouldDefer and NEVER lands in missing[]. The empty guide list was therefore NEVER the deferral trigger (14o was wrong).
 - on_missing default: `if onMissing == "" { onMissing = "skip_field" }`. The REQUIRED-field switch has cases for use_fallback / skip_section / needs_human_review / block but **NO `skip_field` case** → a required field defaulting to skip_field falls to `default:` → shouldDefer=true + recorded in missing[]. (Optional-field switch DOES have a skip_field case → just omits.)
 - guide-list_pre_037's `cta_url`: required=true, on_missing unset (→skip_field), source=site_specs.navigation.guides_index_url (unpopulated). So cta_url → not found → required+skip_field → default → DEFER whole section → guide-list not rendered on guides-index AND index (hero-only hubs). tool-list/game-list carry cta_url as required=FALSE → missing value omitted (empty href="") → section renders. blog-listing_pre_037 ALSO required=true (same latent trap).
 - Confirmation query: content_components input_schema->fields->cta_url->>required: guide-list=true, blog-listing=true, tool-list=false, game-list=false. on_missing empty on all. cta_source differs too: guide/blog use navigation.*/blog.*, tool/game use identity.*_index_url (authoring inconsistency from the per-component list migration; not the bug, but worth aligning later).
 - The needs_section_data items (guides-index/guide-list, index/guide-list) recorded ONLY cta_url (source site_specs.navigation.guides_index_url, on_missing skip_field) — consistent: cta_url was the sole defer trigger. reconcile_section_data would have SKIPPED these anyway (`if !allQuery {skippedHuman++; continue}` — cta_url source is site_specs.*, non-query), so wiring it would NOT have fixed this. Another reason 14o's reconcile angle was a red herring.

FIX (chosen — structural, at source-of-truth):
 - content_components IS the source of truth for component schemas (no repo seed; shared across sites; survives re-adoption). Snapshot content_components_bak_glcta (guide-list + blog-listing), then:
   UPDATE content_components SET input_schema = jsonb_set(input_schema,'{fields,cta_url,required}','false'::jsonb,false) WHERE name='guide-list_pre_037';  (APPLIED)
   same for blog-listing_pre_037 (APPLIED). Verify: all four list components now cta_url required=false.
 - REJECTED alternative: adding a skip_field case to plan_sections' REQUIRED switch. It would fix blog-listing + future cases globally BUT weakens a safety default — a genuinely-required field could go silently missing and render a quietly-broken section. The defer-for-safety on required fields is defensible; the actual defect is one component marking an optional field required. Fix the component, not the engine. (If component-creator ever regenerates list components, its prompt should emit cta_url required=false — separate follow-up, already noted in the directory-builder doc.)

RECOVERY (this site) + VERIFICATION:
 - Manual needs_page work item inserted for guides-index (source manual-rebuild, handler page-build-handler, status triaged, item_key page_rerender:guides-index, ON CONFLICT DO NOTHING). 
 - Result: needs_page item -> status=complete, claimed_by=build-dispatch-loop, no error. needs_section_data(guides-index/guide-list) -> status=complete (section re-resolved on rebuild). => the deferral cleared. NEW LEARNING: a manually-inserted needs_page IS dispatched (build-dispatch-loop claims it) at least while a build is active — answers the open "does the dispatcher route manual items" question.
 - index (root landing) carries the same guide-list; rebuild via the same insert with name='index', OR let the next adoption rebuild it with the now-fixed component (lower effort given frequent re-adoption).
 - Visual confirm pending (data says resolved): latest Rerender: guides/index.html commit should show the guide-list card grid (5 guides), not hero-only.

STATUS: guides-hub guide-list defect RESOLVED (component schema aligned to siblings). Convergence (14n) already RESOLVED separately. Remaining gamesdesign items still open: site-wide hero CTAs to /contact.html + /services.html (phantom /services.html); guide copy tool-flavoured (user open to real embedded demos for guides if simple); polish batch (empty "Browse All X" cta hrefs across all three lists since *_index_url specs unpopulated; source-brand suffix in card titles; one empty tool desc; empty footer brand-tagline/contact).

### Part 14q — index render gap (build_status workaround) + skinner-box sectionless-completes-as-success (2026-06-06)
Two further findings after the guide-list cta_url fix landed. guides hub + root index now both render the 5 guide cards (confirmed in deployed HTML + page_components: index guide-list = 7513 bytes, all 5 sections re-rendered together at 16:59).

A. INDEX RENDER GAP (worked around; mechanism INFERRED, not code-verified):
 - needs_page:index (12:36) completed with NO error, but page_components for index stayed stamped 03:40 — no guide-list row appeared. So the work item "succeeded" without refreshing components.
 - Reset pages.build_status='needs_rebuild' for index + re-queued needs_page → full render: all 5 sections (hero, tool-list, guide-list, game-list, system-stats) freshly written at 16:59, deployed. EMPIRICALLY build_status was the lever.
 - MECHANISM NOT PINNED: the page-build-handler workflow (agent_definitions def — captured this session) has NO build_status branch in its step list (ensure_site_record→load_page_record→check_page_found→load_existing_content→load_spec_sections→plan_sections→check_has_ready_sections→spawn_content_writer→call_content_writer→check_content_produced→validate_content→save_sections→update_status→spawn_rerender_agent→deploy_page→complete). The 12:36 run reached the success `complete` exit, so it went through save_sections→deploy_page — yet list components weren't (re)built. The gate therefore lives INSIDE the spawned page-rerender (page_renderer) agent: it appears to do a minimal re-render reusing existing page_components when build_status='deployed', vs a full plan+assemble (creating missing list components like guide-list) when 'needs_rebuild'. NOT verified against the page-rerender agent's code — would need that def/source to confirm. Stated as inference.
 - PROPER FIX (later): drive the render decision off a planned-vs-rendered section diff (does every planned section have a current page_component?), not off build_status. Then a planned-but-unrendered section always renders regardless of build_status, and the manual reset stops being needed. Workaround (set build_status='needs_rebuild' then re-queue needs_page) is reliable meanwhile.

B. MANUAL needs_page DISPATCH — CONFIRMED: build-dispatch-loop claims manually-inserted needs_page / needs_content_page items (observed repeatedly this session, including with a build active). Hand-triggering a single-page (re)build via a work item works.

C. SKINNER-BOX — planned-but-sectionless page completes as SUCCESS (root + fix):
 - CHAIN: original needs_content_page (2026-06-05 17:33) died "Claim timed out — handler pod likely died" → marked COMPLETE (silent-completion mode 3). Content was never written, AND the page was left with a site_plan_pages row but ZERO site_plan_sections rows (and pages.sections=[]). Every later rebuild (needs_page x2; the mode:recreate needs_content_page retry) completed in ~90s, NO error, ZERO page_components.
 - ROOT (workflow-confirmed): load_spec_sections → plan_sections → check_has_ready_sections (condition `section_plan.ready_count > 0`). With no sections, ready_count=0 → ELSE branch → `complete_error`, which is itself a `complete_workflow` action (a SUCCESS exit) with success_message "Content writer skipped — page has no sections defined." So the content-writer is NEVER spawned and the item completes as success. DISTINCT from silent-completion modes 1–3 (reaper/validate/claim-timeout): this is a designed success-exit for a sectionless page — a mislabeled `complete_error == complete_workflow`. The 90s completion fits (real content gen runs up to 1200s; this never called the writer).
 - SOURCE OF SECTIONS: site_specs aspects for this site = briefing, classification, content_direction, design_intent, design_reference, identity, resolved_composition, site_archetype, strategy, structure. `structure.data.pages` is a FLAT name list (includes bare "skinner-box" — note structure uses bare names while site_plan_pages uses role-prefixed "guide-skinner-box"; pre-existing naming split, not the bug). NO per-page sections in site_specs. load_page_sections_from_spec's stated primary "site_specs.site_plan" does NOT exist for this site → it resolves from the relational plan (site_plan_sections) / pages.sections fallback. Working guide rng-design has 2 site_plan_sections rows: hero@0, generic-text-block@1 (component_version_id/palette_id/layout_id/typography_set_id all NULL). skinner-box: 0 rows.
 - FIX (prescribed, snapshot first): insert 2 site_plan_sections rows for guide-skinner-box mirroring rng-design (hero@0, generic-text-block@1) under the current plan, AND set pages.sections=["hero","generic-text-block"] (BOTH, because the exact source load_page_sections_from_spec reads between relational vs pages.sections was not pinned — populating both is decisive and can't conflict for a working page). Then re-issue the mode:recreate needs_content_page. Expect: writer runs (>90s), page_components populate, build_status→deployed, skinner-box card gains its description.
 - DEEPER FIXES (noted, not done): (1) the original claim-timeout left a plan page with zero sections and nothing reconciles "page in plan, 0 sections" afterward — same family as the empty-guide-list and the render short-circuit: a partial failure no later pass repairs. A planner/convergence guard should re-plan or flag a sectionless page. (2) `complete_error` being a `complete_workflow` with a success message is the silent-completion smell on the LIVE handler path — "no sections defined" must be a distinct non-terminal/flagged state, never success.

STATUS UPDATE: index + 4 guides render with the guide-list; skinner-box fix prescribed (awaiting apply + confirm it builds). Convergence (14n) and guide-list cta_url deferral (14p) RESOLVED. Still open (separate): site-wide hero CTAs → /contact.html + /services.html (phantom /services.html); guide copy tool-flavoured (user open to real embedded demos); polish batch (empty "Browse All X" cta hrefs; "- GameDesign.uk" card-title suffix; one empty tool desc; empty footer brand-tagline/contact).

### Part 14r — skinner-box still not built after the section-row fix (state at session end, 2026-06-06)
Deployed git guides/ shows: economy-basics, fairness-in-rng, p2p-architecture, rng-design, index.html — NO skinner-box/ directory. So guide-skinner-box is STILL not built/deployed.
UNVERIFIED this turn (no work-item / page_components readout taken): whether the 14q fix (add 2 site_plan_sections rows hero@0+generic-text-block@1 mirroring rng-design, set pages.sections=["hero","generic-text-block"], re-issue mode:recreate needs_content_page 'needs_content_page:guide-skinner-box-retry2') was applied and the item completed empty AGAIN, or is still pending/triaged.
NEXT SESSION — verify FIRST (don't re-fire blind):
 1. Did the section rows take?  SELECT page_name,ordering,component_name FROM site_plan_sections WHERE plan_id=(SELECT id FROM site_plans WHERE site_id=(...) AND is_current=true) AND page_name='guide-skinner-box';  and  SELECT sections FROM pages WHERE ... name='guide-skinner-box';
 2. The retry item status:  SELECT created_at,status,claimed_by,attempt_count,left(error,80) FROM site_work_items WHERE site_id=(...) AND item_key='needs_content_page:guide-skinner-box-retry2';
 3. Components:  SELECT slot_name,length(rendered_html),updated_at FROM page_components WHERE page_id=(SELECT id FROM pages WHERE ... name='guide-skinner-box');
INTERPRET:
 - rows ABSENT → the 14q INSERT/UPDATE wasn't run; run it, then re-issue.
 - rows PRESENT but item completed ~90s/empty AGAIN → the relational site_plan_sections + pages.sections are NOT the source plan_sections actually read for readiness. This is the next root to chase: pull load_page_sections_from_spec + plan_sections from the chassis (request fresh; not on disk) and find the ACTUAL source it resolves "sections" from and how ready_count is computed. The handler's load_spec_sections description claims primary "site_specs.site_plan" (which does NOT exist as an aspect for this site — aspects are briefing/classification/.../structure; structure.pages is a flat name list with no per-page sections), so the real resolution path is unconfirmed.
 - item still triaged/claimed → dispatcher slow or stalled; confirm build-dispatch-loop pod alive (kubectl -n ai-persona-system).
