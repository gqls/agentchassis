...
| **Part 4** — interactive page rebuilt as plain content | **CAUSE CONFIRMED**; fix designed; not implemented | build Layer 1 guard + Layer 2 preserve-sections |
...
- **Layer 1 — interactivity-aware guard** in `save_page_sections`: block/escalate replacing a section that
  contains `<canvas>` / a `data-component` interactive block with non-interactive content. (Part-1's text
  guard misses this because the loss is markup/JS, not prose.)
- **Layer 2 — preserve interactive/tool sections through rebuild**: carry non-spec interactive sections
  forward, or represent the tool in the spec as a non-regenerated section. Keeps CTA re-resolution via the
  existing writer; no routing change.
- **Layer 3 (page_rerender routing) is RULED OUT** — P4.2 showed `page_rerender` preserves sections but does
  NOT re-resolve CTAs.
- Plus: stamp `source_item_id` into `page_component_history`; re-create the `game-pathfinding` game (and fix
  its `needs_page:game-pathfinding` mis-key per Part 3); delete the stale duplicate
  `z_context/check_phantom_internal_links.go` (the live `platform/...` copy is correct).
