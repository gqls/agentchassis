...
## P3.1 Decisions to make before building (don't assume)

1. **`needs_page` vs `needs_content_page`** — two types, same `page-build-handler`,
   same effective work. **New evidence (from `apply_adoption_plan_action.go:653` and the
   doc-029 Phase-0 comment): the adoption content item is keyed `needs_page:<name>` *on
   purpose*, so it co-dedups with a planner `needs_page` build of the same page.** So this
   is not a clean "make prefix == type" call — the code already shares the namespace
   deliberately, and that is defensible (same handler, same work). Two coherent resolutions:
   - **Option B (recommended): keep content in the needs_page namespace** —
     `workItemKey("needs_page", page.Name)` — preserving the planner co-dedup. The doc flip
     (P3.3 step 5) then reads "prefix == item_type EXCEPT needs_content_page shares the
     needs_page dedup namespace." Smallest change; fixes only the real bug (the tool
     collision).
   - **Option A: content → `needs_content_page:<name>`** for clean prefix == type
     everywhere — BUT then the planner and `reconcile_site_plan` must also emit
     `needs_content_page` (or the types unify), or adoption-recreate and planner-build of one
     page will double-build. Bigger change, touches more creators.
2. **adoption-recreate vs reconcile-build of the same page** — dedup together or stay
   distinct intents? Below assumes **distinct** (`needs_content_page:<name>` from
   adoption is its own row, separate from a `needs_page:<page>` reconcile build).

If either goes the other way, the key-builder and the repoints in P3.3 change to match
— resolve these first.
...
2. **Code — repoint adoption** (`apply_adoption_plan_action.go`). The work-item INSERT
   ...
   branches. Set an `itemKey` variable in each branch of the `itemType` if-else (620–638)
   and use it at line 655:
   - tool branch → `workItemKey("needs_tool_recreation", page.Name)` — **settled, this is
     the Bug 2 fix** (gives tool recreation its own namespace; it's different work on a
     different handler).
   - content branch → per the **P3.1 decision**: Option B `workItemKey("needs_page",
     page.Name)` (keep the deliberate planner co-dedup) or Option A
     `workItemKey("needs_content_page", page.Name)` (clean prefix == type, but see P3.1 for
     the knock-on planner/reconcile change).
...
5. **Docs:** flip the caveat in `002_system_architecture.md` and
   `003_contracts_and_standards.md` from "route by `item_type`, never filter by
   `item_key`" to "prefix == `item_type` — safe to filter by key." Only **after**
   deploy — until then the old caveat is still true for historical rows.
...
| A page builds twice (`page_rerender` + `needs_page`) | distinct-intents decision (P3.1.2) — may be intended | confirm against the decision; not necessarily a bug |
...
collapse to one open row; and the 002/003 caveat reads "prefix == item_type — safe to
filter." Bug 1 stays closed by Part A (B4).
