...
**To:** the `work_item_completion_integrity` thread. **ASSIGNED 2026-07-20** by
the repo owner's instruction, having been filed unowned the day before — see
§Owner for the reasoning and for how to decline it.
...
## Owner — why you, and how to decline

**Filed unowned on 2026-07-19; assigned to you on 2026-07-20.**

`write_audit_findings_action.go` has no active owning thread of its own — its git
history is old and generic (`44cd2e35b` "forking themes", `8f6ba2ddd` "prior to
fixing audit/fix endless loop") and no workstream directory tracks it. The
candidates and the reasoning:

- **`work_item_completion_integrity` (chosen).** Your remit is whether a
  `site_work_items` row can be trusted to mean what it says — you own
  `CompleteWorkItemAction`, the completion gate and `bugs_open/017`. This adds a
  provenance column to the same table and answers the same class of question at
  the other end of the lifecycle: not "did this really get fixed" but "who said
  it was broken, and were they right". You also now hold `bugs_open/032` and the
  verifier-coverage handoff, so all the work-item-integrity threads sit together
  rather than being split across three owners.
- `content_quality_and_internal_linking` — owns the auditor *agents* that produce
  the findings, but its docs are June-era and it is not obviously active.
- Nobody — leaving it unowned was the state for a day and is not a plan.

**The honest weakness in this assignment:** this is the *creation* end of the
lifecycle and your workstream is named for the *completion* end. If you think
that stretches your remit too far, say so in this file and hand it back — a
declined assignment recorded here is more useful than a silent non-adoption, and
the reasoning-dataset thread will pick the next candidate rather than let it rot.
