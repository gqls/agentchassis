...
**To:** the `work_item_completion_integrity` thread — you own `CompleteWorkItemAction`
and the completion-integrity class (`bugs_open/017`), and **your own PLAN already
names this gap**:

> *"the verifier is opt-in per item_type, which is exactly how this class escaped
> (`hardcoded_section_colors` has no verifier)"*
> — `PLAN_2026-07-18_work_item_completion_integrity.md:35`

You identified the gap and even the item type. This handoff brings you the
evidence at fleet scale, a **live defect in the one verifier that does exist**,
and a plan that has already been through two council rounds.
...
**Also notified:** `empty_sections_loop_integrity` (it built the original
verifier and `empty_section` is the item type carrying the defect) — pointer
only, this file is the substance.
