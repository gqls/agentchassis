...
*v2_45 (2026-06-12): added a §9 entry "two rebuild routes — only page-build-handler re-resolves section sources; page-rebuild does not map section_plan". Documents that a source/template re-resolution rebuild (e.g. picking up a content_components CTA/hub change) must go through page-build-handler via a site_work_items insert, NOT page-rebuild (whose writer call omits section_plan and skips plan_sections) — using page-rebuild silently redeploys without the fix. Corrects an earlier verbal shorthand that wrongly attributed this to "the v2_33 gap": v2_33 is the sectionless-page silent-completion (a prior-failure cause), a different mechanism. Confirmed from both agent rows. Also earlier this session: §9 "complex/array action input read by literal key vs config path" + §0 #15 (see below).*
