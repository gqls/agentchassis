...
page-build-handler by design. So (a) the hero CTA is schema-sourced (`pages.contact`, now stale `/contact.html`) and whether a
`page_rerender` re-resolves it is an open, testable question (P4.2) — the "repoint to
page_rerender" idea is deferred, not dropped; (b) the clobber
hits ANY full rebuild, not just link rebuilds. **Fix (layered, structural first):** (1)
