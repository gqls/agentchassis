...
- *Framework hardening (code, separate, affects every agent):* make `extractWorkflowResultWithSizeLimit` safe — recurse the trimmer into maps/arrays, or persist the large artefact and return a reference — and never return `status: "completed"` when output was dropped. A dropped result must surface as a failure, not a silent stub. This is also the structural fix for the "work item completes despite a no-op save" visibility fault. (Flatten bounds the writer so the stub no longer fires for it, but the fallback and any huge single field still can.)
