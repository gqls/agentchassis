...
to `generic-text-block`. Blast radius: one page (`game-pathfinding`). **Next diagnostic:**
dump `page-build-handler`'s step configs to see whether a preserve-existing branch exists but
is bypassed (→ routing fix) or the writer always runs (→ handler-branch fix). **Fixes:** route
link maintenance through a preserve-sections re-render path (reuses Part 2); stamp
