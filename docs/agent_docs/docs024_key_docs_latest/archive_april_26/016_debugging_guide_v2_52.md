...
If `complete_step` shows `output_field` (singular) while a working sibling handler shows `output_fields` (plural — e.g. `page-build-handler`'s `["sections_saved","deploy_result"]`), that is the bug. Dating it: if a previously-working build returned the correct shape and only one `version` row remains, the prior config is in the snapshot mechanism (§6.1) — the contract regressed when the definition was last updated. (Real case: writer `version 2`, updated 2026-06-13, the same day the symptom began; the 06-06 build returned the compile output *flattened*, which the current singular-key fallback cannot produce.)

**Fix.**
- *Contract (data, both places):* give the child's `complete` step the correct `output_fields` so the coordinator returns only the compiled result, not the whole working state — small enough to clear the limit. Land it in the live `agent_definitions` row **and** the source SQL migration, snapshot first (§6.1). The exact value must reproduce the path the parent reads (`page_content.response.sections_metadata`): `output_fields: ["page_content"]` nests the result under a `page_content` key (the array would land at `page_content.response.page_content.sections_metadata`), so recover the prior working config from the snapshot rather than assuming — the known-good response was the compile output flattened.
- *Framework (code, separate, affects every agent):* make `extractWorkflowResultWithSizeLimit` safe — recurse the trimmer into maps/arrays, or persist the large artefact and return a reference — and never return `status: "completed"` when configured `output_fields` were dropped. A dropped result must surface as a failure, not a silent stub. This is also the structural fix for the "work item completes despite a no-op save" visibility fault.
