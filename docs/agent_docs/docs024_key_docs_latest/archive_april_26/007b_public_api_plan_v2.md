...
### Work Items (build progress + HITL review)

| Method | Path | Action |
|---|---|---|
| `GET` | `/sites/:site_id/work-items` | List, filterable by status/domain/item_type |
| `GET` | `/sites/:site_id/work-items/:item_id` | Get detail with full spec |
| `PATCH` | `/sites/:site_id/work-items/:item_id` | Update status, spec, handler_agent, item_type |
| `POST` | `/sites/:site_id/work-items/:item_id/retry` | Convert to content_rewrite, reset attempts, set triaged |
| `POST` | `/sites/:site_id/work-items/:item_id/resolve` | Mark complete with resolution note |

`GET /sites/:site_id/work-items` supports query params: `?status=needs_human_review&domain=build&item_type=placeholder_content&limit=50`

#### HITL Review Flow

The pipeline creates `needs_human_review` work items when content validation fails (placeholder text detected, missing data sections). The review flow is:

```
Content generated → validate_page_content catches "NEEDS HUMAN REVIEW"
  → fail_work_item with status_override: needs_human_review
  → Section hidden on live site (replaced with HTML comment)
  → Work item visible in review queue
```

Three resolution paths via the API:

**Path 1: Provide data and retry.** Update the site spec with missing data (e.g. add leadership_team to identity spec), then retry the work item. The page-build-handler regenerates the page with the new data.

```
PATCH /sites/:site_id/specs/identity  → add team data
POST  /work-items/:item_id/retry     → converts to content_rewrite, dispatch picks it up
```

**Path 2: Retry without changes.** If the problem was transient (LLM fluke, API timeout), retry directly. The work item is converted to `content_rewrite` with `page-build-handler` and reset to `triaged`.

```
POST /work-items/:item_id/retry
```

**Path 3: Dismiss.** The section stays hidden. The work item is marked complete with a resolution note. Used when the data genuinely isn't available yet.

```
POST /work-items/:item_id/resolve  body: {"resolution": "Awaiting real team data from client"}
```

#### Work item statuses

| Status | Meaning | API actions available |
|---|---|---|
| `triaged` | Ready for dispatch | retry (no-op, already triaged) |
| `claimed` | Handler agent is processing | (wait) |
| `complete` | Done | (none) |
| `failed` | Handler failed after max attempts | retry, resolve |
| `blocked` | Handler agent doesn't exist | retry (after deploying agent) |
| `needs_human_review` | Content validation caught placeholder/contamination | retry, resolve, update spec then retry |
| `detected` | Discovery check found it, not yet triaged | (auto-promoted by triage step) |

### Site Specs (read + write)

| Method | Path | Action |
|---|---|---|
| `GET` | `/sites/:site_id/specs` | All current spec aspects |
| `GET` | `/sites/:site_id/specs/:aspect` | One aspect + recent history |
| `PATCH` | `/sites/:site_id/specs/:aspect` | Update spec — supersedes old version, inserts new |

**PATCH /sites/:site_id/specs/:aspect** — the primary way to provide missing data for the HITL review flow. Supersedes the current spec for this aspect (marks old row `is_current = false`, inserts new row with `source = 'admin-api'`). History is preserved — the old spec remains in the table with `superseded_at` timestamp.
...
```json
{
    "data": {
        ...
        "email": "finetuning@contactforsales.com",
        "leadership_team": [
            {"name": "Peter Grenfell", "title": "Founder", "bio": "..."}
        ]
    }
}
```
...
Common aspects: `identity` (company info, contact, team), `design_intent` (colours, typography, imagery), `content_direction` (voice, emphasis, avoid phrases), `site_plan` (page structure).
...
**Implementation status:** `site_admin_handlers.go` is written and ready to deploy. It covers the HITL review workflow (list/get/update work items, update site specs, retry/resolve). The build queue and site lifecycle management endpoints (assign, trigger-build, delete) are not yet implemented.
...
|---|---|---|---|---|
| 1 | Block 0: site_ownership migration | Nothing | Half day | **Not started** — admin API works without it (no user scoping yet) |
| 2 | Block 1a: HandleCreateSite + HandleListSites | Block 0 | 1 day | **Not started** |
| 3 | Block 2: Auth gateway proxy | Block 1a | 1 hour | **Not started** |
| 4 | Block 1b: Site detail + status + pages | Block 1a | 1 day | **Not started** |
| 5 | Block 1c: Work items list + detail | Block 1b | Half day | **Not started** (admin version done — see Block 3) |
| 6 | Block 1d: Work item retry/resolve | Block 1c | Half day | **Not started** (admin version done — see Block 3) |
| 7 | Block 1e: Specs (read + write) + assets | Block 1b | Half day | **Not started** (admin version done — see Block 3) |
| 8 | Block 3: Admin site handlers | Nothing | 1.5 days | **IMPLEMENTED** — `site_admin_handlers.go` |
| 9 | Block 4: Admin gateway lines | Block 3 | 1 hour | **Ready** — proxy lines documented in `patch_site_admin_routes.go` |
| 10 | Block 6: Briefing bridge | Block 1d | 1-2 days | **Not started** |
| 11 | Block 5: WebSocket events | Block 1c | 1-2 days | **Not started** |

### What Block 3 covers (implemented)

The admin HITL review workflow is functional. These endpoints exist in `site_admin_handlers.go`:

| Method | Path | Handler | Purpose |
|---|---|---|---|
| `GET` | `/admin/sites` | HandleListSites | Sites with work item count breakdown |
| `GET` | `/admin/sites/:site_id` | HandleGetSite | Site detail with all current specs |
| `PATCH` | `/admin/sites/:site_id/specs/:aspect` | HandleUpdateSiteSpec | Supersede spec with new data |
| `GET` | `/admin/work-items` | HandleListWorkItems | Filterable list (?status, ?domain, ?site_id, ?item_type) |
| `GET` | `/admin/work-items/:item_id` | HandleGetWorkItem | Full detail with spec |
| `PATCH` | `/admin/work-items/:item_id` | HandleUpdateWorkItem | Update status, spec, handler, type |
| `POST` | `/admin/work-items/:item_id/retry` | HandleRetryWorkItem | Convert to content_rewrite, reset, re-triage |
| `POST` | `/admin/work-items/:item_id/resolve` | HandleResolveWorkItem | Mark complete with resolution note |

### What Block 3 does NOT cover (still in plan)

- `POST /admin/sites/:site_id/assign` — assign site to user (needs site_ownership table)
- `POST /admin/sites/:site_id/trigger-build` — manual build trigger via Kafka
- `DELETE /admin/sites/:site_id` — soft delete with cascade
- `GET /admin/build-queue` — list queue entries
- `POST /admin/build-queue` — bulk queue domains
- `POST /admin/build-queue/seed` — manual seed trigger

### React Admin Dashboard

A React frontend (`site-admin-dashboard.jsx`) provides the UI for the review workflow:

- **Dashboard view** — site cards with progress bars and work item count breakdown
- **Review Queue view** — lists `needs_human_review` items with Review/Retry/Dismiss actions
- **Review Detail view** — shows work item spec, editable site identity spec (JSON), "Save Spec & Retry" button

Currently uses mock data (toggle `useMock = false` when API is connected). Uses Tailwind utility classes.
...
|---|---|---|---|
| `internal/core-manager/admin/site_admin_handlers.go` | 3 | Admin HITL review: sites, specs, work items | **Written** — ready to deploy |
| `site-admin-dashboard.jsx` | 3 | React admin UI for review workflow | **Written** — uses mock data until API connected |
| `patch_site_admin_routes.go` | 3, 4 | Route registration + gateway proxy documentation | **Written** |
...
| `internal/core-manager/sites/handlers.go` | 1 | Public site CRUD handlers (user-scoped) |
| `internal/core-manager/sites/work_items.go` | 1 | Public work item handlers (user-scoped) |
| `internal/core-manager/sites/specs.go` | 1 | Public spec read/write handlers (user-scoped) |
...
### Modified files (when deploying)

| File | Block | Change |
|---|---|---|
| `internal/core-manager/api/server.go` | 3 | Add site admin route groups to setupRoutes |
| `cmd/auth-service/main.go` (router setup) | 4 | Add gateway proxy lines for `/sites` and `/work-items` |

### Not modified

All existing agent definitions, workflows, Go actions, and the agent-chassis codebase remain untouched. The admin API is purely additive — it reads from and writes to the same tables the agents already use, without changing how the agents work.
