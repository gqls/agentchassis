...
hits ANY full rebuild, not just link rebuilds. Plus a separate, possibly bigger thread surfaced
by P4.2: the phantom CTA may survive a FULL rebuild too (a §5 rebuild left `/contact.html`), i.e.
the `pages.contact` CTA source likely resolves wrong — a phantom-CTA-resolution bug distinct from
