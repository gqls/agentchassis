...
definitions carry step-LEVEL `error_step` on several steps — the documented
silently-ignored form. Never copy that shape; correct adjacent instances when
...
explicit skip log line — a run that died upstream also produces 0 rows.
