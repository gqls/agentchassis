...
The `page-build-handler` wraps the content writer with data triage, validation, and deployment. It processes one page at a time as a dispatched work item.
...
21. **One item per dispatch orchestration.** The dispatch loop loads one item, processes it, and completes. No batch accumulation, no OOM risk. The pipeline trigger fires every 30 seconds and dispatches the next item.