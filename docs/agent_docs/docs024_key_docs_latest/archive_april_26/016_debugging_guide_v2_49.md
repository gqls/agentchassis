...
*v2_49 (2026-06-14): added two §9 entries — (1) "two rebuild routes — only page-build-handler re-resolves section sources; page-rebuild does not map section_plan" (a source/template re-resolution rebuild MUST go via page-build-handler + a site_work_items insert, not page-rebuild, which silently redeploys without the fix; corrects an earlier verbal shorthand that wrongly called this "the v2_33 gap" — v2_33 is the sectionless silent-completion, a different mechanism); (2) "work item is complete but the page is stale — silent-completion from a pod dying mid-flight (often a CONCURRENT DEPLOY)" — `complete` with a non-empty `error` ("Claim timed out — handler pod likely died") is NOT proof the work happened; trust the rendered HTML, re-queue with a fresh item_key, and DON'T roll the chassis image while a rebuild batch is draining. Both surfaced during the gamesdesign linking-fix §5 batch (21 complete / index+games-index stale). Also cross-linked the A1 tool-recreation entry to PLAN_pathfinding_missing_game (pathfinding renders prose, no widget — same adopted-interactive-page-no-widget family as the May–June investigation; diagnosis-first, candidates incl. clobber/never-persisted/the `<div>`-not-`<section>` parser miss).*
...
**Cause — usually NOT load.** With ample capacity and low load, the common trigger is **deploying something else to production concurrently**: a chassis image rollout (or `kubectl rollout restart`) cycles the agent pods, killing any in-flight `page-build-handler` jobs. They present exactly as "claim timed out / pod likely died." Multiple pages failing the same way in one batch, with no resource pressure, points here rather than at dispatcher capacity. **Operational rule: do not roll the chassis image (or restart agent deployments) while a rebuild batch is draining.** Let the batch finish, then deploy.
