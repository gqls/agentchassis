...
| ⚠ Fleet LLM capability | ~~**DOWN until 2026-09-01** — monthly spend cap~~ **CORRECTED 2026-08-15 08:52Z: LIVE, and it never was a three-week outage.** `llm_call_log` success side: **31 ok / 0 failed in the 08:00Z hour**, and unbroken success for 30 hours (last failures were 1–2/hour on 08-14 evening, i.e. noise, not a cap). **The council gate is AVAILABLE — a lane that read this row and deferred a submission should submit.** What caught it: `LANDMINES.md:2028/2032` says in terms that *the stated reset is the vendor's worst case, not a forecast* — the binding variable is a human raising the cap, which on this estate has happened within hours three times. **Verify on the SUCCESS side (`count(*) FILTER (WHERE success)`), never the failure side**, and never from `orchestration_states`, which showed 63 COMPLETED / 0 FAILED straight through a total outage |
