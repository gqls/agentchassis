...
VALUES ('{site_id}', 'identity', {lit(new_identity)}, 'manual', NULL,
        'Divergence D3/D6: positioning as the whole-borrowing-picture site, distinct from the single-subject siblings.', true, 'cqls'),
       ('{site_id}', 'content_direction', {lit(new_cd)}, 'manual', NULL,
        'Divergence D3/D6: positioning block added inside content_direction so `formatted` carries it to page-content-writer. formatted regenerated by a gated port of datahelpers.FormatContentDirection.', true, 'cqls');
COMMIT;
"""
