...
#     (SEED_2026-08-20_identity_and_tools.sql; spec row 6ca809d6).
...
# Pre-state, measured 2026-08-20 11:55Z, before the fire:
PRE_IDENTITY_MD5='86f42aa5d5c122da938c970a01dc6ff4'   # UNCHANGED since 08-17: the lane's repair held   # name|url|page_type|status
PRE_ACTIVE=45; PRE_ARCHIVED=1; PRE_OWNED=24; PRE_GENERIC=22
# Snapshot tables: pages_bak_20260820_preplan_lmc (46), page_components_bak_20260820_preplan_lmc (82)
...
$PSQL -tA -F\$'\t' -c "SELECT count(*) FILTER (WHERE status='active'), count(*) FILTER (WHERE status='archived') FROM pages WHERE site_id='$SITE_ID';"
$PSQL -tA -c "SELECT name, url, created_at FROM pages WHERE site_id='$SITE_ID' AND created_at > '2026-08-20' ORDER BY created_at;"
...
# drop rows): pages_bak_20260820_preplan_lmc holds all 46 rows as of 11:55Z.
EOF
