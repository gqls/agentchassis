...
- [ ] Inspect the 4 oddball responses (`response_text NOT LIKE '{%' AND NOT LIKE '[%' AND NOT LIKE '\`\`\`%'`)
- [ ] Write the export function / view — single query that emits NDJSON, one line per training row, in ChatML+metadata format with response cleaning applied
- [ ] Decide dataset scope on export: recent-only vs all-time-with-join-fallback
- [ ] Run export for `page-content-writer` iter_0 as the first real dataset
...
- `OLLAMA_LOAD_TIMEOUT=10m` and `OLLAMA_KEEP_ALIVE=30m` are now env defaults in the kustomize base.
