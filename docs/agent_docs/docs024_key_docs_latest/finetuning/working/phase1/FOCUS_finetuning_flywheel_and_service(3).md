...
- [ ] Add `search_document:` prefix to content before embedding inside `rag_index` action.
- [ ] Add `search_query:` prefix to query string before embedding inside `rag_lookup` action.
- [ ] Log in production whether embeddings were prefixed (add `prefix_applied` to logs) so we can spot regressions.
- [x] **Step 3 (chassis integration) passed on 2026-04-21.** Created `rag-test-agent` (deterministic 3-step workflow: rag_index → rag_lookup → complete_workflow), triggered via Kafka on `system.agent.generic.requests`, chassis v1.0.979 executed cleanly to COMPLETED status. Content was chunked, embedded, stored; lookup workflow step ran without error. Registry patches (PATCH 03 from 005_PATCHES.md) are confirmed live in v1.0.979.
