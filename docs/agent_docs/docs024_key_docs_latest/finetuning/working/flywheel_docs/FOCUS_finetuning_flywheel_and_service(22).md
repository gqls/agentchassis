...
- **OPEN — SSH connection mechanism (Phase 4, partially resolved 2026-05-24).** The adapter generates its own ed25519 keypair, stores the private half in k8s Secret `thunder-ssh-<db_row_id>`, sends the public half as `public_key` on create (Thunder returns `key:""` — it does not generate its own when we supply one). CONFIRMED via probes: (1) `tnr connect` does server-side setup ("Checking SSH keys... / Waiting for SSH service... / Setting up instance...") before SSH works — a raw dial before it gets `Connection refused`; `ssh_exec` must **wait-for-sshd** (RUNNING ≠ sshd ready). (2) The real login user is **`ubuntu`**, NOT `root`, despite Thunder's `ssh_command` string printing `root@` — every `root` probe was denied on the wrong user, not a bad key. (`ssh_user="ubuntu"` is already the provision hardcode + schema default, so that part is correct.) (3) The SSH port from `/instances/list` is unreliable; use the `tnr connect --json`-resolved port. STILL PENDING: whether raw ssh as `ubuntu` with our stored key authenticates (the decisive test kept getting eaten by a `bash`+heredoc wrapper that only echoes the script). If yes → `ssh_exec` is a plain `x/crypto/ssh` dial as `ubuntu`, our key, wait-for-sshd, and the only provision gap is storing the SSH **port** (no `port` column in `thunder_instances` yet). If no → we must obtain Thunder's per-instance key, which it returns empty to us (harder). Do not build `ssh_exec` until that test prints `OUR_KEY_AS_UBUNTU_OK`.
