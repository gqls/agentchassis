...
- **Still unverified:** the `POST /instances/create` *response* shape (a different endpoint from status/list — may also be camelCase/string-typed). The body-preview logging in `api/client.go` reveals it on the next real provision; fix `CreateInstanceResponse` then rather than guessing.
