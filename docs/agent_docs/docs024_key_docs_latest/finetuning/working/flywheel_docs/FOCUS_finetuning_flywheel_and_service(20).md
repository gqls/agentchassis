...
- **OPEN — SSH key handling.** Create returns a server-generated SSH *private* key in `key`, but the adapter generates its OWN ed25519 keypair, stores the private half in a k8s Secret, and sends the public half as `public_key`. Unresolved: does Thunder honour our `public_key` for SSH auth, or only its own returned key? This gates the Phase 4 SSH path — if Thunder ignores our key, SSH with our stored private key fails and we must store Thunder's returned `key` instead. Probe before relying on the SSH path: provision with our public_key, then SSH in with our stored private key.
