...
- **`POST /instances/create` required body fields:** `gpu_type` (string, case-sensitive, e.g. `"A100"` not `"a100"`), `num_gpus` (int), `cpu_cores` (int — NOT `vcpus`), `disk_size_gb` (int), `mode` (enum: `"prototyping"` or `"production"`), `template` (string — use `"base"` for a plain GPU instance; see template note below). `public_key` is optional. Missing any required field returns `400 invalid_request` with `"Invalid request body"`; an unrecognised value returns `400 validation_error` with a specific message like `"validation: invalid template: <value>"`. Our defaults: cpu_cores=4, disk_size_gb=100, template="base", mode="prototyping".
- **Template values are NOT the OpenAPI example.** The OpenAPI spec shows `"ubuntu-22.04"` as the `template` example — that is a placeholder, not a real template, and Thunder rejects it with `400 invalid template`. Real templates: `base` (plain GPU instance, what we use for fine-tuning), plus pre-built AI stacks `ollama`, `comfy-ui`, `forge-neo`, `unsloth`. Enumerate live templates with `GET /thunder-templates`.
...
- **Request/response field shape is asymmetric (verified 2026-05-20 via `tnr status --json`).** The CREATE request uses snake_case with real ints (`gpu_type`, `cpu_cores`, `num_gpus`, `disk_size_gb`). The STATUS/LIST response uses camelCase with numbers as JSON STRINGS (`gpuType`, `cpuCores`:"4", `numGpus`:"1", `id`:"0", `createdAt`) — only `storage` and `port` come back as real ints. Response enum values are UPPERCASE (`status`:"RUNNING", `gpuType`:"A100XL") even though requests send lowercase. Do not assume request and response share field names or casing. The `Instance` struct in `api/types.go` matches the verified response; `CreateInstanceRequest` matches the verified request.
- **`gpu_type` enum is lowercase (verified):** `t4`, `a100`, `a100xl`, `h100` (prototyping also allows `a6000`; production is `a100`/`h100` only), per `tnr create --help`. Requesting `a100` returns an 80GB A100 reported as `gpuType:"A100XL"` in the response.
- **Status casing — resolved.** Thunder returns `RUNNING`/etc UPPERCASE in API responses. `IsReadyStatus` and `IsTerminalStatus` in `api/types.go` lowercase both sides (`strings.ToLower`), so the polling loop is robust to casing on either side. The numeric `id` arrives as a string; use `Instance.IdentifierInt()` to get the int for delete/get endpoints.
- **`POST /instances/create` response shape (verified 2026-05-22):** `{"uuid":"<str>", "key":"<server SSH private key PEM, often empty>", "identifier":<int>}`. `identifier` is a real int (distinct from the status/list endpoint's string `id`), so `CreateInstanceResponse.Identifier int` is correct. `key` is a Thunder-generated SSH *private* key — see the open SSH-key question below.
- **Provision loop verified end-to-end (2026-05-22).** `provision_instance` runs create → poll `/instances/list` until RUNNING → INSERT `thunder_instances` → success response → chassis `ClaimAwaitedRequest` → `awaited_requests` flips `waiting → processed` (~1s) → orchestration `COMPLETED`. Both success and error responses route and claim correctly. The matcher failures that blocked this for days were two adapter/schema bugs, now fixed and documented: (1) response headers must be a **typed struct** so `is_complete`/`is_error` serialise as JSON bools — a `map[string]string` sent string bools and the chassis dropped the reply on unmarshal (contracts §Adapter Response Envelope; debugging guide §9); (2) `thunder_instance_id` uniqueness must be a **partial index on live rows**, because Thunder recycles numeric ids — a table-wide unique constraint caused duplicate-key on every reprovision (debugging guide §9).
- **Instance lifecycle / billing.** The provision path intentionally leaves the instance RUNNING on success — it's meant to hand off to the training workload, after which decommission (or the reaper at `max_uptime_hours`, default 18) tears it down and computes cost. For provision tests with no training following, delete manually (`tnr delete <id>`) and reconcile the row, or the instance bills until the reaper sweeps. Thunder recycles ids, so consecutive tests reuse low ids (0, 1, …) — expected, and now handled by the partial index.
- **TODO (deferred 2026-05-24) — orphan-sweep for stale live rows.** A manual `tnr delete` (or any out-of-band deletion) leaves the `thunder_instances` row `running` because nothing reconciles it. The reaper's `pre_query` only catches rows past `max_uptime_hours` — it does NOT catch a row whose Thunder instance no longer exists. Because such a stale `running` row holds the recycled id in the partial unique index `thunder_instances_live_identifier_uniq`, it **blocks the next provision that gets the same recycled id** with `duplicate key (SQLSTATE 23505)`. This bit on 2026-05-24: a provision failed to INSERT because the prior probe row (`c22d4448`, id 0) was still `running` until reconciled minutes later. **Agreed design (not yet built):** (1) new adapter action `sweep_orphans` — lists Thunder instances via the existing `thunderAPI` list call, loads live DB rows (`status IN provisioning/running/decommissioning`), computes the set difference (live DB rows whose `thunder_instance_id` is absent from Thunder's live set), and dispatches `decommission_instance` (by `provisioning_id`) for each orphan. Reuses the existing idempotent decommission action end-to-end — no duplicated reconciliation logic; the sweep is a pure detector/orchestrator. (2) new scheduled task `thunder-orphan-sweep`, every 15–30 min, sharing the reaper's `concurrency_group` so they don't fight. **Safety guard (must bake in):** only act on orphans if the Thunder list call clearly succeeded (non-error); never sweep on an empty/partial list while the DB shows recently-provisioned rows — a transient list failure must not be mistaken for "everything is an orphan" and tear down live instances. **Interim workaround until built:** after any manual `tnr delete`, reconcile the row (`UPDATE thunder_instances SET status='decommissioned', decommissioned_at=NOW(), cost_usd=GREATEST(0,EXTRACT(EPOCH FROM(NOW()-running_since))/3600.0)*hourly_rate_usd WHERE id='<uuid>' AND status='running'`), and let the reaper close long-lived ones at `max_uptime_hours`.
- **Spend gating lives in DB, not API.** The adapter calls `thunder_provision_check` (a Postgres view) before every create. The view sums `decommissioned_spend_24h + running_estimated_spend + estimated_new_run_cost_usd` and compares to `thunder_config.daily_cap_usd`. Defaults: cap=$100, per-run estimate=$25. For testing, drop the per-run estimate to ~$2 — otherwise even a fresh cap rejects a single test run. Note the 24h window is rolling: heavy test days can trip the cap on legitimately-accrued past spend (raise the cap for the session rather than clearing rows, which are accurate).
- **RESOLVED & VERIFIED (Phase 4, 2026-05-24) — SSH connection mechanism + ssh_exec/ssh_get_status.** The adapter generates its own ed25519 keypair, stores the private half in k8s Secret `thunder-ssh-<db_row_id>`, sends the public half as `public_key` on create (Thunder returns `key:""` — does not generate its own when we supply one, but it DOES honour ours). All mechanics confirmed against live instances and then end-to-end through the deployed adapter: (1) SSH is a **direct `x/crypto/ssh` dial** to `instance_ip:ssh_port` — no proxy, no tunnel, no shelling out to `tnr`. (2) Login user is **`ubuntu`** (NOT `root`, despite Thunder's `ssh_command` string saying `root@` — every root attempt was denied on the wrong user; `whoami` over a real `tnr connect` returns `ubuntu`). (3) Our adapter key authenticates as `ubuntu` (verified: `OUR_KEY_AS_UBUNTU_OK`). (4) The SSH port is the `/instances/list` `port` field — verified identical to the `tnr connect`-resolved port AND directly dialable (31421==31421 test); provision captures `inst.Port` into the new `thunder_instances.ssh_port` column. (5) The instance reaches Thunder status RUNNING BEFORE sshd accepts, so `ssh_exec` **waits-for-sshd** (retry the dial ~90s). Production verification 2026-05-24: `ssh_exec` with `nvidia-smi -L` → `SSH connected addr=...:31853 user=ubuntu attempt=1`, `exit_code:0`, stdout = the A100 line, `reachable:true`. `ssh_exec`/`ssh_get_status` reuse `store.LookupByID` (same lookup as decommission) and a narrow `privateKeyGetter` interface for `secretMgr.GetPrivateKey` — matching the established action pattern, not raw SQL.
...
### Phase 4 data-flow actions (prepare_dataset_url / prepare_artefact_url) — storage design

**PHASE 4 STATUS: COMPLETE & DEPLOYED (verified in production 2026-05-24).** All adapter actions for the data/SSH plane are live: `prepare_dataset_url` (verified — 200 GET of the real ~21.5MB dataset), `prepare_artefact_url` (built; PUT untested until a real artefact exists, but same client + key has `writeFiles`), `ssh_exec` (verified — exit 0, A100 stdout, via the deployed adapter), `ssh_get_status` (built on the identical dial path), and `decommission_instance` (verified including the idempotent 404/already-deleted path). Provision captures `ssh_port`; the `thunder_instances.ssh_port` migration is applied. Next milestone: Phase 5 — the real `training-launcher` that uses `ssh_exec` (model-trainer's call_launcher passes instance_ip/ssh_user/ssh_key_secret_name; ssh_exec now resolves all of it by provisioning_id).

- **The adapter mints presigned URLs; it never moves the data.** `prepare_dataset_url` returns a presigned GET URL (the VM downloads the training JSONL from B2); `prepare_artefact_url` returns a presigned PUT URL (the VM uploads the trained adapter to B2). Only the URL (a few hundred bytes) travels over Kafka; the actual bytes go **directly between the VM and B2 over HTTPS**, never through Kafka or the adapter. This is the credential-boundary point of the adapter pattern: the VM holds no B2 credentials, just a time-limited URL.
- **Bucket / key convention (VERIFIED end-to-end 2026-05-23 — `b2 ls` + a 200 presigned GET of the real ~21.5MB dataset):** bucket **`personae-model-training`**; dataset key `finetuning/datasets/{export_id}/training.jsonl`; artefact key `finetuning/artefacts/{training_run_id}/adapter.tar.gz`. Note `finetuning/` is a FOLDER PREFIX inside the bucket, not a bucket name — the URL path is `personae-model-training/finetuning/datasets/...` (the repeated word is legitimate). **The training-data-preparer agent_def lists `s3_bucket=finetuning`, but no such bucket exists** — that value is stale/logical and cost a debugging cycle of 403s; the real data lives in `personae-model-training` (last written 2026-05-12, matches iter_0 prep). `model_lifecycle.training_runs` has no `dataset_uri` column, so the bucket can only be confirmed against the live B2 bucket list, not the agent_def. Callers pass `export_id` / `training_run_id`; the adapter builds the keys. Expiry: dataset 60min, artefact 24h (must outlive a training run). The dataset GET is verified; the artefact PUT is verified-by-inspection only (same client + key has `writeFiles`) until a real run produces an artefact.
- **Credentials:** the adapter builds its own `storage.S3Client` (reusing `platform/storage`, mirroring agent.go's optional-storage pattern). B2 keys (`B2_APPLICATION_KEY_ID` / `B2_APPLICATION_KEY`) live in the k8s secret **`personae-storage-secrets`** — NOT `personae-platform-secrets` (an earlier thunder-adapter manifest comment wrongly claimed the latter; verified against the cluster and the agent-chassis deployment, both read B2 from `personae-storage-secrets`). The adapter manifest must `envFrom` that secret plus set `TRAINING_BUCKET=personae-model-training` and `S3_ENDPOINT`. If creds are absent at startup the storage client stays nil and the two actions return `storage_unavailable` (provision/decommission still work). Debugging note: a presigned-GET 403 against a confirmed-existing object is usually test-method noise — `curl -I` (HEAD) and stale/expired URLs both 403; use `curl -s` (GET, body visible) on a freshly-minted URL to see B2's real `<Code>`.

### Storage credential architecture — decision (2026-05-22)

- **Decision: hardcode the adapter's storage env for now; adopt centralised credential *sourcing* later; do NOT build a storage-*adapter* service.**
- **Why not a storage-adapter (a service that owns the creds and others message it):** for the data-flow path it would mean routing dataset/artefact **bytes** through Kafka to/from that service. Datasets are multi-MB and artefacts hundreds of MB; Kafka's default `max.message.bytes` (~1MB) makes that a real failure, and even raised it wrecks broker memory and consumer lag. The presign design already avoids this — only URLs move — so a storage-adapter would add a hop and a boundary that the presign pattern provides more cheaply. A storage-adapter is only safe for *minting URLs*, and is dangerous the moment anyone uses it to move data.
- **Why centralised sourcing (the eventual fix):** today the same B2 creds are sourced four different ways across services (webscrape uses `B2_APPLICATION_KEY_ID`; image-generator uses `AWS_*` from `personae-storage-secrets` + a `storage-config` configmap; the preparer relies on spawn-injection; thunder now hardcodes B2 env). The clean fix is one shared constructor (e.g. `storage.NewDefaultClient(ctx, bucket, logger)`) that reads `personae-storage-secrets` the same way everywhere — kills the four-conventions mess **without ever routing data through Kafka**. If a credential boundary is ever needed for an untrusted/LLM-driven agent, give it presigned URLs (the thunder pattern), not a blob pipe. Deferred to a dedicated platform pass; not built yet.
- **Build-impact note:** `GetPresignedPutURL` was added to the shared `storage.Client` interface for `prepare_artefact_url`. Because the interface is implemented/asserted across the platform, **every binary importing `platform/storage` must be rebuilt** (core-manager, chassis, all adapters) — adding it broke the core-manager build until `*S3Client` gained the method. See debugging guide item 18.
...
## Update 2026-06-04 — training monitor + the checkpoint/adapter durability plan

**Training monitor built (verifying).** The `running → complete/failed` reconcile and
release-on-completion that this doc flagged as missing are now the `thunder-training-monitor`
orchestrator + `-worker` (migrations 106/107/108 + 5 actions). It is the completion-side
counterpart to the orphan-sweep TODO above: the monitor handles *finished* training boxes
(probe run.sh markers → reconcile `training_runs` → decommission), while the orphan-sweep
(still TODO) handles boxes whose Thunder instance no longer exists. Both reuse the idempotent
`decommission_instance` end-to-end; both run as periodic scheduled tasks. Monitor deploy/verify
is in progress — a manual worker test surfaced a reply-topic orphan in
`dispatch_thunder_ssh_get_status` (it put `__parent_responses_topic__` in the adapter envelope
while the coordinator awaited on `execCtx.ResponsesTopic`); fixed to prefer `execCtx.ResponsesTopic`,
pending adapter-log confirmation + redeploy.

**The checkpoint + final-adapter durability gap now has an agreed design** —
`PLAN_checkpoint_and_artefact_upload_b2.md`. Root cause: the Thunder VM disk is ephemeral and
nothing moves training output off it — `run.sh` writes the adapter only to `/workspace/adapter_out`
and emits `RUN_SH_DONE` before any upload (the "later artefact-collector" never built), and
`02_train` runs `save_strategy="no"` (no checkpoints, so a reap/crash = total loss). The same
"VM → B2 upload" mechanism solves all three (checkpoints, final adapter, resume). Approach:
**presigned single-object write-only PUT URLs minted by the adapter**, pre-minted at launch into
`/workspace/upload_manifest.json`, keyed by save-index, write-once (B2 versioning backstop). The
VM holds no B2 key and exposes no inbound endpoint (chosen over a scoped key or a per-save callback
under "assume Thunder is hostile"); integrity is the eval gate's job, not the URL's. Build order:
`02_train` callback + resume → launcher presign/manifest (+ adapter list-keys for resume) →
`run.sh` `RUN_SH_DONE`-after-upload (the flip that makes the monitor's `DONE_OK → decommission`
safe) → resume end-to-end → then enable the monitor schedule.

**Interim for the in-flight iter_0 run:** `run.sh` does not upload, so when `fabfd7fa` hits
`RUN_SH_DONE`, scp `/workspace/adapter_out` off the box BEFORE anything decommissions it.

**Confirmed 2026-06-04 (full chassis log).** The monitor worker's stuck row is the
reply-topic orphan (coordinator awaited `system.agent.generic.responses`; old dispatch
told the adapter to reply to `system.generic.responses`, the unconsumed CLI topic).
Fixed in `dispatch_thunder_ssh_get_status` (envelope reply-to → `execCtx.ResponsesTopic`).
A stale cached worker definition (old reaper-style no-op stub) also showed in the
message envelope while the full workflow executed — cleared by the redeploy that picks
up the fix. Net: redeploy → re-test → expect `last_probe_at` to update on the ALIVE path.

**VERIFIED 2026-06-04 18:21.** Monitor worker ALIVE path confirmed end-to-end after the
reply-topic fix (probe→adapter STATUS=ALIVE→classify→reset_streak→done; `last_probe_at`
updated, no decommission). Still to exercise before enabling the schedule: terminal
verdicts (`mark_training_run_terminal`→decommission), the unreachable/lost path, and the
orchestrator's find+spawn+call loop — and the B2 adapter-upload (so a real DONE_OK does
not decommission away the adapter).
