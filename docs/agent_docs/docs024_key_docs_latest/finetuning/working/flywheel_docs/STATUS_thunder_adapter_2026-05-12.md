...
Live status for the Thunder Compute adapter build. Phase 2 (skeleton) is verified in production. Phase 3 (lifecycle actions) is in flight — sub-phases 3.0–3.4 delivered as code, 3.5 (reaper) pending.
...
| **3.4** | `decommission_instance` action handler. Lookup by `provisioning_id` (DB UUID) or `thunder_identifier` (numeric) → atomic state transition to `decommissioning` (idempotency anchor) → Thunder API delete → Secret delete → compute cost from `running_since` → mark `decommissioned` | ✅ Delivered |
| **3.5** | thunder-reaper scheduled task. Runs every 15 min. Pre-query finds `running` instances older than `max_uptime_hours`. Dispatches `decommission_instance` for each. | ⏳ Pending |
...
### Within Phase 3
- **3.5 thunder-reaper** scheduled task. Brief design:
  - Pre-query: `SELECT id::text AS provisioning_id, thunder_instance_id FROM thunder_instances WHERE status='running' AND running_since IS NOT NULL AND running_since < NOW() - (max_uptime_hours||' hours')::interval ORDER BY running_since LIMIT 10`
  - Dispatch: emit `decommission_instance` to `system.adapter.thunder.requests` with `provisioning_id` and `reason='reaper:max_uptime_exceeded'`
  - Lives as a `scheduled_tasks` row, run every 15 min by kafka-scheduler.

### Verification-blocked (do after first real provision works)
- `api/types.go` has three TODOs flagged for verification on first real call: `ListInstances` response wrapper shape, `Instance` JSON field names, instance status string casing. All testable with `kubectl logs` after step 2 above — JSON decode errors will be obvious.
...
- Stubs for `gpu-provisioner` and `training-launcher` agent definitions (migrations 022 and 023) still return mock data. After thunder-adapter is verified, those should be replaced with real implementations that dispatch to the adapter via Kafka.
...
2026-05-12 — Phase 3.4 (decommission action) delivered. Ready to deploy 3.0+3.1+3.2+3.3+3.4 as a single bundle, verify with the manual sequence in §5, then proceed to 3.5 (reaper).
