...
> **UPDATED 2026-06-08:** the pre-register fix below is now **APPLIED** in
> `thunder_prepare_object_url_dispatch.go` (register-before-send). It has NOT yet been
> built/deployed/verified. Remaining work: `go build`/`go vet`, rebuild + roll the chassis
> (bump from `v1.0.1057`), cleanup, and the verify re-run. See "FIX" and "Done + verified"
> sections below, now marked accordingly.
...
## CURRENT BLOCKER (FIX APPLIED 2026-06-08, pending build + verify): loop-await race
...
## FIX (non-framework) — APPLIED 2026-06-08 in `thunder_prepare_object_url_dispatch.go`
Made our dispatch pre-register the awaited request BEFORE producing the adapter request,
...
```
For our dispatch: `requestID` = `newRequestID` (the id the dispatch already generates and uses in
the envelope header AND returns in `result["request_id"]`); `targetAgentType` =
`"thunder-adapter"`; `targetAgentID` = `""` (matches `createAwaitedRequest`'s `extractTargetAgentID`,
our result has no agent_id key); `requestsTopic` = `thunderAdapterTopic`; `responsesTopic` =
`myResponsesTopic` (the await-responses topic the dispatch already computes). **Arg order is
`(requestsTopic, responsesTopic)` — requests first.**

What was confirmed before applying (all [verified-source] against the extract):
1. **`preRegisterAwaitedRequest` body (L57855).** Sets `step_name = params.CurrentStep`,
   `step_id = params.ExecutionContext.StepID`, `retry_version=0`, `reply_to_request_id=requestID`,
   `timeout_at = now+120s` (hardcoded), `status='waiting'`, `ON CONFLICT (request_id) DO NOTHING`.
   It derives step from `params` — no explicit step fields needed.
2. **`params.CurrentStep` is the expanded loop-substep name at dispatch time.** `buildActionParams`
   sets `ActionParams.CurrentStep = state.CurrentStep`; loop expansion sets `state.CurrentStep` to
   the injected iteration step. `createAwaitedRequest` (the coordinator's current path) ALSO writes
   `state.CurrentStep`, so the pre-register writes the SAME `step_name` — no step regression; the
   resume (`state.CurrentStep = awaited.StepName`) targets the right substep.
3. **Id consistency.** Because `newRequestID` is reused everywhere, `extractRequestID(result, …)`
   returns it and the coordinator's later `InsertAwaitedRequest` hits `RowsAffected==0` →
   "already exists" → `processAwaitResponse` treats it as success and does NOT start a second
   timeout goroutine (`if !alreadyExisted`). One row, one timeout owner. The current ~3-min
   infinite re-dispatch (that skipped goroutine firing on an unclaimable row) is gone; the
   background expiry sweep remains the safety net.

The applied edit (just before `ProduceWithValidation`, guarded `if params.DB != nil`):
```go
if params.DB != nil {
    if err := preRegisterAwaitedRequest(ctx, params, newRequestID,
        ...
        params.Logger.Warn("Failed to pre-register awaited request before prepare_object_url send - response matching may race",
            zap.String("request_id", newRequestID), zap.Error(err))
        // Continue: race mitigation, not a hard requirement (same posture as spawn).
    }
}
```
Keeps returning `await_response:true`. No variable renamed; no import added (`preRegisterAwaitedRequest`
is in `spawn_actions.go`, same `actions` package). Affects ALL presign dispatches
(dataset/scripts/loop/final/resume) — they all become register-before-send; the working ones get
more robust, the loop stops racing.

REMAINING (build + verify):
1. `gofmt` + `go build ./...` + `go vet` in-repo.
2. Rebuild + redeploy the chassis (bump from `v1.0.1057`). The `training-launcher` runs the chassis image.
3. Cleanup (see CLEANUP section): kill stuck launcher jobs, confirm no live `thunder_instances`.
4. Re-run (export `146a9a12` or `fef7be6b`). CONFIRM in the launcher pod: each
   `presign_checkpoints_iter_N_presign_one` should now log `Pre-registered awaited request in
   database` then `ClaimAwaitedRequest: status_before=waiting … claimed:true`, the loop should run
   all 40 iterations → `flatten_checkpoint_urls` → `MANIFEST_WRITTEN` → `ssh_exec_launch`.
...
- exports (`training_exports.runs`): `146a9a12` (1958 ✓), `fef7be6b` (1958 ✓), `a8484922`
  (0 ✗ — do not use).
- outputs: `109` / `109a` / `109b` .sql; **`thunder_prepare_object_url_dispatch.go`** (fix
  APPLIED 2026-06-08 — register-before-send); the 3 Phase B actions (`compute_checkpoint_keys_action.go`,
