    ...
    jsonb_set(
        ...
        (default_config #> '{workflow,steps}') || $newsteps$
        {
              ...
              "sub_workflow": {
                ...
                "steps": {
                  "presign_one": {
                    ...
                    "description": "Presign a PUT for this iteration key (ckpt_key via input_mapping, live in loop substeps). expiry_minutes overrides the 24h PUT default (~50h covers the 48h cap).",
                    "config": { "method": "PUT", "expiry_minutes": 3000, "input_mapping": { "key": "ckpt_key" } },
          ...
          "write_manifest": {
            ...
            "config": {
              "command_template": "mkdir -p /workspace && echo '{manifest_b64}' | base64 -d > /workspace/upload_manifest.json && echo MANIFEST_WRITTEN",
