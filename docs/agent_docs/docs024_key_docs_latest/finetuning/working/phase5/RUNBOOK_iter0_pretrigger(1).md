...
Confirms three things at once without provisioning a VM: the deployed adapter
actually has `prepare_object_url` (not `not_implemented`), the reply key is
`presigned_url` (what the launcher tokens resolve), and the bundle is fetchable
at the B2 key. This is a manual produce + manual consume — it does NOT depend on
the chassis await matching, so it isolates the adapter from the topic question.

Envelope shape (verified against adapter.go handleMessage): the adapter reads
`action` and `reply_to_topic` from the **value's `body`** (`body =
envelope["body"]`), and `correlation_id`/`request_id` from the **Kafka `-H`
headers**. So `action`, `reply_to_topic`, and the B2 `key` all go inside `body`.
(Same shape as the `prepare_dataset_url` test; the only difference is `key`
instead of `export_id`.)

```
# Terminal 1 — start the reply consumer FIRST (so it's listening before we send):
...
Expect, on Terminal 1, a response whose `body` has
`presigned_url`/`key:"finetuning/scripts/bundle.tar.gz"`/`method:"GET"`. Then
confirm the object is really there and the presign is valid:

```
curl -sI "<the presigned_url from the reply>" | head -1     # expect HTTP/... 200
```

Failure reads:
- reply `status:"error"` `code:"not_implemented"` → the deployed adapter image
  doesn't carry `prepare_object_url` (the v1.0.1049 build predates the change);
  rebuild/redeploy the adapter.
- reply `presigned_url` present but `curl -I` → 404 → bundle not at that key, or
  wrong bucket. Re-check the upload (below).
- no reply at all → the adapter didn't see `reply_to_topic` (it must be in
  `body`, not only a `-H` header) or the request never reached the adapter topic.

Bundle-presence cross-check (independent of the adapter), per `UPLOAD_bundle.sh`:

```
b2 ls --long personae-model-training finetuning/scripts/   # expect a bundle.tar.gz row
```
