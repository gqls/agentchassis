...
> **NOTE (2026-06-09): `presign_checkpoints` is being replaced by a single batch step.**
> The 2026-06-09 run confirmed the await-race fix but showed the K-iteration presign loop
> slows O(K²) (state re-persist per iteration) and won't finish at K=40 (see 4d and the debug
> guide). The fix is one `prepare_object_urls` adapter call replacing the loop + `flatten`.
> Once that lands, the expected log shape below changes: `compute_checkpoint_keys` → ONE
> `prepare_object_urls` reply (all K urls) → `assemble_upload_manifest` → `write_manifest` →
> `ssh_exec_launch` (no per-iteration `presign_checkpoints_iter_N`, no `flatten`).
...
```
Expect (loop version, being retired), in order: `compute_checkpoint_keys` (built K keys), a
burst of `prepare_object_url` presigns (the loop, one per key) + the final presign,
