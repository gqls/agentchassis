...
- **Safeguard (corrected — migrations are hand-applied).** There is NO migration runner/directory; 109/110/111
  are applied by hand in psql, so nothing auto-reverts them — the revert was a manual re-run of 109 per the
  runbook's old "safe to re-run" line. Safeguards: the corrected runbook wording (Step 2b), the 2d state check
  after any def change, and never re-running an earlier migration (check state, apply only what's missing).
  Optional future hardening: a migration runner or a tiny applied_migrations log to enforce order.
...
### Update — 2026-06-14 (8): provisioning blocked — thunder-adapter image is the WRONG binary.
- **Symptom.** Triggered run (export fef7be6b, corr ce95c29a) parked: `model_lifecycle.training_runs`
  row e3165182 status=`pending`, no `thunder_instance_id`; zero `thunder_instances` rows; `tnr status`
  empty 85 min later. So model-trainer created the run but no GPU box was ever provisioned.
- **Root cause.** `thunder-adapter` pod in CrashLoopBackOff (~31h, 374 restarts) on
  `exec: "./thunder-adapter": stat ./thunder-adapter: no such file or directory`. Confirmed by
  inspecting the image: `thunder-adapter:v1.0.1063` actually contains the **analyser-adapter** binary
  (`/app` has `analyser-adapter`, entrypoint `./analyser-adapter -config configs/analyser-adapter.yaml`),
  and BOTH `aqls/thunder-adapter` and `aqls/analyser-adapter` resolve to the same
  `sha256:471df7…`. Cause: the `thunder-adapter.dockerfile` was overwritten with the analyser-adapter
  Dockerfile content, so the build produced/pushed the analyser binary under the thunder tag.
  Provisioning routes through the adapter (`system.adapter.thunder.requests`), so with the adapter down
  every provision/presign/ssh/decommission request goes unanswered → runs park at `pending`.
- **Not the chassis.** `agent-chassis` is 1/1 Running, 31h, no restarts — so the `configOrInput` fix and
  the resume dispatch (both chassis-side) are LIVE; migrations 110/111 are applied + 2d-verified. The
  ONLY broken thing is the adapter image. The adapter-side handlers needed (`prepare_object_urls`,
  `prepare_resume_url`) were already in the adapter before this bad push, so restoring the correct
  thunder-adapter binary is the whole fix — no work is lost.
- **Fix delivered.** Corrected `thunder-adapter.dockerfile` (same template as the sibling
  analyser/web-search/agent-chassis Dockerfiles, every name set to thunder-adapter:
  `go build -o thunder-adapter ./cmd/thunder-adapter`, `COPY … thunder-adapter`,
  `COPY configs/thunder-adapter.yaml`, `CMD ["./thunder-adapter","-config","configs/thunder-adapter.yaml"]`).
- **Recovery steps (pending).** (1) confirm `ls cmd/thunder-adapter/` exists; (2) build; (3) BEFORE
  pushing, prove the binary is in the image: `docker run --rm --entrypoint ls …:v1.0.1064 -la /app`
  (expect a `thunder-adapter` file, not `analyser-adapter`); (4) push a FRESH tag `v1.0.1064` (NOT a
  re-push of 1063 — its digest is the poisoned analyser image); (5)
  `kubectl -n ai-persona-system set image deploy/thunder-adapter thunder-adapter=…:v1.0.1064` + rollout,
  confirm 1/1; (6) mark e3165182 `failed` (it won't auto-resume); (7) fire a fresh trigger.
- **On that first good run, confirm:** `check_resume` found=false on a fresh id; PUT URLs at
  `X-Amz-Expires=180000`; one ~2s batch presign.
- **Pattern (third deploy-regression in a row).** 109 re-run, chassis/adapter tag confusion, now the
  Dockerfile overwrite — all "the deploy didn't ship what I thought." No guard between built and running.
  The cheap per-build guard is the `docker run … ls /app` check above; structural version = a CI step
  that fails the adapter build if `/app/thunder-adapter` is absent. (Same spirit as the migration 2d
  check.) Logged in debug guide v2_47.
