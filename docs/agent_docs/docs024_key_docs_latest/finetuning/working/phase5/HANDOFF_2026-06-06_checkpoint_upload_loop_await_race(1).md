...
Steps:
1. **`preRegisterAwaitedRequest` body is now confirmed (L57855).** It does:
   ...
   `retry_version = 0`, `reply_to_request_id = requestID`, and `timeout_at = now + 120s`
   (hardcoded). Implications for our use:
   - The `ON CONFLICT DO NOTHING` makes double-registration safe — the coordinator's later
     `processAwaitResponse` → `InsertAwaitedRequest` no-ops (and its Go path already handles
     "already exists" at L1780). So pre-registering then returning `await_response:true` is fine.
   - The **120s timeout is hardcoded** and, because of `ON CONFLICT DO NOTHING`, it wins over the
     step's configured timeout once pre-registered. For presigns (reply ~1s) 120s is ample — fine
     here — but note it pins the await timeout to 120s for every presign dispatch.
   - It uses `params.CurrentStep` for `step_name`. The dispatch MUST have `params.CurrentStep`
     holding the expanded loop-substep name (`presign_checkpoints_iter_N_presign_one`) so
     `handleCompleteResponse` resumes the right step. Verify `ActionParams.CurrentStep` at dispatch
     time; if it's not the expanded name, the resume will target the wrong step.
