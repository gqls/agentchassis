...
(4b) NEXT — the `thunder-training-monitor` ORCHESTRATOR definition + scheduler + deployment wiring. Orchestrator workflow: `find_active_training_instances → loop(iterate_over:"find...instances", loop_var) → done`, loop substeps = `spawn_agent`(type=thunder-training-monitor-worker, role) + `call_agent`(target_role, input_mapping {provisioning_id:"loop_item.provisioning_id", training_run_id:"loop_item.training_run_id"}). Spawn→call is the proven two-step (call_agent finds the spawned child by role, prefers current loop iteration). Then: a scheduler entry to fire it every ~5–10 min (mirror how thunder-reaper is scheduled), AND — the open wiring question — the chassis deployment must consume the new agent types' process topics (`system.agent.thunder-training-monitor[-worker].process`); to be checked against deployment.yaml/service.yaml/kustomization.yaml + how the existing Phase 5 types are registered there. Reuses `dispatch_thunder_ssh_get_status`(written) + `dispatch_thunder_decommission`(existing) + the new mark/streak actions. Static-only so far — `go build ./...` pending in-repo.
