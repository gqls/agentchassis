...
> **CURRENT STATUS (2026-06-14).** The **chassis is deployed and healthy** (`agent-chassis` 1/1, no
> restarts) — so the configOrInput numeric fix + the resume dispatch are live, and migrations 110
> (batch presign) + 111 (resume wiring) are applied and pass the 2d state check. The **one blocker is
> the `thunder-adapter` image**: `v1.0.1063` was built from an overwritten Dockerfile and contains the
> *analyser-adapter* binary, so the pod CrashLoopBackOffs (`exec: "./thunder-adapter": no such file or
> directory`) and provisioning parks every run at `pending`. Rebuild the adapter from the corrected
> Dockerfile, push a FRESH tag, roll — see **4f** and debug guide v2_47 — then resume the test flow at
> Step 4 (the 2d migration check has already passed). After that the outstanding milestone is the first
> run reaching `RUN_SH_DONE` with the final adapter durable (4e), which gates the monitor (Step 6).
