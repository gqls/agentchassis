...
## What this is (read first, especially if you're returning cold)

This is the **GPU fine-tuning pipeline** of the agent-orchestration platform — the part that turns the
platform's own accumulated work into a better model. The platform logs every successful LLM call; a
**training export** freezes a filtered snapshot of those calls as ChatML training rows (the
`training_exports` schema). A **training run** then fine-tunes a base model (Llama 3.3 70B, LoRA) on one
export, on a GPU box rented from **Thunder Compute**, and lands the trained **LoRA adapter** in
Backblaze **B2** object storage.

One run, end to end: a Kafka `orchestrate` message starts **model-trainer**, which creates a
`model_lifecycle.training_runs` row and streams the chosen export to a B2 dataset JSONL
(`finetuning/datasets/<export_id>/training.jsonl`). It then provisions a GPU via **gpu-provisioner**
(rents the Thunder box, stores the SSH key as a k8s Secret, returns a `provisioning_id`), and hands off
to **training-launcher** — the focus of this runbook. The launcher asks the **thunder-adapter** (the B2
credential boundary) to presign URLs: GET for the dataset + scripts bundle, PUT for each checkpoint and
the final adapter. It assembles `upload_manifest.json` from those, SSH-writes it onto the box, then
SSH-launches `run.sh` detached (`setup → smoke → full train`). `run.sh` uploads each checkpoint and the
final adapter to B2 through the presigned PUTs and prints **`RUN_SH_DONE`** only on durable success
(the final upload is a hard gate). A periodic **thunder-training-monitor** probes each running box over
SSH and **decommissions** it (destroy VM + compute cost + flip the run row) once training is done or the
box is dead — that's the cost-control loop. **Resume** wiring (`check_resume`) lets a re-run of the
*same* `training_run_id` on a fresh box continue from the latest checkpoint in B2.

Mechanics: every agent is a chassis pod (`docker.io/aqls/agent-chassis`) reading/writing Postgres
(`clients_db`; relevant schemas: `public`, `model_lifecycle`, `training_exports`, and the Thunder
tables) and passing messages over Kafka (cluster `personae-kafka-cluster`, namespace `kafka`; agents in
`ai-persona-system`). **Agent-def changes are hand-applied SQL migrations — there is no migration runner
or ledger.** That's why ordering matters and why you must run the migration state checks (Step 2d)
before trusting the def; never re-run an earlier migration to "make sure."

This runbook deploys and tests, in order: **B** (manifest + presign), **C** (`RUN_SH_DONE` means
durable), **D** (resume), and finally enabling the monitor.
...
Deploy + wiring — **done:**
- [x] Chassis built + deployed: Phase B actions, batch + resume dispatches, registry entries, the
      2026-06-08 register-before-send race fix, the configOrInput numeric-coercion fix. Healthy (1/1, no restarts).
- [x] Migration `109` applied (manifest-wiring chain).
- [x] Migration `110` applied (batch presign; retired the per-checkpoint loop + `flatten`). 2d: `m110_batch_ok = t`.
- [x] Migration `111` applied (resume `check_resume`). 2d: `m111_resume_ok = t`.
- [x] **2d migration state check passes** (run it again after any further deploy).
- [x] **thunder-adapter image fixed** — was shipping the analyser-adapter binary; rebuilt from the corrected
      Dockerfile, fresh tag, **live in prod and healthy (no CrashLoop)** as of 2026-06-14.
- [x] Upload path proven once: `ckpt-0.tar.gz` in B2 on run `0ac806ab` (pre-adapter-fix image).

Test flow — **next:**
- [ ] Mark the stale parked run `e3165182` `failed` (it won't auto-resume).
- [ ] **Fire a fresh training run** (the trigger below / Step 4b).
- [ ] 4c/4d: launcher trace clean + `upload_manifest.json` on the box. Confirm: `check_resume` → `found=false`
      on the fresh id; PUT URLs at `X-Amz-Expires=180000`; one ~2s batch presign (not the old O(K²) loop).
- [ ] **4e — the milestone:** run reaches `RUN_SH_DONE` and the final `adapter.tar.gz` is durable in
      `finetuning/artefacts/<run_id>/`. (This is the empirical gate for the monitor.)
- [ ] Step 5 — resume: kill mid-run, relaunch the **same** `training_run_id`, confirm `found=true` + resume.
- [ ] Step 6 — enable the `thunder-training-monitor` schedule (gated on 4e; terminal branch never fired live).
...
> **CURRENT STATUS (2026-06-14) — unblocked, ready to fire a run.** The chassis is healthy (1/1, no
> restarts), migrations 110+111 are applied and pass the 2d check, and the **`thunder-adapter` image is
> now fixed and live** (the earlier `v1.0.1063` shipped the analyser-adapter binary and CrashLoopBackOff'd;
> the rebuilt image with the real `thunder-adapter` binary is running with no errors). So provisioning →
> presign → manifest → launch is clear end to end. **Where we are: pick up at Step 4** — mark the stale
> parked run failed, then fire a fresh run with the trigger below. Outstanding milestone after that: a run
> reaching `RUN_SH_DONE` with the final adapter durable (4e), which gates the monitor (Step 6).

> **NEXT ACTION — mark the stale run failed, then fire a fresh run.** The parked run from the
> adapter-down period (`e3165182`, status `pending`, no box) will not auto-resume; mark it so it isn't
> mistaken for live, then trigger a clean run (fresh correlation). Full annotated trigger is in **Step 4b**;
> this is the ready-to-run form (same export `fef7be6b`, includes the `TIMESTAMP`):
>
> ```sql
> -- in clients_db: retire the parked run (failed is in the CHECK set; no box/secret to clean)
> UPDATE model_lifecycle.training_runs
>    SET status='failed',
>        error_message='adapter image v1.0.1063 crash-looping (analyser binary); no box provisioned'
>  WHERE id='e3165182-ddf6-4b7b-990e-6460290a87a0' AND status='pending';
> ```
>
> ```bash
...
> echo "CORRELATION=$CORRELATION  ORCH=$ORCH  REQ=$REQ  TIMESTAMP=$TIMESTAMP  (write these down — grep logs by CORRELATION; TIMESTAMP bounds the 4f DB queries)"
>
...
> {"action":"orchestrate","config":{"agent_type":"model-trainer"},"input_data":{"export_id":"fef7be6b-887f-4bc9-b118-a5a9992c4179","hyperparameters":{"epochs":1,"batch":1,"grad_accum":8,"lr":0.0002,"lora_r":16,"lora_alpha":16,"max_seq":4096}}}
> JSON
> ```
>
> Then watch it via 4c (chassis logs by CORRELATION) → 4d (manifest on the box) → 4e (B2 durability). If no
> box appears in ~5 min, go to **4f** (it should not, now the adapter is healthy — but 4f is the ladder).

## Staging principle (why the order matters)
Prove the **upload path in isolation first** (steps 2–4), then enable the monitor (step
6). The **resume test (step 5) is deliberately last**: resume reads the checkpoints the upload
path writes, so it tells you nothing until the upload path is
...
```
> ⚠️ **"Idempotent" here means safe to run ONCE / in isolation — NOT safe to re-run after `110`/`111`.**
> `109` rebuilds `presign_checkpoints` as a `loop` and recreates `flatten_checkpoint_urls`. `110`
> later REPLACES that loop with the batch step and DELETES `flatten`; `111` chains `check_resume`
> off `presign_final`. So re-running `109` on a def that already has `110`/`111` **silently reverts
> both** (back to the O(K²) loop, no resume) — a migration is only idempotent against its own prior
> application, never against *later* migrations that mutate the same object. Before re-running ANY
> migration to "make sure," run the **Migration state check (2d)** instead and re-apply only what's
> actually missing. (This bit us 2026-06-09: 109 was re-run per this runbook's old wording and
> reverted 110+111 — see debug guide v2_45.)

### 2d. Migration state check — run after ANY deploy, before launching
These launcher-def migrations (`109`/`110`/`111`) are hand-applied `jsonb` mutations to the
`training-launcher` def; they are NOT recorded in a `schema_migrations` ledger, so the **live def
shape is the only source of truth** for "did they run." This one query gives a per-migration verdict:
```sql
WITH d AS (
...
SELECT
  ( steps ? 'presign_checkpoints'
    AND steps #>> '{presign_checkpoints,action}' = 'dispatch_thunder_prepare_object_urls'
    AND NOT (steps ? 'flatten_checkpoint_urls') )                            AS m110_batch_ok,
  ( steps ? 'check_resume'
    AND steps #>> '{presign_final,next_step}' = 'check_resume'
    AND steps #>> '{assemble_manifest,config,resume_url}' = 'resume_probe.presigned_url'
    AND steps #>> '{assemble_manifest,config,checkpoint_urls}' = 'ckpt_presign_batch.presigned_urls'
  )                                                                          AS m111_resume_ok,
  steps #>> '{presign_checkpoints,action}'   AS presign_checkpoints_action,  -- batch: dispatch_thunder_prepare_object_urls ; reverted: loop
  (steps ? 'flatten_checkpoint_urls')        AS flatten_present_BAD,         -- must be FALSE post-110
  steps #>> '{presign_final,next_step}'      AS presign_final_next           -- post-111: check_resume
  FROM d;
```
**Pass (110 + 111 both applied):** `m110_batch_ok = t`, `m111_resume_ok = t`,
`presign_checkpoints_action = dispatch_thunder_prepare_object_urls`, `flatten_present_BAD = f`,
`presign_final_next = check_resume`. If `m110_batch_ok = f` / `action = loop` / `flatten_present_BAD = t`,
the def is on the reverted 109 chain — re-apply `110` then `111` (both re-snapshot + guard, safe to
re-run in that order on top of the reverted state). Do NOT launch until this passes.
...
CLIENT_ID="demo_client"
TIMESTAMP=$(date -u +"%Y-%m-%dT%H:%M:%SZ")
echo "CORRELATION=$CORRELATION  ORCH=$ORCH  REQ=$REQ  TIMESTAMP=$TIMESTAMP  (write these down — grep the logs by CORRELATION; TIMESTAMP bounds the DB diagnostic queries)"
...
- **`export_id`** (the `<VALID_EXPORT_ID>` placeholder above) — choose it via the
  ...
  - *What it is / why an old one is fine:* an export is an **immutable snapshot** of ChatML training
    rows harvested from `llm_call_log` into `training_exports` (one `runs` row = the `export_id`, many
    `rows`). Its rows never change once written, and the dataset is deterministically keyed by
    `export_id` (`finetuning/datasets/<export_id>/training.jsonl`, re-materialised identically on each
    run). For pipeline tests we only need *a* valid, non-empty export — the data's vintage is
    irrelevant — so reusing a known-good old one (e.g. `fef7be6b`, `146a9a12`) is cheap and
    reproducible. Cut a fresh export only when you care about the model's quality, not the plumbing.
...
### 4f. No GPU instance after a launch — diagnose (provisioning routes through the adapter)

If `tnr status` is empty several minutes after a launch (a real provision is ~3–5 min; if a box were
training it would show), walk this ladder. Don't conclude from `tnr` alone, and remember a `0 rows`
result can be the query's fault (wrong time bound / schema) — sanity-check before deciding. Bound the
queries by the `TIMESTAMP` you captured at launch.

**(i) DB trio — how far did it get?**
```sql
-- run row: created but never provisioned (pending, no instance id) / failed (+error_message)?
SELECT id, export_id, status, error_message, orchestration_id, thunder_instance_id, created_at, started_at
  FROM model_lifecycle.training_runs WHERE created_at > '<TIMESTAMP minus a margin>' ORDER BY created_at DESC;
-- any box at all (incl. failed/lost/decommissioned)?
SELECT id, thunder_instance_id, status, training_run_id, error_message,
       provisioned_at, running_since, decommissioned_at, reaped_at, reaped_reason, created_at
  FROM thunder_instances WHERE created_at > '<TIMESTAMP minus a margin>' ORDER BY created_at DESC;
-- did the orchestration even run? (NB: orchestration_states has NO agent_type column — use owner_agent_type)
SELECT orchestration_id, owner_agent_type, status, current_step, error, created_at, updated_at
  FROM orchestration_states WHERE correlation_id = '<CORRELATION>' ORDER BY created_at;
```
Read: run `pending` + no instance ⇒ never got past provisioning. Run `failed` + `error_message` ⇒ read it.
A `thunder_instances` row with `status=failed/lost` + `error_message` ⇒ provision/box failure (most
informative). No `thunder_instances` row ⇒ provisioning never ran. No `orchestration_states` row ⇒ the
trigger message wasn't consumed (delivery problem, not logic).

**(ii) If no box and the run is stuck `pending`: check the adapter pod** — provisioning, presign, ssh,
and decommission all route through `thunder-adapter` on `system.adapter.thunder.requests`, so a dead
adapter parks every run at `pending`.
```bash
kubectl -n ai-persona-system get pods | grep -E 'thunder-adapter|chassis|provisioner|model-trainer'
kubectl -n ai-persona-system describe pod <thunder-adapter-pod>   # look at Last State / Reason
```
`CrashLoopBackOff` with `exec: "./thunder-adapter": no such file or directory` ⇒ the image lacks the
binary (wrong build context / overwritten Dockerfile / shared tag). Confirm by inspecting the image and
fix per debug guide v2_47:
```bash
docker run --rm --entrypoint ls docker.io/aqls/thunder-adapter:<tag> -la /app          # expect a thunder-adapter file
docker inspect docker.io/aqls/thunder-adapter:<tag> --format '{{index .RepoDigests}}'  # tag-collision tell (two services, one sha256)
```
Fix: restore the thunder-adapter Dockerfile (all names = thunder-adapter, `./cmd/thunder-adapter`),
rebuild, `ls /app` to prove the binary is present, push a **FRESH tag** (never re-push the poisoned one),
`set image` + rollout, confirm `1/1`. Then mark the parked run `failed` (it won't auto-resume) and fire a
fresh trigger.
...
- **After ANY deploy, run the Migration state check (2d) before launching** — `109`/`110`/`111` are
  hand-applied def mutations with no ledger, and re-running an EARLIER one reverts later ones.
- `109` (chain), `110` (batch presign — replaces the loop+flatten), and `111` (resume
  ...
  Never re-run `109` to "make sure" — it reverts `110`+`111`.
