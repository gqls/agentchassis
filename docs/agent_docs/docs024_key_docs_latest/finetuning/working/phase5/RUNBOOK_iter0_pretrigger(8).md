...
## 2b. Data-path verification (input_mapping ↔ producer result shape)

Every `call_agent` step's `input_mapping` reads the *actual* shape of the producing
step's result in `collected_data` — not the shape you'd infer from the child's
return type or description. They diverge whenever the child's `complete` emits its
result under a different key than expected (debugging guide gotcha #16 — singular
`output_field` is ignored; the result falls back to step-name keys). A wrong path
fails at extraction time *after* the provisioner has booked a real a100 — so verify
before firing, not after.

For each `call_*` step, confirm the mapped source paths resolve against the
producing step's real output. Read what the step maps:

```
… psql -d clients_db -c \
"SELECT jsonb_pretty(default_config #> '{workflow,steps,call_launcher,config,input_mapping}') \
 FROM agent_definitions WHERE id='94f5a069-6fb5-4aba-81e5-4fcc9220ed30';"
```

then confirm each `provisioning_result.*` / `preparation_result.*` path exists in the
producer's real result — cheapest from a prior run's persisted state or trace:

```
… psql -d clients_db -c \
"SELECT jsonb_pretty(collected_data #> '{provisioning_result,response}') \
 FROM orchestration_states WHERE orchestration_id='<orch>';"
# or: grep -o '"provisioning_id":"[^"]*"' /var/log/agent-traces/trace_<corr>*.json
```

If a field sits under a step-name key (`provisioning_result.dispatch_provision.X`)
rather than at `provisioning_result.X`, the mapping must name that key — the resolver
auto-unwraps `.response` but does not cross step-name keys. Migration 104 made exactly
this correction; re-verify after any change to the provisioner's `complete.output_fields`.
...
```
# (build the bundle FLAT from the script dir — every file at the archive root)
tar -czf bundle.tar.gz run.sh 00_vm_setup.sh 02_train_llama_3_3_70b.py 03_inference_test.py

# verify FLAT before upload — expect exactly these 4 names, NO leading ./ or dir/:
...
# upload to the EXACT key the launcher's presign_scripts expects (overwrites in place)
b2 file upload personae-model-training bundle.tar.gz finetuning/scripts/bundle.tar.gz
# the upload prints a JSON receipt — confirm "fileName" ends /bundle.tar.gz and "size"
# matches `ls -l bundle.tar.gz` (a flat 4-file bundle is ~7.5k / 7548 bytes).

# confirm it landed (this b2 CLI wants a b2:// URI, not two bare args):
b2 ls --long "b2://personae-model-training/finetuning/scripts/"   # expect a bundle.tar.gz row
```

To update the launch chain later, edit `run.sh` / `00_vm_setup.sh` / the train
script, rebuild FLAT, re-upload to the SAME key — that re-upload is the ENTIRE
deploy for a bundle change (no DB migration, no chassis redeploy; the agent def
only holds the key, which is unchanged). **A bundle fix only reaches FUTURE
provisions** — a VM that already launched curled the old bundle, so to fix a
*running* box either `scp` the changed file onto it (`/workspace/…`) and re-run
`run.sh`, or re-pull a freshly-presigned bundle first. Common rebuild mistake: a
non-flat archive (a wrapping dir), which makes `run.sh`'s `/workspace/00_vm_setup.sh`
paths miss after extraction — the `tar -tzf` check above catches it.
...
Only once the provision await **resolves** — the adapter's reply goes to the
agent's OWN `ResponsesTopic` (for a standalone generic-worker fire that's
`system.agent.generic.responses`, NOT `__parent_responses_topic__`), and the
`awaited` row flips `waiting → processed` — is it worth triggering iter_0. The
exact topic string depends on which worker runs the agent; what matters is
own-topic (not parent) and that the await clears. (Confirmed 2026-06-02: reply to
`system.agent.generic.responses`, await resolved in 42s.)

## 6. Trigger iter_0

Only after: §5b passed (D4 confirmed), the test instance decommissioned, and the
cost gate settled for a ~$18 run (`estimated_new_run_cost_usd` realistic,
`daily_cap_usd` raised; re-check `can_provision`).

Trigger `model-trainer` (same orchestrate envelope as §5b, agent_type
`model-trainer`). The preparer requires `input_data.hyperparameters` present
(it errors if absent); run.sh uses the train script's defaults (3 epochs etc.),
so the values are recorded in `training_runs` but the run uses defaults that
already match iter_0.
...
The orchestrator chain: `spawn_data_preparer → spawn_provisioner → spawn_launcher
→ call_data_preparer → call_provisioner → call_launcher → complete`.

What to watch, in order (the spawned-child seeding is what's still unproven, §5a):
- preparer: dataset uploaded, `training_runs` row INSERTed `pending`. (If it errors
  `hyperparameters not found`, model-trainer's `call_data_preparer` mapping isn't
  threading `input_data.hyperparameters` to the preparer — fix the mapping.)
- provisioner: 3–5 min, returns `provisioning_id` (a fresh instance, this time
  `attached_to_training=true` → 18h reaper, covering the ~9h train).
- **launcher** — the real D4 test for a spawned child: its dispatch log
  `await_responses_topic` should be a topic the chassis consumes, and it should
  step `presign_dataset → presign_scripts → ssh_exec_launch → mark_running →
  complete`, each dispatching to thunder-adapter and resuming on the reply.
  Advancing past `presign_dataset` confirms spawned-child seeding works.
- then the VM markers (§7).
...
On the VM directly — the surest signal, since the orchestration can't see the
detached run. Pull the per-instance SSH key from its secret, then tail the logs.
`scp` uses `-P`, `ssh` uses `-p`; `-o StrictHostKeyChecking=no -o UserKnownHostsFile=/dev/null`
is needed because Thunder recycles IPs, so `known_hosts` goes stale between instances.

```
# key from the per-instance secret (field private_key, base64); chmod 600.
# secret name is thunder-ssh-<provisioning_id>; ip:port from thunder_instances.
SECRET=thunder-ssh-<provisioning_id>
kubectl -n ai-persona-system get secret "$SECRET" -o jsonpath='{.data.private_key}' \
  | base64 -d > /tmp/k && chmod 600 /tmp/k
# (if {.data.private_key} is empty: `… -o jsonpath='{.data}'; echo` and use the right key name)

# is run.sh / training actually running, and what does the log say?
ssh -i /tmp/k -p <port> -o StrictHostKeyChecking=no -o UserKnownHostsFile=/dev/null \
  ubuntu@<ip> 'pgrep -af "run.sh|02_train|[p]ython"; echo ---; tail -n 8 /workspace/train.log'

# live-watch the markers
ssh -i /tmp/k -p <port> -o StrictHostKeyChecking=no -o UserKnownHostsFile=/dev/null \
  ubuntu@<ip> 'tail -f /workspace/train.log'
```

- `/workspace/launch.log` — curl/tar of bundle + dataset (empty/clean on success).
- `/workspace/train.log` markers, in order: `RUN_SH_START` → `RUN_SH_STEP step=setup`
  → `Setup complete.` → `RUN_SH_STEP step=smoke` → `RUN_SH_SMOKE_OK` →
  `RUN_SH_STEP step=full_train` → `RUN_SH_FULL_OK output=/workspace/adapter_out` →
  `RUN_SH_DONE`.

Salvaging an idle box without re-provisioning — if `train.log` is stuck at
`RUN_SH_STEP step=setup` and `run.sh` has already exited (pgrep shows nothing), a
best-effort setup step tripped `set -e`; read the last log line for the exact cause.
Setup is idempotent and the dataset+scripts are already in `/workspace`, so push the
fixed script onto the box and re-launch (this is what salvaged the iter_0 box on
2026-06-03 after the root-owned `.bashrc` abort):

```
scp -i /tmp/k -P <port> -o StrictHostKeyChecking=no -o UserKnownHostsFile=/dev/null \
  00_vm_setup.sh ubuntu@<ip>:/workspace/00_vm_setup.sh
ssh -i /tmp/k -p <port> -o StrictHostKeyChecking=no -o UserKnownHostsFile=/dev/null \
  ubuntu@<ip> \
...
train then runs **detached** outside any await window (`run.sh`'s header targets a
30–90 min full run for 1958 examples × 3 epochs — watch `train.log` for the
actual; comfortably inside the 18h reaper either way). There is no
training-monitor yet, so the `running → complete/failed` transition won't happen
automatically — watch `train.log` for `RUN_SH_FULL_OK`, and note the
artefact-upload path (`prepare_artefact_url` PUT) is still untested until
`adapter_out` exists.
