...
-- 0a. SNAPSHOT the current def BEFORE mutating, via the project's sanctioned
--     helper — NOT a hand-rolled table. agent_definitions_backup ALREADY EXISTS
--     as the snapshot table (it mirrors agent_definitions + snapshot_taken_at /
--     snapshot_reason / restored_at), written by snapshot_agent(). A
--     `CREATE TABLE IF NOT EXISTS agent_definitions_backup (...)` silently no-ops
--     against it, then an INSERT with invented columns fails ("column ... does
--     not exist"). Use the function (see debug guide §6.1):
--       snapshot_agent(p_agent_type text, p_reason text DEFAULT NULL) -> uuid
SELECT snapshot_agent('training-launcher', 'pre-migration-110 batch presign');

-- REVERT (if ever needed): revert_agent rolls the type back to its latest
-- unrestored snapshot. If no snapshot was taken it raises
-- "No unrestored snapshot found for type ...".
--   SELECT revert_agent('training-launcher');
