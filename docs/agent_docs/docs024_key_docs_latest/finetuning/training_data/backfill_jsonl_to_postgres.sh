...
EXPORT_ID=$(kubectl -n "$NAMESPACE" exec -i "$POD" -- \
    psql -U clients_user -d clients_db -tA <<EOF
INSERT INTO training_exports.runs
    (agent_type, step_name, model_filter, format, export_version,
     size_bytes, source_notes, rows_seen, rows_exported, completed_at)
VALUES (
    '$AGENT_TYPE', '$STEP_NAME', '$MODEL', 'chatml', '1',
    $FILE_SIZE,
    'Backfilled from JSONL retrieved via kubectl exec cat on $(date +%Y-%m-%d)',
    $LINE_COUNT, $LINE_COUNT, NOW()
)
RETURNING id::text;
EOF
)
...
EXPORT_ID=$(echo "$EXPORT_ID" | tr -d '[:space:]')
