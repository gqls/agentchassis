...
# Engine repo — STANDALONE, sibling of agentchassis (~/projects/site-engine),
# never nested inside it. Manual creation so YOU choose visibility; the
# git-adapter auto-creates repos as PUBLIC, so don't leave this to it.
cd ~/projects
gh repo create $OWNER/site-engine --private --clone && cd site-engine
cp $OUTPUTS/site-engine/* .          # go.mod, env.go, main.go, service.go, store.go
mkdir -p .github/workflows           # outputs ship workflows FLAT (no dot-dirs)
cp $OUTPUTS/vm-deploy/deploy-engine-to-vm.yml .github/workflows/
go vet ./... && go build -o /dev/null . && echo FILES-OK
...
```bash
# Provision the smallest Ubuntu 24.04 VM (idea.uk model: Hetzner CX23 (EU: Falkenstein/Nuremberg/Helsinki) is
...
ssh root@<IP> 'tail -3 /var/lib/site-engine/events-*.jsonl; cat /var/lib/site-engine/counters.json'
```
