...
- **Content** (VM-sites repo — `deploy-to-vm.yml`): on push to `sites/**`, rsync
  ...
  domains only.
- **Engine** (probe-go repo — TODO): build amd64 → ship binary to box →
  `setup.sh MODE=update` (swap + restart).

Onboarding a NEW domain = extend `DOMAINS` and re-run `setup.sh` (adds its nginx
vhost + cert); the content Action does not provision vhosts. The terminal build
item stays target-agnostic — it commits to the site's repo; the repo's Action
does the VM-specific work.
