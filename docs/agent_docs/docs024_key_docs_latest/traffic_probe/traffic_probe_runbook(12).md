...
DOMAINS="relojistas.com" LETSENCRYPT_EMAIL=you@real.tld DEPLOY_USER=deploy \
  ENGINE_BINARY_PATH=/tmp/site-engine bash /tmp/setup.sh
```
Two traps seen in the field (debug guide #26): (1) setting the variables on
separate prompt lines does NOT work unless you `export` them — shell-local
variables never reach a child process (the one-line prefix form works because
prefix assignments are exported to that command); confirm with `env | grep
DOMAINS`, not `echo $DOMAINS`. (2) If the guard's error text doesn't match the
wording in your current setup.sh, the BOX has a stale copy — re-scp before
debugging further. Domains may also be passed positionally
(`bash /tmp/setup.sh relojistas.com`), but the email/deploy-user/binary
parameters are env-only. **Re-runs are idempotent and need no binary
parameter** — an already-installed engine is kept when no new one is provided;
this is the normal path for "fix DNS, then re-run for the cert".
...
grep -E '^(INTERNAL_API_KEY|THANKS_PATH)=' /etc/site-engine/site-engine.env  # file is the truth
exit
```
Record the key (domain notes). Read it back with the `grep`, not `echo $KEY` —
shell variables die with the session, and a later session printing an empty
`$KEY` does NOT mean the file is unset (debug guide #26 corollary). If running
this remotely in one line, single-quote the whole command so `$KEY` expands on
the box, not in your local shell.

**3.7 First content (laptop; the Action takes over after):**
The two page files (`index.html`, `gracias.html`) are deliverables — download
them into `$OUTPUTS/relojistas-site/` (create the folder) if not already
present. Then either:
```bash
# (a) direct rsync — no dependencies:
rsync -az --delete $OUTPUTS/relojistas-site/ deploy@$BOX:/var/www/vm-sites/$DOMAIN/
```
Flags: `-a` archive (recurse + preserve permissions/times/links — copy
faithfully); `-z` compress in transit; `--delete` remove destination files
absent from the source, making the webroot an exact mirror (so deleting a page
locally deletes it on the box). Foot-gun: with `--delete`, an EMPTY or wrong
source path wipes the destination — and the trailing slash matters
(`relojistas-site/` syncs the folder's CONTENTS into the webroot;
`relojistas-site` without the slash would nest the folder itself).
or (b) commit them to the vm-sites repo as `$DOMAIN/index.html` +
`$DOMAIN/gracias.html` and push — which deploys via the Action AND doubles as
the 3.9 content-seam test (needs the secrets set and the self-hosted runner
able to reach the box; fall back to (a) if the runner isn't ready).
...
**Export endpoint (P4 collector interface):** `GET /events` — key-gated NDJSON
stream of stored events, oldest first; final line is `{"_meta":{count,
truncated,server_time}}`. Params: `since` (RFC3339, strictly-after), `host`,
`limit` (default 5000; `truncated:true` means pull again from the new
checkpoint). Checkpoint contract: store the max `created_at` received;
strictly-after semantics make pulls duplicate-free. Reads are lock-free (an
export can never block live captures); a mid-append torn tail line is skipped
and arrives on the next pull.
```bash
curl -sS -H "X-Internal-Key: $KEY" "https://relojistas.com/events?since=2026-06-12T00:00:00Z" | tail -3
```

**Passive digest (Option A):** `GET /access-digest?host=&since=&top=` — key-gated
JSON rollup of the box's nginx access log for one domain (status mix, top
referers, top paths, top 404 paths, UA buckets, top real client IPs). Needs
per-domain logs + the engine in the `adm` group (both from setup.sh); set
`CLOUDFLARE=true` on proxied boxes so client IPs are real, not Cloudflare's.
```bash
curl -sS -H "X-Internal-Key: $KEY" "https://relojistas.com/access-digest?host=relojistas.com&top=15" | python3 -m json.tool
```
