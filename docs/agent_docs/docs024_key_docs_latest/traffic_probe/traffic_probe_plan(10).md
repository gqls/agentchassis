...
- **P4 — off-box collection + ranking (first chassis integration piece).**
  ...
  source of truth); DB CHECK constraints on the `intent_events` table; values
  escaped at every display surface. Open choice: redact email/phone patterns
  at ingest vs rely on the 90-day prune. *If any of this is ever built as a
  chassis adapter, the 003 envelope contract applies.*
