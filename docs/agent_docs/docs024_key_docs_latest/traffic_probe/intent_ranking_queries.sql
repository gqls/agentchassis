...
SELECT
    host,
    count(*)                                              AS events,
    count(*) FILTER (WHERE event_created_at > now() - interval '7 days')  AS events_7d,
    count(*) FILTER (WHERE event_created_at > now() - interval '30 days') AS events_30d,
    count(DISTINCT lower(btrim(value)))                   AS distinct_terms,
    count(DISTINCT NULLIF(ref_host, ''))                  AS distinct_referers,
    count(*) FILTER (WHERE landing_query <> '')           AS with_landing_query,
