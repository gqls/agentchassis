...
## Open threads
- The four decisions above. Deploy mechanism and repo layout are coupled (a
  shared VM repo makes a per-repo Action a single Action; repo-per-domain would
  multiply it). The workflow-separation question is partly about which build
  generation we're on (monolithic `pageflow-builder` vs the modular
  `build-dispatch-loop` + handler agents).
- Verify `git-adapter`'s path/repo logic before fixing a repo layout (does it
  support folder-per-domain in a shared repo, or is it repo-per-site today?).
- Verify `site_work_items.item_type` constraints + the dispatch loop's
  `input_mapping` before adding any new item type.
