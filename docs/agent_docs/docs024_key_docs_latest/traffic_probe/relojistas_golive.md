...
> OPEN ITEM before relying on the content Action: the git-adapter commits to
> `<domain>/...` at repo ROOT, but the existing b2 Action watches `sites/**` —
> these describe different layouts. `ls` the live sites repo root and tell me
> which is real; deploy-to-vm.yml will be adjusted to match. The manual steps
> below sidestep this by rsyncing content directly.
