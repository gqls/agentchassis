...
--
-- Gating (capability model): the component carries the semantic tag
-- `requires-backend` — the CLASS marker for sections that need a server-side
-- backend (the traffic probe is just the first instance of that class; a chat
-- or board section would carry the same tag). suitable_site_types stays empty:
-- "has a backend" is a property of the site's DEPLOY TARGET
-- (sites.deploy_config.capabilities), not a site type. NOTE the planner's
-- load_components query currently loads ALL active sections — apply the opt-in
-- gate below so requires-* components are reachable ONLY via roadmap
-- section_types pinning.
-- ============================================================================
...
-- ---------------------------------------------------------------------------
-- OPT-IN GATE (separate change, build-site-planner workflow JSON, NOT here):
-- the planner's load_components step currently loads ALL active sections, so a
-- backend-requiring component would be offered to every site, including
-- static-only (B2/Worker) ones. Change its query so requires-* components are
-- opt-in (roadmap-pinned only):
--
--   SELECT name, display_name, "function", category, description
--   FROM content_components
--   WHERE component_level IN ('section', 'element')
--     AND is_active = true
--     AND NOT (COALESCE(semantic_tags, '[]'::jsonb) ? 'requires-backend')  -- << the gate
...
--
-- SITE-SIDE DESIGNATION (the other half of the capability match; applied at
-- probe-site onboarding alongside github_repo, repo name TBC):
--   UPDATE sites SET
--     github_repo  = '<vm-sites-repo>',
--     deploy_config = COALESCE(deploy_config, '{}'::jsonb)
--                     || '{"target": "vm", "capabilities": ["backend"]}'::jsonb
--   WHERE domain = '<domain>';
--
-- LATER (when backend sections multiply): one audit check that compares each
-- placed section's requires-* tags against its site's
-- deploy_config.capabilities and writes site_work_items findings on mismatch.
-- ---------------------------------------------------------------------------
