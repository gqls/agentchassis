...
-- pre_query is a GATE (count>0): the scheduler fires ONE message per tick, and
-- the collect_intent_events action self-queries and loops all backend sites
-- (build-pipeline-trigger gate + thunder-monitor in-agent-loop pattern). No
-- per-row fan-out — the scheduler does not fan out pre_query rows.
-- ---------------------------------------------------------------------------
INSERT INTO scheduled_tasks (
    ...
    600,
    'intent-collection-orchestrator',  -- thin wrapper; spawns the intent-collector worker child (001 wrapper-orchestrator rule). NB: if a prior apply inserted 'intent-collector', UPDATE it (see intent_collector_registration.sql §3).
    'system.agent.generic.requests',
    'dispatch',
    2,
    $q$SELECT count(*)::text AS backend_sites
       FROM sites s
       WHERE s.status IN ('active', 'deployed')
         AND s.deploy_config->>'target' = 'vm'
       HAVING count(*) > 0$q$,
    false,
    300,
    true
