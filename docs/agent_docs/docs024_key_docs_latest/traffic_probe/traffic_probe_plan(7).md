...
- **P1 — manual go-live (Path A)** for 3–5 controlled domains; capture the exact
  ...
  *2026-06-11: relojistas.com go-live bundle delivered — `relojistas_golive.md`
  ...
  verification steps. Blocking nothing; one OPEN ITEM: the sites-repo layout
  (git-adapter writes `<domain>/` at repo ROOT; the b2 Action watches
  `sites/**` — confirm which layout is real before trusting deploy-to-vm.yml).*
- **P2 — wire the deploy on update**: VM-sites repo + its VM-deploy Action
  ...
  *Done: content Action `deploy-to-vm.yml` + engine Action `deploy-engine-to-vm.yml`
  ...
  documented (runbook §4). Pending: layout confirmation above.*
...
- **P4 — off-box collection + ranking (first chassis integration piece).**
  ...
  improvement-loop tie-in. *If any of this is ever built as a chassis adapter,
  the 003 envelope contract applies.*
