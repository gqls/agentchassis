...
## Log
- 2026-06-12 — **VERDICT (access log, 14,961 reqs): overwhelmingly bots/ghosts,
  human intent ≈ 0.** 83% are 404s on OLD vBulletin paths (`/external.php?type=RSS2`,
  `/attachment.php?...`, `/customavatars/...`) — crawlers + stale links hitting
  the dead forum's skeleton; 2,242 × 301 = the www→apex/https redirects (NORMAL,
  not the loop I feared — concern cleared); only 268 × 200; beacon fired 8×
  (all operator). UA top: a Chrome-spoofing crawler (6,173), **Claude-SearchBot
  (2,966)**, **SemrushBot (2,260)**, YandexBot, libredtail-http. Cloudflare's
  1.13k "unique visitors" was an upper bound dominated by these; the 308/h spike
  was crawlers discovering the new www name, not people. **This is a clean
  probe result, not a measurement failure** — relojistas does NOT warrant a
  build on current demand. Caveat retained: a 48h window with the access log as
  arbiter confirms; but day-one signal is unambiguous.
  Useful by-product: those 404 paths ARE intent (what the forum's surviving
  links point at) → P4 collector should harvest 404 request paths from the log.
