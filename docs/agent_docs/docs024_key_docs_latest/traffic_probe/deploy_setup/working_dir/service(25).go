...
type Config struct {
	...
	NginxLogDir  string          // dir holding <domain>.access.log for the access digest
...
func (a *App) routes() http.Handler {
	...
	mux.HandleFunc("/access-digest", a.accessDigest)
...
// accessDigest returns a key-gated digest of this box's nginx access log for one
// host (referer/path/404/UA/IP rollup) — the passive half of P4. Params: host
// (required), since (RFC3339, optional), top (default 20).
func (a *App) accessDigest(w http.ResponseWriter, r *http.Request) {
	if a.cfg.InternalKey == "" || r.Header.Get("X-Internal-Key") != a.cfg.InternalKey {
		http.Error(w, "unauthorised", http.StatusUnauthorized)
		return
	}
	...
	if !safeHost(host) {
		http.Error(w, "host required (bare domain)", http.StatusBadRequest)
		return
	}
	var since time.Time
	if v := r.URL.Query().Get("since"); v != "" {
	...
	logDir := a.cfg.NginxLogDir
	if logDir == "" {
		logDir = "/var/log/nginx"
	}
	digest, err := buildAccessDigest(logDir, host, since, topN)
	if err != nil {
		if os.IsNotExist(err) {
			http.Error(w, "no access log for host", http.StatusNotFound)
			return
		}
