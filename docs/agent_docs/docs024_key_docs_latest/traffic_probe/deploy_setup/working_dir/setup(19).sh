...
# CLOUDFLARE=true installs nginx real_ip config for Cloudflare ranges so
# $remote_addr (and the access log / fail2ban / the access-digest IP rollup)
# reflect the REAL client IP from CF-Connecting-IP, not Cloudflare's edge IP.
# Set true for proxied (orange-cloud) boxes; default false for DNS-only.
...
# the engine reads /var/log/nginx/<domain>.access.log for /access-digest; the
# logs are created 0640 root:adm (or www-data:adm), so the engine needs adm.
if id "$SERVICE_USER" >/dev/null 2>&1 && getent group adm >/dev/null 2>&1; then
...
# Cloudflare real_ip: when the box is proxied (orange cloud), origin nginx sees
# Cloudflare edge IPs as $remote_addr. This restores the real client IP from the
# CF-Connecting-IP header for logs, fail2ban, and the access-digest IP rollup.
# Ranges from https://www.cloudflare.com/ips/ (refresh periodically).
write_cloudflare_realip() {
  local f="/etc/nginx/conf.d/cloudflare-realip.conf"
  if [[ "$CLOUDFLARE" != "true" ]]; then
    rm -f "$f"
    return 0
  fi
  log "installing Cloudflare real_ip config"
  cat > "$f" <<'CF'
# managed by setup.sh (CLOUDFLARE=true). Cloudflare IP ranges — refresh from
# https://www.cloudflare.com/ips/ if CF changes them.
