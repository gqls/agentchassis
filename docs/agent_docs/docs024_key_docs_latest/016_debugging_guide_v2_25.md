...
**Fix directions (read done; not yet chosen — recording, not prescribing).** Two shapes, with the standing "structural over local" lean noted:
1. *Single source of truth (structural; aligns with doc 029).* Make `SyncPagesToDBAction` read identity from `site_plan_pages` and carry `url`/`role`→`page_type` through verbatim, rather than re-canonicalising `page_plan`. Caveat to check first: sync also runs at strategist time, before any `site_plan` exists, so this needs the sequencing confirmed (which invocations have a plan to read) before committing.
2. *Make the two surfaces agree (local).* Have `SyncPagesToDBAction` run `ValidateRoles` before `CanonicalisePage`, reusing the existing `datahelpers.ValidateRoles` (not a copy), so both canonicalisers apply identical rules. Lower blast radius; doesn't remove the two-source duplication, just makes it consistent.
   In either case, also set `built_from_plan_version` on the sync write so the reconciler stops re-emitting rebuilt pages as stale.
