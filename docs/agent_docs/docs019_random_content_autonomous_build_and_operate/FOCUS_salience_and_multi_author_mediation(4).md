...
## 9. Priority-profile representation and maintenance

The priority among dimensions (security / speed / simplicity / generality / functionality / cost) is requirement-relative and lives on the objective-tree node, read off the why-chain. The important reframing: the profile is **not a fixed setting**. External events change priorities — a change of law, a client instruction, a security disclosure in the news. So storage, inheritance, and maintenance are one question: **how does a priority change enter, and how far does it correctly reach?**

### 9.1 Storage

- **Fixed, governed dimension vocabulary.** The dimensions are a fixed set so profiles on different nodes are comparable; changing the vocabulary is a rare, deliberate act owned by the coordinator.
- **A node stores only its differences from what it inherits**, not a full profile. Most nodes store little or nothing; an entry exists only where the node changes something.
- **Each entry carries its premise, its source** (client / law / news / internal judgement), **and an expiry if temporary.**
- **The effective profile is computed on demand**, by walking from the root to the node and merging the entries. It is derived, not stored — if the effective profile were stored it would drift the moment an ancestor changed. Storing authored differences and computing the effective profile on read means an ancestor change is automatically reflected everywhere below it.

### 9.2 Where a change enters determines how far it reaches

Computed-on-demand is what makes broad external changes practical — you change one node at the right level rather than sweeping the tree.

- A data-privacy law affecting everything that handles user data: a single edit at a high node; every descendant's effective profile reflects it on next read.
- A client instruction that applies only to their site: an edit on that client's subtree.
- News raising the security priority for one vertical: an edit on that vertical's node.

Inheritance does the reaching; the entry point sets the scope.

### 9.3 Inheritance: two precedence directions

- **Normal entries are child-wins** — a child is a local refinement, and a leaf knows its requirement better than the mission does.
- **Sealed constraints are ancestor-wins** — inheritable and not overridable by descendants. A legal floor is applied as a sealed constraint precisely so a leaf's earlier relaxation cannot defeat a new law. Used for legal floors and mission non-negotiables.
- **Asymmetry of scrutiny.** Tightening (adding a constraint, raising a priority) is routine. Loosening a non-sealed inherited constraint is a higher-stakes, explicitly recorded judgement. Loosening a sealed constraint is not allowed except by an authority equal to the one that sealed it. So "sometimes a security measure is too strict for the requirement" is permitted — but only as a recorded judgement against a non-sealed constraint, never against a sealed legal floor by leaf convenience.

### 9.4 Maintenance: a change triggers targeted re-validation

When an entry changes high in the tree, some descendants may hold overrides that now conflict with it — most importantly a descendant that relaxed something a new sealed constraint now requires.

- **Not a full sweep:** check exactly those descendants that hold an override on the dimension that changed; flag those whose overrides now conflict for re-mediation.
- This is the same mechanism as the drift detector (§11) and the heuristic-invalidation hook (§7): a change to a premise high up automatically surfaces the downstream decisions made on the old premise. This is the concrete meaning of "priority maintenance" — a priority change is an edit **plus** the flagging of everything below it that the edit invalidates. It is what stops a law change from being quietly defeated by a pre-existing leaf decision.

### 9.5 Lifecycle and authority follow the source

The source of an entry is not just a note — it determines who may undo the entry and when it expires.

- A law-sourced sealed constraint is permanent until repealed.
- A client launch instruction ("speed over polish until the launch date") is time-bounded and expires, after which the effective profile reverts on its own.
- A news-driven security raise may be permanent or last until a patch.
- Authority to change an entry follows its source: a law-sourced constraint is relaxable only by law-level or creator authority, not by an area owner; an internal-judgement entry is revisable by the area owner who made it.

### 9.6 Entry path and representation choice

- **Entry path.** External changes enter through the existing governance path: a process detects or receives the change (a legal-update intake, a news monitor, a client request), drafts it as a profile change with its premise and proposed scope, a human confirms it (confirm-not-initiate), and it is applied at the node matching its scope, which then triggers the §9.4 re-validation.
- **Representation: an order, not numeric weights.** Numeric weights (security 0.7, speed 0.5) are hard to set honestly, hard to maintain, and falsely precise, and external events do not arrive as number tweaks — they arrive as "security now outranks everything here" or "this is now required." So priority among the weighted objectives is represented as an **order** for the node (security outranks speed outranks simplicity), with pairwise relations where known and "no stated relation, balance them" where not, plus the constraint and sealed flags for the non-negotiable parts. The mediator reasons "for this node security outranks speed, so when they conflict security wins the trade" — which is what a law change or client instruction naturally expresses.
- **Direction-of-travel still modulates temporarily.** "Deprioritise hardening for now" is a time-bounded, freshness-stamped reweight over the static order, reverting on expiry.

### 9.7 Open question — parked for later

**The question:** when a law, a piece of news, or a client instruction arrives, what determines which node it attaches to, and therefore how far it propagates?

**Why this is the crux.** Everything else in §9 is mechanical: storage is differences-from-inherited, the effective profile is computed on read, inheritance has fixed precedence rules, and maintenance is targeted re-validation. The one step that is **not** mechanical is choosing the entry node. Because inheritance does the reaching (§9.2), the entry node alone decides the scope of the change — so getting the level right is the whole game for broad changes.

**The failure modes it has to avoid:**
- Attaching **too high** over-applies the change — a client-specific instruction applied across the whole platform, or a single vertical's news item applied to every vertical.
- Attaching **too low** lets the change miss nodes it should reach — a law that should be a high sealed constraint applied to only one leaf, leaving sibling nodes non-compliant.

Misclassification is most costly exactly for the highest-stakes changes (legal floors), because too-low attachment produces silent non-compliance elsewhere in the tree.

**What a resolution would need to settle:**
- How the proposed scope is derived from the change's nature (a law is usually high and sealed; a client instruction is that client's subtree; a news item is the affected vertical or area) — and whether that derivation is heuristic, classifier-driven, or human-judged.
- How a change whose correct scope is genuinely ambiguous, or that spans several subtrees that are not a single ancestor, is handled (multiple entries, or a new shared ancestor).
- How the chosen scope is recorded with the entry so it is auditable and can be re-opened if it turns out to have been wrong.
- The likely shape: scope is proposed (by whatever process intakes the change) and **confirmed by a human via confirm-not-initiate**, since it is a judgement; the open part is how the proposal is derived and how ambiguity and multi-subtree changes are represented.

Parked here, ready to take up another time. The thread that led to it is §9.1–9.6 above.
