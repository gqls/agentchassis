...
> Consolidated canonical guide. Based on the latest main-line revision (formerly
> `016_debugging_guide_v2_17_`), with the unique "Adapter & Service Deployment
> Issues" section rescued from an earlier branch revision, and the 2026-05
> FAQ/news/visual debugging entries merged into Sections 9 and 0. Supersedes all
> prior `016_debugging_guide` revisions.
...
**18. "Renders empty" is a data-binding diagnosis, not a template one.** When
...
### — Content-pipeline failure patterns (FAQ / content_data / CSS / news, 2026-05) —

_The following entries came from the 2026-05 FAQ-empty-items, news-rendering, and visual-pipeline
debugging sessions. Each names the FOCUS doc that owns the corresponding fix._
...
## Adapter & Service Deployment Issues

_Rescued from an earlier guide revision (it appeared in one branch as "Section 10" and was dropped from the main line); preserved here because the content is unique and the failures are real. Placed with the other deployment/operational material._


This section is built from real deployment failures, especially the long chain of issues debugging thunder-adapter Phase 2 (the third deployment in this codebase, none of the issues were obvious from existing docs).

### ImagePullBackOff — `insufficient_scope: authorization failed`

```
Failed to pull image "docker.io/aqls/<adapter>:vX.Y.Z":
... pull access denied, repository does not exist or may require authorization:
server message: insufficient_scope: authorization failed
```

Docker Hub returns the same error message for several causes, so diagnose in order:

**Diagnostic A — does the tag actually exist?**

```bash
docker manifest inspect docker.io/aqls/<adapter>:vX.Y.Z
# Success → tag exists
# "manifest unknown" → tag was never pushed
```

A common gotcha: `make deploy-agents IMAGE_TAG=vX.Y.Z` updates the kustomize `newTag` and re-applies, but doesn't build or push. If the build/push for that tag didn't run in the same session, the image won't exist. Run `make build-<adapter> IMAGE_TAG=vX.Y.Z && docker push docker.io/aqls/<adapter>:vX.Y.Z` to fix.

**Diagnostic B — local pull works but cluster pull fails?**

```bash
docker pull docker.io/aqls/<adapter>:vX.Y.Z    # check from your laptop
```

If your local pull succeeds (you're logged into Docker Hub) but the cluster fails, the cluster is missing or mis-scoped credentials for this specific image. Compare the failing adapter against a working one:

```bash
# Check imagePullSecrets and serviceAccountName side-by-side
kubectl -n ai-persona-system get deploy git-adapter -o yaml | grep -A3 -E 'serviceAccountName|imagePullSecrets'
kubectl -n ai-persona-system get deploy <failing-adapter> -o yaml | grep -A3 -E 'serviceAccountName|imagePullSecrets'

# Check the SA the pod is actually using
kubectl -n ai-persona-system describe pod <pod> | grep "Service Account"
```

Working adapters in this cluster all have:
...
If the failing adapter shows `Service Account: default` in the describe output, neither of those is set. Add them to `base/deployment.yaml` (at the pod-spec level, not container-spec).

**Diagnostic C — per-repo Docker Hub permissions**

If credentials look correct but auth still fails for this image only: a Docker Hub Personal Access Token can have explicit repository scope. Newly-created private repos aren't auto-added. Check `https://hub.docker.com/settings/personal-access-tokens` → the cluster's PAT → ensure the new repo is included (or regenerate with broader scope).

---

### Container crashes immediately: `exec: "--config": executable file not found`

```
Error: failed to create containerd task: ... error during container init:
exec: "--config": executable file not found in $PATH: unknown
```

Means the kubelet tried to exec `--config` as if it were the binary. This happens when:

- The Dockerfile uses `CMD ["./binary", "-config", "path"]` (not `ENTRYPOINT`)
- The deployment provides only `args: ["--config", "/app/configs/x.yaml"]`

Kubernetes semantics: `args:` in a deployment **replaces the entire CMD**, including the binary path. So the final invocation becomes `["--config", "/app/configs/x.yaml"]` with no executable.

**Fix:** use `command:` with the full invocation in the deployment:

```yaml
containers:
  - name: <adapter>
    image: ...
    command: ["./<adapter>", "-config", "/app/configs/<adapter>.yaml"]
```

Compare with working adapters — git-adapter, image-generator-adapter all use `command:` with the full path. Don't use `args:` alone unless the Dockerfile has a proper `ENTRYPOINT`.

---

### Adapter starts but `Unknown Topic Or Partition` on first message

```
Failed to produce Kafka message: [3] Unknown Topic Or Partition:
the request is for a topic or partition that does not exist on this broker
```

Strimzi clusters in this project have `auto.create.topics.enable=false` (intentional — prevents topic sprawl). Any topic referenced in code must exist as a `KafkaTopic` CRD before the producer writes to it.

Common scenarios:

- New adapter's requests topic not provisioned. The adapter consumer hangs forever (no error, just `context deadline exceeded` on fetch).
- Test/smoke-test topics that look "obvious" (e.g. `system.thunder.smoke.responses`) but were never declared.
- A handler emits to `reply_to_topic` from incoming message; the caller chose a topic that doesn't exist.

**Diagnostic:**

```bash
# List existing KafkaTopic CRDs
kubectl -n kafka get kafkatopic | grep <prefix>

# Or query Kafka directly via kafkactl pod
kubectl -n kafka exec personae-kafka-cluster-combined-pool-prod-0 -- \
  bin/kafka-topics.sh --bootstrap-server localhost:9092 --list | grep <prefix>
```

**Fix:** declare the topic as a KafkaTopic resource:
...
spec:
  partitions: 1     # or 3 for higher-throughput topics
  replicas: 1       # raise for production-grade durability
  config:
    retention.ms: 604800000   # 7 days; tune to use case
```

Apply and wait ~30 seconds for the Strimzi entity operator to provision it, then retry.

---

### Deployment essentials checklist (every new adapter)

If you're creating a new adapter, the deployment manifest needs all of these — not optional, not "we'll add later":

| Field | Where | Why |
|---|---|---|
| `serviceAccountName: ai-persona-app` | pod spec | Without this, the pod uses `default` SA which has no permissions. Required for any future k8s API access (Secrets, ConfigMaps from app code). |
| `imagePullSecrets: [{name: docker-hub-creds}]` | pod spec | All aqls/* repos are private. Without this, ImagePullBackOff. |
| `command: ["./<binary>", "-config", "/app/configs/<adapter>.yaml"]` | container spec | Avoids the CMD/args confusion in section 10.2. |
| `envFrom: [{secretRef: {name: personae-default-secrets}}, {secretRef: {name: personae-platform-secrets}}]` | container spec | Provides DB password, S3 creds, API keys. |
| `POD_NAME` from downward API, `AGENT_VERSION` literal | container env | Used in Tier-3 response headers for tracing. |
| Readiness on `/ready`, liveness on `/health` | container probes | `/ready` should reflect actual DB connectivity; `/health` is always-OK if process is alive. |
| ConfigMap mount at `/app/configs/<adapter>.yaml` via `configMapGenerator` | volumes + volumeMounts | Hash-suffixed ConfigMap triggers pod restart on config change. |

The `FOCUS_adapter_design.md` "Deployment essentials" section should mirror this checklist. The first three items in particular were not in the original design doc and cost real time to debug.
