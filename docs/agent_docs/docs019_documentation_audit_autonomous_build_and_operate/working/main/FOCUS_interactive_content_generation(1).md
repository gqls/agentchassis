...
template: rawHTML → Go extractor → external asset fetch → fingerprint
→ LLM brief → site_specs aspect.

For interactive logic, the extension follows the same pattern:

- **Extend or add a Go extractor.** Find `<canvas>` elements, find
  non-trivial `<script>` blocks (skip standard analytics, GTM, etc.),
  find `<script src="...">` references for external JS. Detect
  signals indicating element type — animation loops suggest game,
  input + calculate-button + result pattern suggests calculator, form
  with submit handler suggests interactive form. The signal extraction
  itself is structural, not LLM — same shape as goquery work in the
  design extractor.

- **Fetch external JS** via the existing `firecrawl_scrape` adapter.
  Same pattern as external CSS today.

- **Produce a fingerprint** alongside the design fingerprint, carrying
  per-interactive-element structured data: type, source pointer,
  inputs/outputs detected, library/framework signals.
...
The work isn't a new capability layer. It's an extractor and an LLM
step, each following an established pattern.
...
Extend the existing design-extraction pattern to interactive logic. Add
`extract_interactive_fingerprint` Go step (parallel to
`extract_design_fingerprint`), fetch external JS via the existing
firecrawl_scrape adapter, add an LLM step to produce
`interactive_reference` / `interactive_intent` site_specs aspects. This
benefits every downstream generator — tools, games, future content
types. Smaller leap than I first thought because the pattern is in
place; the work is one extractor and one LLM step.
