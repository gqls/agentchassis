...
How to run the thin slice today. Files: `analyser.go`, `assembler.go`, `dbcontext.go`, `resolve_targets.go`, `thin_slice_constitution.md`. Go toolchain on the machine with the repo (and, for `dbcontext`, `psql` access to the DB).
...
1. **Target resolution — embedding layer.** The lexical baseline (`resolve_targets.go`) proposes a `-scope` set by name/path/doc overlap; it nails symbols whose names echo the task and suggests wiring `-include`s, but misses decisive symbols whose names don't (e.g. it ranked `LoadPageSectionsFromSpecAction` below the cut for the skinner-box task). Embeddings of signature+docstring are the recall layer that closes that gap — added once there's a ground-truth task→files set to measure against.
...
*(Done: call-graph neighbourhood; live schema + row data via `dbcontext` with multipass sizing; `-include` force-include for wiring/shared files; runtime evidence via `dbcontext -runtime-site` + assembler `-runtime`; a lexical target-resolution baseline (`resolve_targets.go`).)*
