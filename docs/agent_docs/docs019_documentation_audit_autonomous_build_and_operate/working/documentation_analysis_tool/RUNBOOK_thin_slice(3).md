...
How to run the thin slice today. Files: `analyser.go`, `assembler.go`, `dbcontext.go`, `thin_slice_constitution.md`. Go toolchain on the machine with the repo (and, for `dbcontext`, `psql` access to the DB).
...
# 2. (Optional) Pull live DB context — schema and/or row data.
...
1. **Target resolution** — from a task description, propose the in-scope set (so `-scope` isn't fully manual) and which tables `dbcontext` should pull. This is the biggest remaining manual step and the thing the hand-built manifests do by judgement.
2. **Runtime evidence** — for `debug` tasks, pull the run trace, step sequence, error trail, and pod logs correlated by `orchestration_id`. Confirmed essential by the hand-built debug manifests.
3. **Matched-guidelines retrieval** — find the relevant docs/standards for a task automatically, instead of naming them by hand with `-doc`. The documentation counterpart to target resolution.
4. **Review checkers (with fixers)** — liability and **morality** review for output that could create legal exposure or be ethically wrong. The morality checker applies a *configured, layered* standard (a chosen base framework + operator judgement + jurisdiction/current-focus, prioritised), and routes contested calls to a human rather than auto-fixing.
5. **Code-freshness guard** — record the commit/SHA the analysis was taken from, so a stale checkout is visible rather than silent.

*(Done: call-graph neighbourhood; live schema + row data via `dbcontext` with multipass sizing; `-include` force-include for wiring/shared files the call graph can't reach.)*
