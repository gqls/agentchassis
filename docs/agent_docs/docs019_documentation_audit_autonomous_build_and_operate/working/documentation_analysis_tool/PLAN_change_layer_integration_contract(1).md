...
First-class storage so events are reproducible, retryable, and auditable. Not transient.

| Field | Type | Purpose |
|---|---|---|
| `id` | uuid PK | |
| `tenant_id` | uuid FK | Scope. |
| `source` | enum | `git_webhook` / `polling` / `in_band` / `periodic_sweep` / `manual` |
...
| `triggers_fired` | jsonb, nullable | `[{trigger_kind, dispatched_to_agent, work_item_id?}]` |
