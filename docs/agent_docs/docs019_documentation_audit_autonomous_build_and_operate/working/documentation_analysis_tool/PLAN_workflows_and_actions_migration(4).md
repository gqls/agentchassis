...
4. **Embedding storage at scale.** A table plus in-Go cosine is fine for one repo (thousands of symbols); multi-tenant / many repos likely needs pgvector or a vector store. Decide the threshold, and whether to start on pgvector to avoid a later migration. **Update (2026-06-09):** a `\df` snippet shows `sparsevec`, a pgvector artefact, so pgvector appears already installed — confirm with `\dx`. If so, prefer pgvector columns + its similarity operators from the start (no in-Go cosine, no later migration), which also settles the at-scale question.
