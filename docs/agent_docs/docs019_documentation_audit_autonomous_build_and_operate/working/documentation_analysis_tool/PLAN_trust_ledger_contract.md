...
| `trust_level` | enum | `confirm_every` / `confirm_exceptions` / `notify` / `autonomous` |
...
| `last_change_direction` | enum, nullable | `up` (graduation) / `down` (de-graduation) / `manual_set` |
| `last_change_decision_id` | uuid FK, nullable | `decision_log` entry that justified the change. |
| `created_at` | timestamptz | |
| `updated_at` | timestamptz | |

Indexes: `(tenant_id, capability_id)` unique; `(tenant_id, trust_level)` for "which capabilities are at level X."
...
So the ceiling lives in a **capability catalog** (likely an extension of `agent_definitions` or a sibling table — `capabilities`), not on the ledger row:
