...
7. [NEW 2026-07-07 — PENDING FORK, may land here too] Guides/tools/games/
   ...
   VERIFICATION GAP (searched 9 files, not found): a guide-WRITING agent,
   tool-suggestion-to-site pipeline, or games mechanism WIRED into the
   fresh-build relay — page_type "guide" etc. exist only as adoption
   CLASSIFICATION vocabulary; blog-content-planner (active, census) never
   appears as a handler_agent in any dumped relay workflow. Fork queries +
   decision rule in the fix-loop runbook — if it resolves to "not wired",
   this becomes a NEW hop (post-plan or a planner extension) equivalent in
   shape to item 6 (a missing decision/connection, fix once, applies to
   every site). The curated top-N list is a NEW page/section TYPE — reuse
   candidate: research-agent (web research + source attribution) or the
   vertical-exemplar-researcher's crawl pattern, feeding a new curation step.
