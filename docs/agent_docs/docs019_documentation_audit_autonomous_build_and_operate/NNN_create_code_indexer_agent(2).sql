...
--   \d agent_definitions
--
-- Schema CONFIRMED [checked 2026-06-12: \d agent_definitions supplied]:
-- NOT NULLs covered (type, display_name, category; the rest defaulted);
-- agent_category CHECK = strategist|executor|analyst|integrator|coordinator|
-- specialist ('integrator' ✓); status CHECK includes 'experimental' ✓;
-- domain_tags is JSONB not text[] (first apply failed on an ARRAY[] literal —
-- fixed to a '["…"]'::jsonb cast); topics/image_repository values here happen
-- to equal the column defaults (kept explicit for readability).
-- Re-run guard: UNIQUE (type, version) — a second INSERT of code-indexer v1
-- is rejected; to revise an existing row, snapshot_agent then UPDATE.
--
...
--
-- LABEL CONVENTION [checked 2026-06-11: composition implemented in
-- IndexCodeSymbolsAction, code_symbols_actions.go]: code_symbols.repo is the
-- "owner/repo" form (e.g. 'gqls/agentchassis'), COMPOSED by index_code_symbols
-- from the analyser reply's owner+repo — so the label always matches what was
-- fetched. The index step therefore sets NO repo config. Every lookup filters
-- the same form: WHERE repo = 'gqls/agentchassis'. Non-git corpora later
-- override via the step's config.repo (e.g. 'domain:kruste.com').
...
INSERT INTO agent_definitions (
    ...
    'v1.0.1061',  -- set the CURRENT tag at apply time (informational for generic-chassis rows)
