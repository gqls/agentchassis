...
text is filtered through `diagnose.IsReadOnlySQL` TWICE — at parse
(`verdict_wire.toVerdict` drops non-read-only requests) and again in
`runDataRequests` before execution; (3) the read-only transaction
