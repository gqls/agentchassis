...
APPLY ORDER: (0) the two pre-apply queries above — if clean → (1) seed →
(2) reroute (handler must exist first or feasibility-recheck holds items
blocked) → (3) nudge (optional, any time) → (4) VERIFY RUN: one blank domain
via domain-submitter; watch the item chain gain needs_vertical_research
(claimed by the researcher), `SELECT aspect FROM site_specs WHERE site_id=…`
gain vertical_landscape, and the strategy spec's competitive_position
reference the landscape's content.
