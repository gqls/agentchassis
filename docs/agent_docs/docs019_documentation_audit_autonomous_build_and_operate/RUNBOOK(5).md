...
- [x] §6C diagnose pair already seeded (diagnose-agent + diagnose-orchestrator); diagnostician dropped
- [x] §6C column-move migration WRITTEN (`NNN_move_diagnose_workflow_to_default_config.sql`)
- [ ] §6C column move APPLIED (orchestration_workflow → default_config) — the current blocker
- [ ] §6C `diagnose_run` + `diagnose_emit` built + registered + `diagnose-verdict-v1` resolves + deployed
...
### 6C. Use the existing diagnose pair + fix its workflow column — [ ]  ← do this next

You already have the agents: `diagnose-agent` (the worker that runs the loop) and
`diagnose-orchestrator` (the thin spawn-and-forward wrapper), seeded 2026-06-20.
Do NOT seed a new one — the `diagnostician` seed drafted earlier is SUPERSEDED
(see its banner). The existing pair is the better design: it runs the loop inside
the tested `diagnose_run` engine action (complexity in Go, thin workflow) rather
than re-expressing the loop as workflow steps. Three things are wrong/missing on it.

**1. The workflow is in the wrong column.** Both agents carry their workflow in
`orchestration_workflow` (the `json` column); the loader reads `default_config`
(where every working agent — build-dispatch-loop, site-work-orchestrator — keeps
it). Apply `NNN_move_diagnose_workflow_to_default_config.sql` (moves the column
verbatim and adds the `processing_mode`/`timeout_seconds` the seeded workflow lacked):
```bash
$PSQL -v ON_ERROR_STOP=1 -f "$CHASSIS/<migrations path>/NNN_move_diagnose_workflow_to_default_config.sql"
$PSQL -c "SELECT type, (default_config ? 'workflow') AS has_workflow,
...
# expect has_workflow=t, mode=orchestrator, orch_null=t for both
```

**2. Two actions in the workflow are not built.** The `diagnose-agent` workflow is
`analyse_repo (request_repo_analysis) → lookup_symbols (lookup_code_symbols) →
load_runtime (diagnose_load_runtime) → assemble_bundle (diagnose_assemble_bundle) →
run_loop (diagnose_run) → emit (diagnose_emit) → complete`. Of these:
- `request_repo_analysis`, `lookup_code_symbols`, `complete_workflow` — EXIST.
- `diagnose_load_runtime`, `diagnose_assemble_bundle` — drafted (this work); finish
  per §1–4 and §6B.
- `diagnose_run` — NOT built. The substantive new action: it wraps the tested engine
  `diagnose.Run(...)` with a `Verdicter` that calls the model via the verdict prompt,
  owns the iteration + re-gather + guards, and returns the diagnosis + evidence
  trail. The chassis-integration design argues for exactly this over a
  workflow-driven loop (keep the guards in the tested Go engine, not re-expressed as
  workflow branches). Config it reads (from the seeded workflow): `bundle_field`,
  `analysis_field`, `max_iterations`, `seed_scope_field`, `verdict_prompt_ref`,
  `seed_hypothesis_field`.
- `diagnose_emit` — NOT built. Thin: read `diagnosis` (config `diagnosis_field`) and
  emit it for a human. Never a fix.

**3. The verdict prompt.** `diagnose_run` uses `verdict_prompt_ref:
"diagnose-verdict-v1"`. Ensure that ref resolves to the cite-or-abstain,
falsify-first prompt (`PROMPT_diagnosis_verdict.md`) in whatever prompt store
`diagnose_run` reads. If there is no registry, `diagnose_run` must take the prompt
inline (or via an `execute_llm_prompt` verdict step).

Then register `diagnose_run` + `diagnose_emit` (Category "diagnose") and deploy:
```bash
cd "$CHASSIS"
grep -RnE "diagnose_(run|emit|load_runtime|assemble_bundle)" platform/orchestration/actions/registry.go
go build ./... && go test ./...
git add -A && git commit -m "diagnose: build diagnose_run + diagnose_emit, register; fix workflow column" && git push
```
*DoD:* both agents show `has_workflow=t` in `default_config` with
`processing_mode=orchestrator`; the four diagnose actions registered;
`diagnose-verdict-v1` resolves; build/test green; image released.

Note: in this design the model's per-iteration read-only `data_requests` (§6B) are
executed by the engine's gather INSIDE `diagnose_run`, not by `diagnose_load_runtime`
(which runs once, for the initial runtime tier). The `runDataRequests` helper from
§6B is reusable there.
...
### 6E. Confirm `diagnose_run` iterates internally — [ ]  (after 6C)

In this design there is NO workflow loop-back: the iteration lives inside the
`diagnose_run` action (the engine `Run()`), so the orchestration runs the gather
steps once, then `run_loop` executes the whole capped loop, then `emit`. Confirm
the loop actually iterated and re-scoped by reading the agent's logs and the
emitted evidence trail (not the orchestration step path, which shows only the
single `run_loop` step):
```bash
