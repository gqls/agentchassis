...
4. TOKEN CHECK before triggering: analyse_repo_local reads env
   ...
   the pod that clones.
   RESOLVED 2026-07-02: code-indexer is DYNAMICALLY SPAWNED (user-confirmed;
   pod list shows the agent-<type>-<id> pattern, no standing indexer
   deployment). So the remedy is the one-line gate addition:
   ...
   VERIFY at run time on the spawned pod:
   `kubectl -n ai-persona-system describe pod agent-code-indexer-<id> | grep -A3 GITHUB_READ_TOKEN`
...
- [ ] spawn gate line added + spawning image rebuilt/DEPLOYED BEFORE the
      trigger (hard ordering: the spawned pod's token comes from the NEW
      image's secretKeyRef); then push the branch; then trigger. Verify on the
      spawned pod: describe | grep -A3 GITHUB_READ_TOKEN
