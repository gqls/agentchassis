...
- [ ] §6E workflow loop-back confirmed — first run stopped at iter 1 (diagnose_route did not seed LoopState); SEEDING FIX applied, redeploy + re-run to confirm re-scope (iterations ≥ 2)
...
>
> **§6F PASSED — the loop ran end-to-end, read-only.** The swap migration is applied
> (`analyse_repo`→`analyse_repo_local`), and a hand-triggered run (orchestration_name
> `manual-diagnose-20260629-154655`, correlation `7edf33b4-…`) walked
> `analyse_repo_local → lookup_symbols → load_runtime → assemble_bundle → verdict →
> route → emit → complete`. `repo_analysis` carried a real `{root, files, commit_sha}`,
> `assemble_bundle` cleared the repo-root check, and a verdict was produced. No writes
> (`last_work_item` 2026-06-26 / `last_page` 2026-06-22 untouched). §6A–6D and §6F done.
>
> **§6E NOT YET — and the first run found a structural bug (now fixed, pending redeploy).**
> The run stopped at **iteration 1** with `UNVERIFIABLE / scope-not-narrowing`. Cause:
> `diagnose_route` did not SEED the LoopState on the first iteration — it rehydrated from an
> absent `diagnose_state`, leaving the zero value (`Hypothesis=""`, `Scope={}`,
> `PrevScopeSize=0`). The narrowing guard is `next.size() > prevSize+2`; the model proposed a
> 3-item `next_scope`, so `3 > 0+2` tripped and stopped before re-scoping. (The verdict itself
> was sound: it abstained with two real citations and asked for the index page's
> `page_components`/`pages` content.) FIX APPLIED: `diagnose_route` now seeds via
> `diagnose.InitLoopState` (hypothesis = symptom, scope = the same seed `assemble_bundle`
> uses), so `PrevScopeSize = seed.size()+1`. Code-only — **no migration**.
>
> **NEXT — rebuild + redeploy, then re-run:**
> ```text
> # 1. Drop in the patched platform/orchestration/actions/diagnose_route_action.go
> go build ./...
> git add -A && git commit -m "diagnose_route: seed LoopState on first iteration (fix scope-not-narrowing at iter 1)"
> git push                     # GitHub Actions -> Backblaze; note the new image tag
> # 2. Re-trigger §6F (same envelope) with a NEW correlation_id; capture it.
> # 3. Read the diagnose-AGENT row by correlation_id (NOT LIMIT 1):
> ```
> ```bash
> $PSQL -c "SELECT orchestration_id, orchestration_name, status, current_step,
>                  collected_data #>> '{emit,status}'     AS emit_status,
>                  collected_data #>> '{emit,stopped_by}' AS stopped_by,
>                  collected_data #>> '{route,iteration}' AS iterations
>           FROM orchestration_states
>           WHERE correlation_id = '<the-new-id>' ORDER BY created_at;"
> ```
> **§6E is satisfied when `iterations` ≥ 2** (the loop re-scoped at least once) — i.e. it did
> not stop at iteration 1. Then read the trail: does the scope advance, and does it converge
> (CONFIRMED) or abstain cleanly?
>
> **Watch on the re-run (do NOT fix yet — re-run first, don't conflate):** the model put TABLE
> names in `next_scope` (`page_components`, `pages`, `site_work_items`). `next_scope` is the
> CODE-symbol channel (→ `Scope.Symbols`, sliced as bodies); the index-page *content* it wants
> is a DB read, which belongs in the verdict's `data_requests` (run read-only by `load_runtime`
> on loop-back) and/or `Scope.Tables`. If iteration 2+ keeps asking for the same rows without
> getting them, that channelling (likely a verdict-prompt tweak) is the next item — but confirm
> the re-scope first now that seeding is fixed.
>
> **Do NOT re-trigger §6D.** §6D is the code-INDEXER run (fills `code_symbols`, done — 436
> rows); its `orchestrate` envelope (owner/repo/ref/language, no symptom) is a different run
> from §6F's diagnosis envelope and does not affect the loop.
...
### 6E. Confirm the loop re-scopes (route → load_runtime → assemble_bundle) — [ ]  ← ACTIVE: seeding fix applied to diagnose_route; rebuild+redeploy, re-run, expect iterations ≥ 2
