# Mapping — the context tool as workflows, agents and contributors
...
| Review the change (build-time) | (none yet) | **build-time review contributors** (reuse, duplication, liability, morality, correctness) | **confirm before merge/deploy** |
| Monitor deployed sites | (separate layer) | **improvement-loop checkers + fixers** (`check_*.go`: deployed vs plan/spec) | — |
...
These are **contributors** — they feed and shape the build. Two kinds, and they're why separate containers help:

- **Context contributors** — code, data, runtime, standards. Each assembles one slice of the bundle from its own angle.
- **Build-time review contributors** — each reviews the *proposed change* from one angle, before it ships:
  - **reuse review** (is there existing code to build on — retrieval, before generation),
  - **near-duplicate review** (pre-merge; clone-check against the index),
  - **liability review** (does the output create legal exposure),
  - **morality review** (is this the right thing to put into the world, beyond mere legality — manipulation, exploitation of vulnerable users, misinformation, harmful claims, and whether a vertical should be served at all),
  - **correctness / standards conformance** (does it meet the constitution + matched standards).
  A review contributor raises a concern; resolving it revises the change or gates via a HITL stop. These are part of the build path.

> **Checkers are a different concept — not these.** In the web tool, *checkers* are the **improvement-loop** agents that continuously monitor **deployed** sites against the plan and spec (or other things) — the existing `check_*.go` family (broken nav links, orphan pages, unresolved sections, tool health, …), each often paired with a fixer. They run after deployment, on a loop, in the builder/operate layer — not in the context tool's build path. It's possible the initial builds reuse some of the same checks, but that's unconfirmed. *(To investigate separately — flagged, not settled here.)*

### The morality review — a configured standard, not a baked-in view

Morality is a distinct angle from liability: liability asks "will this get us sued or fined", morality asks "is this right", and the two overlap but aren't the same (some legal things are still wrong). It is deliberately **not** a single moral view hard-coded into the tool. Instead it is a review contributor that applies a **layered, configured standard** held in the active-config (a concern in the standards taxonomy, with provenance and effective-priority computed on read):
...
- **Front-end**: the admin side shows the resolved scope, the assembled bundle, the HITL decisions, and the build-time review results — extending the existing admin dashboard. Public/self-serve is the later extension.
