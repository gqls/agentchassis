...
- **DB-evidence stance (by design, not a gap)**: re-scope follows the CALL GRAPH
  for code and a vetted query CATALOGUE (`queryselect.go`) for DB evidence — the
  engine deliberately does NOT carry model-written `data_requests` (`Scope` and
  `AdvanceResult` have no such field; "the model never writes SQL"). The
  `runDataRequests` path in `diagnose_load_runtime` is therefore dormant and reaches
  beyond that boundary — drop it or make it a separate, explicitly-bounded
  capability.
- **Verify (real correctness item)**: `diagnose_route` writes `EncodeScope(Scope)`
  (an object) at `route.scope`, while `diagnose_assemble_bundle` reads it as a list
  via `ExtractStringListHelper`. Confirm the field path (`route.scope.symbols`) so
  loop-back re-scope flows; the route→assemble round-trip is not covered by the
  engine tests. The chassis also gathers runtime once + re-assembles code each
  iteration; the per-hypothesis catalogue the standalone runs is not yet wired into
  the assemble action (later parity).
