...
STATUS 2026-07-04: the §B3 dump arrived as an empty converted document —
re-upload requested as a .txt FILE (the working path). Option A image
deploying; post-deploy: NNN_rename_complete_keys_preferred.sql, then optional
diagnose run (warns gone; flattened response; parent has call_diagnoser).
