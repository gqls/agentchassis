...
- [x] §6C agent workflow-fix migration APPLIED (has_route=t, run_loop gone)
- [ ] §6C orchestrator workflow RESTORED (`NNN_restore_diagnose_orchestrator_workflow.sql`) — its move didn't land
- [x] §6C coordinator `next_step` override CONFIRMED (coordinator.go:1093 getNextStepFromResult)
- [ ] §6C four diagnose actions built + registered + deployed
- [ ] §6C VERIFY route.scope→assemble_bundle field shape (route.scope.symbols vs ExtractStringListHelper)
- [ ] §6C decide on diagnose_load_runtime.runDataRequests (drop vs keep as bounded capability)
...
**Not a gap — a deliberate boundary, plus one thing to verify.** Re-checking the
engine: model-written `data_requests` are NOT propagated by design. `Scope` and
`AdvanceResult` carry no `DataRequests`, `DecideStep` never forwards them, and
`queryselect.go` states the rule — DB evidence comes from a vetted, `\d`-verified
query CATALOGUE selected by hypothesis, NOT model SQL ("the model never writes
SQL"). So re-scope is code (call-graph) + the catalogue. Consequence: the
`runDataRequests` path in `diagnose_load_runtime` is dormant (nothing feeds it) and
reaches for the model-SQL capability the standalone deliberately avoids — decide to
drop it (match the catalogue boundary) or keep it as a separate, explicitly-bounded
chassis capability; either way it is not a functional gap.

VERIFY instead (this determines whether code re-scope actually flows on loop-back):
`diagnose_route` writes `EncodeScope(NextScope)` — an OBJECT `{symbols, tables,
runtime_site, ...}` — at `route.scope`, but `diagnose_assemble_bundle` reads
`route.scope` with `ExtractStringListHelper`, which expects a LIST. Confirm
`ExtractStringListHelper` unwraps `.symbols`, or set `loop_scope_field` to
`route.scope.symbols`. The route→assemble round-trip is NOT covered by the engine
tests (those exercise `Advance`/`DecideStep`, not the actions), so a mismatch would
not be caught automatically — check it by hand or add an action-level test.

Separately, the chassis loop gathers runtime ONCE (loop-back returns to
`assemble_bundle`, not `load_runtime`) and re-assembles code each iteration; the
per-hypothesis query catalogue the standalone runs each iteration is not yet wired
into `diagnose_assemble_bundle`. Runtime-once is fine (a completed run's rows don't
change); catalogue parity is a later enhancement, not a blocker.
