...
- [x] §6C diagnose seed migration WRITTEN (`NNN_seed_diagnose_agents.sql`)
- [ ] §6C seed APPLIED — the current blocker (0 rows until applied)
- [ ] §6C the four actions registered + verdict prompt pasted + deployed
...
### 6C. Seed the diagnose agent + workflow — [ ]  ← THE MISSING PIECE (do this next)

The 0-row results above are because no diagnose agent definition exists. The
deployed loop is an AGENT: a row in `agent_definitions` whose workflow drives
gather → verdict → route → [loop | emit]. Until that row exists, there is no agent
to route to and no workflow to run.

**`default_config` is confirmed as the workflow column.** The live reference
agents (build-dispatch-loop, site-work-orchestrator, domain-research-classifier)
all put their workflow in `default_config.workflow`; `task_workflow` /
`orchestrator_workflow` are NULL. Re-confirm at any time:
```bash
$PSQL -x -c "SELECT type, (default_config ? 'workflow') AS dc_has_workflow
             FROM agent_definitions WHERE is_active = true ORDER BY updated_at DESC LIMIT 10;"
```

**The seed is now written: `NNN_seed_diagnose_agents.sql`** (it did not exist
before). It seeds the `diagnostician` agent, modelled on those reference agents,
with the loop as ONE ITERATION PER ORCHESTRATION (full rationale in the file
header): `load_runtime → request_analysis → lookup_symbols → assemble → verdict →
route → [continue: spawn_next → call_next → emit | terminal: emit] → complete`.
Cross-iteration state (revised hypothesis, next_scope, data_requests, iteration+1)
travels in `input_data` via `call_next`'s `input_mapping` — not a workflow cycle
(the `loop` action is for-each, not while) and not a sub_workflow (001 §"Don't
create subworkflows in SQL"). Apply and verify:
```bash
$PSQL -v ON_ERROR_STOP=1 -f "$CHASSIS/<migrations path>/NNN_seed_diagnose_agents.sql"
$PSQL -c "SELECT type, category, agent_category, status, is_active,
                 (default_config ? 'workflow') AS has_workflow
          FROM agent_definitions WHERE type='diagnostician';"
# expect one row, has_workflow = t
```
Confirm before relying on it (all flagged in the file): the `request_repo_analysis`
/ `lookup_code_symbols` config keys; that `diagnose_route` (outputs
`{continue, iteration}`) and `diagnose_emit` (picks child vs this-iteration
verdict) exist and are registered; the verdict prompt is pasted into the `verdict`
step; and the re-invocation nesting (spawn+call same type) vs the flat alternative
(fire to the generic entry point + complete). `agent_category` must be in the
CHECK list (strategist|executor|analyst|integrator|coordinator|specialist) — the
seed uses `analyst`; `diagnose` lives only in the free-text `category` column.
Verify the row landed with a workflow:
```bash
...
**About `type` (this resolves the earlier "<the diagnose agent type from the row
above>" confusion).** `type` is the value in the `agent_definitions.type` column —
the unique string that identifies this agent definition. It is what the trigger's
`target_agent_type` matches, and what fills the `{type}` template in the agent's
`topics` (`system.agent.{type}.process`, `system.responses.{type}`,
`system.errors.{type}`). You CHOOSE it in the INSERT above (e.g. `diagnostician`).
The placeholder meant "use whatever `type` string your seed sets" — but until the
row exists there is nothing to read, which is why the `SELECT … WHERE
category='diagnose'` returned 0 rows. After seeding, that SELECT returns your type.

**Step 3 — register the four actions (Go).** Register the handlers the way the
existing actions are (`RegisterActionInputSpec` in each action's `init()` + the
handler map in `registry.go`, Category `diagnose`), then confirm and deploy:
```bash
cd "$CHASSIS"
grep -RnE "diagnose_(load_runtime|assemble_bundle|route|emit)" platform/orchestration/actions/registry.go
go build ./... && go test ./...
git add -A && git commit -m "diagnose: seed agent+workflow, register actions, verdict prompt" && git push
# GitHub Actions -> Backblaze S3
```
*DoD:* one diagnose agent row with `has_workflow = t`; the four actions in
`registry.go`; build/test green; image released.

> If you can share the drafted `NNN_seed_diagnose_agents.sql` (or one existing
> agent's `default_config`), I can author/review the real INSERT — the workflow
> JSON is the one thing I can't write blind.
...
### 6E. Confirm the workflow loop-back mechanics — [ ]  (after 6C)

With the agent seeded, run a scripted pass (§7.1) and confirm route→assemble
actually re-scopes (vs emitting once) by reading the step path:
```bash
...
# loop-back is real if execution_path shows: assemble -> verdict -> route -> assemble
APP="<diagnose agent app/deployment label>"
kubectl -n ai-persona-system logs -l "app=$APP" --tail=200 | grep -E "diagnose_route|diagnose_assemble_bundle|next_step|scope"
```
Also verify the migration footer mechanics: `diagnose_route` `next_step` override is
read under the STEP NAME; `diagnose_assemble_bundle` re-executes on the loop-back
with `route.*` visible; `{{.bundle.bundle}}` surfaces via input_fields; the
`call_agent` child-result key is correct.
*DoD:* `execution_path` shows a second `assemble` after `route`, not a single emit.
