...
TASK2="When a deployed page is rebuilt the pipeline reports success but re-deploys the old content unchanged: the page-content-writer's compiled page is silently dropped and the work item completes as a stub. Find where a completed workflow's result is extracted and why the writer's declared output key is ignored."
python3 -c "import json,sys; t=json.load(open('groundtruth_targets.json'))['tasks']; \
  task=sys.argv[1]; m=[x['id'] for x in t if x['task']==task]; \
...
go run ./cmd/eval_targets -truth groundtruth_targets.json -candidates /tmp/lex2.json -n 12
...
go run ./cmd/eval_targets -truth groundtruth_targets.json -candidates /tmp/sem2.json -n 12

# ── STEP 4 — FUSED:
go run ./cmd/fuse -in /tmp/lex2.json -in /tmp/sem2.json -json > /tmp/fused2.json
go run ./cmd/eval_targets -truth groundtruth_targets.json -candidates /tmp/fused2.json -n 12
```
