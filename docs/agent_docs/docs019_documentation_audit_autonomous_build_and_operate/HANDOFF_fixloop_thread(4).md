...
scopes resolved) — carry it into the project folder. THE PILOT BUG WAS
REPLACED 2026-07-07 (chrome got fixed — perishability lesson in the
worthiness test): the pilot is now the PLAN/DELIVERY INTEGRITY defect — nav
links to never-rendered pages — with a mandatory pre-check query and the
roadmap-phases root context (runbook "F0 PILOT — REPLACED" + "ROOT CONTEXT"
sections). Imagery is another chat's. First action: slice **F0.1** with
