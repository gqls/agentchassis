...
### Trap-proofed run (skinner-box ground truth, 2026-06-14) — USE THIS
...
python3 -c "import json,os; t=json.load(open('groundtruth_targets.json'))['tasks']; \
  task=os.environ['TASK']; m=[x['id'] for x in t if x['task']==task]; \
  print('OK — matches truth task id:', m[0]) if m else print('MISMATCH — \$TASK is not any truth task; fix before running')"

# 0. clean repo-root analysis (run from inside contextkit; -root arg is the repo):
go run ./cmd/analyser ~/projects/agentchassis \
   -exclude docs/ -exclude _archive/ -exclude test/ -exclude vendor/ > /tmp/chassis_clean.json
python3 -c "import json,re; ps=[f['path'] for f in json.load(open('/tmp/chassis_clean.json'))['files']]; \
  print('files:',len(ps),'| stale:',len([p for p in ps if re.search(r'\(\d+\)\.go|_archive/|go_files_old/',p)]), \
...
#   expect: stale 0, and BOTH decisive symbols' files present (else recall is meaningless)

# 1. LEXICAL:
go run ./cmd/resolve_targets -analysis /tmp/chassis_clean.json -task "$TASK" -n 25 -json > /tmp/lex.json
go run ./cmd/eval_targets -truth groundtruth_targets.json -candidates /tmp/lex.json -n 12
...
go run ./cmd/eval_targets -truth groundtruth_targets.json -candidates /tmp/sem.json -n 12

# 3. FUSED:
go run ./cmd/fuse -in /tmp/lex.json -in /tmp/sem.json > /tmp/fused.json
go run ./cmd/eval_targets -truth groundtruth_targets.json -candidates /tmp/fused.json -n 12
```
