...
### §7A.1 — "adapter staleness" PINNED (one probe to confirm): remote HEAD = stale DEFAULT BRANCH

Read this turn (adapter.go / analyse_action.go / github_source.go /
analyser-adapter.yaml, all uploaded): the adapter's CODE is exonerated —
the ref passes through VERBATIM (`GET {api}/repos/{o}/{r}/tarball/{ref}`,
"HEAD" only as the empty-ref default), there is NO cache (fresh temp dir per
call, caller removes), and empty GITHUB_API_BASE = https://api.github.com.
So on 2026-06-26 GitHub ITSELF resolved `tarball/HEAD` to 4c2c172. In a
remote repository, HEAD is a symbolic ref to the DEFAULT BRANCH — so the
repo's default branch on GitHub has not moved since 2025-07-14 while
development happens on another branch. Every component did what was asked;
the fault is repository configuration plus the assumption that remote HEAD
means "my latest work". (CORRECTION of the §7A note: the adapter path was
the messenger, not the cause.)

Confirm with ONE command (in $CHASSIS), plus two supporting:

```bash
git ls-remote origin HEAD                          # expect 4c2c172b... if pinned
git remote show origin | grep 'HEAD branch'        # names the default branch
git rev-parse --abbrev-ref HEAD                    # your actual working branch
```

Fix (both, in this order):
1. STRUCTURAL: GitHub → repo Settings → Branches → set Default branch to the
   active development branch. Fixes HEAD resolution for every consumer.
2. BELT-AND-BRACES: trigger envelopes carry an EXPLICIT ref (the branch name
   or a sha), never "HEAD" — reproducible regardless of repo settings.

**Consequence for §7C:** the §7B swap alone does NOT fix the reindex —
analyse_repo_local builds the IDENTICAL tarball/HEAD URL. Unpinned + ref:"HEAD"
would re-fetch 4c2c172. The §7C trigger MUST carry the explicit branch.

- [x] CONFIRMED 2026-07-02: `git ls-remote origin HEAD` = 4c2c172bb3faee…;
      `HEAD branch: main`; working branch `083_imagery`. Refined mechanism: the
      default branch IS the conventional `main`, but nothing has merged into it
      since 2025-07-14 — development runs on serial numbered feature branches
      and deploys from their pushes, so `main` (and therefore remote HEAD)
      never advances.
- [x] HEAD hygiene DECIDED (user, 2026-07-02): merge meaningful commits to
      main manually (main stays the default branch and becomes approximately
      truthful); envelopes ALWAYS carry an explicit branch/sha regardless.
...
4. TOKEN CHECK before triggering: analyse_repo_local reads env
   ...
   Failure mode is LOUD and immediate — analyse_repo_local errors before any
   fetch: "GITHUB_READ_TOKEN not set...". TOKEN DESIGN FACT (spawn_actions.go
   ~2503-2520, confirmed): injection is a secretKeyRef from Secret
   `personae-platform-secrets` key GITHUB_READ_TOKEN into the SPAWNED pod spec,
   gated by isRepoCloningAgent (currently: diagnose-agent only). The comment is
   explicit: "the spawning chassis pod never holds it" — so the earlier remedy
   "add the env to the chassis/standing deployment via passthrough" is
   WITHDRAWN for the chassis; the token must come from the Secret, scoped to
   the pod that clones.
   CORRECTED by run 6dfa37cd (2026-07-02): code-indexer is NOT spawned —
   `orchestrate`+agent_type is ADOPTED IN-PLACE by the generic chassis pod
   (log: agent_type="generic", pod agent-chassis-7c65fbdf64 executing
   request_analysis). Typed agent-<type>-<id> pods exist only where a workflow
   contains spawn_agent; code-indexer's has none. §6D worked in-place because
   the ADAPTER did the fetch. The gate addition remains right but is consulted
   at SPAWN time only:
       repoCloningAgents := []string{"diagnose-agent", "code-indexer"}
   in spawn_actions.go isRepoCloningAgent. No other change; no renames.
   WHY THIS IS NEW: §6D never needed the token in the indexer pod — the
   ADAPTER held it (its own Secret) and did the fetch. §7B moved the fetch
   INTO the code-indexer pod, so the pod now needs the secretKeyRef.
   Requires a rebuild of the image(s) that perform spawning (core-manager /
   chassis — and remote-job-spawner if it builds pod specs from this repo).
   VERIFY at run time on the spawned pod (guard the empty-match case — a bare
   `kubectl describe pod` with no name describes EVERY pod, which is how the
   analyser-adapter's env got mistaken for an indexer pod on 2026-07-02):
   `P=$(kubectl -n ai-persona-system get pods -o name | grep agent-code-indexer | head -1)`
   `[ -n "$P" ] && kubectl -n ai-persona-system describe "$P" | grep -A3 GITHUB_READ_TOKEN`

### §7B.1 — wrap the indexer in a spawning orchestrator (run 6dfa37cd fix)

Run 6dfa37cd failed exactly as the design dictates: in-place on the chassis pod,
os.Getenv(GITHUB_READ_TOKEN)="" (the chassis never holds it), loud pre-fetch
error. Fix = the proven diagnose-orchestrator pattern, mirrored:
`index-orchestrator` (spawn_indexer: spawn_agent code-indexer/role=indexer →
call_indexer: call_agent forwarding owner/repo/ref/language, timeout 1800 →
complete: result_from code-indexer_result). Migration:
`NNN_seed_index_orchestrator.sql` — CONFIRM `\d agent_definitions` first; the
INSERT copies version/agent_category/status from the diagnose-orchestrator row.
Trigger retargeted: TARGET_AGENT_TYPE='index-orchestrator'
(TRIGGER_code_indexer_v2.sh updated). Rejected: token on chassis (design
forbids); §7B revert to adapter (572-file Output reply is multi-MB over Kafka
vs ~1MB default max — fine at 69 files, probable failure now); bare k8s Job
(outside every-agent-is-an-orchestrator). Also patched the misleading error
text in analyse_repo_local_action.go (ONE string; no names changed) — it said
"the diagnose-agent pod must carry…" and misdirected this debugging session.

- [x] `\d agent_definitions` checked (pasted 2026-07-02): NO `name` column
      (display_name only); `category` varchar NOT NULL no-default (distinct
      from agent_category); UNIQUE (type,version); snapshots live in-table.
      v1 INSERT failed on these — migration REWRITTEN (v2) against the real
      schema, source-row SELECT filters is_snapshot/deleted_at.
- [x] v2 seed APPLIED 2026-07-02 (INSERT 0 1; verify: category='diagnose'
      copied, agent_category=coordinator, status=experimental, version=1,
      start_step=spawn_indexer, four-field input_mapping). category='diagnose'
      is a harmless copy (no CHECK on that column) — cosmetic tidy-up parked.
- [x] branch pushed (proved by the successful fetch: remote tip = 36710be)
- [x] RUN 93ba14e6 (orch b0365057): spawn_indexer spawned
      agent-code-indexer-65546fca-cssc8; call_indexer forwarded the envelope
      (reply to the PARENT's responses topic); analyse_repo_local SUCCEEDED on
      the spawned pod — no token error (gate + secretKeyRef verified end to
      end) — fetched 083_imagery @ 36710be, analysed 515 files in ~9s.
      index_symbols in flight (single long action ⇒ last_activity static by
      design; first run embeds EVERY symbol via one ollama pod: expect
      10–25 min, ceiling 1800s; a timeout RESUMES on re-trigger — upserts
      persist and unchanged content hashes are skipped).
      515 vs the local find's 572: working tree vs pushed tip + analyser's
      testdata//dup-suffix skips — expected.
- [ ] child COMPLETED; then §7C verification (expect commit 36710be,
      count(DISTINCT path) ≈ 515)

- [x] real code-indexer default_config dumped and pasted (2026-07-02)
- [x] migration written (REVERT block present)
- [x] migration APPLIED 2026-07-02 (snapshot 971da9c9; verify returned
      analyse_repo_local + pin false, analysis_field="repo_analysis",
      complete=["index_result"], timeout 1800)

## §7C — Reindex + verify coverage — [ ]

### Ref strategy (how REF stops being hardcoded)

IMMEDIATE (works today, zero system change) — derive it in the trigger script
from the checkout you are triggering for, instead of a literal:

```bash
REF="${REF:-$(git -C "$CHASSIS" rev-parse --abbrev-ref HEAD)}"   # e.g. 083_imagery
# tarball/<branch> resolves the REMOTE tip of that branch — push first.
```

STRUCTURAL A (recommended next) — CI-triggered indexing: the GitHub Actions
workflow that builds/deploys already knows the exact sha; add a post-deploy
step (the runners live IN the cluster — github-actions-runner pods — so kcat
to the kafka bootstrap works) that sends the SAME §6D envelope with
REF="${GITHUB_SHA}". Then index commit == deployed commit BY CONSTRUCTION and
"choosing a ref" ceases to exist. (Indexing is a safe writer — this is not the
auto-diagnosis trigger that stays human-gated.)

STRUCTURAL B (composable with A) — make HEAD truthful: a post-deploy CI step
fast-forwards main to the deployed sha (`git push origin ${GITHUB_SHA}:main`).
main becomes a "currently deployed" pointer, so remote HEAD is correct for
EVERY consumer forever. Changes main's meaning — user's call.

REJECTED: resolving "most recently pushed branch" inside analyse_repo_local
via the GitHub API — an N+1 branch walk, and latest-pushed ≠ deployed (an
experimental branch would poison the index). The push/deploy EVENT already
knows the sha; ask the event, not a heuristic.

Diagnose side needs nothing: pin_to_index_commit(true) overrides its ref
whenever the index is non-empty, so it follows the new index commit.

PRE-TRIGGER GATES (all three, in order):
- [x] §7A.1 confirmed (ls-remote = 4c2c172b…) — ref strategy above adopted
- [x] envelope REF explicit: user's trigger sets REF='083_imagery' (reviewed;
      corrected copy at TRIGGER_code_indexer_v2.sh — same envelope, accurate
      watch lines; the pasted script's classifier-era greps were cosmetic only)
- [x] spawn gate line deployed (replicaset 7c65fbdf64 — run 6dfa37cd ran on it)
- [ ] §7B.1 gates (seed index-orchestrator; push branch; retrigger via the
      orchestrator) — see §7B.1

Re-trigger the code-indexer (the §6D kcat trigger block in the old RUNBOOK still
applies — NEW correlation_id; REF='<branch>' explicitly; read outcome by
correlation_id only). Then verify:
...
DURING-RUN PROGRESS PROBE (distinguishes work from hang — run twice, 1 min apart):
```sql
SELECT commit_sha, count(*) FROM code_symbols
WHERE repo = 'gqls/agentchassis' GROUP BY 1;
```
Growing 36710be count ⇒ working (measured 2026-07-02: ~5 symbols/sec).
NOTE the old-commit count ALSO decays during the run — the upsert flips
overlapping repo/path/symbol identities in place (commit_sha 4c2c172 ->
36710be); the residue left at the old commit = symbols deleted/renamed since
2025, removed by the end-of-run prune. Static counts ⇒ check the
ollama-adapter log (should scroll with embed requests) and the
agent-code-indexer pod log.

Expected scale: ~515 files ⇒ symbol count in the low thousands; the first run
