...
> **UPDATE 2026-06-17 — the §4/§5/§6 `diagnose_run` recommendation is SUPERSEDED.**
> This doc's STEP-ZERO findings (§0), triggering (§2), and file split (§3) stand and
> are correct. But where §4-§6 recommend a single `diagnose_run` action that calls
> the verdict internally, that was changed: the verdict is now its OWN
> `execute_llm_prompt` step (more observable; one responsibility per step), and the
> loop is driven by a `diagnose_route` controller action that returns a `next_step`
> override. The current, drafted shape is in `RUNBOOK_design_diagnosis_loop.md` §4b
> and the `chassis-drafts/` files (diagnose_load_runtime, diagnose_assemble_bundle,
> diagnose_route, diagnose_emit + the migration). Read those for the build; read §0-§3
> here for the reasoning that still holds. `diagnose_run` below = read as
> `diagnose_route` (a router, no output_field), with the verdict as a separate step.
