...
>
> **§6G is now the focus — and this run did NOT pass it.** The full trail (now readable) shows
> the loop abstained three times (UNVERIFIABLE×3) trying to verify "is the index content a stub"
> by querying section content: iter 1 re-scoped to `save_page_sections`/`plan_sections` (good —
> followed evidence to code); iter 2 issued a `data_request` against `page_sections` — which
> **does not exist** (`SQLSTATE 42P01`), correctly noted; iter 3 finally named real tables
> (`page_components`, `pages`) but `evidence-not-growing` fired (its citations were all
> previously-seen rows) and stopped the loop **one iteration before** those good queries would
> have run in `load_runtime`. It never REFUTED, so it never re-pointed the hypothesis upstream
> toward the build/coordinator path — it never reached `resolveResultSpec`.
>
> **§6G work items (do NOT fix blind — pick one, change one thing, re-run):**
> 1. **`evidence-not-growing` is too eager w.r.t. in-flight data_requests.** A verdict that
>    issues a NEW (unseen) `data_request` is making progress, but if its citations are all old
>    the guard counts it as spinning and stops — one iteration before the answer arrives
>    (exactly what truncated this run). Candidate: count a new data_request as forward progress
>    (thread a request-hash into the guard memory). Structural; this is the likely first step.
> 2. **The model can't find the content table from the bundle schema** (guessed `page_sections`;
>    real candidates are `page_components` / `content_items`). The bundle's schema section may
>    need to surface the content tables, or the seed scope should include them.
> 3. **It stays on the symptom hypothesis** (abstains, never refutes → no upstream pivot). May be
>    the raw-symptom seed keeping it in "verify the symptom" mode; a prompt nudge or mechanistic
>    seed could help it pivot toward the cause.
>
