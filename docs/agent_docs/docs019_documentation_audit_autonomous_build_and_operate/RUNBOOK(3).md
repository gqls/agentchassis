...
- [ ] §6C **diagnose agent + workflow SEEDED** ← the current blocker (0 rows today)
...
**Step 1 — find WHERE a workflow lives (reuse the existing pattern, don't assume).**
`agent_definitions` has several candidate columns (`default_config`,
`task_workflow`, `orchestrator_workflow`, `orchestration_workflow`). Inspect a
working agent to see which one its workflow actually occupies, and mirror that:
```bash
$PSQL -x -c "SELECT type,
       (default_config ? 'workflow')      AS dc_has_workflow,
       (task_workflow IS NOT NULL)         AS has_task_wf,
       (orchestration_workflow IS NOT NULL) AS has_orch_wf,
       (orchestrator_workflow IS NOT NULL)  AS has_orchr_wf
     FROM agent_definitions
     WHERE is_active = true
     ORDER BY updated_at DESC LIMIT 10;"
# then dump ONE working agent's workflow to copy its shape:
$PSQL -Atc "SELECT default_config FROM agent_definitions WHERE type='<an existing agent type>';" | jq .
```
(The design notes say the diagnose workflow goes in `default_config`; this step
confirms it against how live agents are actually seeded before you write the INSERT.)

**Step 2 — apply the seed migration.** If you have the drafted
`NNN_seed_diagnose_agents.sql` (from the chassis-drafts set), apply it:
```bash
$PSQL -v ON_ERROR_STOP=1 -f "$CHASSIS/<path>/NNN_seed_diagnose_agents.sql"
```
If you are writing it fresh, the row shape is (fill the workflow JSON from the
design / the column Step 1 revealed — this is a SKELETON, not a runnable INSERT):
```sql
INSERT INTO agent_definitions (type, display_name, category, status, is_active, default_config)
VALUES (
  'diagnostician',                 -- the agent's TYPE (see "About `type`" below)
  'Diagnosis Loop',
  'diagnose',                      -- free-text category column; the lookup uses this
  'experimental',                  -- must be in the status CHECK list
  true,
  '{ "workflow": { "steps": [
        {"name":"load_runtime","action":"diagnose_load_runtime", ...},
        {"name":"assemble","action":"diagnose_assemble_bundle", ...},
        {"name":"verdict","action":"execute_llm_prompt", "prompt_template":"<verdict prompt, JSON-escaped>", ...},
        {"name":"route","action":"diagnose_route", ...},
        {"name":"emit","action":"diagnose_emit", ...}
  ] } }'::jsonb
);
-- NOTE: agent_category (if set) must be one of strategist|executor|analyst|
-- integrator|coordinator|specialist (CHECK constraint) — 'diagnose' is NOT valid
-- there; it belongs only in the free-text `category` column above.
```
