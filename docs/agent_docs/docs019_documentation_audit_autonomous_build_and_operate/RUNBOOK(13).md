...
echo "  kubectl -n ai-persona-system logs -f -l app=agent-chassis    --tail=500 | grep '$CORRELATION_ID'"
```
If the runtime bundle comes back empty, check `load_runtime`'s `site_id`/domain field configs
against the `input_data` keys actually sent (`site_id` / `runtime_site`) before assuming no
evidence exists — a 0 here is usually a selector miss, not an absence.
