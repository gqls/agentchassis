...
4. TOKEN CHECK before triggering: analyse_repo_local reads env
   ...
   fetch: "GITHUB_READ_TOKEN not set (the diagnose-agent pod must carry the
   read-only GitHub token; see spawn_actions.go isRepoCloningAgent)". Injection
   is gated by agent TYPE in spawn_actions.go's isRepoCloningAgent, which very
   likely lists only diagnose-agent. Remedy if the indexer runs as a spawned
   pod: add 'code-indexer' to isRepoCloningAgent (small code change + rebuild);
   if it executes on a standing deployment instead, add the env there.
