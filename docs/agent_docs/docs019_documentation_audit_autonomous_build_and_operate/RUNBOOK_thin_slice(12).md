...
```bash
# ── 0. Copy the drafts into the repo per the README map. The one file NOT
#      drafted: build/docker/backend/analyser-adapter.dockerfile — copy
#      thunder-adapter.dockerfile and swap the binary/cmd name.
...
kubectl -n kafka get kafkatopic | grep analyser
# 1c. Registry: paste registry_insertions.md into GlobalActionRegistry after
#     training_data_export. Decide Category "code" (new) vs "analysis" (existing).
# 1d. Agent INSERT — schema first, ALWAYS:
#     \d agent_definitions   (NOT NULL / defaults / CHECK incl. agent_category)
#     snapshot_agent is not needed (new row); then run
#     NNN_create_code_indexer_agent.sql with your migration number.
...
#   FROM code_symbols WHERE repo = '<repo>';
# SELECT kind, count(*) FROM code_symbols WHERE repo='<repo>' GROUP BY kind;
...
**Settle before indexing in anger:** the `code_symbols.repo` value convention —
`agentchassis` vs `gqls/agentchassis` vs tenant-prefixed. It keys the table, the
prune, and every lookup filter; pick once and write it into the indexer trigger
and the lookup configs together.
