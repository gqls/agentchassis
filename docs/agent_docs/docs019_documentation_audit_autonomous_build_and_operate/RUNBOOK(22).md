...
>
> **DONE:** the in-process analyse path is BUILT + DEPLOYED to production (the image
> carries `analyse_repo_local` + the lifted `internal/reposource` fetcher; registry
> confirmed). §6A–6D are effectively closed: `code_symbols` is populated (436 rows —
> §6D), the four diagnose actions are registered + deployed (§6C), the `next_step`
> override is confirmed (§6C), and `loop_scope_field = route.scope.Symbols` (§6C
> field-shape check).
>
> **THE GATE RIGHT NOW: the workflow-swap migration** `NNN_swap_analyse_repo_to_local.sql`
> flips `analyse_repo` from `request_repo_analysis` → `analyse_repo_local`. **Deploying
> the image does NOT apply it** — it is a separate DB change. Until it is applied, the
> diagnose-agent still calls the analyser cross-pod and `assemble_bundle` fails with
> `repo root not found at "repo_analysis.root"` — which is exactly the latest failed run
> (orchestration `e6fd0bff`, FAILED at `assemble_bundle`). That run is also the newest by
> `created_at` — the lookup we don't trust on its own; it may simply predate the swap. So
> **don't act on it.** Instead:
>
> **NEXT — two checks, in order:**
>
> **1. Is the workflow swapped? (decisive)**
> ```bash
> $PSQL -c "SELECT default_config #>> '{workflow,steps,analyse_repo,action}'                        AS action,
...
> ```
> - `action = request_repo_analysis` → **NOT applied.** Run `NNN_swap_analyse_repo_to_local.sql`
>   (it snapshots first), re-check, then go to step 2.
> - `action = analyse_repo_local` (pin=true, output_field=repo_analysis) → already applied;
>   `e6fd0bff` was a stale pre-migration run. Go straight to step 2.
>
> **2. Trigger §6F FRESH and read by `correlation_id` (NEVER `LIMIT 1`).** Use the §6F
> envelope below. Capture the `correlation_id` you send and filter on it:
> ```bash
> $PSQL -c "SELECT orchestration_id, status, current_step, error
>           FROM orchestration_states
>           WHERE correlation_id = '<the-id-you-sent>'
>           ORDER BY updated_at DESC;"
> ```
> Confirm the new action fired in the diagnose-agent pod logs (NOT logger.Debug — Info):
> ```bash
> kubectl -n ai-persona-system logs -l app=diagnose-agent --tail=200 | grep "analyse_repo_local"
> # expect: "analyse_repo_local: analysed in-process" with root=<a /tmp/... path> + file_count>0
> # (or a clear analyse_repo_local error — which would fail the run at current_step=analyse_repo, not assemble_bundle)
> ```
>
> **Do NOT re-trigger §6D for this.** §6D is the code-INDEXER run (`request_repo_analysis`
> → `index_code_symbols`) that fills `code_symbols` — already done (436 rows). Its
> `orchestrate` envelope (owner/repo/ref/language, **no symptom**) is a DIFFERENT run from
> §6F's diagnosis envelope. Re-indexing does not create the diagnose-agent's local checkout
> and will not fix `repo root not found`; the `analyse_repo_local` swap is what does.
>
> **AFTER a run gets past `assemble_bundle`:** that ticks §6F → then watch the loop-back for
> §6E (route → load_runtime → assemble_bundle with an advanced scope) → then §6G (the eval
> gate: does it reproduce the gamesdesign REFUTE→REFUTE→CONFIRM and abstain when unsettled).
