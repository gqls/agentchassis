...
- **Clone with go-git, not a shelled `git` binary (turn 26).** Pure Go, no chassis image change
  — keeps the change in code, avoids a Dockerfile edit. Credential wiring is identical either way.
- **Diagnose-agent FULLY self-contained: clone once, analyse in-process (turn 26).** Not "clone
  for bodies AND still call the analyser for the graph" (two clones + a cross-pod hop) — clone
  once and run `analysis` in-process for the call graph + spans, read bodies from that checkout.
  `analyse_repo` stops being a cross-pod call; `repoRoot` becomes a real local path. DEPENDS ON
  `analysis` exposing an in-process entrypoint (AnalyseRepo(root)->Output); else export one
  (chassis change) or fall back to the 2-clone variant. `lookup_symbols` still reads the
  `code_symbols` index for retrieval seeding — self-contained = no runtime analyser-adapter call,
  not independence from the shared index.
- **Clone + slice bodies at the SAME commit the index was built on (turn 26).** Pin
  `repo_analysis.commit_sha` so retrieved symbols and sliced bodies can't drift if a re-index
  lands mid-run. Cheap, correct.
