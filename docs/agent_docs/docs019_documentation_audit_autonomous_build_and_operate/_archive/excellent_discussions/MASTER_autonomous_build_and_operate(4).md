...
### 6.6 Cross-cluster operation (added wrinkle)

Provisioning may now target not only a VM-with-nginx (done before) but a **third-party-hosted Kubernetes cluster** (GCP/AWS spot), with cross-cluster comms and API/tunnel-driven DB access. This is already designed and partly built (`FOCUS_multi_cluster_dispatch_mvp`): a `dispatch_agent` action + `remote-job-spawner` consuming `system.dispatch.requests`, agents talking back on the **same Kafka** via `parent_responses_topic`, and remote DB access via a VPN tunnel + local PgBouncer at the **same in-cluster DNS name**.

**Why the theory barely expands.** The design deliberately makes the cluster boundary invisible to the chassis — *the agent code stays identical, no federation* (per that doc). At the control-loop layer (§7) a remote leaf task behaves like a local one. So this is mostly new **capabilities and map rows**, not new theory.

New capabilities and where they sit:

| Capability | Verifiability | Containment | Cascade tilt | Tier |
|---|---|---|---|---|
| Provision a remote cluster (spot) + spawner | Strong (loopback / dispatch contract / `psql-test` are explicit checks) | Medium (greenfield, but spot can vanish, and it reaches the primary) | reuse (the doc *is* the known-good runbook) + canary (loopback) | B |
| Establish cross-cluster comms / tunnel + per-cluster DB credentials | Medium | Weak (security surface; reaches primary DB) | known-good baseline + HITL | C |

**Why autonomous + verification got harder (specifically):**
- **Defect-vs-partition ambiguity.** A cross-cluster task failure is now ambiguous — the work may be wrong, or the tunnel/Kafka/DB path may have failed. Verification must distinguish these or infrastructure flakiness poisons the trust signal (handled in §7.5).
- **Spot volatility.** A remote cluster can be reclaimed mid-task; a failure may be lost compute, not a bad agent.
- **Containment leaks toward the primary.** The remote cluster shares Kafka and reaches the primary DB, so a remote failure has a path to affect the primary (connection exhaustion, the Gap-A spawner race). Lower containment than local build → lower ratchet ceiling for these capabilities.

**Placement:** these are operate-tier, so they enter late (Phase 6, §8) as **known-good + HITL** capabilities, not early autonomy targets. The dispatch doc's runbook and disaster entries *are* the known-good content the cascade's tier-1 reuse draws on. The same Kafka dispatch + `agent_dispatch_log` + PgBouncer/Prometheus exporters are the cross-cluster monitoring — i.e. derived state flowing back into the §7.6 ops loop, which is exactly the "use it and monitor it until it works as we want" intent. Can be built separately from the critical path without disturbing it.
...
- **Infrastructure-attributed failure** (the one refinement cross-cluster forces, §6.6) → before a failure updates the ledger, the verification harness checks the transport/compute path (tunnel up? spot instance alive? Kafka/DB reachable? — all already monitored). A failure traceable to a partition or a reclaimed spot instance is **quarantined from the trust signal**, not counted as capability failure. Without this, transient cross-cluster flakiness would de-graduate capabilities that are actually fine — poisoning the ratchet. This is a refinement to the verify/feedback step, not a new layer.
...
From today (HITL via chat) to progressively autonomous, reusing the base, sequenced by where reliability is cheapest to earn.

### 8.1 Principle: one capability through the whole machine, under its own ratchet

- **Vertical slice, not horizontal layer.** Walk a *single* capability end-to-end through route→produce→verify→gate→feedback before generalising. If the machinery has a flaw, it surfaces on one cheap capability, not across ten.
- **Dogfood the ratchet.** The automation is itself adopted under the trust model it implements: each new piece starts at `confirm-every` and graduates only on evidence. The adoption plan *is* the trust ratchet applied to the automation itself.
- **Continuous with today, not a replacement.** The current chat-driven workflow is simply the starting state: `confirm-every`, human-in-chat. Adoption progressively automates it; nothing working is thrown away.
- **First capability = writing Go actions.** Strongest verifiability + containment, cheapest failure, and most foundational (other capabilities depend on actions).

### 8.2 Phases

Each phase has an **entry criterion** (the evidence that earns the move — the ratchet's own gates, recursively) and delivers **standalone value**.

**Phase 1 — Verification harness for Go actions.**
Build the build/vet/test runner as an action/adapter. Still `confirm-every`, human decides — but now with ground truth instead of eyeballing.
*Value:* immediately fixes the derived-staleness pain in the current chat workflow (you get a live build/test result, not a guess), and starts logging outcomes. *Entry:* none — start here.

**Phase 2 — Trust ledger + the full loop on Go actions, pinned at `confirm-every`.**
Stand up the ledger (a table) and a minimal cascade router (for actions: reuse-from-library vs generate+verify). Run the whole §7.2 loop, but every produced action still goes to the human — now arriving verified, with the router's reuse/generate choice made, and the human's confirm/reject recorded as evidence.
*Value:* proves the machine end-to-end on the safest capability while the human is still fully in control. *Entry:* Phase 1 harness reliable.

**Phase 3 — First graduation.**
When the ledger shows action production is reliably approved and verification reliably passes, graduate the gate: `confirm-every → confirm-exceptions → notify`. First real autonomy, on the safest capability, evidence-backed and reversible. The known-good library begins filling from verified recurring solutions.
*Value:* the first capability runs with reducing supervision. *Entry:* ledger evidence threshold met (sustained approval + passing verification, no recent regressions).

**Phase 4 — Widen across Tier A.**
Repeat the proven pattern for SQL workflows, component-structural, observability config, rollback. Each gets its verification check + ledger entry and walks the same `confirm-every`→graduate path. Horizontal expansion on proven rails.
*Value:* the safe build capabilities climb. *Entry:* Phase 3 graduation held without de-graduation.

**Phase 5 — The autonomous control loop (the weak part).**
Wire the orchestrator to decompose a real multi-capability sub-goal and let leaf tasks run the loop at their *earned* trust levels without a human orchestrating each; the mediator handles cross-area; competition (tier 3) gets real use for capabilities with decent-but-not-deterministic verification.
*Value:* §7 running for real — multi-capability autonomy on Tier A/B. *Entry:* several Tier-A capabilities graduated and stable.

**Phase 6 — Operate, partial.**
Bring ops in via the same loop driven by derived state. Build the ops verification harness (canary, infra rollback, detection) — the thin part of the base. Rollback autonomous early; detection/diagnosis-as-suggestion; remediation/reshard/security gated on known-good runbooks.
*Value:* runs what was built. *Entry:* build-side autonomy (Phase 5) stable; do not let build success breed overconfidence about ops.

### 8.3 Why this order

Lowest-risk proof first (actions: cheap failure, strong verification), machinery de-risked on one vertical slice before any breadth, autonomy wired only on proven rails, and ops — the thinnest part of the base and the weakest capability set — last and partial. The end goal (build vonc.com from scratch) is reached as enough capabilities graduate; it is not a separate build.

### 8.4 Where it could stall

- **The cascade router's tier choice** is the loop's least-bounded step (a wrong tier is itself an uncertainty the loop doesn't catch). Mitigation: conservative-by-default, itself ledger-gated, and human-reviewed under `confirm-every` in the early phases.
- **Evidence quality.** If "success" is ambiguous (action builds but is subtly wrong), graduation is premature. Mitigation: verification must be *tests*, not just compile, before its signal is trusted; the bidirectional ratchet catches the regressions that slip through.
- **The ops thin spot** (Phase 6) needs the most new building; sequenced last for that reason.

### 8.5 The two goals on one ramp

The early phases (1–2) are exactly the "improve my current workflow" goal — verified, live-state-grounded development in chat. The later phases (3–6) are the "leap" to autonomy. They are the *same* investments in sequence: the verification harness and trust ledger that make today's chat workflow reliable are the first rungs of the autonomy ratchet. There is no fork between improving the workflow and automating it — improving it *is* the first stretch of automating it.

---

## 9. One-line state

The bottleneck is trust, not capability. Automation is a per-capability ratchet driven by a reliability cascade (reuse → generate+verify → compete+judge → HITL), supported by four documentation layers (standards, context substrate, known-good library, trust ledger) and the existing governance. Capabilities are mapped by verifiability + containment (§6): Tier A climbs fast, Tier C stays gated. The control loop (§7) wraps the unchanged orchestrator — route, produce, verify, gate, apply, feed back — with the trust ledger as the single master knob and a bidirectional ratchet for safety. Adoption (§8) walks one capability (Go actions) through the whole machine under its own ratchet, then widens across Tier A, then wires the autonomous loop, then partial ops — with the early phases doubling as the current-workflow improvement. Next: detail a chosen phase, or pressure-test the cascade router (the least-bounded step).
