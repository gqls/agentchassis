...
How standards and the tree change over time. **Ownership roles and the confirm-not-initiate model are detailed in `FOCUS_standards_curation_and_governance.md`; this section is a summary.**

### 4.1 Locking and freshness
Standards are locked (the `permanent` lock pattern) — agents never silently rewrite a rule's meaning. Spawn-fresh agents always read the current `active` version, so there is no stale-standard-baked-into-a-long-running-process problem.

### 4.2 Confirm-not-initiate flow
The decision's *reasoning* is agent-led; the human *confirms*. A concern curator that hits a pattern (via violations/validator failures tagged to its atoms) drafts a change and produces a **decision package** — summary, tradeoffs/impact, explicit choices (including reject and defer). A human confirms, with rigor scaled to stakes (a `blocker` rule or a large `referenced-by` impact gets more than a one-click confirm). Acceptance writes a new `version` and flips the old to `deprecated` (never deletes). The human owns the rule; the curator owns vigilance and drafting — see the curation doc.
