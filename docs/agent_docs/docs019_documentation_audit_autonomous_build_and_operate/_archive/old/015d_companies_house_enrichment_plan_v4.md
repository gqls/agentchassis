...
## Status: Implemented — Transitioning to Bulk Collection

The CH enrichment pipeline is live, running as a standalone agent on the `business-intel` pod. The initial per-business search approach works but has a ~27% match rate limited by name mismatches and API costs. A revised architecture using bulk collection of all SIC 75000 companies into a local table, followed by local matching, is planned. This will improve match rates and eliminate speculative API calls.
...
**Current approach stats (as of March 18, 2026):**
- 138 businesses processed, 37 matched (26.8% match rate), 100 no_match, 1 name_search
- 2,725 remaining unenriched
- Match rate is lower than expected due to name cleaning issues (now fixed) and corporate branches

**Planned transition:** The bulk collection approach will replace per-business searching. Once `ch_vet_companies` is populated with all 5,780 active SIC 75000 companies, matching becomes a local operation and the match rate should improve substantially since we can try multiple matching strategies without API cost.
...
## Revised Architecture: Bulk Collection + Local Matching (Planned)

The current per-business search-and-score approach has structural limitations. Many practices trade under names that don't match their CH registration (e.g. "Castle Veterinary Group" registered as "J.A. Smith Veterinary Services Ltd"). No amount of name cleaning or scoring tuning can find matches that require knowing the connection in advance.

### The Problem with Search-Per-Business

- One search, one chance — if the name cleaning or CH search ranking misses, it's a permanent no_match
- Each tuning iteration requires clearing no_match records and re-burning API quota
- ~8,000 API calls for 2,700 businesses at 15s delay = ~33 hours of API time
- Corporate branches (CVS, IVC, Medivet) will never match by local practice name
- No discovery of vet companies we don't already know about

### The Solution: Local Mirror + Matching

The CH advanced search API supports SIC code filtering:
```
GET /advanced-search/companies?sic_codes=75000&company_status=active&size=100&start_index=0
```

**Confirmed:** 5,780 active companies with SIC 75000 (veterinary activities) as of March 2026. Pagination works at 100 per page = 58 API calls to collect the complete universe.

### New Flow

**Phase 1 — Bulk collect (one-time, then monthly refresh)**

Paginate through all SIC 75000 active companies. Store in a new `business_intel.ch_vet_companies` table:
...
CREATE TABLE IF NOT EXISTS business_intel.ch_vet_companies (
    ...
    sic_codes          TEXT[],
    address            JSONB,          -- from search result "registered_office_address"
    postcode           TEXT,           -- extracted for indexing
    postcode_prefix    TEXT,           -- outward code for matching
...
CREATE INDEX idx_ch_vet_postcode ON business_intel.ch_vet_companies (postcode_prefix);
CREATE INDEX idx_ch_vet_unmatched ON business_intel.ch_vet_companies (matched_business_id) WHERE matched_business_id IS NULL;
```

Collection cost: 58 API calls, ~15 minutes with rate limiting. Run as a scheduled task on business-intel pod.

**Phase 2 — Local matching (no API calls)**

For each business in our DB, search `ch_vet_companies` by postcode prefix + name similarity. Pure SQL/Go, runs in seconds, unlimited retries with different strategies:

- Exact postcode prefix + name contains
- Exact postcode prefix + word overlap
- Nearby postcode + strong name match
- Fuzzy matching for tricky cases
- Could even use LLM-assisted matching for low-confidence candidates

The scoring logic is the same as now, but operating against a local table instead of live API results. Matching can be re-run freely as the algorithm improves.

**Phase 3 — Detail fetch (API calls only for confirmed matches)**

Only for matched companies: fetch officers, PSC, filing history, accounts. Estimated ~1,500-2,000 matches out of 5,780 (the rest are equine, agricultural, or already in our DB under corporate parent names).

This is the only step that needs API calls going forward, and it's targeted — no wasted calls on speculative searches.

**Phase 4 — Discovery (new leads)**

Any `ch_vet_companies` row that doesn't match a business in our DB is a potential new lead — a registered vet company we haven't found via web scraping. A separate workflow could:
- Look up the company's registered address
- Search for a website associated with that address or company name
- Feed discovered practices back into the web scraping and verification pipeline

This is free lead generation — the data is already collected in Phase 1.

### Cost Comparison

| Approach | API calls | Wall time | Re-runnable |
|----------|-----------|-----------|-------------|
| Current (search per business) | ~8,000 | ~33 hours | No (burns quota) |
| Bulk collect + local match | 58 (collect) + ~4,000 (fetch details) | ~15 min + ~17 hours | Matching is free |

The bulk approach front-loads a small collection cost, then makes matching essentially free. Detail fetching is targeted at confirmed matches only.

### Migration Path

1. Build `ch_vet_companies` table and bulk collection action
2. Run collection — 15 minutes to populate
3. Build local matching logic (reuse existing scoring, just against local data)
4. Run matching — validate results against existing `companies_house_data` matches
5. Once validated, switch the scheduled task from search-per-business to local-match-then-fetch
6. The existing `companies_house_data` table and `store_ch_enrichment` action remain unchanged — they store the same enrichment data regardless of how the match was found
...
10. **Name cleaner stripping too much** — The original cleaner stripped "Veterinary Surgery", "Veterinary Centre", "Veterinary Group" etc. as complete phrases, leaving just the practice name (e.g. "Castle Veterinary Group" → "Castle"). This removed the most valuable search signal. Revised to keep "Veterinary" and "Vets" while stripping only generic suffixes (Ltd, Group, Surgery, Clinic, Centre, Practice, Hospital) and parenthetical locations. "Castle Veterinary Group" now searches as "Castle Veterinary" instead of "Castle". The principle: search generality (broad terms to find candidates) then scoring specificity (narrow to the right match).
