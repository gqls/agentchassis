...
| Field | Type | Purpose |
|---|---|---|
| `source` | enum | How the entry was obtained: `inspection` / `probe` / `inferred` / `supplied` / `doc_sourced` |
| `source_ref` | jsonb | Pointer to the source artifact: file path + line range, probe id, doc citation, etc. Shape varies by source. |
| `confidence` | enum | `high` / `medium` / `low`. Set by the producing agent. |
| `status` | enum | Lifecycle: `proposed` / `active` / `deprecated` / `archived` |
...
| `version` | int | Monotonic per-entry version. Bumped on edit. |
| `supersedes` | uuid, nullable | Previous version's id, for lineage. |
...
| Field | Type | Purpose |
|---|---|---|
| `tenant_id` | uuid PK | Identifier. For our own use, a singleton value. |
...
- **No agent writes `active` directly** — this is the operational meaning of confirm-not-initiate. Status transitions are gated.
