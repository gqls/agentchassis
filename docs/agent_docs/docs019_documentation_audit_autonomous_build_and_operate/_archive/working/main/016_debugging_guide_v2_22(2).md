...
so the plan name and the realised name disagree.

**LLM cleared, and the root cause is a `page_type` vocabulary mismatch (confirmed
2026-05-26 from `response_text`).** The 16:27 `plan_site` output emitted the game
faithfully by *name* but re-typed it:
```json
{ "name": "game-auto-battler", "page_type": "tool", "sections": [], ... }
```
`out_kept_game_prefix=t, out_renamed_to_tool=f, out_double_prefixed=f`,
`output_tokens=4388` (no truncation). So the LLM did not rename — it assigned
`page_type: tool`, and the `tool` branch of the §9 canonicalisation produced the
`tool-*` names. The reason it re-typed is structural: the adopted page is
`page_type = game` (adoption writes it; the DB allows it), but the deployed
`plan_site` prompt's Canonical Page Types list is
`index | content | landing | entity-directory | entity-page | tool | blog-index | blog-post`
— **no `game`**. The planner cannot emit `page_type: game`, so every adopted game
is forced to `tool`. That mis-type, not a rename, is the upstream trigger.

**Still to read from code (not assert):** the two downstream steps produce
*different* tool-names from the same `tool`-typed page — `write_site_plan` stored
`tool-auto-battler` in `site_plan_pages`, `sync_pages_to_db` realised
`tool-game-auto-battler` in `pages`. Both follow from `page_type: tool`, but the
strip-vs-no-strip divergence between `WriteSitePlanAction` and `sync_pages_to_db`
needs the canonicalisation code, not inference.

**Final deployed state (render finished 2026-05-26 ~22:24).** The duplication
reached the live site: `page_rerender` ran for all five adopted games
(`game-auto-battler` … `game-pathfinding`) *and* their `tool-game-*` duplicates —
ten game pages for five games. The guides rendered only as `guide-*` (the
de-prefixed plan names did not produce separate rendered pages), and tools/indexes
rendered once each. So the duplication is concentrated on the games — precisely
where the `game`→`tool` re-type occurs — confirming the mechanism end to end.

**Fix direction (now sharper).** The cleanest structural fix is upstream: reconcile
the `page_type` vocabularies so the planner can preserve an adopted `game` (add
`game` to the canonical list, or map adopted games to a type whose `CanonicalisePage`
branch round-trips the `game-` prefix). Failing that, the doc 029 endgame —
deterministic slug-preservation against the adopted inventory — sidesteps `page_type`
entirely for existing pages. Either way, the `WriteSitePlanAction`/`sync_pages_to_db`
canonicalisation split should be unified onto one canonicaliser so a plan name and
its realised page name cannot disagree.
