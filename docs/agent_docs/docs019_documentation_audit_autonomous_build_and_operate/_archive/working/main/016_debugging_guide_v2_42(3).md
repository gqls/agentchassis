...
### Presign `expiry_minutes` override silently ignored — numeric config value dropped before the request (configOrInput suspected)
...
**Suspected cause.** In the dispatch, `expiry_minutes` is read with `configOrInput(params, "expiry_minutes")` (→ string) then `parsePositiveInt(...)` → `requestBody["expiry_minutes"]`. The def config value is a JSON NUMBER (`3000`), which unmarshals to Go `float64`; if `configOrInput` resolves config via a `.(string)` type assertion it returns `""` for a numeric value → `parsePositiveInt("")` fails → the key is omitted → the adapter defaults. Consistent with the evidence: string configs in the same steps DO apply (`method:"PUT"`, `key:"finetuning/scripts/bundle.tar.gz"`), only the numeric one is dropped. CONFIRM by reading `configOrInput` (and `parsePositiveInt`) in the actions package.

**Fix.**
- Structural (preferred): make `configOrInput` coerce numeric config values to string — handle `float64` / `int` / `json.Number`, not just `.(string)` — so any numeric literal in a step config survives. This is the right fix because JSON config naturally carries numbers; every numeric config literal is otherwise silently dropped.
- Stopgap (no code): quote the value in the def — `"expiry_minutes": "3000"` — which `configOrInput` already handles (as `method`/`key` prove). A one-field migration.
