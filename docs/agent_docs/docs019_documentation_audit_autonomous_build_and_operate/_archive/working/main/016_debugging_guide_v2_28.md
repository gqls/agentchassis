...
### Tool/game pages never deploy a file — no rendered `page_components`, so the rerender skips them (IN PROGRESS, 2026-05-28)
...
Open (next read, not yet pinned): read `save_page_sections`. Two leading hypotheses, to be settled by the source, not inferred — (a) it splits the HTML into sections by an element boundary (e.g. `<section>`) and the tool output is a single `<div class="tool-page">…`, yielding zero sections to persist; or (b) it writes to `pages.sections` jsonb while the rerender reads `page_components.rendered_html` — a save/read **surface mismatch** of the same family as Tension #2. Either way the recreated-tool artefact exists upstream (the `recreate_tool` LLM step produced it, training-data was saved) but never becomes a deployable component. Do not prescribe a fix until `save_page_sections` confirms which.
