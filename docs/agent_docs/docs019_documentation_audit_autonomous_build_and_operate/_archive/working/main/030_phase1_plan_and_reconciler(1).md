...
| 3 | **`write_site_plan` Go action**. Takes plan-builder LLM output, runs role validator, runs `CanonicalisePage` (which now produces both name and URL via the Phase 1 `ParentSection` extension), writes one `site_plans` row + N `site_plan_pages` rows. Marks previous `site_plans` row for site as superseded. | Replaces `WriteBuildItemsAction`'s role in the planner workflow. |
