...
**Extensions — what's installed** (refresh from `\dx`). The `\df` snippet that prompted this section also listed `sparsevec` (a function returning `sparsevec`) — a pgvector artefact — so **pgvector appears to be installed**. Confirm with `\dx`; `\dT *vector*` lists its types. If present, embedding storage for the context tool should use pgvector columns + its similarity operators rather than in-Go cosine, which also settles the at-scale storage question (migration plan B4 / B4a).
