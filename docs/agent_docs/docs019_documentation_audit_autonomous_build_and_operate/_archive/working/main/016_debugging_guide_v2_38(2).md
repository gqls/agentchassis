...
**24. Snapshot before you mutate `agent_definitions`, and check `\df` for an existing helper before hand-writing SQL.** Two DB-layer habits that belong with the rest of this checklist. (a) Before *any* change to `agent_definitions` — a config patch, a migration that touches it, a manual edit — run `snapshot_agent('<agent-type>')` first; it writes a restorable backup that `revert_agent('<agent-type>')` can roll back (see §6.1), and an optional reason can be recorded with `snapshot_agent('<agent-type>', 'why')`. This is cheap insurance against the patch-contamination and failed-revert footguns in §9. (b) The database carries helper functions for routine operations; before writing raw SQL for something that sounds standard, run `\df` and check whether a function already does it — reuse-before-recreate applies at the DB layer too. The known helpers are inventoried in §6.2; keep that list refreshed from the live `\df`.
...
### 6.2 Database helper functions (keep a `\df` here)

The database carries helper functions for routine operations. Before hand-writing SQL for something that sounds standard — snapshotting, reverting, claiming — check whether a function already does it; `\df` lists them. The canonical source is the live `\df`, not this copy: treat the block below as a scaffold to refresh whenever functions change, and as the answer to "what helpers exist" rather than re-deriving from the schema.

**Known helpers** (confirmed):

| Function | Arguments | Returns | Use |
|---|---|---|---|
| `snapshot_agent` | `p_agent_type text` | `uuid` | Back up an agent's `default_config` before patching it. Run before ANY `agent_definitions` change. |
| `snapshot_agent` | `p_agent_type text, p_reason text DEFAULT NULL` | `uuid` | Same, with a human-readable reason recorded on the snapshot. |
| `revert_agent` | `p_agent_type text` | — | Restore the most recent unrestored snapshot for the agent (see §6.1). |

**Full inventory — paste the live `\df` output here and keep it current** (this is a scaffold, not a complete list):
...
**pgvector note.** The `\df` snippet that prompted this section also listed `sparsevec` (a function returning `sparsevec`) — a pgvector artefact — so **pgvector appears to be installed**. Confirm with `\dx` (extensions) or `\dT *vector*` (its types). If it is present, embedding storage for the context tool should lean on pgvector columns plus its similarity operators rather than in-Go cosine, which also settles the at-scale storage question (see the migration plan, B4 / B4a).
