...
**What is and isn't established.** Established: the plan is faithful
(`section-index`, nested URLs); `pages` diverges from it post-adoption; the writer
is the plan→`pages` sync, not rerender, not the planner, not adoption (which wrote
it correctly first). Not yet established: whether `sync_pages_to_db` *copies*
`site_plan_pages.url`/`role` (in which case it read a different/stale plan) or
*re-derives* `url`/`page_type` from the page name (in which case it carries the same
de-prefixing the canonicaliser does at plan-write, but without the section-index
round-trip). **Next read, not yet done — do not prescribe before it:** the
`sync_pages_to_db` source, to settle copy-vs-re-derive. Fix direction follows from
that: either carry the plan's `url`/`role` through verbatim, or run the same
canonicalisation `write_site_plan` already runs.
