...
4. `store_generated_component` saves to `content_components` with full selection metadata
5. Marks work item complete
6. Pages waiting for this component are marked `needs_rebuild` (see below)

### Component → page rebuild loop

When `plan_sections` can't find a component (Path 3), the page deploys with that section missing. The component gets created asynchronously. Two mechanisms close the loop:

**Inline trigger (immediate):** `store_generated_component` calls `markPagesForRebuild()` after saving the component — both in the "created" and "already_exists" paths. This queries:
```sql
UPDATE pages SET build_status = 'needs_rebuild'
WHERE status = 'active' AND build_status = 'deployed'
  AND EXISTS (SELECT 1 FROM jsonb_array_elements_text(sections) sec WHERE sec = $section_type)
```
This catches all pages on all sites that reference the section_type, not just the one that triggered the creation.

**Discovery check (sweep):** `check_unresolved_sections` runs as part of the completeness-discovery-agent. It finds pages with sections that reference now-existing components (by function or section_type) but don't have them in `page_components`. Marks them `needs_rebuild`. This catches edge cases: components created manually, inline trigger missed a site, or timing issues.

The normal build pipeline then picks up the `needs_rebuild` pages and `plan_sections` succeeds on the re-run because the component now exists.

**Timeline example:**
```
T+0   plan_sections runs for page X, section "provocation-card" not found
        → page deploys without that section
        → needs_new_component work item created
T+5m  component-creator generates provocation-card template
        → store_generated_component saves it
        → markPagesForRebuild marks page X (and any others) as needs_rebuild
T+10m build pipeline picks up page X
        → plan_sections finds provocation-card via Path 1 or 2
        → section renders
```

**Dedup:** The `needs_new_component` item_key is `needs_new_component:{sectionType}` scoped per site_id. Multiple sites requesting the same section_type create separate work items, but only the first creates the component — subsequent ones hit `already_exists` and still trigger page rebuilds.
...
3. `store_generated_component` saves it and triggers page rebuilds via `markPagesForRebuild()`
4. The new component enters the library with metadata
5. Future sites with similar pages find and reuse it
6. `check_unresolved_sections` discovery check catches any pages missed by the inline trigger
