...
Build everything with `go build ./...`; run a tool with `go run ./cmd/<name>`. `thin_slice_constitution.md` and `groundtruth_targets.json` sit alongside (pass them by path). A Go toolchain is needed (and, for `dbcontext`, `psql` access to the DB; for `embed`'s real recall, reach to your Ollama).
