...
DB/ANTHROPIC creds, so the spawned pod's SA can already read it). DECIDED for option 3 (turn 26): (a) clone with **go-git** (pure Go, no chassis image change);
(b) **fully self-contained** — clone once + run the `analysis` package in-process for the call
graph + spans, read bodies from that checkout, dropping the cross-pod analyse call (`repoRoot`
becomes a real local path); (c) pin `repo_analysis.commit_sha` = the index's commit so symbols
and bodies agree. DEPENDS ON the `analysis` package exposing an in-process entrypoint
(`AnalyseRepo(root)->Output`) — if only the adapter wrapper is exported, export one first
(chassis change) or fall back to clone-for-bodies + keep the analyser call (2 clones/run). NOTE:
`lookup_symbols` still reads the `code_symbols` index for retrieval seeding (self-contained = no
runtime analyser-adapter call, NOT index-independent). TO BUILD NEXT: a clone+in-process-analyse
action (go-git) replacing `analyse_repo` in the diagnose workflow; then `assemble_bundle`'s
`repoRoot` error stops firing. Credential injection already DONE (spawn_actions.go, turn 25).
