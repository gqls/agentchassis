...
- [!] §7E ATTEMPT 1 (correlation 17933a83, orch fc60af15, 2026-07-03) FAILED:
      "message too large". TRIAGE before any fix — the named payloads are all
      bounded (child complete result_from=diagnosis; parent result_from=
      diagnose-agent_result; run f284b749 proved spawn/call/515-file
      collected_data/complete size-safe), so the offender is an unenumerated
      producer or a non-Kafka error paraphrased. Extract the error line:
      orchestration_states + agent_error_log by correlation 17933a83, and pod
      greps for 'too large|MessageSize|max.request|max.message|RecordTooLarge'
      on agent_type=diagnose-agent and app=agent-chassis. The line names
      producer/topic/bytes → fix = SLIM AT SOURCE (§7B precedent) if a Kafka
      producer over-serialises (repo_analysis is the only 7.5×-grown payload);
      bundle/max_body_chars enforcement if it is the verdict prompt. Raising
      max.message.bytes is the structural inversion — last resort only.
