...
> **UPDATE 2026-06-24 — `diagnose_run` is the chosen and SEEDED shape; the brief
> 06-17 `diagnose_route` detour is dropped.** The §4–§6 recommendation below — a
> single `diagnose_run` action wrapping `diagnose.Run(...)`, the verdict via the
> verdict prompt — is what is actually seeded: `diagnose-agent` (worker) +
> `diagnose-orchestrator` (wrapper) exist in `agent_definitions` with the workflow
> `analyse_repo → lookup_symbols → load_runtime → assemble_bundle → run_loop
> (diagnose_run) → emit → complete`. The 06-17 move to a `diagnose_route`
> cyclic-loop-back controller was reconsidered and abandoned: it re-expresses the
> loop and its guards in the workflow and needs a cyclic `next_step` the engine is
> not shown to support — against "keep complexity in Go, workflows thin; don't
> re-express the guards as workflow branches" (which §4–§6 itself argues). So
> `diagnose_run` wins. Build status is in the new `## Status` section just below;
> §0 (step-zero reuse), §2 (triggering), §3 (file split) are unchanged and correct.

## Status (2026-06-24)

- **Engine** (`pkg/diagnose`): the Run / Advance / DecideStep core + convergence
  guards + verdict-wire parser exist and are tested (standalone). Reused verbatim.
- **Actions**: `diagnose_load_runtime` and `diagnose_assemble_bundle` are drafted
  (the bundle composer is the one genuinely new piece §0 named). `diagnose_run`
  (wraps the engine) and `diagnose_emit` (thin) are NOT yet built — the remaining
  work. `request_repo_analysis` + `lookup_code_symbols` are reused as-is.
- **Agents**: `diagnose-agent` + `diagnose-orchestrator` are SEEDED — but with their
  workflow in `orchestration_workflow` (json), which the loader does not read. Fix
  (move to `default_config`): `NNN_move_diagnose_workflow_to_default_config.sql`.
- **Verdict prompt**: referenced as `diagnose-verdict-v1`; ensure it resolves to
  `PROMPT_diagnosis_verdict.md` (the §2 contract).
