...
How to run the thin slice today. Files: `analyser.go`, `assembler.go`, `dbcontext.go`, `resolve_targets.go`, `embed.go`, `fuse.go`, `eval_targets.go`, `groundtruth_targets.json`, `thin_slice_constitution.md`. Go toolchain on the machine with the repo (and, for `dbcontext`, `psql` access to the DB; for `embed`'s real recall, reach to your Ollama).
...
go run resolve_targets.go -analysis analysis.json -task "T" -n 25 -json > lex.json
go run embed.go query -embeddings embeddings.json -task "T" -n 25 -json -ollama ... -model ... > sem.json
go run fuse.go -in lex.json -in sem.json -n 12 -json > fused.json   # combined -scope proposal
go run fuse.go -in lex.json -in sem.json -n 12                       # same, human-readable
go run eval_targets.go -truth groundtruth_targets.json -candidates fused.json -n 12  # recall@N, MRR
