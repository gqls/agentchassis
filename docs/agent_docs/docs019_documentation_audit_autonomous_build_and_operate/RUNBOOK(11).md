...
With the adapter running, the request → adapter → index half is unblocked; the
`code_symbols` count for `gqls/agentchassis` is the remaining check, and rises once a
diagnosis (or a manual analyse request) runs.
