...
(if it matches the executing plan, the embedded `agent_config` is just stale cache).
Fix: `kubectl -n <ns> rollout restart deploy <chassis-deploy>` to reload definitions —
which also happens for free on any redeploy. Note the await deadline comes from the
