...
*v2_31 (2026-06-05): added the adoption-convergence clobber and no-op failure patterns (§9), and guide page_type URL/list specifics (§6.5). See FOCUS_adoption_faithfulness_via_locks for the full subsystem state.* Updated later 2026-06-05: the no-op pattern's confirmed root cause is the `[]map[string]interface{}` vs `[]interface{}` type assertion in ValidateSitePlanAction (see below).*
