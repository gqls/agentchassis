...
(worker) + `diagnose-orchestrator` (wrapper), since 2026-06-20 — so the earlier
"no agent seeded" reading was wrong (those rows exist; my `category='diagnose'`
query had matched them once `diagnose-agent`/`-orchestrator` were created). The
real blocker: their workflow sits in `orchestration_workflow` (the `json` column),
but the loader reads `default_config` — so it does not load. Fix =
`NNN_move_diagnose_workflow_to_default_config.sql` (column move) + build the two
unbuilt actions the workflow names, `diagnose_run` (the engine wrapper) and
`diagnose_emit`. RUNBOOK §6C. The `diagnostician` seed I wrote is SUPERSEDED by
this pair.
