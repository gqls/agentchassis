...
1. Confirm whether a read-only role exists:
   ...
   ALTER ROLE diagnose_ro SET statement_timeout = '5s';
   ```
2. Point the loop's `-psql` at that role and prove it refuses a write:
   ```
   psql 'postgresql://diagnose_ro:…@host/clients_db' -c 'DELETE FROM site_work_items WHERE false;'
   ```
   Expect "cannot execute DELETE in a read-only transaction" (from
   `default_transaction_read_only`). Only then enable model-written data requests
   on the harness.

Alternative (more code, avoids the psql-stacking problem entirely): have
`dbcontext` open its OWN `*sql.DB` and use `BeginTx(ReadOnly:true)` like the
chassis, instead of shelling to `psql -c`. Not required if the read-only role is in
place; noted because it removes the stacking foot-gun structurally.
