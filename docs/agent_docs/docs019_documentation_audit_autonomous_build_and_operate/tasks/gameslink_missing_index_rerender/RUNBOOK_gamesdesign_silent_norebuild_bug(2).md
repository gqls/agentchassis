...
**Step A — the cheapest diagnostic first (read-only): what does the plan expect,
and what is actually deployed?**

CORRECTION (from the real `\d`, 2026-06-14): `site_plan_sections` has NO
`status` column. Its columns are `plan_id, page_name, ordering, component_name,
component_version_id, palette_id, layout_id, typography_set_id`. The
`Status`/`ready`/`deferred`/`LLMFieldSpecs` machinery lives in
`plan_sections_action.go` and is computed AT RUNTIME and passed to the writer —
it is NOT persisted. So you cannot read the planning decision from the plan
table; the plan table only tells you which sections the plan EXPECTS.

What the plan table CAN tell you — does the plan still expect the 5 sections the
deployed page has?
```sql
SELECT sps.ordering, sps.page_name, sps.component_name
FROM site_plan_sections sps
JOIN site_plans sp ON sp.id = sps.plan_id AND sp.is_current = true
JOIN sites s ON s.id = sp.site_id
WHERE s.domain = 'gamesdesign.co.uk' AND sps.page_name = 'index'
ORDER BY sps.ordering;
```
- 5 rows matching the deployed sections (hero, tool-list, guide-list, game-list,
  system-stats) → the plan is intact; the shortfall is in GENERATION (planning
  computed few ready sections at runtime, OR the writer returned little). Either
  way the next evidence is the RUNTIME trace + the writer's output, not the plan
  table.
- Fewer rows / different components → the plan itself changed; investigate what
  rewrote the current plan.

**What the deployed page actually is** (confirmed 2026-06-14 — the 34k is 5
healthy sections, so the existing content is legitimately rich and the ~3k
regeneration is a fraction of the WHOLE, not one failed section):
```
pos slot          len
1   hero          2426
2   tool-list     8951
3   guide-list    7513
4   game-list     8116
5   system-stats  7369   (total ~34k)
```
This sharpens Fault 1's open question to: did the rebuild generate FEWER
sections (e.g. only hero + one list), or ALL FIVE but each near-empty? That is
answerable only from the writer's RUNTIME output (the status isn't persisted),
which is exactly what bundle 2's runtime trace + the writer's pod logs carry.
...
go run ./cmd/bundle -analysis analysis6.json -root ~/projects/agentchassis \
  ...
  -psql 'kubectl exec -n ai-persona-system postgres-clients-0 -- psql -U clients_user -d clients_db' \
  -schema-tables site_plans,site_plan_pages,site_plan_sections,page_components,pages \
...
prompt (the doc), the plan/section schema (incl. `site_plan_*` so you don't hit
the missing-column wall Step A's first attempt did), and the runtime trace —
i.e. the whole generation path in one artefact. That is what tells you why the
generation is short, which is the fix for Fault 1.

> **YOU ARE HERE → next action: build bundle 2 (this block).** Bundle 1
> (save side, already produced) confirmed the guard fires; it cannot show why
> the input is short. Run Step A's SQL first (cheap; tells you planning-vs-writer
> and so which `-scope` matters most), then assemble bundle 2 above. The two
> `-scope` files are `plan_sections_action.go` (have it) + the content-writer
> action found in Step B.

**A finding for this bug (note it):** the ~3k rejected content is currently
UNOBSERVABLE after the fact — the guard returns before the `INSERT`, so it is
never persisted, and it survives only in the content-writer's pod logs at
generation time (if logged at all). Part of why this was hard to see. Worth a
small fix: have the guard log the rejected content (truncated) so a future
regression is inspectable from `agent_error_log` rather than only live pod logs.
