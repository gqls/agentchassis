...
RENDER_COMPONENT="${PIN_RENDER_COMPONENT:-$(resolve_by_key 'render_component action'           'render_component')}"
RERENDER_SECTIONS="${PIN_RERENDER_SECTIONS:-$(resolve_by_key 'rerender_page_sections action'   'rerender_page_sections')}"
RENDER_SNIPPETS="${PIN_RENDER_SNIPPETS:-$(resolve_by_key 'render_js_snippets_for_site action'  'render_js_snippets_for_site')}"
...
if [ -f "$A/save_page_sections_action.go" ]; then
  ...
  SAVE_SECTIONS="${PIN_SAVE_SECTIONS:-$(resolve_by_key 'save_page_sections action' 'save_page_sections')}"
fi
