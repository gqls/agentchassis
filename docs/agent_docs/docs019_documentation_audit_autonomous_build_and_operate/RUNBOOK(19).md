...
DB/ANTHROPIC creds, so the spawned pod's SA can already read it). REMAINING for option 3:
- (a) clone mechanism: go-git (pure Go, no image change) vs shell `git` (needs the binary in
  the chassis image). DECISION NEEDED.
- (b) wire clone+repoRoot in the diagnose path: clone owner/repo at repo_analysis.commit_sha
  to an ephemeral dir = repoRoot, then ReadSymbolBody works as-is. Ideally the SELF-CONTAINED
  variant — clone once + run the analysis package in-process (if it exposes an entrypoint),
  dropping the cross-pod analyse call so repoRoot is a real local path. DECISION NEEDED.
- (c) pin the SAME commit_sha the index was built on, so retrieved symbols and sliced bodies
  agree (a re-index between steps must not leave them on different versions).
