...
Open design points to settle before writing the migration: (1) how many
exemplars (N) and scrape depth — cost/latency budget; (2) does the hop apply
to adopted sites identically or skip when site_archetype exists (fidelity
argument cuts both ways); (3) item pipeline/priority values consistent with
the existing relay items (read one existing row first).
