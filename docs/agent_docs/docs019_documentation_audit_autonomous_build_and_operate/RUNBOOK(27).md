...
> change. Two regression tests added (`loop_datarequest_test.go`). **This is a contextkit engine
> change — rebuild contextkit AND the chassis that imports it; run `go test ./...` on the diagnose
> package first.** Existing guard tests verified safe (none attach `data_requests`).
>
...
>
> **#2 (still open):** the model guesses the content table (`page_sections`/`work_items`/`slug` —
> all wrong) and burns iterations learning the schema; surface the real content tables
> (`pages`, `page_components`, `content_items`) + their columns in the bundle schema section, or
> seed them. (#1's fix routes around #2 via the data_requests path, but #2 still wastes iterations.)
>
> **#3 (still open):** it stays on the symptom hypothesis (abstains, never refutes → no upstream
> pivot). Possibly the raw-symptom seed keeping it in "verify the symptom" mode; a verdict-prompt
> nudge or mechanistic seed could help it pivot toward the cause. Revisit after #1 lands.
>
> **Separate trigger bug:** run `0b76d9bc` came in with `input_data.site_id` and `correlation_id`
> **empty** (earlier runs had `e33263f4-…`). It didn't change the outcome, but the trigger is
> dropping `site_id`, so `load_runtime` isn't pinned to the site — fix the trigger envelope.
>
