...
6. [NEW 2026-07-07, from the fix-loop discussion] THE COMMERCE LEG, REDEFINED
   as three roadmap phases (001 Tier-3 mechanism — EXISTS, was unused for
   dartsonline): P1 content/guides/tools site; P2 affiliate-approved product
   introduction (legal/compliance gate — the F2 council's first job); P3
   catalogue (product-data mechanism; dynamic assembly from scrape+feeds+
   imagery as the build approach). Plus ENFORCEMENT candidates in relay
   territory: strategist prompt recommends phased roadmaps for
   product_category domains; planner validate gains page-type deliverability
   vs live handlers; nav generation grounded in the BUILT set. These land
   here (relay changes) when taken up; the guideline amendment travels the
   fix-loop's side-task path.
