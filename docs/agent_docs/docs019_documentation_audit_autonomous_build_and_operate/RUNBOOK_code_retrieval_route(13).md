...
- [~] reindex TRIGGERED 2026-07-02 19:00:09 (correlation f284b749, orch
      20b33469, REF=083_imagery) — ORDER UNVERIFIED: whether build/push/deploy
      preceded it decides the outcome. Discriminate after completion:
      ```sql
      SELECT commit_sha, count(*) FROM code_symbols WHERE repo='gqls/agentchassis' GROUP BY 1;
      SELECT count(*) AS docs_rows FROM code_symbols WHERE repo='gqls/agentchassis' AND path LIKE 'docs/%';
      ```
      NEW sha + docs_rows=0 → full sequence ran (excludes applied, prune
      cleared 36710be, resolver image live) → §7E now.
      36710be + docs_rows>0 → trigger ran ahead: old image re-analysed the
      same commit WITHOUT excludes (docs rows re-inserted — the regrowth
      caveat), prune no-op, resolver NOT live → build→push→deploy, retrigger
      indexer, then §7E.
      NEW sha + docs_rows>0 → pushed but not deployed: old image analysed the
      new tree → deploy, retrigger, then §7E.
- [ ] §7E diagnosis run (§6F envelope, fresh correlation id) — GATES: resolver
      image DEPLOYED; reindex verified above; site_id NON-EMPTY (add to the
      trigger script: [ -n "$SITE_ID" ] || { echo "site_id empty"; exit 1; }).
      WATCH: "diagnose_route: scope resolver applied" + "resolved fuzzy scope
      entry" similarities (floor-calibration data); no could-not-read-body for
      resolved entries; a static-tier citation; the SEED lookup's similarity
      band on the current corpus (flat ≈0.55 still? → §7F decision).
