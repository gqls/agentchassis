...
- BUILD FAILED in the chassis: `diagnose_route_action.go` referenced
  `verdict.DataRequests` and `diagnose.DataRequest`, which DON'T EXIST in the chassis
  `pkg/diagnose` — that copy is BEHIND the contextkit engine (the uploaded engine
  files import `contextkit/internal/analysis`, i.e. they are the contextkit copies,
  which have DataRequest; the chassis copy does not). Everything else resolved
  (`IsReadOnlySQL`, `Advance`, `ParseVerdictValue`, `Encode*`), so only the
  data_requests references broke.
- FIX (avoids syncing two engine copies I can't see): `diagnose_route` now reads
  data_requests from the RAW verdict wire (`verdict.result`) via a new
  `readOnlyDataRequestsFromWire` helper, keeping only read-only ones via
  `diagnose.IsReadOnlySQL` (which IS in the chassis copy). No dependency on the
  engine's Verdict shape. Added the `strings` import. gofmt-clean, parses; all
  `diagnose.*` symbols used now exist in the chassis (they compiled before).
- Consequence for the 3-layer guarantee: in the chassis the FIRST filter is now the
  route-layer lint (the chassis `verdict_wire` doesn't lint data_requests), then the
  `runDataRequests` re-lint, then the read-only tx. Still prompt + two lints + tx.
  Corrected the load_runtime comment + §6C to say "route layer" not "verdict_wire".
- Flagged the engine-copy divergence in §6C (chassis pkg/diagnose behind contextkit);
  the wire-based route read is robust to it.
