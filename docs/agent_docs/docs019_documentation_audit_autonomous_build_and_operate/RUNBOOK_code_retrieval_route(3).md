...
- [ ] `git ls-remote origin HEAD` output pasted (confirms/refutes)
- [ ] default branch corrected on GitHub
...
4. TOKEN CHECK before triggering: analyse_repo_local reads env
   ...
   the pod that clones.
   PROBE FIRST — where does code-indexer execute?
   `kubectl -n ai-persona-system get deploy,pods | grep -i index`
   Then choose:
   (a) standing code-indexer deployment exists → add the SAME secretKeyRef env
       block to ITS manifest (kustomize overlay) — token scoped to a
       repo-cloning agent, consistent with the design. No code change.
   (b) it executes in-place on the chassis pod (like the diagnose-orchestrator
       trigger did) → the chassis must NOT get the token; instead add
       'code-indexer' to isRepoCloningAgent (one line) and run the indexer as a
       SPAWNED agent (reuse the diagnose-orchestrator spawn→call pattern) — a
       small rebuild + one seeded orchestrator. Heavier; only if (a) is false.
...
- [ ] §7A.1 confirmed and/or default branch corrected
- [ ] envelope REF set to the EXPLICIT branch name (or sha) — NOT "HEAD"
- [ ] the executing pod carries GITHUB_READ_TOKEN (per the §7B probe/remedy)
