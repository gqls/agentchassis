...
- [ ] §6C workflow-fix migration APPLIED — the current blocker
- [ ] §6C four diagnose actions built + registered + coordinator `next_step` override confirmed + deployed
- [ ] §6C (enhancement) wire data_requests/runtime_site re-gather into the loop
...
```
(Do NOT apply the older `NNN_move_diagnose_workflow_to_default_config.sql` for the
agent — it would move the broken `diagnose_run` workflow. It's bannered superseded.)
...
**Known gap (decide, then a follow-up if wanted).** The verdict can ask for
read-only `data_requests` and name a `runtime_site`; `verdict_wire.go` parses and
lint-checks them (Guard 2). But the loop does not act on them: `diagnose_route`
returns `next_step`/`scope`/`hypothesis` only (NOT `data_requests`/`runtime_site`),
and the loop-back goes to `assemble_bundle`, not `load_runtime`. So code-following
re-scope (the call graph) works; runtime-following re-gather does not fire. To wire
it: have `diagnose_route` forward the verdict's `data_requests`/`runtime_site` into
its result, and set `gather_step: load_runtime` (then `load_runtime` reads
`route.data_requests` — already its configured field — and re-gathers runtime).
The `runDataRequests` helper in `diagnose_load_runtime` already does the read-only
execution; it's just not reached on loop-back yet.
