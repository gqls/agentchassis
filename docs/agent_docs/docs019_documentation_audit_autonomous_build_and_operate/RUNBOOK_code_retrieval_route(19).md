...
> 2. Result-contract unification (Go, future build): resolveResultSpec prefers
>    result_from / multiple_output_fields; CompleteWorkflowAction reads
>    NEITHER — `output_fields` is the only spelling both readers act on, so
>    the deprecation warn stays EXPECTED until extractFinalResult shares the
>    contract table (reuse resolveResultSpec), after which configs migrate to
>    preferred names and the warn goes quiet. DO NOT rename keys before the
>    Go unification — it would resurrect the 1.3MB fallback. Fold the
>    ship-everything-fallback size guard into the same change.
