...
INCIDENT 2026-07-06 — the hop's first claim STALLED: pod spawned but booted
as GENERIC (logs: type=generic, topic system.agent.generic.process, group
generic-group; label agent-type=vertical-exemplar-researcher correct — note
the label key is agent-type, hyphen). ROOT CAUSE (spawn_actions.go read):
getAgentDefinition SELECTs image_repository/image_tag/command/resources/
health_config/env_vars/is_active/idle_timeout_seconds and gates on
is_active=true; the §B4 seed set NONE of them. command NULL ⇒ Command:nil on
the container ⇒ the image's DEFAULT ENTRYPOINT (the generic chassis service)
runs and never reads the injected AGENT_TYPE/KAFKA_TOPICS env ⇒ the
dispatcher's call to the per-instance job topic is unheard ⇒ item claimed
forever. (Entrypoint link pending pod-spec confirmation: CMD empty +
AGENT_TYPE present in the jsonpath dump proves it.) Liveness for spawn =
is_active BOOLEAN, not status — consistent with the user's "experimental is
redundant" correction. The two 0-row spec queries were EXPECTED (agent never
worked), not query faults.
FIX: NNN_fix_researcher_spawn_columns.sql — comparison SELECT first, snapshot,
copy the infra columns from spawn-proven donor page-build-handler (NOT
capabilities/topics — semantic; NOT default_config — ours), is_active=true;
cleanup = delete the generic job, reset the item to triaged (or let
claimed-item-timeout's evidence check do it), pump re-dispatches in ~30s.
Seed file AMENDED to include the columns (donor switched to
page-build-handler) so re-use cannot repeat the omission.
