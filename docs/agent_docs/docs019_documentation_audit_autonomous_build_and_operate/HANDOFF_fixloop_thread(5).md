...
scopes resolved) — carry it into the project folder. THE PILOT SLOT IS OPEN (2026-07-07): the nav/undelivered-pages symptom's
ROOT CAUSE was found by direct code reading (082 script has no --roadmap
path; the planner prompt has no else-branch for the roadmap block) — a
known platform gap, not diagnosis material. RECLASSIFIED to
RUNBOOK_builder_route.md item 6 (now MAIN) as a relay-wide fix: a new
post-classification scope-decision hop + three enforcement points. This
thread's role narrows to a VERIFICATION diagnosis once that fix ships
(runbook "F0 PILOT — DOWNGRADED" section has the full disposition). F0.1
plumbing proceeds regardless; propose a genuinely mechanism-unclear bug for
the live pilot when one is in hand. Imagery remains another chat's. First action: slice **F0.1** with
