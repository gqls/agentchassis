...
-- the seeded workflow referenced a `diagnose_run` action that DOES NOT EXIST. The
-- real built design is the workflow-driven loop (diagnose_load_runtime /
-- diagnose_assemble_bundle / diagnose_route / diagnose_emit), so this REWRITES
-- diagnose-agent's workflow to that shape, in default_config (the column the loader
-- reads), with the verdict prompt inline and an ai_service block.
--
-- THE LOOP RETURNS TO load_runtime (gather_step = "load_runtime"), NOT directly to
-- assemble_bundle: load_runtime is the SQL-runner, so returning there each iteration
-- lets it execute the prior verdict's read-only data_requests (forwarded by
-- diagnose_route into route.data_requests) under a read-only transaction, and
-- re-gather the runtime tier, before assemble_bundle re-slices the re-scoped code.
-- Flow: analyse_repo -> lookup_symbols -> load_runtime -> assemble_bundle ->
-- verdict -> route -> {load_runtime (iterate) | emit (stop)}.
--
-- This OVERWRITES diagnose-agent.default_config for the row (installs the correct
-- workflow). Re-running is safe (same value).

-- BACKUP FIRST (standing rule): snapshot each row before changing it.
SELECT snapshot_agent('diagnose-agent', 'rewrite workflow to diagnose_route loop in default_config');
  ...
  "workflow": {
    "steps": {
      "analyse_repo": {
        ...
        "description": "Analyse the repo at ref; awaits the analyser adapter (Output incl. root + commit_sha)"
      },
      "lookup_symbols": {
        ...
        "description": "Seed the FIRST scope from the symptom (retrieval seeds iter-1 only; evidence steers re-scopes)"
      },
      "load_runtime": {
        ...
        "description": "Read-only runtime tier + (on loop-back) the prior verdict''s read-only data_requests under a read-only tx. The loop RETURNS HERE."
      },
      "assemble_bundle": {
        ...
        "next_step": "verdict",
        "output_field": "bundle",
        "description": "Compose bundle: hypothesis + in-scope code bodies (analysis.ReadSymbolBody) + runtime (incl. data_request rows)."
      },
      "verdict": {
        ...
        "description": "Cite-or-abstain verdict over the bundle (its own observable step). JSON at verdict.result; may emit read-only data_requests."
      },
      "route": {
        ...
        "config": {
          ...
          "analysis_field": "repo_analysis",
          "gather_step": "load_runtime",
          "emit_step": "emit",
          "max_iterations": 5
        },
        "output_field": "route",
        "description": "Controller: engine guards + call-graph re-scope (Advance); forwards the verdict''s data_requests; overrides next_step to loop back to load_runtime or stop at emit."
      },
      "emit": {
...
--          (default_config -> 'workflow' -> 'steps' -> 'route' -> 'config' ->> 'gather_step') AS gather_step,
--          (orchestration_workflow IS NULL)                               AS orch_null
--   FROM agent_definitions WHERE type = 'diagnose-agent';
-- expect: has_wf=t, has_route=t, still_run_loop=f, gather_step=load_runtime, orch_null=t
--
-- (diagnose-orchestrator is handled by NNN_restore_diagnose_orchestrator_workflow.sql.)
