...
**Correction (the "data_requests gap" was wrong).** Re-checked the engine: model
`data_requests` are deliberately NOT propagated — `Scope`/`AdvanceResult` carry no
`DataRequests`, and `queryselect.go` is explicit ("the model never writes SQL"; DB
evidence is a vetted catalogue selected by hypothesis). So re-scope = call-graph
(code) + catalogue (DB). The `runDataRequests` in `diagnose_load_runtime` is dormant
and beyond that boundary — decide drop-vs-keep. The real verify item: `diagnose_route`
writes `EncodeScope` (an object) at `route.scope` but `diagnose_assemble_bundle` reads
it as a list — confirm `route.scope.symbols` so loop-back re-scope flows.
