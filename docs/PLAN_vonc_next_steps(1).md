...
Migration 001: Mark broken components for regeneration (via work items, no SQL).
Migration 002: Fix `check_render_mode` condition in `page-content-writer` agent_definition.
    This is a single jsonb_set UPDATE on agent_definitions — NOT a component table sweep.
    The 65-component render_mode update is DROPPED; existing components are fine as-is.
