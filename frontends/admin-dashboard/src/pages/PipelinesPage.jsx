// FILE: frontends/admin-dashboard/src/pages/PipelinesPage.jsx
//
...
function StateBadge({ state, enabled }) {
    ...
    return <span style={badgeStyle(fg, bg)}>{state.replace("_", " ")}</span>;
}
...
export default function PipelinesPage() {
    ...
    const toggleEnabled = async (name, currentlyEnabled) => {
        setActionLoading(name);
        try {
            await apiFetch(`/pipelines/${name}`, {
                method: "PATCH",
    ...
    const triggerPipeline = async (name) => {
        setActionLoading(name);
        try {
            await apiFetch(`/pipelines/${name}/trigger`, { method: "POST" });
            await loadData();
        } catch (e) {
            setError(e.message);
        } finally {
    ...
    return (
        <div style={{ maxWidth: 1100, margin: "0 auto", padding: "24px 16px" }}>
            <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: 24 }}>
                <h1 style={{ fontSize: 22, fontWeight: 600, color: "#0f172a", margin: 0 }}>Data Pipelines</h1>
            ...
            {Object.entries(groups).map(([group, tasks]) => (
                <div key={group} style={{ marginBottom: 24 }}>
                    <h2 style={{ fontSize: 14, fontWeight: 600, color: "#64748b", textTransform: "uppercase", letterSpacing: "0.05em", marginBottom: 10 }}>
                        {group}
                    </h2>
        ...
        <div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fit, minmax(280px, 1fr))", gap: 16, marginBottom: 28 }}>
            ...
            <div style={cardStyle}>
                ...
                <div style={{ marginTop: 8, fontSize: 12, color: "#64748b" }}>
                    Financial data: {formatNumber(financial.net_worth)} net worth, {formatNumber(financial.employees)} employees, {formatNumber(financial.turnover)} turnover
                </div>
            </div>
...
// ── Styles ───────────────────────────────────────────────────────────────
const thStyle = { padding: "10px 14px", textAlign: "left", fontSize: 11, fontWeight: 600, color: "#64748b", textTransform: "uppercase", letterSpacing: "0.03em" };
const tdStyle = { padding: "10px 14px" };
const cardStyle = { background: "#fff", border: "1px solid #e2e8f0", borderRadius: 10, padding: 16 };
const cardTitle = { fontSize: 13, fontWeight: 600, color: "#0f172a", marginBottom: 12 };
