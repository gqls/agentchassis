...
monitor-metrics: ## Show workflow metrics for last 24 hours
 ...
 #################################
 .PHONY: test-spawning
 test-spawning: ## Run agent spawning tests in Kubernetes
	@echo "$(YELLOW)Running agent spawning tests...$(NC)"
	@echo "$(GREEN)Building test image...$(NC)"
	docker build -f build/docker/test/test-spawning.dockerfile -t $(REGISTRY)/test-spawning:$(IMAGE_TAG) .
	docker push $(REGISTRY)/test-spawning:$(IMAGE_TAG)
	@echo "$(GREEN)Running test job...$(NC)"
	@kubectl apply -f - <<EOF
 apiVersion: batch/v1
 kind: Job
 metadata:
   name: test-agent-spawning-$(shell date +%s)
   namespace: $(PROJECT_NAME)
 spec:
   template:
	 spec:
	   ...
	   containers:
	   - name: test-runner
		 image: $(REGISTRY)/test-spawning:$(IMAGE_TAG)
		 env:
		 - name: DB_HOST
		 ...
		 - name: CLIENT_ID
		   value: "demo_client"
 EOF
	@echo "$(GREEN)Test job created. Waiting for completion...$(NC)"
	@kubectl wait --for=condition=complete --timeout=300s job/test-agent-spawning-$(shell date +%s) -n $(PROJECT_NAME) || \
		(kubectl logs job/test-agent-spawning-$(shell date +%s) -n $(PROJECT_NAME) && exit 1)
 ...
 test-spawning-setup: ## Set up database for spawning tests
	...
	@kubectl exec -it postgres-clients-0 -n $(PROJECT_NAME) -- psql -U clients_user -d clients_db <<'EOF'
