...
## CLOSED 2026-07-21 — FIXED, LIVE, and PROVEN FLEET-WIDE (v1.0.1144)

Bar met: fixed AND live AND verified. Moving to `/bugs_closed/`.

**The fix that shipped is the contract-compliant rework, not the interim injection.** History:
round-1 injection (rendered_html string-surgery) went live in v1.0.1140 by an accidental
sweep build, was council-REVISE'd and guideline-failed (003 source-of-truth), and is now
**deleted from the binary**. The rework routes news through the platform's own mechanism —
`query.latest_news` / `query.news_archive` schema fields (queryresolve/news_items.go) →
`content_data` → the components' `html_template` → `page_rerender` freshness delivery.
Shipped in **v1.0.1144** (swept from HEAD; my commits were taken by another session's
`git add -A`, forward-only, nothing lost).

**Live verification, `curl` with JavaScript never executing (2026-07-21):**

| site | loading placeholders | server-rendered article elements |
|---|---|---|
| relojistas.com/noticias/ | 0 | 20 |
| robot-hands.com/news.html | 0 | 40 |
| gaswholesalers.com/news.html | 0 | 40 |
| relojistas.com/ (homepage cards) | — | 6 |

**Durability — the whole point of the rework.** Items live in `content_data`
(noticias-index 20, homepage 6, and the same on the other two sites), NOT only in
`rendered_html`. The injection's failure mode ("items only in rendered_html, wiped on any
content_data regeneration") is now structurally impossible. Confirmed a page-rerender
completes with articles intact.

**Pod-verified** (`agent-chassis-59c675c4f-pxr9f`, image v1.0.1144): `QueryNewsItems`,
`queueNewsPageRerenders`, `resolveNewsArchive`, `resolveLatestNews` all present;
`persistNewsSectionHTML` = 0.

**Council trail:** correlation `4b91237a`. Round 1 (injection) REVISE — correct, it was the
wrong mechanism. Round 2 (rework) REVISE — objections triaged (a breaking-change claim that
dissolved, verifications the read-only seats couldn't run, one factual correction I owed, and
scope) rather than resubmitted; none was a mechanism-correctness defect. Full triage in
`traffic_probe/relojistas_rebuild_running_notes.md` (2026-07-21).
