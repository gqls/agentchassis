...
> ## CLOSED 2026-07-31 (session "bugfix 12") — FIXED, COUNCIL-APPROVED, AND **VERIFIED LIVE ON BOTH REPLICAS**.
>
...
>
> **LIVE, and proven at the artefact rather than at the tag.** Chassis `v1.0.1217`, pods
> `867fc4f77c-9pcww` / `-wd2cg` started `2026-07-31T16:39:59Z` / `16:40:20Z` — **after** the
> fix commit (`16:05:27Z`). Grepped on **both** replicas, with a positive AND a negative
> control in the same exec, because a roll is not evidence your fix shipped (`bugs_open/153`):
>
> | probe | both pods | means |
> |---|---|---|
> | `bodies are read only for files the analyser parsed` — **string this fix ADDED** | **1** | the fix is in the binary |
> | `not in analysis (no FileInfo for that path)` — pre-existing half of the same message | **1** | POSITIVE control: grep + binary are sound |
> | `ReadWholeFileUnbounded_145_shouldnotexist` — never written | **0** | NEGATIVE control: the grep is not matching everything |
>
> A `0 / 1 / 0` would have meant the image predated the fix; `0 / 0 / 0` a broken probe. The
> reproduction command, for any future re-check:
>
> ```bash
...
> ```
> **The roll was NOT made by this session, deliberately, and that turned out fine.** At the
> point the fix was committed another session had `IMAGE_TAG` `v1.0.1215` bumped uncommitted
> (mid-build) and two council review steps from another round were `EXECUTING` — and a roll
> kills an in-flight council. Because `make build-*` builds from committed `HEAD`, the owner's
> own build minutes later carried this fix with no coordination required. **That is the
> shared-HEAD property working as designed**: commit narrowly and promptly, and someone else's
> roll ships you.
>
