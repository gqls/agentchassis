...
> # ✅ CLOSED 2026-08-22 — fixed at source, guarded at both writers, PROVEN at the artefact, damage repaired fleet-wide
>
> **The bar this estate uses is FIXED AND LIVE, and every clause of it is met.** Read §7–§9 for
> the evidence; this banner is the summary a later reader needs before the 1,200 lines below.
>
> | | |
> |---|---|
> | **cause fixed at source** | migration **543** — webdesign-agent persists its render into `css_themes`, so the two writers of `assets/css/styles.css` finally agree. Before this, every per-site repair expired at that site's next design run (~weekly) with no signal. |
> | **guarded at the consumer** | **542** — css-patch refuses a base under 4096 bytes or shared between sites (`fail_on_non_numeric`, so a missing field fails loudly rather than refusing everything silently) |
> | **guarded at the last writer** | **DGH-016** `file_shrink_floor`, opt-in on `git_commit`, enforced in the git-adapter — **LIVE on `v1.0.1323`**, verified at the binary on both halves |
> | **completions stopped lying** | **542/546** — all **7** non-success exits stamp the work item BEFORE their success-labelled terminal |
> | **damage repaired** | **22 of 22** linked theme rows pass; none empty, none shared, none under 13,650 bytes (**547** split the last shared pair on the owner's ruling, **548** seeded the last empty row) |
> | **proven, not asserted** | witnessed live 2026-08-21 19:09–19:11Z: run `76d9bc57` → `complete_refused`, item `needs_human_review` + `parked_by`, `completed_at` NULL — **and the row was still 0 bytes with the repo blob md5 identical and zero commits.** The negatives are the evidence. |
> | **reviewed** | council `5f756c51-cdc6-4a48-b5f9-59e472243601`, APPROVED round 1; 6 advisories — 4 checked, 3 tracked, and the `architecture` seat's correction of the submission's own reasoning acted on |
>
> **WHAT LEFT WITH IT, so nobody reopens this file for them.** Two residuals, neither of which
> is this defect:
> - **`bugs_open/352`** — a contrast finding names a selector that matches nothing, because a
>   class-less element is filed with `Class` = its TAG NAME (`render_audit_action.go:221`/`:329`).
>   The fix is authored, deployed and inert, and the item completes *honestly*. That is a
>   producer defect with its own cause and its own remedies; it was candidate (6) here.
> - **The round-trip-writer inventory**, owed to council round `5249320e` since 2026-08-05 —
>   which other workflows round-trip a whole artefact through an LLM into an unguarded writer.
>   **Not absorbed by this work:** a guard for one seam is not the class survey the architecture
>   seat asked for.
>
> **One observation is opportunistic, not blocking:** DGH-016's enforcement fires only on a
> css-patch deploy, so the git-adapter's `"commit passed the shrink floor"` line is owed on the
> next real dispatch. The refusing arm is proven by its httptest suite and the whole chain by
> the witnessed run above.
>
> ⚠ **Before you verify anything here, read §9:** the in-pod
> `grep -ac '<string>' /proc/1/exe` probe that this file (and much of the estate) recommends
> returns **0 for strings the binary provably contains**. Pull the binary and grep it locally,
> or ask what it was built from. It nearly caused a shipped guard to be reported as unshipped.
>
> **Working record:** `docs/agent_docs/docs024_key_docs_latest/bugfix_198_roundtrip_writers/`
> — cold-start is `HANDOFF_2026-08-22_continue_here.md`.
