...
**Status of the RESIDUAL (§5): ✅ CLOSED 2026-08-25 — Phase 2 is LIVE on `v1.0.1339` and proven at
the artefact (§6n).** The whole chain shipped: the interim `a9002793b` + plural `0f9f7f3ff`
(council `b8df25dc`, APPROVED), Phase 2 `52958897f` + `fa0b513f1` (council `3ed2b792`, APPROVED),
and the pre-roll acronym fix `35f452a0e` (council `64d852b4`, submitted). **The interim's own blind
spot — tracker heroes and CTAs silenced along with their listing — is GONE, demonstrated
behaviourally rather than inferred.** `RFC_053` stays open on one forward question only (whether the
component declaration eventually belongs in a DB column), which is not a live defect.
...
## 6n. ✅ PHASE 2 LIVE AND PROVEN — v1.0.1339, 2026-08-25

Chassis rolled **19:07:18Z / 19:07:49Z**, image `v1.0.1339`. Both replicas probed, with controls:

| probe | result |
|---|---|
| `thirdPartyDataComponents` | 1 |
| `normaliseSurfaceKey` | 2 |
| `adoption-tracker-listing` | 1 |
| `isASCIILetter` (the acronym fix) | 1 |
| **negative control** `isASCIILetterZZZ` | **0** |
| positive control `news-index` | 11 |

**And the behavioural demand control, which is what actually proves it** — a synthetic unsupported
claim injected into three slots of the live `protocol-tracker` copy:

- `hero` → **FLAGGED**
- `call-to-action` → **FLAGGED**
- `protocol-tracker-listing` → **SILENT**

That is the entire design of Phase 2, observed. The site's own first-person voice is policed again;
the third-party listing is not. Both tracker pages score **0** findings on their real copy, and that
zero now means "clean", not "unscanned".

### ⚠ Two instrument findings from this verification, both worth more than the result

1. **`service_binary_capabilities` was WRONG, and I had been treating it as the no-shelf-life
   authority.** Its newest row for `agent-chassis` named `4c996e1b5` — a commit that does **not
   contain Phase 2 at all** — while the running binary plainly did. A second lane dated the same
   build from that table and landed on `a7459a44b`, which predates the acronym fix. **The binary was
   right and the table was stale.** Probe the artefact; treat the table as a hint, not a stamp.
2. **I nearly refuted my own correct probe on a timezone.** The acronym commit reads
   `19:44:10 +0100`; the pods started `19:07:18Z`. I compared them as one clock, concluded the fix
   could not possibly be in the binary, and went hunting for a false positive that did not exist.
   `+0100` is **18:44Z** — 23 minutes *before* the pods. **A commit timestamp is local; a pod
   timestamp is UTC.** `WRONG_CALLS.md` 2026-08-25.
