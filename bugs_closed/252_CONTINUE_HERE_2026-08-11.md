# CONTINUE HERE — the disk-pressure lane (`252`) — ✅ CLOSED 2026-08-15

> **The lane is CLOSED — fixed AND live, effect measured.** Both files now live in
> `/bugs_closed/`. The closing block at the top of the handoff file holds the
> final measurements and the two items deliberately left open (candidate 5 —
> journald, an owner decision; candidates 2/4 — parked). Everything below is the
> working state as it stood before closure, kept for the record.
