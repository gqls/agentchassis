...
## ✅ CLOSED 2026-08-15 — FIXED AND LIVE, EFFECT MEASURED. The fix bar ("fixed AND live") is met on every count.

`[MEASURED 2026-08-15, kubelet configz + stats/summary, all five nodes]` The
owner's release carried the `node-config` DaemonSet (register BLD-021): 5/5 pods
running 16h, and **every kubelet reads `high 70 / low 60 / maxAge 168h` from its
own configz** — the acceptance test this lane defined, passed at the consuming
instrument.

**The effect, against the same instrument's readings the day before:**

| node | images 08-14 → 08-15 | used % | above evict 08-14 → **08-15** |
|---|---|---|---|
| …1148 | 13.9 → 10.7 | 63.9% | 4.26 → **8.75** |
| …1149 | 15.8 → 13.0 | 68.9% | 2.52 → **6.67** |
| …6832 | 15.4 → 11.4 | 67.9% | 2.97 → **7.10** |
| …6833 | 15.6 → 12.5 | 68.0% | 2.42 → **7.03** |
| …1336 | 15.2 → 15.4 | 80.1% | 2.47 → **2.03** |

Four nodes reclaimed to the new 60% low mark and now cycle **6.7–8.8 GB above the
eviction line** — the trough that measured **0.60–0.79 GB** on 08-12 has been
raised by roughly a factor of ten. No rejections, no evictions, no `DiskPressure`,
all nodes Ready throughout (the kubelet restarts were non-disruptive, as
designed). Candidate 1 still proven (census 5), candidate 1b still proven
(3 runners, 3 nodes, max 1/node).

**…1336 is the honest asterisk, and it is a floor, not a failure:** it sits at
80.1% used with GC unable to reclaim further because what remains is in-use
images (it hosts the ollama and runner workloads' larger images) plus the
unreclaimable non-image base — journal ~3.9 GiB, OS, containerd content store.
Its 2.03 GB margin is ~3× the old fleet-worst trough and stable rather than
cyclic, but it is **bounded by composition, not by GC timing**, so the two
remaining levers are exactly the ones still open below.

**Closing statement of the defect and the fix, for the record:** the scheduler
was blind to disk (0/143 containers requested `ephemeral-storage`) AND the image
GC trigger (85% used) was identical to the imagefs eviction line (15% free), so
reclaim could not begin until pods were already being rejected. Fixed by:
candidate 1 (requests on the five big consumers — owner, 08-12), candidate 1b
(spread rule for the runner pool — `85e8818dd`, live 08-14), candidates 3a+3b
(GC thresholds 70/60 + 168h age-based reclaim, via BLD-021 because the
kubelet-config ConfigMap is provider-locked — live 08-15, effect above), all
carried by the makefile release path and the terraform install path
(095/096/097).

**Still open by design, recorded here because this file is now the system of
record for them:**
- **Candidate 5 — journald `SystemMaxUse=512M`** (owner decision on the
  retention trade: ~70 days held today, ~10 days after; returns ~3.4 GiB/node).
  …1336's composition-bound floor is the sharpened case for it. One block in
  BLD-021's script when taken.
- **Candidates 2 and 4** (per-pod limits; bigger disks) — parked, reasons in the
  candidates section.

Everything below this block is the investigation as it unfolded, kept intact —
including the corrected wrong turns, which are the part the next lane cannot
rederive.
