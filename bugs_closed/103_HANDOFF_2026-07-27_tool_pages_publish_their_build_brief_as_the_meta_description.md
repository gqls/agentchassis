# 103 — every deployed tool page publishes its internal build brief as the public meta description (CLOSED — fixed, live, and the 17 live pages repaired and re-served)
...
> ## ✅ CLOSED 2026-07-27 — hold released, backfill applied, every page re-served
>
> **The hold did its job.** The owner held the backfill behind the Gemini thread
> finalising the house voice, and that landed at 19:35 (`voice_style_block` in
> `agent_default_configs`, `platform/voicestyle`, content-creator v1.0.1178). It changed
> the work: the copy I had staged read *"Use our free interactive X in your browser, with
> a companion guide explaining how it works"*, which breaks the voice rules twice —
> "free interactive" is word-weight the claim has not earned, and "explaining how it
> works" is the vague gesture the block asks you to replace with the named action. Writing
> that onto six client sites and then rewriting it would have been the worse outcome.
>
> **Three things done, in order:**
>
> 1. **Code fix live** on chassis v1.0.1177 (rolled 19:22:02Z). Running-pod grep of a
>    string the change created, `"rejected as a build brief"` → 2. Both call sites are
>    gated, including the one this file never named
>    (`create_tool_component_action.go:265`).
> 2. **Backfill applied** (`sql_for_agents/242`, renumbered from 240 after the voice
>    thread took that number): `UPDATE 17` across 6 sites. Census back to **0**, with the
>    negative control still returning **1** — without which the zero is indistinguishable
>    from a broken query, as §5.3 asks.
> 3. **All 17 pages re-rendered** (assemble-only, so stored content is untouched) and
>    **verified on the served HTML, not the row**: 17 of 17 now serve the house-voice
>    description, 0 still serve a brief.
>
> **STEP 1 earned its place.** The read-only pass showed `pages.title` carries the nav
> suffix, so 11 of the 17 would have published *"An interactive AI Agent ROI Estimator
> **| Tools**, free to run in the browser."* The expression now strips from the first pipe.
>
> ### What is NOT proven, stated plainly
>
> §5.1's induced check — deploy a tool to a scratch site and assert its
> `meta_description` differs from the source `content_components.description` — has NOT
> been run. The gate is exercised by unit tests using the **real leaked Arena string** as
> the fixture, and is confirmed in the running binary, but no new tool has been deployed
> through it in production.
>
> ### Left open deliberately, for the voice thread
>
> `composedToolMetaDescription` and `composedGuideMetaDescription` were rewritten to obey
> the house voice (the tool one had an em dash, which the block bans outright). Those
> govern pages created **from now on**. Measured and NOT rewritten: **26 live guide pages
> across 9 sites** still carry the em-dash wording, and **40 live meta descriptions
> contain an em dash** in total. That is an estate-wide copy decision, not a side effect
> of a tool-page fix.

**Status:** **CLOSED 2026-07-27.** Code fix **LIVE on v1.0.1177** (rolled 19:22:02Z; running-pod grep
