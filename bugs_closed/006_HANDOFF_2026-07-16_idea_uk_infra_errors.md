...
# CLOSED 2026-07-26 — all three sections, with two residuals named

Moved to `/bugs_closed/`. The bar there is **fixed AND live**; each section's evidence is below.
Closing on the safe floor with a residual stated is the `032` precedent — see
`/bugs_closed/README.md`, "*A case can close on its safe floor while a better answer stays open; say
so in the file, as `032` does, rather than letting the floor read as the finished shape.*"

**The two residuals, stated up front so nobody has to read to the end for them:**
1. **B:** 9 of 12 live contact forms still serve a non-delivering action until their next organic
   re-render. Mechanism fixed and proven; owner ruled 2026-07-25 that they heal organically.
2. **C:** this fixes the *recovery* layer, not the *cause*. Why a completion write is lost at all is
   `bugs_open/003`'s pod-death / lost-child-response question (F2+F3 live `v1.0.1159`, owner-ratified).

## A — RESOLVED (not by this thread)
...
$ kubectl -n ai-persona-system get deploy github-actions-runner -o jsonpath='{.spec.replicas}' →  2
$ kubectl get nodes                                                     →  5 Ready, 0 cordoned
```
The crash-looping `-lhg9l` (4906 restarts at filing → 6365 by 07-20) is **gone**. Its replacement
`-6f66p` sits on a **different, newer node** with **0 restarts over 5d6h**. Both replicas of the
deployment are `1/1`, and there is no CrashLoopBackOff anywhere in the namespace. The
single-point-of-failure on the fleet deploy path that A was filed about is gone.

> **[INFERRED] — and the distinction matters.** I cannot prove *how* it was resolved. The bad node is
> no longer in the cluster, so "someone fixed that node's containerd `SystemdCgroup`" and "the node
> pool rolled and the misconfigured node was replaced" are now indistinguishable from outside. The
> second is the likelier reading (the replacement pod is on a node whose instance id is newer than
> every other node's), but it is **not verified**, and it is the less reassuring one: if the same
> driver mismatch can be provisioned again, A can recur. **If a runner replica starts crash-looping
> with `expected cgroupsPath to be of format "slice:prefix:name"`, this is not a new bug — reopen
> this case.** The symptom is extinct; the underlying node-config question was never answered.

## B — FIXED, LIVE, PROVEN; residual is the organic tail

No new work this session. Live audit 2026-07-26 of every `data-component="contact-form"` component:

| form action | sites |
|---|---|
| `mailto:…@contactforsales.com?subject=… enquiry` | **fundamentallyai.com, vonc.com, oufe.com** |
| `#contact` | ai-agent-orchestration, dartsonline, gaswholesalers, idea.uk, leopardessconsulting, relojistas, robot-hands, vetcomparison |
| `""` (empty) | finetuning.uk |

**3 of 12 healed, 9 to go** — and `oufe.com` is the interesting one: it is a **new site that was born
correct**, never having carried a broken action at all. That is independent evidence the render seam
works on the creation path and not merely on repair, which nothing before this had shown.

Everything on the code side is done, live (`v1.0.1149` / `v1.0.1156`), council-approved
(`8bfcbc68-cb06-4ee8-a67e-7b8273c5e1e1`) and proven end to end on vonc with zero human touch. The
remaining 9 heal on their next discovery cycle by the owner's 2026-07-25 ruling: no batch `091` runs,
improvement-sweep stays disabled. **The signal worth chasing is unchanged: a site still on `#contact`
*after* a discovery cycle has run there.** Re-audit with the query in the §B verification pass above.

## C — FIXED AND LIVE 2026-07-26 (config: no image roll)

**Re-measured first, not carried forward.** The 07-20 figures in the verification pass above are
stale; the 14-day window on 2026-07-26 reads:
...
84 timeouts against 14 auto-completions. 11 items ended `failed` and 1 `unresolved` — attempts
exhausted on work that had already been done.

### What was actually wrong, and what changed

The sweep was re-deriving, per item type, something the platform already records **once, generically,
for every item type**. Both dispatch loops pass the item id into the handler through
`call_handler.input_mapping`, so the handler's own orchestration row carries it:

```
initial_request_data->'input_data'->>'work_item_id'
```

`220_claimed_item_timeout_generic_evidence.sql` adds one `completed_by_orchestration` branch keyed on
that, ahead of the three artifact branches (retained as the fallback for a claim whose orchestration
row has already been purged — retention is ~2 days). Applied 2026-07-26. **`scheduled_tasks.pre_query`
is DB config, so this is live immediately with no image roll.**

**The standard is parity with the lost write, not something stricter.** The dispatch loop calls
`complete_work_item` on a handler saga that reached its own `complete_workflow`; the sweep now
reproduces that. A recovery that is *stricter* than the write it replaces leaves finished work
re-running, which is the defect. The one place parity would be unsafe is the item types whose Go
verifier can **block** completion (`bugs_open/017`, `/021`) — SQL cannot call one, so those three are
excluded and keep falling through to reset.

### Evidence the link is real, gathered before building

- **Reach 100/100** over a 6-hour window — chosen deliberately shorter than `orchestration_states`
  retention so a purge cannot masquerade as a missing link: `page_rerender` 92/92, `needs_page` 3/3,
  `needs_content_page` 2/2, `nav_drift` 1/1, `missing_*_page` 1/1. The same query over 36 hours gives
  212/298 for `page_rerender`, and that shortfall is **purge, not coverage** (1,125 orchestration rows
  for 07-26 and 694 for 07-25, against ≤4/day before that).
- **Item types that resolve to nothing** (`needs_section_data` 0/7, `cta_names_unknown_destination`,
  `needs_experience_plan`) are completed by paths that **never claim**, so they can never be in the
  `claimed` status this sweep reads.
- **Only two agents claim at all.** `diagnose-dispatch-loop` uses status `diagnosing`, says so
  itself — *"We own this because diagnosing is inert to claimed-item-timeout"* — and runs its own
  75-minute reaper. So the sweep's universe is exactly `build-dispatch-loop`'s items.

### Verified live, both branches, through the production scheduler

Two probes planted at `status='claimed'`, `claimed_at = now()-20min`, with an `item_type` no artifact
branch can match (so a completion could only have come from the new branch) — one with a `COMPLETED`
handler orchestration, one with a `FAILED` one. After one scheduler tick:

| probe | result |
|---|---|
| handler orchestration **COMPLETED** | `status='complete'`, `completed_at` stamped, `error='Auto-completed: handler orchestration completed after claim'` |
| handler orchestration **FAILED** | `status='claimed'`, untouched — **the negative control held** |

Probes and their orchestration rows deleted afterwards (0 remaining, verified). Scheduler log over the
same window: repeated `"Pre-query task completed (no message fired)","task":"claimed-item-timeout"`,
no errors. **This is the running scheduler executing the stored string, not psql executing the file.**

### The guards, and the fact that they were watched to fail

`pre_query` is a SQL string in a text column: nothing validates it at write time, the scheduler runs
it every 120s, and a typo there would silently kill the fleet's only claim self-heal while every
surface still read "enabled, firing normally". So the migration carries three transaction-aborting
guards — the row was updated, the new `pre_query` **parses and plans** (`PREPARE`), and a
positive+negative probe pair swept by **the real stored `pre_query`**, not a restatement of it,
because a check sharing the fix's own expression cannot falsify it.

All four assertions were **fault-injected and watched to fail with the right diagnostic** before being
trusted: branch negated → *"positive probe was not completed … never fired"*; branch widened to accept
`FAILED` → *"negative probe changed status to complete"*; `RETURNING` misspelled → `PREPARE` aborts;
new branch made to write the OLD marker → *"an OLD branch matched, so this migration proves nothing"*.
The Go lockstep test's three failure modes were induced the same way.

### The lockstep, and why it reads the migration

The SQL exclusion list and the `RegisterVerifier()` calls are **one contract in two places**, and the
drift is silent and one-sided: register a fourth verifier, leave the SQL alone, and the sweep
auto-completes an item the verifier would have blocked — with no caller to notice, because a sweep has
none. `TestRegisteredVerifiersMatchClaimTimeoutExclusion` reads the migration file and extracts its
list rather than holding a third copy here, because *a guard whose own copy can drift is not a guard*.
It asserts both directions and fails loudly if the migration is missing or the SQL was reshaped past
its regex.

### A hypothesis that was tested and REFUTED — recorded because it was plausible

I believed for a while that `stale-orchestration-reaper` was the cause: it FAILs a
`build-dispatch-loop` orchestration that has been `AWAITING_RESPONSES` for **>30 minutes**, and killing
the parent while its handler still worked would orphan the claim exactly as described. **Refuted by
measurement:** completed handler orchestrations average **4.9 min** and top out at **8.1 min**
(`page-build-handler` n=17), none over 30; and a dispatch loop's `last_activity` advances on each loop
iteration, so it does not idle out under normal flow. The reaper is not implicated. **Caught by:**
measuring handler runtimes before writing the claim down, rather than after.
...
It does not fix **why** the completion write is lost — pod death, or the lost child response of
`bugs_open/003` (F2+F3 live `v1.0.1159`, owner-ratified). §C asked for a recovery layer that works for
every item type, and that is the scope delivered. The original wording also offered "mark the item
complete atomically with the last deploy step" as an alternative; that was **not** taken, because it
would mean adding a `complete_work_item` step to ~18 handler agent definitions and duplicating the
dispatch loop's own step in each — more surface, more drift, and it would still lose the write when
the pod dies between the artifact and the stamp. The generic evidence branch removes the need for it.
