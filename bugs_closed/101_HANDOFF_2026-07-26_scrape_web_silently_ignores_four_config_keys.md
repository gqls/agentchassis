...
> ## CLOSED 2026-07-28 — fixed AND live on BOTH images, pod-grep verified
>
> **The silence is gone, which is what this bug was about.** Live on
> `agent-chassis:v1.0.1192` and `web-scrape-adapter:v1.0.1192`, verified against the
> RUNNING pods — never the tag (`IMAGE_TAG` had been sitting at the already-deployed
> v1.0.1191, so a build without bumping it would have shipped the stale cached
> binary):
>
> ```
> chassis  agent-chassis-f757fcf65-bg9t7
>   unrecognised_keys              1   (was 0 pre-deploy)
>   "does not read"                1   (was 0)
>   add_protocol                   3   (was 0)
>   "no fetch provenance available" 1  (was 0)
>   POSITIVE CONTROL scrape_web    1   (was 1 — proves the grep works)
>
> adapter  web-scrape-adapter-c576d96b-bzzz4
>   buildScrapePayload             2   <- a symbol this fix CREATED; 0 before
>   POSITIVE CONTROL onlyMainContent 1
> ```
>
> All five keys are now read: `fallback_url_field` and `extract_mode` implemented,
> `add_protocol` implemented, `max_pages`/`follow_links` mapped onto the adapter's
> crawl dialect and **warned** when set on a single-page scrape. `only_main_content:
> false` is expressible for the first time. `bugs_closed/062` payload watch run
