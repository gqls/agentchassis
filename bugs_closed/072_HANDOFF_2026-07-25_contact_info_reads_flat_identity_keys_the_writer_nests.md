...
> ## ✅ CLOSED 2026-07-31 21:18 UTC — FIXED, APPROVED, LIVE, AND PROVEN END-TO-END
>
> **Do not re-diagnose this. Do not start a competing fix.**
>
> **The acceptance test PASSED on the real path**, orchestration
> `a346210a-a245-43b7-8a9f-ca8c22680072` (chassis **v1.0.1223**), on
> `vetcomparison.uk/contact` — the one page fleet-wide that was actually in the
> broken state.
>
> **1. The mechanism fired, and said so itself.** `plan_sections` output:
>
> ```
...
> ```
>
> `contact-info` had been withheld on every build since at least 2026-07-17; it is
> now READY, and the provenance record names exactly why — the email resolved from
> the canonical `sites` row, which no component could previously read.
>
> **2. The artefact is right.** 3 `page_components`; `contact-info` carries
> `vetcomparison@contactforsales.com`. Page `deployed` at 21:18:15, `/contact.html`.
>
> **3. The NEGATIVE CONTROL passed — this is the load-bearing half.** `contact-info`'s
> keys are exactly `email`, `intro_text`, `section_title`. **No `phone`, no
> `address`, no `hours`** — those exist in no store for this site, and the fallback
> did **not** fabricate them. That is the failure mode `ensureSiteRow`'s deliberate
> no-COALESCE decision exists to prevent, and it is now demonstrated, not asserted.
> `source_aliases_used` likewise lists only `email`.
>
> **4. The two-week-old HITL item auto-closed.**
> `section_data_contact_contact-info_72b9e3a6…` sat at `needs_human_review` from
> **2026-07-17** until `closeResolvedDataRequest` marked it `complete` at 21:13:09 —
> the reconciliation path working as designed.
>
> **Root cause was NOT the flat/nested path mismatch.** Both remedies in the "Fix
> candidates" section below fix **0 of the 8 sites** — the nested values are empty on
> exactly those 8. `sourceResolver` could not read the canonical `sites` row. **Read
> the RESOLVED section at the bottom of this file before anything in the middle.**
>
> | | |
> |---|---|
> | fix | `ef9e7e999` + `489ae1e7f` — `plan_sections_action.go`: `resolveSpecAlias`, `ensureSiteRow` |
> | registered | **PBP-026** |
> | diagnosis loop | **CONFIRMED**, first iteration, corr `0f76987c` |
> | council gate | **APPROVED round 1**, corr `dd03a73b` (4 advisory, none high) |
> | proven live | orch `a346210a`, chassis v1.0.1223, 2026-07-31 21:13–21:18 UTC |
> | workstream | `docs024_key_docs_latest/bugfix_072_identity_source_resolver/` |
>
> **Scope, corrected and final: this repaired ONE PAGE, not five sites.** Only 8
> pages fleet-wide name `contact-info` in `pages.sections` and 7 already rendered
> 3-of-3. My earlier "buys 5 sites" measured the *store* (who has an email) and never
> intersected it with the *demand* (who asks for the component) — logged in
> `WRONG_CALLS.md`. The **mechanism** is fleet-wide and makes a *new* site work by
> default; the **repairs** were one.
>
> **Still open elsewhere, deliberately not done here:** `sync_site_identity` is wired
> into **zero** live agents (0 rows vs 9 for `plan_sections` as control) — it exists
> to copy `identity.contact.*` into the `sites` columns and never ran, which is why
> the nested half of the fallback is load-bearing. And adding `contact-info` to the
> four other sites' page plans is an editorial change, not a resolver one.
