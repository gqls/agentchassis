...
# CLOSED 2026-07-31 — a design was saved end to end, in production

**Fixed AND live**, on chassis `v1.0.1216`/`v1.0.1218` (widening pod-grepped on both
replicas, with a control the diff did not touch), config live via migration `272`.

## The discharging run

`FEATURE_CORR 53fff682-9806-4bbc-bd91-500b86a453e5`, orch `e79ee191`, with
`max_edits=1` armed to induce a refusal by a rule **candidate 1 never taught** — the
point being that the fix is rule-agnostic within the structural class.

| time | what | evidence |
|---|---|---|
| 18:08:26 | plan refused, design preserved | `iteration_note` / `note_kind=plan_validation_refusal`, problems `["stage s1: 2 edits exceeds the per-stage cap 1"]`, `body` = the rejected plan verbatim |
| 18:08:26 | operator record | `agent_error_log` `FIX_PLAN_VALIDATION_REFUSED`, severity `warning`, *"1 structural problem(s), attempt 1 of 1 — routed to repair"*, `orchestration_id` populated |
| — | routed | trail: `persist_plan → check_plan_valid → repair_plan` |
| 18:09:31 | **repaired plan PERSISTED** | `fix_plan` artifact, **3 stages / 3 edits** |
| after | run continued | `review_editquality → review_bug_historian …` — **not** `complete_refused` |

**All four conditions of the corrected verification held.** The original plan was 2
stages with `s1` carrying 2 edits; the repaired plan is **3 stages carrying all 3
edits** — the designer split the offending stage and **lost no scope**, which is
exactly what the repair prompt asks for. 65 seconds from refusal to persisted plan.

Before this fix that design was deleted, with `orchestration_states.error` NULL.

## What shipped

- `diagnose_persist_fix_plan` gains `repair_step` + `max_repair_attempts` (default 1).
  **With `repair_step` unset the action is byte-for-byte the old behaviour**, which is
  why `council-gate` and `fix-proposer` needed no change and got none.
- The rejected plan + exact problems persist as `iteration_note` /
  `note_kind=plan_validation_refusal` (no DDL — `kind` is CHECK-constrained).
- An operator record on **both** outcomes, so the row's absence means no refusal.
- `countRunArtifacts` — one shared bounded-retry counter, now used by all three loops.
- Migration `272` wires `persist_plan → check_plan_valid → (review_editquality |
  repair_plan → persist_plan)`.
- **The gate is NOT lowered**: nothing invalid is persisted as a `fix_plan` on either
  path, and truncated JSON stays terminal.

## Left open deliberately (not residue — decisions)

- **`fix-proposer` is still exposed** → **`bugs_open/162`**, filed at the council's
  request, with `sql_for_agents/273_…sql` written and dry-run clean but unapplied
  because that agent belongs to another lane.
- **`council-gate` is not opted in**, on purpose — it is the gate that reviews this
  mechanism, and opting it in would also need `summary` in the refusal result.
- **Size caps stay unrepairable** (`max_stages`, `max_total_edits` ask for less scope
  while the repair prompt forbids dropping it) — `FIX-057`'s open question.

## Council

Corr `f4a4628f-3b90-4054-a875-f2cf72b83e72`, three rounds, advisory. Round 1 REVISE
(13 seats) produced four real improvements: a **wrong rollback block** that would have
failed mid-restore, a **duplicated counter**, **recoverable-but-not-visible**, and the
sibling-consumer gap. Round 3 REVISE (14 seats) was answered by measurement — every
objection is recorded with its evidence in the workstream `NOTES`.

## The most useful thing that happened

The **first** live repair FAILED, on a class I had ruled out of scope: the repaired
plan came back well-formed but wrong-shaped (`"file": [...]`), and I had lumped that
with truncation. They are different — truncation is a half-finished answer that must
not be retried; a schema mismatch is a complete answer in the wrong format, the most
mechanically fixable error there is. **No unit test could have caught it, because the
boundary was the assumption the tests encoded.** It took one real run. Widened, with
the two cases tested as a pair going opposite ways.

Full account: `docs024_key_docs_latest/bugfix_099_plan_refusal_recoverable/`.
