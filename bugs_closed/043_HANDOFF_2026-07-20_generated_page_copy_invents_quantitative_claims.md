...
> **STATUS: CLOSED 2026-07-26 — all four fix candidates live, and the whole chain proven
> end-to-end on production traffic at 19:26Z.** Candidate 3 shipped 07-24 (migration 201);
> candidates 1, 2 and 4 shipped 07-26 (migrations **217** and **218**, and Go in
> **v1.0.1170**). The proof is the artefact, not a status — see
> **§ Final verification** at the foot of this file: a full-writer rebuild of the page that
> had been unbuildable since the 24th completed, saved and deployed, publishing **two
> query-traced figures and three honestly-blank stats**, with zero empty `<strong></strong>`
> and none of the invented metrics it had been serving.
>
> **Carried forward as follow-on work, NOT as this defect** (the mechanism is dead; these
> are coverage):
> - Evidence registers for the remaining publishing sites. Owner asked for all of them
>   (2026-07-26). Done: robot-hands, gamesdesign, ai-agent-orchestration (mig 218), plus
>   the pre-existing leopardess / relojistas / fundamentallyai. Outstanding: webdesign.co.uk
>   (98 live pages), finetuning.uk (41), gaswholesalers.com (28), idea.uk (20),
>   vetcomparison.uk (5), dartsonline.com (4), and `facts[]` for vonc and oufe. The 17
>   `pool-*.internal` rows have zero deployed pages and need none. **Three want their owning
>   thread consulted first**: oufe already runs a stricter no-figures rail, vetcomparison
>   carries a legal record from published fabricated prices, idea.uk is owned elsewhere.
> - **Spec-aspect numeric drift** — a number in a `briefing`/`identity`/`site_plan` spec is
>   an *instruction* and nothing refreshes it. leopardess `identity` says "143 agent
>   definitions, of which 56 are active"; live is **175 active**. The writer is told to
>   follow it. This is wave-2d's parting lesson, still unfixed and now measured fleet-wide.
> - **Prose numbers outside stat fields** were never audited fleet-wide (robot-hands' 9
>   stat fields vs 42 prose fields carrying the same claim is the sizing).
...
>   and a check that can never fire reads as coverage. Needs its own function over raw
>   `content_data`.
> - Council review `569241fb-dd8d-4bcf-b382-234dfca1365c` was **pending at close** (the
>   first submission was lost in a postgres restart and was resubmitted). No
>   `Council-Reviewed:` trailer is claimed on any commit in this lane.
...
# Final verification — 2026-07-26 19:26Z, artefact-checked

The whole chain, exercised end-to-end on the page this bug and `073` were both blocked on.
**Graded on the rendered artefact and the saved row, never on `status='complete'`** — which
is the house rule this lane has twice been caught by.

**Run:** work item `54734027-a910-4d86-9cc1-336f0619fe47`, correlation
`7bb79681-bfef-4be2-bed0-97a6cd8ea74a`, full `page-build-handler` → `page-content-writer`
path over all eight sections of `ai-agent-orchestration.com/index`.

**1. It passed the section that used to kill it.** `page-content-writer` COMPLETED,
having produced `generated_content_0` … `generated_content_7`. Iteration 4 is
`case-studies-grid` — the step that failed deterministically on 07-24 16:28 and 07-25
09:06 and burned the item's three attempts. Two independent runs passed it this evening
(`81efa9cb…` at 19:06 and this one at 19:26).

**2. The gate accepted it and the page saved and deployed.** Work item `complete`, no
error. The 19:06 run had been stopped at `validate_content` by three `1,834` findings —
correctly, see the drift section above — and the same pipeline passed once the drift was
fixed. **The gate discriminates; it does not merely pass.**

**3. What was saved** (`page_components.content_data`, `case-studies-grid`):

```
card1_stat_value = [175]     card2_stat_value = []
card3_stat_value = []        card4_stat_value = [17]
card5_stat_value = []
```

Two figures, both registered facts from migration 218 with a `source.sql` behind them;
three cards with no metric, left blank rather than invented. Note the writer used *fewer*
figures than the 19:06 run, which also wrote `1,834` and `14` — because the volatile ones
were removed from the prose block between the runs. It reaches only for what it is given.

**4. The live page** (`curl https://ai-agent-orchestration.com/`):

```
<span class="csg-card-stat"><strong>175</strong> Agent definitions in registry</span>
<span class="csg-card-stat"><strong>17</strong> Backend services in production</span>

empty <strong></strong> ......... 0     <- 073 candidate 1's failure mode if the
                                           template half had been forgotten
"4 days" / "<10 min" ............ 0     <- the invented metrics it had been serving
"100%" .......................... 8, all CSS (linear-gradient, top:, width:)
```

Three cards render with no stat span at all rather than an empty slot — the `{{if}}` gates
doing their job. The last line is written out because a bare `grep -c '100%'` returns 8 and
reads like a residual; it is `linear-gradient(… 100%)`, `top: 100%`, `width: 100%`.

**Before, for comparison:** that same component was serving `4 days` (time from brief to
production), `<10 min` (time to diagnose first incident), `8+` (agents in parallel),
`1,267` (work items) and `100%` (of steps attributable) — five invented per-case-study
outcome metrics, four of which had no source of any kind.

## What this run proves, precisely

- **Candidate 1** — a required numeric field no longer forces invention, and an absent stat
  renders as nothing rather than an empty slot or a dead build.
- **Candidate 3** — the writer declines when it has nothing true, and reaches for the
  register when it does.
- **Candidates 2 + 4** — live in `v1.0.1170`, and demonstrated *discriminating* on real
  traffic within the hour: they stopped the 19:06 page over an unsupported figure and
  passed the 19:26 page.
- **Migration 218** — the registers are consumed by both the writer and the gate; before
  them, both claims checkers were silent no-ops on this site.

## What it does not prove

The fleet sweep. This is one page on one site. The residuals listed in the status block at
