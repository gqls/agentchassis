...
# ✅ CLOSED 2026-08-26 — FIXED, LIVE ON v1.0.1345, AND PROVEN IN PRODUCTION ON THE DISCONFIRMING PAIR

The bar this file set in §7 is met, both arms, at the queue and at the wire. Moving to
`bugs_closed/`.

## The proof, and it is a PAIR because one arm alone proves nothing

Run `9ca6a795-6151-4496-8c20-26823192e579`, `availability-discovery-agent` on
`robot-hands.com`, 2026-08-26 22:32:32→22:32:51Z. Every archived-and-shipped page on that site,
and what the detector did with it:
...
- **Arm 1** — the two pages this file's own census recorded as archived-and-serving are the two
  that were flagged. `gripper-catalog` is §2's load-bearing datum: **first recorded serving on
  2026-08-14, still serving, and now — twelve days on — something has finally said so.**
- **Arm 2** — the four archived pages that correctly 404 were **not** flagged. §7 is explicit that
  *"a detector that flags every archived page satisfies the first arm and is useless"*, and this is
  the arm that rules that out.

The items are exactly the designed shape: `severity medium`, `priority 50`, `status detected`,
**`handler_agent` EMPTY** (flag-only, per §6.4 — a wrongly-archived page serving correctly is
indistinguishable on the wire from the defect, so nothing auto-acts), `item_key
archived_page_still_serving:<page_id>`.

**`checks_failed: []`** — so both per-domain instrument controls held on that run. That matters as
much as the findings: this check REFUSES rather than reporting zero when it cannot prove its own
instrument, and a clean run is only readable because the refusal path exists and did not fire.

## Enablement, and the probe trap it walked past

Migration `648` applied by hand 2026-08-26 after the `v1.0.1345` roll, once the capability was
probed on both live chassis pods with a positive AND a negative control
(`archived_page_still_serving` present, `site_unreachable` present, `…_NOTREAL` 0 rows). The
checks array reads 3. `DISCOVERY_CHECK_ERROR` rows in the 40 minutes after: **0**.

⚠ **The first capability reading said 95 of 428 pods and would have stopped the apply.** It was
wrong: `agent-chassis` spawns short-lived pods, so a 45-minute window mixes pre- and post-roll
ones. There are two long-lived deployment pods, both had it, and a 10-minute window gave **153 of
153, zero without**. Recorded in the migration's discharge banner: **ask the WINDOW, not just the
count**, on any service that spawns.

## How the pair was made to happen, and what was left behind

The rotation takes one site per 300s with a 4-hour per-site floor, so it had not yet reached an
affected site and the detector had filed nothing. **A zero at that point is not a pass** — this
file says so — so `robot-hands.com`'s `site_discovery_rotation.last_selected_at` was set back to
bring it forward one tick. It was picked at 22:29:44 and the run followed.

**Nothing needs restoring.** The site was then legitimately swept, so its new stamp is the correct
one; putting the old value back would make it immediately eligible again, which is the opposite of
tidy.

## What is genuinely still open, and it is not this bug

The other four sites' serving pages (`ai-agent-orchestration.com`, `finetuning.uk`,
`fundamentallyai.com` ×2, `leopardessconsulting.co.uk`) will be flagged as the rotation reaches
them — within ~4–8h by the cadence, and **`scripts/audit-archived-still-serving.sh` is the
expected set on the day**. ⚠ Re-run it rather than quoting this file: the population MOVES (it went
3→7 between filing and the census, and two of the filed three had 404'd by then).

And the items are flag-only by design, so **draining them is a human act**. Each carries a
`triage_hint` naming both remedies — `retract_page_deployment`, or **un-archiving** if the page was
retired by mistake — and the `agent_error_log` `RETRACTION_AUDIT`/`RETRACTION_REFUSED` query to
read first. `bugs_open/083`'s cost of a flag-only detector applies and is accepted, not forgotten.
