...
// lives HERE, at the shared seam, and not as a private helper in the layer
...
// nil guard) and platform/errors' IsRecoverable (twelve case-folded
// patterns, ZERO callers — dead, and disagreeing with its live twin on
// case). Both are deleted in the commit that adds this.
//
...
// re-spends it).
var RetryableTransientCodes = []errors.ErrorCode{
	errors.ErrAgentTimeout,
	errors.ErrAgentOverloaded,
...
// errors.InternalError() defaults Retryable=false while routinely WRAPPING
...
// (mirrors NonRetryablePermanentCodes' argument in validation_drop.go).
