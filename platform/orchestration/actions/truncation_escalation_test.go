...
func TestTruncationEscalationApplies(t *testing.T) {
	...
	cases := []struct {
		...
		{"truncated with taller ceiling escalates", truncated(16000), cfgWith(float64(32000)), 16000, 32000, true},

		// No key -> byte-for-byte the old behaviour (the opt-in contract).
		{"truncated with no ceiling stays off", truncated(16000),
			map[string]interface{}{"max_tokens": float64(16000)}, 16000, 0, false},
		...
		{"ceiling equal to sent cap refuses", truncated(16000), cfgWith(float64(16000)), 16000, 0, false},
		{"ceiling below sent cap refuses", truncated(16000), cfgWith(float64(8000)), 16000, 0, false},

		// Only a typed truncation escalates — never a 5xx, never nil. A 529
		// retried at a taller cap would just be a more expensive 529.
		{"non-truncation error never escalates", errors.New("529 overloaded"), cfgWith(float64(32000)), 16000, 0, false},
		{"wrapped non-truncation error never escalates", fmt.Errorf("call: %w", errors.New("overloaded")), cfgWith(float64(32000)), 16000, 0, false},
		{"nil error never escalates", nil, cfgWith(float64(32000)), 16000, 0, false},

		// A wrapped TruncatedError still counts — errors.As, not equality.
		{"wrapped truncation escalates", fmt.Errorf("AI call failed: %w", truncated(16000)), cfgWith(float64(32000)), 16000, 32000, true},

		// An unconfigured step runs at the provider fallback (2048); a declared
		// ceiling still rescues it.
		{"escalates over the provider fallback", truncated(2048), cfgWith(float64(32000)), 2048, 32000, true},

		// Zero / negative / junk ceilings are unset, not tiny.
		{"zero ceiling is unset", truncated(16000), cfgWith(float64(0)), 16000, 0, false},
		{"negative ceiling is unset", truncated(16000), cfgWith(float64(-1)), 16000, 0, false},
		{"string ceiling is unset", truncated(16000), cfgWith("32000"), 16000, 0, false},
		{"nil config never escalates", truncated(16000), nil, 16000, 0, false},
	}

	for _, tc := range cases {
		t.Run(tc.name, func(t *testing.T) {
			ceiling, ok := truncationEscalationApplies(tc.err, tc.cfg, tc.sent)
