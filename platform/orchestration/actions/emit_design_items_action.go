...
// ----------------------------------------------------------------------------
// WORKFLOW WIRING — build-site-planner default_config.workflow.steps
//
// Add one step and repoint reconcile_site_plan at it. (Order vs reconcile is
// cosmetic; priorities drive dispatch. Placing it last keeps emission grouped.)
//
//   "reconcile_site_plan": { ... "next_step": "emit_design" ... }   // was "complete"
//
//   "emit_design": {
//       "action": "emit_design_items",
//       "config": { "site_id": "input_data.site_id" },
//       "next_step": "complete",
//       "description": "Queue needs_composition + needs_design (plan-time, guarded on style_collection_id IS NULL)",
//       "output_field": "design_items"
//   }
//
// site_id: input_data.site_id is set by the dispatch loop's input_mapping
// (current_item.site_id). site_record.site_id (from ensure_site_record) is an
// equivalent source if preferred.
// ----------------------------------------------------------------------------
