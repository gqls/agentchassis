...
var pageLifecyclePostures = map[string]posture{
	...
	"check_heading_promise.go":            {Posture: PostureObserves, Reason: "heading_promise_unmet, handler_agent \"\" — flag only (RFC_056 promise seat). Reads the served-URL list at status = 'active'; a heading a visitor can read is worth reading whatever the lifecycle says."},
