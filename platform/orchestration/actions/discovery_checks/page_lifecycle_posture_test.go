...
var pageLifecyclePostures = map[string]posture{
	...
	"check_cta_rank_anomaly.go":            {Posture: PostureObserves, Reason: "bugs_open/436: cta_rank_anomaly files at handler_agent \"\", flag-only — whether a fossil-ranked primary button is deliberate needs a human who knows the site's premise, so nothing is routed and no page is mutated. Its `pages` query is ONLY the migration-750 acknowledgement lookup (cta_rank_deliberate_nav_order = COALESCE(nav_order,100)), keyed on a page the RANKING has already chosen — and the ranking's own supply (datahelpers.CTAPositionalInteractiveSQL) carries the lifecycle arm already, via status IN ('active','deployed') plus PageMayBeLinkedPredicateFor. Arming this second query would add nothing reachable: an archived page cannot be rank-1, so the lookup can only ever run for a page that already passed the arm."},
