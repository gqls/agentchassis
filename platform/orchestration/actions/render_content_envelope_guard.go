...
func writeRenderEnvelopeLog(
	...
	if err != nil {
		params.Logger.Warn("render content envelope: failed to marshal context", zap.Error(err))
		return
	}
	...
	if _, err := params.DB.ExecContext(ctx, `
		...
		siteIDArg,
		domain,
		componentRepairAgentType(params), componentRepairStepName(params, "render_component"),
		"render_component",
		message, contentDataEnvelopeErrorCode, severity, string(contextJSON),
	); err != nil {
		params.Logger.Warn("render content envelope: failed to write record", zap.Error(err))
	}
}
