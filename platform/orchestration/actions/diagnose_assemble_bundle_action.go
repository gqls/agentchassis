...
var DiagnoseAssembleBundleInputSpec = datahelpers.ActionInputSpec{
	// Nothing is hard-required: the action resolves scope from EITHER the loop's
	// scope field OR lookup_code_symbols' code_results, and validates at runtime
	// with a clear error if neither is present (a required[] here would reject the
	// code_results path). repo_root is needed but is read from the analyser Output
	// already in collected_data, not supplied by the caller.
	Optional: []string{
		"scope_field", "code_results_field", "analysis_field",
		"repo_root_field", "runtime_field", "max_body_chars",
	},
	Defaults: map[string]interface{}{
		...
		"runtime_field":      "runtime_evidence",
...
func DiagnoseAssembleBundleAction(ctx context.Context, params ActionParams) (interface{}, error) {
	...
	scopeField := datahelpers.GetStringField(config, "scope_field", "scope")
	...
	runtimeField := datahelpers.GetStringField(config, "runtime_field", "runtime_evidence")
	maxBodyChars := datahelpers.GetIntField(config, "max_body_chars", 60000)

	// The current in-scope symbols (path or path:Symbol) the loop wants in full.
	scope := datahelpers.ExtractStringSlice(params.CollectedData, scopeField)
	if len(scope) == 0 {
		// fall back to lookup_code_symbols' code_results (path+symbol per hit)
	...
	if len(scope) == 0 {
		return nil, fmt.Errorf("diagnose_assemble_bundle: no scope (set %q or %q in collected_data)",
			scopeField, "code_lookup.code_results")
	}
	...
	// Compose: constitution/task are added by the workflow's verdict step; here we
	// build the CODE section (bodies from the repo at the analysed commit) + append
	// the already-gathered runtime/DB evidence by reference.
