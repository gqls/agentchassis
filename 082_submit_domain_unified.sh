...
echo "  WHERE s.domain='${DOMAIN}' ORDER BY created_at;"
echo ""
...
echo "  SELECT item_type, status, handler_agent, LEFT(summary,60) FROM site_work_items wi JOIN sites s ON s.id=wi.site_id"
