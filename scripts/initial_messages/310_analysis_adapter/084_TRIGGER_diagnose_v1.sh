...
# (3b, once the orchestrator forwards them) subject for persistence:
#   INPUT_DATA="${INPUT_DATA},\"subject_type\":\"tool\",\"subject_key\":\"<function>\""
...
echo "  kubectl -n ai-persona-system get pods -l agent_type=diagnose-agent --sort-by=.metadata.creationTimestamp | tail -3"
...
echo "     kubectl -n ai-persona-system logs -l agent_type=diagnose-agent --tail=2000 | grep 'persist_diagnosis_note'"
