...
#
# Expected outcome:
#   - 22 work items of type page_rerender created with status='triaged'
#     (one per page where build_status='deployed').
#   - Build-dispatch-loop claims them over the next several minutes.
#   - 21 produce git commits ("Rerender: <page>.html").
#   - 1 (wholesale-pricing-explained) goes via complete_skipped, no commit.
#   - chassis logs for the 21 should show how many sections each page kept
#     vs skipped (new logging from getPageSections empty-section filter).
#
# Notes:
#   refresh_site_components=false  — header/footer/head are 10 days old but
#                                    not stale enough to need a re-render.
#                                    We only want pages re-assembled from
#                                    their existing components.
#
#   This will produce ~21 commits to gqls/sites, each firing the b2 sync
...
{"action":"orchestrate","config":{"agent_type":"rerender-pages"},"input_data":{"site_id":"5fe15466-4e2e-4ff2-981e-98c1b7074002","domain":"gaswholesalers.com","refresh_site_components":false}}
JSON
