...
====
    DecisionRationaleAgents own their domainBuilders stay simple, agents handle complexityDedicated load actionsEach agent loads exactly what it needsReuse before creatingPatch existing code (4 lines vs new action)Workflows simple, complexity in GoJSON declarative, logic in actionsNo container configspawn_actions.go handles dynamicallyStandardized schemassite_context works from DB, scrape, or manualStandalone + integratedSame agent works both waysSpawn before callFramework pattern for builder workflows
