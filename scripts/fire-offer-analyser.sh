...
kubectl -n kafka run -i --rm "kcat-offer-$(date +%s)" --image=edenhill/kcat:1.7.1 --restart=Never -- \
  ...
  -H message_type=request -H client_id=cli-offer-analyser -H action=process \
