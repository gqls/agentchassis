...
kubectl -n kafka run -i --rm "kcat-linker-$(date +%s)" --image=edenhill/kcat:1.7.1 --restart=Never -- \
  ...
  -H message_type=request -H client_id=cli-linker-canary -H action=process \
  -H sender_agent_type=cli -H sender_agent_id=cli-user \
  -H responses_topic=system.agent.generic.responses -H timestamp=$TS \
  < "$MSG_F" >/dev/null 2>&1

echo "dispatched (exit 0 proves nothing — check the durable record):"
echo "  CORR=$CORR"
